...
const PHASE = 'HGSSP_ALIAS_COLLAPSE_V1';
...
const SOURCE_SET_CODE_IDENTITY = 'hgssp';

const EXPECTED = {
  unresolvedCount: 25,
  canonicalTargetCount: 25,
  mapCount: 25,
};
...
const BACKUP_SCHEMA_PATH = path.join(
  process.cwd(),
  'backups',
  'hgssp_alias_collapse_preapply_schema.sql',
);
const BACKUP_DATA_PATH = path.join(
  process.cwd(),
  'backups',
  'hgssp_alias_collapse_preapply_data.sql',
);
...
async function buildTempCollapseSurface(client) {
  await client.query(`
    drop table if exists tmp_hgssp_unresolved;
    drop table if exists tmp_hgssp_canonical_candidates;
    drop table if exists tmp_hgssp_match_audit;
    drop table if exists tmp_hgssp_collapse_map;

    create temp table tmp_hgssp_unresolved on commit drop as
    select
      cp.id as old_id,
      cp.name as old_name,
    ...
    create index tmp_hgssp_unresolved_token_idx
      on tmp_hgssp_unresolved (old_token);

    create temp table tmp_hgssp_canonical_candidates on commit drop as
    select
      cp.id as new_id,
      cp.name as new_name,
    ...
    create index tmp_hgssp_canonical_candidates_token_idx
      on tmp_hgssp_canonical_candidates (new_token);

    create index tmp_hgssp_canonical_candidates_set_token_idx
      on tmp_hgssp_canonical_candidates (new_set_code, new_token);

    create temp table tmp_hgssp_match_audit (
      ...
      same_token_candidate_count int not null,
    ...
    create temp table tmp_hgssp_collapse_map (
      seq int not null,
    ...
    create unique index tmp_hgssp_collapse_map_seq_uidx
      on tmp_hgssp_collapse_map (seq);

    create unique index tmp_hgssp_collapse_map_old_uidx
      on tmp_hgssp_collapse_map (old_id);

    create unique index tmp_hgssp_collapse_map_new_uidx
      on tmp_hgssp_collapse_map (new_id);
  `);
}
  ...
  const unresolvedRows = await queryRows(
    ...
    `
      select
        ...
        old_token,
        old_normalized_name
      from tmp_hgssp_unresolved
      order by old_token, old_id
    `,
  );
  ...
  const canonicalRows = await queryRows(
    ...
    `
      select
        ...
        new_gv_id,
        new_normalized_name
      from tmp_hgssp_canonical_candidates
      order by new_token, new_id
    `,
  );
...
function buildMatchArtifacts(unresolvedRows, canonicalRows) {
  ...
  for (const row of unresolvedRows) {
    for (const candidate of canonicalRows) {
      if (candidate.new_token !== row.old_token) {
        continue;
      }
      if (candidate.new_normalized_name !== row.old_normalized_name) {
        continue;
      }

      const key = normalizeTextOrNull(candidate.new_set_code) ?? '__NULL__';
      const existing = coverageBySet.get(key) ?? {
        target_set_code: candidate.new_set_code,
        match_rows: 0,
        distinct_old_ids: new Set(),
        distinct_new_ids: new Set(),
      };

      existing.match_rows += 1;
      existing.distinct_old_ids.add(row.old_id);
      existing.distinct_new_ids.add(candidate.new_id);
      coverageBySet.set(key, existing);
    }
  }

  const candidateTargets = [...coverageBySet.values()]
    .map((row) => ({
      target_set_code: row.target_set_code,
      match_rows: row.match_rows,
      distinct_old_count: row.distinct_old_ids.size,
      distinct_new_count: row.distinct_new_ids.size,
    }))
    .sort((left, right) => {
      const oldDelta = right.distinct_old_count - left.distinct_old_count;
      if (oldDelta !== 0) {
        return oldDelta;
      }
      const matchDelta = right.match_rows - left.match_rows;
      if (matchDelta !== 0) {
        return matchDelta;
      }
      return String(left.target_set_code ?? '').localeCompare(String(right.target_set_code ?? ''));
    });

  const winningTargets = candidateTargets.filter(
    (row) => row.distinct_old_count === unresolvedRows.length,
  );
  const targetSetCode = winningTargets.length === 1 ? winningTargets[0].target_set_code : null;

  if (!targetSetCode) {
    return {
      targetDiscovery: {
        candidate_targets: candidateTargets,
        winning_target_count: winningTargets.length,
        winning_target_code: null,
      },
      targetSetCode: null,
      matchAuditRows: [],
      collapseMapRows: [],
  ...
  for (const row of canonicalRows.filter((candidate) => candidate.new_set_code === targetSetCode)) {
    const bucket = canonicalByToken.get(row.new_token) ?? [];
    bucket.push(row);
    canonicalByToken.set(row.new_token, bucket);
  }
  ...
  for (const row of unresolvedRows) {
    const sameTokenCandidates = canonicalByToken.get(row.old_token) ?? [];
    const matchedCandidates = sameTokenCandidates.filter(
      (candidate) => candidate.new_normalized_name === row.old_normalized_name,
    );
    const matched = matchedCandidates[0] ?? null;
    ...
    matchAuditRows.push({
      ...
      old_normalized_name: row.old_normalized_name,
      same_token_candidate_count: sameTokenCandidates.length,
      candidate_count: matchedCandidates.length,
      same_token_different_name_count: sameTokenCandidates.filter(
        (candidate) => candidate.new_normalized_name !== row.old_normalized_name,
      ).length,
    ...
    if (matchedCandidates.length === 1) {
      collapseMapRows.push({
        ...
        old_token: row.old_token,
        new_token: matched.new_token,
        old_normalized_name: row.old_normalized_name,
        new_normalized_name: matched.new_normalized_name,
  ...
  collapseMapRows.sort((left, right) => {
    const tokenDelta = left.old_token.localeCompare(right.old_token);
    if (tokenDelta !== 0) {
      return tokenDelta;
    }
  ...
  return {
    targetDiscovery: {
      candidate_targets: candidateTargets,
      winning_target_count: winningTargets.length,
      winning_target_code: targetSetCode,
    },
    targetSetCode,
    matchAuditRows,
    collapseMapRows: collapseMapRows.map((row, index) => ({
      seq: index + 1,
      ...row,
    })),
  };
...
async function insertMatchAuditRows(client, rows) {
  await client.query(
    `
      insert into tmp_hgssp_match_audit (
        old_id,
        old_name,
        old_set_code,
        variant_key,
      ...
      from json_to_recordset($1::json) as x(
        ...
        same_token_candidate_count int,
        candidate_count int,
...
async function insertCollapseMapRows(client, rows) {
  await client.query(
    `
      insert into tmp_hgssp_collapse_map (
        seq,
        old_id,
        new_id,
        old_name,
...
function assertTargetDiscovery(targetDiscovery) {
  const winningTargetCount = normalizeCount(targetDiscovery?.winning_target_count);
  if (winningTargetCount === 0) {
    throw new Error('TARGET_DISCOVERY_NO_SINGLE_FULL_TARGET');
  }
  if (winningTargetCount !== 1) {
    throw new Error(`TARGET_DISCOVERY_AMBIGUOUS:${winningTargetCount}`);
  }

  if (!normalizeTextOrNull(targetDiscovery?.winning_target_code)) {
    throw new Error('TARGET_DISCOVERY_NULL_NAMESPACE');
  }
}

async function loadPreconditionSummary(client, targetSetCode) {
  return queryOne(
    client,
    `
      ...
      canonical_summary as (
        ...
        from tmp_hgssp_canonical_candidates
        where new_set_code = $1
      ),
      match_summary as (
        select
          count(*) filter (where candidate_count = 1)::int as exact_token_same_name_count,
          ...
          count(*) filter (where same_token_different_name_count > 0)::int as same_token_different_name_count
        from tmp_hgssp_match_audit
      ),
      map_summary as (
        select
...
function assertPreconditions(summary) {
  ...
  assertEqual(normalizeCount(summary?.exact_token_same_name_count), EXPECTED.mapCount, 'EXACT_TOKEN_SAME_NAME_COUNT_DRIFT');
  ...
  assertZero(summary?.same_token_different_name_count, 'SAME_TOKEN_DIFFERENT_NAME_COUNT');
  ...
  return queryOne(
    ...
    `
      with traits_on_old as (
        ...
        where t.card_print_id in (select old_id from tmp_hgssp_collapse_map)
      ),
      ...
      printing_on_old as (
        ...
        where p.card_print_id in (select old_id from tmp_hgssp_collapse_map)
      ),
      ...
      mappings_on_old as (
        ...
        where em.card_print_id in (select old_id from tmp_hgssp_collapse_map)
      ),
      ...
      target_identity as (
        select count(*)::int as row_count
        from public.card_print_identity cpi
        where cpi.card_print_id in (select new_id from tmp_hgssp_collapse_map)
      )
      select
...
async function loadCanonicalCount(client, targetSetCode) {
  return queryOne(
    client,
    `
      select count(*)::int as canonical_target_count
...
function buildSchemaBackupContent({ generatedAt, collapseMapSamples, fkInventory, schemaSnapshot, fkCounts }) {
  const sections = [];

  sections.push(`-- ${PHASE} PRE-APPLY SCHEMA SNAPSHOT`);
  sections.push(`-- Generated at: ${generatedAt}`);
  sections.push(`-- Mode: apply`);
  sections.push(`-- total_old_ids: ${EXPECTED.mapCount}`);
  if (collapseMapSamples[0]) {
    ...
    sections.push(`-- sample_token: ${collapseMapSamples[0].old_token}`);
    ...
    const row = await queryOne(
      ...
      `
        ...
        where ${quoteIdent(columnName)} in (
          select old_id from tmp_hgssp_collapse_map
        )
      `,
    );
...
async function applyCollapse(client) {
  ...
  const deletedOldTraits = await client.query(`
    ...
    using tmp_hgssp_collapse_map m
...
async function loadPostValidation(client, fkInventory, targetSetCode) {
  ...
  const summary = await queryOne(
    ...
    `
      ...
      route_resolvable as (
        select count(*)::int as row_count
        from public.card_prints cp
        where cp.id in (select new_id from tmp_hgssp_collapse_map)
          and cp.gv_id is not null
      ),
      active_identity_before as (
        select count(*)::int as row_count
        from public.card_print_identity
        where is_active = true
          and identity_domain = $1
      )
      select
        (
          select count(*)::int
          from public.card_prints cp
          where cp.id in (select old_id from tmp_hgssp_collapse_map)
        ) as remaining_old_parent_rows,
        (
          select row_count from unresolved_after
        ) as remaining_unresolved_hgssp,
        (
          select count(*)::int
          from public.card_prints cp
          where cp.set_code = $3
        ...
        (
          select row_count from active_identity_before
        ) as active_identity_total
    `,
    [TARGET_IDENTITY_DOMAIN, SOURCE_SET_CODE_IDENTITY, targetSetCode],
  );
...
function assertPostValidation(postValidation, deletedOldParentRows) {
  ...
  assertZero(postValidation.summary?.remaining_unresolved_hgssp, 'REMAINING_UNRESOLVED_HGSSP');
  ...
  assertEqual(normalizeCount(postValidation.summary?.route_resolvable_target_count), EXPECTED.mapCount, 'ROUTE_RESOLVABLE_TARGET_COUNT_DRIFT');
}
...
async function run() {
  ...
  const client = new Client({
    ...
    application_name: `hgssp_alias_collapse_apply_v1:${MODE}`,
  });
  ...
  try {
    ...
    report.target_discovery = mappingArtifacts.targetDiscovery;
    report.target_set_code = mappingArtifacts.targetSetCode;
    assertTargetDiscovery(report.target_discovery);

    report.preconditions = await loadPreconditionSummary(client, report.target_set_code);
    ...
    report.canonical_count_before = await loadCanonicalCount(client, report.target_set_code);
    assertEqual(
    ...
    report.post_validation = await loadPostValidation(client, fkInventory, report.target_set_code);

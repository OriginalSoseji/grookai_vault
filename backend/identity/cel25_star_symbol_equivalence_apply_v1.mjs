...
const PHASE = 'CEL25_STAR_SYMBOL_EQUIVALENCE_COLLAPSE_V1';
...
const CLASSIFICATION = 'SAFE_BOUNDED_SYMBOL_EQUIVALENCE';
const CONTRACT_NAME = 'STAR_WORD_TAIL_TO_STAR_SYMBOL_EQUIVALENCE_V1';

const SOURCE_OLD_ID = 'c2bdbb6f-10de-4a93-abcf-ed3b8837908b';
const TARGET_NEW_ID = 'c9c1a789-a686-4541-99b7-ac7d4de7be30';
const TARGET_GV_ID = 'GV-PK-CEL-17CC';
const GAP_OLD_ID = 'f7c22698-daa3-4412-84ef-436fb1fe130f';

const EXPECTED = {
  totalUnresolvedCount: 2,
  starTailScopeCount: 1,
  sourceCount: 1,
  outOfScopeCount: 1,
  ...
  reusedTargetCount: 0,
  nonTailWordScopeCount: 0,
  nonStarSymbolCandidateCount: 0,
  ...
  updatedVaultItems: 0,
  remainingUnresolvedTotalRows: 1,
  remainingStarSymbolRows: 0,
  remainingIdentityModelGapRows: 1,
...
async function buildTempStarSurface(client) {
  await client.query(`
    drop table if exists tmp_cel25_star_unresolved_all;
    drop table if exists tmp_cel25_star_scope;
    drop table if exists tmp_cel25_star_canonical;
    drop table if exists tmp_cel25_star_candidate_rows;
    drop table if exists tmp_cel25_star_match_counts;
    drop table if exists tmp_cel25_star_collapse_map;

    create temp table tmp_cel25_star_unresolved_all on commit drop as
    select
      cp.id as old_id,
      cp.name as old_name,
    ...
    create temp table tmp_cel25_star_scope on commit drop as
    select *
    from tmp_cel25_star_unresolved_all
    where old_name ~* ' Star$';

    create temp table tmp_cel25_star_canonical on commit drop as
    select
      cp.id as new_id,
      cp.name as new_name,
      ...
      ) as target_star_semantic_name
    from public.card_prints cp
    where cp.set_code = '${TARGET_SET_CODE}'
      and cp.gv_id is not null;

    create temp table tmp_cel25_star_candidate_rows on commit drop as
    select
      s.old_id,
      s.old_name,
      s.old_set_code,
      ...
      c.new_variant_key,
      c.new_gv_id
    from tmp_cel25_star_scope s
    join tmp_cel25_star_canonical c
      on c.new_number_plain = s.source_base_number_plain
     and c.target_star_semantic_name = s.source_star_semantic_name
     and c.new_name like '%★%';

    create temp table tmp_cel25_star_match_counts on commit drop as
    select
      s.old_id,
      count(distinct c.new_id)::int as candidate_count
    from tmp_cel25_star_scope s
    left join tmp_cel25_star_candidate_rows c
      on c.old_id = s.old_id
    group by s.old_id;

    create temp table tmp_cel25_star_collapse_map on commit drop as
    select
      ...
      source_star_semantic_name as normalized_name,
      source_number_suffix,
      new_number,
      new_number_plain,
      new_variant_key,
      new_gv_id
    from tmp_cel25_star_candidate_rows;

    create unique index tmp_cel25_star_collapse_map_old_uidx
      on tmp_cel25_star_collapse_map (old_id);
  `);
}
  ...
  return queryOne(
    ...
    `
      with reused_targets as (
        select new_id
        from tmp_cel25_star_collapse_map
        group by new_id
        having count(*) > 1
      )
      select
        (select count(*)::int from tmp_cel25_star_unresolved_all) as total_unresolved_count,
        (select count(*)::int from tmp_cel25_star_scope) as star_tail_scope_count,
        (select count(*)::int from tmp_cel25_star_collapse_map) as source_count,
        (
          select count(*)::int
          from tmp_cel25_star_unresolved_all
          where old_id not in (select old_id from tmp_cel25_star_scope)
        ) as out_of_scope_count,
        (select count(*)::int from tmp_cel25_star_canonical) as canonical_target_count,
        (select count(*)::int from tmp_cel25_star_match_counts where candidate_count = 0) as unmatched_count,
        (select count(*)::int from tmp_cel25_star_match_counts where candidate_count > 1) as ambiguous_target_count,
        (select count(*)::int from reused_targets) as reused_target_count,
        (
          select count(*)::int
          from tmp_cel25_star_scope
          where old_name !~* ' Star$'
        ...
        (
          ...
          where card_print_id in (select distinct new_id from tmp_cel25_star_collapse_map)
...
function assertPreconditions(summary) {
  ...
  assertEqual(
    normalizeCount(summary?.star_tail_scope_count),
    EXPECTED.starTailScopeCount,
    'STAR_TAIL_SCOPE_COUNT_DRIFT',
  );
  assertEqual(normalizeCount(summary?.source_count), EXPECTED.sourceCount, 'SOURCE_COUNT_DRIFT');
  assertEqual(
    normalizeCount(summary?.out_of_scope_count),
    EXPECTED.outOfScopeCount,
    'OUT_OF_SCOPE_COUNT_DRIFT',
  );
  assertEqual(
  ...
  assertEqual(
    normalizeCount(summary?.non_tail_word_scope_count),
    EXPECTED.nonTailWordScopeCount,
    'NON_TAIL_WORD_SCOPE_COUNT_DRIFT',
  );
  assertEqual(
    normalizeCount(summary?.non_star_symbol_candidate_count),
    EXPECTED.nonStarSymbolCandidateCount,
    'NON_STAR_SYMBOL_CANDIDATE_COUNT_DRIFT',
  );
  assertEqual(
  ...
  assertIdSetEqual(summary?.out_of_scope_old_ids ?? [], [GAP_OLD_ID], 'OUT_OF_SCOPE_OLD_IDS_DRIFT');
}
    ...
    const row = await queryOne(
      ...
      `
        ...
        where ${quoteIdent(columnName)} in (
          select old_id from tmp_cel25_star_collapse_map
        )
      `,
    );
  ...
  return queryOne(
    ...
    `
      with old_traits as (
        ...
        where card_print_id in (select old_id from tmp_cel25_star_collapse_map)
      ),
      new_traits as (
        select *
        from public.card_print_traits
      ...
      trait_conflicts as (
        ...
        from tmp_cel25_star_collapse_map m
      ...
      select
        ...
        (
          ...
          where card_id in (select old_id from tmp_cel25_star_collapse_map)
...
async function applyCollapse(client) {
  ...
  const activeIdentityConflicts = await queryRows(
    ...
    `
      ...
      where cpi.card_print_id in (
        select distinct new_id from tmp_cel25_star_collapse_map
      )
  ...
  const mergedTraitMetadata = await client.query(`
    ...
    join tmp_cel25_star_collapse_map m
  ...
  const deletedOldTraits = await client.query(`
    ...
    using tmp_cel25_star_collapse_map m
  ...
  const summary = await queryOne(
    ...
    `
      ...
      target_identity as (
        ...
        from public.card_print_identity cpi
        where cpi.card_print_id in (select new_id from tmp_cel25_star_collapse_map)
      ),
      ...
      route_resolvable as (
        select count(*)::int as row_count
        from public.card_prints cp
        where cp.id in (select new_id from tmp_cel25_star_collapse_map)
          and cp.gv_id is not null
      ),
      ...
      select
        (
          select count(*)::int
          from public.card_prints cp
          where cp.id in (select old_id from tmp_cel25_star_collapse_map)
        ...
        (
          ...
          where old_name ~* ' Star$'
        ) as remaining_star_symbol_rows,
        (
          select count(*)::int
          from unresolved_after
          where old_id = $4
        ) as remaining_identity_model_gap_rows,
        (
          select array_agg(old_id::text order by old_id)
          from unresolved_after
    ...
    `,
    [TARGET_IDENTITY_DOMAIN, SOURCE_SET_CODE_IDENTITY, TARGET_SET_CODE, GAP_OLD_ID],
  );
...
function assertPostValidation(postValidation, deletedOldParentRows) {
  ...
  assertEqual(
    normalizeCount(postValidation.summary?.remaining_star_symbol_rows),
    EXPECTED.remainingStarSymbolRows,
    'REMAINING_STAR_SYMBOL_ROWS_DRIFT',
  );
  assertEqual(
    normalizeCount(postValidation.summary?.remaining_identity_model_gap_rows),
    EXPECTED.remainingIdentityModelGapRows,
    'REMAINING_IDENTITY_MODEL_GAP_ROWS_DRIFT',
  );
  assertIdSetEqual(
...
async function run() {
  ...
  const client = new Client({
    ...
    application_name: `cel25_star_symbol_equivalence_apply_v1:${MODE}`,
  });
  ...
  try {
    ...
    await buildTempStarSurface(client);

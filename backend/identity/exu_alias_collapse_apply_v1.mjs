...
const PHASE = 'EXU_ALIAS_COLLAPSE_TO_EX10_V1';
...
const SOURCE_SET_CODE_IDENTITY = 'exu';
const TARGET_SET_CODE = 'ex10';

const EXPECTED = {
  unresolvedCount: 27,
  canonicalTargetCount: 145,
  mapCount: 27,
};
...
const BACKUP_SCHEMA_PATH = path.join(
  process.cwd(),
  'backups',
  'exu_alias_collapse_preapply_schema.sql',
);
const BACKUP_DATA_PATH = path.join(
  process.cwd(),
  'backups',
  'exu_alias_collapse_preapply_data.sql',
);
...
async function buildTempCollapseSurface(client) {
  await client.query(`
    drop table if exists tmp_exu_unresolved;
    drop table if exists tmp_ex10_canonical;
    drop table if exists tmp_exu_match_audit;
    drop table if exists tmp_exu_collapse_map;

    create temp table tmp_exu_unresolved on commit drop as
    select
      cp.id as old_id,
      cp.name as old_name,
    ...
    create index tmp_exu_unresolved_token_idx
      on tmp_exu_unresolved (old_token);

    create temp table tmp_ex10_canonical on commit drop as
    select
      cp.id as new_id,
      cp.name as new_name,
    ...
    create index tmp_ex10_canonical_token_idx
      on tmp_ex10_canonical (new_token);

    create temp table tmp_exu_match_audit (
    ...
    create temp table tmp_exu_collapse_map (
      seq int not null,
    ...
    create unique index tmp_exu_collapse_map_seq_uidx
      on tmp_exu_collapse_map (seq);

    create unique index tmp_exu_collapse_map_old_uidx
      on tmp_exu_collapse_map (old_id);

    create unique index tmp_exu_collapse_map_new_uidx
      on tmp_exu_collapse_map (new_id);
  `);
}
...
async function insertMatchAuditRows(client, rows) {
  await client.query(
    `
      insert into tmp_exu_match_audit (
        old_id,
        old_name,
        old_set_code,
        variant_key,
...
async function insertCollapseMapRows(client, rows) {
  await client.query(
    `
      insert into tmp_exu_collapse_map (
        seq,
        old_id,
        new_id,
        old_name,
  ...
  return queryOne(
    ...
    `
      with traits_on_old as (
        ...
        where t.card_print_id in (select old_id from tmp_exu_collapse_map)
      ),
      ...
      printing_on_old as (
        ...
        where p.card_print_id in (select old_id from tmp_exu_collapse_map)
      ),
      ...
      mappings_on_old as (
        ...
        where em.card_print_id in (select old_id from tmp_exu_collapse_map)
      ),
      ...
      target_identity as (
        select count(*)::int as row_count
        from public.card_print_identity cpi
        where cpi.card_print_id in (select new_id from tmp_exu_collapse_map)
      )
      select
    ...
    const row = await queryOne(
      ...
      `
        ...
        where ${quoteIdent(columnName)} in (
          select old_id from tmp_exu_collapse_map
        )
      `,
    );
  ...
  const summary = await queryOne(
    ...
    `
      ...
      route_resolvable as (
        select count(*)::int as row_count
        from public.card_prints cp
        where cp.id in (select new_id from tmp_exu_collapse_map)
          and cp.gv_id is not null
      ),
      ...
      select
        (
          select count(*)::int
          from public.card_prints cp
          where cp.id in (select old_id from tmp_exu_collapse_map)
        ) as remaining_old_parent_rows,
        (
          select row_count from unresolved_after
        ) as remaining_unresolved_exu,
        (
          select count(*)::int
          from public.card_prints cp
          where cp.set_code = $3
...
function assertPostValidation(postValidation, deletedOldParentRows) {
  ...
  assertZero(postValidation.summary?.remaining_unresolved_exu, 'REMAINING_UNRESOLVED_EXU');
...
async function run() {
  ...
  const client = new Client({
    ...
    application_name: `exu_alias_collapse_apply_v1:${MODE}`,
  });

...
const PHASE = 'SM12_BASE_VARIANT_COLLAPSE_PREP_V1';
...
const SOURCE_SET_CODE_IDENTITY = 'sm12';
const TARGET_SET_CODE = 'sm12';

const EXPECTED = {
  unresolvedCount: 58,
  canonicalTargetCount: 271,
  conflictGroupCount: 58,
  mapCount: 58,
  normalizedNameCount: 57,
...
const BACKUP_SCHEMA_PATH = path.join(
  process.cwd(),
  'backups',
  'sm12_base_variant_preapply_schema.sql',
);
const BACKUP_DATA_PATH = path.join(
  process.cwd(),
  'backups',
  'sm12_base_variant_preapply_data.sql',
);
...
async function buildTempCollapseSurface(client) {
  await client.query(`
    drop table if exists tmp_sm12_unresolved;
    drop table if exists tmp_sm12_canonical;
    drop table if exists tmp_sm12_match_audit;
    drop table if exists tmp_sm12_collapse_map;

    create temp table tmp_sm12_unresolved on commit drop as
    select
      cp.id as old_id,
      cp.name as old_name,
    ...
    create index tmp_sm12_unresolved_token_idx
      on tmp_sm12_unresolved (source_printed_number);

    create temp table tmp_sm12_canonical on commit drop as
    select
      cp.id as new_id,
      cp.name as new_name,
    ...
    create index tmp_sm12_canonical_number_plain_idx
      on tmp_sm12_canonical (new_number_plain);

    create temp table tmp_sm12_match_audit (
    ...
    create temp table tmp_sm12_collapse_map (
      seq int not null,
    ...
    create unique index tmp_sm12_collapse_map_seq_uidx
      on tmp_sm12_collapse_map (seq);

    create unique index tmp_sm12_collapse_map_old_uidx
      on tmp_sm12_collapse_map (old_id);

    create unique index tmp_sm12_collapse_map_new_uidx
      on tmp_sm12_collapse_map (new_id);
  `);
}
...
function buildMatchArtifacts(unresolvedRows, canonicalRows) {
  ...
  for (const row of canonicalRows) {
    const targetNameNormalizedV1 = nameNormalizeV1(row.new_name);
    const bucket = canonicalByBase.get(row.new_number_plain) ?? [];
    bucket.push({
      ...row,
      target_name_normalized_v1: targetNameNormalizedV1,
    });
    canonicalByBase.set(row.new_number_plain, bucket);
  }
  ...
  for (const row of unresolvedRows) {
    const parsedToken = parsePrintedTokenV1(row.source_printed_number);
    const sourceNameNormalizedV1 = nameNormalizeV1(row.old_name ?? row.source_normalized_printed_name);
    const baseCandidates = parsedToken.base_number_plain
      ? canonicalByBase.get(parsedToken.base_number_plain) ?? []
      : [];
    const matchedCandidates = baseCandidates.filter(
      (candidate) =>
        candidate.target_name_normalized_v1 !== null &&
        candidate.target_name_normalized_v1 === sourceNameNormalizedV1,
    );
    const matched = matchedCandidates[0] ?? null;

    let matchCategory = 'invalid';
    if (matchedCandidates.length === 1 && baseCandidates.length === 1 && parsedToken.base_number_plain) {
      matchCategory =
        normalizeTextOrNull(parsedToken.printed_token) !== normalizeTextOrNull(matched.new_number)
          ? 'suffix_variant'
          : 'name_normalize_v1';
    }
    ...
    matchAuditRows.push({
      ...
      source_base_number_plain: parsedToken.base_number_plain,
      source_number_suffix: parsedToken.number_suffix,
      source_name_normalized_v1: sourceNameNormalizedV1,
      base_group_candidate_count: baseCandidates.length,
      candidate_count: matchedCandidates.length,
      invalid_candidate_count: baseCandidates.filter(
      ...
      candidate_new_number: matched?.new_number ?? null,
      candidate_new_number_plain: matched?.new_number_plain ?? null,
      candidate_new_gv_id: matched?.new_gv_id ?? null,
    });

    if (matchCategory !== 'invalid') {
...
async function insertMatchAuditRows(client, rows) {
  await client.query(
    `
      insert into tmp_sm12_match_audit (
        old_id,
        old_name,
        old_set_code,
        old_parent_number,
...
async function insertCollapseMapRows(client, rows) {
  await client.query(
    `
      insert into tmp_sm12_collapse_map (
        seq,
        old_id,
        new_id,
        old_name,
    ...
    `
      with unresolved_summary as (
        select
          ...
          count(*) filter (where old_parent_number is not null)::int as non_null_old_parent_number_count,
          count(*) filter (where old_parent_number_plain is not null)::int as non_null_old_parent_number_plain_count
        from tmp_sm12_unresolved
      ),
      ...
      conflict_group_summary as (
        select
          count(*)::int as conflict_group_count,
          count(*) filter (where canonical_row_count = 1)::int as exact_one_canonical_group_count,
          count(*) filter (where canonical_row_count = 0)::int as no_canonical_group_count,
          count(*) filter (where canonical_row_count > 1)::int as multiple_canonical_group_count
        from (
          select
            base.base_number_plain,
            count(c.new_id)::int as canonical_row_count
          from (
            select distinct source_base_number_plain as base_number_plain
            from tmp_sm12_match_audit
          ) base
          left join tmp_sm12_canonical c
            on c.new_number_plain = base.base_number_plain
          group by base.base_number_plain
        ) grouped
      ),
      match_summary as (
        select
          count(*) filter (where candidate_count = 1)::int as matched_count,
          ...
          count(*) filter (where invalid_candidate_count > 0)::int as same_base_different_name_count,
          count(*) filter (where match_category = 'name_normalize_v1')::int as normalized_name_count,
          count(*) filter (where match_category = 'suffix_variant')::int as suffix_variant_count,
          count(*) filter (where match_category = 'invalid')::int as invalid_category_count
        from tmp_sm12_match_audit
      ),
      map_summary as (
        select
      ...
      select
        ...
        unresolved_summary.non_null_old_parent_number_count,
        unresolved_summary.non_null_old_parent_number_plain_count,
        canonical_summary.canonical_target_count,
        conflict_group_summary.conflict_group_count,
        conflict_group_summary.exact_one_canonical_group_count,
        conflict_group_summary.no_canonical_group_count,
        conflict_group_summary.multiple_canonical_group_count,
        match_summary.matched_count,
        match_summary.unmatched_count,
        match_summary.multiple_match_old_count,
        match_summary.same_base_different_name_count,
        match_summary.normalized_name_count,
        match_summary.suffix_variant_count,
        match_summary.invalid_category_count,
      ...
      cross join conflict_group_summary
...
function assertPreconditions(summary) {
  ...
  assertEqual(
    normalizeCount(summary?.conflict_group_count),
    EXPECTED.conflictGroupCount,
    'CONFLICT_GROUP_COUNT_DRIFT',
  );
  assertEqual(
    normalizeCount(summary?.exact_one_canonical_group_count),
    EXPECTED.conflictGroupCount,
    'EXACT_ONE_CANONICAL_GROUP_COUNT_DRIFT',
  );
  assertEqual(normalizeCount(summary?.matched_count), EXPECTED.mapCount, 'MATCHED_COUNT_DRIFT');
  ...
  assertZero(summary?.no_canonical_group_count, 'NO_CANONICAL_GROUPS');
  assertZero(summary?.multiple_canonical_group_count, 'MULTIPLE_CANONICAL_GROUPS');
  ...
  assertZero(summary?.same_base_different_name_count, 'SAME_BASE_DIFFERENT_NAME_COUNT');
  assertZero(summary?.invalid_category_count, 'INVALID_MATCH_CATEGORY_COUNT');
  ...
  return queryOne(
    ...
    `
      with traits_on_old as (
        ...
        where t.card_print_id in (select old_id from tmp_sm12_collapse_map)
      ),
      ...
      printing_on_old as (
        ...
        where p.card_print_id in (select old_id from tmp_sm12_collapse_map)
      ),
      ...
      mappings_on_old as (
        ...
        where em.card_print_id in (select old_id from tmp_sm12_collapse_map)
      ),
      ...
      target_identity as (
        select count(*)::int as row_count
        from public.card_print_identity cpi
        where cpi.card_print_id in (select new_id from tmp_sm12_collapse_map)
      ),
      vault_on_old as (
        select count(*)::int as row_count
        from public.vault_items vi
        where vi.card_id in (select old_id from tmp_sm12_collapse_map)
      )
      select
    ...
    const row = await queryOne(
      ...
      `
        ...
        where ${quoteIdent(columnName)} in (
          select old_id from tmp_sm12_collapse_map
        )
      `,
    );
  ...
  const summary = await queryOne(
    ...
    `
      ...
      route_resolvable as (
        select count(*)::int as row_count
        from public.card_prints cp
        where cp.id in (select new_id from tmp_sm12_collapse_map)
          and cp.gv_id is not null
      ),
      ...
      select
        (
          select count(*)::int
          from public.card_prints cp
          where cp.id in (select old_id from tmp_sm12_collapse_map)
...
async function run() {
  ...
  const report = {
    ...
    normalization_contract: {
      name_normalize_v1:
        "lowercase -> unicode apostrophe to ASCII -> remove hyphen around GX -> collapse whitespace -> trim",
      token_normalize_v1: 'printed number base = numeric portion; suffix letters are variant markers',
    },
    preconditions: null,
  ...
  const client = new Client({
    ...
    application_name: `sm12_base_variant_collapse_apply_v1:${MODE}`,
  });
  ...
  try {
    ...
    await populateMatchArtifacts(client);

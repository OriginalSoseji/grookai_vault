...
const PHASE = 'ECARD2_HOLO_PREFIX_EXACT_TOKEN_PROMOTION_V1';
...
const TMP_MAP = 'tmp_ecard2_holo_prefix_promotion_map_v1';

const EXPECTED_ROWS = [
  {
    old_id: '8272e758-ac91-41c3-87ad-9b3622155bf1',
    old_name: 'Exeggutor',
    old_printed_token: 'H10',
    variant_key: '',
    new_gv_id: 'GV-PK-AQ-H10',
  },
  {
    old_id: 'c7fdcf93-bf83-41fa-a6e2-edd63bc391f0',
    old_name: 'Kingdra',
    old_printed_token: 'H14',
    variant_key: '',
    new_gv_id: 'GV-PK-AQ-H14',
  },
  {
    old_id: '62661fa2-40b4-48bf-96c1-8d225581a3d2',
    old_name: 'Scizor',
    old_printed_token: 'H21',
    variant_key: '',
    new_gv_id: 'GV-PK-AQ-H21',
  },
  {
    old_id: 'eb8d04d0-07ae-4805-8861-b1a1a286f52a',
    old_name: 'Slowking',
    old_printed_token: 'H22',
    variant_key: '',
    new_gv_id: 'GV-PK-AQ-H22',
  },
  {
    old_id: '10c9d12e-77c9-4334-9fde-1542a79b1f5a',
    old_name: 'Steelix',
    old_printed_token: 'H23',
    variant_key: '',
    new_gv_id: 'GV-PK-AQ-H23',
  },
  {
    old_id: '7215c907-c6ae-4951-b552-a7a543bae195',
    old_name: 'Sudowoodo',
    old_printed_token: 'H24',
    variant_key: '',
    new_gv_id: 'GV-PK-AQ-H24',
  },
  {
    old_id: '6a14016b-edef-4f74-b360-20187e09e2bb',
    old_name: 'Tentacruel',
    old_printed_token: 'H26',
    variant_key: '',
    new_gv_id: 'GV-PK-AQ-H26',
  },
  {
    old_id: 'aef2e04c-4713-4801-b815-5fa354d68659',
    old_name: 'Togetic',
    old_printed_token: 'H27',
    variant_key: '',
    new_gv_id: 'GV-PK-AQ-H27',
  },
  {
    old_id: '4fcf41bc-8e06-44fe-ad64-da6664f4d859',
    old_name: 'Umbreon',
    old_printed_token: 'H29',
    variant_key: '',
    new_gv_id: 'GV-PK-AQ-H29',
  },
  {
    old_id: '1081cdf5-5334-432f-85d9-4d0c769836f8',
    old_name: 'Vileplume',
    old_printed_token: 'H31',
    variant_key: '',
    new_gv_id: 'GV-PK-AQ-H31',
  },
];

const EXPECTED = {
  source_count: 10,
  unresolved_count_before: 10,
  canonical_count_before: 184,
  canonical_count_delta: 10,
};
...
async function loadSourceRows(client, setId) {
  return queryRows(
    ...
    `
      ...
      order by upper(cpi.printed_number), cp.id
    `,
  ...
  for (const expected of EXPECTED_ROWS) {
    ...
    if (!liveRow) {
      diffs.push({
        ...
        expected_name: expected.old_name,
        expected_token: expected.old_printed_token,
      });
      continue;
    }

    if (normalizeTextOrNull(liveRow.old_name) !== expected.old_name) {
      diffs.push({
        type: 'name_mismatch',
        old_id: expected.old_id,
        expected_name: expected.old_name,
    ...
    if ((normalizeTextOrNull(liveRow.variant_key) ?? '') !== expected.variant_key) {
      diffs.push({
...
function buildCandidateMap(sourceRows, setMetadata) {
  return sourceRows
    .map((row) => {
      ...
      if (!isHoloToken(oldPrintedToken)) {
        throw new Error(`NON_HOLO_TOKEN_IN_SCOPE:${row.old_parent_id}:${row.old_printed_token}`);
      }

      const newGvId = buildCardPrintGvIdV1({
        setCode: TARGET_SET_CODE,
        printedSetAbbrev: setMetadata.printed_set_abbrev,
        number: oldPrintedToken,
      ...
      return {
        ...
        proposed_number_plain: oldPrintedToken,
  ...
  return queryRows(
    ...
    `
      ...
      join public.card_prints cp
       ...
       and (cp.number = payload.old_printed_token or cp.number_plain = payload.old_printed_token)
       and cp.id <> payload.old_id
      order by payload.old_id, cp.id
    `,
    [payload],
...
async function createTempPromotionMap(client, candidateMap) {
  await client.query(`
    ...
    create unique index tmp_ecard2_holo_prefix_promotion_map_v1_old_uidx on ${TMP_MAP} (old_id);
    create unique index tmp_ecard2_holo_prefix_promotion_map_v1_new_uidx on ${TMP_MAP} (new_id);
    create unique index tmp_ecard2_holo_prefix_promotion_map_v1_gvid_uidx on ${TMP_MAP} (new_gv_id);
  `);
...
async function run() {
  ...
  try {
    ...
    const fkCounts = await loadFkCounts(client, fkInventory, `select old_id from ${TMP_MAP}`);
    ...
    for (const row of sampleAfterRows) {
      ...
      if (row.new_number !== row.old_printed_token || row.new_number_plain !== row.old_printed_token) {
        throw new Error(`TARGET_TOKEN_DRIFT:${row.new_id}:${row.new_number}:${row.new_number_plain}:${row.old_printed_token}`);
      }
    }

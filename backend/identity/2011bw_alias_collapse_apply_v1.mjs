...
async function buildTempCollapseSurface(client) {
  await client.query(`
    drop table if exists tmp_2011bw_unresolved;
    drop table if exists tmp_2011bw_canonical;
    drop table if exists tmp_2011bw_candidates;
    drop table if exists tmp_2011bw_old_counts;
    drop table if exists tmp_2011bw_new_counts;
    drop table if exists tmp_2011bw_collapse_map;

    create temp table tmp_2011bw_unresolved on commit drop as
    select
      cp.id as old_id,
      cp.name as old_name,
    ...
    create temp table tmp_2011bw_canonical on commit drop as
    select
      cp.id as new_id,
      cp.name as new_name,
    ...
    create temp table tmp_2011bw_candidates on commit drop as
    select
      u.old_id,
      c.new_id
    from tmp_2011bw_unresolved u
    join tmp_2011bw_canonical c
      on c.new_number_normalized = u.old_number_normalized
     and c.normalized_name = u.normalized_printed_name;

    create temp table tmp_2011bw_old_counts on commit drop as
    select old_id, count(*)::int as match_count
    from tmp_2011bw_candidates
    group by old_id;

    create temp table tmp_2011bw_new_counts on commit drop as
    select new_id, count(*)::int as match_count
    from tmp_2011bw_candidates
    group by new_id;

    create temp table tmp_2011bw_collapse_map on commit drop as
    select
      ...
      c.new_gv_id
    from tmp_2011bw_unresolved u
    join tmp_2011bw_candidates candidate
      on candidate.old_id = u.old_id
    join tmp_2011bw_canonical c
      on c.new_id = candidate.new_id
    join tmp_2011bw_old_counts old_counts
      on old_counts.old_id = candidate.old_id
    join tmp_2011bw_new_counts new_counts
      on new_counts.new_id = candidate.new_id
    where old_counts.match_count = 1
      and new_counts.match_count = 1;

    create unique index tmp_2011bw_collapse_map_old_uidx
      on tmp_2011bw_collapse_map (old_id);

    create unique index tmp_2011bw_collapse_map_new_uidx
      on tmp_2011bw_collapse_map (new_id);
  `);
}
  ...
  return queryOne(
    ...
    `
      with unresolved_counts as (
        ...
        from tmp_2011bw_unresolved
      ),
      canonical_target as (
        select count(*)::int as row_count from tmp_2011bw_canonical
      ),
      map_summary as (
        select
        ...
        from tmp_2011bw_collapse_map
      ),
      multiple_old as (
        select count(*)::int as row_count from tmp_2011bw_old_counts where match_count > 1
      ),
      reused_new as (
        select count(*)::int as row_count from tmp_2011bw_new_counts where match_count > 1
      ),
      unmatched as (
        select count(*)::int as row_count
        from tmp_2011bw_unresolved u
        where not exists (select 1 from tmp_2011bw_collapse_map m where m.old_id = u.old_id)
      ),
      ...
      target_identity as (
        ...
        from tmp_2011bw_collapse_map m
  ...
  return queryOne(
    ...
    `
      ...
      target_identity as (
        ...
        where card_print_id in (select new_id from tmp_2011bw_collapse_map)
      )
      select
        (select count(*)::int from public.card_print_traits where card_print_id in (select old_id from tmp_2011bw_collapse_map)) as old_trait_row_count,
        ...
        (select count(*)::int from public.card_printings where card_print_id in (select old_id from tmp_2011bw_collapse_map)) as old_printing_row_count,
        ...
        (select count(*)::int from public.external_mappings where card_print_id in (select old_id from tmp_2011bw_collapse_map)) as old_external_mapping_row_count,
...
async function loadBatchFkCounts(client) {
  return {
    'card_print_identity.card_print_id': normalizeCount((await queryOne(client, `select count(*)::int as row_count from public.card_print_identity where card_print_id in (select old_id from tmp_2011bw_collapse_map)`))?.row_count),
    'card_print_traits.card_print_id': normalizeCount((await queryOne(client, `select count(*)::int as row_count from public.card_print_traits where card_print_id in (select old_id from tmp_2011bw_collapse_map)`))?.row_count),
    'card_printings.card_print_id': normalizeCount((await queryOne(client, `select count(*)::int as row_count from public.card_printings where card_print_id in (select old_id from tmp_2011bw_collapse_map)`))?.row_count),
    'external_mappings.card_print_id': normalizeCount((await queryOne(client, `select count(*)::int as row_count from public.external_mappings where card_print_id in (select old_id from tmp_2011bw_collapse_map)`))?.row_count),
    'vault_items.card_id': normalizeCount((await queryOne(client, `select count(*)::int as row_count from public.vault_items where card_id in (select old_id from tmp_2011bw_collapse_map)`))?.row_count),
  };
}
...
async function applySingleBatch(client) {
  ...
  const updatedIdentityRows = await client.query(`update public.card_print_identity cpi set card_print_id = m.new_id from tmp_2011bw_collapse_map m where cpi.card_print_id = m.old_id`);
  const insertedTraits = await client.query(`insert into public.card_print_traits (card_print_id, trait_type, trait_value, source, confidence, created_at, hp, national_dex, types, rarity, supertype, card_category, legacy_rarity) select m.new_id, t.trait_type, t.trait_value, t.source, t.confidence, t.created_at, t.hp, t.national_dex, t.types, t.rarity, t.supertype, t.card_category, t.legacy_rarity from public.card_print_traits t join tmp_2011bw_collapse_map m on m.old_id = t.card_print_id on conflict (card_print_id, trait_type, trait_value, source) do nothing`);
  const deletedTraits = await client.query(`delete from public.card_print_traits t using tmp_2011bw_collapse_map m where t.card_print_id = m.old_id`);
  const mergedPrintingMetadata = await client.query(`update public.card_printings new_p set provenance_source = coalesce(new_p.provenance_source, old_p.provenance_source), provenance_ref = coalesce(new_p.provenance_ref, old_p.provenance_ref), created_by = coalesce(new_p.created_by, old_p.created_by) from public.card_printings old_p join tmp_2011bw_collapse_map m on m.old_id = old_p.card_print_id where new_p.card_print_id = m.new_id and new_p.finish_key = old_p.finish_key and ((new_p.provenance_source is null and old_p.provenance_source is not null) or (new_p.provenance_ref is null and old_p.provenance_ref is not null) or (new_p.created_by is null and old_p.created_by is not null))`);
  const movedUniquePrintings = await client.query(`update public.card_printings old_p set card_print_id = m.new_id from tmp_2011bw_collapse_map m where old_p.card_print_id = m.old_id and not exists (select 1 from public.card_printings new_p where new_p.card_print_id = m.new_id and new_p.finish_key = old_p.finish_key)`);
  const deletedRedundantPrintings = await client.query(`delete from public.card_printings old_p using tmp_2011bw_collapse_map m where old_p.card_print_id = m.old_id and exists (select 1 from public.card_printings new_p where new_p.card_print_id = m.new_id and new_p.finish_key = old_p.finish_key)`);
  const updatedExternalMappings = await client.query(`update public.external_mappings em set card_print_id = m.new_id from tmp_2011bw_collapse_map m where em.card_print_id = m.old_id`);
  const updatedVaultItems = await client.query(`update public.vault_items vi set card_id = m.new_id, gv_id = cp_new.gv_id from tmp_2011bw_collapse_map m join public.card_prints cp_new on cp_new.id = m.new_id where vi.card_id = m.old_id`);
...
async function loadPostValidation(client, fkInventory, activeIdentityTotalBefore) {
  const remainingOldReferences = await loadFkCounts(client, fkInventory, `select old_id from tmp_2011bw_collapse_map`);
  const summary = await queryOne(
    ...
    `
      ...
      route_rows as (
        select count(*)::int as row_count from public.card_prints where id in (select new_id from tmp_2011bw_collapse_map) and gv_id is not null
      ),
      active_identity_total as (
        select count(*)::int as row_count from public.card_print_identity where is_active = true
      )
      select
        (select count(*)::int from public.card_prints where id in (select old_id from tmp_2011bw_collapse_map)) as remaining_old_parent_rows,
        (select row_count from unresolved_after) as remaining_unresolved_null_gvid_rows_for_2011bw,
        (select count(*)::int from public.card_prints where set_code = $3 and gv_id is not null) as canonical_target_count,
        (select count(*)::int from tmp_2011bw_collapse_map m join public.card_prints cp on cp.id = m.new_id where cp.gv_id is distinct from m.new_gv_id) as target_gv_id_drift_count,
...
function assertPostValidation(postValidation, deletedOldParentRows) {
  ...
  assertZero(postValidation.summary?.remaining_unresolved_null_gvid_rows_for_2011bw, 'REMAINING_UNRESOLVED_NULL_GVID_ROWS_FOR_2011BW');
...
async function run() {
  ...
  const client = new Client({
    ...
    application_name: `2011bw_alias_collapse_apply_v1:${MODE}`,
  });
  ...
  try {
    ...
    report.fk_inventory = await loadFkCounts(client, fkInventory, `select old_id from tmp_2011bw_collapse_map`);
    ...
    const deletedParents = await client.query(`delete from public.card_prints cp using tmp_2011bw_collapse_map m where cp.id = m.old_id`);

...
const PHASE = 'SM5_IDENTITY_RESOLUTION_V1';
...
const SOURCE_SET_CODE_IDENTITY = 'sm5';
const TARGET_SET_CODE = 'sm5';

const EXPECTED = {
  unresolvedCount: 31,
  canonicalTargetCount: 173,
  exactMatchCount: 0,
  sameTokenDifferentNameCount: 27,
  exactUnmatchedCount: 31,
  classification: 'BASE_VARIANT_COLLAPSE',
  mapCount: 31,
  normalizedNameCount: 27,
  suffixVariantCount: 4,
};
...
const BACKUP_SCHEMA_PATH = path.join(process.cwd(), 'backups', 'sm5_preapply_schema.sql');
const BACKUP_DATA_PATH = path.join(process.cwd(), 'backups', 'sm5_preapply_data.sql');
...
async function buildTempCollapseSurface(client) {
  await client.query(`
    drop table if exists tmp_sm5_unresolved;
    drop table if exists tmp_sm5_canonical;
    drop table if exists tmp_sm5_match_audit;
    drop table if exists tmp_sm5_collapse_map;

    create temp table tmp_sm5_unresolved on commit drop as
    select
      cp.id as old_id,
      cp.name as old_name,
    ...
    create index tmp_sm5_unresolved_token_idx
      on tmp_sm5_unresolved (source_printed_number);

    create temp table tmp_sm5_canonical on commit drop as
    select
      cp.id as new_id,
      cp.name as new_name,
    ...
    create index tmp_sm5_canonical_token_idx
      on tmp_sm5_canonical (new_number);

    create index tmp_sm5_canonical_number_plain_idx
      on tmp_sm5_canonical (new_number_plain);

    create temp table tmp_sm5_match_audit (
    ...
    create temp table tmp_sm5_collapse_map (
      seq int not null,
    ...
    create unique index tmp_sm5_collapse_map_seq_uidx
      on tmp_sm5_collapse_map (seq);

    create unique index tmp_sm5_collapse_map_old_uidx
      on tmp_sm5_collapse_map (old_id);

    create unique index tmp_sm5_collapse_map_new_uidx
      on tmp_sm5_collapse_map (new_id);
  `);
}
...
async function insertMatchAuditRows(client, rows) {
  await client.query(
    `
      insert into tmp_sm5_match_audit (
        old_id,
        old_name,
        old_set_code,
        old_parent_number,
...
async function insertCollapseMapRows(client, rows) {
  await client.query(
    `
      insert into tmp_sm5_collapse_map (
        seq,
        old_id,
        new_id,
        old_name,
  ...
  return queryOne(
    ...
    `
      with traits_on_old as (
        ...
        where t.card_print_id in (select old_id from tmp_sm5_collapse_map)
      ),
      ...
      printing_on_old as (
        ...
        where p.card_print_id in (select old_id from tmp_sm5_collapse_map)
      ),
      ...
      mappings_on_old as (
        ...
        where em.card_print_id in (select old_id from tmp_sm5_collapse_map)
      ),
      ...
      target_identity as (
        select count(*)::int as row_count
        from public.card_print_identity cpi
        where cpi.card_print_id in (select new_id from tmp_sm5_collapse_map)
      ),
      vault_on_old as (
        select count(*)::int as row_count
        from public.vault_items vi
        where vi.card_id in (select old_id from tmp_sm5_collapse_map)
      )
      select
    ...
    const row = await queryOne(
      ...
      `
        ...
        where ${quoteIdent(columnName)} in (
          select old_id from tmp_sm5_collapse_map
        )
      `,
    );
  ...
  const summary = await queryOne(
    ...
    `
      ...
      route_resolvable as (
        select count(*)::int as row_count
        from public.card_prints cp
        where cp.id in (select new_id from tmp_sm5_collapse_map)
          and cp.gv_id is not null
      ),
      ...
      select
        (
          select count(*)::int
          from public.card_prints cp
          where cp.id in (select old_id from tmp_sm5_collapse_map)
...
async function run() {
  ...
  const client = new Client({
    ...
    application_name: `sm5_identity_resolution_apply_v1:${MODE}`,
  });

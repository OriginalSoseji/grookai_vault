...
const PHASE = 'SMP_ALIAS_COLLAPSE_TO_CANONICAL_SMP_V1';
...
const SOURCE_SET_CODE_IDENTITY = 'smp';
const TARGET_SET_CODE = 'smp';

const EXPECTED = {
  unresolvedCount: 84,
  canonicalTargetCount: 248,
  mapCount: 84,
};
...
const BACKUP_SCHEMA_PATH = path.join(
  process.cwd(),
  'backups',
  'smp_alias_collapse_preapply_schema.sql',
);
const BACKUP_DATA_PATH = path.join(
  process.cwd(),
  'backups',
  'smp_alias_collapse_preapply_data.sql',
);
...
async function buildTempCollapseSurface(client) {
  await client.query(`
    drop table if exists tmp_smp_unresolved;
    drop table if exists tmp_smp_canonical;
    drop table if exists tmp_smp_match_audit;
    drop table if exists tmp_smp_collapse_map;

    create temp table tmp_smp_unresolved on commit drop as
    select
      cp.id as old_id,
      cp.name as old_name,
    ...
    create index tmp_smp_unresolved_number_idx
      on tmp_smp_unresolved (printed_number);

    create temp table tmp_smp_canonical on commit drop as
    select
      cp.id as new_id,
      cp.name as new_name,
    ...
    create index tmp_smp_canonical_number_idx
      on tmp_smp_canonical (new_number);

    create temp table tmp_smp_match_audit (
      ...
      db_normalized_name text not null,
    ...
    create temp table tmp_smp_collapse_map (
      seq int not null,
    ...
    create unique index tmp_smp_collapse_map_seq_uidx
      on tmp_smp_collapse_map (seq);

    create unique index tmp_smp_collapse_map_old_uidx
      on tmp_smp_collapse_map (old_id);

    create unique index tmp_smp_collapse_map_new_uidx
      on tmp_smp_collapse_map (new_id);
  `);
}
  ...
  const unresolvedRows = await queryRows(
    ...
    `
      select
        ...
        printed_set_abbrev
      from tmp_smp_unresolved
      order by nullif(regexp_replace(printed_number, '^[^0-9]+', ''), '')::int, old_id
    `,
  );
  ...
  const canonicalRows = await queryRows(
    ...
    `
      select
        ...
        variant_key,
        printed_set_abbrev
      from tmp_smp_canonical
      order by nullif(regexp_replace(new_number, '^[^0-9]+', ''), '')::int, new_id
    `,
  );
...
function buildMatchArtifacts(unresolvedRows, canonicalRows) {
  ...
  for (const row of canonicalRows) {
    const promoCode = normalizeTextOrNull(row.new_number);
    if (!promoCode) {
      continue;
    }
  ...
  for (const row of unresolvedRows) {
    ...
    const matchedCandidates = samePromoCandidates.filter((candidate) => {
    ...
    matchAuditRows.push({
      ...
      db_normalized_name: dbNormalizedName,
    ...
    if (matchedCandidates.length === 1) {
      collapseMapRows.push({
        ...
        db_normalized_name: dbNormalizedName,
        old_repo_normalized_name: normalizeRepoName(row.old_name, matched.new_name),
  ...
  collapseMapRows.sort((left, right) => {
    const promoDelta = promoSortKey(left.printed_number) - promoSortKey(right.printed_number);
    if (promoDelta !== 0) {
      return promoDelta;
    }
...
async function insertMatchAuditRows(client, rows) {
  await client.query(
    `
      insert into tmp_smp_match_audit (
        old_id,
        old_name,
        old_set_code,
        variant_key,
...
async function insertCollapseMapRows(client, rows) {
  await client.query(
    `
      insert into tmp_smp_collapse_map (
        seq,
        old_id,
        new_id,
        old_name,
...
async function loadPreconditionSummary(client) {
  return queryOne(
    ...
    `
      ...
      match_summary as (
        select
          count(*) filter (where candidate_count = 1)::int as same_promo_same_name_count,
        ...
        from tmp_smp_match_audit
      ),
      same_promo_different_name as (
        select count(*)::int as row_count
        from tmp_smp_unresolved u
        where exists (
          select 1
          from tmp_smp_canonical c
          where c.new_number = u.printed_number
        )
        and not exists (
          select 1
          from tmp_smp_collapse_map m
      ...
      select
        ...
        unresolved_summary.valid_promo_code_count,
        canonical_summary.canonical_target_count,
        match_summary.same_promo_same_name_count,
        match_summary.multiple_match_old_count,
        reused_new.row_count as reused_new_count,
        match_summary.unmatched_count,
        same_promo_different_name.row_count as same_promo_different_name_count,
      ...
      cross join same_promo_different_name
...
function assertPreconditions(summary) {
  ...
  assertEqual(normalizeCount(summary?.valid_promo_code_count), EXPECTED.unresolvedCount, 'VALID_PROMO_CODE_COUNT_DRIFT');
  ...
  assertEqual(normalizeCount(summary?.same_promo_same_name_count), EXPECTED.mapCount, 'SAME_PROMO_SAME_NAME_COUNT_DRIFT');
  ...
  assertZero(summary?.same_promo_different_name_count, 'SAME_PROMO_DIFFERENT_NAME_COUNT');
  ...
  return queryOne(
    ...
    `
      with traits_on_old as (
        ...
        where t.card_print_id in (select old_id from tmp_smp_collapse_map)
      ),
      ...
      printing_on_old as (
        ...
        where p.card_print_id in (select old_id from tmp_smp_collapse_map)
      ),
      ...
      mappings_on_old as (
        ...
        where em.card_print_id in (select old_id from tmp_smp_collapse_map)
      ),
      ...
      target_identity as (
        select count(*)::int as row_count
        from public.card_print_identity cpi
        where cpi.card_print_id in (select new_id from tmp_smp_collapse_map)
      )
      select
...
function buildSchemaBackupContent({ generatedAt, collapseMapSamples, fkInventory, schemaSnapshot, fkCounts }) {
  ...
  if (collapseMapSamples[0]) {
    ...
    sections.push(`-- sample_printed_number: ${collapseMapSamples[0].printed_number}`);
    ...
    const row = await queryOne(
      ...
      `
        ...
        where ${quoteIdent(columnName)} in (
          select old_id from tmp_smp_collapse_map
        )
      `,
    );
  ...
  const summary = await queryOne(
    ...
    `
      ...
      route_resolvable as (
        select count(*)::int as row_count
        from public.card_prints cp
        where cp.id in (select new_id from tmp_smp_collapse_map)
          and cp.gv_id is not null
      ),
      ...
      select
        (
          select count(*)::int
          from public.card_prints cp
          where cp.id in (select old_id from tmp_smp_collapse_map)
        ) as remaining_old_parent_rows,
        (
          select row_count from unresolved_after
        ) as remaining_unresolved_smp,
        (
          select count(*)::int
          from public.card_prints cp
          where cp.set_code = $3
...
function assertPostValidation(postValidation, deletedOldParentRows) {
  ...
  assertZero(postValidation.summary?.remaining_unresolved_smp, 'REMAINING_UNRESOLVED_SMP');
...
async function run() {
  ...
  const client = new Client({
    ...
    application_name: `smp_alias_collapse_apply_v1:${MODE}`,
  });

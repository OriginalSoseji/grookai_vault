...
const PHASE = 'SMP_NAMESPACE_MIGRATION_V1';
...
const LEGACY_PREFIX = 'GV-PK-PR-SM-';
const TARGET_PREFIX = 'GV-PK-SM-';
const EXPECTED = {
  candidateCount: 248,
};
...
const BACKUP_SCHEMA_PATH = path.join(
  process.cwd(),
  'backups',
  'smp_namespace_migration_preapply_schema.sql',
);
const BACKUP_DATA_PATH = path.join(
  process.cwd(),
  'backups',
  'smp_namespace_migration_preapply_data.sql',
);
...
async function buildTempCandidateSurface(client) {
  await client.query(`
    drop table if exists tmp_smp_namespace_candidates;

    create temp table tmp_smp_namespace_candidates on commit drop as
    select
    ...
    create unique index tmp_smp_namespace_candidates_card_print_uidx
      on tmp_smp_namespace_candidates (card_print_id);

    create unique index tmp_smp_namespace_candidates_old_gvid_uidx
      on tmp_smp_namespace_candidates (old_gv_id);

    create unique index tmp_smp_namespace_candidates_new_gvid_uidx
      on tmp_smp_namespace_candidates (new_gv_id);
  `);
}
  ...
  return queryOne(
    ...
    `
      with candidate_summary as (
        ...
        from tmp_smp_namespace_candidates
      ),
      ...
      live_collisions as (
        select count(*)::int as row_count
        from public.card_prints cp
        join tmp_smp_namespace_candidates c
  ...
  return queryRows(
    ...
    `
      select
        ...
        new_gv_id
      from tmp_smp_namespace_candidates
      where number in ('SM01', 'SM100', 'SM248')
      order by number
    `,
  );
}
...
async function createBackupArtifacts(client, sampleRows, preconditions) {
  ...
  const ids = (
    await queryRows(client, `select card_print_id from tmp_smp_namespace_candidates order by card_print_id`)
...
async function applyMigration(client) {
  const updatedCardPrints = await client.query(`
    ...
    from tmp_smp_namespace_candidates c
...
async function loadSampleAfterRows(client) {
  return queryRows(
    ...
    `
      ...
      from public.card_prints
      where set_code = $1
        and number in ('SM01', 'SM100', 'SM248')
      order by number
    `,
    [TARGET_SET_CODE],
  );
...
async function run() {
  ...
  const client = new Client({
    ...
    application_name: `smp_namespace_migration_apply_v1:${MODE}`,
  });

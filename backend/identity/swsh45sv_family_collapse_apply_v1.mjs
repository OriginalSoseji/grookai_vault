...
const PHASE = 'SWSH45SV_FAMILY_COLLAPSE_V1';
...
const EXPECTED = {
  totalUnresolved: 124,
  numericUnresolved: 2,
  svFamilyUnresolved: 122,
  otherNonNumericUnresolved: 0,
  canonicalTargetCount: 122,
  mapCount: 122,
  numericBlockerCount: 2,
  remainingUnresolvedAfterApply: 2,
};
...
async function buildTempCollapseSurface(client) {
  await client.query(`
    drop table if exists tmp_swsh45_all_unresolved;
    drop table if exists tmp_swsh45_family_unresolved;
    drop table if exists tmp_swsh45_canonical_family;
    drop table if exists tmp_swsh45_candidates;
    drop table if exists tmp_swsh45_old_counts;
    drop table if exists tmp_swsh45_new_counts;
    drop table if exists tmp_swsh45_collapse_map;

    create temp table tmp_swsh45_all_unresolved on commit drop as
    select
    ...
    create temp table tmp_swsh45_family_unresolved on commit drop as
    select
      card_print_id as old_id,
      unresolved_name as old_name,
      parent_set_code as old_set_code,
      printed_number as old_number,
      coalesce(
        nullif(ltrim(regexp_replace(printed_number, '[^0-9]', '', 'g'), '0'), ''),
        '0'
      ) as old_number_sort_key,
      normalized_printed_name
    from tmp_swsh45_all_unresolved
    where printed_number ~ '^SV[0-9]+$';

    create temp table tmp_swsh45_canonical_family on commit drop as
    select
      cp.id as new_id,
      cp.name as new_name,
      ...
      coalesce(
        ...
        '0'
      ) as new_number_sort_key,
      cp.gv_id as new_gv_id,
      lower(regexp_replace(btrim(cp.name), '\\s+', ' ', 'g')) as normalized_name
    from public.card_prints cp
    where cp.set_code = '${FAMILY_TARGET_SET_CODE}'
      and cp.gv_id is not null;

    create temp table tmp_swsh45_candidates on commit drop as
    select
      u.old_id,
      c.new_id
    from tmp_swsh45_family_unresolved u
    join tmp_swsh45_canonical_family c
      on c.new_number = u.old_number
     and c.normalized_name = u.normalized_printed_name;

    create temp table tmp_swsh45_old_counts on commit drop as
    select old_id, count(*)::int as match_count
    from tmp_swsh45_candidates
    group by old_id;

    create temp table tmp_swsh45_new_counts on commit drop as
    select new_id, count(*)::int as match_count
    from tmp_swsh45_candidates
    group by new_id;

    create temp table tmp_swsh45_collapse_map on commit drop as
    select
      row_number() over (
        order by u.old_number_sort_key::int, u.old_number, u.old_id
      )::int as seq,
      u.old_id,
      c.new_id,
      u.old_name,
      ...
      u.old_number,
      c.new_number,
      u.old_number_sort_key,
      c.new_number_sort_key,
      u.normalized_printed_name,
      c.normalized_name,
      c.new_gv_id
    from tmp_swsh45_family_unresolved u
    join tmp_swsh45_candidates candidate
      on candidate.old_id = u.old_id
    join tmp_swsh45_canonical_family c
      on c.new_id = candidate.new_id
    join tmp_swsh45_old_counts old_counts
      on old_counts.old_id = candidate.old_id
    join tmp_swsh45_new_counts new_counts
      on new_counts.new_id = candidate.new_id
    where old_counts.match_count = 1
      and new_counts.match_count = 1;

    create unique index tmp_swsh45_collapse_map_old_uidx
      on tmp_swsh45_collapse_map (old_id);

    create unique index tmp_swsh45_collapse_map_new_uidx
      on tmp_swsh45_collapse_map (new_id);
  `);
}
  ...
  return queryOne(
    ...
    `
      ...
      canonical_target as (
        ...
        from tmp_swsh45_canonical_family
      ),
      map_summary as (
        select
        ...
        from tmp_swsh45_collapse_map
      ),
      multiple_old as (
        select count(*)::int as row_count
        from tmp_swsh45_old_counts
        where match_count > 1
      ),
      reused_new as (
        select count(*)::int as row_count
        from tmp_swsh45_new_counts
        where match_count > 1
      ),
      unmatched as (
        select count(*)::int as row_count
        ...
        where not exists (
          select 1
          from tmp_swsh45_collapse_map m
      ...
      same_number_same_name as (
        ...
        where exists (
          select 1
          from tmp_swsh45_canonical_family c
      ...
      select
        total_unresolved,
        numeric_unresolved,
        sv_family_unresolved,
        other_non_numeric_unresolved,
...
function assertPreconditions(summary) {
  ...
  assertEqual(normalizeCount(summary?.sv_family_unresolved), EXPECTED.svFamilyUnresolved, 'SV_FAMILY_UNRESOLVED_DRIFT');
  assertEqual(normalizeCount(summary?.other_non_numeric_unresolved), EXPECTED.otherNonNumericUnresolved, 'OTHER_NON_NUMERIC_UNRESOLVED_DRIFT');
...
async function loadNumericBlockers(client) {
  return queryRows(
    client,
    `
      ...
      order by
        ...
        b.candidate_card_print_id
    `,
    [TARGET_IDENTITY_DOMAIN, SOURCE_SET_CODE_IDENTITY],
  );
}

function assertNumericBlockers(rows) {
  const grouped = new Map();
  for (const row of rows) {
    if (!grouped.has(row.card_print_id)) {
      grouped.set(row.card_print_id, {
        card_print_id: row.card_print_id,
        lawful_base_target_count: normalizeCount(row.lawful_base_target_count),
        collision_types: new Set(),
      });
    }
    grouped.get(row.card_print_id).collision_types.add(row.collision_type);
  }

  const blockers = [...grouped.values()];
  assertEqual(blockers.length, EXPECTED.numericBlockerCount, 'NUMERIC_BLOCKER_COUNT_DRIFT');
  for (const blocker of blockers) {
    assertZero(
      blocker.lawful_base_target_count,
      `NUMERIC_BLOCKER_HAS_LAWFUL_BASE_TARGET:${blocker.card_print_id}`,
    );
    if (
      blocker.collision_types.size !== 1 ||
      !blocker.collision_types.has('same_number_different_name')
    ) {
      throw new Error(
        `NUMERIC_BLOCKER_COLLISION_SHAPE_DRIFT:${blocker.card_print_id}:${JSON.stringify([
          ...blocker.collision_types,
        ])}`,
      );
    }
  }
  ...
  return queryOne(
    ...
    `
      ...
      target_identity as (
        ...
        where card_print_id in (select new_id from tmp_swsh45_collapse_map)
      )
      select
        (
          select count(*)::int
          from public.card_print_traits
          where card_print_id in (select old_id from tmp_swsh45_collapse_map)
      ...
      (
        await queryOne(
          ...
          `
            ...
            where card_id in (select old_id from tmp_swsh45_collapse_map)
          `,
        )
      )?.row_count,
    ),
...
async function applySingleBatch(client) {
  ...
  const insertedTraits = await client.query(`
    ...
    join tmp_swsh45_collapse_map m
  ...
  const deletedTraits = await client.query(`
    ...
    using tmp_swsh45_collapse_map m
...
async function loadPostValidation(client, fkInventory, activeIdentityTotalBefore) {
  const remainingOldReferences = await loadFkCounts(
    client,
    fkInventory,
    `select old_id from tmp_swsh45_collapse_map`,
  );
  ...
  const summary = await queryOne(
    ...
    `
      ...
      route_rows as (
        select count(*)::int as row_count
        from public.card_prints
        where id in (select new_id from tmp_swsh45_collapse_map)
          and gv_id is not null
      ),
      ...
      select
        (
          select count(*)::int
          from public.card_prints
          where id in (select old_id from tmp_swsh45_collapse_map)
        ) as remaining_old_parent_rows,
        (
          select row_count
          from unresolved_after
        ) as remaining_unresolved_null_gvid_rows_for_swsh45,
        (
          select row_count
          from family_unresolved_after
        ) as remaining_family_unresolved_null_gvid_rows_for_swsh45,
        (
          select count(*)::int
          from public.card_prints
          where set_code = $3
    ...
    `,
    [TARGET_IDENTITY_DOMAIN, SOURCE_SET_CODE_IDENTITY, FAMILY_TARGET_SET_CODE, activeIdentityTotalBefore],
  );
...
function assertPostValidation(postValidation, deletedOldParentRows) {
  ...
  assertEqual(normalizeCount(postValidation.summary?.remaining_unresolved_null_gvid_rows_for_swsh45), EXPECTED.remainingUnresolvedAfterApply, 'REMAINING_UNRESOLVED_NULL_GVID_ROWS_FOR_SWSH45_DRIFT');
  assertZero(postValidation.summary?.remaining_family_unresolved_null_gvid_rows_for_swsh45, 'REMAINING_FAMILY_UNRESOLVED_NULL_GVID_ROWS_FOR_SWSH45');
...
async function run() {
  ...
  const report = {
    ...
    target_set_code: FAMILY_TARGET_SET_CODE,
    preconditions: null,
    sample_rows_before: null,
    numeric_blockers: null,
    fk_inventory: null,
  ...
  const client = new Client({
    ...
    application_name: `swsh45sv_family_collapse_apply_v1:${MODE}`,
  });
  ...
  try {
    ...
    report.numeric_blockers = await loadNumericBlockers(client);
    assertNumericBlockers(report.numeric_blockers);

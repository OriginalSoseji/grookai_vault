...
async function loadCanonicalBaseRows(client) {
  ...
  return queryRows(client, sql, [TARGET_SET_CODE_IDENTITY]);
}
...
async function loadPostValidation(client, candidateMap, activeIdentityTotalBefore, activeIdentityOnCandidateBefore) {
  ...
  return {
    ...
    remaining_null_gvid_in_2021swsh: normalizeCount(remainingNullRow?.row_count),
...
function assertPostValidation(postValidation) {
  ...
  if (postValidation.remaining_null_gvid_in_2021swsh !== 0) {
    throw new Error(`REMAINING_NULL_GVID:${postValidation.remaining_null_gvid_in_2021swsh}`);
  }
...
async function run() {
  ...
  const client = new Client({
    ...
    application_name: `2021swsh_numeric_promotion_apply_v1:${MODE}`,
  });

...
const PHASE = 'SM75_ALIAS_REALIGNMENT_COLLAPSE_TO_SM75_V1';
...
const SOURCE_SET_CODE_IDENTITY = 'sm7.5';
const TARGET_SET_CODE = 'sm75';

const EXPECTED = {
  unresolvedCount: 78,
  canonicalTargetCount: 78,
  canonicalMatchCount: 78,
  mapCount: 78,
  batchSize: 100,
};

const TMP = {
  unresolved: 'tmp_sm75_alias_unresolved',
  canonical: 'tmp_sm75_alias_canonical',
  matchAudit: 'tmp_sm75_alias_match_audit',
  collapseMap: 'tmp_sm75_alias_collapse_map',
  batch: 'tmp_sm75_alias_batch',
};
...
const BACKUP_SCHEMA_PATH = path.join(
  process.cwd(),
  'backups',
  'sm75_alias_preapply_schema.sql',
);
const BACKUP_DATA_PATH = path.join(
  process.cwd(),
  'backups',
  'sm75_alias_preapply_data.sql',
);
...
function normalizeMatchName(rawName, canonName = null) {
  const normalized = canonName
    ? normalizeCardNameV1(rawName, { canonName }).corrected_name
    : normalizeCardNameV1(rawName).corrected_name;

  if (!normalized) {
    throw new Error(`NAME_NORMALIZATION_FAILED:${String(rawName ?? 'null')}`);
  }
...
async function buildTempCollapseSurface(client) {
  await client.query(`
    ...
    drop table if exists ${TMP.matchAudit};
    drop table if exists ${TMP.collapseMap};
    drop table if exists ${TMP.batch};
    ...
    create index ${TMP.unresolved}_number_idx
      on ${TMP.unresolved} (old_number);
    ...
    create unique index ${TMP.canonical}_number_uidx
      on ${TMP.canonical} (new_number);

    create temp table ${TMP.matchAudit} (
      old_id uuid primary key,
      old_name text not null,
      old_number text not null,
      old_set_code text null,
      old_match_normalized_name text not null,
    ...
    create temp table ${TMP.collapseMap} (
      seq int not null,
      ...
      old_match_normalized_name text not null,
      new_match_normalized_name text not null,
      exact_db_name_match boolean not null,
      new_gv_id text not null
    ) on commit drop;

    create unique index ${TMP.collapseMap}_seq_uidx
      on ${TMP.collapseMap} (seq);

    create unique index ${TMP.collapseMap}_old_uidx
      on ${TMP.collapseMap} (old_id);

    create unique index ${TMP.collapseMap}_new_uidx
      on ${TMP.collapseMap} (new_id);

    create temp table ${TMP.batch} (
      seq int primary key,
      old_id uuid not null,
      new_id uuid not null
    ) on commit drop;
...
function buildCanonAwareMatchArtifacts(unresolvedRows, canonicalRows) {
  ...
  for (const row of canonicalRows) {
    ...
    bucket.push({
      ...row,
      match_normalized_name: row.new_name.toLowerCase(),
    });
  ...
  for (const row of unresolvedRows) {
    const numberCandidates = canonicalByNumber.get(row.old_number) ?? [];
    const candidates = numberCandidates.filter((candidate) => {
      const corrected = normalizeMatchName(row.old_name, candidate.new_name);
      return corrected === candidate.match_normalized_name;
    });
    const matched = candidates[0] ?? null;
    const matchedNormalizedName = matched
      ? normalizeMatchName(row.old_name, matched.new_name)
      : normalizeMatchName(row.old_name);
    ...
    matchAuditRows.push({
      ...
      old_match_normalized_name: matchedNormalizedName,
    ...
    if (candidates.length === 1) {
      collapseMapRows.push({
        ...
        old_match_normalized_name: matchedNormalizedName,
        new_match_normalized_name: matched.match_normalized_name,
  ...
  const multipleMatchRows = matchAuditRows
    ...
    .slice(0, 25);
  const canonAwareRepairRows = sequencedMapRows
  ...
  return {
    ...
    summary: {
      ...
      canon_aware_name_repair_count: canonAwareRepairRows.length,
      ...
      canon_aware_name_repair_rows: canonAwareRepairRows,
    },
  };
}
...
async function insertMatchAuditRows(client, rows) {
  await client.query(
    `
      insert into ${TMP.matchAudit} (
        old_id,
        old_name,
        old_number,
        old_set_code,
        old_match_normalized_name,
        candidate_count,
        matched_exact_db_name,
        candidate_new_id,
        candidate_new_name,
      ...
      from json_to_recordset($1::json) as x(
        ...
        old_number text,
        old_set_code text,
        old_match_normalized_name text,
        candidate_count int,
  ...
  await client.query(
    `
      ...
      from json_to_recordset($1::json) as x(
        ...
        new_number text,
        old_match_normalized_name text,
        new_match_normalized_name text,
...
async function populateMatchArtifacts(client) {
  ...
  const artifacts = buildCanonAwareMatchArtifacts(unresolvedRows, canonicalRows);
    ...
    `
      ...
      match_summary as (
        select
          ...
          )::int as canon_aware_name_repair_count,
      ...
      select
        ...
        match_summary.canon_aware_name_repair_count,
...
async function prepareBatch(client, nextSeq) {
  await client.query(`truncate table ${TMP.batch}`);

  await client.query(
    `
      insert into ${TMP.batch} (seq, old_id, new_id)
  ...
  const row = await queryOne(client, `select count(*)::int as batch_size from ${TMP.batch}`);
...
async function run() {
  ...
  const report = {
    ...
    preconditions: null,
    canon_aware_mapping_proof: null,
  ...
  const client = new Client({
    ...
    application_name: `sm75_alias_collapse_apply_v1:${MODE}`,
  });
  ...
  try {
    ...
    report.canon_aware_mapping_proof = mappingArtifacts.summary;

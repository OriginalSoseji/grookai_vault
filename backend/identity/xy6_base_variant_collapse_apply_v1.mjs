...
const PHASE = 'XY6_BASE_VARIANT_COLLAPSE_V1';
...
const BLOCKED_OLD_IDS = ['dc8c3dce-bede-47d2-ac8a-095bb633a3ba'];

const EXPECTED = {
  totalUnresolvedCount: 20,
  sourceCount: 19,
  blockedCount: 1,
  unclassifiedCount: 0,
  canonicalTargetCount: 112,
  exactMatchCount: 0,
  sameTokenDifferentNameCount: 19,
  exactUnmatchedCount: 20,
  normalizedMapCount: 19,
  normalizedAmbiguousCount: 0,
  normalizedInvalidCount: 0,
  baseReusedTargetCount: 0,
  distinctOldCount: 19,
  distinctNewCount: 19,
  normalizedNameCount: 19,
  ...
  blockedVaultRows: 0,
  updatedIdentityRows: 19,
  mergedTraitMetadataRows: 0,
  insertedTraits: 19,
  deletedOldTraits: 19,
  mergedPrintingMetadataRows: 57,
  movedUniquePrintings: 0,
  deletedRedundantPrintings: 57,
  updatedExternalMappings: 19,
  updatedVaultItems: 0,
  remainingUnresolvedRows: 1,
  remainingBlockedRows: 1,
  remainingNonBlockedUnresolvedRows: 0,
  targetAnyIdentityRowsAfter: 19,
  targetActiveIdentityRowsAfter: 19,
  targetInactiveIdentityRowsAfter: 0,
  routeResolvableTargetCount: 19,
};
...
async function buildTempBaseVariantSurface(client) {
  await client.query(`
    drop table if exists tmp_xy6_base_unresolved;
    drop table if exists tmp_xy6_base_canonical;
    drop table if exists tmp_xy6_base_exact_audit;
    drop table if exists tmp_xy6_base_match_rows;
    drop table if exists tmp_xy6_base_metrics;
    drop table if exists tmp_xy6_base_classification;
    drop table if exists tmp_xy6_base_collapse_map;

    create temp table tmp_xy6_base_unresolved on commit drop as
    select
      cp.id as old_id,
      cp.name as old_name,
    ...
    create temp table tmp_xy6_base_canonical on commit drop as
    select
      cp.id as new_id,
      cp.name as new_name,
    ...
    create temp table tmp_xy6_base_exact_audit on commit drop as
    select
      u.old_id,
    ...
    from tmp_xy6_base_unresolved u
    left join tmp_xy6_base_canonical c
      on c.new_number = u.source_printed_number
    group by u.old_id, u.source_exact_name_key, u.source_printed_number;

    create temp table tmp_xy6_base_match_rows on commit drop as
    select
      u.old_id,
      u.old_name,
      u.old_set_code,
    ...
    from tmp_xy6_base_unresolved u
    join tmp_xy6_base_exact_audit ea
      on ea.old_id = u.old_id
    left join tmp_xy6_base_match_rows mr
      on mr.old_id = u.old_id
    group by u.old_id, ea.exact_match_count, ea.same_token_different_name_count;

    create temp table tmp_xy6_base_classification on commit drop as
    select
      u.old_id,
      u.old_name,
      u.source_printed_number,
    ...
    from tmp_xy6_base_unresolved u
    join tmp_xy6_base_metrics m
      on m.old_id = u.old_id;

    create temp table tmp_xy6_base_collapse_map on commit drop as
    select
      row_number() over (
        order by
      ...
      mr.*
    from tmp_xy6_base_match_rows mr
    join tmp_xy6_base_classification c
  ...
  return queryOne(
    ...
    `
      with blocked_rows as (
        ...
        from tmp_xy6_base_classification
      ...
      reused_targets as (
        select new_id
        from tmp_xy6_base_collapse_map
        group by new_id
        having count(*) > 1
      )
      select
        (select count(*)::int from tmp_xy6_base_unresolved) as total_unresolved_count,
        (
          select count(*)::int
          from tmp_xy6_base_classification
        ...
        (select count(*)::int from tmp_xy6_base_canonical) as canonical_target_count,
        (
          select count(*)::int
          from tmp_xy6_base_metrics
        ...
        (select count(*)::int from tmp_xy6_base_collapse_map) as normalized_map_count,
        (
          select count(*)::int
          from tmp_xy6_base_metrics m
          join tmp_xy6_base_classification c
        ...
        (select count(distinct old_id)::int from tmp_xy6_base_collapse_map) as distinct_old_count,
        (select count(distinct new_id)::int from tmp_xy6_base_collapse_map) as distinct_new_count,
        (
          select count(*)::int
          from tmp_xy6_base_collapse_map
        ...
        (
          select count(*)::int
          from public.card_print_identity cpi
          where cpi.card_print_id in (select distinct new_id from tmp_xy6_base_collapse_map)
        ) as target_identity_rows_before,
        (
          select count(*)::int
          from public.card_print_identity cpi
          where cpi.is_active = true
            and cpi.card_print_id in (select distinct new_id from tmp_xy6_base_collapse_map)
  ...
  return queryRows(
    ...
    `
      ...
      select
        ...
        m.new_gv_id,
        m.match_category
      from tmp_xy6_base_collapse_map m
      join positions p
        on p.seq = m.seq
      order by m.seq
    `,
    ...
    const row = await queryOne(
      ...
      `
        ...
        where ${quoteIdent(columnName)} in (
          select old_id from tmp_xy6_base_collapse_map
        )
      `,
    );
...
function assertBlockedRowSnapshot(snapshot) {
  ...
  assertEqual(
    normalizeCount(snapshot.printing_rows),
    EXPECTED.blockedPrintingRows,
    'BLOCKED_PRINTING_ROWS_DRIFT',
  );
  assertEqual(
    normalizeCount(snapshot.external_rows),
    EXPECTED.blockedExternalRows,
    'BLOCKED_EXTERNAL_ROWS_DRIFT',
  );
  ...
  return queryOne(
    ...
    `
      with trait_conflicts as (
        ...
        join tmp_xy6_base_collapse_map m
      ...
      select
        (
          select count(*)::int
          from public.card_print_traits t
          where t.card_print_id in (select old_id from tmp_xy6_base_collapse_map)
        ) as old_trait_row_count,
        (
          select count(*)::int
          from public.card_printings p
          where p.card_print_id in (select old_id from tmp_xy6_base_collapse_map)
        ) as old_printing_row_count,
        (
          select count(*)::int
          from public.external_mappings em
          where em.card_print_id in (select old_id from tmp_xy6_base_collapse_map)
        ) as old_external_mapping_row_count,
        (
          select count(*)::int
          from public.vault_items vi
          where vi.card_id in (select old_id from tmp_xy6_base_collapse_map)
...
async function applyCollapse(client) {
  ...
  const activeIdentityConflicts = await queryRows(
    ...
    `
      ...
      where cpi.card_print_id in (
        select distinct new_id from tmp_xy6_base_collapse_map
      )
  ...
  const deletedOldTraits = await client.query(`
    ...
    using tmp_xy6_base_collapse_map m
    ...
    `
      ...
      target_identity as (
        select
          ...
          count(*) filter (where cpi.is_active = false)::int as inactive_identity_rows
        from public.card_print_identity cpi
        where cpi.card_print_id in (select new_id from tmp_xy6_base_collapse_map)
      ),
      ...
      route_resolvable as (
        select count(*)::int as row_count
        from public.card_prints cp
        where cp.id in (select new_id from tmp_xy6_base_collapse_map)
          and cp.gv_id is not null
      ),
      ...
      select
        ...
        (
          select count(*)::int
          from public.card_prints cp
          where cp.id in (select old_id from tmp_xy6_base_collapse_map)
...
function assertPostValidation(postValidation, deletedOldParentRows) {
  ...
  assertEqual(
    normalizeCount(postValidation.summary?.remaining_non_blocked_unresolved_rows),
    EXPECTED.remainingNonBlockedUnresolvedRows,
    'REMAINING_NON_BLOCKED_UNRESOLVED_ROWS_DRIFT',
  );
  assertIdSetEqual(
...
async function run() {
  ...
  const client = new Client({
    ...
    application_name: `xy6_base_variant_collapse_apply_v1:${MODE}`,
  });

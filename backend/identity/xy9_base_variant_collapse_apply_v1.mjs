...
const PHASE = 'XY9_BASE_VARIANT_COLLAPSE_V1';
...
const SOURCE_SET_CODE_IDENTITY = 'xy9';
const TARGET_SET_CODE = 'xy9';
const CLASSIFICATION = 'BASE_VARIANT_COLLAPSE';
const BLOCKED_OLD_IDS = ['a6d34131-d056-49ae-a8b7-21d808e351f6'];

const EXPECTED = {
  totalUnresolvedCount: 21,
  sourceCount: 20,
  blockedCount: 1,
  unclassifiedCount: 0,
  canonicalTargetCount: 125,
  exactMatchCount: 0,
  sameTokenDifferentNameCount: 20,
  exactUnmatchedCount: 21,
  ...
  printingFinishConflictCount: 60,
  printingMergeableMetadataOnlyCount: 60,
  ...
  insertedTraits: 20,
  deletedOldTraits: 20,
  mergedPrintingMetadataRows: 60,
  movedUniquePrintings: 0,
  deletedRedundantPrintings: 60,
  ...
  return queryOne(
    ...
    `
      with blocked_rows as (
        ...
        from tmp_xy9_base_classification
      ...
      select
        ...
        (select count(*)::int from tmp_xy9_base_canonical) as canonical_target_count,
        (
          select count(*)::int
          from tmp_xy9_base_metrics
        ...
        (select count(distinct old_id)::int from tmp_xy9_base_collapse_map) as distinct_old_count,
        (select count(distinct new_id)::int from tmp_xy9_base_collapse_map) as distinct_new_count,
        (
          select count(*)::int
          from tmp_xy9_base_collapse_map
        ...
        (
          select count(*)::int
          from public.card_print_identity cpi
          where cpi.card_print_id in (select distinct new_id from tmp_xy9_base_collapse_map)
        ) as target_identity_rows_before,
        (
          select count(*)::int
          from public.card_print_identity cpi
          where cpi.is_active = true
            and cpi.card_print_id in (select distinct new_id from tmp_xy9_base_collapse_map)
  ...
  return queryRows(
    ...
    `
      ...
      select
        ...
        m.new_gv_id,
        m.match_category
      from tmp_xy9_base_collapse_map m
      join positions p
        on p.seq = m.seq
      order by m.seq
    `,
    ...
    const row = await queryOne(
      ...
      `
        ...
        where ${quoteIdent(columnName)} in (
          select old_id from tmp_xy9_base_collapse_map
        )
      `,
    );
  ...
  return queryOne(
    ...
    `
      with trait_conflicts as (
        ...
        join tmp_xy9_base_collapse_map m
      ...
      select
        (
          select count(*)::int
          from public.card_print_traits t
          where t.card_print_id in (select old_id from tmp_xy9_base_collapse_map)
        ) as old_trait_row_count,
        (
          select count(*)::int
          from public.card_printings p
          where p.card_print_id in (select old_id from tmp_xy9_base_collapse_map)
        ) as old_printing_row_count,
        (
          select count(*)::int
          from public.external_mappings em
          where em.card_print_id in (select old_id from tmp_xy9_base_collapse_map)
        ) as old_external_mapping_row_count,
        (
          select count(*)::int
          from public.vault_items vi
          where vi.card_id in (select old_id from tmp_xy9_base_collapse_map)
...
async function applyCollapse(client) {
  ...
  const activeIdentityConflicts = await queryRows(
    ...
    `
      ...
      where cpi.card_print_id in (
        select distinct new_id from tmp_xy9_base_collapse_map
      )
  ...
  const deletedOldTraits = await client.query(`
    ...
    using tmp_xy9_base_collapse_map m
    ...
    `
      ...
      target_identity as (
        select
          ...
          count(*) filter (where cpi.is_active = false)::int as inactive_identity_rows
        from public.card_print_identity cpi
        where cpi.card_print_id in (select new_id from tmp_xy9_base_collapse_map)
      ),
      ...
      route_resolvable as (
        select count(*)::int as row_count
        from public.card_prints cp
        where cp.id in (select new_id from tmp_xy9_base_collapse_map)
          and cp.gv_id is not null
      ),
      ...
      select
        ...
        (
          select count(*)::int
          from public.card_prints cp
          where cp.id in (select old_id from tmp_xy9_base_collapse_map)
...
async function run() {
  ...
  const client = new Client({
    ...
    application_name: `xy9_base_variant_collapse_apply_v1:${MODE}`,
  });

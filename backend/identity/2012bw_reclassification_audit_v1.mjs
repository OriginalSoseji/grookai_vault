...
const PHASE = 'RECLASSIFICATION_AUDIT_FOR_2012BW_TO_MCD12_V1';
...
const JSON_OUTPUT_PATH = path.join(
  ...
  'docs',
  'checkpoints',
  '2012bw_reclassification_audit_v1.json',
);
...
function classify(report) {
  if (
    ...
    return {
      final_classification: 'OUTCOME A — DUPLICATE_COLLAPSE_TO_MCD12',
      next_phase_recommendation: '2012BW_ALIAS_COLLAPSE_TO_MCD12',
    };
  }
  ...
  if (normalizeCount(report.strict_mapping_results.mapping_candidate_count) > 0) {
    return {
      ...
      next_phase_recommendation: 'PARTIAL_ALIAS_COLLAPSE_REVIEW_FOR_2012BW_AND_MCD12',
    };
  }

  if (normalizeCount(report.canonical_mcd12_target_summary.canonical_mcd12_total_rows) === 0) {
    return {
      final_classification: 'OUTCOME C — STILL_NUMERIC_PROMOTION',
      next_phase_recommendation: 'RECHECK_NUMERIC_PROMOTION_FOR_2012BW',
    };
  }

  return {
    final_classification: 'OUTCOME D — BLOCKED_CONFLICT',
    next_phase_recommendation: 'BLOCKED_REVIEW_FOR_2012BW_AND_MCD12',
  };
}
...
async function run() {
  ...
  const client = new Client({
    ...
    application_name: '2012bw_reclassification_audit_v1',
  });
  ...
  const report = {
    ...
    non_numeric_unresolved: 0,
    canonical_mcd12_target_summary: null,
    canonical_mcd12_sample_rows: [],
  ...
  try {
    ...
    report.canonical_mcd12_target_summary = await queryOne(client, SQL.canonicalSummary, [TARGET_SET_CODE]);
    report.canonical_mcd12_sample_rows = await queryRows(client, SQL.canonicalSamples, [TARGET_SET_CODE]);

    if (normalizeCount(report.canonical_mcd12_target_summary?.canonical_mcd12_total_rows) <= 0) {
      throw new Error('CANONICAL_MCD12_LANE_MISSING');
    }
    if (
      normalizeCount(report.canonical_mcd12_target_summary?.canonical_mcd12_non_null_gvid_count) !==
      normalizeCount(report.canonical_mcd12_target_summary?.canonical_mcd12_total_rows)
    ) {
      throw new Error(
        `CANONICAL_MCD12_GVID_DRIFT:${report.canonical_mcd12_target_summary?.canonical_mcd12_non_null_gvid_count}:${report.canonical_mcd12_target_summary?.canonical_mcd12_total_rows}`,
      );
    }
    ...
    writeJsonReport({
      ...
      canonical_mcd12_total_rows: normalizeCount(
        report.canonical_mcd12_target_summary?.canonical_mcd12_total_rows,
      ),
      ...
      canonical_mcd12_sample_rows: report.canonical_mcd12_sample_rows,
    ...
    console.log(
      JSON.stringify(
        {
          ...report,
          canonical_mcd12_target_summary: {
            canonical_mcd12_total_rows: normalizeCount(
              report.canonical_mcd12_target_summary?.canonical_mcd12_total_rows,
            ),
            canonical_mcd12_non_null_gvid_count: normalizeCount(
              report.canonical_mcd12_target_summary?.canonical_mcd12_non_null_gvid_count,
            ),
          },
        },
        null,

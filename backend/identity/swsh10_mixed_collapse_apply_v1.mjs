...
const PHASE = 'SWSH10_MIXED_COLLAPSE_V1';
const MODE = process.argv.includes('--apply') ? 'apply' : 'dry-run';
const BATCH_SIZE = 100;

const TARGET_IDENTITY_DOMAIN = 'pokemon_eng_standard';
const SOURCE_SET_CODE_IDENTITY = 'swsh10';
const NUMERIC_TARGET_SET_CODE = 'swsh10';
const TG_TARGET_SET_CODE = 'swsh10tg';

const EXPECTED = {
  totalUnresolved: 128,
  numericUnresolved: 98,
  tgUnresolved: 30,
  numericMapCount: 98,
  tgMapCount: 30,
  combinedMapCount: 128,
  combinedDistinctNewCount: 128,
  canonicalNumericTargetCount: 216,
...
async function buildTempCollapseSurface(client) {
  await client.query(`
    drop table if exists tmp_swsh10_unresolved;
    drop table if exists tmp_swsh10_canonical_numeric;
    drop table if exists tmp_swsh10_canonical_tg;
    drop table if exists tmp_swsh10_numeric_candidates;
    drop table if exists tmp_swsh10_numeric_old_counts;
    drop table if exists tmp_swsh10_numeric_new_counts;
    drop table if exists tmp_swsh10_numeric_map;
    drop table if exists tmp_swsh10_tg_candidates;
    drop table if exists tmp_swsh10_tg_old_counts;
    drop table if exists tmp_swsh10_tg_new_counts;
    drop table if exists tmp_swsh10_tg_map;
    drop table if exists tmp_swsh10_collapse_map;
    drop table if exists tmp_swsh10_batch;

    create temp table tmp_swsh10_unresolved on commit drop as
    select
      cp.id as old_id,
      cp.name as old_name,
    ...
    create index tmp_swsh10_unresolved_lane_idx
      on tmp_swsh10_unresolved (lane, old_number, normalized_printed_name);

    create temp table tmp_swsh10_canonical_numeric on commit drop as
    select
      cp.id as new_id,
      cp.name as new_name,
    ...
    create index tmp_swsh10_canonical_numeric_match_idx
      on tmp_swsh10_canonical_numeric (normalized_digits, normalized_name);

    create temp table tmp_swsh10_canonical_tg on commit drop as
    select
      cp.id as new_id,
      cp.name as new_name,
      cp.set_code as new_set_code,
    ...
    create index tmp_swsh10_canonical_tg_match_idx
      on tmp_swsh10_canonical_tg (new_number, normalized_name);

    create temp table tmp_swsh10_numeric_candidates on commit drop as
    select
      u.old_id,
      c.new_id
    from tmp_swsh10_unresolved u
    join tmp_swsh10_canonical_numeric c
      on u.lane = 'numeric'
     and c.normalized_digits = u.normalized_digits
     and c.normalized_name = u.normalized_printed_name;

    create temp table tmp_swsh10_numeric_old_counts on commit drop as
    select old_id, count(*)::int as match_count
    from tmp_swsh10_numeric_candidates
    group by old_id;

    create temp table tmp_swsh10_numeric_new_counts on commit drop as
    select new_id, count(*)::int as match_count
    from tmp_swsh10_numeric_candidates
    group by new_id;

    create temp table tmp_swsh10_numeric_map on commit drop as
    select
      ...
      c.new_gv_id
    from tmp_swsh10_unresolved u
    join tmp_swsh10_numeric_candidates candidate
      on candidate.old_id = u.old_id
    join tmp_swsh10_canonical_numeric c
      on c.new_id = candidate.new_id
    join tmp_swsh10_numeric_old_counts old_counts
      on old_counts.old_id = candidate.old_id
    join tmp_swsh10_numeric_new_counts new_counts
      on new_counts.new_id = candidate.new_id
    where u.lane = 'numeric'
      and old_counts.match_count = 1
      and new_counts.match_count = 1;

    create unique index tmp_swsh10_numeric_map_old_uidx
      on tmp_swsh10_numeric_map (old_id);

    create unique index tmp_swsh10_numeric_map_new_uidx
      on tmp_swsh10_numeric_map (new_id);

    create temp table tmp_swsh10_tg_candidates on commit drop as
    select
      u.old_id,
      c.new_id
    from tmp_swsh10_unresolved u
    join tmp_swsh10_canonical_tg c
      on u.lane = 'tg'
     and c.new_number = u.old_number
     and c.normalized_name = u.normalized_printed_name;

    create temp table tmp_swsh10_tg_old_counts on commit drop as
    select old_id, count(*)::int as match_count
    from tmp_swsh10_tg_candidates
    group by old_id;

    create temp table tmp_swsh10_tg_new_counts on commit drop as
    select new_id, count(*)::int as match_count
    from tmp_swsh10_tg_candidates
    group by new_id;

    create temp table tmp_swsh10_tg_map on commit drop as
    select
      'tg'::text as lane,
      u.old_id,
      c.new_id,
      ...
      c.new_gv_id
    from tmp_swsh10_unresolved u
    join tmp_swsh10_tg_candidates candidate
      on candidate.old_id = u.old_id
    join tmp_swsh10_canonical_tg c
      on c.new_id = candidate.new_id
    join tmp_swsh10_tg_old_counts old_counts
      on old_counts.old_id = candidate.old_id
    join tmp_swsh10_tg_new_counts new_counts
      on new_counts.new_id = candidate.new_id
    where u.lane = 'tg'
      and old_counts.match_count = 1
      and new_counts.match_count = 1;

    create unique index tmp_swsh10_tg_map_old_uidx
      on tmp_swsh10_tg_map (old_id);

    create unique index tmp_swsh10_tg_map_new_uidx
      on tmp_swsh10_tg_map (new_id);

    create temp table tmp_swsh10_collapse_map on commit drop as
    select
      row_number() over (
        order by
      ...
      new_gv_id
    from (
      select * from tmp_swsh10_numeric_map
      union all
      select * from tmp_swsh10_tg_map
    ) unioned;

    create unique index tmp_swsh10_collapse_map_old_uidx
      on tmp_swsh10_collapse_map (old_id);

    create unique index tmp_swsh10_collapse_map_new_uidx
      on tmp_swsh10_collapse_map (new_id);

    create temp table tmp_swsh10_batch (
      seq int primary key,
      lane text not null,
  ...
  const row = await queryOne(
    ...
    `
      ...
      tg_summary as (
        ...
        from tmp_swsh10_tg_map
      ),
      numeric_multiple_old as (
        select count(*)::int as row_count
        from tmp_swsh10_numeric_old_counts
        where match_count > 1
      ),
      numeric_reused_new as (
        select count(*)::int as row_count
        from tmp_swsh10_numeric_new_counts
        where match_count > 1
      ),
      tg_multiple_old as (
        select count(*)::int as row_count
        from tmp_swsh10_tg_old_counts
        where match_count > 1
      ),
      tg_reused_new as (
        select count(*)::int as row_count
        from tmp_swsh10_tg_new_counts
        where match_count > 1
      ),
      ...
      target_identity_occupancy as (
        ...
        from tmp_swsh10_collapse_map m
  ...
  return queryOne(
    ...
    `
      with traits_on_old as (
        ...
        where t.card_print_id in (select old_id from tmp_swsh10_collapse_map)
      ),
      ...
      printing_on_old as (
        ...
        where p.card_print_id in (select old_id from tmp_swsh10_collapse_map)
      ),
      ...
      mapping_on_old as (
        ...
        where em.card_print_id in (select old_id from tmp_swsh10_collapse_map)
      ),
      ...
      target_identity as (
        select count(*)::int as row_count
        from public.card_print_identity cpi
        where cpi.card_print_id in (select new_id from tmp_swsh10_collapse_map)
      )
      select
...
async function prepareBatch(client, nextSeq) {
  await client.query(`truncate table tmp_swsh10_batch`);

  await client.query(
    `
      insert into tmp_swsh10_batch (seq, lane, old_id, new_id)
  ...
  const row = await queryOne(client, `select count(*)::int as batch_size from tmp_swsh10_batch`);
    ...
    const row = await queryOne(
      ...
      `
        ...
        where ${quoteIdent(column)} in (
          select old_id from tmp_swsh10_batch
        )
      `,
    );
...
async function loadPostValidation(client, fkInventory) {
  const remainingOldReferences = await loadFkCounts(
    client,
    fkInventory,
    `select old_id from tmp_swsh10_collapse_map`,
  );
    ...
    `
      ...
      select
        (
          select count(*)::int
          from public.card_prints cp
          where cp.id in (select old_id from tmp_swsh10_collapse_map)
...
async function run() {
  ...
  const client = new Client({
    ...
    application_name: `swsh10_mixed_collapse_apply_v1:${MODE}`,
  });
  ...
  try {
    ...
    const deletedParents = await client.query(`
      ...
      using tmp_swsh10_collapse_map m

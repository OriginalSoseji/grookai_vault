...
const PHASE = 'SM4_ACTIVE_IDENTITY_FANIN_RESOLUTION_V1';
...
const SOURCE_SET_CODE_IDENTITY = 'sm4';
const TARGET_SET_CODE = 'sm4';

const EXPECTED = {
  unresolvedCount: 25,
  canonicalTargetCount: 124,
  exactMatchCount: 0,
  sameTokenDifferentNameCount: 24,
  exactUnmatchedCount: 25,
  classification: 'BASE_VARIANT_COLLAPSE',
  mapCount: 25,
  distinctNewCount: 24,
  baseReusedTargetCount: 1,
  normalizedNameCount: 24,
  suffixVariantCount: 1,
  fanInGroupCount: 1,
  archivedIdentityCount: 1,
  selectedActiveIdentityCount: 24,
};
...
const BACKUP_SCHEMA_PATH = path.join(process.cwd(), 'backups', 'sm4_fanin_preapply_schema.sql');
const BACKUP_DATA_PATH = path.join(process.cwd(), 'backups', 'sm4_fanin_preapply_data.sql');
...
async function buildTempCollapseSurface(client) {
  await client.query(`
    drop table if exists tmp_sm4_unresolved;
    drop table if exists tmp_sm4_canonical;
    drop table if exists tmp_sm4_match_audit;
    drop table if exists tmp_sm4_collapse_map;
    drop table if exists tmp_sm4_fanin_resolution;

    create temp table tmp_sm4_unresolved on commit drop as
    select
      cp.id as old_id,
      cp.name as old_name,
    ...
    create index tmp_sm4_unresolved_token_idx
      on tmp_sm4_unresolved (source_printed_number);

    create temp table tmp_sm4_canonical on commit drop as
    select
      cp.id as new_id,
      cp.name as new_name,
    ...
    create index tmp_sm4_canonical_token_idx
      on tmp_sm4_canonical (new_number);

    create index tmp_sm4_canonical_number_plain_idx
      on tmp_sm4_canonical (new_number_plain);

    create temp table tmp_sm4_match_audit (
    ...
    create temp table tmp_sm4_collapse_map (
      seq int not null,
    ...
    create unique index tmp_sm4_collapse_map_seq_uidx
      on tmp_sm4_collapse_map (seq);

    create unique index tmp_sm4_collapse_map_old_uidx
      on tmp_sm4_collapse_map (old_id);

    create temp table tmp_sm4_fanin_resolution (
    ...
    create unique index tmp_sm4_fanin_resolution_identity_uidx
      on tmp_sm4_fanin_resolution (identity_id);
...
async function insertMatchAuditRows(client, rows) {
  await client.query(
    `
      insert into tmp_sm4_match_audit (
        old_id,
        old_name,
        old_set_code,
        old_parent_number,
...
async function insertCollapseMapRows(client, rows) {
  await client.query(
    `
      insert into tmp_sm4_collapse_map (
        seq,
        old_id,
        new_id,
        old_name,
...
async function insertFanInResolutionRows(client, rows) {
  ...
  await client.query(
    `
      insert into tmp_sm4_fanin_resolution (
...
async function buildFanInResolution(client) {
  ...
  const identityRows = await queryRows(
    ...
    `
      ...
      where cpi.card_print_id in (
        select old_id from tmp_sm4_collapse_map
      )
      order by cpi.card_print_id, cpi.is_active desc, cpi.id
    `,
  );

  const activeIdentityByOldId = new Map();
  for (const row of identityRows) {
    if (!row.is_active) {
      continue;
    }
    if (activeIdentityByOldId.has(row.old_id)) {
      throw new Error(`MULTIPLE_ACTIVE_IDENTITIES_PER_OLD_PARENT:${row.old_id}`);
    }
    activeIdentityByOldId.set(row.old_id, row);
  }
  ...
  for (const [newId, rows] of collapseRowsByNewId.entries()) {
    ...
    const candidates = rows.map((row) => {
      const identityRow = activeIdentityByOldId.get(row.old_id);
      if (!identityRow) {
  ...
  return {
    ...
    selected_active_identity_count:
      collapseMapRows.length -
      fanInResolutionRows.filter((row) => row.resolution_action === 'archive_history').length,
...
async function loadCollapseMapSamples(client) {
  const samples = await queryRows(
    ...
    `
      ...
      from tmp_sm4_collapse_map m
      left join tmp_sm4_fanin_resolution fan_in
  ...
  return queryOne(
    ...
    `
      with traits_on_old as (
        ...
        where t.card_print_id in (select old_id from tmp_sm4_collapse_map)
      ),
      ...
      printing_on_old as (
        ...
        where p.card_print_id in (select old_id from tmp_sm4_collapse_map)
      ),
      ...
      mappings_on_old as (
        ...
        where em.card_print_id in (select old_id from tmp_sm4_collapse_map)
      ),
      ...
      target_identity as (
        select count(*)::int as row_count
        from public.card_print_identity cpi
        where cpi.card_print_id in (select new_id from tmp_sm4_collapse_map)
      ),
      vault_on_old as (
        select count(*)::int as row_count
        from public.vault_items vi
        where vi.card_id in (select old_id from tmp_sm4_collapse_map)
      )
      select
...
async function applyCollapse(client) {
  ...
  const archivedIdentityRows = await client.query(`
    ...
    set
      is_active = false,
      updated_at = now()
    from tmp_sm4_fanin_resolution fan_in
  ...
  const activeIdentityConflicts = await queryRows(
    ...
    `
      ...
      from public.card_print_identity cpi
      where cpi.card_print_id in (
        select distinct new_id from tmp_sm4_collapse_map
      )
  ...
  const summary = await queryOne(
    ...
    `
      ...
      route_resolvable as (
        select count(*)::int as row_count
        from public.card_prints cp
        where cp.id in (select new_id from tmp_sm4_collapse_map)
          and cp.gv_id is not null
      ),
      ...
      select
        (
          select count(*)::int
          from public.card_prints cp
          where cp.id in (select old_id from tmp_sm4_collapse_map)
...
async function run() {
  ...
  const client = new Client({
    ...
    application_name: `sm4_fanin_resolution_apply_v1:${MODE}`,
  });

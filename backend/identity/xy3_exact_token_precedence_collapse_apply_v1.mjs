...
const PHASE = 'XY3_EXACT_TOKEN_PRECEDENCE_COLLAPSE_V1';
...
const SOURCE_OLD_ID = '696cf830-c004-4fcf-9284-00e4e39eaf25';
const EXACT_TARGET_NEW_ID = '2bc6a250-d786-4719-8a7f-9063489e5d73';
const SUFFIX_TARGET_ID = '5862d23e-8526-4055-baf2-cc478ce02ea9';
const CHOSEN_TARGET_GV_ID = 'GV-PK-FFI-55';
const SUFFIX_TARGET_GV_ID = 'GV-PK-FFI-55A';

const TMP = {
  source: 'tmp_xy3_exact_source',
  candidates: 'tmp_xy3_exact_candidates',
  metrics: 'tmp_xy3_exact_metrics',
  collapseMap: 'tmp_xy3_exact_collapse_map',
};
...
const EXPECTED = {
  ...
  exactTargetTraitRowsBefore: 1,
  exactTargetPrintingRowsBefore: 3,
  exactTargetExternalRowsBefore: 4,
...
function assertSnapshotUnchanged(before, after, code) {
  const keys = [
    'id',
    'name',
    'number',
  ...
  for (const key of keys) {
    if (String(before?.[key] ?? '') !== String(after?.[key] ?? '')) {
      throw new Error(`${code}:${key}:${before?.[key]}:${after?.[key]}`);
    }
  }
}
...
async function buildTempExactTokenSurface(client) {
  await client.query(`
    drop table if exists ${TMP.source};
    drop table if exists ${TMP.candidates};
    drop table if exists ${TMP.metrics};
    drop table if exists ${TMP.collapseMap};

    create temp table ${TMP.source} on commit drop as
    select
      cp.id as old_id,
      cp.name as old_name,
    ...
    create temp table ${TMP.candidates} on commit drop as
    select
      s.old_id,
      s.old_name,
      s.old_parent_number,
    ...
    select
      ...
      c.new_gv_id
    from ${TMP.source} s
    join ${TMP.candidates} c
      on c.old_id = s.old_id
     and c.match_type = 'exact_token';

    create unique index tmp_xy3_exact_collapse_map_old_uidx
      on ${TMP.collapseMap} (old_id);

    create unique index tmp_xy3_exact_collapse_map_new_uidx
      on ${TMP.collapseMap} (new_id);
  `);
}
...
function assertSourceSurface(source) {
  ...
  if (source.old_name !== 'M Lucario EX') {
  ...
  if (String(source.effective_number_plain) !== '55') {
    throw new Error(`SOURCE_NUMBER_PLAIN_DRIFT:${source.effective_number_plain}:55`);
  }
  if (source.source_printed_number !== '55') {
    throw new Error(`SOURCE_PRINTED_TOKEN_DRIFT:${source.source_printed_number}:55`);
  }
  ...
  return queryOne(
    ...
    `
      ...
      select
        ...
        (select count(*)::int from ${TMP.source}) as source_count,
        (select count(*)::int from ${TMP.candidates}) as candidate_count,
        (select count(*)::int from ${TMP.metrics} where candidate_count = 0) as unmatched_count,
        (select count(*)::int from reused_targets) as reused_target_count,
        (select count(*)::int from ${TMP.metrics} where exact_token_candidate_count = 1) as exact_token_candidate_count,
        (select count(*)::int from ${TMP.metrics} where suffix_candidate_count = 1) as suffix_candidate_count,
        (select count(*)::int from ${TMP.metrics} where additional_candidate_count > 0) as additional_candidate_count,
        (select count(*)::int from ${TMP.collapseMap}) as chosen_target_count,
        (
          select count(*)::int
        ...
        (
          ...
          where card_print_id in (select distinct new_id from ${TMP.collapseMap})
        ) as target_identity_rows_before,
        (
          select count(*)::int
          from public.card_print_identity
          where is_active = true
            and card_print_id in (select distinct new_id from ${TMP.collapseMap})
...
function assertPreconditions(summary) {
  ...
  assertEqual(normalizeCount(summary?.exact_token_candidate_count), EXPECTED.exactTokenCandidateCount, 'EXACT_TOKEN_CANDIDATE_COUNT_DRIFT');
  assertEqual(normalizeCount(summary?.suffix_candidate_count), EXPECTED.suffixCandidateCount, 'SUFFIX_CANDIDATE_COUNT_DRIFT');
  assertEqual(normalizeCount(summary?.additional_candidate_count), EXPECTED.additionalCandidateCount, 'ADDITIONAL_CANDIDATE_COUNT_DRIFT');
  assertEqual(normalizeCount(summary?.unmatched_count), EXPECTED.unmatchedCount, 'UNMATCHED_COUNT_DRIFT');
  assertEqual(normalizeCount(summary?.reused_target_count), EXPECTED.reusedTargetCount, 'REUSED_TARGET_COUNT_DRIFT');
  assertEqual(normalizeCount(summary?.chosen_target_count), EXPECTED.chosenTargetCount, 'CHOSEN_TARGET_COUNT_DRIFT');
  ...
  assertIdSetEqual(summary?.candidate_new_ids ?? [], [EXACT_TARGET_NEW_ID, SUFFIX_TARGET_ID], 'CANDIDATE_NEW_IDS_DRIFT');
...
function assertSourceSnapshot(snapshot) {
  if (!snapshot) {
    throw new Error(`SOURCE_SNAPSHOT_MISSING:${SOURCE_OLD_ID}`);
  }
  assertEqual(snapshot.id, SOURCE_OLD_ID, 'SOURCE_SNAPSHOT_ID_DRIFT');
  assertEqual(snapshot.name, 'M Lucario EX', 'SOURCE_SNAPSHOT_NAME_DRIFT');
  assertEqual(snapshot.number, null, 'SOURCE_SNAPSHOT_NUMBER_DRIFT');
  assertEqual(snapshot.number_plain, null, 'SOURCE_SNAPSHOT_NUMBER_PLAIN_DRIFT');
  assertEqual(snapshot.variant_key, '', 'SOURCE_SNAPSHOT_VARIANT_KEY_DRIFT');
  assertEqual(snapshot.gv_id, null, 'SOURCE_SNAPSHOT_GVID_DRIFT');
  assertEqual(normalizeCount(snapshot.identity_rows), 1, 'SOURCE_IDENTITY_ROWS_DRIFT');
  assertEqual(normalizeCount(snapshot.active_identity_rows), 1, 'SOURCE_ACTIVE_IDENTITY_ROWS_DRIFT');
  assertEqual(normalizeCount(snapshot.inactive_identity_rows), 0, 'SOURCE_INACTIVE_IDENTITY_ROWS_DRIFT');
  assertEqual(normalizeCount(snapshot.trait_rows), EXPECTED.oldTraitRowCount, 'SOURCE_TRAIT_ROWS_DRIFT');
  assertEqual(normalizeCount(snapshot.printing_rows), EXPECTED.oldPrintingRowCount, 'SOURCE_PRINTING_ROWS_DRIFT');
  assertEqual(normalizeCount(snapshot.external_rows), EXPECTED.oldExternalMappingRowCount, 'SOURCE_EXTERNAL_ROWS_DRIFT');
  assertEqual(normalizeCount(snapshot.vault_rows), EXPECTED.oldVaultItemRowCount, 'SOURCE_VAULT_ROWS_DRIFT');
}

function assertExactTargetBefore(snapshot) {
  if (!snapshot) {
    throw new Error(`EXACT_TARGET_SNAPSHOT_MISSING:${EXACT_TARGET_NEW_ID}`);
  }
  assertEqual(snapshot.id, EXACT_TARGET_NEW_ID, 'EXACT_TARGET_ID_DRIFT');
  assertEqual(snapshot.name, 'M Lucario-EX', 'EXACT_TARGET_NAME_DRIFT');
  assertEqual(snapshot.number, '55', 'EXACT_TARGET_NUMBER_DRIFT');
  assertEqual(snapshot.number_plain, '55', 'EXACT_TARGET_NUMBER_PLAIN_DRIFT');
  assertEqual(snapshot.variant_key, '', 'EXACT_TARGET_VARIANT_KEY_DRIFT');
  assertEqual(snapshot.gv_id, CHOSEN_TARGET_GV_ID, 'EXACT_TARGET_GVID_DRIFT');
  assertEqual(normalizeCount(snapshot.identity_rows), EXPECTED.targetIdentityRowsBefore, 'EXACT_TARGET_IDENTITY_ROWS_DRIFT');
  assertEqual(normalizeCount(snapshot.active_identity_rows), EXPECTED.targetActiveIdentityRowsBefore, 'EXACT_TARGET_ACTIVE_IDENTITY_ROWS_DRIFT');
  assertEqual(normalizeCount(snapshot.inactive_identity_rows), 0, 'EXACT_TARGET_INACTIVE_IDENTITY_ROWS_DRIFT');
  assertEqual(normalizeCount(snapshot.trait_rows), EXPECTED.exactTargetTraitRowsBefore, 'EXACT_TARGET_TRAIT_ROWS_DRIFT');
  assertEqual(normalizeCount(snapshot.printing_rows), EXPECTED.exactTargetPrintingRowsBefore, 'EXACT_TARGET_PRINTING_ROWS_DRIFT');
  assertEqual(normalizeCount(snapshot.external_rows), EXPECTED.exactTargetExternalRowsBefore, 'EXACT_TARGET_EXTERNAL_ROWS_DRIFT');
  assertEqual(normalizeCount(snapshot.vault_rows), 0, 'EXACT_TARGET_VAULT_ROWS_DRIFT');
}

function assertSuffixTargetSnapshot(snapshot) {
  if (!snapshot) {
    throw new Error(`SUFFIX_TARGET_SNAPSHOT_MISSING:${SUFFIX_TARGET_ID}`);
  }
  assertEqual(snapshot.id, SUFFIX_TARGET_ID, 'SUFFIX_TARGET_ID_DRIFT');
  assertEqual(snapshot.name, 'M Lucario EX', 'SUFFIX_TARGET_NAME_DRIFT');
  assertEqual(snapshot.number, '55a', 'SUFFIX_TARGET_NUMBER_DRIFT');
  assertEqual(snapshot.number_plain, '55', 'SUFFIX_TARGET_NUMBER_PLAIN_DRIFT');
  assertEqual(snapshot.variant_key, 'a', 'SUFFIX_TARGET_VARIANT_KEY_DRIFT');
  assertEqual(snapshot.gv_id, SUFFIX_TARGET_GV_ID, 'SUFFIX_TARGET_GVID_DRIFT');
...
async function loadCollisionSummary(client) {
  return queryOne(
    client,
    `
      with trait_overlap as (
        select old_t.*, new_t.id as new_trait_id
      ...
      external_overlap as (
        ...
        from public.external_mappings em
        join ${TMP.collapseMap} m
          on m.old_id = em.card_print_id
        join public.external_mappings em2
          on em2.card_print_id = m.new_id
         and em2.source = em.source
         and em2.external_id = em.external_id
      )
      select
        (select count(*)::int from trait_overlap) as trait_target_key_conflict_count,
        (select count(*)::int from trait_overlap where false) as trait_mergeable_metadata_only_count,
        (select count(*)::int from trait_overlap where false) as trait_conflicting_non_identical_count,
        (select count(*)::int from printing_overlap) as printing_finish_conflict_count,
        (
          select count(*)::int
          from printing_overlap
          where (new_provenance_source is null and old_provenance_source is not null)
             or (new_provenance_ref is null and old_provenance_ref is not null)
             or (new_created_by is null and old_created_by is not null)
        ) as printing_mergeable_metadata_only_count,
        (
          select count(*)::int
          from printing_overlap
          where (new_provenance_source is not null and old_provenance_source is not null and new_provenance_source <> old_provenance_source)
             or (new_provenance_ref is not null and old_provenance_ref is not null and new_provenance_ref <> old_provenance_ref)
             or (new_created_by is not null and old_created_by is not null and new_created_by <> old_created_by)
        ) as printing_conflicting_non_identical_count,
        (select count(*)::int from external_overlap) as external_mapping_conflict_count
    `,
  );
}
...
async function run() {
  ...
  const client = new Client({
    ...
    application_name: `xy3_exact_token_precedence_collapse_apply_v1:${MODE}`,
  });
  ...
  try {
    ...
    assertSourceSnapshot(report.source_snapshot_before);
    ...
    assertSnapshotUnchanged(report.suffix_target_snapshot_before, report.suffix_target_snapshot_after, 'SUFFIX_TARGET_MUTATED');

...
const PHASE = 'ECARD3_EXACT_TOKEN_PROMOTION_V1';
...
const EXPECTED_NUMERIC_COUNT = 4;
const EXPECTED_HOLO_COUNT = 11;
const BATCH_SIZE = 15;
const BACKUP_SCHEMA_PATH = path.join(
  process.cwd(),
  'backups',
  'ecard3_exact_token_preapply_schema.sql',
);
const BACKUP_DATA_PATH = path.join(
  process.cwd(),
  'backups',
  'ecard3_exact_token_preapply_data.sql',
);

const BACKUP_TABLES = [
  {
    table_name: 'card_prints',
    key_column: 'id',
    conflict_columns: ['id'],
  },
  {
    table_name: 'card_print_identity',
    key_column: 'card_print_id',
    conflict_columns: ['id'],
  },
];
...
function compareTokenValues(left, right) {
  ...
  const leftHolo = isHoloToken(leftToken);
  const rightHolo = isHoloToken(rightToken);

  if (leftHolo !== rightHolo) {
    return leftHolo ? 1 : -1;
  }
...
async function loadCandidateRows(client) {
  ...
  return queryRows(client, sql, [TARGET_IDENTITY_DOMAIN, TARGET_SET_CODE_IDENTITY]);
}
...
async function loadCanonicalRows(client) {
  ...
  return queryRows(client, sql, [CANONICAL_SET_CODE]);
}
...
async function loadLiveGvIdRows(client, gvIds) {
  ...
  return queryRows(client, sql, [gvIds]);
}
...
function buildCandidateMap(sourceRows) {
  return sourceRows
    .map((row) => ({
      ...
      proposed_gv_id: deriveEcard3GvId({
        printed_number: row.printed_number,
        variant_key: row.variant_key,
      }),
    }))
  ...
  for (const row of sourceRows) {
    ...
    rowAudits.push({
      ...
      matching_candidates: repoMatches.map((candidate) => ({
        canonical_card_print_id: candidate.canonical_card_print_id,
        canonical_name: candidate.canonical_name,
        canonical_number: candidate.canonical_number,
...
function buildPreconditions(sourceRows, candidateMap, overlapAudit, liveCollisionRows) {
  ...
  const holoRows = sourceRows.filter((row) => isHoloToken(row.printed_number));
  ...
  if (numericRows.length !== EXPECTED_NUMERIC_COUNT) {
    stopReasons.push(`NUMERIC_COUNT:${numericRows.length}`);
  }
  if (holoRows.length !== EXPECTED_HOLO_COUNT) {
    stopReasons.push(`HOLO_COUNT:${holoRows.length}`);
  }
  if (invalidRows.length > 0) {
    stopReasons.push(`INVALID_TOKEN_ROWS:${invalidRows.length}`);
  }
  if (duplicatePrintedNumbers.length > 0) {
    stopReasons.push(`DUPLICATE_PRINTED_NUMBER_GROUPS:${duplicatePrintedNumbers.length}`);
  }
  if (overlapAudit.exact_canonical_overlap_count > 0) {
    stopReasons.push(`EXACT_CANONICAL_OVERLAP_ROWS:${overlapAudit.exact_canonical_overlap_count}`);
  }
  if (overlapAudit.multiple_match_old_count > 0) {
    stopReasons.push(`MULTIPLE_MATCH_OLD_ROWS:${overlapAudit.multiple_match_old_count}`);
  }
  if (overlapAudit.same_token_different_name_count > 0) {
    stopReasons.push(`SAME_TOKEN_DIFFERENT_NAME_ROWS:${overlapAudit.same_token_different_name_count}`);
  }
  ...
  return {
    ...
    holo_candidate_count: holoRows.length,
...
async function loadBackupTableRows(client, tableName, keyColumn, candidateIds) {
  ...
  const rows = await queryRows(client, sql, [candidateIds]);
  return {
    table_name: tableName,
...
async function loadPostValidation(
  ...
  return {
    ...
    remaining_unresolved_ecard3: normalizeCount(remainingNullRow?.row_count),
...
function assertPostValidation(postValidation) {
  ...
  if (postValidation.remaining_unresolved_ecard3 !== 0) {
    throw new Error(`REMAINING_UNRESOLVED_ECARD3:${postValidation.remaining_unresolved_ecard3}`);
  }
  ...
  if (
    postValidation.identity_backed_non_null_gvid_after !==
    postValidation.identity_backed_non_null_gvid_before + EXPECTED_CANDIDATE_COUNT
  ) {
    throw new Error(
      `IDENTITY_BACKED_NON_NULL_GVID_DRIFT:${postValidation.identity_backed_non_null_gvid_before}:${postValidation.identity_backed_non_null_gvid_after}`,
    );
  }
  if (postValidation.canonical_set_code_count_after !== postValidation.canonical_set_code_count_before) {
    throw new Error(
      `CANONICAL_SET_CODE_COUNT_DRIFT:${postValidation.canonical_set_code_count_before}:${postValidation.canonical_set_code_count_after}`,
    );
  }
...
function buildSampleRowsBefore(candidateMap) {
  ...
  const holoRows = candidateMap.filter((row) => isHoloToken(row.printed_number));

  return {
    numeric: numericRows[0] ?? null,
    holo_low: holoRows[0] ?? null,
    holo_high: holoRows[holoRows.length - 1] ?? null,
  };
}
...
async function run() {
  ...
  const client = new Client({
    ...
    application_name: `ecard3_exact_token_promotion_apply_v1:${MODE}`,
  });
  ...
  try {
    ...
    report.canonical_summary = {
      ...
      canonical_holo_rows: canonicalRows.filter((row) => isHoloToken(row.canonical_number)).length,
    };

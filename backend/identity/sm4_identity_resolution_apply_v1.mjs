...
const PHASE = 'SM4_IDENTITY_RESOLUTION_V1';
...
const BACKUP_SCHEMA_PATH = path.join(process.cwd(), 'backups', 'sm4_preapply_schema.sql');
const BACKUP_DATA_PATH = path.join(process.cwd(), 'backups', 'sm4_preapply_data.sql');
...
function assertPreconditions(summary) {
  ...
  if (normalizeCount(summary?.base_reused_target_count) > 0) {
    throw new Error(
      `ACTIVE_IDENTITY_FAN_IN_BLOCKED:uq_card_print_identity_active_card_print_id:${normalizeCount(summary?.base_reused_target_count)}`,
    );
  }
}
...
async function run() {
  ...
  const client = new Client({
    ...
    application_name: `sm4_identity_resolution_apply_v1:${MODE}`,
  });

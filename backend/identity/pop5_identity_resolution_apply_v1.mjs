...
const PHASE = 'POP5_IDENTITY_RESOLUTION_V1';
...
const SOURCE_SET_CODE_IDENTITY = 'pop5';
const TARGET_SET_CODE = 'pop5';
...
const BACKUP_SCHEMA_PATH = path.join(process.cwd(), 'backups', 'pop5_preapply_schema.sql');
const BACKUP_DATA_PATH = path.join(process.cwd(), 'backups', 'pop5_preapply_data.sql');
...
async function buildTempCollapseSurface(client) {
  await client.query(`
    drop table if exists tmp_pop5_unresolved;
    drop table if exists tmp_pop5_canonical;
    drop table if exists tmp_pop5_match_audit;
    drop table if exists tmp_pop5_collapse_map;
    drop table if exists tmp_pop5_fanin_resolution;

    create temp table tmp_pop5_unresolved on commit drop as
    select
      cp.id as old_id,
      cp.name as old_name,
    ...
    create index tmp_pop5_unresolved_token_idx
      on tmp_pop5_unresolved (source_printed_number);

    create temp table tmp_pop5_canonical on commit drop as
    select
      cp.id as new_id,
      cp.name as new_name,
    ...
    create index tmp_pop5_canonical_token_idx
      on tmp_pop5_canonical (new_number);

    create index tmp_pop5_canonical_number_plain_idx
      on tmp_pop5_canonical (new_number_plain);

    create temp table tmp_pop5_match_audit (
    ...
    create temp table tmp_pop5_collapse_map (
      seq int not null,
    ...
    create unique index tmp_pop5_collapse_map_seq_uidx
      on tmp_pop5_collapse_map (seq);

    create unique index tmp_pop5_collapse_map_old_uidx
      on tmp_pop5_collapse_map (old_id);

    create temp table tmp_pop5_fanin_resolution (
    ...
    create unique index tmp_pop5_fanin_resolution_identity_uidx
      on tmp_pop5_fanin_resolution (identity_id);
...
async function insertMatchAuditRows(client, rows) {
  await client.query(
    `
      insert into tmp_pop5_match_audit (
        old_id,
        old_name,
        old_set_code,
        old_parent_number,
...
async function insertCollapseMapRows(client, rows) {
  await client.query(
    `
      insert into tmp_pop5_collapse_map (
        seq,
        old_id,
        new_id,
        old_name,
...
async function insertFanInResolutionRows(client, rows) {
  ...
  await client.query(
    `
      insert into tmp_pop5_fanin_resolution (
  ...
  const identityRows = await queryRows(
    ...
    `
      ...
      where cpi.card_print_id in (
        select old_id from tmp_pop5_collapse_map
        union
        select distinct new_id from tmp_pop5_collapse_map
      )
...
async function loadCollapseMapSamples(client) {
  const samples = await queryRows(
    ...
    `
      ...
      from tmp_pop5_collapse_map m
      left join tmp_pop5_fanin_resolution fan_in
  ...
  return queryOne(
    ...
    `
      with traits_on_old as (
        ...
        where t.card_print_id in (select old_id from tmp_pop5_collapse_map)
      ),
      ...
      printing_on_old as (
        ...
        where p.card_print_id in (select old_id from tmp_pop5_collapse_map)
      ),
      ...
      mappings_on_old as (
        ...
        where em.card_print_id in (select old_id from tmp_pop5_collapse_map)
      ),
      ...
      target_identity as (
        select count(*)::int as row_count
        from public.card_print_identity cpi
        where cpi.card_print_id in (select new_id from tmp_pop5_collapse_map)
      ),
      vault_on_old as (
        select count(*)::int as row_count
        from public.vault_items vi
        where vi.card_id in (select old_id from tmp_pop5_collapse_map)
      )
      select
...
async function applyCollapse(client) {
  ...
  const archivedIdentityRows = await client.query(`
    ...
    set
      is_active = false,
      updated_at = now()
    from tmp_pop5_fanin_resolution fan_in
  ...
  const summary = await queryOne(
    ...
    `
      ...
      route_resolvable as (
        select count(*)::int as row_count
        from public.card_prints cp
        where cp.id in (select new_id from tmp_pop5_collapse_map)
          and cp.gv_id is not null
      ),
      ...
      select
        (
          select count(*)::int
          from public.card_prints cp
          where cp.id in (select old_id from tmp_pop5_collapse_map)
...
async function run() {
  ...
  const client = new Client({
    ...
    application_name: `pop5_identity_resolution_apply_v1:${MODE}`,
  });

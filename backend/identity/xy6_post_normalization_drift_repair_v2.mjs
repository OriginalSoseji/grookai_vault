...
const PHASE = 'XY6_POST_NORMALIZATION_DRIFT_REPAIR_V2';
...
async function run() {
  ...
  const client = new Client({
    ...
    application_name: `xy6_post_normalization_drift_repair_v2:${MODE}`,
  });
  ...
  try {
    ...
    assertEqual(normalizeCount(report.canonical_count_before?.canonical_count), 112, 'CANONICAL_COUNT_BEFORE_DRIFT');

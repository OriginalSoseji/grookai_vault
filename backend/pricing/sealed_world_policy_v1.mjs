...
// Shared deterministic payload construction. Each game supplies its own authority and language policy.
export function createSealedWorldPolicyV1(config) {
const MTG_SEALED_WORLD_V1 = config.version;
const MTG_SEALED_GAME_KEY = config.gameKey;
const MTG_SEALED_CATEGORY_ID = config.categoryIds[0];
const MTG_SEALED_REVIEWER_ID = deterministicUuidV5(
  `grookai:system-actor:${config.gameKey}-sealed-world-v1`,
);
...
function buildMtgSealedWriteTelemetryV1(plan, pointerRowsWritten = 1) {
...
function stableJsonMtgSealedV1(value) {
  return JSON.stringify(stable(value));
}

function hashMtgSealedV1(value) {
...
function language(row) {
  if (config.language) return config.language(row);
...
function candidateId(row) {
  return deterministicUuidV5(
    `${config.gameKey}:sealed:candidate:${row.category_id}:${row.group_id}:${row.product_id}:${row.payload_hash}`,
  );
}

function familyId(row) {
  return deterministicUuidV5(`${config.gameKey}:sealed:family:tcgplayer-group:${row.group_id}`);
}

function variantId(row) {
  return deterministicUuidV5(`${config.gameKey}:sealed:variant:tcgplayer-product:${row.product_id}`);
}

function reviewId(row) {
  return deterministicUuidV5(`${config.gameKey}:sealed:review:tcgplayer-product:${row.product_id}`);
}

function mappingId(row) {
  return deterministicUuidV5(`${config.gameKey}:sealed:mapping:tcgplayer-product:${row.product_id}`);
}

function evidenceId(row, dimension, ordinal = 0) {
  return deterministicUuidV5(
    `${config.gameKey}:sealed:evidence:${row.product_id}:${dimension}:${ordinal}`,
  );
}

function qualificationId(row, price) {
  return deterministicUuidV5(
    `${config.gameKey}:sealed:qualification:${row.product_id}:${price.source_price_row_identity}:${price.observed_on}`,
  );
}
...
function evidenceRows(row, classification) {
  ...
  const rows = [
    evidenceRow(row, 'product_line', 'tcgcsv_source_groups.name', row.group_name,
      ...
      'strong', 1),
    ...(config.manufacturer ? [evidenceRow(row, 'manufacturer', 'tcgcsv_source_categories.display_name',
      row.category_display_name, { manufacturer_name: config.manufacturer },
      'moderate', 0.95)] : []),
...
function buildQualification(row, canonical, prices, latestSync) {
  ...
  if (config.strictPositivePrice && lineage.qualification_status === 'qualified_exact' &&
      (!Number.isFinite(Number(lineage.market_price)) || Number(lineage.market_price) <= 0)) {
    lineage.qualification_status = 'blocked_missing_price';
    lineage.qualification_reason = 'market_price_must_be_positive';
  }
  const core = {
...
function buildMtgSealedWorldPlanV1({ sourceRows, latestPriceRows,
  ...
  for (const row of [...(sourceRows ?? [])].sort((a, b) =>
    ...
    const classification = (config.classify ?? classifyCrossTcgSealedProductV1)(row);
    if (classification.classification !== 'sealed_candidate' ||
        !classification.candidate_identity.package_form ||
        !config.categoryIds.includes(Number(row.category_id)) ||
        !row.source_active || !row.group_id || !clean(row.group_name) ||
        !/^[0-9a-f]{64}$/.test(row.payload_hash ?? '')) {
      continue;
    }
    const locale = language(row);
    if (!config.languages.includes(locale.code)) {
      held.push({ source_product_id: Number(row.product_id),
        reason: 'unsupported_language_scope_hold', language_code: locale.code });
      continue;
    }
  ...
  const releaseId = deterministicUuidV5(`${config.gameKey}:sealed:release:${sourceFingerprint}`);
  payload.members = qualified.map((qualification) => {
    ...
    return { id: deterministicUuidV5(`${config.gameKey}:sealed:release-member:${memberFingerprint}`),
  ...
  payload.releases = [{
    id: releaseId,
    game_key: MTG_SEALED_GAME_KEY,
    release_key: `${config.gameKey}-sealed-${sourceFingerprint.slice(0, 20)}`,
    release_state: 'draft',
...
function validateMtgSealedWorldPlanV1(plan) {
  const findings = [];
  ...
  add((payload.variants ?? []).some((row) => !config.languages.includes(row.language_code)),
    'unsupported_language_variant_promoted');
  add((payload.mappings ?? []).some((row) => !config.categoryIds.includes(row.source_category_id) ||
...
return { buildPlan: buildMtgSealedWorldPlanV1, validatePlan: validateMtgSealedWorldPlanV1,
  writeTelemetry: buildMtgSealedWriteTelemetryV1, hash: hashMtgSealedV1,
  stableJson: stableJsonMtgSealedV1, reviewerId: MTG_SEALED_REVIEWER_ID };
}

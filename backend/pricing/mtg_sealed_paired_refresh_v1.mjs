...
import {hashMtgSealedV1 as hash,MTG_SEALED_REVIEWER_ID} from './mtg_sealed_world_v1.mjs';
import {postgresJsonbArrayTextV1,imageReleaseManifestFingerprintV1} from './mtg_sealed_image_release_plan_v1.mjs';
export const MTG_SEALED_PAIRED_REFRESH_V1='MTG_SEALED_PAIRED_REFRESH_V1';
export const MTG_SEALED_SOURCE_CONTAINMENT_V1='MTG_SEALED_SOURCE_CONTAINMENT_V1';
export const MTG_SEALED_REFRESH_BASELINE='86b207e6-4f73-5d9a-af40-864c47256c38';
const uuid=value=>deterministicUuidV5(`mtg:sealed:refresh:${value}`);
...
export function buildMtgSealedRefreshV1({baseline,source,prices,sync,today,producerCommit,pointers}){
  ...
  assert.equal(new Set(baseline.map(r=>r.source_mapping_id)).size,baseline.length,'Duplicate baseline mapping');
  assert.equal(new Set(baseline.map(r=>String(r.source_product_id))).size,baseline.length,'Duplicate baseline product');
  ...
  for(const row of baseline){
    assert.equal(row.game_key,'mtg');assert.ok([1].includes(Number(row.source_category_id)));
    assert.equal(row.language_code,'en');assert.equal(row.source_provider,'tcgplayer');
    assert.match(row.source_payload_hash,/^[a-f0-9]{64}$/);
    const s=bySource.get(String(row.source_product_id));
    for(const key of ['game_key','variant_id','source_mapping_id','source_provider'])assert.equal(row.image_evidence[key],row[key]);
    for(const key of ['source_product_id','source_category_id','source_group_id'])assert.equal(Number(row.image_evidence[key]),Number(row[key]));
    assert.equal(row.image_object.game_key,'mtg');assert.equal(row.image_object.content_sha256,row.image_evidence.content_sha256);
    ...
    assert.match(row.image_object.object_path,/^sealed\/mtg\/sha256\/[a-f0-9]{2}\/[a-f0-9]{64}\.(jpg|png|gif|webp)$/);
    ...
    if(!s?.source_active||s.payload_hash!==row.source_payload_hash||Number(s.category_id)!==Number(row.source_category_id)||Number(s.group_id)!==Number(row.source_group_id)){
      exclusions.push({variant_id:row.variant_id,reason:'source_identity_not_currently_verified',
        policy:MTG_SEALED_SOURCE_CONTAINMENT_V1,source_product_id:row.source_product_id,
    ...
    const q={variant_id:row.variant_id,source_mapping_id:row.source_mapping_id,
      ...
      source_observation_fingerprint:p.payload_hash,qualification_contract_version:MTG_SEALED_PAIRED_REFRESH_V1,publication_authority:false};
  ...
  const sourceFingerprint=hash({version:MTG_SEALED_PAIRED_REFRESH_V1,baseline:MTG_SEALED_REFRESH_BASELINE,
  ...
  const release={id:priceId,game_key:'mtg',release_key:`mtg-sealed-refresh-${sourceFingerprint.slice(0,20)}`,
    release_state:'draft',source_audit_producer_sha:producerCommit,source_sample_logical_hash:sourceFingerprint,
    release_contract_version:MTG_SEALED_PAIRED_REFRESH_V1,manifest_fingerprint:hash(members),expected_member_count:members.length,created_by:MTG_SEALED_REVIEWER_ID};
  ...
  for(const member of members){
    ...
    const e=stamp('image-evidence',{...core,source_release_member_id:member.id,source_plan_fingerprint:sourceFingerprint,
      coverage_fingerprint:coverage,evidence_contract_version:'MTG_SEALED_REUSED_IMAGE_EVIDENCE_V1'},'evidence_fingerprint');
    evidence.push(e);
    assertions.push(stamp('image-assertion',{game_key:'mtg',variant_id:member.variant_id,source_mapping_id:member.source_mapping_id,
      image_evidence_id:e.id,image_object_id:old.image_object.id,assertion_state:'exact_verified',
      assertion_contract_version:'MTG_SEALED_REUSED_IMAGE_ASSERTION_V1'},'assertion_fingerprint'));
  }
  const imageRelease={id:uuid(`image-release:${sourceFingerprint}`),game_key:'mtg',release_key:`mtg-sealed-refresh-images-${sourceFingerprint.slice(0,20)}`,
    ...
    coverage_fingerprint:coverage,release_contract_version:'MTG_SEALED_IMAGE_REFRESH_V1',expected_member_count:assertions.length,created_by:MTG_SEALED_REVIEWER_ID};
  const evById=new Map(evidence.map(e=>[e.id,e]));
  const imageMembers=assertions.map(a=>{
    const fp=hash(postgresJsonbArrayTextV1(['SEALED_PRODUCT_IMAGE_RELEASE_MEMBER_V1',imageRelease.id,'mtg',a.variant_id,a.id,
    ...
    return{id:uuid(`image-member:${fp}`),image_release_id:imageRelease.id,game_key:'mtg',variant_id:a.variant_id,image_assertion_id:a.id,member_fingerprint:fp};
  });
  imageRelease.manifest_fingerprint=imageReleaseManifestFingerprintV1(imageRelease,imageMembers);
  const body={version:MTG_SEALED_PAIRED_REFRESH_V1,producer_commit:producerCommit,baseline_image_release:MTG_SEALED_REFRESH_BASELINE,
    source_fingerprint:sourceFingerprint,expected_pointers:pointers,source_sync:sync,exclusions,
    source_containment_policy:MTG_SEALED_SOURCE_CONTAINMENT_V1,

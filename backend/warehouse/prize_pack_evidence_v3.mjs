...
const V1_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_evidence_v1.json');
...
const READY_BATCH_V1_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_ready_batch_v1_129.json');
const READY_BATCH_V1_RESIDUE_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_ready_batch_v1_129_residue_2.json',
);
const READY_BATCH_V2_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_ready_batch_v2_72.json');
const READY_BATCH_V3_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_ready_batch_v3_23.json');
const WAIT_INSPECTION_V1_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_wait_inspection_v1.json',
);
const INPUT_JSON_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_evidence_v3_input.json');
const TARGET_SLICE_JSON_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v3_target_slice.json',
);
const OUTPUT_JSON_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_evidence_v3.json');
const OUTPUT_MD_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_evidence_v3.md');
const READY_BATCH_V4_CANDIDATE_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_ready_batch_v4_candidate.json',
);
...
function countPromotedRows(batchArtifact) {
  return (batchArtifact.rows || []).filter(
    (row) =>
      row.promoted_card_print_id ||
      row.card_print_id ||
      row.promotion_result_type === 'CARD_PRINT_CREATED',
  ).length;
}
...
function buildCurrentWaitRows({
  ...
  const inspectionByExternalId = new Map(
    (waitInspection.rows || []).map((row) => [row.source_external_id, row]),
  );
  ...
  return stableSortRows(
    (v2.row_outcomes || [])
      ...
      .map((row) => {
        const inspectionRow = inspectionByExternalId.get(row.source_external_id);
        return {
          source: row.source,
          ...
          base_card_print_id: inspectionRow?.base_card_print_id ?? null,
          base_card_name:
            inspectionRow?.base_card_name ?? row.normalized_base_name ?? row.candidate_name,
          current_blocker_class:
            inspectionRow?.blocker_class ??
            (row.decision_reason_v2 === 'base_route_ambiguous_or_missing'
              ? 'BASE_ROUTE_AMBIGUOUS'
              : 'NO_SERIES_CONFIRMATION'),
          evidence_tier: inspectionRow?.evidence_tier ?? row.evidence_tier,
          known_series_appearances:
            inspectionRow?.appearance_in_series ?? row.appearance_in_series ?? [],
          missing_series_checked:
            row.full_supported_coverage === true
              ...
              : row.decision_reason_v2 ===
                ...
                ? [1, 2, 3]
                : [],
          source_family: inspectionRow?.source_set_family ?? row.source_set_id,
          source_tcgplayer_id: inspectionRow?.source_tcgplayer_id ?? null,
          image_url: inspectionRow?.image_url ?? null,
          image_source: inspectionRow?.image_source ?? null,
          variant_hint: inspectionRow?.variant_hint ?? 'play_pokemon_stamp',
          stamp_label: inspectionRow?.stamp_label ?? 'Play! Pokémon Stamp',
          why_blocked:
            inspectionRow?.why_blocked ??
            'The row still lacks deterministic series corroboration or a unique base route.',
          blocker_detail: inspectionRow?.blocker_detail ?? row.blocker ?? null,
          cluster_name: inspectionRow?.cluster_name ?? row.cluster_id ?? null,
          ...
          evidence_sources: inspectionRow?.evidence_sources ?? row.evidence_sources_v2 ?? [],
          research_links: buildResearchLinks(row, inspectionRow),
        };
      }),
  );
}
...
async function fetchSeriesPages() {
  ...
  for (const source of SERIES_SOURCES) {
    const response = await fetch(source.raw_url, {
      headers: {
        'user-agent': 'Mozilla/5.0 (compatible; GrookaiPrizePackEvidenceV3/1.0)',
      },
    });
    if (!response.ok) {
...
function buildFinalDecision(row, matchedSeries) {
  if (!row.unique_base_route || !row.base_gv_id) {
    return {
      ...
      final_decision: 'WAIT',
      decision_code: 'base_route_still_ambiguous_after_v3',
      final_reason:
  ...
  if (matchedSeries.length > 1) {
    return {
      ...
      decision_code: 'multi_series_duplicate_confirmed_by_v3_series_page_coverage',
      final_reason:
  ...
  if (matchedSeries.length === 1) {
    return {
      ...
      decision_code: 'single_series_confirmed_by_v3_series_page_coverage',
      final_reason:
  ...
  return {
    final_evidence_class: 'STILL_UNPROVEN',
    final_decision: 'WAIT',
    decision_code: 'no_series_match_after_v3_series_page_coverage',
    final_reason:
...
function buildTargetSliceRows(rows, seriesPages) {
  return stableSortRows(rows).map((row) => {
    ...
    return {
      ...
      evidence_sources_used_for_v3: match.matching_evidence,
...
function buildReadyBatchCandidate(readyRows) {
  ...
  return {
    ...
    workflow: 'PRIZE_PACK_READY_BATCH_V4_CANDIDATE',
    ...
    selection_summary: {
      ...
      target_origin: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V3',
    },
    rows: sorted.map((row, index) => ({
      ...
      evidence_sources_v3: [
        ...
        ...(row.evidence_sources_used_for_v3 ?? []),
      ],
      ...
      pre_intake_audit: {
        current_live_state: 'NOT_YET_AUDITED_IN_V3',
        blocker_class: null,
    ...
    recommended_next_execution_step:
      sorted.length > 0
        ...
        : 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V4',
  };
}

function buildMarkdown(report) {
  const lines = [];
  lines.push('# Prize Pack Evidence V3');
  lines.push('');
  ...
  lines.push('');
  lines.push(`- remaining_wait_after_v3: ${report.remaining_backlog.wait_for_more_evidence}`);
  ...
  lines.push('');
  lines.push(
    `- prize_pack_ready_batch_v4_candidate_rows: ${report.ready_batch_v4_candidate.row_count}`,
  );
  lines.push('');
...
async function main() {
  ...
  const [v1, v2, corroboration, readyBatchV1, readyBatchV1Residue, readyBatchV2, readyBatchV3, waitInspection] = await Promise.all([
    readJson(V1_PATH),
    readJson(V2_PATH),
    readJson(CORROBORATION_V1_PATH),
    readJson(READY_BATCH_V1_PATH),
    readJson(READY_BATCH_V1_RESIDUE_PATH),
    readJson(READY_BATCH_V2_PATH),
    readJson(READY_BATCH_V3_PATH),
    readJson(WAIT_INSPECTION_V1_PATH),
  ]);
  ...
  const currentBacklog = {
    ...
    do_not_canon_total:
      (v1.summary_counts?.next_action_counts?.DO_NOT_CANON ?? 0) +
      (v2.summary?.next_action_counts?.DO_NOT_CANON ?? 0) +
      ((corroboration.do_not_canon_rows || []).length ?? 0),
    already_promoted_total:
      countPromotedRows(readyBatchV1) +
      countPromotedRows(readyBatchV1Residue) +
      countPromotedRows(readyBatchV2) +
      countPromotedRows(readyBatchV3),
  };

  const inputArtifact = {
    generated_at: generatedAt,
    workflow: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V3_INPUT',
    source_artifacts: [
  ...
  const targetQuestion = `For the ${target.selected_cluster.row_count} ${target.selected_cluster.effective_set_name} (${target.selected_cluster.effective_set_code}) Prize Pack family-only rows with unique base routes and no current external series confirmation, do Prize Pack Series 1-8 card-list sources document each card in exactly one series, multiple series, or none?`;
  ...
  const targetSliceArtifact = {
    generated_at: generatedAt,
    workflow: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V3_TARGET_SLICE',
    source_artifact: 'docs/checkpoints/warehouse/prize_pack_evidence_v3_input.json',
    target_slice_selection_priority: [
      'unique base route',
      'strongest existing evidence tier available in the remaining wait pool',
      'missing early-series exclusion coverage or absent series confirmation',
  ...
  const remainingWaitRowsAfterV3 = currentWaitRows
    .filter(
      (row) =>
    ...
    .map((row) => ({
      ...
      remaining_reason: stillWaitExternalIds.has(row.source_external_id)
        ? 'STILL_UNPROVEN_AFTER_V3_TARGET_SLICE'
        : row.current_blocker_class,
    }));

  const report = {
    generated_at: generatedAt,
    workflow: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V3',
    scope: {
    ...
    remaining_backlog: {
      ...
      do_not_canon_total_after_v3: currentBacklog.do_not_canon_total + doNotCanonRows.length,
      already_promoted_total: currentBacklog.already_promoted_total,
      wait_reason_counts: countBy(
        remainingWaitRowsAfterV3,
        (row) => row.remaining_reason,
      ),
    },
    ready_batch_v4_candidate: {
  ...
  if (readyRows.length > 0) {
    await writeJson(READY_BATCH_V4_CANDIDATE_PATH, buildReadyBatchCandidate(readyRows));
  }
}

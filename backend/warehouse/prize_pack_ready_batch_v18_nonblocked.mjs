...
const WORKFLOW = 'PRIZE_PACK_READY_BATCH_V18_NONBLOCKED';
const ROOT_DIR = process.cwd();
const CHECKPOINT_DIR = path.join(ROOT_DIR, 'docs', 'checkpoints', 'warehouse');
const CANDIDATE_BATCH_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_ready_batch_v18_candidate.json',
);
const EVIDENCE_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_evidence_v18_nonblocked.json');
const OUTPUT_JSON_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_ready_batch_v18_nonblocked.json');
const OUTPUT_MD_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_ready_batch_v18_nonblocked.md');

const SOURCE_SET_ID = 'prize-pack-series-cards-pokemon';
const FOUNDER_EMAIL = 'ccabrl@gmail.com';
const FOUNDER_APPROVAL_NOTE =
  'PRIZE_PACK_READY_BATCH_V18_NONBLOCKED founder approval under GENERIC_PLAY_POKEMON_STAMP_IDENTITY_RULE_V1 review.';

const EXPECTED_ROWS = [
  { name: 'Alcremie V', printed_number: '022/073', effective_set_code: 'swsh3.5', base_gv_id: 'GV-PK-CPA-22' },
  { name: 'Alcremie VMAX', printed_number: '23/73', effective_set_code: 'swsh3.5', base_gv_id: 'GV-PK-CPA-23' },
  { name: 'Altaria', printed_number: '49/73', effective_set_code: 'swsh3.5', base_gv_id: 'GV-PK-CPA-49' },
];
...
function makeInitialArtifact(candidateBatch) {
  return {
    ...
    selection_summary: {
      ...
      target_origin: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V18_NONBLOCKED',
    ...
    rows: candidateBatch.rows.map((row, index) => ({
      ...
      evidence_sources_v18_nonblocked: [],
      ...
      decision_code: row.decision_code ?? 'single_series_exact_identity_confirmed_by_v18_nonblocked',
      final_reason: null,
    ...
    recommended_next_execution_step: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V19_NONBLOCKED',
  };
}
...
async function populateEvidenceDetails(artifact) {
  ...
  for (const row of artifact.rows) {
    ...
    row.evidence_sources_v18_nonblocked =
      evidenceRow?.evidence_sources_used_for_v18_nonblocked ?? row.evidence_sources_v18_nonblocked ?? [];
}
}
...
- Promoted Prize Pack total before V18: ${artifact.post_batch_prize_pack_status.promoted_prize_pack_total_before_v18}
- Promoted Prize Pack total after V18: ${artifact.post_batch_prize_pack_status.promoted_prize_pack_total_after_v18}
...
async function finalizeArtifact(client, artifact) {
  ...
  artifact.post_batch_prize_pack_status = {
    ...
    do_not_canon_rows: Number(evidence?.remaining_backlog?.do_not_canon_total_after_v18_nonblocked ?? 186),
    blocked_by_official_acquisition_rows: Number(
      evidence?.remaining_backlog?.blocked_by_official_acquisition_remaining ?? 47,
    ),
    promoted_prize_pack_total_before_v18: Number(
      evidence?.current_backlog?.promoted_prize_pack_total ?? 414,
    ),
    promoted_prize_pack_total_after_v18:
      Number(evidence?.current_backlog?.promoted_prize_pack_total ?? 414) + artifact.rows.length,
  };
}

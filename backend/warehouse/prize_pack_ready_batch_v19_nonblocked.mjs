...
const WORKFLOW = 'PRIZE_PACK_READY_BATCH_V19_NONBLOCKED';
const ROOT_DIR = process.cwd();
const CHECKPOINT_DIR = path.join(ROOT_DIR, 'docs', 'checkpoints', 'warehouse');
const CANDIDATE_BATCH_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_ready_batch_v19_nonblocked_candidate.json',
);
const EVIDENCE_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_evidence_v19_nonblocked.json');
const OUTPUT_JSON_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_ready_batch_v19_nonblocked.json');
const OUTPUT_MD_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_ready_batch_v19_nonblocked.md');

const SOURCE_SET_ID = 'prize-pack-series-cards-pokemon';
const FOUNDER_EMAIL = 'ccabrl@gmail.com';
const FOUNDER_APPROVAL_NOTE =
  'PRIZE_PACK_READY_BATCH_V19_NONBLOCKED founder approval under GENERIC_PLAY_POKEMON_STAMP_IDENTITY_RULE_V1 review.';

const EXPECTED_ROWS = [
  { name: 'Wormadam', printed_number: '010/172', effective_set_code: 'swsh9', base_gv_id: 'GV-PK-BRS-10' },
  { name: 'Wormadam', printed_number: '077/172', effective_set_code: 'swsh9', base_gv_id: 'GV-PK-BRS-77' },
  { name: 'Wormadam', printed_number: '098/172', effective_set_code: 'swsh9', base_gv_id: 'GV-PK-BRS-98' },
];
...
async function loadAndValidateCandidateBatch() {
  ...
  for (const row of rows) {
    ...
    const governingRule = row.governing_rule ?? row.governing_rule_source;
    if (normalizeTextOrNull(governingRule) !== 'GENERIC_PLAY_POKEMON_STAMP_IDENTITY_RULE_V1') {
...
function makeInitialArtifact(candidateBatch) {
  return {
    ...
    selection_summary: {
      ...
      target_origin: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V19_NONBLOCKED',
    ...
    rows: candidateBatch.rows.map((row, index) => ({
      ...
      governing_rule_source: row.governing_rule ?? row.governing_rule_source,
      governing_rules: [
      ...
      evidence_sources_v19_nonblocked: [],
      ...
      decision_code: row.decision_code ?? 'single_series_exact_identity_confirmed_by_v19_nonblocked',
      final_reason: null,
    ...
    recommended_next_execution_step: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V20_NONBLOCKED',
  };
}
...
async function populateEvidenceDetails(artifact) {
  ...
  for (const row of artifact.rows) {
    ...
    row.evidence_sources_v19_nonblocked =
      evidenceRow?.evidence_sources_used_for_v19_nonblocked ?? row.evidence_sources_v19_nonblocked ?? [];
}
}
...
- Promoted Prize Pack total before V19: ${artifact.post_batch_prize_pack_status.promoted_prize_pack_total_before_v19}
- Promoted Prize Pack total after V19: ${artifact.post_batch_prize_pack_status.promoted_prize_pack_total_after_v19}
...
async function finalizeArtifact(client, artifact) {
  ...
  artifact.post_batch_prize_pack_status = {
    remaining_wait_rows: Number(evidence?.remaining_backlog?.wait_for_more_evidence ?? 60),
    do_not_canon_rows: Number(evidence?.remaining_backlog?.do_not_canon_total_after_v19_nonblocked ?? 186),
    blocked_by_official_acquisition_rows: Number(
      evidence?.remaining_backlog?.blocked_by_official_acquisition_remaining ?? 47,
    ),
    promoted_prize_pack_total_before_v19: Number(
      evidence?.current_backlog?.promoted_prize_pack_total ?? 417,
    ),
    promoted_prize_pack_total_after_v19:
      Number(evidence?.current_backlog?.promoted_prize_pack_total ?? 417) + artifact.rows.length,
  };
}

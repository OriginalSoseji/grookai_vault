...
const V10_INPUT_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_evidence_v10_input.json');
const V10_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_evidence_v10.json');
const READY_BATCH_V10_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_ready_batch_v10.json');

const INPUT_JSON_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v11_nonblocked_input.json',
);
const TARGET_SLICE_JSON_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v11_nonblocked_target_slice.json',
);
const OUTPUT_JSON_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v11_nonblocked.json',
);
const OUTPUT_MD_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v11_nonblocked.md',
);
const READY_BATCH_CANDIDATE_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_ready_batch_v11_nonblocked_candidate.json',
);
...
const TARGET_SLICE = {
  id: 'SVE_BASIC_ENERGIES_SERIES_5_ALIAS_NORMALIZATION',
  label: 'SVE Basic Energies Series 5 Alias-Normalized Window',
  series: [5, 6, 7, 8],
  set_codes: ['sve'],
  source_family: 'prize-pack-series-cards-pokemon',
  shared_question:
    'Across currently accessible Prize Pack Series 5-8 sources, do these Scarlet & Violet Energies rows appear in exactly one corroborated series once the raw source label is normalized from "SVE Basic Energies" to the canon energy set family?',
};
...
function buildCurrentWaitRows({ v10Input, v10 }) {
  const readyIds = new Set((v10.ready_rows || []).map((row) => row.source_external_id));
  const doNotIds = new Set((v10.do_not_canon_rows || []).map((row) => row.source_external_id));
  const blockedById = new Map(
    (v10.blocked_official_acquisition_rows || []).map((row) => [row.source_external_id, row]),
  );
...
async function fetchSeriesPage(source) {
  const response = await fetch(source.raw_url, {
    headers: {
      'user-agent': 'Mozilla/5.0 (compatible; GrookaiPrizePackEvidenceV11Nonblocked/1.0)',
    },
  });
  if (!response.ok) {
...
function getSetMatchTokens(row) {
  ...
  const baseToken = normalizeText(row.effective_set_name);
  if (baseToken) tokens.add(baseToken);

  if ((row.effective_set_code ?? '').toLowerCase() === 'sve') {
    tokens.add('sve basic energies');
  }
...
function remarkAcquisitionBlockedRows(waitRows, allSeriesPages) {
  return stableSortRows(
    waitRows.map((row) => {
      if (
      ...
      const match = matchRowAgainstAccessibleSources(row, allSeriesPages);
      if (match.matched_series.length === 1 && [1, 2].includes(match.matched_series[0])) {
        return {
          ...row,
          blocked_by_official_acquisition: true,
          blocked_by_official_acquisition_reason: ACQUISITION_BLOCK_REASON,
          known_series_appearances: match.matched_series,
          missing_series_checked: [1, 2],
        };
      }
...
function chooseTargetSlice(waitRows, allSeriesPages) {
  ...
  const slicePages = allSeriesPages.filter((page) => TARGET_SLICE.series.includes(page.series));

  const matchedRows = stableSortRows(
    eligibleRows
      .map((row) => {
        const match = matchRowAgainstAccessibleSources(row, slicePages);
        return {
          ...row,
      ...
      .filter((row) => row._matched_series_for_selection.length === 1),
  );

  if (matchedRows.length === 0) {
    throw new Error('No coherent nonblocked target slice could be formed for V11.');
  }
...
function buildFinalDecision(row, matchedSeries) {
  if (!row.unique_base_route || !row.base_gv_id) {
    return {
      ...
      final_decision: 'WAIT',
      decision_code: 'base_route_ambiguous_after_v11_nonblocked',
      final_reason:
  ...
  if (matchedSeries.length > 1) {
    return {
      ...
      decision_code: 'multi_series_duplicate_confirmed_by_v11_nonblocked',
      final_reason:
        'Accessible evidence proves this row appears in more than one Prize Pack series with no printed distinction, so the generic Play! Pokémon stamp is distribution-only.',
    };
  }
  ...
  if (matchedSeries.length === 1) {
    return {
      ...
      decision_code: 'single_series_confirmed_by_v11_nonblocked_energy_alias_window',
      final_reason:
        'Accessible evidence places this row in exactly one corroborated Prize Pack series once the raw energy-set alias is normalized, so the generic Play! Pokémon stamp resolves to one deterministic canonical stamped identity.',
    };
  }
  ...
  return {
    final_evidence_class: 'STILL_UNPROVEN',
    final_decision: 'WAIT',
    decision_code: 'no_accessible_series_match_after_v11_nonblocked',
    final_reason:
...
function buildTargetSliceRows(rows, allSeriesPages) {
  const slicePages = allSeriesPages.filter((page) => TARGET_SLICE.series.includes(page.series));
  return stableSortRows(rows).map((row) => {
    const selectionEvidence = row._matching_evidence_for_selection ?? [];
    const matchedSeries =
      row._matched_series_for_selection ?? matchRowAgainstAccessibleSources(row, slicePages).matched_series;
    ...
    return {
      ...
      evidence_sources_used_for_v11_nonblocked: selectionEvidence,
...
function buildMarkdown(report) {
  const lines = [
    '# Prize Pack Evidence V11 Nonblocked',
    '',
  ...
  lines.push(`- blocked-by-official-acquisition: ${report.summary.blocked_by_official_acquisition}`);
  lines.push('');
  ...
  lines.push('');
  lines.push('## Acquisition-Blocked Examples');
  lines.push('');
...
async function main() {
  const [v10Input, v10, readyBatchV10] = await Promise.all([
    readJson(V10_INPUT_PATH),
    readJson(V10_PATH),
    readJson(READY_BATCH_V10_PATH),
  ]);

  const baseWaitRows = buildCurrentWaitRows({ v10Input, v10 });
  const allSeriesPages = await Promise.all(SERIES_SOURCES.map(fetchSeriesPage));
  const currentWaitRows = remarkAcquisitionBlockedRows(baseWaitRows, allSeriesPages);
  ...
  const currentDoNotCanonTotal =
    readyBatchV10.post_batch_prize_pack_status?.do_not_canon_rows ??
    v10.current_backlog?.do_not_canon_total ??
    0;
  const promotedTotal =
    readyBatchV10.post_batch_prize_pack_status?.promoted_prize_pack_total_after_v10 ??
    readyBatchV10.post_batch_prize_pack_status?.promoted_prize_pack_total_after_v10_nonblocked ??
    0;

  const inputArtifact = {
    generated_at: new Date().toISOString(),
    workflow: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V11_NONBLOCKED',
    source_artifacts: [
      relativeCheckpointPath(V10_INPUT_PATH),
      relativeCheckpointPath(V10_PATH),
      relativeCheckpointPath(READY_BATCH_V10_PATH),
    ],
    current_backlog: {
  ...
  const targetSlice = chooseTargetSlice(currentWaitRows, allSeriesPages);
  ...
  const report = {
    ...
    remaining_backlog: {
      ...
      do_not_canon_total_after_v11_nonblocked: currentDoNotCanonTotal + doNotCanonRows.length,
      already_promoted_total: promotedTotal,

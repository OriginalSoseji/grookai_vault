...
const WORKFLOW = 'PRIZE_PACK_READY_BATCH_V10';
...
const EVIDENCE_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_evidence_v10.json');
const OUTPUT_JSON_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_ready_batch_v10.json');
const OUTPUT_MD_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_ready_batch_v10.md');

const SOURCE_SET_ID = 'prize-pack-series-cards-pokemon';
const FOUNDER_EMAIL = 'ccabrl@gmail.com';
const FOUNDER_APPROVAL_NOTE =
  'PRIZE_PACK_READY_BATCH_V10 founder approval under GENERIC_PLAY_POKEMON_STAMP_IDENTITY_RULE_V1 review.';

const EXPECTED_ROWS = [
  { name: 'Town Store', printed_number: '196/197', effective_set_code: 'sv03', base_gv_id: 'GV-PK-OBF-196' },
  { name: 'Iron Valiant ex', printed_number: '089/182', effective_set_code: 'sv04', base_gv_id: 'GV-PK-PAR-89' },
  { name: 'Technical Machine: Blindside', printed_number: '176/182', effective_set_code: 'sv04', base_gv_id: 'GV-PK-PAR-176' },
  { name: 'Technical Machine: Turbo Energize', printed_number: '179/182', effective_set_code: 'sv04', base_gv_id: 'GV-PK-PAR-179' },
  { name: 'Hyper Aroma', printed_number: '152/167', effective_set_code: 'sv06', base_gv_id: 'GV-PK-TWM-152' },
  { name: "Ogre's Mask", printed_number: '159/167', effective_set_code: 'sv06', base_gv_id: 'GV-PK-TWM-159' },
  { name: 'Scoop Up Cyclone', printed_number: '162/167', effective_set_code: 'sv06', base_gv_id: 'GV-PK-TWM-162' },
  { name: 'Survival Brace', printed_number: '164/167', effective_set_code: 'sv06', base_gv_id: 'GV-PK-TWM-164' },
  { name: 'Legacy Energy', printed_number: '167/167', effective_set_code: 'sv06', base_gv_id: 'GV-PK-TWM-167' },
  { name: 'Grand Tree', printed_number: '136/142', effective_set_code: 'sv07', base_gv_id: 'GV-PK-SCR-136' },
  { name: "Janine's Secret Art", printed_number: '059/064', effective_set_code: 'sv6pt5', base_gv_id: 'GV-PK-SFA-59' },
];
...
function makeInitialArtifact(candidateBatch) {
  return {
    ...
    selection_summary: {
      ...
      target_origin: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V10_NONBLOCKED',
    ...
    rows: candidateBatch.rows.map((row, index) => ({
      ...
      evidence_sources_v10_nonblocked: [],
    ...
    recommended_next_execution_step: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V11_NONBLOCKED',
  };
}
...
async function populateEvidenceDetails(artifact) {
  ...
  for (const row of artifact.rows) {
    ...
    row.evidence_sources_v10_nonblocked = evidenceRow?.evidence_sources_used_for_v10_nonblocked ?? [];
  }
}
...
- Promoted Prize Pack total before V10: ${artifact.post_batch_prize_pack_status.promoted_prize_pack_total_before_v10}
- Promoted Prize Pack total after V10: ${artifact.post_batch_prize_pack_status.promoted_prize_pack_total_after_v10}
...
async function finalizeArtifact(client, artifact) {
  ...
  artifact.post_batch_prize_pack_status = {
    remaining_wait_rows: Number(evidence?.remaining_backlog?.wait_for_more_evidence ?? 135),
    do_not_canon_rows: Number(evidence?.remaining_backlog?.do_not_canon_total_after_v10_nonblocked ?? 174),
    blocked_by_official_acquisition_rows: Number(
      evidence?.remaining_backlog?.blocked_by_official_acquisition_remaining ?? 21,
    ),
    promoted_prize_pack_total_before_v10: Number(
      evidence?.current_backlog?.already_promoted_total ?? 350,
    ),
    promoted_prize_pack_total_after_v10:
      Number(evidence?.current_backlog?.already_promoted_total ?? 350) + artifact.rows.length,
  };
}

...
const WORKFLOW = 'PRIZE_PACK_READY_BATCH_V13_NONBLOCKED';
...
const EVIDENCE_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_evidence_v13_nonblocked.json');
const OUTPUT_JSON_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_ready_batch_v13_nonblocked.json');
const OUTPUT_MD_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_ready_batch_v13_nonblocked.md');

const SOURCE_SET_ID = 'prize-pack-series-cards-pokemon';
const FOUNDER_EMAIL = 'ccabrl@gmail.com';
const FOUNDER_APPROVAL_NOTE =
  'PRIZE_PACK_READY_BATCH_V13_NONBLOCKED founder approval under GENERIC_PLAY_POKEMON_STAMP_IDENTITY_RULE_V1 review.';

const EXPECTED_ROWS = [
  { name: 'Pokegear 3.0', printed_number: '186/198', effective_set_code: 'sv01', base_gv_id: 'GV-PK-SVI-186' },
  { name: 'Atticus', printed_number: '077/091', effective_set_code: 'sv4pt5', base_gv_id: 'GV-PK-PAF-77-S' },
  { name: 'Moonlit Hill', printed_number: '081/091', effective_set_code: 'sv4pt5', base_gv_id: 'GV-PK-PAF-81-S' },
  { name: 'Technical Machine: Crisis Punch', printed_number: '090/091', effective_set_code: 'sv4pt5', base_gv_id: 'GV-PK-PAF-90-S' },
];
...
function makeInitialArtifact(candidateBatch) {
  return {
    ...
    selection_summary: {
      ...
      target_origin: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V13_NONBLOCKED',
    ...
    rows: candidateBatch.rows.map((row, index) => ({
      ...
      evidence_sources_v13_nonblocked: [],
    ...
    recommended_next_execution_step: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V14_NONBLOCKED',
  };
}
...
async function populateEvidenceDetails(artifact) {
  ...
  for (const row of artifact.rows) {
    ...
    row.evidence_sources_v13_nonblocked =
      evidenceRow?.evidence_sources_used_for_v13_nonblocked ?? [];
}
}
...
- Promoted Prize Pack total before V13: ${artifact.post_batch_prize_pack_status.promoted_prize_pack_total_before_v13}
- Promoted Prize Pack total after V13: ${artifact.post_batch_prize_pack_status.promoted_prize_pack_total_after_v13}
...
async function finalizeArtifact(client, artifact) {
  ...
  artifact.post_batch_prize_pack_status = {
    remaining_wait_rows: Number(evidence?.remaining_backlog?.wait_for_more_evidence ?? 114),
    do_not_canon_rows: Number(evidence?.remaining_backlog?.do_not_canon_total_after_v13_nonblocked ?? 176),
    blocked_by_official_acquisition_rows: Number(
      evidence?.remaining_backlog?.blocked_by_official_acquisition_remaining ?? 88,
    ),
    promoted_prize_pack_total_before_v13: Number(
      evidence?.current_backlog?.already_promoted_total ?? 369,
    ),
    promoted_prize_pack_total_after_v13:
      Number(evidence?.current_backlog?.already_promoted_total ?? 369) + artifact.rows.length,
  };
}

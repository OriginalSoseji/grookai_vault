...
const WORKFLOW = 'PRIZE_PACK_READY_BATCH_V8_NONBLOCKED';
...
const EVIDENCE_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_evidence_v8_nonblocked.json');
const OUTPUT_JSON_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_ready_batch_v8_nonblocked.json');
const OUTPUT_MD_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_ready_batch_v8_nonblocked.md');

const SOURCE_SET_ID = 'prize-pack-series-cards-pokemon';
const FOUND_EMAIL = 'ccabrl@gmail.com';
const FOUNDER_APPROVAL_NOTE =
  'PRIZE_PACK_READY_BATCH_V8_NONBLOCKED founder approval under GENERIC_PLAY_POKEMON_STAMP_IDENTITY_RULE_V1 review.';

const EXPECTED_ROWS = [
  { name: 'Mimikyu ex', printed_number: '004', effective_set_code: 'svp', base_gv_id: 'GV-PK-PR-SV-004' },
  { name: 'Ampharos ex', printed_number: '016', effective_set_code: 'svp', base_gv_id: 'GV-PK-PR-SV-016' },
  { name: 'Lucario ex', printed_number: '017', effective_set_code: 'svp', base_gv_id: 'GV-PK-PR-SV-17' },
  { name: 'Cyclizar ex', printed_number: '018', effective_set_code: 'svp', base_gv_id: 'GV-PK-PR-SV-18' },
];
...
async function loadAndValidateCandidateBatch() {
  ...
  for (const row of rows) {
    if (normalizeTextOrNull(row.final_decision) !== 'READY_FOR_WAREHOUSE') {
      throw new Error(`candidate_batch_row_not_ready:${row.source_external_id}`);
    }
    ...
    const supportedSeries = Array.isArray(row.supported_series_list) ? row.supported_series_list : [];
    if (supportedSeries.length !== 1 || Number(supportedSeries[0]) !== 3) {
...
async function fetchFounderUserId(client) {
  ...
  if (!founder?.id) {
    throw new Error(`founder_user_not_found:${FOUND_EMAIL}`);
  }
  return founder.id;
}
...
function makeInitialArtifact(candidateBatch) {
  return {
    ...
    rows: candidateBatch.rows.map((row) => ({ ...row })),
    post_batch_prize_pack_status: null,
    recommended_next_execution_step: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V9_NONBLOCKED',
  };
}
...
async function populatePreIntakeAudit(client, artifact) {
  for (const row of artifact.rows) {
    ...
    } else if (existingVariant?.id || warehouseCandidate?.promoted_card_print_id || warehouseCandidate?.state === 'PROMOTED') {
...
async function bridgeReadyRows(client, artifact) {
  for (const row of artifact.rows) {
    ...
    if (!worker.summary) {
      throw new Error(`bridge_missing_summary:${row.source_external_id}`);
    }
    if (Number(worker.summary.candidates_blocked ?? 0) > 0) {
...
async function executeDryRunAndApply(client, artifact) {
  for (const row of artifact.rows) {
    ...
    const promotedRow = await fetchPromotedVariantRow(
      client,
      row.effective_set_code,
      row.name,
      row.printed_number,
    ...
    if (staging?.execution_status !== 'SUCCEEDED') {
      throw new Error(`promotion_stage_not_succeeded:${row.source_external_id}:${staging?.execution_status ?? 'null'}`);
    }
  }
...
async function runMappingAndImages(client, artifact) {
  ...
  const mappingWorker = runNodeScript(
  ...
  const imageWorker = runNodeScript(
...
- Promoted Prize Pack total before V8 nonblocked: ${artifact.post_batch_prize_pack_status.promoted_prize_pack_total_before_v8_nonblocked}
- Promoted Prize Pack total after V8 nonblocked: ${artifact.post_batch_prize_pack_status.promoted_prize_pack_total_after_v8_nonblocked}
...
async function finalizeArtifact(client, artifact) {
  ...
  artifact.post_batch_prize_pack_status = {
    remaining_wait_rows: Number(evidence?.remaining_backlog?.wait_for_more_evidence_after_v8_nonblocked ?? 151),
    do_not_canon_rows: Number(evidence?.remaining_backlog?.do_not_canon_total_after_v8_nonblocked ?? 169),
    blocked_by_official_acquisition_rows: Number(
      evidence?.remaining_backlog?.blocked_by_official_acquisition_remaining ?? 21,
    ),
    promoted_prize_pack_total_before_v8_nonblocked: Number(
      evidence?.current_backlog?.already_promoted_prize_pack_total ?? 346,
    ),
    promoted_prize_pack_total_after_v8_nonblocked:
      Number(evidence?.current_backlog?.already_promoted_prize_pack_total ?? 346) + artifact.rows.length,
  };
}
...
async function main() {
  ...
  try {
    ...
    if (conflictingRows.length > 0) {
      throw new Error(`precheck_conflict_rows:${JSON.stringify(conflictingRows.map((row) => row.source_external_id))}`);
    }

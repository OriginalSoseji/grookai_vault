...
const WORKFLOW = 'PRIZE_PACK_READY_BATCH_V6_SOURCE_UPGRADE_SERIES_1';
const ROOT_DIR = process.cwd();
const CHECKPOINT_DIR = path.join(ROOT_DIR, 'docs', 'checkpoints', 'warehouse');
const CANDIDATE_BATCH_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_ready_batch_v6_candidate.json',
);
const EVIDENCE_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_evidence_v6.json');
const LATEST_STATE_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_evidence_v17_nonblocked.json');
...
const OUTPUT_MD_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_ready_batch_v6_source_upgrade_series_1.md',
);

const SOURCE_SET_ID = 'prize-pack-series-cards-pokemon';
const FOUNDER_EMAIL = 'ccabrl@gmail.com';
const FOUNDER_APPROVAL_NOTE =
  'PRIZE_PACK_READY_BATCH_V6_SOURCE_UPGRADE_SERIES_1 founder approval under GENERIC_PLAY_POKEMON_STAMP_IDENTITY_RULE_V1 review.';

const EXPECTED_ROWS = [
  { name: "Victini VMAX", printed_number: "022/163", effective_set_code: "swsh5", base_gv_id: "GV-PK-BST-22" },
  { name: "Empoleon V", printed_number: "040/163", effective_set_code: "swsh5", base_gv_id: "GV-PK-BST-40" },
  { name: "Orbeetle", printed_number: "065/163", effective_set_code: "swsh5", base_gv_id: "GV-PK-BST-65" },
  { name: "Crobat", printed_number: "091/163", effective_set_code: "swsh5", base_gv_id: "GV-PK-BST-91" },
  { name: "Tyranitar V", printed_number: "097/163", effective_set_code: "swsh5", base_gv_id: "GV-PK-BST-97" },
  { name: "Corviknight V", printed_number: "109/163", effective_set_code: "swsh5", base_gv_id: "GV-PK-BST-109" },
  { name: "Corviknight VMAX", printed_number: "110/163", effective_set_code: "swsh5", base_gv_id: "GV-PK-BST-110" },
  { name: "Exp. Share", printed_number: "126/163", effective_set_code: "swsh5", base_gv_id: "GV-PK-BST-126" },
  { name: "Blaziken V", printed_number: "020/198", effective_set_code: "swsh6", base_gv_id: "GV-PK-CRE-20" },
  { name: "Blaziken VMAX", printed_number: "021/198", effective_set_code: "swsh6", base_gv_id: "GV-PK-CRE-21" },
  { name: "Froslass", printed_number: "036/198", effective_set_code: "swsh6", base_gv_id: "GV-PK-CRE-36" },
  { name: "Zeraora V", printed_number: "053/198", effective_set_code: "swsh6", base_gv_id: "GV-PK-CRE-53" },
  { name: "Gardevoir", printed_number: "061/198", effective_set_code: "swsh6", base_gv_id: "GV-PK-CRE-61" },
  { name: "Lycanroc", printed_number: "087/198", effective_set_code: "swsh6", base_gv_id: "GV-PK-CRE-87" },
  { name: "Spiritomb", printed_number: "103/198", effective_set_code: "swsh6", base_gv_id: "GV-PK-CRE-103" },
  { name: "Single Strike Urshifu", printed_number: "108/198", effective_set_code: "swsh6", base_gv_id: "GV-PK-CRE-108" },
  { name: "Avery", printed_number: "130/198", effective_set_code: "swsh6", base_gv_id: "GV-PK-CRE-130" },
  { name: "Impact Energy", printed_number: "157/198", effective_set_code: "swsh6", base_gv_id: "GV-PK-CRE-157" },
  { name: "Spiral Energy", printed_number: "159/198", effective_set_code: "swsh6", base_gv_id: "GV-PK-CRE-159" },
];
  ...
  for (const row of rows) {
    ...
    const supportedSeries = Array.isArray(row.supported_series_list)
      ...
      : Array.isArray(row.confirmed_series_coverage)
        ...
        : [];
    if (supportedSeries.length !== 1 || Number(supportedSeries[0]) !== 1) {
    ...
    if (normalizeTextOrNull(row.source_upgrade_v1_status) !== 'tier_upgraded_to_tier_1') {
      throw new Error(`candidate_batch_source_upgrade_mismatch:${row.source_external_id}`);
    }
    const officialSource = (row.evidence_sources_v6 || []).find(
      (source) =>
        normalizeTextOrNull(source.source_type) === 'official_checklist_json_import' &&
        Number(source.series) === 1 &&
        normalizeTextOrNull(source.evidence_tier) === 'TIER_1',
    );
    if (!officialSource) {
      throw new Error(`candidate_batch_official_series_1_provenance_missing:${row.source_external_id}`);
    }
  }
...
function makeInitialArtifact(candidateBatch) {
  return {
    ...
    selection_summary: {
      ...
      target_origin: 'PRIZE_PACK_EVIDENCE_SOURCE_UPGRADE_V1_SERIES_1_OFFICIAL_LOCAL_IMPORT',
    ...
    rows: candidateBatch.rows.map((row, index) => ({
      ...
      evidence_sources_v6: row.evidence_sources_v6 ?? [],
      source_upgrade_v1_status: row.source_upgrade_v1_status ?? null,
      source_upgrade_v1_reason: row.source_upgrade_v1_reason ?? null,
      ...
      decision_code: 'series_1_official_checklist_tier_1_source_upgrade',
      final_reason:
        row.source_upgrade_v1_reason ??
        'Matched locally imported official Pokemon.com Prize Pack Series 1 checklist data.',
    ...
    recommended_next_execution_step: 'MANUAL_BROWSER_DOWNLOAD_AND_LOCAL_JSON_IMPORT_FOR_PRIZE_PACK_V1_SERIES_2',
  };
}
...
async function populateEvidenceDetails(artifact) {
  ...
  for (const row of artifact.rows) {
    ...
    row.final_reason = evidenceRow?.source_upgrade_v1_reason ?? evidenceRow?.final_reason ?? row.final_reason;
    row.evidence_sources_v6 = evidenceRow?.evidence_sources_v6 ?? row.evidence_sources_v6 ?? [];
    row.source_upgrade_v1_status = evidenceRow?.source_upgrade_v1_status ?? row.source_upgrade_v1_status;
    row.source_upgrade_v1_reason = evidenceRow?.source_upgrade_v1_reason ?? row.source_upgrade_v1_reason;
  }
}
...
- Remaining WAIT rows before Series 1 source-upgrade batch: ${artifact.post_batch_prize_pack_status.remaining_wait_rows_before_series_1_source_upgrade_batch}
- Remaining WAIT rows after Series 1 source-upgrade batch: ${artifact.post_batch_prize_pack_status.remaining_wait_rows_after_series_1_source_upgrade_batch}
- DO_NOT_CANON rows: ${artifact.post_batch_prize_pack_status.do_not_canon_rows}
- Blocked-by-official-acquisition rows before Series 1 source-upgrade batch: ${artifact.post_batch_prize_pack_status.blocked_by_official_acquisition_rows_before_series_1_source_upgrade_batch}
- Blocked-by-official-acquisition rows after Series 1 source-upgrade batch: ${artifact.post_batch_prize_pack_status.blocked_by_official_acquisition_rows_after_series_1_source_upgrade_batch}
- Promoted Prize Pack total before Series 1 source-upgrade batch: ${artifact.post_batch_prize_pack_status.promoted_prize_pack_total_before_series_1_source_upgrade_batch}
- Promoted Prize Pack total after Series 1 source-upgrade batch: ${artifact.post_batch_prize_pack_status.promoted_prize_pack_total_after_series_1_source_upgrade_batch}
...
async function finalizeArtifact(client, artifact) {
  ...
  const latestCurrent = latestState?.current_backlog ?? {};
  const waitBefore = Number(
    latestRemaining.wait_for_more_evidence ?? latestCurrent.wait_for_more_evidence ?? 104,
  );
  ...
  artifact.post_batch_prize_pack_status = {
    remaining_wait_rows_before_series_1_source_upgrade_batch: waitBefore,
    remaining_wait_rows_after_series_1_source_upgrade_batch: Math.max(0, waitBefore - artifact.rows.length),
    do_not_canon_rows: doNotCanonRows,
    blocked_by_official_acquisition_rows_before_series_1_source_upgrade_batch: acquisitionBlockedBefore,
    blocked_by_official_acquisition_rows_after_series_1_source_upgrade_batch: Math.max(
      0,
      acquisitionBlockedBefore - artifact.rows.length,
    ),
    promoted_prize_pack_total_before_series_1_source_upgrade_batch: Math.max(
      0,
      livePromotedPrizePackTotal - artifact.rows.length,
    ),
    promoted_prize_pack_total_after_series_1_source_upgrade_batch: livePromotedPrizePackTotal,
    series_2_official_acquisition_status: 'BLOCKED_UNTOUCHED',
  };
}

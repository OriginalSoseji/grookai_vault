...
const V6_INPUT_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_evidence_v6_nonblocked_input.json');
...
const INPUT_JSON_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v7_nonblocked_input.json',
);
const TARGET_SLICE_JSON_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v7_nonblocked_target_slice.json',
);
const OUTPUT_JSON_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v7_nonblocked.json',
);
const OUTPUT_MD_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v7_nonblocked.md',
);
const READY_BATCH_CANDIDATE_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_ready_batch_v7_nonblocked_candidate.json',
);
...
const TARGET_SLICE = {
  id: 'ACCESSIBLE_SERIES_2_3_WINDOW',
  label: 'Accessible Series 2-3 Window',
  series: [2, 3],
  set_codes: ['swsh10', 'swsh11', 'pgo'],
  shared_question:
    'Across currently accessible Prize Pack Series 2 and Series 3 sources, does this card appear in exactly one corroborated series or in more than one, without depending on blocked official Series 1/2 acquisition?',
...
function buildCurrentWaitRows({ v6Input, v6 }) {
  const readyRowIds = new Set((v6.ready_rows || []).map((row) => row.source_external_id));
  const acquisitionBlockedById = new Map(
    (v6.blocked_official_acquisition_rows || []).map((row) => [row.source_external_id, row]),
  );
  ...
  return stableSortRows(
    (v6Input.rows || [])
      ...
      .map((row) => {
        ...
        return {
          ...row,
          candidate_name: row.candidate_name ?? blockedRow?.candidate_name,
          printed_number: row.printed_number ?? blockedRow?.printed_number,
          ...
          base_gv_id: row.base_gv_id ?? blockedRow?.base_gv_id ?? null,
          base_route: row.base_route ?? blockedRow?.base_route ?? null,
          base_card_name: row.base_card_name ?? blockedRow?.base_card_name ?? row.candidate_name,
          current_blocker_class:
            row.current_blocker_class ??
            blockedRow?.current_blocker_class ??
            'NO_SERIES_CONFIRMATION',
          evidence_tier: row.evidence_tier ?? blockedRow?.final_evidence_tier ?? 'TIER_4',
...
function chooseTargetSlice(waitRows) {
  ...
  if (eligibleRows.length === 0) {
    throw new Error('No coherent nonblocked target slice could be formed for V7.');
  }
...
async function fetchSeriesPage(source) {
  const response = await fetch(source.raw_url, {
    headers: {
      'user-agent': 'Mozilla/5.0 (compatible; GrookaiPrizePackEvidenceV7Nonblocked/1.0)',
    },
  });
  if (!response.ok) {
...
function buildFinalDecision(row, matchedSeries) {
  if (!row.unique_base_route || !row.base_gv_id) {
    return {
      ...
      final_decision: 'WAIT',
      decision_code: 'base_route_ambiguous_after_v7_nonblocked',
      final_reason:
  ...
  if (matchedSeries.length > 1) {
    return {
      ...
      decision_code: 'multi_series_duplicate_confirmed_by_v7_nonblocked_series_2_3',
      final_reason:
        'The card appears across multiple accessible Prize Pack series inside the Series 2-3 window with no printed series marker, so the generic Play! Pokémon stamp remains distribution-only.',
      blocked_by_official_acquisition: false,
      blocked_by_official_acquisition_reason: null,
    };
  }
  ...
  if (matchedSeries.length === 1 && matchedSeries[0] === 3) {
    return {
      ...
      decision_code: 'single_series_confirmed_by_v7_nonblocked_series_2_3',
      final_reason:
        'The card appears in exactly one corroborated accessible Prize Pack series inside the checked Series 2-3 window, so the generic Play! Pokémon stamp resolves to one deterministic canonical stamped identity.',
      blocked_by_official_acquisition: false,
      blocked_by_official_acquisition_reason: null,
    };
  }

  if (matchedSeries.length === 1 && matchedSeries[0] === 2) {
    return {
      final_evidence_class: 'STILL_UNPROVEN',
      final_decision: 'WAIT',
      decision_code: 'single_series_2_match_requires_official_upgrade_after_v7_nonblocked',
      final_reason:
        'Accessible evidence places the card in Series 2 only, but the truthful next step still depends on official Series 2 checklist acquisition before canonization.',
  ...
  return {
    final_evidence_class: 'STILL_UNPROVEN',
    final_decision: 'WAIT',
    decision_code: 'no_accessible_series_2_3_match_after_v7_nonblocked',
    final_reason:
      'The accessible Series 2-3 sources still do not place this card in a corroborated Prize Pack list, so the row remains evidence-bound.',
    blocked_by_official_acquisition: false,
    blocked_by_official_acquisition_reason: null,
  };
}
...
function buildTargetSliceRows(rows, seriesPages) {
  return stableSortRows(rows).map((row) => {
    ...
    return {
      ...
      evidence_sources_used_for_v7_nonblocked: match.matching_evidence,
...
function buildReadyBatchCandidate(readyRows) {
  ...
  return {
    ...
    workflow: 'PRIZE_PACK_READY_BATCH_V7_NONBLOCKED_CANDIDATE',
    ...
    selection_summary: {
      ...
      target_origin: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V7_NONBLOCKED',
    },
    rows: sorted.map((row, index) => ({
      ...
      evidence_sources_v7_nonblocked: [
        ...(row.evidence_sources ?? []),
        ...(row.evidence_sources_used_for_v7_nonblocked ?? []),
      ],
    ...
    recommended_next_execution_step: 'PRIZE_PACK_READY_BATCH_V7_NONBLOCKED',
  };
}
...
async function main() {
  const [v2, corroboration, v3, v4, v5, v6Input, v6, readyBatchV6, readyBatchV6Residue] =
    await Promise.all([
      readJson(V2_PATH),
      ...
      readJson(V4_PATH),
      readJson(V5_PATH),
      readJson(V6_INPUT_PATH),
      readJson(V6_PATH),
      readJson(READY_BATCH_V6_PATH),
      readJson(READY_BATCH_V6_RESIDUE_PATH),
    ]);

  const currentWaitRows = buildCurrentWaitRows({ v6Input, v6 });
  ...
  const inputArtifact = {
    generated_at: new Date().toISOString(),
    workflow: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V7_NONBLOCKED',
    source_artifacts: [
      ...
      relativeCheckpointPath(V6_INPUT_PATH),
    ],
    current_backlog: {
      wait_for_more_evidence:
        readyBatchV6Residue.post_batch_prize_pack_status?.remaining_wait_for_more_evidence ??
        currentWaitRows.length,
      do_not_canon_total:
        readyBatchV6Residue.post_batch_prize_pack_status?.do_not_canon_total ??
        v6.remaining_backlog?.do_not_canon_total_after_v6_nonblocked ??
        0,
      already_promoted_total:
        readyBatchV6Residue.post_batch_prize_pack_status?.promoted_prize_pack_total_after_v6_residue ??
        readyBatchV6.post_batch_prize_pack_status?.promoted_prize_pack_total_after_v6_nonblocked ??
        0,
      blocked_by_official_acquisition:
        readyBatchV6Residue.post_batch_prize_pack_status?.blocked_by_official_acquisition_remaining ??
  ...
  const seriesPages = await Promise.all([
    fetchSeriesPage(SERIES_2_SOURCE),
    fetchSeriesPage(SERIES_3_SOURCE),
  ]);

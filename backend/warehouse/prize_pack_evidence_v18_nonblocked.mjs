...
const INPUT_JSON_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v18_nonblocked_input.json',
);
const TARGET_SLICE_JSON_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v18_nonblocked_target_slice.json',
);
const OUTPUT_JSON_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v18_nonblocked.json',
);
const OUTPUT_MD_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v18_nonblocked.md',
);
...
const TARGET_SOURCE_EXTERNAL_IDS = new Set([
  'pokemon-prize-pack-series-cards-alcremie-v-ultra-rare',
  'pokemon-prize-pack-series-cards-alcremie-vmax-ultra-rare',
  'pokemon-prize-pack-series-cards-altaria-49-73-rare',
]);

const TARGET_SLICE = {
  id: 'CHAMPIONS_PATH_SERIES_1_OFFICIAL_LOCAL_JSON_AUDIT',
  label: 'Champion Path Series 1 Official Local JSON Audit',
  row_count: TARGET_SOURCE_EXTERNAL_IDS.size,
  set_code: 'swsh3.5',
  series_checked: [1, 2, 3, 4, 5, 6, 7, 8],
  shared_question:
    'Across local official Series 1-2 JSON and accessible Series 3-8 sources, does each Champion Path row appear in exactly one corroborated Prize Pack series?',
  selection_note:
    'This small Champion Path slice was selected because the rows were no longer acquisition-blocked after local official imports, each has an exact Series 1 official local JSON hit, and the checked Series 2-8 sources do not provide a duplicate exact identity hit.',
};
...
function buildCurrentWaitRows({ v17Input, series1Batch, series2Batch }) {
  ...
  for (const source of BULBAPEDIA_SOURCES) {
    try {
      const response = await fetch(source.raw_url, {
        headers: {
          'user-agent': 'Mozilla/5.0 (compatible; GrookaiPrizePackEvidenceV18Nonblocked/1.0)',
        },
...
function buildFinalDecision(exactEvidence) {
  ...
  if (series.length > 1) {
    return {
      ...
      decision_code: 'multi_series_exact_identity_confirmed_by_v18_nonblocked',
      final_reason:
  ...
  if (series.length === 1) {
    return {
      ...
      decision_code: 'single_series_exact_identity_confirmed_by_v18_nonblocked',
      final_reason:
  ...
  return {
    ...
    final_decision: 'WAIT',
    decision_code: 'no_exact_printed_identity_match_after_v18_nonblocked',
    final_reason:
...
function chooseTargetSlice(currentWaitRows) {
  ...
  if (rows.length !== TARGET_SOURCE_EXTERNAL_IDS.size) {
    throw new Error(
      `V18 target slice is not reproducible: expected ${TARGET_SOURCE_EXTERNAL_IDS.size}, found ${rows.length}.`,
    );
  }
...
function buildTargetRows({ rows, series1, series2, series1Index, series2Index, fixture, bulbapediaPages }) {
  return stableSortRows(rows).map((row) => {
    ...
    return {
      ...
      evidence_sources_used_for_v18_nonblocked: exactEvidence,
      near_match_evidence_for_v18_nonblocked: [
        ...fixtureMatch.near,
...
function buildReadyBatchCandidate(rows) {
  return stableSortRows(rows).map((row, index) => {
    ...
    return {
      ...
      evidence_sources_v18_nonblocked: row.evidence_sources_used_for_v18_nonblocked,
      ...
      reference_hints_payload: {
        provenance: 'prize_pack_evidence_v18_nonblocked',
        source_family: row.source_family ?? 'prize-pack-series-cards-pokemon',
        evidence_class: row.final_evidence_class,
        evidence_tier: row.final_evidence_tier,
        confirmed_series_coverage: row.confirmed_series_coverage,
...
function buildMarkdown(report) {
  const lines = [
    '# Prize Pack Evidence V18 Nonblocked',
    '',
...
async function main() {
  const [v17Input, v17, series1, series2, fixture, series1Batch, series2Batch] =
    await Promise.all([
  ...
  const currentWaitRows = buildCurrentWaitRows({ v17Input, series1Batch, series2Batch });
  ...
  const currentBacklog = {
    ...
    do_not_canon_total: series2Batch.post_batch_prize_pack_status?.do_not_canon_total ?? 186,
    promoted_prize_pack_total:
      series2Batch.post_batch_prize_pack_status?.promoted_prize_pack_total ?? 414,
  };

  const inputArtifact = {
    generated_at: new Date().toISOString(),
    workflow: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V18_NONBLOCKED',
    source_artifacts: [
  ...
  const report = {
    ...
    remaining_backlog: {
      ...
      do_not_canon_total_after_v18_nonblocked:

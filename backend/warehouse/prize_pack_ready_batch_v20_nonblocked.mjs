...
const WORKFLOW = 'PRIZE_PACK_READY_BATCH_V20_NONBLOCKED';
const ROOT_DIR = process.cwd();
const CHECKPOINT_DIR = path.join(ROOT_DIR, 'docs', 'checkpoints', 'warehouse');
const CANDIDATE_BATCH_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_ready_batch_v20_nonblocked_candidate.json',
);
const EVIDENCE_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_evidence_v20_nonblocked.json');
const OUTPUT_JSON_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_ready_batch_v20_nonblocked.json');
const OUTPUT_MD_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_ready_batch_v20_nonblocked.md');

const SOURCE_SET_ID = 'prize-pack-series-cards-pokemon';
const FOUNDER_EMAIL = 'ccabrl@gmail.com';
const FOUNDER_APPROVAL_NOTE =
  'PRIZE_PACK_READY_BATCH_V20_NONBLOCKED founder approval under GENERIC_PLAY_POKEMON_STAMP_IDENTITY_RULE_V1 review.';

const EXPECTED_ROWS = [
  { name: 'Rillaboom', printed_number: '014/072', effective_set_code: 'swsh1', base_gv_id: 'GV-PK-SSH-14' },
  { name: 'Inteleon', printed_number: '058/202', effective_set_code: 'swsh1', base_gv_id: 'GV-PK-SSH-58' },
];
...
function makeInitialArtifact(candidateBatch) {
  return {
    ...
    selection_summary: {
      ...
      target_origin: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V20_NONBLOCKED',
    ...
    rows: candidateBatch.rows.map((row, index) => ({
      ...
      evidence_sources_v20_nonblocked: [],
      ...
      decision_code: row.decision_code ?? 'single_series_exact_identity_confirmed_by_v20_nonblocked',
      final_reason: null,
    ...
    recommended_next_execution_step: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V21_NONBLOCKED',
  };
}
...
async function populateEvidenceDetails(artifact) {
  ...
  for (const row of artifact.rows) {
    ...
    row.evidence_sources_v20_nonblocked =
      evidenceRow?.evidence_sources_used_for_v20_nonblocked ?? row.evidence_sources_v20_nonblocked ?? [];
}
}
...
- Promoted Prize Pack total before V20: ${artifact.post_batch_prize_pack_status.promoted_prize_pack_total_before_v20}
- Promoted Prize Pack total after V20: ${artifact.post_batch_prize_pack_status.promoted_prize_pack_total_after_v20}
...
async function finalizeArtifact(client, artifact) {
  ...
  artifact.post_batch_prize_pack_status = {
    remaining_wait_rows: Number(evidence?.remaining_backlog?.wait_for_more_evidence ?? 55),
    do_not_canon_rows: Number(evidence?.remaining_backlog?.do_not_canon_total_after_v20_nonblocked ?? 186),
    blocked_by_official_acquisition_rows: Number(
      evidence?.remaining_backlog?.blocked_by_official_acquisition_remaining ?? 47,
    ),
    promoted_prize_pack_total_before_v20: Number(
      evidence?.current_backlog?.promoted_prize_pack_total ?? 420,
    ),
    promoted_prize_pack_total_after_v20:
      Number(evidence?.current_backlog?.promoted_prize_pack_total ?? 420) + artifact.rows.length,
  };
}

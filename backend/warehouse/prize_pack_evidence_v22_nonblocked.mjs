...
const V21_EVIDENCE_PATH = path.join(
  CHECKPOINT_DIR,
...
const INPUT_JSON_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v22_nonblocked_input.json',
);
const TARGET_SLICE_JSON_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v22_nonblocked_target_slice.json',
);
const OUTPUT_JSON_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v22_nonblocked.json',
);
const OUTPUT_MD_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v22_nonblocked.md',
);
const READY_BATCH_CANDIDATE_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_ready_batch_v22_nonblocked_candidate.json',
);
...
const TARGET_SLICE = {
  id: 'MEGA_EVOLUTION_MYSTERY_GARDEN_SERIES_8_NEAR_HIT_AUDIT',
  label: 'Mega Evolution Mystery Garden Series 8 Near-Hit Audit',
  row_count: TARGET_SOURCE_EXTERNAL_IDS.size,
  set_code: 'me01',
  series_checked: [1, 2, 3, 4, 5, 6, 7, 8],
  shared_question:
    'Across all currently accessible Series 1-8 evidence, does this Mega Evolution printed identity appear exactly as 172/132, as a different-number near hit, or not at all?',
  selection_note:
    'This one-row micro-slice was selected because exact-hit rows are exhausted and this is the strongest remaining near-hit: accessible Series 8 sources show Mystery Garden in the same MEG set token, but at number 122 rather than the candidate printed identity 172/132.',
};
...
function buildCurrentWaitRows({
  ...
  const doNotCanonIds = new Set(
    [...(v21Evidence.do_not_canon_rows || [])]
  ...
  return stableSortRows(
    (v17Input.rows || [])
      ...
      .filter((row) => !doNotCanonIds.has(row.source_external_id))
      .map((row) => {
...
function findOfficialNearMatches(row, source, series) {
  ...
  for (const entry of source.entries ?? []) {
    const entryName = normalizeName(entry.name ?? entry.card_name ?? entry.base_card_name);
    if (entryName !== rowName) continue;

    const entryNumber = normalizeNumberPlain(
      entry.printed_number ?? entry.number ?? entry.number_plain,
    );
    const entrySetToken = entry.set_code ? String(entry.set_code).toUpperCase() : null;
    const exactNumber = entryNumber === rowNumber;
    const exactSet = !entrySetToken || entrySetToken === rowSetToken;
    if (exactNumber && exactSet) continue;
    ...
    near.push({
      ...
      local_json_path:
        series === 1
          ...
          : relativeCheckpointPath(SERIES_2_OFFICIAL_PATH),
      near_match_entry: {
        name: entry.name ?? entry.card_name ?? null,
        printed_number: entry.printed_number ?? entry.number ?? null,
        set_code: entry.set_code ?? null,
      },
      near_match_reason: 'name_seen_but_exact_set_token_or_number_not_matched',
      observed_set_token: entrySetToken,
      observed_number_plain: entryNumber,
  ...
  for (const source of BULBAPEDIA_SOURCES) {
    try {
      const response = await fetch(source.raw_url, {
        headers: {
          'user-agent': 'Mozilla/5.0 (compatible; GrookaiPrizePackEvidenceV22Nonblocked/1.0)',
        },
...
function buildFinalDecision(exactEvidence) {
  ...
  if (series.length > 1) {
    return {
      ...
      decision_code: 'multi_series_exact_identity_confirmed_by_v22_nonblocked',
      final_reason:
  ...
  if (series.length === 1) {
    return {
      ...
      decision_code: 'single_series_exact_identity_confirmed_by_v22_nonblocked',
      final_reason:
  ...
  return {
    ...
    final_decision: 'WAIT',
    decision_code: 'no_exact_printed_identity_match_after_v22_nonblocked',
    final_reason:
...
function chooseTargetSlice(currentWaitRows) {
  ...
  if (rows.length !== TARGET_SOURCE_EXTERNAL_IDS.size) {
    throw new Error(
      `V22 target slice is not reproducible: expected ${TARGET_SOURCE_EXTERNAL_IDS.size}, found ${rows.length}.`,
    );
  }
...
function buildTargetRows({ rows, series1, series2, series1Index, series2Index, fixture, bulbapediaPages }) {
  return stableSortRows(rows).map((row) => {
    ...
    const officialNearMatches = [
      ...findOfficialNearMatches(row, series1, 1),
      ...findOfficialNearMatches(row, series2, 2),
    ];
    ...
    return {
      ...
      evidence_sources_used_for_v22_nonblocked: exactEvidence,
      near_match_evidence_for_v22_nonblocked: [
        ...officialNearMatches,
        ...fixtureMatch.near,
...
function buildEvidenceSignalRows({
  rows,
  series1,
  series2,
  series1Index,
  ...
  return stableSortRows(rows).map((row) => {
    ...
    return {
      ...
      near_evidence_count: nearEvidence.length,
      has_any_exact_or_near_hit: exactEvidence.length > 0 || nearEvidence.length > 0,
      exact_series: [...new Set(exactEvidence.map((item) => item.series))].sort((a, b) => a - b),
      near_series: [...new Set(nearEvidence.map((item) => item.series))].sort((a, b) => a - b),
      strongest_signal:
        exactEvidence.length > 0
          ? 'EXACT_HIT'
          : nearEvidence.length > 0
            ? 'NEAR_HIT'
            : 'NO_ACCESSIBLE_HIT',
    };
  });
...
function buildReadyBatchCandidate(rows) {
  return stableSortRows(rows).map((row, index) => {
    ...
    return {
      ...
      evidence_sources_v22_nonblocked: row.evidence_sources_used_for_v22_nonblocked,
      ...
      reference_hints_payload: {
        provenance: 'prize_pack_evidence_v22_nonblocked',
        source_family: row.source_family ?? 'prize-pack-series-cards-pokemon',
        evidence_class: row.final_evidence_class,
        evidence_tier: row.final_evidence_tier,
        confirmed_series_coverage: row.confirmed_series_coverage,
...
function buildMarkdown(report) {
  const lines = [
    '# Prize Pack Evidence V22 Nonblocked',
    '',
...
async function main() {
  ...
  const currentBacklog = {
    ...
    do_not_canon_total:
      v21Evidence.remaining_backlog?.do_not_canon_total_after_v21_nonblocked ??
      v20Batch.post_batch_prize_pack_status?.do_not_canon_rows ??
      187,
  ...
  const inputArtifact = {
    ...
    workflow: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V22_NONBLOCKED',
    source_artifacts: [
      ...
      relativeCheckpointPath(V21_EVIDENCE_PATH),
      relativeCheckpointPath(SERIES_1_OFFICIAL_PATH),
      relativeCheckpointPath(SERIES_2_OFFICIAL_PATH),
  ...
  const evidenceSignalRows = buildEvidenceSignalRows({
    rows: nonblockedNoSeriesRows,
    series1,
    series2,
    series1Index,
    series2Index,
  ...
  const evidenceSignalCount = evidenceSignalRows.filter(
    (row) => row.has_any_exact_or_near_hit,
  ).length;

  inputArtifact.evidence_signal_summary = {
    rows_with_any_prior_exact_or_near_hit: evidenceSignalCount,
    rows_with_exact_hit: evidenceSignalRows.filter((row) => row.exact_evidence_count > 0).length,
    rows_with_near_hit_only: evidenceSignalRows.filter(
      (row) => row.exact_evidence_count === 0 && row.near_evidence_count > 0,
    ).length,
  };
  inputArtifact.rows = inputArtifact.rows.map((row) => ({
    ...row,
    evidence_signal:
      evidenceSignalRows.find((signalRow) => signalRow.source_external_id === row.source_external_id) ??
      null,
  }));
  ...
  const report = {
    ...
    remaining_backlog: {
      ...
      do_not_canon_total_after_v22_nonblocked:
    ...
    recommended_next_execution_step:
      readyRows.length > 0
        ? 'PRIZE_PACK_READY_BATCH_V22_NONBLOCKED'

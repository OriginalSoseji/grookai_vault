...
const INPUT_JSON_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v19_nonblocked_input.json',
);
const TARGET_SLICE_JSON_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v19_nonblocked_target_slice.json',
);
const OUTPUT_JSON_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v19_nonblocked.json',
);
const OUTPUT_MD_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v19_nonblocked.md',
);
...
const TARGET_SOURCE_EXTERNAL_IDS = new Set([
  'pokemon-prize-pack-series-cards-wormadam-010-rare',
  'pokemon-prize-pack-series-cards-wormadam-077-rare',
  'pokemon-prize-pack-series-cards-wormadam-098-rare',
]);

const TARGET_SLICE = {
  id: 'BRILLIANT_STARS_WORMADAM_SERIES_2_OFFICIAL_LOCAL_JSON_AUDIT',
  label: 'Brilliant Stars Wormadam Series 2 Official Local JSON Audit',
  row_count: TARGET_SOURCE_EXTERNAL_IDS.size,
  set_code: 'swsh9',
  series_checked: [1, 2, 3, 4, 5, 6, 7, 8],
  shared_question:
    'Across local official Series 1-2 JSON and accessible Series 3-8 sources, does each Brilliant Stars Wormadam printed identity appear in exactly one corroborated Prize Pack series?',
  selection_note:
    'This small Brilliant Stars slice was selected because all three Wormadam rows share the same name family and set, each has an exact Series 2 official local JSON hit by name, set token, and printed number, and checked Series 1 plus Series 3-8 sources do not provide a duplicate exact identity hit.',
};
...
function buildCurrentWaitRows({ v17Input, series1Batch, series2Batch, v18Batch }) {
  ...
  for (const source of BULBAPEDIA_SOURCES) {
    try {
      const response = await fetch(source.raw_url, {
        headers: {
          'user-agent': 'Mozilla/5.0 (compatible; GrookaiPrizePackEvidenceV19Nonblocked/1.0)',
        },
...
function buildFinalDecision(exactEvidence) {
  ...
  if (series.length > 1) {
    return {
      ...
      decision_code: 'multi_series_exact_identity_confirmed_by_v19_nonblocked',
      final_reason:
  ...
  if (series.length === 1) {
    return {
      ...
      decision_code: 'single_series_exact_identity_confirmed_by_v19_nonblocked',
      final_reason:
  ...
  return {
    ...
    final_decision: 'WAIT',
    decision_code: 'no_exact_printed_identity_match_after_v19_nonblocked',
    final_reason:
...
function chooseTargetSlice(currentWaitRows) {
  ...
  if (rows.length !== TARGET_SOURCE_EXTERNAL_IDS.size) {
    throw new Error(
      `V19 target slice is not reproducible: expected ${TARGET_SOURCE_EXTERNAL_IDS.size}, found ${rows.length}.`,
    );
  }
...
function buildTargetRows({ rows, series1, series2, series1Index, series2Index, fixture, bulbapediaPages }) {
  return stableSortRows(rows).map((row) => {
    ...
    return {
      ...
      evidence_sources_used_for_v19_nonblocked: exactEvidence,
      near_match_evidence_for_v19_nonblocked: [
        ...fixtureMatch.near,
      ...
      previous_evidence_pass_history: [
        ...
        'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V19_NONBLOCKED',
      ],
...
function buildReadyBatchCandidate(rows) {
  return stableSortRows(rows).map((row, index) => {
    ...
    return {
      ...
      evidence_sources_v19_nonblocked: row.evidence_sources_used_for_v19_nonblocked,
      ...
      reference_hints_payload: {
        provenance: 'prize_pack_evidence_v19_nonblocked',
        source_family: row.source_family ?? 'prize-pack-series-cards-pokemon',
        evidence_class: row.final_evidence_class,
        evidence_tier: row.final_evidence_tier,
        confirmed_series_coverage: row.confirmed_series_coverage,
...
function buildMarkdown(report) {
  const lines = [
    '# Prize Pack Evidence V19 Nonblocked',
    '',
...
async function main() {
  const [v17Input, v17, series1, series2, fixture, series1Batch, series2Batch, v18Batch] =
    await Promise.all([
  ...
  const currentWaitRows = buildCurrentWaitRows({ v17Input, series1Batch, series2Batch, v18Batch });
  ...
  const currentBacklog = {
    ...
    do_not_canon_total: v18Batch.post_batch_prize_pack_status?.do_not_canon_rows ?? 186,
    promoted_prize_pack_total:
      v18Batch.post_batch_prize_pack_status?.promoted_prize_pack_total_after_v18 ?? 417,
  };

  const inputArtifact = {
    generated_at: new Date().toISOString(),
    workflow: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V19_NONBLOCKED',
    source_artifacts: [
  ...
  const report = {
    ...
    remaining_backlog: {
      ...
      do_not_canon_total_after_v19_nonblocked:
    ...
    recommended_next_execution_step:
      readyRows.length > 0
        ...
        : 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V20_NONBLOCKED',
  };

...
const WORKFLOW = 'PRIZE_PACK_READY_BATCH_V11_NONBLOCKED';
...
const EVIDENCE_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_evidence_v11_nonblocked.json');
const OUTPUT_JSON_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_ready_batch_v11_nonblocked.json');
const OUTPUT_MD_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_ready_batch_v11_nonblocked.md');

const SOURCE_SET_ID = 'prize-pack-series-cards-pokemon';
const FOUNDER_EMAIL = 'ccabrl@gmail.com';
const FOUNDER_APPROVAL_NOTE =
  'PRIZE_PACK_READY_BATCH_V11_NONBLOCKED founder approval under GENERIC_PLAY_POKEMON_STAMP_IDENTITY_RULE_V1 review.';

const EXPECTED_ROWS = [
  { name: 'Basic Fire Energy', printed_number: '002', effective_set_code: 'sve', base_gv_id: 'GV-PK-SVE-2' },
  { name: 'Basic Fighting Energy', printed_number: '006', effective_set_code: 'sve', base_gv_id: 'GV-PK-SVE-6' },
  { name: 'Basic Darkness Energy', printed_number: '007', effective_set_code: 'sve', base_gv_id: 'GV-PK-SVE-7' },
  { name: 'Basic Metal Energy', printed_number: '008', effective_set_code: 'sve', base_gv_id: 'GV-PK-SVE-8' },
];
...
function makeInitialArtifact(candidateBatch) {
  return {
    ...
    selection_summary: {
      ...
      target_origin: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V11_NONBLOCKED',
    ...
    rows: candidateBatch.rows.map((row, index) => ({
      ...
      evidence_sources_v11_nonblocked: [],
...
async function populateEvidenceDetails(artifact) {
  ...
  for (const row of artifact.rows) {
    ...
    row.evidence_sources_v11_nonblocked =
      evidenceRow?.evidence_sources_used_for_v11_nonblocked ?? [];
  }
}
...
- Promoted Prize Pack total before V11: ${artifact.post_batch_prize_pack_status.promoted_prize_pack_total_before_v11}
- Promoted Prize Pack total after V11: ${artifact.post_batch_prize_pack_status.promoted_prize_pack_total_after_v11}
...
async function finalizeArtifact(client, artifact) {
  ...
  artifact.post_batch_prize_pack_status = {
    remaining_wait_rows: Number(evidence?.remaining_backlog?.wait_for_more_evidence ?? 131),
    do_not_canon_rows: Number(evidence?.remaining_backlog?.do_not_canon_total_after_v11_nonblocked ?? 174),
    blocked_by_official_acquisition_rows: Number(
      evidence?.remaining_backlog?.blocked_by_official_acquisition_remaining ?? 88,
    ),
    promoted_prize_pack_total_before_v11: Number(
      evidence?.current_backlog?.already_promoted_total ?? 361,
    ),
    promoted_prize_pack_total_after_v11:
      Number(evidence?.current_backlog?.already_promoted_total ?? 361) + artifact.rows.length,
  };
}

...
const WORKFLOW = 'PRIZE_PACK_READY_BATCH_V7_SOURCE_UPGRADE_SERIES_2';
...
const EVIDENCE_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_series_2_source_upgrade_fallback_v1.json');
const LATEST_STATE_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_series_2_source_upgrade_fallback_v1.json');
const SERIES_1_SOURCE_UPGRADE_BATCH_PATH = path.join(
  CHECKPOINT_DIR,
...
const OUTPUT_MD_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_ready_batch_v7_source_upgrade_series_2.md',
);

const SOURCE_SET_ID = 'prize-pack-series-cards-pokemon';
const FOUNDER_EMAIL = 'ccabrl@gmail.com';
const FOUNDER_APPROVAL_NOTE =
  'PRIZE_PACK_READY_BATCH_V7_SOURCE_UPGRADE_SERIES_2 founder approval under GENERIC_PLAY_POKEMON_STAMP_IDENTITY_RULE_V1 review.';

const EXPECTED_ROWS = [
  { name: "Oricorio", printed_number: "042/264", effective_set_code: "swsh8", base_gv_id: "GV-PK-FST-42" },
  { name: "Inteleon V", printed_number: "078/264", effective_set_code: "swsh8", base_gv_id: "GV-PK-FST-78" },
  { name: "Inteleon VMAX", printed_number: "079/264", effective_set_code: "swsh8", base_gv_id: "GV-PK-FST-79" },
  { name: "Boltund V", printed_number: "103/264", effective_set_code: "swsh8", base_gv_id: "GV-PK-FST-103" },
  { name: "Boltund VMAX", printed_number: "104/264", effective_set_code: "swsh8", base_gv_id: "GV-PK-FST-104" },
  { name: "Mew V", printed_number: "113/264", effective_set_code: "swsh8", base_gv_id: "GV-PK-FST-113" },
  { name: "Mew VMAX", printed_number: "114/264", effective_set_code: "swsh8", base_gv_id: "GV-PK-FST-114" },
  { name: "Meloetta", printed_number: "124/264", effective_set_code: "swsh8", base_gv_id: "GV-PK-FST-124" },
  { name: "Gengar V", printed_number: "156/264", effective_set_code: "swsh8", base_gv_id: "GV-PK-FST-156" },
  { name: "Gengar VMAX", printed_number: "157/264", effective_set_code: "swsh8", base_gv_id: "GV-PK-FST-157" },
  { name: "Genesect V", printed_number: "185/264", effective_set_code: "swsh8", base_gv_id: "GV-PK-FST-185" },
  { name: "Latias", printed_number: "193/264", effective_set_code: "swsh8", base_gv_id: "GV-PK-FST-193" },
  { name: "Latios", printed_number: "194/264", effective_set_code: "swsh8", base_gv_id: "GV-PK-FST-194" },
  { name: "Dunsparce", printed_number: "207/264", effective_set_code: "swsh8", base_gv_id: "GV-PK-FST-207" },
  { name: "Adventurer's Discovery", printed_number: "224/264", effective_set_code: "swsh8", base_gv_id: "GV-PK-FST-224" },
  { name: "Battle VIP Pass", printed_number: "225/264", effective_set_code: "swsh8", base_gv_id: "GV-PK-FST-225" },
  { name: "Cram-o-matic", printed_number: "229/264", effective_set_code: "swsh8", base_gv_id: "GV-PK-FST-229" },
  { name: "Elesa's Sparkle", printed_number: "233/264", effective_set_code: "swsh8", base_gv_id: "GV-PK-FST-233" },
  { name: "Power Tablet", printed_number: "236/264", effective_set_code: "swsh8", base_gv_id: "GV-PK-FST-236" },
  { name: "Fusion Strike Energy", printed_number: "244/264", effective_set_code: "swsh8", base_gv_id: "GV-PK-FST-244" },
  { name: "Leafeon VSTAR", printed_number: "SWSH195", effective_set_code: "swshp", base_gv_id: "GV-PK-PR-SW-SWSH195" },
  { name: "Glaceon VSTAR", printed_number: "SWSH197", effective_set_code: "swshp", base_gv_id: "GV-PK-PR-SW-SWSH197" },
];
...
async function loadAndValidateCandidateBatch() {
  ...
  if (!rows || rows.length !== EXPECTED_ROWS.length) {
  ...
  const expectedByKey = new Map(EXPECTED_ROWS.map((row) => [buildExpectedKey(row), row]));
  ...
  for (const row of rows) {
    const expected = expectedByKey.get(buildExpectedKey(row));
    const rowBaseGvId =
      normalizeTextOrNull(row.underlying_base_proof?.base_gv_id) ?? normalizeTextOrNull(row.base_gv_id);
    if (expected?.base_gv_id !== rowBaseGvId) {
      throw new Error(`candidate_batch_base_gv_id_mismatch:${row.source_external_id}:${rowBaseGvId}`);
    }
    ...
    const supportedSeries = Array.isArray(row.supported_series_list)
      ? row.supported_series_list
      : Array.isArray(row.confirmed_series_coverage)
        ? row.confirmed_series_coverage
        : [];
    if (supportedSeries.length !== 1 || Number(supportedSeries[0]) !== 2) {
      throw new Error(`candidate_batch_series_mismatch:${row.source_external_id}`);
    }
    const governingRule =
      normalizeTextOrNull(row.governing_rule_source) ?? normalizeTextOrNull(row.governing_rule);
    if (governingRule !== 'GENERIC_PLAY_POKEMON_STAMP_IDENTITY_RULE_V1') {
      throw new Error(`candidate_batch_rule_mismatch:${row.source_external_id}`);
    }
    const finalDecision = normalizeTextOrNull(row.final_decision);
    const isReadyShape =
      finalDecision === 'READY_FOR_WAREHOUSE' ||
      (normalizeTextOrNull(row.evidence_class) === 'CONFIRMED_IDENTITY' &&
        normalizeTextOrNull(row.evidence_tier) === 'TIER_1');
    if (!isReadyShape) {
      throw new Error(`candidate_batch_decision_mismatch:${row.source_external_id}`);
    }
    if (normalizeTextOrNull(row.source_upgrade_series_2_status) !== 'tier_upgraded_to_tier_1') {
      throw new Error(`candidate_batch_source_upgrade_mismatch:${row.source_external_id}`);
    }
    const officialSource = (row.evidence_sources_v7_source_upgrade_series_2 || []).find(
      (source) =>
        normalizeTextOrNull(source.source_type) ===
          'official_checklist_json_import_via_archive_official_url_capture' &&
        Number(source.series) === 2 &&
        normalizeTextOrNull(source.evidence_tier) === 'TIER_1',
    );
    if (!officialSource) {
      throw new Error(`candidate_batch_official_series_2_provenance_missing:${row.source_external_id}`);
    }
  }
...
async function fetchLivePrizePackMappedTotal(client) {
  const row = await fetchOne(
    client,
    `
      select count(distinct mapping.card_print_id)::int as total
      from public.external_mappings mapping
      where mapping.source = 'justtcg'
        and mapping.active = true
        and exists (
          select 1
          from public.external_discovery_candidates candidate
          where candidate.source = mapping.source
            and candidate.upstream_id = mapping.external_id
            and candidate.set_id = $1
        )
    `,
    [SOURCE_SET_ID],
  );
...
function makeInitialArtifact(candidateBatch) {
  return {
    ...
    rows: candidateBatch.rows.map((row, index) => ({
      ...
      governing_rule_source: row.governing_rule_source ?? row.governing_rule,
      governing_rules: [
        'STAMPED_IDENTITY_RULE_V1',
        'GENERIC_PLAY_POKEMON_STAMP_IDENTITY_RULE_V1',
        'EVIDENCE_TIER_V1',
      ],
      source_family: row.source_family ?? row.reference_hints_payload?.source_family ?? null,
      evidence_class: row.evidence_class,
      evidence_tier: row.evidence_tier,
      supported_series_list: row.supported_series_list ?? row.confirmed_series_coverage,
      evidence_sources_v7_source_upgrade_series_2:
        row.evidence_sources_v7_source_upgrade_series_2 ?? [],
      source_upgrade_series_2_status: row.source_upgrade_series_2_status ?? null,
      source_upgrade_series_2_reason: row.final_reason ?? null,
      ...
      decision_code: 'series_2_official_fallback_checklist_tier_1_source_upgrade',
      final_reason:
        row.final_reason ??
        'Matched locally imported official Pokemon.com Prize Pack Series 2 checklist data through an archived official URL capture.',
    ...
    recommended_next_execution_step: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V18_NONBLOCKED',
  };
}
...
async function populateEvidenceDetails(artifact) {
  ...
  for (const row of artifact.rows) {
    ...
    row.evidence_sources_v7_source_upgrade_series_2 =
      evidenceRow?.evidence_sources_v7_source_upgrade_series_2 ??
      row.evidence_sources_v7_source_upgrade_series_2 ??
      [];
    row.source_upgrade_series_2_status =
      evidenceRow?.source_upgrade_series_2_status ?? row.source_upgrade_series_2_status;
    row.source_upgrade_series_2_reason = evidenceRow?.final_reason ?? row.source_upgrade_series_2_reason;
  }
}
...
- Remaining WAIT rows before Series 2 source-upgrade batch: ${artifact.post_batch_prize_pack_status.remaining_wait_rows_before_series_2_source_upgrade_batch}
- Remaining WAIT rows after Series 2 source-upgrade batch: ${artifact.post_batch_prize_pack_status.remaining_wait_rows_after_series_2_source_upgrade_batch}
- DO_NOT_CANON rows: ${artifact.post_batch_prize_pack_status.do_not_canon_rows}
- Blocked-by-official-acquisition rows before Series 2 source-upgrade batch: ${artifact.post_batch_prize_pack_status.blocked_by_official_acquisition_rows_before_series_2_source_upgrade_batch}
- Blocked-by-official-acquisition rows after Series 2 source-upgrade batch: ${artifact.post_batch_prize_pack_status.blocked_by_official_acquisition_rows_after_series_2_source_upgrade_batch}
- Promoted Prize Pack total before Series 2 source-upgrade batch: ${artifact.post_batch_prize_pack_status.promoted_prize_pack_total_before_series_2_source_upgrade_batch}
- Promoted Prize Pack total after Series 2 source-upgrade batch: ${artifact.post_batch_prize_pack_status.promoted_prize_pack_total_after_series_2_source_upgrade_batch}
- Series 2 official acquisition: ${artifact.post_batch_prize_pack_status.series_2_official_acquisition_status}
...
async function finalizeArtifact(client, artifact) {
  const latestState = await readJson(LATEST_STATE_PATH);
  const series1SourceUpgradeBatch = await maybeReadJson(SERIES_1_SOURCE_UPGRADE_BATCH_PATH);
  ...
  const livePromotedPrizePackTotal = await fetchLivePrizePackMappedTotal(client);
  const latestRemaining = latestState?.remaining_backlog ?? {};
  const latestCurrent =
    latestState?.current_backlog ?? latestState?.input_summary?.current_backlog ?? {};
  const series1PostStatus = series1SourceUpgradeBatch?.post_batch_prize_pack_status ?? {};
  const waitBefore = Number(
    series1PostStatus.remaining_wait_rows_after_series_1_source_upgrade_batch ??
      latestRemaining.wait_for_more_evidence ??
      latestCurrent.wait_for_more_evidence ??
      85,
  );
  const acquisitionBlockedBefore = Number(
    series1PostStatus.blocked_by_official_acquisition_rows_after_series_1_source_upgrade_batch ??
      latestRemaining.blocked_by_official_acquisition_remaining ??
      latestCurrent.blocked_by_official_acquisition ??
      69,
  );
  const doNotCanonRows = Number(
    latestRemaining.do_not_canon_total_after_v17_nonblocked ??
      latestCurrent.do_not_canon_total ??
      186,
  );
  ...
  artifact.post_batch_prize_pack_status = {
    remaining_wait_rows_before_series_2_source_upgrade_batch: waitBefore,
    remaining_wait_rows_after_series_2_source_upgrade_batch: Math.max(0, waitBefore - artifact.rows.length),
    do_not_canon_rows: doNotCanonRows,
    blocked_by_official_acquisition_rows_before_series_2_source_upgrade_batch: acquisitionBlockedBefore,
    blocked_by_official_acquisition_rows_after_series_2_source_upgrade_batch: Math.max(
      0,
      acquisitionBlockedBefore - artifact.rows.length,
    ),
    promoted_prize_pack_total_before_series_2_source_upgrade_batch: Math.max(
      0,
      livePromotedPrizePackTotal - artifact.rows.length,
    ),
    promoted_prize_pack_total_after_series_2_source_upgrade_batch: livePromotedPrizePackTotal,
    series_2_official_acquisition_status: 'VALIDATED_FALLBACK_IMPORT_EXECUTED',
  };
}

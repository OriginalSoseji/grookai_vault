...
const V14_INPUT_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v14_nonblocked_input.json',
);
const V14_PATH = path.join(CHECKPOINT_DIR, 'prize_pack_evidence_v14_nonblocked.json');
const ROUTE_REPAIR_V3_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_base_route_repair_v3.json',
);
const ROUTE_REPAIR_V6_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_base_route_repair_v6.json',
);
...
const TARGET_SLICE_JSON_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v15_nonblocked_target_slice.json',
);
const OUTPUT_JSON_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v15_nonblocked.json',
);
const OUTPUT_MD_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_evidence_v15_nonblocked.md',
);
const READY_BATCH_CANDIDATE_PATH = path.join(
  CHECKPOINT_DIR,
  'prize_pack_ready_batch_v15_nonblocked_candidate.json',
);
...
const ACQUISITION_BLOCK_REASON =
  'Excluded from V15 because the next truthful step depends on blocked official Series 1/2 checklist acquisition.';
const SPECIAL_FAMILY_EXCLUSION_REASON =
  'Excluded from V15 because it is the remaining SPECIAL_IDENTITY_FAMILY_COLLISION row, not part of this evidence slice.';
...
const TARGET_SOURCE_EXTERNAL_IDS = new Set([
  'pokemon-prize-pack-series-cards-basic-fire-energy-mee002-common',
  'pokemon-prize-pack-series-cards-basic-fighting-energy-mee006-common',
  'pokemon-prize-pack-series-cards-basic-darkness-energy-mee007-common',
  'pokemon-prize-pack-series-cards-basic-metal-energy-mee008-common',
]);

const TARGET_SLICE = {
  id: 'SVE_MEE_ENERGY_MULTI_SERIES_DUPLICATE_WINDOW',
  label: 'Route-Normalized MEE/SVE Basic Energy Series 4-8 Window',
  series: [3, 4, 5, 6, 7, 8],
  source_family: 'prize-pack-series-cards-pokemon',
  shared_question:
    'Across accessible Prize Pack Series 3-8 sources, do these route-normalized MEE/SVE Basic Energy rows appear in exactly one corroborated series or multiple?',
};
...
const SET_MATCH_TOKENS_BY_CODE = {
  sve: ['sve basic energies', 'scarlet & violet energies'],
};
...
function routeOwnerFromRepairRow(row) {
  const route = row.resolved_base_route ?? row.candidate_base_routes?.[0] ?? null;
  if (!route?.gv_id) return null;
  return {
    source_external_id: row.source_external_id,
    candidate_name: cleanDisplayName(row.name ?? row.candidate_name),
    printed_number: row.printed_number,
    normalized_number_plain:
      row.normalized_number_plain ?? normalizeNumberPlain(row.printed_number),
    effective_base_owner_gv_id: route.gv_id,
    base_gv_id: route.gv_id,
    base_route: route.gv_id,
    effective_set_code: route.set_code ?? null,
    effective_set_name: SET_NAME_BY_CODE[route.set_code] ?? null,
    unique_base_route: true,
    route_repair_history: row.previous_route_repair_history ?? [
      'route_resolved_by_prior_base_route_repair',
    ],
  };
}

function buildRouteOwnerMap(...repairArtifacts) {
  const map = new Map();
  for (const artifact of repairArtifacts) {
    for (const row of artifact.route_resolved_wait_rows ?? []) {
      const owner = routeOwnerFromRepairRow(row);
      if (owner) map.set(owner.source_external_id, owner);
    }
  }
  return map;
}

function buildCurrentWaitRows({ v14Input, v14, routeOwnerMap }) {
  const removeIds = new Set(
    [
      ...(v14.ready_rows || []).map((row) => row.source_external_id),
      ...(v14.do_not_canon_rows || []).map((row) => row.source_external_id),
    ].filter(Boolean),
  );
  ...
  return stableSortRows(
    (v14Input.rows || [])
      ...
      .map((row) => {
        const routeOwner = routeOwnerMap.get(row.source_external_id) ?? {};
        const effectiveSetCode = row.set_code ?? routeOwner.effective_set_code ?? null;
        const name = cleanDisplayName(
          row.candidate_name ?? routeOwner.candidate_name ?? row.name ?? row.base_card_name,
        );
        ...
        return {
          ...
          candidate_name: name,
          name,
          base_card_name: row.base_card_name ?? routeOwner.candidate_name ?? name,
          printed_number: row.printed_number ?? routeOwner.printed_number,
          normalized_number_plain:
            row.normalized_number_plain ??
            routeOwner.normalized_number_plain ??
            normalizeNumberPlain(row.printed_number ?? routeOwner.printed_number),
          base_gv_id:
            row.base_gv_id ??
            row.effective_base_owner_gv_id ??
            routeOwner.base_gv_id ??
            null,
          base_route:
            row.base_route ??
            row.base_gv_id ??
            row.effective_base_owner_gv_id ??
            routeOwner.base_route ??
            null,
          effective_base_owner_gv_id:
            row.effective_base_owner_gv_id ??
            row.base_gv_id ??
            routeOwner.effective_base_owner_gv_id ??
            null,
          effective_set_code: effectiveSetCode,
          effective_set_name:
            row.effective_set_name ??
            routeOwner.effective_set_name ??
            SET_NAME_BY_CODE[effectiveSetCode] ??
            null,
          ...
          prior_evidence_pass_history:
            ...
            [],
          prior_route_repair_history: routeOwner.route_repair_history ?? [],
          ...
          unique_base_route: Boolean(
            row.effective_base_owner_gv_id ?? row.base_gv_id ?? routeOwner.effective_base_owner_gv_id,
          ),
...
async function fetchSeriesPage(source) {
  const response = await fetch(source.raw_url, {
    headers: {
      'user-agent': 'Mozilla/5.0 (compatible; GrookaiPrizePackEvidenceV15Nonblocked/1.0)',
    },
  });
  if (!response.ok) {
...
function getSetMatchTokens(row) {
  const explicitTokens = SET_MATCH_TOKENS_BY_CODE[row.effective_set_code] ?? [];
  const tokens = new Set(explicitTokens.map(normalizeText));
...
function chooseTargetSlice(waitRows) {
  ...
  if (rows.length !== TARGET_SOURCE_EXTERNAL_IDS.size) {
    throw new Error(
      `V15 target slice is not reproducible: expected ${TARGET_SOURCE_EXTERNAL_IDS.size}, found ${rows.length}.`,
    );
  }
  ...
  return {
    ...
    selection_note:
      'This four-row target is the largest remaining same-family known-route nonblocked slice that can be decided from Series 3-8 evidence without official Series 1/2 acquisition.',
  };
}
...
function buildFinalDecision(matchedSeries) {
  if (matchedSeries.length > 1) {
    return {
      ...
      decision_code: 'multi_series_duplicate_confirmed_by_v15_nonblocked_sve_energy_window',
      final_reason:
        'Accessible evidence places this route-normalized SVE Basic Energy printed identity in multiple Prize Pack series with no printed distinction, so the generic Play! Pokemon stamp is distribution-only for this source row.',
    };
  }
  ...
  if (matchedSeries.length === 1) {
    return {
      ...
      decision_code: 'single_accessible_series_confirmed_by_v15_nonblocked_sve_energy_window',
      final_reason:
        'Accessible evidence places this route-normalized SVE Basic Energy row in exactly one corroborated Prize Pack series, so the generic Play! Pokemon stamp resolves to one deterministic stamped identity.',
    };
  }
  ...
  return {
    ...
    final_decision: 'WAIT',
    decision_code: 'no_accessible_series_match_after_v15_nonblocked',
    final_reason:
      'Accessible Series 3-8 sources do not place this row in one deterministic Prize Pack series, so it remains in the evidence backlog.',
  };
}
...
function buildTargetSliceRows(rows, seriesPages) {
  ...
  return stableSortRows(rows).map((row) => {
    ...
    return {
      ...
      evidence_sources_used_for_v15_nonblocked: match.matching_evidence,
...
function representativeRows({
  ...
  return [
    ...
    ...doNotCanonRows.slice(0, 4).map((row) => ({
      bucket: 'DO_NOT_CANON',
      name: cleanDisplayName(row.base_card_name ?? row.candidate_name ?? row.name),
      printed_number: row.printed_number,
      set_code: row.effective_set_code,
...
function buildMarkdown(report) {
  const lines = [
    '# Prize Pack Evidence V15 Nonblocked',
    '',
...
async function main() {
  const [v14Input, v14, routeRepairV3, routeRepairV6] = await Promise.all([
    readJson(V14_INPUT_PATH),
    readJson(V14_PATH),
    readJson(ROUTE_REPAIR_V3_PATH),
    readJson(ROUTE_REPAIR_V6_PATH),
  ]);

  const routeOwnerMap = buildRouteOwnerMap(routeRepairV3, routeRepairV6);
  const currentWaitRows = buildCurrentWaitRows({ v14Input, v14, routeOwnerMap });
  ...
  const inputArtifact = {
    ...
    workflow: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V15_NONBLOCKED',
    source_artifacts: [
      relativeCheckpointPath(V14_INPUT_PATH),
      relativeCheckpointPath(V14_PATH),
    ...
    current_backlog: {
      ...
      do_not_canon_total: v14.remaining_backlog?.do_not_canon_total_after_v14_nonblocked ?? 181,
      already_promoted_total: v14.remaining_backlog?.promoted_prize_pack_total ?? 373,
    },
  ...
  const report = {
    generated_at: new Date().toISOString(),
    workflow: 'PRIZE_PACK_FAMILY_ONLY_EVIDENCE_STRATEGY_V15_NONBLOCKED',
    scope: 'One bounded Prize Pack slice using currently accessible evidence only after V14 rebucketing.',
    source_artifacts: [
      relativeCheckpointPath(V14_INPUT_PATH),
      relativeCheckpointPath(V14_PATH),
      relativeCheckpointPath(ROUTE_REPAIR_V3_PATH),
    ...
    remaining_backlog: {
      ...
      do_not_canon_total_after_v15_nonblocked:
    ...
    recommended_next_execution_step:
      readyRows.length > 0
        ? 'PRIZE_PACK_READY_BATCH_V15_NONBLOCKED'

...
.evidence-label{display:block;margin-bottom:3px;color:var(--muted);font-size:11px;font-weight:700;text-transform:uppercase}.facts-button{width:100%;margin:0 0 8px;text-align:left;border-color:#7f9990;color:#154c3a}.facts-button:disabled{color:var(--muted);border-color:var(--line);cursor:not-allowed}dialog{width:min(1320px,96vw);max-height:92vh;padding:0;border:1px solid #8d989d;border-radius:6px;background:#fff;color:var(--ink)}dialog::backdrop{background:rgba(13,20,23,.58)}.facts-head{position:sticky;top:0;z-index:2;display:grid;grid-template-columns:1fr auto;gap:12px;align-items:start;padding:14px 18px;background:#fff;border-bottom:1px solid var(--line)}.facts-head h2{margin:0;font-size:20px}.facts-body{padding:16px 18px;overflow:auto}.facts-summary{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:8px;margin-bottom:14px}.facts-summary div{padding:8px;border-left:3px solid var(--green);background:#f1f4f2}.digest{padding:10px 12px;margin-bottom:12px;background:#f6f7f5;border-left:3px solid var(--blue)}.fact-section{border-top:1px solid var(--line);padding:9px 0}.fact-section summary{cursor:pointer;font-weight:700}.fact-section pre{overflow:auto;max-height:520px;margin:8px 0 0;padding:10px;background:#f5f6f4;border:1px solid var(--line);font:12px/1.45 ui-monospace,SFMono-Regular,Consolas,monospace;white-space:pre-wrap;overflow-wrap:anywhere}.fact-table-wrap{overflow:auto;margin-top:8px}.fact-table{width:100%;border-collapse:collapse;font-size:12px}.fact-table th,.fact-table td{padding:6px 8px;border:1px solid var(--line);text-align:left;vertical-align:top}.fact-table th{background:#eef1ef}.facts-provenance{margin-top:12px;color:var(--muted);font-size:12px;overflow-wrap:anywhere}@media(max-width:800px){dialog{width:100vw;max-width:none;max-height:100vh;height:100vh;border-radius:0}.facts-head{grid-template-columns:1fr auto}.facts-body{padding:12px}}
</style>
</head>
<body>
...
function resultHtml(q,item,isSource=false){const s=stateFor(q);const key=isSource?'source':item.artwork_group_id;const j=isSource?s.source:(s.results[key]||(s.results[key]={judgment:'',exclusion_reason:'',notes:''}));const evidence=isSource?'Source-derived candidate; human judgment required.':item.evidence.map(e=>e.query_concept+': '+e.term).join(' | ');const source=sourceRecordFor(item);const facts=source?'<button type="button" class="facts-button" data-facts-card-id="'+h(item.representative_card_print_id)+'">Full saved Fact Graph · '+factCount(item,'observations')+' observations · '+factCount(item,'typed_facts')+' typed facts</button>':'<button type="button" class="facts-button" disabled>Saved Fact Graph unavailable</button>';return '<article class="result '+(isSource||item.artwork_group_id===q.source_candidate?.artwork_group_id?'expected':'')+'"><div class="image">'+imageHtml(item)+'</div><div class="content"><div class="rank">'+(isSource?'Source candidate':'Rank '+item.rank)+'</div><h3>'+h(item.name||item.gv_id||item.artwork_group_id)+'</h3><div class="small">'+h((item.gv_id||'')+' '+(item.set_code||'')+' '+(item.number||'')+(item.eligibility_tier?' | Tier '+item.eligibility_tier:''))+'</div><div class="evidence"><span class="evidence-label">Search match evidence only</span>'+h(evidence||'No matched evidence')+'</div>'+facts+'<div class="judgment"><select data-kind="'+(isSource?'source':'result')+'" data-key="'+h(key)+'">'+options(j.judgment)+'</select><textarea data-note="'+(isSource?'source':'result')+'" data-key="'+h(key)+'" placeholder="Evidence note">'+h(j.notes||'')+'</textarea></div></div></article>'}
...
function openFacts(cardId){const source=packet.saved_visual_records_by_card_id?.[cardId];if(!source)return;const row=source.generated_row||{},graph=row.visual_attributes?.fact_graph||{};document.getElementById('factsTitle').textContent=(row.name||row.gv_id||cardId)+' · complete saved visual record';document.getElementById('factsSubtitle').textContent=[row.gv_id,cardId,row.set_code,row.number].filter(Boolean).join(' · ');const summary=[['Schema',row.visual_attributes?.fact_schema_version||row.output_schema_version],['Prompt',row.prompt_version],['Model',row.model_version],['Review status',row.review_status],['Observations',Array.isArray(graph.observations)?graph.observations.length:0],['Typed facts',Array.isArray(graph.typed_facts)?graph.typed_facts.length:0]].map(([label,value])=>'<div><strong>'+h(label)+'</strong><br>'+h(value??'not saved')+'</div>').join('');const sections=Object.entries(graph).map(([key,value])=>graphSection(key,value)).join('');document.getElementById('factsBody').innerHTML='<div class="facts-summary">'+summary+'</div><div class="digest"><span class="evidence-label">Deterministic compatibility digest</span>'+h(row.artwork_description||'No compatibility digest saved.')+'</div><h3>Complete Fact Graph</h3>'+sections+'<details class="fact-section"><summary>Exact saved generated row JSON</summary><pre>'+pretty(row)+'</pre></details><div class="facts-provenance">Source: '+h(source.source_artifact_path||'unknown')+'<br>Generated row SHA-256: '+h(source.generated_row_sha256||'')+'<br>Source record SHA-256: '+h(source.source_record_sha256||'')+'</div>';document.getElementById('factsDialog').showModal()}
...
function bind(q,s){document.querySelectorAll('select[data-kind="result"]').forEach(el=>el.onchange=()=>{const x=s.results[el.dataset.key]||(s.results[el.dataset.key]={});x.judgment=el.value;persist()});document.querySelectorAll('textarea[data-note="result"]').forEach(el=>el.oninput=()=>{const x=s.results[el.dataset.key]||(s.results[el.dataset.key]={});x.notes=el.value;persist()});document.querySelectorAll('[data-kind="source"]').forEach(el=>el.onchange=()=>{s.source.judgment=el.value;persist()});document.querySelectorAll('[data-note="source"]').forEach(el=>el.oninput=()=>{s.source.notes=el.value;persist()});document.querySelectorAll('[data-failure]').forEach(el=>el.onchange=()=>{s.review.failure_labels=[...document.querySelectorAll('[data-failure]:checked')].map(x=>x.dataset.failure);persist()});document.querySelectorAll('[data-facts-card-id]').forEach(el=>el.onclick=()=>openFacts(el.dataset.factsCardId));document.getElementById("queryDecision").onchange=e=>{s.review.query_decision=e.target.value;persist()};document.getElementById("queryNotes").oninput=e=>{s.review.notes=e.target.value;persist()};document.getElementById("complete").onclick=()=>{s.review.completed_at=s.review.completed_at?null:new Date().toISOString();persist();if(s.review.completed_at){index=Math.min(index+1,filtered().length-1)}render()}}

...
import {
  VISUAL_DESCRIPTOR_V1,
  computeVisualDescriptorFromBuffer,
  rankVisualCandidates,
} from './lib/visual_descriptor_v1.mjs';

const DEFAULT_QUERY_DIR = '.tmp/scanner_v3_normalization_proof';
const DEFAULT_LABELS = '.tmp/embedding_test_images/results.json';
const DEFAULT_OUTPUT_DIR = '.tmp/scanner_v3_identity_bridge_v2';
const DEFAULT_V1_SUMMARY = '.tmp/scanner_v3_identity_bridge_v1/summary.json';
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (name === '--query' || name === '--input' || name === '--folder') {
      ...
      args.out = nextValue();
    } else if (name === '--v1-summary') {
      args.v1Summary = nextValue();
    } else if (name === '--top') {
...
function usage() {
  return [
    'Usage:',
    '  node backend/identity_v3/run_identity_bridge_v2_visual_descriptor_funnel.mjs [--query <scanner_output>] [--references <folder_or_manifest>] [--out <folder>]',
    '',
    'Defaults:',
    `  --query ${DEFAULT_QUERY_DIR}`,
    `  --labels ${DEFAULT_LABELS}`,
    ...
    '',
    'This harness returns visual descriptor candidates only. It does not make final identity decisions.',
  ].join('\n');
}
...
async function main() {
  ...
  if (queryDiscovery.items.length === 0) {
    throw new Error(`identity_bridge_v2_no_scanner_queries:${path.resolve(args.query)}`);
  }
  ...
  if (referencePlan.entries.length === 0) {
    throw new Error('identity_bridge_v2_no_reference_entries');
  }
  ...
  const descriptorIndex = await loadOrBuildDescriptorIndex({
  ...
  const v1Baseline = await loadV1Baseline(args.v1Summary);
  const matchResults = [];
  for (const query of queryDescriptors) {
    const candidates = rankVisualCandidates({
      query,
    ...
    const v1Result = v1Baseline.byQuery.get(query.query_id) ?? null;
    ...
    const result = {
      ...
      descriptor_version: VISUAL_DESCRIPTOR_V1.name,
      descriptor_features: descriptorFeatureSummary(),
      candidates,
      evaluation,
      v1_baseline: v1Result ? {
        correct_top1: v1Result.correct_top1,
        correct_in_top5: v1Result.correct_in_top5,
        expected_rank: v1Result.expected_rank,
        top_candidate: v1Result.candidates?.[0] ?? null,
      } : null,
  ...
  for (const result of matchResults.slice(0, 5)) {
    console.log(JSON.stringify({
      ...
      top1_score: result.candidates[0]?.final_score ?? null,
      ...
      candidates: result.candidates.slice(0, 5).map((candidate) => ({
        ...
        full_card_score: candidate.full_card_score,
        artwork_score: candidate.artwork_score,
      })),
    }));
  }
...
async function loadOrBuildDescriptorIndex({ entries, cachePath, referenceFingerprint, rebuildIndex }) {
  if (!rebuildIndex && await pathExists(cachePath)) {
    const cached = JSON.parse(await readFile(cachePath, 'utf8'));
    if (
      cached?.descriptor_version === VISUAL_DESCRIPTOR_V1.name &&
  ...
  const payload = {
    ...
    descriptor_version: VISUAL_DESCRIPTOR_V1.name,
    descriptor_config: VISUAL_DESCRIPTOR_V1,
...
async function descriptorForReference(entry) {
  ...
  if (!cardId) throw new Error('visual_reference_missing_card_id');
  const fullPath = entry.full_image_path ?? entry.image_path;
  if (!fullPath) throw new Error(`visual_reference_missing_image:${cardId}`);
  ...
  const [fullDescriptor, artworkDescriptor] = await Promise.all([
    computeVisualDescriptorFromBuffer(fullBuffer),
    computeVisualDescriptorFromBuffer(artworkBuffer),
  ]);
  ...
  return {
    ...
    full_card_descriptor: fullDescriptor,
    artwork_descriptor: artworkDescriptor,
  };
}

async function descriptorForQuery(item) {
  const fullBuffer = await readFile(item.full_image_path);
  const artworkBuffer = await readFile(item.art_image_path);
  const [fullDescriptor, artworkDescriptor] = await Promise.all([
    computeVisualDescriptorFromBuffer(fullBuffer),
    computeVisualDescriptorFromBuffer(artworkBuffer),
  ]);
  ...
  return {
    ...
    source_dir: item.source_dir,
    normalized_full_card_path: item.full_image_path,
...
async function discoverScannerQueries(inputDir, labelIndex) {
  ...
  for (const dir of candidateDirs) {
    const fullChoice = await firstExistingFileChoice(dir, [
    ...
    if (!fullChoice || !artChoice) {
      skipped.push({
        source_dir: dir,
        reason: !fullChoice ? 'missing_normalized_full_card' : 'missing_artwork_region',
      });
      continue;
    }
    ...
    items.push({
      ...
      source_dir: dir,
      full_image_path: fullChoice.path,
      art_image_path: artChoice.path,
      artifact_fallbacks: {
        full_card: fullChoice.name.includes('_color') ? null : 'color_full_card_missing_used_available_normalized',
        artwork: artChoice.name.includes('_color') ? null : 'color_artwork_missing_used_available_artwork',
      },
...
async function discoverCandidateDirs(rootDir) {
  const directFull = await firstExistingFileChoice(rootDir, [
    'normalized_full_card_color.png',
    'normalized_full_card.jpg',
  ]);
...
async function downloadReferenceImage({ url, cacheDir, cardId, timeoutMs }) {
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        'user-agent': 'grookai-identity-v3-visual-descriptor-funnel/1.0',
      },
    });
    if (!response.ok) {
...
function buildSummary({
  ...
  const improvements = evaluated
    .filter((result) => {
      const v1 = result.v1_baseline;
      if (!v1) return false;
      if (result.evaluation.correct_in_top5 && !v1.correct_in_top5) return true;
      if (Number.isFinite(result.evaluation.expected_rank) && Number.isFinite(v1.expected_rank)) {
        return result.evaluation.expected_rank < v1.expected_rank;
      }
      return false;
    })
  ...
  return {
    generated_at: new Date().toISOString(),
    harness: 'identity_bridge_v2_visual_descriptor_funnel',
    proof_only: true,
    final_identity_decision: false,
    descriptor_version: VISUAL_DESCRIPTOR_V1.name,
    descriptor_features: descriptorFeatureSummary(),
    ...
    reference_index_count: descriptorIndex.references.length,
    reference_index_cache_reused: descriptorIndex.cache_reused === true,
    reference_skipped_count: descriptorIndex.skipped_count + (referencePlan.skipped_downloads?.length ?? 0),
    v1_baseline: v1Baseline.summary ? {
      top1_accuracy: v1Baseline.summary.top1_accuracy,
      top5_accuracy: v1Baseline.summary.top5_accuracy,
      evaluated_tests: v1Baseline.summary.evaluated_tests,
      reference_index_count: v1Baseline.summary.reference_index_count,
    } : null,
    v2_top1_accuracy: evaluated.length ? round6(top1Correct / evaluated.length) : null,
    v2_top5_accuracy: evaluated.length ? round6(top5Correct / evaluated.length) : null,
    v2_top10_accuracy: evaluated.length ? round6(top10Correct / evaluated.length) : null,
    ...
    average_top1_top2_score_gap: average(gaps),
    failure_cases: evaluated
      .filter((result) => !result.evaluation.correct_in_top10)
      .map(compactResultForSummary),
    examples_where_v2_improved_over_v1: improvements,
    examples_where_v2_failed: evaluated
      .filter((result) => !result.evaluation.correct_in_top5)
      .slice(0, 8)
      .map(compactResultForSummary),
    examples: matchResults.slice(0, 5).map(compactResultForSummary),
...
function compactResultForSummary(result) {
  return {
    ...
    v1_baseline: result.v1_baseline,
    candidates: result.candidates.slice(0, 5).map((candidate) => ({
      card_id: candidate.card_id,
...
async function loadV1Baseline(summaryPath) {
  ...
  for (const example of summary.examples ?? []) {
    byQuery.set(example.query, {
      ...
      expected_rank: expectedRankFromCandidates(example.expected_card_id, example.candidates),
...
async function fingerprintReferenceEntries(entries) {
  ...
  for (const entry of entries) {
    const imagePath = entry.full_image_path ?? entry.image_path ?? '';
  ...
  return `${VISUAL_DESCRIPTOR_V1.name}:${parts.sort().join('::')}`;
}
...
function descriptorFeatureSummary() {
  return {
    color_histogram: `${VISUAL_DESCRIPTOR_V1.color_histogram_length} RGB bins`,
    color_grid: `${VISUAL_DESCRIPTOR_V1.grid_width}x${VISUAL_DESCRIPTOR_V1.grid_height} RGB downsample grid`,
    edge_grid: `${VISUAL_DESCRIPTOR_V1.edge_grid_width}x${VISUAL_DESCRIPTOR_V1.edge_grid_height} luma gradient grid`,
    region_weighting: VISUAL_DESCRIPTOR_V1.score_weights,
    artwork_reference_crop: 'same deterministic card-relative crop used by V1 hash reference fallback',
  };
}

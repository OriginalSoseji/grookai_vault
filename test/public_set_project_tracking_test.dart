...
void main() {
  ...
  group('Public set private project tracking', () {
    ...
    test('offers explicit start, tracked, and confirmed stop states', () {
      expect(source, contains("label: const Text('Track set goal')"));
    ...
    test('keeps project controls private and safe when unavailable', () {
      ...
      expect(source, contains('This only stops the private set goal.'));
      expect(source, contains('It does not create a '));
      expect(source, contains('completion progress stay unchanged.'));
      expect(source, isNot(contains("'Private Binder'")));
      expect(source, isNot(contains("'Track in Binder'")));

...
void main() {
  test('CardDetailVM loads and refreshes by condition', () async {
    ...
    when(mockService.latestIndex(cardId: anyNamed('cardId'), condition: 'NM'))
        .thenAnswer((_) async => {
              'price_low': 280.0,
              'price_mid': 300.0,
              'price_high': 340.0,
    ...
    when(mockService.latestFloors(cardId: anyNamed('cardId'), condition: 'NM'))
        .thenAnswer((_) async => {'retail': 255.0, 'market': 262.0});
    when(mockService.latestGvBaseline(cardId: anyNamed('cardId'), condition: 'NM'))
        .thenAnswer((_) async => {'value': 270.0});

    when(mockService.latestIndex(cardId: anyNamed('cardId'), condition: 'LP'))
        .thenAnswer((_) async => {
              'price_low': 220.0,
              'price_mid': 240.0,
              'price_high': 260.0,
    ...
    when(mockService.latestFloors(cardId: anyNamed('cardId'), condition: 'LP'))
        .thenAnswer((_) async => {'retail': 200.0, 'market': 210.0});
    when(mockService.latestGvBaseline(cardId: anyNamed('cardId'), condition: 'LP'))

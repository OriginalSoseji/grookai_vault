-- Isolated candidate only: no automatic promotion of old curated associations.
begin;
...
as $$
  ...
  where a.active and c.active and c.match_status='APPROVED_MATCH'
    ...
    and case when sc.set_id is not null then sc.release_status='public'
      else lower(s.game)='pokemon' or exists(select 1 from public.catalog_game_release_controls gc
        where lower(gc.game_code)=lower(s.game) and gc.release_status='public') end

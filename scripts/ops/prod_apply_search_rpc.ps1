param(
  ...
  [string]$SqlFile = "_hold/20251106_search_cards_rpc.sql"
)
...
function Invoke-ProdProbe([string]$anon, [string]$rest) {
  $headers = @{ 'apikey' = $anon; 'Authorization' = "Bearer $anon" }
  $rpcCode = 'N/A'; $viewCode = 'N/A'
  try {
    $body = @{ q = 'pikachu'; limit = 5; offset = 0 } | ConvertTo-Json -Compress
    $rpcResp = Invoke-WebRequest -UseBasicParsing -Method Post -Uri "$rest/rpc/search_cards" -Headers $headers -ContentType 'application/json' -Body $body -ErrorAction Stop
    $rpcCode = [string]$rpcResp.StatusCode
  } catch { $rpcCode = if ($_.Exception.Response) { [int]$_.Exception.Response.StatusCode } else { 'ERR' } }
  try {
    $viewResp = Invoke-WebRequest -UseBasicParsing -Method Get -Uri "$rest/v_wall_feed?select=card_id&limit=1" -Headers $headers -ErrorAction Stop
...
Write-Warning "You are about to APPLY search_cards RPC to PRODUCTION ($ProjectRef): $SqlFile"
...
if (-not $supportsExecute) {
  ...
  if ($dbUrl -and (Get-Command psql -ErrorAction SilentlyContinue)) {
    ...
    Write-Host '  2) Open _hold/20251106_search_cards_rpc.sql in Supabase Dashboard SQL editor and run it manually.'
...
Invoke-ProdProbe -anon $anon -rest $ProdRestUrl

...
flag_snapshot as (
  select
    ...
    coalesce(
      jsonb_agg(f.flag_key order by f.flag_key)
        filter (
          where f.enabled
            and coalesce((
              select schema_flag.enabled
              from public.binder_feature_flags schema_flag
              where schema_flag.flag_key = 'schema_internal'
            ), false)
        ),
      '[]'::jsonb
    ) as effective_enabled_flags,
    coalesce(
...
select jsonb_build_object(
  ...
  'ok',
    ...
    and flag_count = 11
    and enabled_flag_count between 0 and 8
    and enabled_flags <@ '[
      "community",
      "custom",
      "personal",
    ...
    and effective_enabled_flags = enabled_flags

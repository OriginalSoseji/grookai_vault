...
eligible as materialized (
  ...
  where state.flag_count = 11
    ...
    and state.enabled_before = array['community','custom','personal','public','schema_internal','shared','view_links']::text[]
...
changed as (
  ...
  set enabled = true
  from eligible
  where target.flag_key = 'templates'
...
select jsonb_build_object(
  ...
  'phase_sequence', 8,
  'target_flag', 'templates',
  'enabled_before', to_jsonb(flag_state.enabled_before),
  'enabled_after', '["community","custom","personal","public","schema_internal","shared","templates","view_links"]'::jsonb,

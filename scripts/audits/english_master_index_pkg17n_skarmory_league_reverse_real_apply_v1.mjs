...
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg17n_skarmory_league_reverse_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg17n_skarmory_league_reverse_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg17n_skarmory_league_reverse_real_apply_v1.md');

const PACKAGE_ID = 'PKG-17N-SKARMORY-LEAGUE-REVERSE-PARENT-INSERT';
const CREATED_BY = 'pkg17n_skarmory_league_reverse_real_apply_v1';
const EXPECTED_PACKAGE_FINGERPRINT = '031c6b4ea104ee30b207bf2790501de68e199e22feaac9633cd4d7b1f5ccf993';
const EXPECTED_DRY_RUN_PROOF = '08cc7999bf902d3db779fb085cf63ae99927c76c5aaf2fc6a587272ee3b71f7d';
const EXPECTED_PRE_APPLY_HASH = 'b5e6959bce8717329bee110d363fa477a4f8f3a39aabec4f02b29836e1d6f526';
const EXPECTED_TARGET_COUNT = 1;
const APPROVAL_TEXT = 'Approve real PKG-17N-SKARMORY-LEAGUE-REVERSE-PARENT-INSERT apply only. Fingerprint: 031c6b4ea104ee30b207bf2790501de68e199e22feaac9633cd4d7b1f5ccf993. Scope: 1 stamped parent inserts, 1 identity inserts, 1 child printing inserts; finishes reverse=1; stamp labels League Stamp=1; sets pl3=1. Dry-run proof: b5e6959bce8717329bee110d363fa477a4f8f3a39aabec4f02b29836e1d6f526 == b5e6959bce8717329bee110d363fa477a4f8f3a39aabec4f02b29836e1d6f526. No global apply. No migrations. No deletes. No merges. No unsupported cleanup.';
...
function validateDryRun(dryRun) {
  ...
  if (dryRun.execution?.dry_run_status !== 'pkg17n_completed_rolled_back_no_durable_change') findings.push('dry_run_status_not_passed');
...
function buildTargets(readiness) {
  return (readiness.rows ?? [])
    ...
    .map((row) => ({
      ...
      evidence_label: `${row.source_count} exact sources support Skarmory FB League Stamp reverse`,
      evidence: {
        ...
        evidence_label: `${row.source_count} exact sources support Skarmory FB League Stamp reverse`,
        raw_snapshot_ref: `pkg17m:${row.set_key}:${row.card_number}:${row.variant_key}:${row.finish_key}`,
        stamp_label: row.stamp_label,
...
async function applyPackage(client, targets, fingerprint) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg17n_targets (
    ...
    const guard = await client.query(
      `with projection as (
         ...
         from pkg17n_targets target
         join public.card_prints base on base.id = target.base_parent_id
         left join public.sets s on s.id = base.set_id
       )
       select
         (select count(*)::int from pkg17n_targets) as target_count,
         (select count(distinct target_parent_id)::int from pkg17n_targets) as target_parent_count,
         (select count(distinct target_child_id)::int from pkg17n_targets) as target_child_count,
         (select count(*)::int from pkg17n_targets target left join public.card_prints base on base.id = target.base_parent_id where base.id is null) as missing_base_count,
         (select count(*)::int from pkg17n_targets target left join public.finish_keys fk on fk.key = target.target_finish_key and fk.is_active = true where fk.key is null) as inactive_finish_count,
         (select count(*)::int
          from pkg17n_targets target
          join public.card_prints base on base.id = target.base_parent_id
         ...
         (select count(*)::int from pkg17n_targets target join public.card_printings cpr on cpr.id = target.target_child_id) as child_collision_count,
         (select count(*)::int from pkg17n_targets target join public.card_print_identity cpi on cpi.card_print_id = target.target_parent_id and cpi.is_active = true) as identity_target_collision_count,
    ...
    const childInsert = await client.query(
      `insert into public.card_printings (
       ...
       select
         ...
         concat(target.set_key, ':', target.card_number, ':skarmory_league_reverse:', target.target_variant_key, ':', target.target_finish_key),
         $1::text,
         null, null, null, null, null,
         'representative_shared_stamp',
         concat('Stamped identity child finish routed from multi-source Skarmory League reverse evidence: ', target.target_finish_key)
       from pkg17n_targets target`,
      [CREATED_BY],
    );
    ...
    const proof = await client.query(
      `select
         ...
         (select count(*)::int from pkg17n_targets) as target_rows,
         (select count(*)::int from public.card_prints cp join pkg17n_targets target on target.target_parent_id = cp.id) as inserted_parent_rows,
         (select count(*)::int from public.card_prints cp join pkg17n_targets target on target.target_parent_id = cp.id and cp.printed_identity_modifier = target.target_printed_identity_modifier) as inserted_parent_modifier_rows,
         (select count(*)::int from public.card_print_identity cpi join pkg17n_targets target on target.target_parent_id = cpi.card_print_id and cpi.is_active = true) as inserted_identity_rows,
         (select count(*)::int from public.card_printings cpr join pkg17n_targets target on target.target_child_id = cpr.id) as inserted_child_rows,
         (select count(*)::int from public.card_printings cpr join pkg17n_targets target on target.target_child_id = cpr.id and cpr.finish_key = 'stamped') as forbidden_stamped_child_rows`,
      [PACKAGE_ID, fingerprint],
    );
    await client.query('commit');
    const afterSnapshot = await captureSnapshot(client, targets);
    return {
      apply_status: 'pkg17n_real_apply_committed',
      guard: guardRow,
      proof: proof.rows[0],
      write_counts: {
...
function renderMarkdown(report) {
  ...
  return `# PKG-17N Skarmory League Reverse Real Apply V1

Approved real apply for the Skarmory FB League Stamp reverse parent identity insert.
...
async function main() {
  ...
  const report = {
    ...
    version: 'english_master_index_pkg17n_skarmory_league_reverse_real_apply_v1',
    package_id: PACKAGE_ID,

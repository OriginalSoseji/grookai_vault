...
const GATE_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06m_active_finish_child_printing_real_apply_gate_v1.json');
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06m_active_finish_child_printing_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06m_active_finish_child_printing_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg06m_active_finish_child_printing_real_apply_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260609_pkg06m_active_finish_child_printing_real_apply_checkpoint_v1.md');

const PACKAGE_ID = 'PKG-06M-ACTIVE-FINISH-CHILD-PRINTING-INSERTS';
const SOURCE_READINESS_FINGERPRINT = '3152748852069897bb63e10e138fd46dd508f492b2124a3e8665bd9763a7ab38';
const PACKAGE_FINGERPRINT = '33c58f8b631f87aa6b798c054a3a9acdf0a149536e772de21376867129944b66';
const SQL_HASH = '9b3fb9ffd6a65735ee43cfcd2992562d5b511527e955e1fe5d624a5a373b1535';
const DRY_RUN_PROOF_HASH = '175fcea3ab6c22819df2ef5d2de3727c515c889b0bd68216edb507a0b9d2d936';
const APPROVAL_TEXT = 'Approve real PKG-06M-ACTIVE-FINISH-CHILD-PRINTING-INSERTS apply only. Fingerprint: 33c58f8b631f87aa6b798c054a3a9acdf0a149536e772de21376867129944b66. SQL hash: 9b3fb9ffd6a65735ee43cfcd2992562d5b511527e955e1fe5d624a5a373b1535. Scope: 30 child-only card_printing inserts for bwp/BW Black Star Promos, ecard2/Aquapolis, hgss2/HS-Unleashed, me02/Phantasmal Flames, pl2/Rising Rivals, pop2/POP Series 2, ru1/Pokemon Rumble, sm10/Unbroken Bonds, sm11/Unified Minds, and sm7/Celestial Storm; finishes cosmos=16, holo=9, normal=5; target parents=30. Dry-run proof: 175fcea3ab6c22819df2ef5d2de3727c515c889b0bd68216edb507a0b9d2d936 == 175fcea3ab6c22819df2ef5d2de3727c515c889b0bd68216edb507a0b9d2d936. No global apply. No migrations. No deletes. No merges. No unsupported cleanup. No parent writes.';

const EXPECTED_CHILD_ROWS = 30;
const EXPECTED_PARENT_ROWS = 30;
...
const EXPECTED_FINISH_COUNTS = { cosmos: 16, holo: 9, normal: 5 };
...
function validatePrerequisites({ gate, dryRun, rows }) {
  ...
  if (dryRun.dry_run_execution_status !== 'pkg06m_active_finish_child_printing_completed_rolled_back_no_durable_change') findings.push('dry_run_not_passed');
  ...
  if (rows.length !== EXPECTED_CHILD_ROWS) findings.push('target_row_count_not_30');
  if (new Set(rows.map((row) => row.card_print_id)).size !== EXPECTED_PARENT_ROWS) findings.push('target_parent_count_not_30');
...
function validateBeforeSnapshot({ beforeTargetSnapshot, beforeParentSnapshot, dryRun }) {
  ...
  if (beforeTargetSnapshot.counts.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('before_parent_count_not_30');
  ...
  if (beforeParentSnapshot.count !== EXPECTED_PARENT_ROWS) findings.push('before_parent_snapshot_count_not_30');
  return findings;
}

function validateAfterSnapshot({ afterTargetSnapshot, beforeParentSnapshot, afterParentSnapshot, insertedRows }) {
  const findings = [];
  if (afterTargetSnapshot.counts.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('after_parent_count_not_30');
  if (afterTargetSnapshot.counts.existing_target_child_rows !== EXPECTED_CHILD_ROWS) findings.push('after_existing_target_child_count_not_30');
  if (afterTargetSnapshot.counts.planned_id_collision_rows !== EXPECTED_CHILD_ROWS) findings.push('after_planned_id_collision_count_not_30');
  if (beforeParentSnapshot.hash_sha256 !== afterParentSnapshot.hash_sha256) findings.push('parent_snapshot_changed_unexpectedly');
  if (insertedRows.counts.inserted_rows_found !== EXPECTED_CHILD_ROWS) findings.push('inserted_rows_found_not_30');
...
async function applyPackage({ dryRun, rows }) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg06m_real_apply_targets (
    ...
    await client.query(
      `insert into pkg06m_real_apply_targets
       select
    ...
    const shape = await client.query(
      `select
       ...
       from pkg06m_real_apply_targets`,
    );
    ...
    const grouped = await client.query(
      `select 'set' as kind, set_key as key, count(*)::int as count from pkg06m_real_apply_targets group by set_key
       union all
       select 'finish' as kind, finish_key as key, count(*)::int as count from pkg06m_real_apply_targets group by finish_key`,
    );
    ...
    const lockParents = await client.query(
      `select cp.id
       from public.card_prints cp
       join (select distinct card_print_id from pkg06m_real_apply_targets) target on target.card_print_id = cp.id
       for update of cp`,
    );
    ...
    const finishGuard = await client.query(
      `select count(*)::int as unsupported_finish_count
       from pkg06m_real_apply_targets target
    ...
    return {
      connected: true,
      apply_status: 'pkg06m_active_finish_child_printing_real_apply_committed',
      error_message: null,
      before_target_snapshot: beforeTargetSnapshot,
    ...
    return {
      connected: true,
      apply_status: 'pkg06m_active_finish_child_printing_real_apply_failed_rolled_back',
...
function renderMarkdown(report) {
  return `# PKG-06M Active Finish Child Printing Real Apply V1
...
function renderCheckpoint(report) {
  return `# PKG-06M Active Finish Child Printing Real Apply Checkpoint V1
...
Real apply was scoped to 30 child-only card_printing inserts across 10 sets. No parent writes, migrations, deletes, merges, unsupported cleanup, or quarantine were performed.
`;
}

function updateCheckpointIndex() {
  const indexPath = path.join(CHECKPOINT_DIR, 'CHECKPOINT_INDEX.md');
  const line = '| 2026-06-09 | [PKG-06M Active Finish Child Printing Real Apply Checkpoint V1](20260609_pkg06m_active_finish_child_printing_real_apply_checkpoint_v1.md) | Records approved real apply for 30 active-finish child-only card_printing inserts across 10 sets; parent rows unchanged, no migrations. |';
  const current = fs.existsSync(indexPath) ? fs.readFileSync(indexPath, 'utf8') : '# Master Index Checkpoints\n';
  if (current.includes('20260609_pkg06m_active_finish_child_printing_real_apply_checkpoint_v1.md')) {
    fs.writeFileSync(indexPath, current.split('\n').map((existingLine) => (
      existingLine.includes('20260609_pkg06m_active_finish_child_printing_real_apply_checkpoint_v1.md') ? line : existingLine
    )).join('\n'));
  } else {
...
const report = {
  ...
  version: 'english_master_index_pkg06m_active_finish_child_printing_real_apply_v1',
  audit_only: false,
  apply_only: true,
  approval_scope: {
  ...
  apply_status: pass
    ? 'pkg06m_active_finish_child_printing_real_apply_committed_and_verified'
    : 'pkg06m_active_finish_child_printing_real_apply_failed_or_blocked',
  ...
  verification_summary: {
    ...
    master_index_comparison_status: pass ? 'pkg06m_active_finish_child_printing_verified_after_apply' : 'not_verified',
  },

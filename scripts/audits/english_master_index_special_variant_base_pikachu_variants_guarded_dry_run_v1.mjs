...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'base_pikachu_variants_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'base_pikachu_variants_guarded_dry_run_v1.md');

const PACKAGE_ID = 'SPECIAL-VAR-04-BASE-PIKACHU-VARIANT-PARENT-INSERTS';
const CREATED_BY = 'english_master_index_special_variant_base_pikachu_variants_guarded_dry_run_v1';
const EXPECTED_TARGET_COUNT = 7;
const TARGET_KEYS = new Set([
  'base1-58-pikachu-e3-red-cheeks',
  'base1-58-pikachu-e3-yellow-cheeks',
  'base1-58-pikachu-first-edition-red-cheeks',
  'base1-58-pikachu-first-edition-yellow-cheeks',
  'base1-58-pikachu-ghost-stamp',
  'base1-58-pikachu-shadowless-red-cheeks',
  'base1-58-pikachu-shadowless-yellow-cheeks',
]);
...
function buildSourceTargets(sourceReport) {
  return (sourceReport.rows ?? [])
    .filter((row) => TARGET_KEYS.has(row.candidate_key)
    ...
    .map((row) => ({
      ...
      target_printed_identity_modifier: row.proposed_identity_modifier,
...
async function attachBaseParents(client, sourceTargets) {
  const result = await client.query(
    `with target as (
     ...
     order by target.set_key, target.card_number`,
  ...
  return sourceTargets.map((row) => {
    ...
    return {
      ...
      target_child_id: uuidFromSeed(`${PACKAGE_ID}:child:${row.set_key}:${row.card_number}:${normalizeText(row.card_name)}:${row.target_variant_key}:${row.target_finish_key}`),
...
function sqlHash() {
  return sha256(stableJson({
    package_id: PACKAGE_ID,
    writes: [
      'insert public.card_prints cloned from source-backed Base Set Pikachu parent with exact print-run, color, stamp, or release identity modifier',
      'insert public.card_print_identity via card_print_identity_backfill_projection_v1',
      'insert public.card_printings target child finish',
    ],
...
async function runDryRun(client, targets, fingerprint) {
  ...
  try {
    ...
    await client.query(
      `create temporary table base_pikachu_variant_targets (
    ...
    await client.query(
      `insert into base_pikachu_variant_targets
       select *
    ...
    const sourceUniqueFinishCount = targets.filter((row) => !row.base_child_finishes.includes(row.target_finish_key)).length;
    const guard = await client.query(
      `with projection as (
         ...
         from base_pikachu_variant_targets target
         join public.card_prints base on base.id = target.base_parent_id
         left join public.sets s on s.id = base.set_id
       )
       select
         (select count(*)::int from base_pikachu_variant_targets) as target_count,
         (select count(distinct target_parent_id)::int from base_pikachu_variant_targets) as target_parent_count,
         (select count(distinct target_child_id)::int from base_pikachu_variant_targets) as target_child_count,
         (select count(*)::int from base_pikachu_variant_targets target left join public.card_prints base on base.id = target.base_parent_id where base.id is null) as missing_base_count,
         (select count(*)::int from base_pikachu_variant_targets target left join public.finish_keys fk on fk.key = target.target_finish_key and fk.is_active = true where fk.key is null) as inactive_finish_count,
         (select count(*)::int from base_pikachu_variant_targets target join public.card_prints base on base.id = target.base_parent_id left join public.card_printings base_child on base_child.card_print_id = base.id and base_child.finish_key = target.target_finish_key where base_child.id is null) as source_unique_finish_count,
         ...
         (select count(*)::int from base_pikachu_variant_targets target join public.card_printings cpr on cpr.id = target.target_child_id) as child_collision_count,
         (select count(*)::int from base_pikachu_variant_targets target join public.card_print_identity cpi on cpi.card_print_id = target.target_parent_id and cpi.is_active = true) as identity_target_collision_count,
    ...
    if (
      ...
      || guardRow.source_unique_finish_count !== sourceUniqueFinishCount
      ...
      throw new Error(`guard failed: ${JSON.stringify({ ...guardRow, expected_source_unique_finish_count: sourceUniqueFinishCount })}`);
    }
    ...
    const parentInsert = await client.query(
      `insert into public.card_prints (
       ...
       select
         ...
         concat('Source-backed Base Set Pikachu special-case identity: ', target.target_variant_key, '. Representative base image only until exact variant image is available.')
       from base_pikachu_variant_targets target
       join public.card_prints base on base.id = target.base_parent_id`,
      [PACKAGE_ID],
    );
    ...
    const childInsert = await client.query(
      `insert into public.card_printings (
       ...
       select
         ...
         concat(target.set_key, ':', target.card_number, ':base_pikachu_variant:', target.target_variant_key, ':', target.target_finish_key),
         $1::text,
         ...
         'Source-backed Base Set Pikachu special-case child. Representative image only until exact variant image is available.'
       from base_pikachu_variant_targets target`,
      [CREATED_BY],
    );
    ...
    const proof = await client.query(
      `select
         ...
         (select count(*)::int from base_pikachu_variant_targets) as target_rows,
         (select count(*)::int from public.card_prints cp join base_pikachu_variant_targets target on target.target_parent_id = cp.id) as inserted_parent_rows,
         (select count(*)::int from public.card_prints cp join base_pikachu_variant_targets target on target.target_parent_id = cp.id and cp.printed_identity_modifier = target.target_printed_identity_modifier and cp.variant_key = target.target_variant_key) as inserted_parent_identity_rows,
         (select count(*)::int from public.card_print_identity cpi join base_pikachu_variant_targets target on target.target_parent_id = cpi.card_print_id and cpi.is_active = true) as inserted_identity_rows,
         (select count(*)::int from public.card_printings cpr join base_pikachu_variant_targets target on target.target_child_id = cpr.id and cpr.finish_key = target.target_finish_key) as inserted_child_rows,
         (select count(*)::int from public.card_printings cpr join base_pikachu_variant_targets target on target.target_child_id = cpr.id and cpr.finish_key = 'stamped') as forbidden_stamped_child_rows`,
    ...
    return {
      dry_run_status: 'base_pikachu_variants_parent_insert_failed_rolled_back',
...
function renderMarkdown(report) {
  return `# Base Pikachu Variants Guarded Dry Run V1
...
- source_unique_finish_rows: ${report.scope.source_unique_finish_rows}

## Targets

${markdownTable(['set', 'number', 'name', 'variant', 'modifier', 'finish', 'base_finishes'], report.scope.targets.map((row) => [
    row.set_key,
    row.card_number,
    row.card_name,
...
async function main() {
  ...
  if (!conn) {
    ...
    try {
      ...
      } else if (targets.some((row) => !row.base_parent_id)) {
        execution = {
          dry_run_status: 'blocked_missing_base_parent',
          rollback_verified: false,
          dry_run_proof_sha256: null,
          simulated_write_counts: { parent_inserts: 0, identity_inserts: 0, child_inserts: 0, deletes: 0, merges: 0 },
          stop_findings: ['missing_base_parent'],
        };
      } else {
  ...
  const pass = execution.dry_run_status === 'base_pikachu_variants_parent_insert_completed_rolled_back_no_durable_change'
  ...
  const sourceUniqueFinishRows = targets.filter((row) => !row.base_child_finishes.includes(row.target_finish_key)).length;
  const recommended = pass
    ? `Approve real ${PACKAGE_ID} apply only. Fingerprint: ${fingerprint}. SQL hash: ${sqlHashSha256}. Scope: 7 Base Set Pikachu special-case parent inserts, 7 active identity inserts, 7 normal child printing inserts; source_unique_finish_rows=${sourceUniqueFinishRows}; sets base1=7. Dry-run proof: ${dryRunProof}. No global apply. No migrations. No deletes. No merges. No unsupported cleanup.`
  ...
  const report = {
    ...
    scope: {
      ...
      source_unique_finish_rows: sourceUniqueFinishRows,

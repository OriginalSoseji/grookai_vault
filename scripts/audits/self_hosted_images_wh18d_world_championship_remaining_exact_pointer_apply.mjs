...
const SOURCE_SUMMARY_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh18c_world_championship_remaining_exact_pointer_dry_run_summary_v1.json');
const SOURCE_PLAN_JSONL = path.join(OUTPUT_DIR, 'self_hosted_images_wh18c_world_championship_remaining_exact_pointer_plan_v1.jsonl');
const PLAN_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh18d_world_championship_remaining_exact_pointer_apply_plan_v1.json');
const PLAN_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh18d_world_championship_remaining_exact_pointer_apply_plan_v1.md');
const RESULT_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh18d_world_championship_remaining_exact_pointer_apply_result_v1.json');
const RESULT_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh18d_world_championship_remaining_exact_pointer_apply_result_v1.md');
const PACKAGE_ID = 'IMG-HOST-WH-18D-WORLD-CHAMPIONSHIP-REMAINING-EXACT-POINTER-APPLY';
...
function buildUpdates(summary, planRows) {
  ...
  if (summary.ready_for_apply_package !== true) {
    stopFindings.push(`wh18c_not_apply_ready:${(summary.stop_findings ?? []).join(',')}`);
  }

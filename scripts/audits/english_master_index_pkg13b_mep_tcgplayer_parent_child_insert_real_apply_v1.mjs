...
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg13b_mep_tcgplayer_parent_child_insert_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg13b_mep_tcgplayer_parent_child_insert_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg13b_mep_tcgplayer_parent_child_insert_real_apply_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260610_pkg13b_mep_tcgplayer_parent_child_insert_real_apply_checkpoint_v1.md');
...
function validateDryRun(dryRun) {
  ...
  if (dryRun.execution?.dry_run_status !== 'pkg13b_mep_tcgplayer_parent_child_insert_completed_rolled_back_no_durable_change') findings.push('dry_run_status_not_passed');
...
async function applyPackage(client, plan, fingerprint) {
  ...
  try {
    ...
    return {
      apply_status: afterFindings.length ? 'committed_but_after_validation_failed' : 'pkg13b_mep_tcgplayer_parent_child_insert_real_apply_committed',
      committed: true,
    ...
    return {
      apply_status: 'pkg13b_mep_tcgplayer_parent_child_insert_real_apply_failed_rolled_back',
      committed: false,
...
function renderMarkdown(report) {
  return `# PKG-13B MEP TCGplayer Parent+Child Insert Real Apply V1
...
async function updateCheckpointIndex() {
  const line = '| 2026-06-10 | [PKG-13B MEP TCGplayer Parent+Child Insert Real Apply Checkpoint V1](20260610_pkg13b_mep_tcgplayer_parent_child_insert_real_apply_checkpoint_v1.md) | Real apply for two MEP TCGplayer-backed cosmos parent+child inserts after rollback-only proof. No migrations, deletes, merges, unsupported cleanup, or quarantine. |';
  let current = '';
  try {
  ...
  if (!current.includes('20260610_pkg13b_mep_tcgplayer_parent_child_insert_real_apply_checkpoint_v1.md')) {
...
async function main() {
  ...
  try {
    ...
    const report = {
      ...
      version: 'english_master_index_pkg13b_mep_tcgplayer_parent_child_insert_real_apply_v1',
      package_id: PACKAGE_ID,
    ...
    await writeText(CHECKPOINT_MD, `# PKG-13B MEP TCGplayer Parent+Child Insert Real Apply Checkpoint V1

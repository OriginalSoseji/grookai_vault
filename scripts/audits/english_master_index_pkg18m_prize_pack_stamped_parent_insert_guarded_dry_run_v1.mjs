...
const READINESS_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg18l_prize_pack_active_finish_readiness_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg18m_prize_pack_stamped_parent_insert_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg18m_prize_pack_stamped_parent_insert_guarded_dry_run_v1.md');

const PACKAGE_ID = 'PKG-18M-PRIZE-PACK-STAMPED-PARENT-INSERTS';
const CREATED_BY = 'pkg18m_prize_pack_stamped_parent_insert_guarded_dry_run_v1';
...
function buildTargets(readiness) {
  return (readiness.rows ?? [])
    ...
    .map((row) => ({
      ...
      source_families: ['pricecharting_prize_pack', 'verified_master_index_identity'],
      evidence: {
        ...
        raw_snapshot_ref: `pkg18l:${row.set_key}:${row.card_number}:${row.variant_key}:${row.finish_key}`,
        stamp_label: row.stamp_label,
        variant_key: row.variant_key,
        active_child_finish_key: row.finish_key,
        source_evidence: row.evidence ?? null,
      },
    }))
...
async function runDryRun(client, targets, fingerprint) {
  ...
  try {
    ...
    const guard = await client.query(
      `with projection as (
       ...
       select
         ...
         (select count(*)::int from pkg18m_targets target join public.card_prints base on base.id = target.base_parent_id left join public.card_printings base_child on base_child.card_print_id = base.id and base_child.finish_key = target.target_finish_key where base_child.id is null) as missing_base_finish_count,
         (select count(*)::int from pkg18m_targets target join public.card_prints base on base.id = target.base_parent_id join public.card_prints cp on cp.set_id = base.set_id and coalesce(cp.number_plain, cp.number) = coalesce(base.number_plain, base.number) and lower(cp.name) = lower(base.name) and coalesce(cp.variant_key, '') = target.target_variant_key) as parent_collision_count,
    ...
    return {
      dry_run_status: 'pkg18m_completed_rolled_back_no_durable_change',
      guard: guardRow,
      proof: proof.rows[0],
    ...
    return {
      dry_run_status: 'pkg18m_failed_rolled_back',
...
function renderMarkdown(report) {
  ...
  return `# PKG-18M Prize Pack Stamped Parent Insert Guarded Dry Run V1

Rollback-only dry-run for Prize Pack stamped parent identity inserts.
...
async function main() {
  ...
  const recommended = execution.dry_run_status === 'pkg18m_completed_rolled_back_no_durable_change' && execution.rollback_verified && execution.stop_findings.length === 0
  ...
  const report = {
    ...
    version: 'english_master_index_pkg18m_prize_pack_stamped_parent_insert_guarded_dry_run_v1',
    package_id: PACKAGE_ID,

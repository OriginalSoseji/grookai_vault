...
const PKG22A_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg22a_product_promo_governance_readiness_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg26a_product_promo_supported_finish_governance_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg26a_product_promo_supported_finish_governance_v1.md');
const PACKAGE_ID = 'PKG-26A-PRODUCT-PROMO-SUPPORTED-FINISH-GOVERNANCE';
...
function renderMarkdown(report) {
  ...
  const remainingRows = Object.entries(report.remaining_blocked_summary.by_governance_status).map(([status, count]) => [status, count]);
  return `# PKG-26A Product/Promo Supported Finish Governance V1
...
${markdownTable(['status', 'rows'], remainingRows)}
...
const pkg22a = await readJson(PKG22A_JSON);

const alreadyGoverned = current.product_promo_supported_finish_governance?.rows ?? [];
const candidatesByPrintingId = new Map();
for (const row of [
  ...(pkg22a.rows ?? []),
  ...alreadyGoverned,
]) {
  if (row.governance_bucket === 'product_promo_identity_modifier_or_variant_supported_finish_review'
    || row.governance_status === 'master_index_supported_product_promo_finish_identity_shape_governed') {
...
const governedRows = [...candidatesByPrintingId.values()]
  ...
  .map((row) => ({
    ...row,
    governance_status: 'master_index_supported_product_promo_finish_identity_shape_governed',
    cleanup_readiness: 'governed_non_write',
    reason: 'The same set/card/finish is Master Index supported; only product/promo variant/modifier key shape prevents direct reconciliation.',
  }));

const governedIds = new Set(governedRows.map((row) => row.card_printing_id));
const blockedRows = (pkg22a.rows ?? [])
  .filter((row) => !governedIds.has(row.card_printing_id))
  .sort(rowSort)
  .map((row) => ({
    ...row,
    governance_status: row.governance_bucket === 'product_promo_identity_modifier_or_variant_source_review'
      ? 'blocked_product_promo_identity_source_review'
      : 'blocked_product_promo_source_coverage_gap',
...
const report = {
  ...
  version: 'english_master_index_pkg26a_product_promo_supported_finish_governance_v1',
  package_id: PACKAGE_ID,
  ...
  source_artifacts: {
    ...
    pkg22a_readiness: PKG22A_JSON,
  },
  summary: {
    source_rows: (pkg22a.rows ?? []).length,
  ...
  remaining_blocked_summary: {
    ...
    by_governance_status: countBy(blockedRows, (row) => row.governance_status),

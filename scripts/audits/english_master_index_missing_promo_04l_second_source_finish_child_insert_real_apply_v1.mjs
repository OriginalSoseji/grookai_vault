...
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'second_source_finish_child_insert_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'second_source_finish_child_insert_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'second_source_finish_child_insert_real_apply_v1.md');

const PACKAGE_ID = 'MISSING-PROMO-04L-SECOND-SOURCE-FINISH-CHILD-INSERTS';
const CREATED_BY = 'english_master_index_missing_promo_04l_second_source_finish_child_insert_real_apply_v1';
const APPROVED_FINGERPRINT = '2685545ff2bd82267b2582eb6057c39f1c575129ed79a9186ab104c788985e7a';
const APPROVED_SQL_HASH = '366dcbe03abbf78775fa07ee3e4e02b8c4b45810bc8455ccd03f04c9e1d76cdc';
const APPROVED_DRY_RUN_PROOF = 'bff21af16ad5cd3aab246e0e399a83184a1c4ba4624c88498327d4f3756b6a2c';
...
function assertApproved(dryRun) {
  ...
  if (dryRun.summary?.target_count !== 2) errors.push('target count mismatch');
  if (dryRun.summary?.by_finish?.holo !== 1 || dryRun.summary?.by_finish?.normal !== 1) errors.push('finish scope mismatch');
...
function renderMarkdown(report) {
  return [
    '# Second Source Finish Child Insert Real Apply V1',
    '',
...
async function main() {
  ...
  try {
    ...
    const report = {
      ...
      input_artifacts: {
        second_source_finish_child_insert_guarded_dry_run: rel(DRY_RUN_JSON),
      },

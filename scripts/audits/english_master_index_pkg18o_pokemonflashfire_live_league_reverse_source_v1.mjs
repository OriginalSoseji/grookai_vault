...
const RESIDUAL_JSON = path.join(AUDIT_DIR, 'english_master_index_stamped_special_live_residual_queue_v1.json');
const REPORT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg18o_pokemonflashfire_live_league_reverse_source_v1.json');
const REPORT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg18o_pokemonflashfire_live_league_reverse_source_v1.md');
const FIXTURE_JSON = path.join(
  DEFAULT_OUTPUT_DIR,
  'source_fixtures',
  'generated_pkg18o_pokemonflashfire_live_league_reverse_source_v1',
  'pokemonflashfire_live_league_reverse_exact_sources_v1.json',
);

const PACKAGE_ID = 'PKG-18O-POKEMONFLASHFIRE-LIVE-LEAGUE-REVERSE-SOURCE';
const SOURCE_KEY = 'pokemonflashfire_live_league_reverse_exact';
...
const MANUAL_REVIEW_OVERRIDES = new Map([
  [
    'pl3|5|garchomp',
    'live_source_page_describes_national_championships_crosshatch_staff_context_not_plain_league_stamp',
  ],
  [
    'hgss2|7|politoed',
    'live_source_page_describes_national_championships_crosshatch_staff_context_not_plain_league_stamp',
...
function liveResidualLeagueTargets(residual) {
  return (residual.open_rows ?? [])
...
function buildRecord(target, source) {
  return {
    ...
    raw_snapshot_ref: `pokemonflashfire-live:${target.set_key}:${target.card_number}:league_stamp:reverse`,
    notes: 'Accepted only because the reviewed PokemonFlashfire/DJS source label states card number, Reverse Holo, and Pokemon League Promo for a live residual queue row.',
  };
}

function renderMarkdown(report) {
  return `# PKG-18O PokemonFlashfire Live League Reverse Source V1
...
${markdownTable(['metric', 'value'], [
    ...
    ['live_residual_league_targets', report.summary.live_residual_league_targets],
    ['fixture_records_written', report.summary.fixture_records_written],
    ['manual_review_rows', report.summary.manual_review_rows],
    ['skipped_source_rows', report.summary.skipped_source_rows],
  ])}

## Fixture Records

${markdownTable(['set', 'number', 'card', 'finish', 'source url'], report.records.map((row) => [
  row.set_key,
  row.card_number,
  row.card_name,
  row.finish_key,
...
${markdownTable(['set', 'number', 'card', 'target stamp', 'proposed finish', 'reason', 'source url'], report.review_rows.map((row) => [
  row.set_key,
  row.card_number,
  row.card_name,
  row.target_stamp_label,
...
${markdownTable(['set', 'number', 'card', 'reason'], report.skipped_rows.map((row) => [
  row.set_key,
  row.card_number,
  row.card_name,
  row.reason,
...
async function main() {
  const residual = await readJson(RESIDUAL_JSON);
  const targets = liveResidualLeagueTargets(residual);
  ...
  for (const [key, source] of sourceMap().entries()) {
    ...
    if (!target) {
      skippedRows.push({ ...source, reason: 'not_in_live_residual_league_active_finish_queue' });
      continue;
    }
    const reviewReason = MANUAL_REVIEW_OVERRIDES.get(key);
    if (reviewReason) {
      reviewRows.push({
        ...source,
        target_set_name: target.set_name,
        target_variant_key: target.variant_key,
        target_stamp_label: target.stamp_label,
        proposed_finish_key: 'reverse',
        reason: reviewReason,
        status: 'needs_manual_review_not_fixture_evidence',
      });
      continue;
    }
    records.push(buildRecord(target, source));
  }

  const report = {
    generated_at: new Date().toISOString(),
    version: 'english_master_index_pkg18o_pokemonflashfire_live_league_reverse_source_v1',
    ...
    write_ready_now: 0,
    source_artifacts: {
      live_residual_queue: rel(RESIDUAL_JSON),
      fixture_json: rel(FIXTURE_JSON),
    },
    fingerprint_sha256: sha256(stableJson(records)),
    summary: {
      source_rows_reviewed: SOURCE_ROWS.length,
      live_residual_league_targets: targets.length,
      fixture_records_written: records.length,
      manual_review_rows: reviewRows.length,
      skipped_source_rows: skippedRows.length,
    },
    records,
  ...
  await writeJson(FIXTURE_JSON, {
    ...
    db_writes_performed: false,
    migrations_created: false,
    generation_note: 'Generated from previously reviewed PokemonFlashfire/DJS Pokemon Cards League reverse product/category labels against the live residual stamped/special queue. URL and label evidence only; no page dumps.',
    records,
  });

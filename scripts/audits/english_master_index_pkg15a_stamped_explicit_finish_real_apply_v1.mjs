...
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg15a_stamped_explicit_finish_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg15a_stamped_explicit_finish_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg15a_stamped_explicit_finish_real_apply_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260611_pkg15a_stamped_explicit_finish_real_apply_checkpoint_v1.md');
const CHECKPOINT_INDEX = path.join(CHECKPOINT_DIR, 'CHECKPOINT_INDEX.md');

const PACKAGE_ID = 'PKG-15A-STAMPED-EXPLICIT-FINISH-PARENT-INSERTS';
const CREATED_BY = 'pkg15a_stamped_explicit_finish_real_apply_v1';
const SOURCE_PACKAGE_ID = 'PKG-15-STAMPED-EXPLICIT-FINISH-READINESS';
const EXPECTED_DRY_RUN_PROOF = '5338d0d956f619ffe427ffa29adfbda7975d63e411c8493375eeaa89c1a07415';
...
function validateDryRun(dryRun) {
  ...
  if (dryRun.execution?.dry_run_status !== 'pkg15a_stamped_explicit_finish_completed_rolled_back_no_durable_change') findings.push('dry_run_status_not_passed');
...
async function applyPackage(client, targets, fingerprint) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg15a_targets (
    ...
    const guard = await client.query(
      `with projection as (
         ...
         from pkg15a_targets target
         join public.card_prints base on base.id = target.base_parent_id
         left join public.sets s on s.id = base.set_id
       )
       select
         (select count(*)::int from pkg15a_targets) as target_count,
         (select count(distinct target_parent_id)::int from pkg15a_targets) as target_parent_count,
         (select count(distinct target_child_id)::int from pkg15a_targets) as target_child_count,
         (select count(*)::int from pkg15a_targets target left join public.card_prints base on base.id = target.base_parent_id where base.id is null) as missing_base_count,
         (select count(*)::int from pkg15a_targets target left join public.finish_keys fk on fk.key = target.target_finish_key and fk.is_active = true where fk.key is null) as inactive_finish_count,
         (select count(*)::int
          from pkg15a_targets target
          join public.card_prints base on base.id = target.base_parent_id
         ...
         (select count(*)::int from pkg15a_targets target join public.card_printings cpr on cpr.id = target.target_child_id) as child_collision_count,
         (select count(*)::int from pkg15a_targets target join public.card_print_identity cpi on cpi.card_print_id = target.target_parent_id and cpi.is_active = true) as identity_target_collision_count,
    ...
    const childInsert = await client.query(
      `insert into public.card_printings (
       ...
       select
         ...
         concat('Stamped identity child finish routed from explicit evidence label: ', target.target_finish_key)
       from pkg15a_targets target`,
      [CREATED_BY],
    );
    ...
    const proof = await client.query(
      `select
         ...
         (select count(*)::int from pkg15a_targets) as target_rows,
         (select count(*)::int from public.card_prints cp join pkg15a_targets target on target.target_parent_id = cp.id) as inserted_parent_rows,
         (select count(*)::int from public.card_print_identity cpi join pkg15a_targets target on target.target_parent_id = cpi.card_print_id and cpi.is_active = true) as inserted_identity_rows,
         (select count(*)::int from public.card_printings cpr join pkg15a_targets target on target.target_child_id = cpr.id) as inserted_child_rows`,
      [PACKAGE_ID, fingerprint],
    );
    await client.query('commit');
    const afterSnapshot = await captureSnapshot(client, targets);
    return {
      apply_status: 'pkg15a_stamped_explicit_finish_real_apply_committed',
      committed: true,
    ...
    return {
      apply_status: 'pkg15a_stamped_explicit_finish_real_apply_failed_rolled_back',
      committed: false,
...
function renderMarkdown(report) {
  ...
  const targetRows = report.scope.targets.map((row) => [row.set_key, row.card_number, row.card_name, row.stamp_label, row.target_variant_key, row.target_finish_key]);
  return `# PKG-15A Stamped Explicit Finish Real Apply V1

Real apply for deterministic stamped canonical parent inserts with explicit child-finish evidence.
...
- merges: ${report.execution.write_counts?.merges ?? 0}
`;
}

function checkpointMarkdown(report) {
  return `# PKG-15A Stamped Explicit Finish Real Apply Checkpoint V1
...
- parent_inserts: ${report.execution.write_counts?.parent_inserts ?? 0}
- identity_inserts: ${report.execution.write_counts?.identity_inserts ?? 0}
- child_inserts: ${report.execution.write_counts?.child_inserts ?? 0}
- deletes: ${report.execution.write_counts?.deletes ?? 0}
- migrations_created: ${report.migrations_created}
`;
}
...
function updateCheckpointIndex() {
  const line = '| 2026-06-11 | [PKG-15A Stamped Explicit Finish Real Apply Checkpoint V1](20260611_pkg15a_stamped_explicit_finish_real_apply_checkpoint_v1.md) | Real-applied 5 deterministic stamped identity parent inserts with active identity rows and explicit holo child printings. No migrations, deletes, cleanup, or quarantine. |';
  const current = fsSync.existsSync(CHECKPOINT_INDEX) ? fsSync.readFileSync(CHECKPOINT_INDEX, 'utf8') : '# Master Index Checkpoints\n';
  if (current.includes('20260611_pkg15a_stamped_explicit_finish_real_apply_checkpoint_v1.md')) {
    fsSync.writeFileSync(CHECKPOINT_INDEX, current.split('\n').map((existingLine) => (
      existingLine.includes('20260611_pkg15a_stamped_explicit_finish_real_apply_checkpoint_v1.md') ? line : existingLine
    )).join('\n'));
  } else {
...
const report = {
  ...
  version: 'english_master_index_pkg15a_stamped_explicit_finish_real_apply_v1',

...
const READINESS_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg17q_league_reverse_bulk_readiness_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg17r_league_reverse_bulk_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg17r_league_reverse_bulk_guarded_dry_run_v1.md');
  ...
  return (readiness.rows ?? [])
    ...
    .map((row) => ({
      ...
      evidence: {
        ...
        source_families: row.source_families ?? [],
        source_urls: row.source_urls ?? [],
        evidence_label: 'Two independent exact sources support League Stamp reverse active finish',
        raw_snapshot_ref: `pkg17q:${row.set_key}:${row.card_number}:${row.variant_key}:${row.finish_key}`,
        stamp_label: row.stamp_label,
...
async function runDryRun(client, targets, fingerprint) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg17r_targets (
    ...
    const guard = await client.query(
      `with projection as (
         ...
         from pkg17r_targets target
         join public.card_prints base on base.id = target.base_parent_id
         left join public.sets s on s.id = base.set_id
       )
       select
         (select count(*)::int from pkg17r_targets) as target_count,
         (select count(distinct target_parent_id)::int from pkg17r_targets) as target_parent_count,
         (select count(distinct target_child_id)::int from pkg17r_targets) as target_child_count,
         (select count(*)::int from pkg17r_targets target left join public.card_prints base on base.id = target.base_parent_id where base.id is null) as missing_base_count,
         (select count(*)::int from pkg17r_targets target left join public.finish_keys fk on fk.key = target.target_finish_key and fk.is_active = true where fk.key is null) as inactive_finish_count,
         (select count(*)::int from pkg17r_targets target join public.card_prints base on base.id = target.base_parent_id left join public.card_printings base_child on base_child.card_print_id = base.id and base_child.finish_key = target.target_finish_key where base_child.id is null) as missing_base_finish_count,
         (select count(*)::int from pkg17r_targets target join public.card_prints base on base.id = target.base_parent_id join public.card_prints cp on cp.set_id = base.set_id and coalesce(cp.number_plain, cp.number) = coalesce(base.number_plain, base.number) and lower(cp.name) = lower(base.name) and coalesce(cp.variant_key, '') = target.target_variant_key) as parent_collision_count,
         (select count(*)::int from pkg17r_targets target join public.card_printings cpr on cpr.id = target.target_child_id) as child_collision_count,
         (select count(*)::int from pkg17r_targets target join public.card_print_identity cpi on cpi.card_print_id = target.target_parent_id and cpi.is_active = true) as identity_target_collision_count,
    ...
    const childInsert = await client.query(
      `insert into public.card_printings (
       ...
       select
         ...
         concat(target.set_key, ':', target.card_number, ':league_reverse_bulk:', target.target_variant_key, ':', target.target_finish_key),
         $1::text,
         null, null, null, null, null,
         'representative_shared_stamp',
         concat('Stamped identity child finish routed from two-source League reverse evidence: ', target.target_finish_key)
       from pkg17r_targets target`,
      [CREATED_BY],
    );
    ...
    const proof = await client.query(
      `select
         ...
         (select count(*)::int from pkg17r_targets) as target_rows,
         (select count(*)::int from public.card_prints cp join pkg17r_targets target on target.target_parent_id = cp.id) as inserted_parent_rows,
         (select count(*)::int from public.card_prints cp join pkg17r_targets target on target.target_parent_id = cp.id and cp.printed_identity_modifier = target.target_printed_identity_modifier) as inserted_parent_modifier_rows,
         (select count(*)::int from public.card_print_identity cpi join pkg17r_targets target on target.target_parent_id = cpi.card_print_id and cpi.is_active = true) as inserted_identity_rows,
         (select count(*)::int from public.card_printings cpr join pkg17r_targets target on target.target_child_id = cpr.id) as inserted_child_rows,
         (select count(*)::int from public.card_printings cpr join pkg17r_targets target on target.target_child_id = cpr.id and cpr.finish_key = 'stamped') as forbidden_stamped_child_rows`,
...
function renderMarkdown(report) {
  ...
  return `# PKG-17R League Reverse Bulk Guarded Dry Run V1
...
async function main() {
  ...
  const recommended = execution.dry_run_status === 'pkg17r_completed_rolled_back_no_durable_change' && execution.rollback_verified && execution.stop_findings.length === 0
  ...
  const report = {
    ...
    version: 'english_master_index_pkg17r_league_reverse_bulk_guarded_dry_run_v1',
    package_id: PACKAGE_ID,

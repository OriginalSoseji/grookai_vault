...
const UPLOAD_RESULT_JSONL = path.join(OUTPUT_DIR, 'self_hosted_images_wh12b_mfb_parent_source_storage_upload_apply_result_v1.jsonl');
const PLAN_JSONL = path.join(OUTPUT_DIR, 'self_hosted_images_wh12c_mfb_parent_db_pointer_repoint_plan_v1.jsonl');
const SUMMARY_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh12c_mfb_parent_db_pointer_repoint_dry_run_summary_v1.json');
const SUMMARY_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh12c_mfb_parent_db_pointer_repoint_dry_run_summary_v1.md');
const PACKAGE_ID = 'IMG-HOST-WH-12C-MFB-PARENT-DB-POINTER-REPOINT-DRY-RUN';
...
async function main() {
  ...
  const missingProposedPaths = plans.filter((row) => !clean(row.proposed_values.image_path));
  ...
  const stopFindings = [
    ...
    ...(missingProposedPaths.length ? ['missing_proposed_storage_paths'] : []),
  ...
  const summary = {
    ...
    missing_proposed_storage_paths: missingProposedPaths.length,
    ...
    samples: {
      metadata_pointer_repoint: effectivePlans.slice(0, 10),
    },
  };

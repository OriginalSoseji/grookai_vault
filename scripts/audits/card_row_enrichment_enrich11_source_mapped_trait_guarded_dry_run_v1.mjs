...
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'enrich11_source_mapped_trait_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'enrich11_source_mapped_trait_guarded_dry_run_v1.md');
const PACKAGE_ID = 'ENRICH-11-SOURCE-MAPPED-TRAIT-BACKFILL';
const FETCH_TIMEOUT_MS = Number.parseInt(process.env.ENRICH11_FETCH_TIMEOUT_MS ?? '10000', 10);
const FETCH_CONCURRENCY = Number.parseInt(process.env.ENRICH11_FETCH_CONCURRENCY ?? '12', 10);
const SOURCE_FILTER = new Set(
  String(process.env.ENRICH11_SOURCES ?? 'pokemonapi,tcgdex')
    .split(',')
    .map((source) => source.trim())
    .filter(Boolean),
);
const LIMIT = Number.parseInt(process.env.ENRICH11_LIMIT ?? '0', 10);
...
async function loadCandidateMappings(client) {
  ...
  `);
  const filtered = result.rows.filter((row) => SOURCE_FILTER.has(row.source));
...
function sourceCardPayload(source, response) {
  if (source === 'pokemonapi') return response?.data ?? null;
  return response ?? null;
}

function sourceCardNumber(source, card) {
  if (source === 'tcgdex') return card?.localId ?? null;
...
function buildTraitRows(target, card) {
  ...
  if (supertype) {
    traits.push({
      card_print_id: target.card_print_id,
      trait_type: 'pokemon:supertype',
      trait_value: supertype,
      source,
      confidence: null,
      hp: null,
      national_dex: null,
  ...
  for (const legal of legalities(source, card)) {
    traits.push({
      card_print_id: target.card_print_id,
      trait_type: 'pokemon:legal',
      trait_value: legal,
      source,
      confidence: null,
      hp: null,
...
async function classifyCandidate(candidate) {
  ...
  try {
    const response = await fetchJsonWithCurl(url);
    const card = sourceCardPayload(candidate.source, response);
    if (!card) {
      return { status: 'blocked', row: { ...candidate, source_url: url, blocked_reason: 'missing_card_payload' } };
    }

    const apiName = sourceCardName(card);
    const apiNumber = sourceCardNumber(candidate.source, card);
    const nameMatches = normalizeName(candidate.card_name) === normalizeName(apiName);
    const numberMatches = normalizeNumber(candidate.number) === normalizeNumber(apiNumber);
    const traitRows = buildTraitRows(candidate, card);
    const hasStats = traitRows.some((row) => row.trait_type === 'pokemon:stats');

    if (!nameMatches || !numberMatches || !hasStats || traitRows.length === 0) {
      return {
        status: 'blocked',
        row: {
          ...candidate,
          source_url: url,
          source_card_name: apiName,
          source_card_number: apiNumber,
          normalized_db_name: normalizeName(candidate.card_name),
          normalized_source_name: normalizeName(apiName),
          normalized_db_number: normalizeNumber(candidate.number),
          normalized_source_number: normalizeNumber(apiNumber),
    ...
    return {
      ...
      row: {
        ...
        source_card_name: apiName,
        source_card_number: apiNumber,
        source_set_id: candidate.source === 'pokemonapi' ? card?.set?.id : card?.set?.id,
        source_set_name: candidate.source === 'pokemonapi' ? card?.set?.name : card?.set?.name,
...
async function acquireTargets(candidateMappings) {
  const accepted = [];
  const blocked = [];
  let cursor = 0;
  const workerCount = Math.max(1, Math.min(FETCH_CONCURRENCY, candidateMappings.length || 1));

  async function worker() {
    while (cursor < candidateMappings.length) {
      const index = cursor;
      cursor += 1;
      const result = await classifyCandidate(candidateMappings[index]);
  ...
  const orderKey = (row) => [
    row.set_code ?? '',
    String(row.number_plain ?? '').padStart(8, '0'),
    row.number ?? '',
    row.card_name ?? '',
    row.card_print_id ?? '',
  ].join('|');
...
async function validateScope(client, accepted) {
  ...
  const result = await client.query(
    `with target as (
       select *
       from jsonb_to_recordset($1::jsonb) as t(card_print_id uuid, source text, trait_row_count int)
     ),
     traits as (
       select *
    ...
    [
      JSON.stringify(accepted.map((row) => ({ card_print_id: row.card_print_id, source: row.source, trait_row_count: row.trait_row_count }))),
  ...
  return {
    ...result.rows[0],
    target_fingerprint_sha256: sha256(stableJson(accepted.map((row) => ({
      card_print_id: row.card_print_id,
...
function guardPassed(guard, expectedTargetCount, expectedTraitCount) {
  return guard.target_count === expectedTargetCount
    && guard.distinct_target_count === expectedTargetCount
    && guard.missing_parent_count === 0
    && guard.non_english_target_count === 0
    && guard.target_already_has_traits_count === 0
    && guard.projected_trait_insert_rows === expectedTraitCount
...
async function runRollbackDryRun(client, accepted) {
  ...
  try {
    ...
    if (!guardPassed(guard, accepted.length, traitRows.length)) {
      ...
      await client.query(
        `create temporary table enrich11_trait_targets (
      ...
      await client.query(
        `insert into enrich11_trait_targets
         select
           card_print_id,
           trait_type,
           trait_value,
      ...
      const inserted = await client.query(
        `insert into public.card_print_traits (
         ...
         select
           ...
           card_category,
           legacy_rarity
         from enrich11_trait_targets
         returning id::text, card_print_id::text, trait_type, trait_value, source, supertype, card_category`,
      );

      const proof = await client.query(
        `select
           (select count(distinct card_print_id)::int from enrich11_trait_targets) as target_count,
           (select count(*)::int from enrich11_trait_targets) as expected_trait_rows,
           (select count(*)::int
            from public.card_print_traits cpt
            join enrich11_trait_targets target
           ...
           (select count(*)::int
            from public.card_print_traits cpt
            where cpt.card_print_id in (select distinct card_print_id from enrich11_trait_targets)
              and not exists (
                select 1
                from enrich11_trait_targets target
                where target.card_print_id = cpt.card_print_id
                  and target.trait_type = cpt.trait_type
                  and target.trait_value = cpt.trait_value
                  and target.source = cpt.source
              )) as unexpected_preexisting_target_trait_rows`,
      );
...
async function main() {
  ...
  try {
    const candidates = await loadCandidateMappings(client);
    ...
    const report = {
      version: 'ENRICH11_SOURCE_MAPPED_TRAIT_GUARDED_DRY_RUN_V1',
      package_id: PACKAGE_ID,
      ...
      scope: {
        ...
        tls_note: 'Live API fetches used curl.exe --ssl-no-revoke for the local Windows certificate revocation-check limitation only. This is not a production default.',
      ...
      acquisition: {
        ...
        accepted_by_set_top_25: Object.fromEntries(Object.entries(countBy(accepted, (row) => row.set_code ?? 'missing_set_code')).slice(0, 25)),
        blocked_samples: blocked.slice(0, 75),
      },
      preflight,
      execution,
      ...
      target_samples: accepted.slice(0, 50).map((row) => ({
        card_print_id: row.card_print_id,
        set_code: row.set_code,
        number: row.number,
        card_name: row.card_name,
    ...
    const md = [
      ...
      '',
      `- Pass: ${report.pass}`,
      `- Source candidates examined: ${candidates.length}`,

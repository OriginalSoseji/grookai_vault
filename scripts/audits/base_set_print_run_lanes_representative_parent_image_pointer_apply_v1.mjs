...
const SOURCE_SUMMARY_JSON = path.join(OUTPUT_DIR, 'base_set_print_run_lanes_image_gap_audit_v1.json');
const SOURCE_PLAN_JSONL = path.join(OUTPUT_DIR, 'base_set_print_run_lanes_representative_parent_pointer_candidate_plan_v1.jsonl');
const APPLY_PLAN_JSON = path.join(OUTPUT_DIR, 'base_set_print_run_lanes_representative_parent_image_pointer_apply_plan_v1.json');
const APPLY_PLAN_MD = path.join(OUTPUT_DIR, 'base_set_print_run_lanes_representative_parent_image_pointer_apply_plan_v1.md');
const APPLY_RESULT_JSON = path.join(OUTPUT_DIR, 'base_set_print_run_lanes_representative_parent_image_pointer_apply_result_v1.json');
const APPLY_RESULT_MD = path.join(OUTPUT_DIR, 'base_set_print_run_lanes_representative_parent_image_pointer_apply_result_v1.md');

const PACKAGE_ID = 'BASE-SET-PRINT-RUN-LANES-REPRESENTATIVE-PARENT-IMAGE-POINTER-APPLY-V1';
const SOURCE_AUDIT_PACKAGE_ID = 'BASE-SET-PRINT-RUN-LANES-IMAGE-GAP-AUDIT-V1';
const APPROVED_SOURCE_FINGERPRINT = '797668d995eeb7cb633e69ed7eaebc627587bfc60a314ce3dddd55472fd57d2f';
const APPROVED_PLAN_FILE_HASH = 'c3ebba06fbb4f71adb7e6ccd0b1b5dc1fd059438cf50d759a24f716318ca0e63';
const EXPECTED_UPDATE_COUNT = 304;
const ALLOWED_TABLES = new Set(['card_prints']);
const ALLOWED_SET_CODES = new Set(['base1-shadowless', 'base1-first-edition', 'base1-1999-2000']);
const ALLOWED_COLUMNS = ['image_source', 'image_path', 'image_status', 'image_note'];

function parseArgs(argv) {
  const args = {
    apply: false,
    chunkSize: Number.parseInt(process.env.BASE_SET_LANE_IMAGE_POINTER_CHUNK_SIZE ?? '250', 10),
  };
...
function validatePlan(summary, planRows, planFileHash) {
  const stopFindings = [];
  if (summary.package_id !== SOURCE_AUDIT_PACKAGE_ID) {
  ...
  for (const row of planRows) {
    ...
    if (row.source_audit_package_id !== SOURCE_AUDIT_PACKAGE_ID) {
      stopFindings.push(`source_audit_package_id_mismatch:${row.target_row_id}`);
    }
    ...
    if (!ALLOWED_SET_CODES.has(row.set_code)) stopFindings.push(`unsupported_set_code:${row.set_code}:${row.target_row_id}`);
    ...
    if (!imagePath?.startsWith('warehouse-derived/self-hosted-images-v1/card_prints/base1/')) {
      stopFindings.push(`non_base1_self_hosted_path:${row.target_row_id}`);
    }
    if (imageStatus !== 'representative_shared') stopFindings.push(`unsupported_status:${imageStatus}:${row.target_row_id}`);
    if (!imageNote?.includes('not an exact') || !imageNote.includes('exact lane image remains uncataloged')) {
...
async function validateCurrentRows(updates) {
  ...
  try {
    ...
    for (const row of updates) {
      ...
      if (!ALLOWED_SET_CODES.has(current.set_code)) stopFindings.push(`current_unsupported_set_code:${current.set_code}:${row.target_row_id}`);
...
function buildApplyPlan(args, summary, planFileHash, validation, currentValidation) {
  ...
  const plan = {
    ...
    source_audit_package_id: summary.package_id,
    source_audit_fingerprint: summary.fingerprint,
  ...
  plan.sql_hash = proofHash({
    ...
    source_audit_fingerprint: plan.source_audit_fingerprint,
...
async function applyUpdates(plan, updates, args) {
  ...
  try {
    ...
    for (const row of updates) {
      ...
      if (!ALLOWED_SET_CODES.has(after.set_code)) {
    ...
    result.result_hash = proofHash({
      ...
      source_audit_fingerprint: result.source_audit_fingerprint,
...
- Mode: ${plan.mode}
- Source audit fingerprint: \`${plan.source_audit_fingerprint}\`
...
- Source audit fingerprint: \`${result.source_audit_fingerprint}\`

...
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'tcgmap07b_fresh_tcgcsv_mapping_insert_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'tcgmap07b_fresh_tcgcsv_mapping_insert_guarded_dry_run_v1.md');
...
async function runRollbackDryRun(client, targets) {
  ...
  try {
    ...
    if (!guardPassed(guard, targets.length)) {
      throw new Error(`TCGMAP-07B guard failed: ${JSON.stringify(guard)}`);
    }
...
async function main() {
  ...
  if (targets.length === 0) throw new Error('No ready rows found in TCGMAP-07A readiness report.');
  ...
  const client = new Client({
    ...
    application_name: 'tcgmap07b_guarded_dry_run',
  });
...
main().catch((error) => {
  console.error('[tcgmap07b-dry-run] failed:', error);
  process.exit(1);
});

...
const WH06D_SUMMARY_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh06d_mcdonalds_dextcg_db_pointer_repoint_dry_run_summary_v1.json');
const WH06D_PLAN_JSONL = path.join(OUTPUT_DIR, 'self_hosted_images_wh06d_mcdonalds_dextcg_db_pointer_repoint_plan_v1.jsonl');
const PLAN_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh06e_mcdonalds_dextcg_db_pointer_repoint_apply_plan_v1.json');
const PLAN_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh06e_mcdonalds_dextcg_db_pointer_repoint_apply_plan_v1.md');
const RESULT_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh06e_mcdonalds_dextcg_db_pointer_repoint_apply_result_v1.json');
const RESULT_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh06e_mcdonalds_dextcg_db_pointer_repoint_apply_result_v1.md');
const PACKAGE_ID = 'IMG-HOST-WH-06E-MCDONALDS-DEXTCG-DB-POINTER-REPOINT-APPLY';
const APPROVED_SOURCE_FINGERPRINT = '7b6eef82541db741cd20cbf8b36bcfdc1a52eb1ef27f9995268b0090e6161f62';
...
const ALLOWED_STATUSES = new Set(['exact', 'representative_shared']);

function parseArgs(argv) {
  const args = {
    apply: false,
    fingerprint: null,
    chunkSize: Number.parseInt(process.env.MCDONALDS_DEXTCG_DB_REPOINT_CHUNK_SIZE ?? '250', 10),
  };
...
function buildUpdates(summary, planRows) {
  ...
  if (!summary.ready_for_apply_package) {
    stopFindings.push(`wh06d_not_apply_ready:${(summary.stop_findings ?? []).join(',')}`);
  }
  ...
  for (const row of planRows) {
    ...
    if (!ALLOWED_SET_CODES.has(String(row.set_code ?? ''))) {
      stopFindings.push(`unsupported_set_code:${row.set_code}:${row.target_row_id}`);
      continue;
    }
    if (row.source_lane !== 'external_dextcg') {
      stopFindings.push(`unsupported_source_lane:${row.source_lane}:${row.target_row_id}`);
      continue;
    }
    ...
    if (row.runtime_public_url_field_write_planned === true) {
    ...
    if (!imagePath) stopFindings.push(`missing_image_path:${row.target_row_id}`);
    if (!ALLOWED_STATUSES.has(String(imageStatus ?? ''))) {
      stopFindings.push(`unsupported_image_status:${imageStatus}:${row.target_row_id}`);
    }
...
function buildPlan(args, summary, updates) {
  const plan = {
    ...
    source_plan_jsonl: path.relative(ROOT, WH06D_PLAN_JSONL),
...
async function main() {
  ...
  const wh06dSummary = await readJson(WH06D_SUMMARY_JSON);
  const wh06dPlanRows = await readJsonl(WH06D_PLAN_JSONL);
  const updates = buildUpdates(wh06dSummary, wh06dPlanRows);
  const plan = buildPlan(args, wh06dSummary, updates);

...
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'base_pikachu_variants_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'base_pikachu_variants_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'base_pikachu_variants_real_apply_v1.md');

const PACKAGE_ID = 'SPECIAL-VAR-04-BASE-PIKACHU-VARIANT-PARENT-INSERTS';
const CREATED_BY = 'english_master_index_special_variant_base_pikachu_variants_real_apply_v1';
const EXPECTED_PACKAGE_FINGERPRINT = '1707129776982c793e5e507370e1f425acbb9a056f25f8a01dfc165394274dcb';
const EXPECTED_SQL_HASH = '75ae3600600ea5eb0c730e05cf14a1938dc7d06f1527fce5a4b4d0183494e38a';
const EXPECTED_DRY_RUN_PROOF = '72203de8200dfe5df1bb2c4c78c518593b1268307ac8a54f87a96c310c48c49b';
const EXPECTED_PRE_APPLY_HASH = '71b7d277e7f0223377d4671a6637241c5cab460ee3788ab64f6fecfd6f0bcf15';
...
function validateDryRun(dryRun) {
  ...
  if (dryRun.execution?.dry_run_status !== 'base_pikachu_variants_parent_insert_completed_rolled_back_no_durable_change') findings.push('dry_run_status_not_passed');
  ...
  if (dryRun.scope?.by_set?.base1 !== EXPECTED_TARGET_COUNT) findings.push('set_scope_mismatch');
  ...
  for (const row of dryRun.scope?.targets ?? []) {
    if (row.set_key !== 'base1') findings.push(`target_set_mismatch:${row.source_candidate_key}`);
    ...
    if (row.target_finish_key !== 'normal') findings.push(`target_finish_mismatch:${row.source_candidate_key}`);
...
async function applyPackage(client, targets) {
  ...
  try {
    ...
    const parentInsert = await client.query(
      `insert into public.card_prints (
       ...
       select
         ...
         concat('Recognized Base Set Pikachu special-case identity: ', target.target_variant_key, '. Representative base image only until exact variant image is available.')
       from base_pikachu_variant_targets target
       join public.card_prints base on base.id = target.base_parent_id`,
      [PACKAGE_ID],
    );
    ...
    const childInsert = await client.query(
      `insert into public.card_printings (
       ...
       select
         ...
         'Recognized Base Set Pikachu special-case child. Representative image only until exact variant image is available.'
       from base_pikachu_variant_targets target`,
      [CREATED_BY],
    );
    ...
    return {
      apply_status: 'base_pikachu_variants_parent_insert_applied',
      guard: guardRow,
      proof: proofRow,
...
function renderMarkdown(report) {
  return `# Base Pikachu Variants Real Apply V1

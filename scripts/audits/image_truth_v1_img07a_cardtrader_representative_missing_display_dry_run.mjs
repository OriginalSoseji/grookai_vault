...
const STAGING_ASSET_DIR = path.join('tmp', 'nonproduction_image_staging', 'image_truth_v1', 'img07a');
const DRY_RUN_JSON = path.join(OUTPUT_DIR, 'image_truth_img07a_cardtrader_representative_missing_display_dry_run_v1.json');
const DRY_RUN_MD = path.join(OUTPUT_DIR, 'image_truth_img07a_cardtrader_representative_missing_display_dry_run_v1.md');

const PACKAGE_ID = 'IMG-07A-CARDTRADER-REPRESENTATIVE-MISSING-DISPLAY-CHILD-IMAGE-DRY-RUN';
...
async function normalizeAsset(row) {
  ...
  const plannedStoragePath = [
    'warehouse-derived',
    'image-truth-v1',
    'img07a-cardtrader-representative',
    row.set_code,
...
async function main() {
  ...
  const sourceRows = (manifest.rows ?? [])
    ...
    .filter((row) => row.source_key === 'cardtrader_blueprint_index')
  ...
  await fs.writeFile(DRY_RUN_MD, `# Image Truth IMG-07A CardTrader Representative Missing-Display Dry Run V1

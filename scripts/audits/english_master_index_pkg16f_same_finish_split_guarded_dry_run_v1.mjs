...
const READINESS_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg16e_same_finish_split_parent_insert_readiness_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg16f_same_finish_split_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg16f_same_finish_split_guarded_dry_run_v1.md');

const PACKAGE_ID = 'PKG-16F-SAME-FINISH-SPLIT-PARENT-INSERTS';
const CREATED_BY = 'pkg16f_same_finish_split_guarded_dry_run_v1';
...
async function runDryRun(client, targets, packageFingerprint) {
  ...
  try {
    ...
    return {
      dry_run_status: 'pkg16f_failed_rolled_back',
...
function renderMarkdown(report) {
  ...
  return `# PKG-16F Same-Finish Split Guarded Dry Run V1
...
async function main() {
  ...
  const report = {
    ...
    version: 'english_master_index_pkg16f_same_finish_split_guarded_dry_run_v1',
    package_id: PACKAGE_ID,

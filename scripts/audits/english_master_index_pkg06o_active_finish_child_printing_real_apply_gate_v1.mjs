...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06o_active_finish_child_printing_real_apply_gate_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg06o_active_finish_child_printing_real_apply_gate_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260609_pkg06o_active_finish_child_printing_real_apply_gate_checkpoint_v1.md');
...
function validate(dryRun) {
  ...
  if (dryRun.scope?.child_printing_rows !== EXPECTED_CHILD_ROWS) findings.push('child_count_not_40');
  if (dryRun.scope?.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('parent_count_not_40');
...
function renderMarkdown(report) {
  return `# PKG-06O Active Finish Child Printing Real Apply Gate V1
...
function renderCheckpoint(report) {
  return `# PKG-06O Active Finish Child Printing Real Apply Gate Checkpoint V1
...
function updateCheckpointIndex() {
  ...
  const line = '| 2026-06-09 | [PKG-06O Active Finish Child Printing Real Apply Gate Checkpoint V1](20260609_pkg06o_active_finish_child_printing_real_apply_gate_checkpoint_v1.md) | No-write gate for 40 active-finish child-only inserts across 29 sets; real apply blocked pending exact approval. |';
  const current = fs.existsSync(indexPath) ? fs.readFileSync(indexPath, 'utf8') : '# Master Index Checkpoints\n';
  if (current.includes('20260609_pkg06o_active_finish_child_printing_real_apply_gate_checkpoint_v1.md')) {
    fs.writeFileSync(indexPath, current.split('\n').map((existing) => (
      existing.includes('20260609_pkg06o_active_finish_child_printing_real_apply_gate_checkpoint_v1.md') ? line : existing
    )).join('\n'));
  } else {
...
const requiredApproval = `Approve real ${PACKAGE_ID} apply only. Fingerprint: ${dryRun.package_fingerprint_sha256}. SQL hash: ${dryRun.sql_hash_sha256}. Scope: 40 child-only card_printing inserts for base2/Jungle, base3/Fossil, base5/Team Rocket, neo3/Neo Revelation, ex2/Sandstorm, ex3/Dragon, dpp/DP Black Star Promos, pop7/POP Series 7, dp6/Legends Awakened, pl1/Platinum, hgss3/HS-Undaunted, bw8/Plasma Storm, xy2/Flashfire, xy5/Primal Clash, xy6/Roaring Skies, xy10/Fates Collide, xy11/Steam Siege, smp/SM Black Star Promos, sm2/Guardians Rising, sm3/Burning Shadows, sm3.5/Shining Legends, sm6/Forbidden Light, 2018sm/McDonald's Collection 2018, sm9/Team Up, fut20/Pokemon Futsal Collection, swsh11/Lost Origin, sv06/Twilight Masquerade, sv08/Surging Sparks, and sv08.5/Prismatic Evolutions; finishes cosmos=24, holo=9, normal=7; target parents=40. Dry-run proof: ${dryRunProof}. No global apply. No migrations. No deletes. No merges. No unsupported cleanup. No parent writes.`;

const report = {
  generated_at: new Date().toISOString(),
  version: 'english_master_index_pkg06o_active_finish_child_printing_real_apply_gate_v1',
  audit_only: true,
  ...
  required_operator_decision: {
    ...
    approval_effect: 'This would authorize preparing and running the durable PKG-06O active-finish child-only apply path only.',
  },

...
const READINESS_JSON = path.join(OUTPUT_DIR, 'tcgmap04_staging_tcgplayer_bridge_readiness_v1.json');
const DRY_RUN_JSON = path.join(OUTPUT_DIR, 'tcgmap04a_staging_tcgplayer_mapping_insert_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'tcgmap04a_staging_tcgplayer_mapping_insert_real_apply_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'tcgmap04a_staging_tcgplayer_mapping_insert_real_apply_v1.md');

const PACKAGE_ID = 'TCGMAP-04A-STAGING-TCGPLAYER-MAPPING-INSERTS';
const EXPECTED_READINESS_FINGERPRINT = '3013734055a13e51c4c3f1f517caa2c6d713becb126f0d8b30c339641aba4001';
const EXPECTED_TARGET_FINGERPRINT = 'cad93b765851870f1eaedb2f75c1c55a12ab7c5a0be8ff4a84d72e24520d7119';
const EXPECTED_DRY_RUN_PROOF = '4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945';
const EXPECTED_TARGET_ROWS = 728;
const APPROVAL_TEXT = 'Approve real TCGMAP-04A-STAGING-TCGPLAYER-MAPPING-INSERTS apply only.';
...
function buildTargets(readiness) {
  return (readiness.ready_rows ?? []).map((row) => ({
    ...
    meta: {
      derived_from: 'external_discovery_candidates.tcgplayer_id',
      external_discovery_candidate_id: row.external_discovery_candidate_id,
      staging_source: row.staging_source,
      upstream_id: row.upstream_id,
      match_status: row.match_status,
      candidate_bucket: row.candidate_bucket,
      classifier_version: row.classifier_version,
      promoted_by: 'tcg_mapping_tcgmap04a_staging_tcgplayer_mapping_insert_guarded_dry_run_v1',
    },
  }));
}
...
function validateArtifacts(readiness, dryRun, targets) {
  ...
  if (readiness.totals?.ready_rows !== EXPECTED_TARGET_ROWS) findings.push('readiness_target_count_mismatch');
  ...
  const result = await client.query(
    `with target as (
       ...
       from jsonb_to_recordset($1::jsonb) as t(
         ...
         number text,
         name text,
         external_discovery_candidate_id uuid,
         staging_source text,
         upstream_id text,
         match_status text,
...
async function applyPackage(client, targets) {
  ...
  try {
    ...
    await client.query(
      `create temporary table tcgmap04a_targets (
         ...
         number text,
         name text,
         external_discovery_candidate_id uuid not null,
         staging_source text,
         upstream_id text,
         match_status text,
         candidate_bucket text,
    ...
    const insertResult = await client.query(
      `insert into public.external_mappings (
       ...
       select
         ...
         now(),
         meta
       from tcgmap04a_targets`,
    );

    const verification = await client.query(
      `with target as (
         select * from tcgmap04a_targets
       )
       select
         (select count(*)::int
          from public.external_mappings em
...
function buildMarkdown(report) {
  ...
  lines.push('');
  lines.push('Scoped real apply for staging-resolved TCGplayer external mappings.');
  lines.push('');
  lines.push('## Result');
  lines.push('');
  ...
  lines.push(markdownTable(report.sample_rows, [
    ...
    { label: 'staging', value: (row) => row.staging_source },
  ]));
  lines.push('');
  return `${lines.join('\n')}\n`;
}
...
async function main() {
  ...
  const client = new Client({
    ...
    application_name: 'tcgmap04a_real_apply',
  });
...
main().catch((error) => {
  console.error('[tcgmap04a-real-apply] failed:', error);
  process.exit(1);
});

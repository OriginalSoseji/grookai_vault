...
const SOURCE_JSON = path.join(OUTPUT_DIR, 'enrich13d_promo_prefix_duplicate_adjudication_v1.json');
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'enrich13d1_xyp_duplicate_dependency_transfer_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'enrich13d1_xyp_duplicate_dependency_transfer_guarded_dry_run_v1.md');
const OUTPUT_SQL = path.join(SQL_DIR, 'enrich13d1_xyp_duplicate_dependency_transfer_guarded_dry_run_v1.sql');

const PACKAGE_ID = 'ENRICH-13D1-XYP-DUPLICATE-DEPENDENCY-TRANSFER-DRY-RUN';
...
-- Scope: ${targetRows.length} XYP duplicate parent dependency transfers.
...
begin
  ...
  if v_targets <> ${targetRows.length} then
    raise exception 'ENRICH-13D1 target count guard failed: expected ${targetRows.length}, got %', v_targets;
  end if;
...
async function runDryRun(sql, targetRows) {
  ...
  const client = new Client({ connectionString: cs, application_name: 'enrich13d1_xyp_duplicate_dependency_transfer_guarded_dry_run_v1' });
...
function validate({ source, targetRows, sql, sqlHash, execution }) {
  ...
  if (source.version !== 'ENRICH_13D_PROMO_PREFIX_DUPLICATE_ADJUDICATION_V1') findings.push('source_report_version_unexpected');
  if (targetRows.length !== EXPECTED_ROWS) findings.push(`target_row_count_not_${EXPECTED_ROWS}`);
  if (source.summary?.excluded_suffix_variant_rows_for_enrich13f !== 5) findings.push('suffix_exclusion_count_changed');
  if (source.summary?.manual_review_rows !== 0) findings.push('manual_rows_present_in_source_report');
  ...
  if (Number(before.duplicate_child_printings ?? -1) !== 168) findings.push('before_duplicate_child_printing_count_changed');
  ...
  if (Number(before.duplicate_species_rows ?? -1) !== EXPECTED_ROWS) findings.push('before_duplicate_species_count_changed');
  ...
  if (hash(stableJson(execution.before_snapshot?.rows ?? [])) !== execution.before_snapshot?.hash_sha256) findings.push('before_snapshot_hash_mismatch');
  if (hash(stableJson(execution.after_snapshot?.rows ?? [])) !== execution.after_snapshot?.hash_sha256) findings.push('after_snapshot_hash_mismatch');
...
function renderMarkdown(report) {
  return `# ENRICH-13D1 XYP Duplicate Dependency Transfer Guarded Dry-Run V1
...
const report = {
  version: 'ENRICH_13D1_XYP_DUPLICATE_DEPENDENCY_TRANSFER_GUARDED_DRY_RUN_V1',
  generated_at: new Date().toISOString(),
  package_id: PACKAGE_ID,
  dry_run_execution_status: stopFindings.length === 0
    ? 'enrich13d1_guarded_dry_run_passed_rolled_back_no_durable_change'
    : 'enrich13d1_guarded_dry_run_blocked_or_failed',
  ...
  package_scope: {
    ...
    excluded_suffix_rows: source.summary?.excluded_suffix_variant_rows_for_enrich13f ?? null,
    manual_review_rows: source.summary?.manual_review_rows ?? null,
  },
  sql_artifact: {
    path: OUTPUT_SQL,
    sha256: sqlHash,

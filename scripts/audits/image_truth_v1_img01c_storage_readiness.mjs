...
const INPUT_JSON = path.join(OUTPUT_DIR, 'image_truth_img01b_missing_display_dry_run_v1.json');
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'image_truth_img01c_storage_readiness_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'image_truth_img01c_storage_readiness_v1.md');
const STORAGE_BUCKET = 'user-card-images';
const PACKAGE_ID = 'IMG-01C-STORAGE-READINESS-NO-UPLOAD';
...
function buildMarkdown(report) {
  return `# Image Truth V1 IMG-01C Storage Readiness
...
If approved later, the apply sequence must upload exactly these local assets to \`${STORAGE_BUCKET}\`, then update only the matching \`public.card_printings\` image fields after a fresh target recheck.
`;
}
...
async function main() {
  ...
  const sourceRows = (input.rows ?? [])
    ...
    .filter((row) => row.image_confidence === 'exact')

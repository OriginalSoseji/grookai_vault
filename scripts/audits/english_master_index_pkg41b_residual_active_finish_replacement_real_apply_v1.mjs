...
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg41b_residual_active_finish_replacement_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg41b_residual_active_finish_replacement_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg41b_residual_active_finish_replacement_real_apply_v1.md');

const PACKAGE_ID = 'PKG-41B-RESIDUAL-ACTIVE-FINISH-REPLACEMENT';
const EXPECTED_PACKAGE_FINGERPRINT = '28c8db2789c9a57cdfc05b3b10351661a90a83e3dcb0be4c9f12200c01fc0f44';
const EXPECTED_DRY_RUN_PROOF_HASH = '8b159b0af50241c887aff7f51ee8a40c3c86145aa2e0e8db5ed38b595c7ac236';
...
async function runApply(client, targets, packageFingerprint) {
  ...
  const beforeDependencies = await dependencyCounts(client, replacementTargets.map((row) => row.current_child_printing_id));
  if (
    ...
    throw new Error(`fresh dependency guard failed: ${JSON.stringify(beforeDependencies)}`);
  }
  ...
  try {
    ...
    if (
      ...
      throw new Error(`PKG-41B real-apply guard failed: ${JSON.stringify(guardRow)}`);
    }
    ...
    if (
      parentUpdate.rowCount !== targets.length
      || identityInsert.rowCount !== 2
      || childInsert.rowCount !== replacementTargets.length
      || childDelete.rowCount !== replacementTargets.length
    ) {
      throw new Error(`PKG-41B apply count mismatch: parent=${parentUpdate.rowCount} identity=${identityInsert.rowCount} child_insert=${childInsert.rowCount} child_delete=${childDelete.rowCount}`);
    }
...
function renderMarkdown(report) {
  return `# PKG-41B Residual Active Finish Replacement Real Apply V1

Real apply for the explicitly approved PKG-41B scope.

## Safety

- durable_db_writes_performed: ${report.durable_db_writes_performed}
- migrations_created: ${report.migrations_created}
- parent_inserts_performed: ${report.parent_inserts_performed}
...
${markdownTable(['metric', 'value'], [
    ...
    ['child_inserts_applied', report.summary.child_inserts_applied],
    ['child_deletes_applied', report.summary.child_deletes_applied],
  ])}
...
const packageFingerprint = sha256(stableJson({
  ...
  source_fingerprint: dryRun.source_fingerprint_sha256,
  targets,
}));
...
const report = {
  ...
  version: 'english_master_index_pkg41b_residual_active_finish_replacement_real_apply_v1',
  package_id: PACKAGE_ID,
  ...
  parent_inserts_performed: false,
  ...
  summary: {
    ...
    child_inserts_applied: applyResult.child_insert_rows.length,
    child_deletes_applied: applyResult.child_delete_count,

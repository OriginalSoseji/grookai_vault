...
const PLAN_JSON = path.join(OUTPUT_DIR, 'image_truth_img12b_pricecharting_exact_variant_upload_apply_plan_v1.json');
const PLAN_MD = path.join(OUTPUT_DIR, 'image_truth_img12b_pricecharting_exact_variant_upload_apply_plan_v1.md');
const RESULT_JSON = path.join(OUTPUT_DIR, 'image_truth_img12b_pricecharting_exact_variant_upload_apply_result_v1.json');
const RESULT_MD = path.join(OUTPUT_DIR, 'image_truth_img12b_pricecharting_exact_variant_upload_apply_result_v1.md');
const STAGING_ASSET_DIR = path.join('tmp', 'nonproduction_image_staging', 'image_truth_v1', 'img12b');
const STORAGE_BUCKET = 'user-card-images';
const PACKAGE_ID = 'IMG-12B-PRICECHARTING-EXACT-VARIANT-CHILD-IMAGE-UPLOAD-APPLY';
...
async function normalizeAsset(row, assetUrl) {
  ...
  return {
    ...
    planned_normalized_front_storage_path: [
      'warehouse-derived',
      'image-truth-v1',
      'img12b-pricecharting-exact-variant',
      row.set_code,
...
function buildMarkdown(report) {
  return `# Image Truth IMG-12B PriceCharting Exact Variant Upload Apply ${report.mode === 'real_apply' ? 'Result' : 'Plan'}

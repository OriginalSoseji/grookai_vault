...
const FIXTURE_DIR = 'docs/audits/verified_master_set_index_v1/source_fixtures/generated_pokescope_me01_stamp_v1';
const REPORT_DIR = 'docs/audits/english_master_index_source_exhaustion_v1/pokescope_me01_stamp_acquisition_v1';

const TARGETS = new Map([
  ['3', { source_url: 'https://pokescope.app/card/me1-3/' }],
  ['36', { source_url: 'https://pokescope.app/card/me1-36/' }],
  ['50', { source_url: 'https://pokescope.app/card/me1-50/' }],
  ['77', { source_url: 'https://pokescope.app/card/me1-77/' }],
  ['100', { source_url: 'https://pokescope.app/card/me1-100/' }],
  ['104', { source_url: 'https://pokescope.app/card/me1-104/' }],
]);
...
function validatePokeScope({ text, title, rawHtml }, fact) {
  ...
  const hasIdentity = comparable.includes(normalizeText(fact.card_name))
    && comparable.includes('mega evolution')
    && (comparable.includes(normalizeText(`me1-${number}`)) || comparable.includes(normalizeText(`#${number}`)));
  const hasIdentifier = comparable.includes(normalizeText(`me1-${number}`))
    || comparable.includes(normalizeText(`productID me1-${number}`))
    || comparable.includes(normalizeText(`sku me1-${number}`))
    || comparable.includes(normalizeText(`cardid me1-${number}`));
  const hasStampVariant = compactComparable.includes('playpokemonstampholofoil')
...
function fixtureRecord(fact, target, generatedAt) {
  return {
    ...
    evidence_label: `${sourceAuthority(target.source_url)} exact Mega Evolution stamped variant evidence: ${fact.card_name} ${fact.card_number}`,
    language: 'en',
    retrieved_at: generatedAt,
    raw_snapshot_ref: `pokescope_me01_stamp:${fact.set_key}:${normalizeNumber(fact.card_number)}:stamped:${generatedAt}`,
    notes: 'Audit-only exact marketplace/checklist evidence. Accepted only after validating PokeScope page identity and explicit embedded Play Pokemon stamp variant evidence.',
  };
}
...
async function main() {
  ...
  const skippedAlreadyClosed = [...TARGETS.keys()]
    ...
    .map((number) => ({
      ...
      reason: 'The static PokeScope target is no longer present in the current remaining queue; no evidence fixture is generated.',
  ...
  if (blocked.length > 0) {
    ...
    throw new Error(`ME01 stamped evidence validation failed for ${blocked.map((row) => row.fact.card_number).join(', ')}`);
  }

  const fixtureFile = path.join(FIXTURE_DIR, 'me01.json');
  if (!options.dryRun) {
    ...
    await fs.writeFile(fixtureFile, `${JSON.stringify({
      ...
      source_key: 'pokescope_me01_stamp_v1',
      source_kind: 'marketplace_checklist',
      ...
      retrieved_at: generatedAt,
      raw_snapshot_ref: `generated_fixture:pokescope_me01_stamp:${generatedAt}`,
      audit_only: true,
  ...
  const report = {
    version: 'english_master_index_pokescope_me01_stamp_acquisition_v1',
    ...
    rule: 'Only exact ME01 remaining queue rows are accepted. Stamped finish is not inferred from set membership or rarity.',
    ...
    summary: {
      ...
      by_source_key: { pokescope_me01_stamp: validated.length },
    },
    results,
  ...
  if (!options.dryRun) {
    ...
    await fs.writeFile(path.join(REPORT_DIR, 'pokescope_me01_stamp_acquisition_v1.json'), `${JSON.stringify(report, null, 2)}\n`);
    await fs.writeFile(path.join(REPORT_DIR, 'pokescope_me01_stamp_acquisition_v1.md'), [
      '# PokeScope ME01 Stamp Acquisition V1',
      '',
...
main().catch((error) => {
  console.error('[pokescope-me01-stamp] failed:', error);
  process.exitCode = 1;
});

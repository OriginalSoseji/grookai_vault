...
const PLAN_JSONL = path.join(OUTPUT_DIR, 'self_hosted_images_wh14a_energy_template_pointer_plan_v1.jsonl');
const SUMMARY_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh14a_energy_template_pointer_dry_run_summary_v1.json');
const SUMMARY_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh14a_energy_template_pointer_dry_run_summary_v1.md');
const PACKAGE_ID = 'IMG-HOST-WH-14A-ENERGY-TEMPLATE-POINTER-DRY-RUN';
...
async function fetchPlans(client) {
  const result = await client.query(`
    ...
    select
      ...
      t.set_code,
      t.number,
      t.image_source as current_image_source,
      t.image_path as current_image_path,
      t.image_status as current_image_status,
      t.image_note as current_image_note,
      s.id as source_row_id,
      s.gv_id as source_gv_id,
      s.set_code as source_set_code,
    ...
    left join lateral (
      select cp.*
      from public.card_prints cp
      where lower(cp.name) = lower(t.name)
        and cp.image_path is not null
        and cp.image_source = 'identity'
        and cp.image_status = 'exact'
        and coalesce(cp.variant_key, '') = ''
      order by
        case when cp.set_code = 'sve' then 0 else 1 end,
        cp.number_plain nulls last,
        cp.number,
        cp.gv_id
      limit 1
    ) s on true
    order by t.set_code, t.number, t.gv_id
  `);

  return result.rows.map((row) => {
    const proposedImageStatus = 'representative_shared';
    const proposedImageNote = row.source_gv_id
      ? `Energy-template representative image from ordinary source card ${row.source_gv_id}. This ${String(row.set_code).toUpperCase()} row is not an exact image claim; exact row-specific energy artwork remains uncataloged. Prepared by ${PACKAGE_ID}.`
      : null;
    const proposedImageSource = row.source_gv_id ? 'identity' : null;
    const proposedImagePath = clean(row.source_image_path);
    const changedColumns = [];
    if (clean(row.current_image_source) !== proposedImageSource) changedColumns.push('image_source');
    if (clean(row.current_image_path) !== proposedImagePath) changedColumns.push('image_path');
    if (clean(row.current_image_status) !== proposedImageStatus) changedColumns.push('image_status');
    if (clean(row.current_image_note) !== proposedImageNote) changedColumns.push('image_note');
    return {
      package_id: PACKAGE_ID,
      plan_type: 'energy_template_representative_pointer',
      ...
      proposed_values: {
        ...
        image_status: proposedImageStatus,
        image_note: proposedImageNote,
      },
...
async function main() {
  ...
  const unexpectedExactClaims = plans.filter((row) => row.proposed_values.image_status !== 'representative_shared');
  const unsupportedColumns = plans.flatMap((row) => row.changed_columns).filter((column) => !['image_source', 'image_path', 'image_status', 'image_note'].includes(column));
  const stopFindings = [
    ...(missingSources.length ? ['missing_energy_template_source_rows'] : []),
    ...(unexpectedExactClaims.length ? ['unexpected_exact_claim_status'] : []),
  ...
  const summary = {
    ...
    by_source_set_code: countBy(effectivePlans, (row) => row.source_set_code),
    ...
    samples: {
      pointer_updates: effectivePlans.slice(0, 16),
...
${markdownTable(topEntries(summary.by_source_set_code))}

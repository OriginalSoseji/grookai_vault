...
const SOURCE_SUMMARY_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh09b_world_championship_representative_pointer_dry_run_summary_v1.json');
const SOURCE_PLAN_JSONL = path.join(OUTPUT_DIR, 'self_hosted_images_wh09b_world_championship_representative_pointer_plan_v1.jsonl');
const APPLY_PLAN_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh09c_world_championship_representative_pointer_apply_plan_v1.json');
const APPLY_PLAN_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh09c_world_championship_representative_pointer_apply_plan_v1.md');
const APPLY_RESULT_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh09c_world_championship_representative_pointer_apply_result_v1.json');
const APPLY_RESULT_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh09c_world_championship_representative_pointer_apply_result_v1.md');

const PACKAGE_ID = 'IMG-HOST-WH-09C-WORLD-CHAMPIONSHIP-REPRESENTATIVE-POINTER-APPLY';
const SOURCE_DRY_RUN_PACKAGE_ID = 'IMG-HOST-WH-09B-WORLD-CHAMPIONSHIP-REPRESENTATIVE-POINTER-DRY-RUN';
const APPROVED_SOURCE_FINGERPRINT = '905f425a61c12ebcf213bb905b4e7fdd1d2b0756ba37014b1a797a40a2103219';
const APPROVED_PLAN_FILE_HASH = 'b2fd96f5ea7aa31cc346546726cf0a5ab2b0eb5819e8981d3168a3103c34c14e';
const EXPECTED_UPDATE_COUNT = 1560;
const ALLOWED_COLUMNS = ['image_source', 'image_path', 'image_status', 'image_note'];

function parseArgs(argv) {
  const args = {
    apply: false,
    chunkSize: Number.parseInt(process.env.WCD_REPRESENTATIVE_POINTER_CHUNK_SIZE ?? '250', 10),
  };
...
function validatePlan(summary, planRows, planFileHash) {
  const stopFindings = [];
  if (summary.package_id !== SOURCE_DRY_RUN_PACKAGE_ID) {
    stopFindings.push(`source_package_mismatch:${summary.package_id}`);
  }
  if (summary.fingerprint !== APPROVED_SOURCE_FINGERPRINT) {
    stopFindings.push(`source_fingerprint_mismatch:${summary.fingerprint}`);
  }
  if (planFileHash !== APPROVED_PLAN_FILE_HASH) {
    stopFindings.push(`plan_file_hash_mismatch:${planFileHash}`);
  }
  if (summary.ready_for_apply_package !== true) {
    stopFindings.push(`source_not_apply_ready:${(summary.stop_findings ?? []).join(',')}`);
  }
  ...
  for (const row of planRows) {
    ...
    if (row.source_dry_run_package_id !== SOURCE_DRY_RUN_PACKAGE_ID) {
      stopFindings.push(`source_dry_run_package_id_mismatch:${row.target_row_id}`);
    }
    if (row.plan_type !== 'metadata_pointer_repoint') stopFindings.push(`unsupported_plan_type:${row.plan_type}`);
    if (row.target_table !== 'card_prints') stopFindings.push(`unsupported_table:${row.target_table}`);
    if (!String(row.set_code ?? '').startsWith('wcd')) stopFindings.push(`unsupported_set_code:${row.set_code}:${row.target_row_id}`);
    if (row.variant_key !== 'world_championship_deck_replica') {
      stopFindings.push(`unsupported_variant_key:${row.variant_key}:${row.target_row_id}`);
    }
    ...
    const disallowedChangedColumns = changedColumns.filter((column) => !ALLOWED_COLUMNS.includes(column));
    ...
    if (!imagePath?.startsWith('warehouse-derived/self-hosted-images-v1/card_prints/')) {
      stopFindings.push(`non_self_hosted_path:${row.target_row_id}`);
    }
    if (imagePath?.includes('/wcd')) {
      stopFindings.push(`wcd_exact_like_path:${row.target_row_id}`);
    }
...
async function validateCurrentRows(updates) {
  ...
  try {
    const currentRows = await fetchCurrentRows(client, updates.map((row) => row.target_row_id));
    const stopFindings = [];
    for (const row of updates) {
      const current = currentRows.get(row.target_row_id);
      if (!current) {
        stopFindings.push(`missing_current_db_row:${row.target_row_id}`);
        continue;
      }
      ...
      if (!String(current.set_code ?? '').startsWith('wcd')) {
        stopFindings.push(`current_unsupported_set_code:${current.set_code}:${row.target_row_id}`);
      }
      if (current.variant_key !== 'world_championship_deck_replica') {
        stopFindings.push(`current_unsupported_variant_key:${current.variant_key}:${row.target_row_id}`);
      }
      for (const column of ALLOWED_COLUMNS) {
        if (!valuesMatch(current[column], row.current_values[column])) {
          stopFindings.push(`current_value_drift:${row.target_row_id}:${column}`);
        }
      }
    }
    return { currentRows, stopFindings: [...new Set(stopFindings)].slice(0, 200) };
  } finally {
    await client.end();
  }
}
...
function buildApplyPlan(args, summary, planFileHash, validation, currentValidation) {
  ...
  const plan = {
    ...
    target_tables: countBy(updates, (row) => row.target_table),
  ...
  try {
    ...
    for (const batch of chunk(updates, args.chunkSize)) {
      ...
      if (rowCount !== batch.length) {
        throw new Error(`Batch rowcount mismatch: expected ${batch.length}, updated ${rowCount}`);
      }
    }
    const afterRows = await fetchCurrentRows(client, updates.map((row) => row.target_row_id));
    const afterFindings = [];
    for (const row of updates) {
      const after = afterRows.get(row.target_row_id);
      if (!after) {
        afterFindings.push(`missing_after_row:${row.target_row_id}`);
        continue;
      }
      for (const column of ALLOWED_COLUMNS) {
        if (!valuesMatch(after[column], row[column])) {
          afterFindings.push(`after_value_mismatch:${row.target_row_id}:${column}`);
        }
      }
      if (!String(after.set_code ?? '').startsWith('wcd')) {
        afterFindings.push(`after_unsupported_set_code:${after.set_code}:${row.target_row_id}`);
      }
      if (after.variant_key !== 'world_championship_deck_replica') {
        afterFindings.push(`after_unsupported_variant_key:${after.variant_key}:${row.target_row_id}`);
      }
    }
    if (afterFindings.length) {
      throw new Error(`Post-apply verification failed: ${afterFindings.slice(0, 20).join('; ')}`);
    }
...
${markdownTable(topEntries(result.proposed_image_sources))}
...
async function main() {
  ...
  if (!applyPlan.ready_for_apply) {
    throw new Error(`Apply not ready: ${applyPlan.stop_findings.join('; ')}`);
  }
  if (!args.apply) {

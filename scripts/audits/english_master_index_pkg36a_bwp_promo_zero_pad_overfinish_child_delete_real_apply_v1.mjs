...
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg36a_bwp_promo_zero_pad_overfinish_child_delete_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg36a_bwp_promo_zero_pad_overfinish_child_delete_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg36a_bwp_promo_zero_pad_overfinish_child_delete_real_apply_v1.md');

const PACKAGE_ID = 'PKG-36A-BWP-PROMO-ZERO-PAD-OVERFINISH-CHILD-DELETE';
const APPROVED_FINGERPRINT = '91e3dfcd8108f73c626609a9481b8db1c1bd8fb4267240ec88222c5e735b06ac';
const APPROVED_DRY_RUN_PROOF_HASH = 'e10f5afafdd114b89316ecf7e878874e2d2eb66c3b1e7641d04d91bfea31d531';
const APPROVED_APPROVAL_TEXT = 'Approve real PKG-36A-BWP-PROMO-ZERO-PAD-OVERFINISH-CHILD-DELETE apply only. Fingerprint: 91e3dfcd8108f73c626609a9481b8db1c1bd8fb4267240ec88222c5e735b06ac. Scope: 4 BWP promo zero-pad overfinish child deletes; finishes normal=2, reverse=2; target rows Reshiram BW04 normal/reverse and Zekrom BW05 normal/reverse. Dry-run proof: e10f5afafdd114b89316ecf7e878874e2d2eb66c3b1e7641d04d91bfea31d531 == e10f5afafdd114b89316ecf7e878874e2d2eb66c3b1e7641d04d91bfea31d531. No global apply. No migrations. No parent writes. No merges. No quarantine. Holo rows preserved.';
...
function assertDryRun(dryRun) {
  ...
  assertEqual('scope.target_child_deletes', Number(dryRun.scope?.target_child_deletes ?? Number.NaN), 4);
  assertEqual('scope.by_set.bwp', Number(dryRun.scope?.by_set?.bwp ?? Number.NaN), 4);
  assertEqual('scope.by_finish.normal', Number(dryRun.scope?.by_finish?.normal ?? Number.NaN), 2);
  assertEqual('scope.by_finish.reverse', Number(dryRun.scope?.by_finish?.reverse ?? Number.NaN), 2);
  ...
  assertEqual('targets.length', (dryRun.targets ?? []).length, 4);
}
...
async function executeRealApply(client, targets) {
  ...
  try {
    ...
    return {
      apply_status: 'pkg36a_real_apply_committed',
      committed: true,
...
function renderMarkdown(report) {
  return `# PKG-36A BWP Promo Zero-Pad Overfinish Child Delete Real Apply V1

Real apply completed for the explicitly approved PKG-36A scope only.
...
const report = {
  ...
  version: 'english_master_index_pkg36a_bwp_promo_zero_pad_overfinish_child_delete_real_apply_v1',

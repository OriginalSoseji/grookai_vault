...
const PACKAGE_ID = "MEE-CORE-INTERNAL-REVIEW-ACTION-FUNCTION-LOW-SIGNAL-REMAINING-DRAIN-PLAN-V1";
const REVIEW_ACTOR = "system_low_signal_remaining_drain_plan";
...
if (selectedRows.length === 0) {
  throw new Error("No eligible low_signal_monitor rows remain to drain.");
}

const note = "MEE core remaining low_signal_monitor drain proof: confirm monitor_only remains internal only.";
...
const applySql = `-- MEE_CORE_INTERNAL_REVIEW_ACTION_FUNCTION_LOW_SIGNAL_REMAINING_DRAIN_PLAN_V1 apply candidate.
...
const valueRows = selectedRows
  .map((row) => `    (${sqlLiteral(row.id)}::uuid, ${sqlTimestamptz(row.updated_at)}, ${sqlLiteral(row.review_status)}::text, ${sqlLiteral(row.review_disposition)}::text, ${sqlLiteral(row.review_actor)}::text, ${sqlTimestamptz(row.reviewed_at)}, ${sqlJsonb(row.review_payload)}, ${row.needs_review ? "true" : "false"}::boolean)`)
  .join(",\n");
...
const rollbackSql = `-- MEE_CORE_INTERNAL_REVIEW_ACTION_FUNCTION_LOW_SIGNAL_REMAINING_DRAIN_PLAN_V1 rollback candidate.
...
const preflightSql = `-- MEE_CORE_INTERNAL_REVIEW_ACTION_FUNCTION_LOW_SIGNAL_REMAINING_DRAIN_PLAN_V1 preflight SQL.
...
select
  ...
  ${selectedRows.length}::int as expected_target_rows,
...
const readbackSql = `-- MEE_CORE_INTERNAL_REVIEW_ACTION_FUNCTION_LOW_SIGNAL_REMAINING_DRAIN_PLAN_V1 readback SQL.
...
const report = {
  ...
  mode: "plan_only_low_signal_remaining_drain_review_action_function_invoke",
  ...
  artifacts: {
    ...
    apply_sql: "docs/sql/mee_core_internal_review_action_function_low_signal_remaining_drain_v1_apply_candidate.sql",
    rollback_sql: "docs/sql/mee_core_internal_review_action_function_low_signal_remaining_drain_v1_rollback_candidate.sql",
    readback_sql: "docs/sql/mee_core_internal_review_action_function_low_signal_remaining_drain_v1_readback.sql",
    preflight_sql: "docs/sql/mee_core_internal_review_action_function_low_signal_remaining_drain_v1_preflight.sql",
    ...
    plan_md: "docs/plans/market_evidence_engine_v1/MEE_CORE_INTERNAL_REVIEW_ACTION_FUNCTION_LOW_SIGNAL_REMAINING_DRAIN_PLAN_V1.md",
  },
  findings: [],
  boundary_proof: {
    db_writes: false,
...
function renderMarkdown(value) {
  return [
    "# MEE Core Internal Review Action Function Low Signal Remaining Drain Plan V1",
    "",
...
const planMd = `# MEE Core Internal Review Action Function Low Signal Remaining Drain Plan V1

Status: plan only

## Objective

Prepare one controlled invocation package for every currently eligible \`low_signal_monitor\` disposition.

## Selected Action

\`confirm_monitor_only\`, still fully internal-only.
`;
...
writeFileSync(path.join(PLAN_DIR, "MEE_CORE_INTERNAL_REVIEW_ACTION_FUNCTION_LOW_SIGNAL_REMAINING_DRAIN_PLAN_V1.md"), planMd);
writeFileSync(path.join(SQL_DIR, "mee_core_internal_review_action_function_low_signal_remaining_drain_v1_apply_candidate.sql"), applySql);
writeFileSync(path.join(SQL_DIR, "mee_core_internal_review_action_function_low_signal_remaining_drain_v1_rollback_candidate.sql"), rollbackSql);
writeFileSync(path.join(SQL_DIR, "mee_core_internal_review_action_function_low_signal_remaining_drain_v1_readback.sql"), readbackSql);
writeFileSync(path.join(SQL_DIR, "mee_core_internal_review_action_function_low_signal_remaining_drain_v1_preflight.sql"), preflightSql);

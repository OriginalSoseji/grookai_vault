...
const STAGING_ASSET_DIR = path.join('tmp', 'nonproduction_image_staging', 'image_truth_v1', 'img01b');
const DRY_RUN_JSON = path.join(OUTPUT_DIR, 'image_truth_img01b_missing_display_dry_run_v1.json');
const DRY_RUN_MD = path.join(OUTPUT_DIR, 'image_truth_img01b_missing_display_dry_run_v1.md');

const PACKAGE_ID = 'IMG-01B-MISSING-DISPLAY-EXACT-CHILD-IMAGE-DRY-RUN';
...
async function main() {
  ...
  const sourceRows = (manifest.rows ?? [])
    ...
    .filter((row) => row.asset_status === 'source_image_url_preserved')
  ...
  await fs.writeFile(DRY_RUN_MD, `# Image Truth IMG-01B Missing-Display Dry Run V1

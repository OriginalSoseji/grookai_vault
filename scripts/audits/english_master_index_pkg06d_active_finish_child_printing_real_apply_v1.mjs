...
const GATE_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06d_active_finish_child_printing_real_apply_gate_v1.json');
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06d_active_finish_child_printing_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06d_active_finish_child_printing_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg06d_active_finish_child_printing_real_apply_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260609_pkg06d_active_finish_child_printing_real_apply_checkpoint_v1.md');
...
async function captureTargetSnapshot(client, rows) {
  ...
  return {
    ...
    finish_counts: countBy(snapshotRows.filter((row) => row.row_type === 'existing_target_child'), (row) => row.finish_key),
    set_counts: countBy(snapshotRows.filter((row) => row.row_type === 'existing_target_child'), (row) => row.set_code),
  };
}
...
function validatePrerequisites({ gate, dryRun, rows }) {
  ...
  if (rows.length !== EXPECTED_CHILD_ROWS) findings.push('target_row_count_not_319');
  if (new Set(rows.map((row) => row.card_print_id)).size !== EXPECTED_PARENT_ROWS) findings.push('target_parent_count_not_318');
...
function validateBeforeSnapshot({ beforeTargetSnapshot, beforeParentSnapshot, dryRun }) {
  ...
  if (beforeTargetSnapshot.counts.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('before_parent_count_not_318');
  ...
  if (beforeParentSnapshot.count !== EXPECTED_PARENT_ROWS) findings.push('before_parent_snapshot_count_not_318');
  return findings;
}

function validateAfterSnapshot({ afterTargetSnapshot, beforeParentSnapshot, afterParentSnapshot, insertedRows }) {
  const findings = [];
  if (afterTargetSnapshot.counts.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('after_parent_count_not_318');
  if (afterTargetSnapshot.counts.existing_target_child_rows !== EXPECTED_CHILD_ROWS) findings.push('after_existing_target_child_count_not_319');
  if (afterTargetSnapshot.counts.planned_id_collision_rows !== EXPECTED_CHILD_ROWS) findings.push('after_planned_id_collision_count_not_319');
  if (beforeParentSnapshot.hash_sha256 !== afterParentSnapshot.hash_sha256) findings.push('parent_snapshot_changed_unexpectedly');
  if (insertedRows.counts.inserted_rows_found !== EXPECTED_CHILD_ROWS) findings.push('inserted_rows_found_not_319');
...
async function applyPackage({ dryRun, rows }) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg06d_real_apply_targets (
    ...
    await client.query(
      `insert into pkg06d_real_apply_targets (
         card_printing_id,
         card_print_id,
         set_key,
         card_number,
    ...
    const shape = await client.query(
      `select
         ...
         count(*) filter (where set_key = 'ex11')::int as ex11_rows,
         count(*) filter (where set_key = 'ex16')::int as ex16_rows,
         count(*) filter (where set_key = 'ex6')::int as ex6_rows,
         ...
         count(*) filter (where finish_key not in ('reverse', 'holo', 'cosmos', 'normal'))::int as unsupported_rows
       from pkg06d_real_apply_targets`,
    );
    ...
    if (
      ...
      shapeRow.set_rows !== 3 ||
      shapeRow.ex11_rows !== EXPECTED_SET_COUNTS.ex11 ||
      shapeRow.ex16_rows !== EXPECTED_SET_COUNTS.ex16 ||
      shapeRow.ex6_rows !== EXPECTED_SET_COUNTS.ex6 ||
      shapeRow.reverse_rows !== EXPECTED_FINISH_COUNTS.reverse ||
      shapeRow.holo_rows !== EXPECTED_FINISH_COUNTS.holo ||
      shapeRow.cosmos_rows !== EXPECTED_FINISH_COUNTS.cosmos ||
      shapeRow.normal_rows !== EXPECTED_FINISH_COUNTS.normal ||
    ...
    const lockParents = await client.query(
      `select cp.id
       from public.card_prints cp
       join (select distinct card_print_id from pkg06d_real_apply_targets) target on target.card_print_id = cp.id
       for update of cp`,
    );
    ...
    const finishGuard = await client.query(
      `select count(*)::int as unsupported_finish_count
       from pkg06d_real_apply_targets target
    ...
    const finalGuard = await client.query(
      `select
       ...
       from public.card_printings cpr
       join pkg06d_real_apply_targets target on target.card_printing_id = cpr.id`,
    );
    const final = finalGuard.rows[0];
    if (final.inserted_rows !== EXPECTED_CHILD_ROWS || final.provisional_rows !== 0) {
    ...
    return {
      connected: true,
      apply_status: 'pkg06d_active_finish_child_printing_real_apply_committed',
      error_message: null,
      before_target_snapshot: beforeTargetSnapshot,
    ...
    return {
      connected: true,
      apply_status: 'pkg06d_active_finish_child_printing_real_apply_failed_rolled_back',
...
function renderMarkdown(report) {
  const lines = [];
  lines.push('# PKG-06D Active Finish Child Printing Real Apply V1');
  lines.push('');
  lines.push('This report records the approved real apply for the PKG-06D active-finish child-only insert subset.');
  lines.push('');
  lines.push('## Status');
  lines.push('');
  ...
  lines.push('');
  for (const [key, value] of Object.entries(report.verification_summary)) lines.push(`- ${key}: ${mdEscape(JSON.stringify(value))}`);
  lines.push('');
  ...
  lines.push('```');
  lines.push('');
  lines.push('The JSON report contains all inserted row IDs for exact rollback targeting.');
  lines.push('');
...
function renderCheckpoint(report) {
  return `# PKG-06D Active Finish Child Printing Real Apply Checkpoint V1
...
function updateCheckpointIndex() {
  ...
  const line = '| 2026-06-09 | [PKG-06D Active Finish Child Printing Real Apply Checkpoint V1](20260609_pkg06d_active_finish_child_printing_real_apply_checkpoint_v1.md) | Records approved real apply for 319 active-finish child-only card_printing inserts for ex11, ex16, and ex6; parent rows unchanged, no migrations. |';
  const current = fs.existsSync(indexPath) ? fs.readFileSync(indexPath, 'utf8') : '# Master Index Checkpoints\n';
  if (current.includes('20260609_pkg06d_active_finish_child_printing_real_apply_checkpoint_v1.md')) {
    fs.writeFileSync(indexPath, current.split('\n').map((existingLine) => (
      existingLine.includes('20260609_pkg06d_active_finish_child_printing_real_apply_checkpoint_v1.md') ? line : existingLine
    )).join('\n'));
  } else {
...
const report = {
  ...
  version: 'english_master_index_pkg06d_active_finish_child_printing_real_apply_v1',
  audit_only: false,
  apply_only: true,
  approval_scope: {
  ...
  apply_status: pass
    ? 'pkg06d_active_finish_child_printing_real_apply_committed_and_verified'
    : 'pkg06d_active_finish_child_printing_real_apply_failed_or_blocked',
  ...
  verification_summary: {
    ...
    master_index_comparison_status: pass ? 'pkg06d_active_finish_child_printing_verified_after_apply' : 'not_verified',
  },

...
const READINESS_JSON = path.join(OUTPUT_DIR, 'tcgmap08a_tcgcsv_snapshot_readiness_v1.json');
const DRY_RUN_JSON = path.join(OUTPUT_DIR, 'tcgmap08b_fresh_tcgcsv_mapping_insert_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'tcgmap08b_fresh_tcgcsv_mapping_insert_real_apply_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'tcgmap08b_fresh_tcgcsv_mapping_insert_real_apply_v1.md');

const PACKAGE_ID = 'TCGMAP-08B-FRESH-TCGCSV-TCGPLAYER-MAPPING-INSERTS';
const EXPECTED_READINESS_FINGERPRINT = '36880bc60c2a7c07dc6928ca6275554c4c08c1b4b073c84b66def28a67d737bb';
const EXPECTED_TARGET_FINGERPRINT = '7faae7a3102682c3ca22ac7bcda5a8297eace7dce2198d92080c3edbf88de8c0';
const EXPECTED_DRY_RUN_PROOF = '4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945';
const EXPECTED_TARGET_ROWS = 71;
const APPROVAL_TEXT = 'Approve real TCGMAP-08B-FRESH-TCGCSV-TCGPLAYER-MAPPING-INSERTS apply only.';
...
function buildTargets(readiness) {
  return (readiness.ready_rows ?? []).map((row) => ({
    ...
    meta: {
      ...
      promoted_by: 'tcg_mapping_tcgmap08b_fresh_tcgcsv_mapping_insert_guarded_dry_run_v1',
    },
  }));
}
...
async function applyPackage(client, targets) {
  ...
  try {
    ...
    await client.query(
      `create temporary table tcgmap08b_targets (
    ...
    const insertResult = await client.query(
      `insert into public.external_mappings (
       ...
       select
         ...
         now(),
         meta
       from tcgmap08b_targets`,
    );

    const verification = await client.query(
      `with target as (
         select * from tcgmap08b_targets
       )
       select
         (select count(*)::int
          from public.external_mappings em
...
async function main() {
  ...
  const client = new Client({
    ...
    application_name: 'tcgmap08b_real_apply',
  });
...
main().catch((error) => {
  console.error('[tcgmap08b-real-apply] failed:', error);
  process.exit(1);
});

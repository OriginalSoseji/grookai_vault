...
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg16f_same_finish_split_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg16f_same_finish_split_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg16f_same_finish_split_real_apply_v1.md');

const PACKAGE_ID = 'PKG-16F-SAME-FINISH-SPLIT-PARENT-INSERTS';
const CREATED_BY = 'pkg16f_same_finish_split_real_apply_v1';
const EXPECTED_PACKAGE_FINGERPRINT = 'ae2c5c6f588eec7b48f7af97a23cc4cc24fddb167dc688b14368dc80c86bdecf';
const EXPECTED_DRY_RUN_PROOF = '48d2200c54e62cdb29a1c68570cc796291605b4738286ceeff3eb9d3cb2ab7e4';
...
function validateDryRun(dryRun) {
  ...
  if (dryRun.execution?.dry_run_status !== 'pkg16f_completed_rolled_back_no_durable_change') findings.push('dry_run_status_not_passed');
...
async function applyPackage(client, targets, fingerprint) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg16f_targets (
    ...
    const guard = await client.query(
      `with projection as (
         select
           ...
           from pkg16f_targets target
         join public.card_prints base on base.id = target.base_parent_id
         left join public.sets s on s.id = base.set_id
       )
       select
         (select count(*)::int from pkg16f_targets) as target_count,
         (select count(distinct target_parent_id)::int from pkg16f_targets) as target_parent_count,
         (select count(distinct target_child_id)::int from pkg16f_targets) as target_child_count,
         (select count(*)::int from pkg16f_targets target left join public.card_prints base on base.id = target.base_parent_id where base.id is null) as missing_base_count,
         (select count(*)::int from pkg16f_targets target left join public.finish_keys fk on fk.key = target.target_finish_key and fk.is_active = true where fk.key is null) as inactive_finish_count,
         (select count(*)::int
          from pkg16f_targets target
          join public.card_prints base on base.id = target.base_parent_id
         ...
         (select count(*)::int from pkg16f_targets target join public.card_printings cpr on cpr.id = target.target_child_id) as child_collision_count,
         (select count(*)::int from pkg16f_targets target join public.card_print_identity cpi on cpi.card_print_id = target.target_parent_id and cpi.is_active = true) as identity_target_collision_count,
    ...
    const proof = await client.query(
      `select
         ...
         (select count(*)::int from pkg16f_targets) as target_rows,
         (select count(*)::int from public.card_prints cp join pkg16f_targets target on target.target_parent_id = cp.id) as inserted_parent_rows,
         (select count(*)::int from public.card_print_identity cpi join pkg16f_targets target on target.target_parent_id = cpi.card_print_id and cpi.is_active = true) as inserted_identity_rows,
         (select count(*)::int from public.card_printings cpr join pkg16f_targets target on target.target_child_id = cpr.id) as inserted_child_rows`,
...
function renderMarkdown(report) {
  ...
  return `# PKG-16F Same-Finish Split Real Apply V1
...
async function main() {
  ...
  const report = {
    ...
    version: 'english_master_index_pkg16f_same_finish_split_real_apply_v1',
    package_id: PACKAGE_ID,
    ...
    db_writes_performed: execution.apply_status === 'pkg16f_real_apply_committed',
    durable_db_writes_performed: execution.apply_status === 'pkg16f_real_apply_committed',

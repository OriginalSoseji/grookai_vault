...
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg18m_prize_pack_stamped_parent_insert_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg18m_prize_pack_stamped_parent_insert_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg18m_prize_pack_stamped_parent_insert_real_apply_v1.md');

const PACKAGE_ID = 'PKG-18M-PRIZE-PACK-STAMPED-PARENT-INSERTS';
const CREATED_BY = 'pkg18m_prize_pack_stamped_parent_insert_real_apply_v1';
const EXPECTED_PACKAGE_FINGERPRINT = '2eccac5fc5f0f2a9db366b48aa135370d06a4f08f247f722bfea8dbd14bc917d';
const EXPECTED_DRY_RUN_PROOF = '56fc20603626c53ccaab3aa57addf28c6abe29e9636d88316c9bde2f262cb068';
const EXPECTED_PRE_APPLY_HASH = '7e5152fe7a90bfaa2e472a9e6fbeacef4361612e561787ff70e9ce50c337a28c';
const EXPECTED_TARGET_COUNT = 12;
const APPROVAL_TEXT = `Approve real PKG-18M-PRIZE-PACK-STAMPED-PARENT-INSERTS apply only. Fingerprint: 2eccac5fc5f0f2a9db366b48aa135370d06a4f08f247f722bfea8dbd14bc917d. Scope: 12 stamped parent inserts, 12 identity inserts, 12 child printing inserts; finishes normal=12; stamp labels Prize Pack Stamp=12; sets swsh5=3, swsh7=3, swsh9=3, swsh1=2, swsh6=1. Dry-run proof: 7e5152fe7a90bfaa2e472a9e6fbeacef4361612e561787ff70e9ce50c337a28c == 7e5152fe7a90bfaa2e472a9e6fbeacef4361612e561787ff70e9ce50c337a28c. No global apply. No migrations. No deletes. No merges. No unsupported cleanup.`;
...
function validateDryRun(dryRun) {
  ...
  if (dryRun.execution?.dry_run_status !== 'pkg18m_completed_rolled_back_no_durable_change') findings.push('dry_run_status_not_passed');
...
function buildTargets(readiness) {
  return (readiness.rows ?? [])
    ...
    .map((row) => ({
      ...
      evidence_label: 'PriceCharting exact Prize Pack product title corroborates active finish',
      evidence: {
...
async function applyPackage(client, targets, fingerprint) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg18m_targets (
    ...
    const guard = await client.query(
      `with projection as (
         ...
         from pkg18m_targets target
         join public.card_prints base on base.id = target.base_parent_id
         left join public.sets s on s.id = base.set_id
       )
       select
         (select count(*)::int from pkg18m_targets) as target_count,
         (select count(distinct target_parent_id)::int from pkg18m_targets) as target_parent_count,
         (select count(distinct target_child_id)::int from pkg18m_targets) as target_child_count,
         (select count(*)::int from pkg18m_targets target left join public.card_prints base on base.id = target.base_parent_id where base.id is null) as missing_base_count,
         (select count(*)::int from pkg18m_targets target left join public.finish_keys fk on fk.key = target.target_finish_key and fk.is_active = true where fk.key is null) as inactive_finish_count,
         (select count(*)::int
          from pkg18m_targets target
          join public.card_prints base on base.id = target.base_parent_id
         ...
         (select count(*)::int from pkg18m_targets target join public.card_printings cpr on cpr.id = target.target_child_id) as child_collision_count,
         (select count(*)::int from pkg18m_targets target join public.card_print_identity cpi on cpi.card_print_id = target.target_parent_id and cpi.is_active = true) as identity_target_collision_count,
    ...
    const childInsert = await client.query(
      `insert into public.card_printings (
       ...
       select
         ...
         concat('Stamped identity child finish routed from PriceCharting Prize Pack corroboration: ', target.target_finish_key)
       from pkg18m_targets target`,
      [CREATED_BY],
    );
    ...
    const proof = await client.query(
      `select
         ...
         (select count(*)::int from pkg18m_targets) as target_rows,
         (select count(*)::int from public.card_prints cp join pkg18m_targets target on target.target_parent_id = cp.id) as inserted_parent_rows,
         (select count(*)::int from public.card_prints cp join pkg18m_targets target on target.target_parent_id = cp.id and cp.printed_identity_modifier = target.target_printed_identity_modifier) as inserted_parent_modifier_rows,
         (select count(*)::int from public.card_print_identity cpi join pkg18m_targets target on target.target_parent_id = cpi.card_print_id and cpi.is_active = true) as inserted_identity_rows,
         (select count(*)::int from public.card_printings cpr join pkg18m_targets target on target.target_child_id = cpr.id) as inserted_child_rows,
         (select count(*)::int from public.card_printings cpr join pkg18m_targets target on target.target_child_id = cpr.id and cpr.finish_key = 'stamped') as forbidden_stamped_child_rows`,
      [PACKAGE_ID, fingerprint],
    );
    await client.query('commit');
    const afterSnapshot = await captureSnapshot(client, targets);
    return {
      apply_status: 'pkg18m_real_apply_committed',
      guard: guardRow,
      proof: proof.rows[0],
      write_counts: {
...
function renderMarkdown(report) {
  ...
  return `# PKG-18M Prize Pack Stamped Parent Insert Real Apply V1

Approved real apply for Prize Pack stamped parent identity inserts.
...
async function main() {
  ...
  const report = {
    ...
    version: 'english_master_index_pkg18m_prize_pack_stamped_parent_insert_real_apply_v1',
    package_id: PACKAGE_ID,

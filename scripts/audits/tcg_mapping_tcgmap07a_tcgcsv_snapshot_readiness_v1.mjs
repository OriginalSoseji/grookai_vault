...
const CACHE_DIR = path.join(OUTPUT_DIR, 'tcgmap07a_tcgcsv_snapshot_cache_v1');
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'tcgmap07a_tcgcsv_snapshot_readiness_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'tcgmap07a_tcgcsv_snapshot_readiness_v1.md');
const CATEGORY_ID = 3;
...
function buildMarkdown(report) {
  const lines = [];
  lines.push('# TCGMAP-07A TCGCSV Snapshot Readiness V1');
  lines.push('');
  ...
  if (report.summary.ready_rows > 0) {
    lines.push('Recommended next package: `TCGMAP-07B-FRESH-TCGCSV-TCGPLAYER-MAPPING-INSERTS` guarded rollback-only dry-run.');
    lines.push('');
...
async function main() {
  ...
  const client = new Client({
    ...
    application_name: 'tcgmap07a_tcgcsv_snapshot_readiness_v1',
  });
  ...
  try {
    ...
    const reportCore = {
      contract: 'TCGMAP07A_TCGCSV_SNAPSHOT_READINESS_V1',
      ...
      recommended_next_package: readyRows.length > 0
        ? {
            package_id: 'TCGMAP-07B-FRESH-TCGCSV-TCGPLAYER-MAPPING-INSERTS',

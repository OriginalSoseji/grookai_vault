...
const READINESS_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg15_stamped_explicit_finish_readiness_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg15a_stamped_explicit_finish_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg15a_stamped_explicit_finish_guarded_dry_run_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260611_pkg15a_stamped_explicit_finish_guarded_dry_run_checkpoint_v1.md');
...
const CREATED_BY = 'pkg15a_stamped_explicit_finish_guarded_dry_run_v1';
...
async function runDryRun(client, targets, fingerprint) {
  ...
  try {
    ...
    return {
      dry_run_status: 'pkg15a_stamped_explicit_finish_completed_rolled_back_no_durable_change',
      guard: guardRow,
      proof: proof.rows[0],
    ...
    return {
      dry_run_status: 'pkg15a_stamped_explicit_finish_failed_rolled_back',
...
function renderMarkdown(report) {
  ...
  return `# PKG-15A Stamped Explicit Finish Guarded Dry Run V1

Rollback-only dry-run for deterministic stamped canonical parent inserts with explicit child-finish evidence.
...
function checkpointMarkdown(report) {
  return `# PKG-15A Stamped Explicit Finish Guarded Dry Run Checkpoint V1
...
function updateCheckpointIndex() {
  const line = '| 2026-06-11 | [PKG-15A Stamped Explicit Finish Guarded Dry Run Checkpoint V1](20260611_pkg15a_stamped_explicit_finish_guarded_dry_run_checkpoint_v1.md) | Rollback-only dry-run for 5 deterministic stamped identity parent inserts with active identity rows and explicit holo child printings. No durable writes or migrations. |';
  const current = fsSync.existsSync(CHECKPOINT_INDEX) ? fsSync.readFileSync(CHECKPOINT_INDEX, 'utf8') : '# Master Index Checkpoints\n';
  if (current.includes('20260611_pkg15a_stamped_explicit_finish_guarded_dry_run_checkpoint_v1.md')) {
    fsSync.writeFileSync(CHECKPOINT_INDEX, current.split('\n').map((existingLine) => (
      existingLine.includes('20260611_pkg15a_stamped_explicit_finish_guarded_dry_run_checkpoint_v1.md') ? line : existingLine
    )).join('\n'));
  } else {
...
const report = {
  ...
  version: 'english_master_index_pkg15a_stamped_explicit_finish_guarded_dry_run_v1',
  package_id: PACKAGE_ID,
  package_fingerprint_sha256: fingerprint,
  source_artifact: path.relative(ROOT, READINESS_JSON),

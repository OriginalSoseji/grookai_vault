...
const PLAN_JSON = path.join(OUTPUT_DIR, 'image_truth_img11a_mep_official_final_display_upload_apply_plan_v1.json');
const PLAN_MD = path.join(OUTPUT_DIR, 'image_truth_img11a_mep_official_final_display_upload_apply_plan_v1.md');
const RESULT_JSON = path.join(OUTPUT_DIR, 'image_truth_img11a_mep_official_final_display_upload_apply_result_v1.json');
const RESULT_MD = path.join(OUTPUT_DIR, 'image_truth_img11a_mep_official_final_display_upload_apply_result_v1.md');
const STAGING_ASSET_DIR = path.join('tmp', 'nonproduction_image_staging', 'image_truth_v1', 'img11a');
const STORAGE_BUCKET = 'user-card-images';
const PACKAGE_ID = 'IMG-11A-MEP-OFFICIAL-FINAL-DISPLAY-REPRESENTATIVE-CHILD-IMAGE-UPLOAD-APPLY';

const TARGETS = [
  { number: '031', card_name: "N's Zekrom", finish_key: 'holo', card_printing_id: '14436fbf-b2cf-4057-a3ec-e2ab8f0caa5f' },
  { number: '064', card_name: 'Serperior', finish_key: 'holo', card_printing_id: 'd771059f-c418-4ded-86a3-8316c7ed3670' },
  { number: '067', card_name: 'Doublade', finish_key: 'holo', card_printing_id: '249f318b-94f9-478c-bc9d-caee9ab3851f' },
  { number: '069', card_name: 'Chikorita', finish_key: 'holo', card_printing_id: 'e0521254-0ac6-4731-b6ef-2ae7cfdec997' },
];
...
function officialSourceUrl(target) {
  return `https://www.pokemon.com/us/pokemon-tcg/pokemon-cards/series/mep/${normalizeNumber(target.number)}/`;
}

function officialAssetUrl(target) {
  return `https://assets.pokemon.com/static-assets/content-assets/cms2/img/cards/web/MEP/MEP_EN_${normalizeNumber(target.number)}.png`;
}
...
async function normalizeAsset(row) {
  ...
  const fileName = `mep_${normalizeNumber(row.number)}_${row.finish_key}_${row.card_printing_id.slice(0, 8)}_${normalizedSha256.slice(0, 16)}.png`;
  ...
  return {
    ...
    planned_normalized_front_storage_path: [
      'warehouse-derived',
      'image-truth-v1',
      'img11a-mep-official-final-display-representative',
      'mep',
      row.card_printing_id,
      `${normalizedSha256.slice(0, 24)}.png`,
    ].join('/'),
  };
}
  ...
  try {
    ...
    for (const target of TARGETS) {
      const sourceRow = {
        ...
        source_key: 'official_pokemon_card_page',
        source_kind: 'official_gallery',
        source_url: officialSourceUrl(target),
        asset_url: officialAssetUrl(target),
        evidence_label: `Official Pokemon card page and web image for MEP ${target.number} ${target.card_name}`,
      };
...
function buildMarkdown(report) {
  return `# Image Truth IMG-11A MEP Official Final Display Representative Upload Apply ${report.mode === 'real_apply' ? 'Result' : 'Plan'}

...
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'enrich08a_species_name_rule_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'enrich08a_species_name_rule_guarded_dry_run_v1.md');
...
function buildTargets(cards, traitByCardPrintId, speciesSeed) {
  ...
  const seen = new Set();
  const dedupedTargets = targets.filter((row) => {
  ...
  return { classified, targets: dedupedTargets };
}
...
async function runRollbackDryRun(client, targets, speciesIdBySlug) {
  ...
  try {
    ...
    guard = await validateScope(client, targets, speciesIdBySlug);
    if (guardPassed(guard, targets.length)) {
      ...
      insideProof = { inserted_species_rows: 0, guard_blocked: true, guard };
    }
  } catch (error) {
    caughtError = {
...
async function main() {
  ...
  try {
    ...
    const { classified, targets } = buildTargets(cards, traits, speciesSeed);
    const packageFingerprint = sha256(stableJson({
      package_id: PACKAGE_ID,
      source: SOURCE,
      targets,
    }));
    const execution = await runRollbackDryRun(client, targets, speciesIdBySlug);
    const stopFindings = [];
    ...
    if (!execution.caught_error && !execution.inside_transaction_proof?.guard_blocked && execution.inside_transaction_proof?.inserted_species_rows !== targets.length) {
      stopFindings.push('inserted_species_row_count_mismatch');
    }
    if (!execution.caught_error && !execution.inside_transaction_proof?.guard_blocked && execution.inside_transaction_proof?.proof?.matching_active_species_count !== targets.length) {
      stopFindings.push('matching_active_species_count_mismatch');
    }

    const report = {
      version: 'ENRICH08A_SPECIES_NAME_RULE_GUARDED_DRY_RUN_V1',
      package_id: PACKAGE_ID,
      generated_at: new Date().toISOString(),
      package_fingerprint_sha256: packageFingerprint,
      scope: {
        missing_species_parent_rows_scanned: cards.length,
        target_species_mapping_inserts: targets.length,
        target_parent_rows: new Set(targets.map((row) => row.card_print_id)).size,
        durable_db_writes_performed: false,
        migrations_created: false,
        writes_simulated_then_rolled_back: ['card_print_species inserts'],
        forbidden: ['parent writes', 'child writes', 'identity writes', 'mapping writes', 'deletes', 'merges', 'migrations', 'image writes', 'global apply'],
      },
      classification_counts: countBy(classified, (row) => row.status),
      ...
      blocked_samples: classified.filter((row) => row.status !== 'mapped').slice(0, 100),
      stop_findings: stopFindings,
      pass: stopFindings.length === 0,
      recommended_approval_text: stopFindings.length === 0
        ? `Approve real ${PACKAGE_ID} apply only. Fingerprint: ${packageFingerprint}. Scope: ${targets.length} card_print_species inserts across ${new Set(targets.map((row) => row.card_print_id)).size} English physical parents using ${SOURCE}. Dry-run proof: ${execution.before_snapshot.hash_sha256} == ${execution.after_rollback_snapshot.hash_sha256}. No parent writes. No child writes. No identity writes. No mapping writes. No deletes. No merges. No migrations. No image writes. No global apply.`
        : null,
    };
    ...
    const md = [
      ...
      '',
      `- Pass: ${report.pass}`,
      `- Missing-species parents scanned: ${cards.length}`,
      ...
      `- Inserted inside transaction: ${execution.inside_transaction_proof?.inserted_species_rows ?? 0}`,
      ...
      '',
      markdownTable(Object.entries(report.classification_counts).map(([status, rows]) => ({ status, rows })), [

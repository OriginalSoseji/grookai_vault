...
const DRY_RUN_JSON = path.join(OUTPUT_DIR, 'enrich18a_residual_child_printing_gv_id_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'enrich18a_residual_child_printing_gv_id_real_apply_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'enrich18a_residual_child_printing_gv_id_real_apply_v1.md');
const PACKAGE_ID = 'ENRICH-18A-RESIDUAL-READY-CHILD-PRINTING-GV-ID-BACKFILL';
const EXPECTED_FINGERPRINT = '5e8e14b6982c8aa7766285db618ac6c1bcb416e82e51fb602ecc9747f98dfb9c';
const EXPECTED_DRY_RUN_PROOF = 'f1fba00d3ddb96c361719d0f2db4820a35769db58c5120a6f1da338906d42158';
const EXPECTED_TARGET_ROWS = 297;
const APPROVAL_TEXT = 'Approve real ENRICH-18A-RESIDUAL-READY-CHILD-PRINTING-GV-ID-BACKFILL apply only. Fingerprint: 5e8e14b6982c8aa7766285db618ac6c1bcb416e82e51fb602ecc9747f98dfb9c. Scope: 297 residual child card_printing printing_gv_id updates using governed finish suffixes normal=STD, holo=HOLO, reverse=RH. Dry-run proof: f1fba00d3ddb96c361719d0f2db4820a35769db58c5120a6f1da338906d42158 == f1fba00d3ddb96c361719d0f2db4820a35769db58c5120a6f1da338906d42158. No parent writes. No identity writes. No external mapping writes. No species writes. No deletes. No merges. No migrations. No image writes. No global apply.';
  ...
  const validation = await client.query(
    `with target as (
     ...
     select
       ...
       (select count(*)::int from target where suffix is distinct from case finish_key when 'normal' then 'STD' when 'holo' then 'HOLO' when 'reverse' then 'RH' end) as suffix_mismatch_count,
       ...
       (select count(*)::int
        ...
        join public.card_prints cp on cp.id = cpr.card_print_id
        join public.sets s on s.id = cp.set_id
        where s.identity_domain_default not like 'pokemon_eng%'
           or cp.gv_id is null
...
async function applyPackage(client, targets) {
  ...
  try {
    ...
    await client.query(
      `create temporary table enrich18a_targets (
    ...
    const proof = await client.query(
      `select
         (select count(*)::int from enrich18a_targets) as target_count,
         (select count(*)::int
          from enrich18a_targets target
          join public.card_printings cpr on cpr.id = target.card_printing_id
...
async function main() {
  ...
  try {
    ...
    const report = {
      version: 'ENRICH18A_RESIDUAL_READY_CHILD_PRINTING_GV_ID_REAL_APPLY_V1',
      package_id: PACKAGE_ID,
    ...
    await writeText(OUTPUT_MD, [
      '# ENRICH-18A Residual Ready Child Printing GV-ID Real Apply V1',
      '',
      `Package: \`${PACKAGE_ID}\``,
      '',
      '## Result',

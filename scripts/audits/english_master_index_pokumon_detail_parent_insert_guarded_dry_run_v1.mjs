...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pokumon_detail_parent_insert_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pokumon_detail_parent_insert_guarded_dry_run_v1.md');

const PACKAGE_ID = 'POKUMON-DETAIL-PARENT-INSERTS';
const CREATED_BY = 'english_master_index_pokumon_detail_parent_insert_guarded_dry_run_v1';
...
async function runDryRun(client, targets, parents, fingerprint) {
  ...
  try {
    ...
    const inTransactionSnapshot = await captureSnapshot(client, targets, parents);
...
function renderMarkdown(report) {
  return `# Pokumon Detail Parent Insert Guarded Dry Run V1
...
async function main() {
  ...
  const parents = buildParents(targets);
  const fingerprint = packageFingerprint(readiness, targets, parents);

  const client = new Client({ connectionString: conn, ssl: { rejectUnauthorized: false } });
  await client.connect();
  try {
    const dryRun = targets.length ? await runDryRun(client, targets, parents, fingerprint) : null;
    const output = {
      package_id: PACKAGE_ID,

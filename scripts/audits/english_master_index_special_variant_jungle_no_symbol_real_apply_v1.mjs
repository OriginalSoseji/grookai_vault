...
const GATE_JSON = path.join(AUDIT_DIR, 'jungle_no_symbol_real_apply_gate_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'jungle_no_symbol_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'jungle_no_symbol_real_apply_v1.md');

const PACKAGE_ID = 'SPECIAL-VAR-01-JUNGLE-NO-SYMBOL-PARENT-INSERTS';
const CREATED_BY = 'english_master_index_special_variant_jungle_no_symbol_real_apply_v1';
...
const EXPECTED_GATE_FINGERPRINT = 'cc29dc97f3b064c73beb52edb736eb43e69cb2eb89620fec2607aee5f966c489';
...
async function applyPackage(client, targets) {
  ...
  try {
    ...
    return {
      apply_status: 'jungle_no_symbol_parent_insert_applied',
      guard: guardRow,
      proof: proof.rows[0],
      write_counts: {
...
function renderMarkdown(report) {
  return `# Jungle No Symbol Real Apply V1
...
async function main() {
  ...
  const report = {
    ...
    version: 'jungle_no_symbol_real_apply_v1',

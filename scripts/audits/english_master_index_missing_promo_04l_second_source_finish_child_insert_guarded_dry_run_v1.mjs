...
const ACQUISITION_JSON = path.join(AUDIT_DIR, 'second_source_finish_acquisition_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'second_source_finish_child_insert_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'second_source_finish_child_insert_guarded_dry_run_v1.md');

const PACKAGE_ID = 'MISSING-PROMO-04L-SECOND-SOURCE-FINISH-CHILD-INSERTS';
const CREATED_BY = 'english_master_index_missing_promo_04l_second_source_finish_child_insert_guarded_dry_run_v1';
...
async function buildTargets(client, acquisitionReport) {
  const approvedRows = (acquisitionReport.acquired_finish_rows ?? [])
    .filter((row) => row.evidence_status === 'approved_for_guarded_dry_run');
  if (!approvedRows.length) return [];

  const parentResult = await client.query(
    `select cp.id::text, cp.set_code, cp.number, cp.name, cp.variant_key, cp.printed_identity_modifier, cp.gv_id
     from public.card_prints cp
     where cp.id = any($1::uuid[])
     order by cp.set_code, cp.number, cp.name, cp.id`,
    [approvedRows.map((row) => row.parent_id)],
  );
  const parentById = new Map(parentResult.rows.map((row) => [row.id, row]));

  return approvedRows.map((row) => {
    const parent = parentById.get(row.parent_id);
    if (!parent) throw new Error(`Missing parent row for ${row.parent_id}`);
    if (!parent.gv_id) throw new Error(`Missing parent gv_id for ${row.parent_id}`);
    const targetChildId = uuidFromSeed(`${PACKAGE_ID}:child:${row.parent_id}:${row.finish_key}`);
    return {
      parent_id: row.parent_id,
      target_child_id: targetChildId,
      finish_key: row.finish_key,
      printing_gv_id: `${parent.gv_id}-${finishSuffix(row.finish_key)}`,
      ...
      family: row.family,
      evidence_mode: 'second_source_finish_evidence',
      provenance: {
        source: 'verified_master_index_v1',
        package_id: PACKAGE_ID,
        second_source_acquisition_package_id: acquisitionReport.package_id,
        second_source_acquisition_fingerprint_sha256: acquisitionReport.fingerprint_sha256,
        evidence_mode: 'second_source_finish_evidence',
  ...
  }).sort((left, right) => left.set_code.localeCompare(right.set_code) || left.number.localeCompare(right.number, undefined, { numeric: true }) || left.name.localeCompare(right.name));
}

function packageFingerprint(targets, acquisitionReport) {
...
function sqlHash() {
  return sha256(stableJson({
    package_id: PACKAGE_ID,
    writes: ['insert child-only public.card_printings rows for second-source special finish evidence'],
...
async function runDryRun(client, targets) {
  ...
  try {
    ...
    await client.query(
      `create temporary table second_source_finish_child_targets (
    ...
    await client.query(
      `insert into second_source_finish_child_targets
       select *
    ...
    const guard = await client.query(
      `select
         (select count(*)::int from second_source_finish_child_targets) as target_count,
         (select count(distinct parent_id)::int from second_source_finish_child_targets) as parent_count,
         (select count(distinct target_child_id)::int from second_source_finish_child_targets) as child_id_count,
         (select count(distinct printing_gv_id)::int from second_source_finish_child_targets) as printing_gv_id_count,
         (select count(*)::int from second_source_finish_child_targets t left join public.card_prints cp on cp.id = t.parent_id where cp.id is null) as missing_parent_count,
         (select count(*)::int from second_source_finish_child_targets t left join public.finish_keys fk on fk.key = t.finish_key and fk.is_active = true where fk.key is null) as inactive_finish_count,
         (select count(*)::int from second_source_finish_child_targets t join public.card_printings cpr on cpr.id = t.target_child_id) as child_id_collision_count,
         (select count(*)::int from second_source_finish_child_targets t join public.card_printings cpr on cpr.printing_gv_id = t.printing_gv_id) as printing_gv_id_collision_count,
         (select count(*)::int from second_source_finish_child_targets t join public.card_printings cpr on cpr.card_print_id = t.parent_id and cpr.finish_key = t.finish_key) as existing_finish_collision_count,
         (select count(*)::int from second_source_finish_child_targets t join public.card_print_identity cpi on cpi.card_print_id = t.parent_id and cpi.is_active = true) as active_identity_count,
         (select count(*)::int from second_source_finish_child_targets where finish_key = 'stamped') as forbidden_stamped_finish_count`,
    );
    ...
    const insertResult = await client.query(
      `insert into public.card_printings (
       ...
       select
         ...
         null, null, null, null,
         null,
         concat('Child printing completed from second-source special finish evidence: ', evidence_mode)
       from second_source_finish_child_targets
...
async function main() {
  ...
  const acquisitionReport = await readJson(ACQUISITION_JSON);

  const client = new pg.Client({ connectionString: conn });
  await client.connect();
  try {
    const targets = await buildTargets(client, acquisitionReport);
    const packageHash = packageFingerprint(targets, acquisitionReport);
    ...
    const report = {
      ...
      input_artifacts: {
        second_source_finish_acquisition: rel(ACQUISITION_JSON),
      },
      package_fingerprint_sha256: packageHash,
      sql_hash_sha256: sqlHash(),
      dry_run_proof_sha256: dryRun ? sha256(stableJson({
        package_id: PACKAGE_ID,
        package_fingerprint_sha256: packageHash,
      ...
      recommended_real_apply_approval_text: targets.length
        ? `Approve real ${PACKAGE_ID} apply only. Fingerprint: ${packageHash}. SQL hash: ${sqlHash()}. Scope: ${targets.length} child-only card_printing inserts from second-source finish evidence; finishes ${Object.entries(countBy(targets, (target) => target.finish_key)).map(([key, value]) => `${key}=${value}`).join(', ')}. Dry-run proof: ${dryRun ? sha256(stableJson({ package_id: PACKAGE_ID, package_fingerprint_sha256: packageHash, inserted_rows: dryRun.inserted_rows, guard: dryRun.guard })) : 'not_applicable'} == ${dryRun ? sha256(stableJson({ package_id: PACKAGE_ID, package_fingerprint_sha256: packageHash, inserted_rows: dryRun.inserted_rows, guard: dryRun.guard })) : 'not_applicable'}. No parent writes. No identity writes. No external mapping writes. No pricing writes. No image writes. No global apply. No migrations. No deletes. No merges. No unsupported cleanup.`
        : 'No real apply recommended: no ready targets.',
    };

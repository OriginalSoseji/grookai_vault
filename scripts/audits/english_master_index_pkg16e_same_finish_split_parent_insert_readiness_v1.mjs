...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg16e_same_finish_split_parent_insert_readiness_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg16e_same_finish_split_parent_insert_readiness_v1.md');

const PACKAGE_ID = 'PKG-16E-SAME-FINISH-SPLIT-PARENT-INSERT-READINESS';
const FUTURE_PACKAGE_ID = 'PKG-16F-SAME-FINISH-SPLIT-PARENT-INSERTS';
...
function buildTargets(pkg16b) {
  return (pkg16b.rows ?? [])
    .filter((row) => row.status === 'ready_for_future_guarded_split_insert')
    .map((row) => {
      const numberPlain = normalizeNumber(row.card_number);
      const targetParentId = uuidFromSeed(`${FUTURE_PACKAGE_ID}:parent:${row.set_key}:${numberPlain}:${normalizeText(row.card_name)}:${row.variant_key}`);
      const targetChildId = uuidFromSeed(`${FUTURE_PACKAGE_ID}:child:${row.set_key}:${numberPlain}:${normalizeText(row.card_name)}:${row.variant_key}:${row.finish_key}`);
      return {
        ...
        current_master_variant_key: row.variant_key,
        expansion_status: row.status,
        independent_finish_sources: row.source_rows?.map((source) => source.source_key) ?? [],
        independent_identity_sources: row.source_rows?.map((source) => source.source_key) ?? [],
        evidence_context: {
          pkg16b_source_rows: row.source_rows ?? [],
          pkg16b_source_variant_label: row.source_variant_label,
        },
      };
    })
...
function packageFingerprint(pkg16b, targets) {
  return sha256(stableJson({
    package_id: FUTURE_PACKAGE_ID,
    source_pkg16b_fingerprint: pkg16b.fingerprint_sha256,
...
function renderMarkdown(report) {
  ...
  return `# PKG-16E Same-Finish Split Parent Insert Readiness V1
...
async function main() {
  const pkg16b = await readJson(PKG16B_JSON);
  const targets = buildTargets(pkg16b);
  const pkgFingerprint = packageFingerprint(pkg16b, targets);
  ...
  const report = {
    ...
    version: 'english_master_index_pkg16e_same_finish_split_parent_insert_readiness_v1',
    package_id: PACKAGE_ID,
    future_package_id: FUTURE_PACKAGE_ID,
    audit_only: true,
    ...
    write_ready_now: 0,
    source_artifacts: {
      pkg16b_same_finish_stamped_split_readiness: path.relative(ROOT, PKG16B_JSON).replaceAll('\\', '/'),
    },
    source_fingerprints: {
      pkg16b: pkg16b.fingerprint_sha256,
    },
    package_fingerprint_sha256: pkgFingerprint,

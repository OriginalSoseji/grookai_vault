...
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'enrich19a_col1_parent_gv_id_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'enrich19a_col1_parent_gv_id_guarded_dry_run_v1.md');
...
async function runRollbackDryRun(client, targets) {
  ...
  try {
    ...
    if (
      ...
      guard.non_col_namespace_count !== 0 ||
...
async function main() {
  ...
  try {
    ...
    if (preflight.non_col_namespace_count !== 0) stop_findings.push('non_col_namespace');
    ...
    const report = {
      version: 'ENRICH19A_COL1_PARENT_GV_ID_GUARDED_DRY_RUN_V1',
      package_id: PACKAGE_ID,
      ...
      namespace_governance: 'col1_identity_contract_v1 / COL namespace',
      preflight,
      execution,
      ...
      recommended_approval_text: stop_findings.length === 0
        ? `Approve real ${PACKAGE_ID} apply only. Fingerprint: ${packageFingerprint}. Scope: ${targets.length} Call of Legends parent card_print gv_id updates using governed COL namespace. Dry-run proof: ${execution.before_snapshot.hash_sha256} == ${execution.after_rollback_snapshot.hash_sha256}. No child writes. No identity writes. No external mapping writes. No species writes. No deletes. No merges. No migrations. No image writes. No global apply.`
        : null,
    };

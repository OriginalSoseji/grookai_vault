...
const STAGING_ASSET_DIR = path.join('tmp', 'nonproduction_image_staging', 'image_truth_v1', 'img04a');
const DRY_RUN_JSON = path.join(OUTPUT_DIR, 'image_truth_img04a_reverseholo_representative_missing_display_dry_run_v1.json');
const DRY_RUN_MD = path.join(OUTPUT_DIR, 'image_truth_img04a_reverseholo_representative_missing_display_dry_run_v1.md');

const PACKAGE_ID = 'IMG-04A-REVERSEHOLO-REPRESENTATIVE-MISSING-DISPLAY-CHILD-IMAGE-DRY-RUN';
...
async function normalizeAsset(row) {
  ...
  const plannedStoragePath = [
    'warehouse-derived',
    'image-truth-v1',
    'img04a-reverseholo-representative',
    row.set_code,
...
async function main() {
  ...
  const sourceRows = (manifest.rows ?? [])
    ...
    .filter((row) => row.source_key === 'reverseholo_set_checklist')
  ...
  await fs.writeFile(DRY_RUN_MD, `# Image Truth IMG-04A ReverseHolo Representative Missing-Display Dry Run V1

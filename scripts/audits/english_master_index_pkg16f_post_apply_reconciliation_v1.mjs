...
const APPLY_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg16f_same_finish_split_real_apply_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg16f_post_apply_reconciliation_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg16f_post_apply_reconciliation_v1.md');
...
function renderMarkdown(report) {
  ...
  return `# PKG-16F Post-Apply Reconciliation V1
...
async function main() {
  ...
  if (apply.execution?.apply_status !== 'pkg16f_real_apply_committed') stopFindings.push('apply_status_not_committed');
  ...
  const reportBase = {
    ...
    version: 'english_master_index_pkg16f_post_apply_reconciliation_v1',

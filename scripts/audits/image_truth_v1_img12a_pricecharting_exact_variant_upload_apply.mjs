...
const PLAN_JSON = path.join(OUTPUT_DIR, 'image_truth_img12a_pricecharting_exact_variant_upload_apply_plan_v1.json');
const PLAN_MD = path.join(OUTPUT_DIR, 'image_truth_img12a_pricecharting_exact_variant_upload_apply_plan_v1.md');
const RESULT_JSON = path.join(OUTPUT_DIR, 'image_truth_img12a_pricecharting_exact_variant_upload_apply_result_v1.json');
const RESULT_MD = path.join(OUTPUT_DIR, 'image_truth_img12a_pricecharting_exact_variant_upload_apply_result_v1.md');
const STAGING_ASSET_DIR = path.join('tmp', 'nonproduction_image_staging', 'image_truth_v1', 'img12a');
const STORAGE_BUCKET = 'user-card-images';
const PACKAGE_ID = 'IMG-12A-PRICECHARTING-EXACT-VARIANT-CHILD-IMAGE-UPLOAD-APPLY';
...
async function fetchHtmlWithCurl(url) {
  ...
  const result = await execFileAsync('curl.exe', [
    ...
    '35',
    '--user-agent',
    'GrookaiImageTruthAudit/1.0 (+audit-only; pricecharting-exact-variant)',
    url,
  ], {
    maxBuffer: 5 * 1024 * 1024,
    timeout: 45000,
...
async function fetchHtml(url) {
  try {
    const response = await fetch(url, {
      headers: {
        'user-agent': 'GrookaiImageTruthAudit/1.0 (+audit-only; pricecharting-exact-variant)',
        accept: 'text/html',
      },
    });
...
function findExactPriceChartingImage(row, html) {
  ...
  return { status: 'exact_image_found', reason: 'pricecharting_title_and_image_alt_exact', title, asset_url: match.src, image_alt: match.alt };
}
...
async function normalizeAsset(row, assetUrl) {
  ...
  return {
    ...
    planned_normalized_front_storage_path: [
      'warehouse-derived',
      'image-truth-v1',
      'img12a-pricecharting-exact-variant',
      row.set_code,
  ...
  try {
    ...
    for (const candidate of candidates) {
      ...
      if (validationErrors.length === 0) {
        const note = `${PACKAGE_ID}:pricecharting_exact:${candidate.best_source.source_url}`;
    ...
    for (const row of rows) {
      ...
      const update = await client.query(
        `
          ...
          returning id, image_source, image_path, image_status, image_note
        `,
        [row.card_printing_id, row.image_source, row.image_path, row.image_status, `${PACKAGE_ID}:pricecharting_exact:${row.source_url}`],
      );
...
function buildMarkdown(report) {
  return `# Image Truth IMG-12A PriceCharting Exact Variant Upload Apply ${report.mode === 'real_apply' ? 'Result' : 'Plan'}

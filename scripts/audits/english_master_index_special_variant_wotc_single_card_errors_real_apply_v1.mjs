...
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'wotc_single_card_errors_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'wotc_single_card_errors_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'wotc_single_card_errors_real_apply_v1.md');

const PACKAGE_ID = 'SPECIAL-VAR-03-WOTC-SINGLE-CARD-ERROR-PARENT-INSERTS';
const CREATED_BY = 'english_master_index_special_variant_wotc_single_card_errors_real_apply_v1';
const EXPECTED_PACKAGE_FINGERPRINT = 'e86d05bb30c630306e57cc4bdab5ab53f6b101d050c0e395913a2e99798d6c61';
const EXPECTED_SQL_HASH = '5ca77d000f034f205e014d638bed8a2ad163bf7b499d5dd752721f2245873347';
const EXPECTED_DRY_RUN_PROOF = 'd8549f3b59828daf4894ab8dddf5ec7d95af102029506f51e7f0681fee1ce3f2';
const EXPECTED_PRE_APPLY_HASH = '2b49d3651a487f226efde4207a41711e345e66e0960627ed45f7eb6016138abf';
const EXPECTED_TARGET_COUNT = 8;
const EXPECTED_SOURCE_UNIQUE_FINISH_ROWS = 1;
...
function validateDryRun(dryRun) {
  ...
  if (dryRun.execution?.dry_run_status !== 'wotc_single_card_errors_parent_insert_completed_rolled_back_no_durable_change') findings.push('dry_run_status_not_passed');
  ...
  if (dryRun.scope?.by_set?.base1 !== 3 || dryRun.scope?.by_set?.base5 !== 1 || dryRun.scope?.by_set?.basep !== 4) findings.push('set_scope_mismatch');
  if (dryRun.scope?.by_finish?.holo !== 3 || dryRun.scope?.by_finish?.normal !== 5) findings.push('finish_scope_mismatch');
  ...
  for (const row of dryRun.scope?.targets ?? []) {
    if (!['base1', 'base5', 'basep'].includes(row.set_key)) findings.push(`target_set_mismatch:${row.source_candidate_key}`);
    ...
    if (!['holo', 'normal'].includes(row.target_finish_key)) findings.push(`target_finish_mismatch:${row.source_candidate_key}`);
...
function renderMarkdown(report) {
  return `# WOTC Single Card Errors Real Apply V1

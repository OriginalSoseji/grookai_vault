...
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'enrich23a_residual_parent_core_identity_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'enrich23a_residual_parent_core_identity_guarded_dry_run_v1.md');

const PACKAGE_ID = 'ENRICH-23A-RESIDUAL-PARENT-CORE-IDENTITY-BACKFILL';
...
function buildTargets(readiness) {
  return (readiness.rows ?? [])
    ...
    .map((row) => ({
      ...
      set_code: row.parsed_source?.source_set_code ?? row.proposed_set_code,
      number: row.parsed_source?.source_card_number ?? row.proposed_number,
  ...
  const result = await client.query(
    `with target as (
     ...
     select
       ...
       count(em.id) filter (where coalesce(em.active, true) = true)::int as active_mapping_count,
       count(cpt.id)::int as trait_count,
       count(cps.id)::int as species_count
     from target
     ...
     group by
       target.card_print_id, target.set_code, target.number,
       cp.set_code, cp.number, cp.number_plain, cp.name, cp.gv_id, cp.identity_domain, cp.printed_identity_modifier,
...
async function duplicateIdentityGroups(client) {
  const result = await client.query(
...
async function runRollbackDryRun(client, targets) {
  ...
  const beforeDuplicateIdentityGroups = await duplicateIdentityGroups(client);
  ...
  try {
    ...
    if (beforeDuplicateIdentityGroups !== 0) guardFindings.push(`preexisting_active_identity_duplicate_groups:${beforeDuplicateIdentityGroups}`);
    if (beforeCoreIdentityGaps !== EXPECTED_TARGET_ROWS) guardFindings.push(`core_identity_gap_count_mismatch:${beforeCoreIdentityGaps}`);
    if (guardFindings.length) {
      throw new Error(`preflight guard failed: ${guardFindings.join(', ')}`);
    }
    ...
    const afterDuplicateIdentityGroups = await duplicateIdentityGroups(client);
    ...
    return {
      pass: updated.rows.length === EXPECTED_TARGET_ROWS
        && afterCoreIdentityGaps === 0
        && beforeDuplicateIdentityGroups === 0
        && afterDuplicateIdentityGroups === 0,
      ...
      before_active_identity_duplicate_groups: beforeDuplicateIdentityGroups,
      after_active_identity_duplicate_groups: afterDuplicateIdentityGroups,
      ...
      stop_findings: [
        ...
        ...(afterCoreIdentityGaps === 0 ? [] : [`after_core_identity_gaps_remaining:${afterCoreIdentityGaps}`]),
        ...(afterDuplicateIdentityGroups === 0 ? [] : [`after_active_identity_duplicate_groups:${afterDuplicateIdentityGroups}`]),
      ],
    };
  } catch (error) {
...
function markdown(report) {
  ...
  return `# ENRICH-23A Residual Parent Core Identity Backfill Dry Run
...
Backfill only parent mirror fields from existing active identity rows:

- \`card_prints.set_code\`
- \`card_prints.number\`
- generated \`card_prints.number_plain\`

This package does not write GV IDs, child rows, active identities, external mappings, traits, species, images, deletes, merges, migrations, or global apply.
...
async function main() {
  ...
  const client = new Client({ connectionString: conn, application_name: 'card_row_enrichment_enrich23a_residual_parent_core_identity_guarded_dry_run_v1' });
  await client.connect();
  try {
    const dryRun = await runRollbackDryRun(client, targets);
    const report = {
      version: 'ENRICH23A_RESIDUAL_PARENT_CORE_IDENTITY_GUARDED_DRY_RUN_V1',
      package_id: PACKAGE_ID,

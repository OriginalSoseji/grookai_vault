...
const GATE_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06e_active_finish_child_printing_real_apply_gate_v1.json');
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06e_active_finish_child_printing_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06e_active_finish_child_printing_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg06e_active_finish_child_printing_real_apply_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260609_pkg06e_active_finish_child_printing_real_apply_checkpoint_v1.md');
...
function validatePrerequisites({ gate, dryRun, rows }) {
  ...
  if (rows.length !== EXPECTED_CHILD_ROWS) findings.push('target_row_count_not_391');
  if (new Set(rows.map((row) => row.card_print_id)).size !== EXPECTED_PARENT_ROWS) findings.push('target_parent_count_not_387');
...
function validateBeforeSnapshot({ beforeTargetSnapshot, beforeParentSnapshot, dryRun }) {
  ...
  if (beforeTargetSnapshot.counts.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('before_parent_count_not_387');
  ...
  if (beforeParentSnapshot.count !== EXPECTED_PARENT_ROWS) findings.push('before_parent_snapshot_count_not_387');
  return findings;
}

function validateAfterSnapshot({ afterTargetSnapshot, beforeParentSnapshot, afterParentSnapshot, insertedRows }) {
  const findings = [];
  if (afterTargetSnapshot.counts.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('after_parent_count_not_387');
  if (afterTargetSnapshot.counts.existing_target_child_rows !== EXPECTED_CHILD_ROWS) findings.push('after_existing_target_child_count_not_391');
  if (afterTargetSnapshot.counts.planned_id_collision_rows !== EXPECTED_CHILD_ROWS) findings.push('after_planned_id_collision_count_not_391');
  if (beforeParentSnapshot.hash_sha256 !== afterParentSnapshot.hash_sha256) findings.push('parent_snapshot_changed_unexpectedly');
  if (insertedRows.counts.inserted_rows_found !== EXPECTED_CHILD_ROWS) findings.push('inserted_rows_found_not_391');
...
async function applyPackage({ dryRun, rows }) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg06e_real_apply_targets (
    ...
    await client.query(
      `insert into pkg06e_real_apply_targets (
         card_printing_id,
         card_print_id,
         set_key,
         card_number,
    ...
    const shape = await client.query(
      `select
         ...
         count(*) filter (where set_key = 'ex10')::int as ex10_rows,
         count(*) filter (where set_key = 'ex13')::int as ex13_rows,
         count(*) filter (where set_key = 'ex7')::int as ex7_rows,
         count(*) filter (where set_key = 'ex8')::int as ex8_rows,
       ...
       from pkg06e_real_apply_targets`,
    );
    ...
    if (
      ...
      shapeRow.set_rows !== 4 ||
      shapeRow.ex10_rows !== EXPECTED_SET_COUNTS.ex10 ||
      shapeRow.ex13_rows !== EXPECTED_SET_COUNTS.ex13 ||
      shapeRow.ex7_rows !== EXPECTED_SET_COUNTS.ex7 ||
      shapeRow.ex8_rows !== EXPECTED_SET_COUNTS.ex8 ||
    ...
    const lockParents = await client.query(
      `select cp.id
       from public.card_prints cp
       join (select distinct card_print_id from pkg06e_real_apply_targets) target on target.card_print_id = cp.id
       for update of cp`,
    );
    ...
    const finishGuard = await client.query(
      `select count(*)::int as unsupported_finish_count
       from pkg06e_real_apply_targets target
    ...
    return {
      connected: true,
      apply_status: 'pkg06e_active_finish_child_printing_real_apply_committed',
      error_message: null,
      before_target_snapshot: beforeTargetSnapshot,
    ...
    return {
      connected: true,
      apply_status: 'pkg06e_active_finish_child_printing_real_apply_failed_rolled_back',
...
function renderMarkdown(report) {
  return `# PKG-06E Active Finish Child Printing Real Apply V1
...
function renderCheckpoint(report) {
  return `# PKG-06E Active Finish Child Printing Real Apply Checkpoint V1
...
Real apply was scoped to 391 child-only card_printing inserts for ex10, ex13, ex7, and ex8. No parent writes, migrations, deletes, merges, unsupported cleanup, or quarantine were performed.
`;
}

function updateCheckpointIndex() {
  const indexPath = path.join(CHECKPOINT_DIR, 'CHECKPOINT_INDEX.md');
  const line = '| 2026-06-09 | [PKG-06E Active Finish Child Printing Real Apply Checkpoint V1](20260609_pkg06e_active_finish_child_printing_real_apply_checkpoint_v1.md) | Records approved real apply for 391 active-finish child-only card_printing inserts for ex10, ex13, ex7, and ex8; parent rows unchanged, no migrations. |';
  const current = fs.existsSync(indexPath) ? fs.readFileSync(indexPath, 'utf8') : '# Master Index Checkpoints\n';
  if (current.includes('20260609_pkg06e_active_finish_child_printing_real_apply_checkpoint_v1.md')) {
    fs.writeFileSync(indexPath, current.split('\n').map((existingLine) => (
      existingLine.includes('20260609_pkg06e_active_finish_child_printing_real_apply_checkpoint_v1.md') ? line : existingLine
    )).join('\n'));
  } else {
...
const report = {
  ...
  version: 'english_master_index_pkg06e_active_finish_child_printing_real_apply_v1',
  audit_only: false,
  apply_only: true,
  approval_scope: {
  ...
  apply_status: pass
    ? 'pkg06e_active_finish_child_printing_real_apply_committed_and_verified'
    : 'pkg06e_active_finish_child_printing_real_apply_failed_or_blocked',
  ...
  verification_summary: {
    ...
    master_index_comparison_status: pass ? 'pkg06e_active_finish_child_printing_verified_after_apply' : 'not_verified',
  },

...
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'enrich20b_modern_mcd_child_printing_gv_id_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'enrich20b_modern_mcd_child_printing_gv_id_guarded_dry_run_v1.md');
...
async function main() {
  ...
  try {
    ...
    const report = {
      version: 'ENRICH20B_MODERN_MCD_CHILD_PRINTING_GV_ID_GUARDED_DRY_RUN_V1',
      package_id: PACKAGE_ID,
      ...
      target_samples: targets.slice(0, 25).map((row) => ({
        card_printing_id: row.card_printing_id,
        set_code: row.set_code,
        number: row.number,
        card_name: row.card_name,

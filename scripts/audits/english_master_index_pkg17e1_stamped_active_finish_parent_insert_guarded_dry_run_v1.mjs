...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg17e1_stamped_active_finish_parent_insert_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg17e1_stamped_active_finish_parent_insert_guarded_dry_run_v1.md');

const PACKAGE_ID = 'PKG-17E1-STAMPED-ACTIVE-FINISH-PARENT-INSERTS';
const CREATED_BY = 'pkg17e1_stamped_active_finish_parent_insert_guarded_dry_run_v1';
...
async function runDryRun(client, targets, fingerprint) {
  ...
  try {
    ...
    return {
      dry_run_status: 'pkg17e1_failed_rolled_back',
...
function buildMarkdown(report) {
  return `# PKG-17E1 Stamped Active Finish Parent Insert Guarded Dry Run V1
...
async function main() {
  ...
  const recommended = execution.dry_run_status === 'pkg17e1_completed_rolled_back_no_durable_change' && execution.rollback_verified && execution.stop_findings.length === 0
  ...
  const report = {
    ...
    version: 'english_master_index_pkg17e1_stamped_active_finish_parent_insert_guarded_dry_run_v1',
    package_id: PACKAGE_ID,

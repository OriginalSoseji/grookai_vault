...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg11b_stamped_parent_identity_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg11b_stamped_parent_identity_guarded_dry_run_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260610_pkg11b_stamped_parent_identity_guarded_dry_run_checkpoint_v1.md');
  ...
  try {
    ...
    const guard = await client.query(
      `select
         ...
         (select count(distinct target_parent_id)::int from pkg11b_targets) as target_parent_count,
         (select count(distinct target_child_id)::int from pkg11b_targets) as target_child_count,
    ...
    const childInsert = await client.query(
      `insert into public.card_printings (
       ...
       select
         ...
         'english_master_index_pkg11b_stamped_parent_identity_guarded_dry_run_v1',
    ...
    return {
      status: 'pkg11b_stamped_parent_identity_completed_rolled_back_no_durable_change',
      error_message: null,
      ...
      dry_run_proof_hash: afterSnapshot.hash_sha256,
      dry_run_rollback_verified: beforeSnapshot.hash_sha256 === afterSnapshot.hash_sha256,
    };
  } catch (error) {
    await client.query('rollback').catch(() => {});
    const afterSnapshot = await captureSnapshot(client, targets).catch(() => beforeSnapshot);
    return {
      status: 'pkg11b_stamped_parent_identity_failed_rolled_back',
...
function renderMarkdown(report) {
  const targetRows = report.scope.targets.map((target) => [
    target.set_key,
    target.card_number,
    target.card_name,
    target.stamp_label,
  ...
  return `# English Master Index PKG-11B Stamped Parent Identity Guarded Dry Run V1

Rollback-only guarded dry run for exact-finish routed stamped canonical parent inserts.
...
- rollback_verified: ${report.execution.dry_run_rollback_verified}

## Scope

- package_id: \`${report.package_id}\`
- fingerprint_sha256: \`${report.package_fingerprint_sha256}\`
- target_rows: ${report.scope.target_count}
- target_finishes: ${JSON.stringify(report.scope.by_target_finish_key)}
- dry_run_proof: \`${report.execution.before_snapshot.hash_sha256}\` == \`${report.execution.after_snapshot.hash_sha256}\`

${markdownTable(['set', 'number', 'name', 'stamp_label', 'finish', 'variant_key'], targetRows)}
`;
}
...
async function main() {
  ...
  if (targets.length === 0) throw new Error('No PKG-11B routable stamped targets found');
  const packageFingerprint = sha256(stableJson(targets));
  const client = new Client({ connectionString: conn });
  await client.connect();
  try {
    const execution = await runDryRun(client, targets, packageFingerprint);
    const report = {
      generated_at: new Date().toISOString(),
      version: 'english_master_index_pkg11b_stamped_parent_identity_guarded_dry_run_v1',
    ...
    await writeText(CHECKPOINT_MD, `# PKG-11B Stamped Parent Identity Guarded Dry Run Checkpoint V1

- Package: \`${PACKAGE_ID}\`
- Fingerprint: \`${packageFingerprint}\`
- Target rows: ${targets.length}
- Target finishes: ${JSON.stringify(report.scope.by_target_finish_key)}
- Dry-run proof: \`${execution.before_snapshot.hash_sha256}\` == \`${execution.after_snapshot.hash_sha256}\`
- Rollback verified: ${execution.dry_run_rollback_verified}
...
`);
    console.log(JSON.stringify({
      ...
      dry_run_rollback_verified: execution.dry_run_rollback_verified,

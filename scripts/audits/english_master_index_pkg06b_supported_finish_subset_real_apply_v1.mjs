...
const GATE_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06b_supported_finish_subset_real_apply_gate_v1.json');
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06b_supported_finish_subset_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06b_supported_finish_subset_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg06b_supported_finish_subset_real_apply_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260609_pkg06b_supported_finish_subset_real_apply_checkpoint_v1.md');

const PACKAGE_ID = 'PKG-06B-SUPPORTED-FINISH-SUBSET-CHILD-PRINTING-INSERTS';
const PACKAGE_FINGERPRINT = 'caf8126b5941cf9b4c43b9d3027415ae1dfc94dc204e64b2f00fb16ff0089cad';
const SQL_HASH = '6f6d9fc18e8642cd2c6cbd7a29e26c54f400d11054519626bfd2dc78156b93a6';
const SOURCE_READINESS_FINGERPRINT = '723fba910048e21e8f1df079f3269ecd4b81e3a441cdf2657f3f26d88666a9be';
const APPROVAL_TEXT = 'Approve real PKG-06B-SUPPORTED-FINISH-SUBSET-CHILD-PRINTING-INSERTS apply only. Fingerprint: caf8126b5941cf9b4c43b9d3027415ae1dfc94dc204e64b2f00fb16ff0089cad. SQL hash: 6f6d9fc18e8642cd2c6cbd7a29e26c54f400d11054519626bfd2dc78156b93a6. Scope: 120 child-only card_printing inserts for me03/Perfect Order, finishes normal=65 and holo=55. Blocked finish-taxonomy rows remain excluded: 268. Dry-run proof: 98d147969837ba9bb50874233ddd8876df3e8c8459b9b4514a0f7a7bc093a6c7 == 98d147969837ba9bb50874233ddd8876df3e8c8459b9b4514a0f7a7bc093a6c7. No global apply. No migrations. No deletes. No merges. No unsupported cleanup. No parent writes.';

const EXPECTED_SET_KEY = 'me03';
const EXPECTED_SET_NAME = 'Perfect Order';
const EXPECTED_CHILD_ROWS = 120;
const EXPECTED_PARENT_ROWS = 120;
const EXPECTED_NORMAL_ROWS = 65;
const EXPECTED_HOLO_ROWS = 55;
const EXPECTED_BLOCKED_ROWS = 268;
...
async function captureInsertedRows(client, rows) {
  ...
  return {
    ...
    counts: {
      ...
      holo: result.rows.filter((row) => row.finish_key === 'holo').length,
...
function validatePrerequisites({ gate, dryRun, rows }) {
  ...
  if (dryRun.dry_run_execution_status !== 'pkg06b_supported_finish_subset_completed_rolled_back_no_durable_change') {
  ...
  if (dryRun.supported_subset?.child_printing_rows !== EXPECTED_CHILD_ROWS) findings.push('dry_run_child_count_not_120');
  if (dryRun.supported_subset?.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('dry_run_parent_count_not_120');
  if (dryRun.supported_subset?.set_counts?.[EXPECTED_SET_KEY] !== EXPECTED_CHILD_ROWS) findings.push('dry_run_me03_count_not_120');
  if (dryRun.supported_subset?.finish_counts?.normal !== EXPECTED_NORMAL_ROWS) findings.push('dry_run_normal_count_not_65');
  if (dryRun.supported_subset?.finish_counts?.holo !== EXPECTED_HOLO_ROWS) findings.push('dry_run_holo_count_not_55');
  if (dryRun.blocked_subset?.child_printing_rows !== EXPECTED_BLOCKED_ROWS) findings.push('dry_run_blocked_count_not_268');

  if (rows.length !== EXPECTED_CHILD_ROWS) findings.push('target_row_count_not_120');
  if (new Set(rows.map((row) => row.card_print_id)).size !== EXPECTED_PARENT_ROWS) findings.push('target_parent_count_not_120');
  if (rows.some((row) => row.set_key !== EXPECTED_SET_KEY)) findings.push('target_contains_non_me03_row');
  if (rows.filter((row) => row.finish_key === 'normal').length !== EXPECTED_NORMAL_ROWS) findings.push('target_normal_count_not_65');
  if (rows.filter((row) => row.finish_key === 'holo').length !== EXPECTED_HOLO_ROWS) findings.push('target_holo_count_not_55');
  if (rows.some((row) => !['normal', 'holo'].includes(row.finish_key))) {
...
function validateBeforeSnapshot({ beforeTargetSnapshot, beforeParentSnapshot, dryRun }) {
  ...
  if (beforeTargetSnapshot.counts.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('before_parent_count_not_120');
  ...
  if (beforeParentSnapshot.count !== EXPECTED_PARENT_ROWS) findings.push('before_parent_snapshot_count_not_120');
  return findings;
}

function validateAfterSnapshot({ afterTargetSnapshot, beforeParentSnapshot, afterParentSnapshot, insertedRows }) {
  const findings = [];
  if (afterTargetSnapshot.counts.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('after_parent_count_not_120');
  if (afterTargetSnapshot.counts.existing_target_child_rows !== EXPECTED_CHILD_ROWS) {
    findings.push('after_existing_target_child_count_not_120');
  }
  if (afterTargetSnapshot.counts.planned_id_collision_rows !== EXPECTED_CHILD_ROWS) {
    findings.push('after_planned_id_collision_count_not_120');
  }
  if (afterTargetSnapshot.finish_counts.normal !== EXPECTED_NORMAL_ROWS) findings.push('after_normal_count_not_65');
  if (afterTargetSnapshot.finish_counts.holo !== EXPECTED_HOLO_ROWS) findings.push('after_holo_count_not_55');
  if (beforeParentSnapshot.hash_sha256 !== afterParentSnapshot.hash_sha256) {
    findings.push('parent_snapshot_changed_unexpectedly');
  }
  if (insertedRows.counts.inserted_rows_found !== EXPECTED_CHILD_ROWS) findings.push('inserted_rows_found_not_120');
  if (insertedRows.counts.normal !== EXPECTED_NORMAL_ROWS) findings.push('inserted_normal_count_not_65');
  if (insertedRows.counts.holo !== EXPECTED_HOLO_ROWS) findings.push('inserted_holo_count_not_55');
...
async function applyPackage({ dryRun, rows }) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg06b_real_apply_targets (
    ...
    await client.query(
      `insert into pkg06b_real_apply_targets (
         card_printing_id,
         card_print_id,
         set_key,
         card_number,
    ...
    const shape = await client.query(
      `select
         ...
         count(*) filter (where finish_key = 'holo')::int as holo_rows,
         count(*) filter (where finish_key not in ('normal', 'holo'))::int as unsupported_rows
       from pkg06b_real_apply_targets`,
    );
    ...
    if (
      ...
      shapeRow.set_rows !== 1 ||
      shapeRow.normal_rows !== EXPECTED_NORMAL_ROWS ||
      shapeRow.holo_rows !== EXPECTED_HOLO_ROWS ||
    ...
    const lockParents = await client.query(
      `select cp.id
       from public.card_prints cp
       join pkg06b_real_apply_targets target on target.card_print_id = cp.id
       for update of cp`,
    );
    ...
    const finishGuard = await client.query(
      `select count(*)::int as unsupported_finish_count
       from pkg06b_real_apply_targets target
    ...
    const finalGuard = await client.query(
      `select
         ...
         count(*) filter (where cpr.finish_key = 'holo')::int as holo_rows,
         count(*) filter (where cpr.is_provisional = true)::int as provisional_rows
       from public.card_printings cpr
       join pkg06b_real_apply_targets target on target.card_printing_id = cpr.id`,
    );
    const final = finalGuard.rows[0];
    if (
      final.inserted_rows !== EXPECTED_CHILD_ROWS ||
      final.normal_rows !== EXPECTED_NORMAL_ROWS ||
      final.holo_rows !== EXPECTED_HOLO_ROWS ||
    ...
    return {
      connected: true,
      apply_status: 'pkg06b_supported_finish_subset_real_apply_committed',
      error_message: null,
      before_target_snapshot: beforeTargetSnapshot,
    ...
    return {
      connected: true,
      apply_status: 'pkg06b_supported_finish_subset_real_apply_failed_rolled_back',
...
function renderMarkdown(report) {
  const lines = [];
  lines.push('# PKG-06B Supported Finish Subset Real Apply V1');
  lines.push('');
  lines.push('This report records the approved real apply for the PKG-06B supported-finish child-only insert subset.');
  lines.push('');
  lines.push('## Status');
  lines.push('');
  ...
  lines.push('```');
  lines.push('');
  lines.push('The JSON report contains all 120 inserted row IDs for exact rollback targeting.');
  lines.push('');
...
function renderCheckpoint(report) {
  return `# PKG-06B Supported Finish Subset Real Apply Checkpoint V1

Date: 2026-06-09

## Purpose

Record the approved real apply for 120 supported child-only PKG-06B card_printing inserts.
...
| holo_inserted | ${report.verification_summary.holo_inserted} |
...
- Real apply was scoped to 120 child-only card_printing inserts.
...
function updateCheckpointIndex() {
  ...
  const line = '| 2026-06-09 | [PKG-06B Supported Finish Subset Real Apply Checkpoint V1](20260609_pkg06b_supported_finish_subset_real_apply_checkpoint_v1.md) | Records approved real apply for 120 child-only me03 card_printing inserts, normal=65 and holo=55, parent rows unchanged, 268 finish-taxonomy rows excluded, no migrations. |';
  const current = fs.existsSync(indexPath) ? fs.readFileSync(indexPath, 'utf8') : '# Master Index Checkpoints\n';
  if (current.includes('20260609_pkg06b_supported_finish_subset_real_apply_checkpoint_v1.md')) {
    fs.writeFileSync(
      indexPath,
      current
        .split('\n')
        .map((existingLine) =>
          existingLine.includes('20260609_pkg06b_supported_finish_subset_real_apply_checkpoint_v1.md') ? line : existingLine)
        .join('\n'),
    );
  } else {
...
const report = {
  ...
  version: 'english_master_index_pkg06b_supported_finish_subset_real_apply_v1',
  audit_only: false,
  apply_only: true,
  approval_scope: {
    ...
    approved_for_set_key: EXPECTED_SET_KEY,
  ...
  apply_status: pass
    ? 'pkg06b_supported_finish_subset_real_apply_committed_and_verified'
    : 'pkg06b_supported_finish_subset_real_apply_failed_or_blocked',
  ...
  package_scope: {
    ...
    set_key: EXPECTED_SET_KEY,
    set_name: EXPECTED_SET_NAME,
    child_card_printing_inserts: EXPECTED_CHILD_ROWS,
    target_parent_rows: EXPECTED_PARENT_ROWS,
    finish_counts: {
      normal: EXPECTED_NORMAL_ROWS,
      holo: EXPECTED_HOLO_ROWS,
    },
    blocked_finish_taxonomy_rows_excluded: EXPECTED_BLOCKED_ROWS,
  ...
  verification_summary: {
    ...
    holo_inserted: applyResult.inserted_rows?.counts?.holo ?? 0,
    ...
    master_index_comparison_status: pass ? 'pkg06b_supported_finish_subset_verified_after_apply' : 'not_verified',
  },

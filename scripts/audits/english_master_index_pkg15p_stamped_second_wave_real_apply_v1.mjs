...
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg15p_stamped_second_wave_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg15p_stamped_second_wave_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg15p_stamped_second_wave_real_apply_v1.md');

const PACKAGE_ID = 'PKG-15P-STAMPED-SECOND-WAVE-PARENT-INSERTS';
const CREATED_BY = 'pkg15p_stamped_second_wave_real_apply_v1';
const EXPECTED_PACKAGE_FINGERPRINT = 'a7fe0f5d934e825ef73fd662881b4263721ad164a01bc48f0ad6a0eb55d42c44';
const EXPECTED_DRY_RUN_PROOF = '14d6405f2e4294b8f0afe3f03cb0a553c9e36a7466e828159bec2f4cb12e1c83';
...
function validateDryRun(dryRun) {
  ...
  if (dryRun.execution?.dry_run_status !== 'pkg15p_completed_rolled_back_no_durable_change') findings.push('dry_run_status_not_passed');
  ...
  if (counts.parent_inserts !== 6) findings.push('dry_run_parent_insert_scope_mismatch');
  if (counts.identity_inserts !== 6) findings.push('dry_run_identity_insert_scope_mismatch');
  if (counts.child_inserts !== 6) findings.push('dry_run_child_insert_scope_mismatch');
  ...
  if ((dryRun.scope?.targets ?? []).length !== 6) findings.push('dry_run_targets_not_6');
  return findings;
}
...
async function applyPackage(client, targets, fingerprint) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg15p_targets (
    ...
    const guard = await client.query(
      `with projection as (
         select
           ...
           from pkg15p_targets target
         join public.card_prints base on base.id = target.base_parent_id
         left join public.sets s on s.id = base.set_id
       )
       select
         (select count(*)::int from pkg15p_targets) as target_count,
         (select count(distinct target_parent_id)::int from pkg15p_targets) as target_parent_count,
         (select count(distinct target_child_id)::int from pkg15p_targets) as target_child_count,
         (select count(*)::int from pkg15p_targets target left join public.card_prints base on base.id = target.base_parent_id where base.id is null) as missing_base_count,
         (select count(*)::int from pkg15p_targets target left join public.finish_keys fk on fk.key = target.target_finish_key and fk.is_active = true where fk.key is null) as inactive_finish_count,
         (select count(*)::int
          from pkg15p_targets target
          join public.card_prints base on base.id = target.base_parent_id
         ...
         (select count(*)::int from pkg15p_targets target join public.card_printings cpr on cpr.id = target.target_child_id) as child_collision_count,
         (select count(*)::int from pkg15p_targets target join public.card_print_identity cpi on cpi.card_print_id = target.target_parent_id and cpi.is_active = true) as identity_target_collision_count,
    ...
    const proof = await client.query(
      `select
         ...
         (select count(*)::int from pkg15p_targets) as target_rows,
         (select count(*)::int from public.card_prints cp join pkg15p_targets target on target.target_parent_id = cp.id) as inserted_parent_rows,
         (select count(*)::int from public.card_print_identity cpi join pkg15p_targets target on target.target_parent_id = cpi.card_print_id and cpi.is_active = true) as inserted_identity_rows,
         (select count(*)::int from public.card_printings cpr join pkg15p_targets target on target.target_child_id = cpr.id) as inserted_child_rows`,
      [PACKAGE_ID, fingerprint],
    );
    await client.query('commit');
    const afterSnapshot = await captureSnapshot(client, targets);
    return {
      apply_status: 'pkg15p_real_apply_committed',
      guard: guardRow,
      proof: proof.rows[0],
      write_counts: {
...
function renderMarkdown(report) {
  ...
  return `# PKG-15P Stamped Second-Wave Real Apply V1

Guarded real apply artifact for the second wave of stamped parent identities and child printings.
...
async function main() {
  ...
  const report = {
    ...
    version: 'english_master_index_pkg15p_stamped_second_wave_real_apply_v1',
    package_id: PACKAGE_ID,
    ...
    db_writes_performed: execution.apply_status === 'pkg15p_real_apply_committed',
    durable_db_writes_performed: execution.apply_status === 'pkg15p_real_apply_committed',

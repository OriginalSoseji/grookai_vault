...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg20b_residual_overgeneration_child_delete_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg20b_residual_overgeneration_child_delete_guarded_dry_run_v1.md');
...
function buildTargets(source) {
  ...
  for (const row of source.rows ?? []) {
    if (row.cleanup_readiness !== 'dry_run_candidate') continue;
    const finish = String(row.finish_key ?? '');
    const id = row.card_printing_id;
    const dependencyTotal = Number(row.child_dependency_total ?? 0);
    if (!id || seen.has(id) || !ALLOWED_LANES.has(row.lane) || !ALLOWED_FINISHES.has(finish) || dependencyTotal !== 0) {
      blocked.push({ ...row, blocked_reason: 'target_shape_not_allowed_for_pkg20b' });
      continue;
    }
    seen.add(id);
    targets.push({
...
async function runDryRun(client, targets) {
  ...
  try {
    ...
    await client.query(
      `insert into pkg20b_targets
       select row.card_printing_id::uuid, row.card_print_id::uuid, row.finish_key, row.lane
       from jsonb_to_recordset($1::jsonb) as row(card_printing_id text, card_print_id text, finish_key text, lane text)`,
    ...
    const guards = await client.query(
      `select
         ...
         (select count(*)::int from pkg20b_targets where lane not in ('holo_overgeneration_candidate_no_dependencies', 'normal_overgeneration_candidate_no_dependencies')) as disallowed_lane_rows,
    ...
    if ((await existingOptionalTables(client)).has('justtcg_grookai_mappings')) {
      const justtcg = await client.query(
        `select count(*)::int as refs from public.justtcg_grookai_mappings jgm join pkg20b_targets t on t.card_printing_id = jgm.card_printing_id`,
      );
    ...
    if (
      ...
      throw new Error(`PKG-20B guard failed: ${JSON.stringify(guard)}`);
    }
    const deleteResult = await client.query(
      `delete from public.card_printings cpr using pkg20b_targets target where cpr.id = target.card_printing_id`,
    );
    if (deleteResult.rowCount !== targets.length) throw new Error(`PKG-20B delete simulation mismatch: ${deleteResult.rowCount}`);
  ...
  return {
    dry_run_status: 'pkg20b_guarded_dry_run_passed_rolled_back_no_durable_change',
...
function renderMarkdown(report) {
  return `# PKG-20B Residual Overgeneration Child Delete Guarded Dry Run V1
...
if (targets.length !== 100) throw new Error(`PKG-20B expected 100 targets, found ${targets.length}`);
...
const report = {
  ...
  version: 'english_master_index_pkg20b_residual_overgeneration_child_delete_guarded_dry_run_v1',
  package_id: PACKAGE_ID,

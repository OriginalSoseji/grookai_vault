...
const READINESS_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg17i4_pricecharting_stamp_label_readiness_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg17j_pricecharting_stamped_parent_identity_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg17j_pricecharting_stamped_parent_identity_guarded_dry_run_v1.md');

const PACKAGE_ID = 'PKG-17J-PRICECHARTING-STAMPED-PARENT-IDENTITY-CANDIDATES';
const CREATED_BY = 'pkg17j_pricecharting_stamped_parent_identity_guarded_dry_run_v1';
...
async function runDryRun(client, targets, fingerprint) {
  ...
  try {
    ...
    return {
      dry_run_status: 'pkg17j_failed_rolled_back',
...
function renderMarkdown(report) {
  ...
  return `# PKG-17J PriceCharting Stamped Parent Identity Guarded Dry Run V1
...
async function main() {
  ...
  const recommended = execution.dry_run_status === 'pkg17j_completed_rolled_back_no_durable_change' && execution.rollback_verified && execution.stop_findings.length === 0
  ...
  const report = {
    ...
    version: 'english_master_index_pkg17j_pricecharting_stamped_parent_identity_guarded_dry_run_v1',
    package_id: PACKAGE_ID,

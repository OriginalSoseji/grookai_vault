...
const SUMMARY_MD = path.join(
  OUTPUT_DIR,
  'self_hosted_images_wh09h_world_championship_unique_source_number_pointer_dry_run_summary_v1.md',
);
...
const PACKAGE_ID = 'IMG-HOST-WH-09H-WORLD-CHAMPIONSHIP-UNIQUE-SOURCE-NUMBER-POINTER-DRY-RUN';
const FUTURE_APPLY_PACKAGE_ID = 'IMG-HOST-WH-09I-WORLD-CHAMPIONSHIP-UNIQUE-SOURCE-NUMBER-POINTER-APPLY';
const EXPECTED_ALREADY_REPRESENTATIVE_ROWS = 1833;
const EXPECTED_CURRENT_MISSING_ROWS = 111;
...
function normalizeSourceNumber(sourceSetName, sourceCardNumber) {
  ...
  if (normalize(sourceSetName) === 'scarlet' && number.startsWith('& violet ')) {
    number = number.replace(/^& violet\s+/, '');
  }
...
function sourceAliases(sourceSetName, sourceCardNumber) {
  ...
  if (normalize(sourceSetName) === 'scarlet' && String(sourceCardNumber ?? '').trim().toLowerCase().startsWith('& violet ')) {
    aliases.add('scarlet and violet');
  }
  ...
  if (normalize(sourceSetName) === 'ex') {
    ...
    if (match) aliases.add(normalize(match[1]));
  }
...
function sourceMatchKind(sourceSetName, sourceCardNumber) {
  if (normalize(sourceSetName) === 'ex') return 'ex_set_number_unique_source_match';
  if (normalize(sourceSetName) === 'scarlet' && String(sourceCardNumber ?? '').trim().toLowerCase().startsWith('& violet ')) {
    return 'split_scarlet_violet_number_unique_source_match';
  }
  if (String(sourceCardNumber ?? '').includes('|')) return 'pipe_number_unique_source_match';
  return 'source_set_number_unique_source_match';
}
...
async function main() {
  ...
  for (const wcdRow of missingWcd) {
    ...
    planRows.push(buildPlanRow({
      wcdRow,
      manifestRow,
      sourceRow,
      sourceMatchKind: sourceMatchKind(sourceSetName, sourceCardNumber),
    }));
  }

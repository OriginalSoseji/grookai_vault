...
const GATE_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06f_active_finish_child_printing_real_apply_gate_v1.json');
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06f_active_finish_child_printing_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06f_active_finish_child_printing_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg06f_active_finish_child_printing_real_apply_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260609_pkg06f_active_finish_child_printing_real_apply_checkpoint_v1.md');

const PACKAGE_ID = 'PKG-06F-ACTIVE-FINISH-CHILD-PRINTING-INSERTS';
const SOURCE_READINESS_FINGERPRINT = '27e3873d92679d3255f13f65f90e14597511535a3f4567c89099515ea1ada8f2';
const PACKAGE_FINGERPRINT = '795cb07aed903181a3f671c1ad76d55139ca31dcf73c84269a13494d71b25a5f';
const SQL_HASH = 'e76cf579592e7f005518aa24d7356d0ddaa266e811d4c5cd71945306cebefc9b';
const DRY_RUN_PROOF_HASH = '8410c763077cfb51af7243eb025b52f9f0c0b14ead0f60307ea716e77e5a2d24';
const APPROVAL_TEXT = 'Approve real PKG-06F-ACTIVE-FINISH-CHILD-PRINTING-INSERTS apply only. Fingerprint: 795cb07aed903181a3f671c1ad76d55139ca31dcf73c84269a13494d71b25a5f. SQL hash: e76cf579592e7f005518aa24d7356d0ddaa266e811d4c5cd71945306cebefc9b. Scope: 355 child-only card_printing inserts for ex5/Hidden Legends, ex15/Dragon Frontiers, ex14/Crystal Guardians, and ex9/Emerald; finishes reverse=355; target parents=355. Dry-run proof: 8410c763077cfb51af7243eb025b52f9f0c0b14ead0f60307ea716e77e5a2d24 == 8410c763077cfb51af7243eb025b52f9f0c0b14ead0f60307ea716e77e5a2d24. No global apply. No migrations. No deletes. No merges. No unsupported cleanup. No parent writes.';

const EXPECTED_CHILD_ROWS = 355;
const EXPECTED_PARENT_ROWS = 355;
const EXPECTED_SET_COUNTS = { ex5: 90, ex15: 89, ex14: 88, ex9: 88 };
const EXPECTED_FINISH_COUNTS = { reverse: 355 };
...
function validatePrerequisites({ gate, dryRun, rows }) {
  ...
  if (dryRun.dry_run_execution_status !== 'pkg06f_active_finish_child_printing_completed_rolled_back_no_durable_change') findings.push('dry_run_not_passed');
  ...
  if (rows.length !== EXPECTED_CHILD_ROWS) findings.push('target_row_count_not_355');
  if (new Set(rows.map((row) => row.card_print_id)).size !== EXPECTED_PARENT_ROWS) findings.push('target_parent_count_not_355');
  for (const [setKey, count] of Object.entries(EXPECTED_SET_COUNTS)) {
    if (rows.filter((row) => row.set_key === setKey).length !== count) findings.push(`target_${setKey}_count_not_${count}`);
  }
  for (const [finishKey, count] of Object.entries(EXPECTED_FINISH_COUNTS)) {
    if (rows.filter((row) => row.finish_key === finishKey).length !== count) findings.push(`target_${finishKey}_count_not_${count}`);
  }
...
function validateBeforeSnapshot({ beforeTargetSnapshot, beforeParentSnapshot, dryRun }) {
  ...
  if (beforeTargetSnapshot.counts.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('before_parent_count_not_355');
  ...
  if (beforeParentSnapshot.count !== EXPECTED_PARENT_ROWS) findings.push('before_parent_snapshot_count_not_355');
  return findings;
}

function validateAfterSnapshot({ afterTargetSnapshot, beforeParentSnapshot, afterParentSnapshot, insertedRows }) {
  const findings = [];
  if (afterTargetSnapshot.counts.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('after_parent_count_not_355');
  if (afterTargetSnapshot.counts.existing_target_child_rows !== EXPECTED_CHILD_ROWS) findings.push('after_existing_target_child_count_not_355');
  if (afterTargetSnapshot.counts.planned_id_collision_rows !== EXPECTED_CHILD_ROWS) findings.push('after_planned_id_collision_count_not_355');
  if (beforeParentSnapshot.hash_sha256 !== afterParentSnapshot.hash_sha256) findings.push('parent_snapshot_changed_unexpectedly');
  if (insertedRows.counts.inserted_rows_found !== EXPECTED_CHILD_ROWS) findings.push('inserted_rows_found_not_355');
  if (insertedRows.counts.provisional_rows !== 0) findings.push('inserted_provisional_rows_present');
  for (const [setKey, count] of Object.entries(EXPECTED_SET_COUNTS)) {
    if (insertedRows.counts.by_set[setKey] !== count) findings.push(`inserted_${setKey}_count_not_${count}`);
  }
  for (const [finishKey, count] of Object.entries(EXPECTED_FINISH_COUNTS)) {
    if (insertedRows.counts.by_finish[finishKey] !== count) findings.push(`inserted_${finishKey}_count_not_${count}`);
  }
  return findings;
}
...
async function applyPackage({ dryRun, rows }) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg06f_real_apply_targets (
    ...
    await client.query(
      `insert into pkg06f_real_apply_targets (
         card_printing_id,
         card_print_id,
         set_key,
         card_number,
    ...
    const shape = await client.query(
      `select
         ...
         count(*) filter (where set_key = 'ex5')::int as ex5_rows,
         count(*) filter (where set_key = 'ex15')::int as ex15_rows,
         count(*) filter (where set_key = 'ex14')::int as ex14_rows,
         count(*) filter (where set_key = 'ex9')::int as ex9_rows,
         count(*) filter (where finish_key = 'reverse')::int as reverse_rows,
         count(*) filter (where finish_key <> 'reverse')::int as unsupported_rows
       from pkg06f_real_apply_targets`,
    );
    ...
    if (
      ...
      shapeRow.set_rows !== 4 ||
      shapeRow.ex5_rows !== EXPECTED_SET_COUNTS.ex5 ||
      shapeRow.ex15_rows !== EXPECTED_SET_COUNTS.ex15 ||
      shapeRow.ex14_rows !== EXPECTED_SET_COUNTS.ex14 ||
      shapeRow.ex9_rows !== EXPECTED_SET_COUNTS.ex9 ||
      shapeRow.reverse_rows !== EXPECTED_FINISH_COUNTS.reverse ||
    ...
    const lockParents = await client.query(
      `select cp.id
       from public.card_prints cp
       join (select distinct card_print_id from pkg06f_real_apply_targets) target on target.card_print_id = cp.id
       for update of cp`,
    );
    ...
    const finishGuard = await client.query(
      `select count(*)::int as unsupported_finish_count
       from pkg06f_real_apply_targets target
    ...
    return {
      connected: true,
      apply_status: 'pkg06f_active_finish_child_printing_real_apply_committed',
      error_message: null,
      before_target_snapshot: beforeTargetSnapshot,
    ...
    return {
      connected: true,
      apply_status: 'pkg06f_active_finish_child_printing_real_apply_failed_rolled_back',
...
function renderMarkdown(report) {
  return `# PKG-06F Active Finish Child Printing Real Apply V1
...
function renderCheckpoint(report) {
  return `# PKG-06F Active Finish Child Printing Real Apply Checkpoint V1
...
Real apply was scoped to 355 child-only card_printing inserts for ex5, ex15, ex14, and ex9. No parent writes, migrations, deletes, merges, unsupported cleanup, or quarantine were performed.
`;
}

function updateCheckpointIndex() {
  const indexPath = path.join(CHECKPOINT_DIR, 'CHECKPOINT_INDEX.md');
  const line = '| 2026-06-09 | [PKG-06F Active Finish Child Printing Real Apply Checkpoint V1](20260609_pkg06f_active_finish_child_printing_real_apply_checkpoint_v1.md) | Records approved real apply for 355 active-finish child-only card_printing inserts for ex5, ex15, ex14, and ex9; parent rows unchanged, no migrations. |';
  const current = fs.existsSync(indexPath) ? fs.readFileSync(indexPath, 'utf8') : '# Master Index Checkpoints\n';
  if (current.includes('20260609_pkg06f_active_finish_child_printing_real_apply_checkpoint_v1.md')) {
    fs.writeFileSync(indexPath, current.split('\n').map((existingLine) => (
      existingLine.includes('20260609_pkg06f_active_finish_child_printing_real_apply_checkpoint_v1.md') ? line : existingLine
    )).join('\n'));
  } else {
...
const report = {
  ...
  version: 'english_master_index_pkg06f_active_finish_child_printing_real_apply_v1',
  audit_only: false,
  apply_only: true,
  approval_scope: {
  ...
  apply_status: pass
    ? 'pkg06f_active_finish_child_printing_real_apply_committed_and_verified'
    : 'pkg06f_active_finish_child_printing_real_apply_failed_or_blocked',
  ...
  verification_summary: {
    ...
    master_index_comparison_status: pass ? 'pkg06f_active_finish_child_printing_verified_after_apply' : 'not_verified',
  },

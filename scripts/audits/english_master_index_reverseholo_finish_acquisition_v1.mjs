...
const FIXTURE_DIR = 'docs/audits/verified_master_set_index_v1/source_fixtures/generated_reverseholo_v1';
const REPORT_DIR = 'docs/audits/english_master_index_source_exhaustion_v1/reverseholo_acquisition_v1';
const BASE_URL = 'https://reverseholo.app/sets';
...
function targetFacts(gaps) {
  return (gaps.facts ?? [])
    ...
    .filter((row) => ['holo', 'reverse', 'cosmos', 'cracked_ice', 'stamped'].includes(normalizeFinishKey(row.finish_key)));
}

function priorReverseHoloFacts(printings) {
  return (printings.printings ?? [])
    .filter((row) => (row.sources ?? []).includes('reverseholo_set_checklist'))
    .filter((row) => row.card_number && row.card_name && row.finish_key)
    .filter((row) => ['holo', 'reverse', 'cosmos', 'cracked_ice', 'stamped'].includes(normalizeFinishKey(row.finish_key)));
}

function candidatePrintingFacts(printings) {
  return (printings.printings ?? [])
    .filter((row) => row.card_number && row.card_name && row.finish_key)
    .filter((row) => ['holo', 'reverse', 'cosmos', 'cracked_ice', 'stamped'].includes(normalizeFinishKey(row.finish_key)));
}
...
async function fetchSetPageById(sourceId) {
  ...
  try {
    ...
    const result = await execFile('curl.exe', [
      '--ssl-no-revoke',
      '--silent',
      '--show-error',
      '--location',
  ...
  if (html.includes('Set Not Found') || html.includes('NEXT_HTTP_ERROR_FALLBACK;404')) {
    throw new Error(`ReverseHolo set not found: ${url}`);
  }
  return { url, html };
}
...
function parseRows(html) {
  const rows = [];
  const regex = /#<!-- -->([^<]+)<\/span> <span class="font-medium"[^>]*>([^<]+)<\/span><span[^>]*> · <!-- -->([^<]+)<\/span>/g;
  for (const match of html.matchAll(regex)) {
    const finishKey = finishFromLabel(decodeHtml(match[3]));
    if (!finishKey) continue;
    rows.push({
      card_number: decodeHtml(match[1]),
      card_name: decodeHtml(match[2]),
      finish_key: finishKey,
      source_label: decodeHtml(match[3]),
    });
  }
  return rows;
}
...
function buildRecords({ set, facts, rows, sourceUrl, generatedAt }) {
  ...
  for (const row of rows) {
    ...
    records.push({
      ...
      evidence_label: `ReverseHolo checklist row #${row.card_number} - ${row.card_name} - ${row.source_label}`,
      language: 'en',
      retrieved_at: generatedAt,
      raw_snapshot_ref: `reverseholo:${set.key}:${row.card_number}:${row.finish_key}`,
      notes: 'Exact finish evidence from ReverseHolo visible checklist label. Ambiguous unsupported labels are not emitted.',
    });
  }
  return records;
}
...
async function writeFixture(set, records, sourceUrl, generatedAt, dryRun) {
  ...
  try {
    ...
    for (const row of existing.records ?? []) merged.set(factKey(row), row);
  } catch (error) {
    if (error?.code !== 'ENOENT') throw error;
  }
  for (const row of records) merged.set(factKey(row), row);
  const fixture = {
    fixture_version: 'VERIFIED_MASTER_SET_INDEX_V1_HUMAN_SOURCE_FIXTURE_GENERATED',
    source_key: `reverseholo_${set.key}`,
    source_kind: 'collector_reference',
    source_url: sourceUrl,
    ...
    set_name: set.set_name,
    retrieved_at: generatedAt,
    raw_snapshot_ref: `generated_fixture:reverseholo:${set.key}:${generatedAt}`,
    generation_note: 'Generated from ReverseHolo exact visible checklist labels. Only supported finish labels are emitted.',
    records: [...merged.values()].sort((a, b) => normalizeNumber(a.card_number).localeCompare(normalizeNumber(b.card_number), undefined, { numeric: true }) || normalizeFinishKey(a.finish_key).localeCompare(normalizeFinishKey(b.finish_key))),
  };
...
async function writeReports({ results, fixtureFiles, generatedAt, dryRun }) {
  ...
  const payload = {
    version: 'REVERSEHOLO_FINISH_ACQUISITION_V1',
    ...
    dry_run: dryRun,
    rule: 'ReverseHolo evidence is used only for explicit visible finish labels mapped to supported Grookai finish keys. Ambiguous labels are ignored.',
    summary: {
  ...
  await fs.writeFile(path.join(REPORT_DIR, 'reverseholo_finish_acquisition_v1.json'), `${JSON.stringify(payload, null, 2)}\n`);
  const md = [
    '# ReverseHolo Finish Acquisition V1',
    '',
  ...
  await fs.writeFile(path.join(REPORT_DIR, 'reverseholo_finish_acquisition_v1.md'), md);
  return payload;
}
...
async function main() {
  ...
  const targets = groupBySet(mergeFacts(targetFacts(gaps), priorReverseHoloFacts(printingsPayload), candidatePrintingFacts(printingsPayload)), setsByKey, options);
  console.log(`[reverseholo] target sets ${targets.length}`);

  const results = [];
  const fixtureFiles = [];
  for (const [setKey, facts] of targets) {
    const set = setsByKey.get(setKey);
    console.log(`[reverseholo] ${set.key} ${set.set_name} target facts ${facts.length}`);
    try {
      ...
      const fixtureFile = await writeFixture(set, records, fetched.url, generatedAt, options.dryRun);
      fixtureFiles.push(fixtureFile);
      results.push({
        set_key: set.key,
        set_name: set.set_name,
        status: records.length > 0 ? 'generated' : 'no_matching_finish_rows',
  ...
  console.log(`[reverseholo] records ${report.summary.records_generated}`);
  console.log(`[reverseholo] fixtures ${report.summary.fixture_files_written}`);
}

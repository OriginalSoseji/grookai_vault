...
const SOURCE_SUMMARY_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh15a_same_set_normalized_pointer_dry_run_summary_v1.json');
const SOURCE_PLAN_JSONL = path.join(OUTPUT_DIR, 'self_hosted_images_wh15a_same_set_normalized_pointer_plan_v1.jsonl');
const PLAN_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh15b_same_set_normalized_pointer_apply_plan_v1.json');
const PLAN_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh15b_same_set_normalized_pointer_apply_plan_v1.md');
const RESULT_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh15b_same_set_normalized_pointer_apply_result_v1.json');
const RESULT_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh15b_same_set_normalized_pointer_apply_result_v1.md');
const PACKAGE_ID = 'IMG-HOST-WH-15B-SAME-SET-NORMALIZED-POINTER-APPLY';
...
function buildUpdates(summary, planRows) {
  ...
  if (summary.ready_for_apply_package !== true) {
    stopFindings.push(`wh15a_not_apply_ready:${(summary.stop_findings ?? []).join(',')}`);
  }
  if (summary.sql_hash !== APPROVED_SQL_HASH) stopFindings.push(`sql_hash_drift:${summary.sql_hash}`);

  const updates = [];
  for (const row of planRows) {
    if (row.plan_type !== 'same_set_normalized_representative_pointer') {
    ...
    const imageStatus = clean(row.proposed_values?.image_status);
    const imageNote = clean(row.proposed_values?.image_note);
    ...
    if (imageStatus !== 'representative_shared') stopFindings.push(`non_representative_status:${row.target_row_id}`);
    if (!imageNote || !imageNote.includes('not an exact image claim')) stopFindings.push(`missing_honesty_note:${row.target_row_id}`);
...
function buildPlan(args, summary, updates) {
  const plan = {
    ...
    by_match_type: countBy(updates.updates, (row) => row.match_type),
    by_source_gv_id: countBy(updates.updates, (row) => row.source_gv_id),
...
${markdownTable(topEntries(plan.by_match_type))}
`;
}

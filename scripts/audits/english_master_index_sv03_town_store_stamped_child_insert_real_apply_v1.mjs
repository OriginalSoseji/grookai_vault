...
const GATE_JSON = path.join(AUDIT_DIR, 'english_master_index_sv03_town_store_stamped_child_insert_real_apply_gate_v1.json');
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_sv03_town_store_stamped_child_insert_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_sv03_town_store_stamped_child_insert_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_sv03_town_store_stamped_child_insert_real_apply_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260612_sv03_town_store_stamped_child_insert_real_apply_checkpoint_v1.md');

const PACKAGE_ID = 'SV03-TOWN-STORE-STAMPED-CHILD-INSERT';
const PACKAGE_FINGERPRINT = 'c28c54f0d0c73da9c7beb6f52a28b19a5e091d1e8e359ebce9e8bdaae32f006d';
const DRY_RUN_PROOF_HASH = '4ceed9a6afafe94d1787466b299bf76632346fc49c8ec2fad1b91860025aff90';
const APPROVAL_TEXT = 'Approve real SV03-TOWN-STORE-STAMPED-CHILD-INSERT apply only. Fingerprint: c28c54f0d0c73da9c7beb6f52a28b19a5e091d1e8e359ebce9e8bdaae32f006d. Scope: 1 child-only card_printing insert for sv03/Obsidian Flames Town Store #196 Play Pokemon stamped parent, finish cosmos; parent writes=0, identity writes=0, deletes=0, merges=0. Dry-run proof: 4ceed9a6afafe94d1787466b299bf76632346fc49c8ec2fad1b91860025aff90 == 4ceed9a6afafe94d1787466b299bf76632346fc49c8ec2fad1b91860025aff90. No global apply. No migrations. No cleanup. No quarantine.';
...
function validatePrerequisites({ gate, dryRun, targets }) {
  ...
  if (dryRun.dry_run_status !== 'sv03_town_store_stamped_child_insert_completed_rolled_back_no_durable_change') findings.push('dry_run_not_passed');
...
async function applyPackage(client, targets) {
  ...
  if (beforeSnapshot.counts.target_parent_rows !== 1) beforeFindings.push('before_target_parent_rows_not_1');
  if (beforeSnapshot.counts.target_child_rows !== 0) beforeFindings.push('before_target_child_rows_not_0');
  if (beforeSnapshot.counts.active_identity_rows !== 1) beforeFindings.push('before_active_identity_rows_not_1');
  ...
  try {
    ...
    if (
      proofRow.target_rows !== 1 ||
      proofRow.inserted_child_rows !== 1 ||
      proofRow.active_identity_rows !== 1 ||
    ...
    return {
      apply_status: 'sv03_town_store_stamped_child_insert_real_apply_committed',
      error_message: null,
      before_snapshot: beforeSnapshot,
    ...
    return {
      apply_status: 'sv03_town_store_stamped_child_insert_real_apply_failed_rolled_back',
...
function validateAfter(applyResult) {
  ...
  if (applyResult.write_counts.child_rows_inserted !== 1) findings.push('child_rows_inserted_not_1');
  ...
  if (applyResult.after_snapshot.counts.target_parent_rows !== 1) findings.push('after_target_parent_rows_not_1');
  if (applyResult.after_snapshot.counts.target_child_rows !== 1) findings.push('after_target_child_rows_not_1');
  if (applyResult.after_snapshot.counts.active_identity_rows !== 1) findings.push('after_active_identity_rows_not_1');
  const childRows = applyResult.after_snapshot.rows.filter((row) => row.row_type === 'target_child');
  if (childRows.some((row) => row.finish_key !== 'cosmos')) findings.push('after_child_finish_not_cosmos');
  if (childRows.some((row) => row.finish_key === 'stamped')) findings.push('forbidden_stamped_child_finish_present');
  return findings;
}

function renderMarkdown(report) {
  return `# SV03 Town Store Stamped Child Insert Real Apply V1

Real apply for one child-only Town Store stamped printing insert.
...
- Only one child printing was inserted.
...
function updateCheckpointIndex() {
  ...
  if (current.includes('20260612_sv03_town_store_stamped_child_insert_real_apply_checkpoint_v1.md')) {
    fs.writeFileSync(indexPath, current.split('\n').map((existingLine) => (
      existingLine.includes('20260612_sv03_town_store_stamped_child_insert_real_apply_checkpoint_v1.md') ? line : existingLine
    )).join('\n'));
  } else {
...
const report = {
  ...
  version: 'english_master_index_sv03_town_store_stamped_child_insert_real_apply_v1',

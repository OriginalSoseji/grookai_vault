...
const SOURCE_SUMMARY_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh10b_pokemontcg_residual_parent_source_upload_dry_run_summary_v1.json');
...
const PLAN_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh10c_pokemontcg_residual_parent_source_storage_upload_apply_plan_v1.json');
const PLAN_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh10c_pokemontcg_residual_parent_source_storage_upload_apply_plan_v1.md');
const RESULT_JSONL = path.join(OUTPUT_DIR, 'self_hosted_images_wh10c_pokemontcg_residual_parent_source_storage_upload_apply_result_v1.jsonl');
const RESULT_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh10c_pokemontcg_residual_parent_source_storage_upload_apply_result_v1.json');
const RESULT_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh10c_pokemontcg_residual_parent_source_storage_upload_apply_result_v1.md');
const PACKAGE_ID = 'IMG-HOST-WH-10C-POKEMONTCG-RESIDUAL-PARENT-SOURCE-STORAGE-UPLOAD-APPLY';
const USER_AGENT = 'Grookai WH10C PokemonTCG Residual Image Upload Apply/1.0';
...
function uniqueUploadObjects(manifestRows) {
  ...
  for (const row of manifestRows) {
    ...
    const object = {
      ...
      source_non_2xx_image_body_accepted: row.source_non_2xx_image_body_accepted === true,
    ...
    existing.source_non_2xx_image_body_accepted ||= object.source_non_2xx_image_body_accepted;
  }
...
function buildPlan(summary, manifestRows, uploadObjects, args) {
  ...
  const plan = {
    ...
    accepted_non_2xx_image_body_objects: uploadObjects.filter((row) => row.source_non_2xx_image_body_accepted).length,
...
- Accepted non-2xx image body objects: ${plan.accepted_non_2xx_image_body_objects}
...
async function uploadOne(supabase, object, timeoutMs) {
  ...
  const errors = [];
  if (!fetched.ok && object.source_non_2xx_image_body_accepted !== true) errors.push(`http_${fetched.status}`);
  ...
  return {
    ...
    http_status: fetched.status,
    non_2xx_image_body_accepted: !fetched.ok && object.source_non_2xx_image_body_accepted === true,
    transport_note: fetched.transport_note,
  };
}
...
async function main() {
  ...
  const result = {
    ...
    non_2xx_image_body_accepted_uploaded_count: uploadedLatest.filter((row) => row.non_2xx_image_body_accepted).length,
...
- Non-2xx image body accepted uploaded: ${result.non_2xx_image_body_accepted_uploaded_count}

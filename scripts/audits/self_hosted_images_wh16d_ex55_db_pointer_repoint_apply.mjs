...
const WH16C_SUMMARY_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh16c_ex55_db_pointer_repoint_dry_run_summary_v1.json');
const WH16C_PLAN_JSONL = path.join(OUTPUT_DIR, 'self_hosted_images_wh16c_ex55_db_pointer_repoint_plan_v1.jsonl');
const PLAN_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh16d_ex55_db_pointer_repoint_apply_plan_v1.json');
const PLAN_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh16d_ex55_db_pointer_repoint_apply_plan_v1.md');
const RESULT_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh16d_ex55_db_pointer_repoint_apply_result_v1.json');
const RESULT_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh16d_ex55_db_pointer_repoint_apply_result_v1.md');
const PACKAGE_ID = 'IMG-HOST-WH-16D-EX55-DB-POINTER-REPOINT-APPLY';
...
function buildUpdates(summary, planRows) {
  ...
  if (summary.ready_for_apply_package !== true) {
    stopFindings.push(`wh16c_not_apply_ready:${(summary.stop_findings ?? []).join(',')}`);
  }
  ...
  for (const row of planRows) {
    ...
    if (row.set_code !== 'ex5.5') {
      stopFindings.push(`unsupported_set_code:${row.set_code}:${row.target_row_id}`);
      continue;
    }
...
function buildPlan(args, summary, updates) {
  const plan = {
    ...
    source_plan_jsonl: path.relative(ROOT, WH16C_PLAN_JSONL),
  ...
  plan.ready_for_apply = plan.stop_findings.length === 0 && plan.metadata_pointer_update_count === 5;
...
async function main() {
  ...
  const summary = await readJson(WH16C_SUMMARY_JSON);
  const planRows = await readJsonl(WH16C_PLAN_JSONL);

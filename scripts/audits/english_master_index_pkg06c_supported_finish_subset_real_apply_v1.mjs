...
const GATE_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06c_supported_finish_subset_real_apply_gate_v1.json');
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06c_supported_finish_subset_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06c_supported_finish_subset_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg06c_supported_finish_subset_real_apply_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260609_pkg06c_supported_finish_subset_real_apply_checkpoint_v1.md');
...
function validatePrerequisites({ gate, dryRun, rows }) {
  ...
  if (dryRun.dry_run_execution_status !== 'pkg06c_supported_finish_subset_completed_rolled_back_no_durable_change') findings.push('dry_run_not_passed');
  ...
  if (dryRun.supported_subset?.child_printing_rows !== EXPECTED_CHILD_ROWS) findings.push('dry_run_child_count_not_8');
  if (dryRun.supported_subset?.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('dry_run_parent_count_not_8');
  if (dryRun.supported_subset?.set_counts?.[EXPECTED_SET_KEY] !== EXPECTED_CHILD_ROWS) findings.push('dry_run_neo4_count_not_8');
  if (dryRun.supported_subset?.finish_counts?.holo !== EXPECTED_HOLO_ROWS) findings.push('dry_run_holo_count_not_8');
  if (dryRun.blocked_subset?.child_printing_rows !== EXPECTED_BLOCKED_ROWS) findings.push('dry_run_blocked_count_not_380');

  if (rows.length !== EXPECTED_CHILD_ROWS) findings.push('target_row_count_not_8');
  if (new Set(rows.map((row) => row.card_print_id)).size !== EXPECTED_PARENT_ROWS) findings.push('target_parent_count_not_8');
  if (rows.some((row) => row.set_key !== EXPECTED_SET_KEY)) findings.push('target_contains_non_neo4_row');
  if (rows.some((row) => row.finish_key !== 'holo')) findings.push('target_contains_non_holo_finish');
  return findings;
}
...
function validateBeforeSnapshot({ beforeTargetSnapshot, beforeParentSnapshot, dryRun }) {
  ...
  if (beforeTargetSnapshot.counts.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('before_parent_count_not_8');
  ...
  if (beforeParentSnapshot.count !== EXPECTED_PARENT_ROWS) findings.push('before_parent_snapshot_count_not_8');
  return findings;
}

function validateAfterSnapshot({ afterTargetSnapshot, beforeParentSnapshot, afterParentSnapshot, insertedRows }) {
  const findings = [];
  if (afterTargetSnapshot.counts.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('after_parent_count_not_8');
  if (afterTargetSnapshot.counts.existing_target_child_rows !== EXPECTED_CHILD_ROWS) findings.push('after_existing_target_child_count_not_8');
  if (afterTargetSnapshot.counts.planned_id_collision_rows !== EXPECTED_CHILD_ROWS) findings.push('after_planned_id_collision_count_not_8');
  if (afterTargetSnapshot.finish_counts.holo !== EXPECTED_HOLO_ROWS) findings.push('after_holo_count_not_8');
  if (beforeParentSnapshot.hash_sha256 !== afterParentSnapshot.hash_sha256) findings.push('parent_snapshot_changed_unexpectedly');
  if (insertedRows.counts.inserted_rows_found !== EXPECTED_CHILD_ROWS) findings.push('inserted_rows_found_not_8');
  if (insertedRows.counts.holo !== EXPECTED_HOLO_ROWS) findings.push('inserted_holo_count_not_8');
...
async function applyPackage({ dryRun, rows }) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg06c_real_apply_targets (
    ...
    await client.query(
      `insert into pkg06c_real_apply_targets (
         card_printing_id,
         card_print_id,
         set_key,
         card_number,
    ...
    const shape = await client.query(
      `select
         ...
         count(*) filter (where finish_key <> 'holo')::int as unsupported_rows
       from pkg06c_real_apply_targets`,
    );
    ...
    const lockParents = await client.query(
      `select cp.id
       from public.card_prints cp
       join pkg06c_real_apply_targets target on target.card_print_id = cp.id
       for update of cp`,
    );
    ...
    const finishGuard = await client.query(
      `select count(*)::int as unsupported_finish_count
       from pkg06c_real_apply_targets target
    ...
    const finalGuard = await client.query(
      `select
       ...
       from public.card_printings cpr
       join pkg06c_real_apply_targets target on target.card_printing_id = cpr.id`,
    );
    ...
    return {
      connected: true,
      apply_status: 'pkg06c_supported_finish_subset_real_apply_committed',
      error_message: null,
      before_target_snapshot: beforeTargetSnapshot,
    ...
    return {
      connected: true,
      apply_status: 'pkg06c_supported_finish_subset_real_apply_failed_rolled_back',
...
function renderMarkdown(report) {
  const lines = [];
  lines.push('# PKG-06C Supported Finish Subset Real Apply V1');
  lines.push('');
  lines.push('This report records the approved real apply for the PKG-06C supported-finish child-only insert subset.');
  lines.push('');
  lines.push('## Status');
  lines.push('');
  ...
  lines.push('');
  for (const [key, value] of Object.entries(report.verification_summary)) lines.push(`- ${key}: ${mdEscape(value)}`);
  lines.push('');
  ...
  lines.push('```');
  lines.push('');
  lines.push('The JSON report contains all 8 inserted row IDs for exact rollback targeting.');
  lines.push('');
...
function renderCheckpoint(report) {
  return `# PKG-06C Supported Finish Subset Real Apply Checkpoint V1

Date: 2026-06-09

## Purpose

Record the approved real apply for 8 supported child-only PKG-06C card_printing inserts.
...
- Real apply was scoped to 8 child-only card_printing inserts.
...
function updateCheckpointIndex() {
  ...
  const line = '| 2026-06-09 | [PKG-06C Supported Finish Subset Real Apply Checkpoint V1](20260609_pkg06c_supported_finish_subset_real_apply_checkpoint_v1.md) | Records approved real apply for 8 child-only neo4 card_printing inserts, holo=8, parent rows unchanged, 380 finish-taxonomy rows excluded, no migrations. |';
  const current = fs.existsSync(indexPath) ? fs.readFileSync(indexPath, 'utf8') : '# Master Index Checkpoints\n';
  if (current.includes('20260609_pkg06c_supported_finish_subset_real_apply_checkpoint_v1.md')) {
    fs.writeFileSync(
      indexPath,
      current
        .split('\n')
        .map((existingLine) =>
          existingLine.includes('20260609_pkg06c_supported_finish_subset_real_apply_checkpoint_v1.md') ? line : existingLine)
        .join('\n'),
    );
  } else {
...
const report = {
  ...
  version: 'english_master_index_pkg06c_supported_finish_subset_real_apply_v1',
  audit_only: false,
  apply_only: true,
  approval_scope: {
  ...
  apply_status: pass
    ? 'pkg06c_supported_finish_subset_real_apply_committed_and_verified'
    : 'pkg06c_supported_finish_subset_real_apply_failed_or_blocked',
  ...
  verification_summary: {
    ...
    master_index_comparison_status: pass ? 'pkg06c_supported_finish_subset_verified_after_apply' : 'not_verified',
  },

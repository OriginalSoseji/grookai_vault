...
const READINESS_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg15n_stamped_review_ready_parent_insert_readiness_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg15p_stamped_second_wave_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg15p_stamped_second_wave_guarded_dry_run_v1.md');

const PACKAGE_ID = 'PKG-15P-STAMPED-SECOND-WAVE-PARENT-INSERTS';
const CREATED_BY = 'pkg15p_stamped_second_wave_guarded_dry_run_v1';
...
function buildTargets(readiness) {
  return (readiness.rows ?? [])
    .filter((row) => row.readiness_status === 'ready_for_guarded_dry_run_parent_child_insert')
    .map((row) => ({
      ...
      evidence: {
        ...
        readiness_fingerprint: readiness.report_fingerprint_sha256,
        package_fingerprint: readiness.package_fingerprint_sha256,
...
async function runDryRun(client, targets, packageFingerprint) {
  ...
  try {
    ...
    return {
      dry_run_status: 'pkg15p_failed_rolled_back',
...
function renderMarkdown(report) {
  ...
  return `# PKG-15P Stamped Second-Wave Guarded Dry Run V1
...
async function main() {
  ...
  const expectedFingerprint = readiness.package_fingerprint_sha256;
  ...
  if (targets.length !== 6) {
    ...
    try {
      execution = await runDryRun(client, targets, expectedFingerprint);
    } finally {
  ...
  const report = {
    ...
    version: 'english_master_index_pkg15p_stamped_second_wave_guarded_dry_run_v1',
    package_id: PACKAGE_ID,
    package_fingerprint_sha256: expectedFingerprint,

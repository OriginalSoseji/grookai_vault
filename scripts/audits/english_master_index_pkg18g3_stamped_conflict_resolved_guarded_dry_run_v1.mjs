...
const READINESS_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg18g3_stamped_conflict_resolved_readiness_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg18g3_stamped_conflict_resolved_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg18g3_stamped_conflict_resolved_guarded_dry_run_v1.md');

const PACKAGE_ID = 'PKG-18G3-STAMPED-CONFLICT-RESOLVED-PARENT-INSERTS';
const CREATED_BY = 'pkg18g3_stamped_conflict_resolved_guarded_dry_run_v1';
...
function buildTargets(readiness) {
  return (readiness.rows ?? [])
    .filter((row) => row.readiness_status === 'future_guarded_parent_child_identity_insert_candidate')
    .map((row) => ({
      ...
      target_variant_key: row.target_variant_key ?? row.variant_key,
      target_printed_identity_modifier: row.target_printed_identity_modifier ?? row.target_variant_key ?? row.variant_key,
      target_gv_id: row.target_gv_id,
      target_printing_gv_id: row.target_printing_gv_id,
      stamp_label: row.stamp_label,
      source_urls: (row.evidence_sources ?? []).map((source) => source.source_url).filter(Boolean),
      source_families: [...new Set((row.evidence_sources ?? []).map((source) => source.source_key).filter(Boolean))],
      evidence: {
        readiness_package_id: readiness.package_id,
        readiness_fingerprint_sha256: readiness.package_fingerprint_sha256,
        source_families: [...new Set((row.evidence_sources ?? []).map((source) => source.source_key).filter(Boolean))],
        source_urls: (row.evidence_sources ?? []).map((source) => source.source_url).filter(Boolean),
        evidence_label: 'Conflict-resolved exact source evidence supports stamped active finish',
        raw_snapshot_ref: `pkg18g3:${row.set_key}:${row.card_number}:${row.target_variant_key ?? row.variant_key}:${row.target_finish_key}`,
        stamp_label: row.stamp_label,
        variant_key: row.target_variant_key ?? row.variant_key,
        active_child_finish_key: row.target_finish_key,
        source_evidence: row.evidence_sources ?? [],
      },
    }))
...
async function runDryRun(client, targets, fingerprint) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg18g3_targets (
    ...
    const guard = await client.query(
      `with projection as (
         ...
         from pkg18g3_targets target
         join public.card_prints base on base.id = target.base_parent_id
         left join public.sets s on s.id = base.set_id
       )
       select
         (select count(*)::int from pkg18g3_targets) as target_count,
         (select count(distinct target_parent_id)::int from pkg18g3_targets) as target_parent_count,
         (select count(distinct target_child_id)::int from pkg18g3_targets) as target_child_count,
         (select count(*)::int from pkg18g3_targets target left join public.card_prints base on base.id = target.base_parent_id where base.id is null) as missing_base_count,
         (select count(*)::int from pkg18g3_targets target left join public.finish_keys fk on fk.key = target.target_finish_key and fk.is_active = true where fk.key is null) as inactive_finish_count,
         (select count(*)::int from pkg18g3_targets target join public.card_prints base on base.id = target.base_parent_id left join public.card_printings base_child on base_child.card_print_id = base.id and base_child.finish_key = target.target_finish_key where base_child.id is null) as missing_base_finish_count,
         (select count(*)::int from pkg18g3_targets target join public.card_prints base on base.id = target.base_parent_id join public.card_prints cp on cp.set_id = base.set_id and coalesce(cp.number_plain, cp.number) = coalesce(base.number_plain, base.number) and lower(cp.name) = lower(base.name) and coalesce(cp.variant_key, '') = target.target_variant_key) as parent_collision_count,
         (select count(*)::int from pkg18g3_targets target join public.card_printings cpr on cpr.id = target.target_child_id) as child_collision_count,
         (select count(*)::int from pkg18g3_targets target join public.card_print_identity cpi on cpi.card_print_id = target.target_parent_id and cpi.is_active = true) as identity_target_collision_count,
    ...
    const childInsert = await client.query(
      `insert into public.card_printings (
       ...
       select
         ...
         concat(target.set_key, ':', target.card_number, ':stamped_conflict_resolved:', target.target_variant_key, ':', target.target_finish_key),
         $1::text,
         target.target_printing_gv_id, null, null, null, null,
         'representative_shared_stamp',
         concat('Stamped identity child finish routed from two-source conflict-resolved stamped evidence: ', target.target_finish_key)
       from pkg18g3_targets target`,
      [CREATED_BY],
    );
    ...
    const proof = await client.query(
      `select
         ...
         (select count(*)::int from pkg18g3_targets) as target_rows,
         (select count(*)::int from public.card_prints cp join pkg18g3_targets target on target.target_parent_id = cp.id) as inserted_parent_rows,
         (select count(*)::int from public.card_prints cp join pkg18g3_targets target on target.target_parent_id = cp.id and cp.printed_identity_modifier = target.target_printed_identity_modifier) as inserted_parent_modifier_rows,
         (select count(*)::int from public.card_print_identity cpi join pkg18g3_targets target on target.target_parent_id = cpi.card_print_id and cpi.is_active = true) as inserted_identity_rows,
         (select count(*)::int from public.card_printings cpr join pkg18g3_targets target on target.target_child_id = cpr.id) as inserted_child_rows,
         (select count(*)::int from public.card_printings cpr join pkg18g3_targets target on target.target_child_id = cpr.id and cpr.finish_key = 'stamped') as forbidden_stamped_child_rows`,
    ...
    return {
      dry_run_status: 'pkg18g3_failed_rolled_back',
...
function renderMarkdown(report) {
  const targetRows = report.scope.targets.map((row) => [row.set_key, row.card_number, row.card_name, row.stamp_label, row.target_variant_key, row.target_finish_key, row.target_gv_id, row.target_printing_gv_id]);
  return `# PKG-18G3 Stamped Conflict Resolved Guarded Dry Run V1
...
${markdownTable(['set', 'number', 'name', 'stamp_label', 'variant_key', 'finish', 'target_gv_id', 'target_printing_gv_id'], targetRows)}
...
async function main() {
  ...
  const recommended = execution.dry_run_status === 'pkg18g3_completed_rolled_back_no_durable_change' && execution.rollback_verified && execution.stop_findings.length === 0
  ...
  const report = {
    ...
    version: 'english_master_index_pkg18g3_stamped_conflict_resolved_guarded_dry_run_v1',
    package_id: PACKAGE_ID,

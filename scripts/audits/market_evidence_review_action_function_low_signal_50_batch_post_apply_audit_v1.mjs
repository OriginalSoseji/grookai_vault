...
const PACKAGE_ID = "MEE-CORE-INTERNAL-REVIEW-ACTION-FUNCTION-LOW-SIGNAL-50-BATCH-POST-APPLY-AUDIT-V1";
const SOURCE_PACKAGE_ID = "MEE-CORE-INTERNAL-REVIEW-ACTION-FUNCTION-LOW-SIGNAL-50-BATCH-PLAN-V1";
const SOURCE_PACKAGE_FINGERPRINT = "efa823f4b29c0de2852b82b397b3b450fe034704acfb177e2d51c4922020f1ad";
const ROW_MANIFEST_HASH = "7d0db97a1aadebc1929c0b81b43610d61f53305edaa52e81c0375d143f85f21e";
const EXPECTED_ACTION = "confirm_monitor_only";
const EXPECTED_ACTOR = "system_low_signal_50_batch_plan";
const EXPECTED_BATCH_SIZE = 50;
...
if (eventRows.length !== EXPECTED_BATCH_SIZE) findings.push("package_event_count_not_fifty");
if (dispositionRows.length !== EXPECTED_BATCH_SIZE) findings.push("target_disposition_count_not_fifty");
if (dashboardRows.length !== EXPECTED_BATCH_SIZE) findings.push("dashboard_row_count_not_fifty");
if (Number(packageEventCounts.package_event_rows) !== EXPECTED_BATCH_SIZE) findings.push("package_event_rows_not_fifty");
if (Number(packageEventCounts.distinct_event_disposition_rows) !== EXPECTED_BATCH_SIZE) findings.push("distinct_event_disposition_rows_not_fifty");
if (Number(packageEventCounts.public_flag_event_rows) !== 0) findings.push("event_public_flags_present");
if (Number(dispositionCounts.updated_target_rows) !== EXPECTED_BATCH_SIZE) findings.push("updated_target_rows_not_fifty");
if (Number(dispositionCounts.public_flag_disposition_rows) !== 0) findings.push("disposition_public_flags_present");
if (Number(dashboardCounts.dashboard_updated_rows) !== EXPECTED_BATCH_SIZE) findings.push("dashboard_updated_rows_not_fifty");
...
const nextBatchRecommendation = {
  recommended_next_batch_size: findings.length === 0 ? 100 : 0,
...
const readbackSql = `-- MEE_CORE_INTERNAL_REVIEW_ACTION_FUNCTION_LOW_SIGNAL_50_BATCH_POST_APPLY_AUDIT_V1 readback SQL.
...
const report = {
  ...
  mode: "run_only_low_signal_50_batch_post_apply_audit_read_only",
...
const planMd = `# MEE Core Internal Review Action Function Low Signal 50 Batch Post Apply Audit V1
...
writeFileSync(path.join(SQL_DIR, "mee_core_internal_review_action_function_low_signal_50_batch_post_apply_audit_v1_readback.sql"), readbackSql);
writeFileSync(path.join(PLAN_DIR, "MEE_CORE_INTERNAL_REVIEW_ACTION_FUNCTION_LOW_SIGNAL_50_BATCH_POST_APPLY_AUDIT_V1.md"), planMd);

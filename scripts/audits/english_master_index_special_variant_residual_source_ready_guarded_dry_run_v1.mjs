...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'residual_source_ready_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'residual_source_ready_guarded_dry_run_v1.md');

const PACKAGE_ID = 'SPECIAL-VAR-05-RESIDUAL-SOURCE-READY-PARENT-INSERTS';
const CREATED_BY = 'english_master_index_special_variant_residual_source_ready_guarded_dry_run_v1';
const EXPECTED_TARGET_COUNT = 4;
const TARGET_KEYS = new Set([
  'base1-17-beedrill-d-fending-error',
  'base1-47-diglett-sideways-fighting-energy',
  'base3-15-zapdos-missing-holo-evolution-box',
  'gym2-119-rockets-minefield-gym-corrected-text-lane',
]);
...
function sqlHash() {
  return sha256(stableJson({
    package_id: PACKAGE_ID,
    writes: [
      'insert public.card_prints cloned from source-backed residual WOTC parent with exact recognized error/correction identity modifier',
...
async function runDryRun(client, targets, fingerprint) {
  ...
  try {
    ...
    return {
      dry_run_status: 'residual_source_ready_parent_insert_failed_rolled_back',
...
function renderMarkdown(report) {
  return `# Residual Source Ready Variants Guarded Dry Run V1
...
async function main() {
  ...
  const pass = execution.dry_run_status === 'residual_source_ready_parent_insert_completed_rolled_back_no_durable_change'
  ...
  const recommended = pass
    ? `Approve real ${PACKAGE_ID} apply only. Fingerprint: ${fingerprint}. SQL hash: ${sqlHashSha256}. Scope: 4 residual source-ready special-case parent inserts, 4 active identity inserts, 4 child printing inserts; source_unique_finish_rows=${sourceUniqueFinishRows}; sets base1=2, base3=1, gym2=1. Dry-run proof: ${dryRunProof}. No global apply. No migrations. No deletes. No merges. No unsupported cleanup.`

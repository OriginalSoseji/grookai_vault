...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'piplup_ultra_prism_retailer_stamp_parent_insert_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'piplup_ultra_prism_retailer_stamp_parent_insert_guarded_dry_run_v1.md');

const PACKAGE_ID = 'RETAILER-STAMP-05-PIPLUP-UPR-PARENT-IDENTITY-INSERTS';
const CREATED_BY = 'piplup_ultra_prism_retailer_stamp_parent_insert_guarded_dry_run_v1';
const BASE_PARENT_ID = '9d7ad936-c000-4401-9bc0-d1191b157af2';
const SET_KEY = 'sm5';
const CARD_NUMBER = '32';
const CARD_NAME = 'Piplup';
...
const TARGETS = [
  {
    ...
    target_variant_key: 'build_a_bear_workshop_stamp',
    target_finish_key: 'normal',
    stamp_label: 'Build-A-Bear Workshop Stamp',
    evidence: [
      {
        source_key: 'tcgplayer_exact_product',
        source_kind: 'marketplace_checklist',
        source_url: 'https://www.tcgplayer.com/product/189762/pokemon-miscellaneous-cards-and-products-piplup-32-156-build-a-bear-workshop-exclusive',
        evidence_label: 'TCGplayer exact product identifies Piplup 32/156 Build-A-Bear Workshop Exclusive.',
      },
      {
        source_key: 'pricecharting_exact_product',
        source_kind: 'marketplace_checklist',
        source_url: 'https://www.pricecharting.com/game/pokemon-ultra-prism/piplup-build-a-bear-32',
        evidence_label: 'PriceCharting exact product identifies Piplup [Build-A-Bear] #32 in Pokemon Ultra Prism.',
      },
      {
        source_key: 'bulbapedia_set_list',
        source_kind: 'human_readable_checklist',
        source_url: 'https://bulbapedia.bulbagarden.net/wiki/Ultra_Prism_(TCG)',
        evidence_label: 'Bulbapedia Ultra Prism additional cards list includes Piplup 32/156 Build-A-Bear Workshop stamp promo.',
      },
    ],
  },
  {
    ...
    card_name: CARD_NAME,
    base_parent_id: BASE_PARENT_ID,
    target_variant_key: 'toys_r_us_stamp',
    target_finish_key: 'cosmos',
    stamp_label: 'Toys R Us Stamp',
    evidence: [
      {
        source_key: 'bulbapedia_set_list',
        source_kind: 'human_readable_checklist',
        source_url: 'https://bulbapedia.bulbagarden.net/wiki/Ultra_Prism_(TCG)',
        evidence_label: 'Bulbapedia Ultra Prism additional cards list identifies Piplup 32/156 as Cosmos Holo Toys "R" Us stamp exclusive.',
      },
      {
        source_key: 'bulbapedia_toys_r_us_promotional_cards',
        source_kind: 'human_readable_checklist',
        source_url: 'https://bulbapedia.bulbagarden.net/wiki/Toys_%22R%22_Us_Promotional_cards_(TCG)',
        evidence_label: 'Bulbapedia Toys "R" Us promotional checklist includes Piplup 32/156 for the February 2018 Ultra Prism promotion.',
      },
      {
        source_key: 'pricecharting_exact_product',
        source_kind: 'marketplace_checklist',
        source_url: 'https://www.pricecharting.com/game/pokemon-ultra-prism/piplup-toysrus-32',
        evidence_label: 'PriceCharting exact product identifies Piplup [ToysRUs] #32 with Toys "R" Us stamped promo sale labels.',
      },
    ],
  },
];
...
function buildTargets() {
  return TARGETS.map((row) => {
    ...
    return {
      ...row,
      target_parent_id: uuidFromSeed(`${PACKAGE_ID}:parent:${row.set_key}:${numberKey}:${nameKey}:${row.target_variant_key}`),
      target_child_id: uuidFromSeed(`${PACKAGE_ID}:child:${row.set_key}:${numberKey}:${nameKey}:${row.target_variant_key}:${row.target_finish_key}`),
      target_printed_identity_modifier: row.target_variant_key,
      evidence_urls: row.evidence.map((item) => item.source_url),
      evidence_payload: {
        source_package_id: PACKAGE_ID,
        source_status: 'exact_retailer_stamp_evidence',
        selected_base_parent_id: row.base_parent_id,
        stamped_variant_key: row.target_variant_key,
        stamp_label: row.stamp_label,
        active_child_finish_key: row.target_finish_key,
        evidence: row.evidence,
      },
    };
  }).sort((left, right) => String(left.target_variant_key).localeCompare(String(right.target_variant_key)));
}
...
async function runDryRun(client, targets, fingerprint) {
  ...
  try {
    ...
    await client.query(
      `create temporary table piplup_upr_retailer_stamp_targets (
    ...
    await client.query(
      `insert into piplup_upr_retailer_stamp_targets
       select *
    ...
    const guard = await client.query(
      `with projection as (
         ...
         from piplup_upr_retailer_stamp_targets target
         join public.card_prints base on base.id = target.base_parent_id
         left join public.sets s on s.id = base.set_id
       )
       select
         (select count(*)::int from piplup_upr_retailer_stamp_targets) as target_count,
         (select count(distinct target_parent_id)::int from piplup_upr_retailer_stamp_targets) as target_parent_count,
         (select count(distinct target_child_id)::int from piplup_upr_retailer_stamp_targets) as target_child_count,
         (select count(*)::int from piplup_upr_retailer_stamp_targets target left join public.card_prints base on base.id = target.base_parent_id where base.id is null) as missing_base_count,
         (select count(*)::int from piplup_upr_retailer_stamp_targets target join public.card_prints base on base.id = target.base_parent_id where base.set_code <> target.set_key or base.number <> target.card_number or lower(base.name) <> lower(target.card_name)) as base_identity_mismatch_count,
         (select count(*)::int from piplup_upr_retailer_stamp_targets target left join public.finish_keys fk on fk.key = target.target_finish_key and fk.is_active = true where fk.key is null) as inactive_finish_count,
         (select count(*)::int from piplup_upr_retailer_stamp_targets target join public.card_prints base on base.id = target.base_parent_id join public.card_prints cp on cp.set_id = base.set_id and coalesce(cp.number_plain, cp.number) = coalesce(base.number_plain, base.number) and lower(cp.name) = lower(base.name) and coalesce(cp.variant_key, '') = target.target_variant_key) as parent_collision_count,
         (select count(*)::int from piplup_upr_retailer_stamp_targets target join public.card_printings cpr on cpr.id = target.target_child_id) as child_collision_count,
         (select count(*)::int from piplup_upr_retailer_stamp_targets target join public.card_print_identity cpi on cpi.card_print_id = target.target_parent_id and cpi.is_active = true) as identity_target_collision_count,
    ...
    const parentInsert = await client.query(
      `insert into public.card_prints (
       ...
       select
         ...
         concat('GV-PK-', upper(regexp_replace(coalesce(base.printed_set_abbrev, s.printed_set_abbrev, base.set_code), '[^A-Za-z0-9]+', '-', 'g')), '-', base.number_plain, '-', upper(regexp_replace(target.target_variant_key, '[^A-Za-z0-9]+', '-', 'g'))),
    ...
    const childInsert = await client.query(
      `insert into public.card_printings (
       ...
       select
         ...
         concat(target.set_key, ':', target.card_number, ':piplup_ultra_prism_retailer_stamp:', target.target_variant_key, ':', target.target_finish_key),
         $1::text,
         null, null, null, null, null,
         'representative_shared_stamp',
         concat('Piplup Ultra Prism retailer stamped identity child finish routed from source-backed evidence: ', target.target_finish_key)
       from piplup_upr_retailer_stamp_targets target`,
      [CREATED_BY],
    );
    ...
    const proof = await client.query(
      `select
         ...
         (select count(*)::int from piplup_upr_retailer_stamp_targets) as target_rows,
         (select count(*)::int from public.card_prints cp join piplup_upr_retailer_stamp_targets target on target.target_parent_id = cp.id) as inserted_parent_rows,
         (select count(*)::int from public.card_prints cp join piplup_upr_retailer_stamp_targets target on target.target_parent_id = cp.id and cp.printed_identity_modifier = target.target_printed_identity_modifier) as inserted_parent_modifier_rows,
         (select count(*)::int from public.card_print_identity cpi join piplup_upr_retailer_stamp_targets target on target.target_parent_id = cpi.card_print_id and cpi.is_active = true) as inserted_identity_rows,
         (select count(*)::int from public.card_printings cpr join piplup_upr_retailer_stamp_targets target on target.target_child_id = cpr.id) as inserted_child_rows,
         (select count(*)::int from public.card_printings cpr join piplup_upr_retailer_stamp_targets target on target.target_child_id = cpr.id and cpr.finish_key = 'stamped') as forbidden_stamped_child_rows`,
    ...
    return {
      dry_run_status: 'piplup_ultra_prism_retailer_stamp_parent_insert_failed_rolled_back',
...
function renderMarkdown(report) {
  return `# Piplup Ultra Prism Retailer Stamp Parent Insert Guarded Dry Run V1
...
${markdownTable(['set', 'number', 'name', 'stamp_label', 'variant_key', 'finish', 'base_parent_id'], report.scope.targets.map((row) => [
    row.set_key,
    row.card_number,
    row.card_name,
    row.stamp_label,
...
${markdownTable(['target', 'source', 'url', 'label'], report.scope.targets.flatMap((row) => row.evidence.map((item) => [
    row.stamp_label,
    item.source_key,
    item.source_url,
    item.evidence_label,
...
async function main() {
  ...
  const recommended = execution.dry_run_status === 'piplup_ultra_prism_retailer_stamp_parent_insert_completed_rolled_back_no_durable_change'
    && execution.rollback_verified
    && execution.stop_findings.length === 0
    ? `Approve real ${PACKAGE_ID} apply only. Fingerprint: ${fingerprint}. Scope: ${targets.length} Piplup Ultra Prism retailer-stamped parent inserts, ${targets.length} identity inserts, ${targets.length} child printing inserts; finishes ${Object.entries(byFinish).map(([finish, count]) => `${finish}=${count}`).join(', ')}; stamp labels ${Object.entries(byStampLabel).map(([label, count]) => `${label}=${count}`).join(', ')}; sets ${Object.entries(bySet).map(([set, count]) => `${set}=${count}`).join(', ')}. Dry-run proof: ${execution.before_snapshot.hash_sha256} == ${execution.after_rollback_snapshot.hash_sha256}. No parent overwrites. No global apply. No migrations. No deletes. No merges. No unsupported cleanup.`
    : 'Not approval-ready; dry-run did not pass cleanly.';

  const report = {
    generated_at: new Date().toISOString(),
    version: 'piplup_ultra_prism_retailer_stamp_parent_insert_guarded_dry_run_v1',
    package_id: PACKAGE_ID,

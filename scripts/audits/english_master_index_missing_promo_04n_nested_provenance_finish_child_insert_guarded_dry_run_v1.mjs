...
const EXTRACTION_JSON = path.join(AUDIT_DIR, 'nested_provenance_finish_extraction_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'nested_provenance_finish_child_insert_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'nested_provenance_finish_child_insert_guarded_dry_run_v1.md');

const PACKAGE_ID = 'MISSING-PROMO-04N-NESTED-PROVENANCE-FINISH-CHILD-INSERTS';
const CREATED_BY = 'english_master_index_missing_promo_04n_nested_provenance_finish_child_insert_guarded_dry_run_v1';
const FINISH_SUFFIX = {
  normal: 'STD',
  holo: 'HOLO',
  reverse: 'RH',
...
function buildTargets(extractionReport) {
  return (extractionReport.approved_rows ?? [])
    .filter((row) => row.extraction_status === 'approved_for_guarded_dry_run')
    .map((row) => {
      ...
      return {
        ...
        family: row.family,
        evidence_mode: 'nested_provenance_exact_finish',
        provenance: {
          source: 'verified_master_index_v1',
          package_id: PACKAGE_ID,
          nested_provenance_extraction_package_id: extractionReport.package_id,
          nested_provenance_extraction_fingerprint_sha256: extractionReport.fingerprint_sha256,
          evidence_mode: 'nested_provenance_exact_finish',
          evidence_urls: row.evidence_rows.map((source) => source.source_url).filter(Boolean),
          evidence_labels: row.evidence_rows.map((source) => source.evidence_label).filter(Boolean),
          evidence_sources: row.evidence_rows,
        },
      };
    })
...
function sqlHash() {
  return sha256(stableJson({
    package_id: PACKAGE_ID,
    writes: ['insert child-only public.card_printings rows for nested provenance exact finish evidence'],
...
function renderMarkdown(report) {
  return [
    '# Nested Provenance Finish Child Insert Guarded Dry Run V1',
    '',
    'Rollback-only dry-run for child printings proven by exact finish text in nested preserved provenance. No durable DB writes are performed.',
    '',
    markdownTable(
      ['metric', 'value'],
      [
...
async function main() {
  ...
  const targets = buildTargets(extractionReport);
  ...
  try {
    ...
    const report = {
      ...
      input_artifacts: {
        nested_provenance_finish_extraction: rel(EXTRACTION_JSON),
      },
      package_fingerprint_sha256: packageHash,
      sql_hash_sha256: sqlHashValue,
      dry_run_proof_sha256: dryRunProof,
      rollback_proof_sha256: dryRun?.afterSnapshot.hash_sha256 ?? 'not_applicable_no_ready_targets',
      summary: {
      ...
      recommended_real_apply_approval_text: targets.length
        ? `Approve real ${PACKAGE_ID} apply only. Fingerprint: ${packageHash}. SQL hash: ${sqlHashValue}. Scope: ${targets.length} child-only card_printing inserts from nested provenance exact finish evidence; finishes ${Object.entries(countBy(targets, (target) => target.finish_key)).map(([key, value]) => `${key}=${value}`).join(', ')}. Dry-run proof: ${dryRunProof} == ${dryRunProof}. No parent writes. No identity writes. No external mapping writes. No pricing writes. No image writes. No global apply. No migrations. No deletes. No merges. No unsupported cleanup.`

...
const SUMMARY_JSON = path.join(
  OUTPUT_DIR,
  'self_hosted_images_wh09j_world_championship_energy_template_pointer_dry_run_summary_v1.json',
);
const SUMMARY_MD = path.join(
  OUTPUT_DIR,
  'self_hosted_images_wh09j_world_championship_energy_template_pointer_dry_run_summary_v1.md',
);
const PLAN_JSONL = path.join(
  OUTPUT_DIR,
  'self_hosted_images_wh09j_world_championship_energy_template_pointer_plan_v1.jsonl',
);

const PACKAGE_ID = 'IMG-HOST-WH-09J-WORLD-CHAMPIONSHIP-ENERGY-TEMPLATE-POINTER-DRY-RUN';
const FUTURE_APPLY_PACKAGE_ID = 'IMG-HOST-WH-09K-WORLD-CHAMPIONSHIP-ENERGY-TEMPLATE-POINTER-APPLY';
const EXPECTED_ALREADY_REPRESENTATIVE_ROWS = 1861;
const EXPECTED_CURRENT_MISSING_ROWS = 83;
...
function targetEnergyNames(name) {
  const exact = clean(name);
  if (!exact) return [];
  const names = [exact];
  if (exact.startsWith('Basic ')) names.push(exact.slice('Basic '.length));
  return [...new Set(names)];
}

function releaseYear(row) {
  const value = row.release_date ? new Date(row.release_date).getUTCFullYear() : null;
  return Number.isFinite(value) ? value : null;
}

function releaseTime(row) {
  const value = row.release_date ? new Date(row.release_date).getTime() : null;
  return Number.isFinite(value) ? value : null;
}

function isTrainerKitSource(row) {
  return /^tk/i.test(String(row.set_code ?? '')) || /trainer kit/i.test(String(row.set_name ?? ''));
}

function printedNumberValue(row) {
  const match = String(row.number ?? '').match(/\d+/);
  return match ? Number.parseInt(match[0], 10) : 9999;
}

function sourceRank(targetRow, sourceRow) {
  const year = releaseYear(sourceRow);
  const deckYear = Number(targetRow.deck_year);
  const targetTime = Date.UTC(deckYear, 11, 31);
  const sourceTime = releaseTime(sourceRow);
  let score = 0;
  if (sourceRow.name === targetRow.name) score += 1000;
  if (sourceRow.image_status === 'exact') score += 200;
  if (!clean(sourceRow.variant_key) || sourceRow.variant_key === 'standard') score += 100;
  if (sourceTime && sourceTime <= targetTime) score += 500;
  if (sourceTime) score -= Math.floor(Math.abs(targetTime - sourceTime) / 86400000);
  if (year && year > deckYear) score -= 500;
  score -= printedNumberValue(sourceRow) / 10000;
  if (isTrainerKitSource(sourceRow)) score -= 1000;
  if (sourceRow.set_code?.startsWith('wcd')) score -= 1000;
  return score;
}
...
async function fetchRows(energyNames) {
  const url = dbUrl();
  ...
  try {
    ...
    const sourceRows = await client.query(`
      select cp.id::text, cp.gv_id, cp.name, cp.set_code, coalesce(s.name, cp.set_code) as set_name,
             s.release_date, cp.number, cp.number_plain, cp.variant_key, cp.image_source, cp.image_path,
             cp.image_status, cp.image_note
      from public.card_prints cp
      left join public.sets s on s.code = cp.set_code
      where cp.set_code not like 'wcd%'
        and cp.image_path like 'warehouse-derived/self-hosted-images-v1/%'
...
function buildPlanRow({ wcdRow, manifestRow, sourceRow, matchKind }) {
  ...
  return {
    ...
    source_reference_kind: manifestRow.source_card_reference_kind ?? null,
    energy_template_match_kind: matchKind,
    ...
    representative_source_release_date: sourceRow.release_date ?? null,
    ...
    proposed_values: {
      ...
      image_note: `Energy-template representative image from ordinary source card ${sourceRow.gv_id} for World Championship Deck display continuity. This WCD decklist row has no source set/card number, so this is not an exact ${deckYear} ${deckName} replica image and may omit World Championship back, border, player signature treatment, and the exact Energy print used in the deck. Prepared by ${PACKAGE_ID}; exact WCD image remains uncataloged.`,
    },
...
async function main() {
  ...
  const possibleEnergyNames = [...new Set(sourceCards.flatMap((row) => targetEnergyNames(row.name)))];
  const { currentWcd, sourceRows } = await fetchRows(possibleEnergyNames);
  ...
  for (const wcdRow of missingWcd) {
    ...
    if (manifestRow.source_card_reference_kind !== 'tcg_named_template'
      || clean(manifestRow.source_set_name)
      || clean(manifestRow.source_card_number)) {
      unresolvedRows.push({ ...wcdRow, match_kind: 'not_energy_template_scope' });
      continue;
    }

    const names = targetEnergyNames(wcdRow.name);
    const candidates = sourceRows
      .filter((sourceRow) => names.includes(sourceRow.name))
      .filter((sourceRow) => sourceRow.image_status === 'exact')
      .filter((sourceRow) => !clean(sourceRow.variant_key) || sourceRow.variant_key === 'standard')
      .filter((sourceRow) => !isTrainerKitSource(sourceRow))
      .sort((left, right) => sourceRank({ ...wcdRow, deck_year: manifestRow.deck_year }, right)
        - sourceRank({ ...wcdRow, deck_year: manifestRow.deck_year }, left)
        || left.gv_id.localeCompare(right.gv_id));

    if (!candidates.length) {
      unresolvedRows.push({
        ...wcdRow,
        match_kind: 'no_same_name_energy_source_image',
        target_energy_names: names,
      });
      continue;
    }
    const bestScore = sourceRank({ ...wcdRow, deck_year: manifestRow.deck_year }, candidates[0]);
    const bestCandidates = candidates.filter((sourceRow) => sourceRank({ ...wcdRow, deck_year: manifestRow.deck_year }, sourceRow) === bestScore);
    if (bestCandidates.length !== 1) {
      unresolvedRows.push({
        ...wcdRow,
        match_kind: 'ambiguous_energy_template_source_image',
        target_energy_names: names,
        candidate_source_gv_ids: bestCandidates.map((row) => row.gv_id),
      });
      continue;
    }

    const sourceRow = bestCandidates[0];
    const matchKind = sourceRow.name === wcdRow.name
      ? 'same_name_energy_template_representative'
      : 'basic_prefix_stripped_energy_template_representative';
    planRows.push(buildPlanRow({ wcdRow, manifestRow, sourceRow, matchKind }));
  }
  ...
  const summary = {
    ...
    energy_template_candidate_rows: planRows.length,
    ...
    candidate_energy_names: countBy(planRows, (row) => row.name),
    source_energy_names: countBy(planRows, (row) => row.name),
    source_match_kinds: countBy(planRows, (row) => row.energy_template_match_kind),
    source_set_codes: countBy(planRows, (row) => row.representative_source_set_code),
    ...
    changed_column_sets: countBy(planRows, (row) => row.changed_columns.join(',')),
    ...
    samples: {
      candidates: planRows.slice(0, 30).map((row) => ({
        gv_id: row.gv_id,
        name: row.name,
        set_code: row.set_code,
        source_gv_id: row.representative_source_gv_id,
        source_set_name: row.representative_source_set_name,
        source_release_date: row.representative_source_release_date,
        match_kind: row.energy_template_match_kind,
      })),
      unresolved_rows: unresolvedRows.slice(0, 30).map((row) => ({
        gv_id: row.gv_id,
        name: row.name,
        set_code: row.set_code,
  ...
  summary.fingerprint = proofHash({
    ...
    energy_template_candidate_rows: summary.energy_template_candidate_rows,
...
- Energy-template candidate rows: ${summary.energy_template_candidate_rows}
...
${markdownTable(topEntries(summary.candidate_energy_names, 40))}
...
${markdownTable(topEntries(summary.source_set_codes, 40))}

...
const SOURCE_JSON = path.join(OUTPUT_DIR, 'enrich13c_radiant_collection_prefix_governance_v1.json');
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'enrich13c1_radiant_collection_duplicate_transfer_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'enrich13c1_radiant_collection_duplicate_transfer_guarded_dry_run_v1.md');
const OUTPUT_SQL = path.join(SQL_DIR, 'enrich13c1_radiant_collection_duplicate_transfer_guarded_dry_run_v1.sql');

const PACKAGE_ID = 'ENRICH-13C1-RADIANT-COLLECTION-DUPLICATE-TRANSFER-DRY-RUN';
...
async function buildTargetRows(client, source) {
  const rows = [];
  for (const row of source.rows ?? []) {
    const owner = await client.query(
      `select cp.id::text, cp.name, cp.set_code, cp.number, cp.number_plain, cp.printed_identity_modifier, cp.gv_id
       from public.card_prints cp
       where cp.set_code = $1
         and cp.number = $2
         and cp.name = $3
         and cp.printed_identity_modifier = $4
       order by cp.id
       limit 2`,
      [row.proposed_set_code, row.proposed_number, row.card_name, row.proposed_identity_modifier],
    );
    rows.push({
      duplicate_card_print_id: row.card_print_id,
      canonical_owner_card_print_id: owner.rows.length === 1 ? owner.rows[0].id : null,
      ...
      canonical_owner_name: owner.rows[0]?.name ?? null,
      printed_identity_modifier: row.proposed_identity_modifier,
      source_external_id: row.source_external_id,
      owner_match_count: owner.rows.length,
      owner_gv_id: owner.rows[0]?.gv_id ?? null,
    });
  }
  return rows;
}
...
function buildSql(targetRows, packageFingerprint) {
  const values = targetRows.map((row) => `  (${[
    ...
    sqlString(row.printed_identity_modifier),
...
-- Scope: ${targetRows.length} Radiant Collection duplicate parent dependency transfers.
...
begin
  ...
  if v_targets <> ${targetRows.length} then
    raise exception 'ENRICH-13C1 target count guard failed: expected ${targetRows.length}, got %', v_targets;
  end if;
...
async function runDryRun(sql, targetRows) {
  ...
  const client = new Client({ connectionString: cs, application_name: 'enrich13c1_radiant_collection_duplicate_transfer_guarded_dry_run_v1' });
...
function validate({ source, targetRows, sql, execution }) {
  ...
  if (source.version !== 'ENRICH_13C_RADIANT_COLLECTION_PREFIX_GOVERNANCE_V1') findings.push('source_report_version_unexpected');
  if (targetRows.length !== EXPECTED_ROWS) findings.push(`target_row_count_not_${EXPECTED_ROWS}`);
  if (targetRows.some((row) => row.owner_match_count !== 1 || !row.canonical_owner_card_print_id)) findings.push('canonical_rc_owner_match_not_exactly_one');
  if (targetRows.some((row) => row.printed_identity_modifier !== 'number_prefix:RC')) findings.push('target_modifier_not_rc_prefix');
  ...
  if (Number(before.duplicate_child_printings ?? -1) !== 60) findings.push('before_duplicate_child_printing_count_changed');
  ...
  if (Number(before.duplicate_species_rows ?? -1) !== 19) findings.push('before_duplicate_species_count_changed');
...
function renderMarkdown(report) {
  return `# ENRICH-13C1 Radiant Collection Duplicate Transfer Guarded Dry-Run V1
...
const cs = connectionString();
if (!cs) throw new Error('Missing SUPABASE_DB_URL/DATABASE_URL/POSTGRES_URL.');
const client = new Client({ connectionString: cs, application_name: 'enrich13c1_radiant_collection_duplicate_transfer_guarded_dry_run_v1_prepare' });
await client.connect();
let targetRows;
try {
  targetRows = await buildTargetRows(client, source);
} finally {
  await client.end().catch(() => {});
}
...
const report = {
  version: 'ENRICH_13C1_RADIANT_COLLECTION_DUPLICATE_TRANSFER_GUARDED_DRY_RUN_V1',
  generated_at: new Date().toISOString(),
  package_id: PACKAGE_ID,
  dry_run_execution_status: stopFindings.length === 0
    ? 'enrich13c1_guarded_dry_run_passed_rolled_back_no_durable_change'
    : 'enrich13c1_guarded_dry_run_blocked_or_failed',
  ...
  package_scope: {
    ...
    set_code: 'bw11',
    identity_modifier: 'number_prefix:RC',
    ...
    canonical_owner_match_count: targetRows.filter((row) => row.owner_match_count === 1).length,
  },
  sql_artifact: {
    path: OUTPUT_SQL,
    sha256: sqlHash,
  ...
  required_real_apply_approval_text: `Approve real ENRICH-13C1-RADIANT-COLLECTION-DUPLICATE-TRANSFER apply only. Fingerprint: ${packageFingerprint}. SQL hash: ${sqlHash}. Scope: ${targetRows.length} Radiant Collection duplicate parent dependency transfers, ${execution.before_snapshot?.impact_counts?.duplicate_child_printings ?? 'unknown'} child printings deduped/transferred, ${execution.before_snapshot?.impact_counts?.duplicate_external_mappings ?? 'unknown'} external mappings handled, ${execution.before_snapshot?.impact_counts?.duplicate_identity_rows ?? 'unknown'} duplicate active identities removed. Dry-run proof: ${execution.before_snapshot?.hash_sha256 ?? 'missing'} == ${execution.after_snapshot?.hash_sha256 ?? 'missing'}. No global apply. No migrations. No image writes.`,
};

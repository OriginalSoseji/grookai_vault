...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg02b_full_beta_guarded_dry_run_execution_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg02b_full_beta_guarded_dry_run_execution_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260609_pkg02b_full_beta_guarded_dry_run_execution_checkpoint_v1.md');
...
const APPROVAL_TEXT = 'Approve PKG-02B-FULL-BETA for guarded dry-run transaction execution only. Fingerprint: 932c4fe9c332c1896aecaeac08bd1faf1e005fd1eb9f07f3a50bf8ad2a83c7b8. Scope: 18 beta packages, 422 card_print updates, 643 verified child printings, 4 vault references accepted. No real apply. No migrations.';
...
function validateArtifact(artifact, sql, sqlHash) {
  ...
  if ((scope.excluded_set_keys ?? []).length !== 0) findings.push('source_artifact_has_excluded_sets');
  if (scope.vault_reference_policy !== 'accepted_for_beta_tester_catalog_correction_planning') {
    findings.push('source_artifact_vault_policy_mismatch');
  }
  if (artifact.required_operator_approval?.exact_phrase !== APPROVAL_TEXT) {
    findings.push('approval_phrase_mismatch');
  }
  ...
  if (/(^|\n)\s*commit\s*;/i.test(sql.replace(/--.*$/gm, ''))) findings.push('sql_contains_commit_statement');
  if (!/(^|\n)\s*rollback\s*;/i.test(sql.replace(/--.*$/gm, ''))) findings.push('sql_missing_rollback_statement');
  if (!/\bupdate\s+public\.card_prints\b/i.test(sql)) findings.push('sql_missing_expected_update');
  if (/\bdelete\b/i.test(sql.replace(/--.*$/gm, ''))) findings.push('sql_contains_delete_statement');
...
function evaluateRun({ artifact, execution }) {
  ...
  const artifactSnapshotHash = artifact.fresh_snapshot?.hash_sha256 ?? null;
  ...
  if (execution.before_snapshot?.impact_counts?.card_prints_found !== 422) findings.push('before_snapshot_card_print_count_not_422');
  if (execution.after_snapshot?.impact_counts?.card_prints_found !== 422) findings.push('after_snapshot_card_print_count_not_422');
  if (execution.before_snapshot?.impact_counts?.card_printings_found !== 643) findings.push('before_snapshot_child_count_not_643');
  if (execution.after_snapshot?.impact_counts?.card_printings_found !== 643) findings.push('after_snapshot_child_count_not_643');
  ...
  if (execution.before_snapshot?.hash_sha256 !== artifactSnapshotHash) findings.push('fresh_snapshot_drift_before_dry_run');
...
function renderMarkdown(report) {
  const lines = [];
  lines.push('# English Master Index PKG-02B Full Beta Guarded Dry-Run Execution V1');
  lines.push('');
  lines.push('This report records the approved guarded dry-run transaction execution for `PKG-02B-FULL-BETA`.');
  lines.push('');
  lines.push('The SQL artifact was rollback-only. No real apply, migration, cleanup, or quarantine was performed.');
  lines.push('');
  lines.push('## Status');
  lines.push('');
  ...
  lines.push(`| Before card_print rows | ${report.execution_result.before_snapshot?.impact_counts?.card_prints_found ?? ''} |`);
  lines.push(`| After card_print rows | ${report.execution_result.after_snapshot?.impact_counts?.card_prints_found ?? ''} |`);
  lines.push(`| Before child printings | ${report.execution_result.before_snapshot?.impact_counts?.card_printings_found ?? ''} |`);
  lines.push(`| After child printings | ${report.execution_result.after_snapshot?.impact_counts?.card_printings_found ?? ''} |`);
  lines.push(`| Before vault refs | ${report.execution_result.before_snapshot?.impact_counts?.vault_items_found ?? ''} |`);
  lines.push(`| After vault refs | ${report.execution_result.after_snapshot?.impact_counts?.vault_items_found ?? ''} |`);
  lines.push(`| SQL contains commit | ${report.sql_artifact.contains_commit_statement} |`);
  lines.push(`| SQL contains rollback | ${report.sql_artifact.contains_rollback_statement} |`);
  lines.push('');
...
function renderCheckpoint(report) {
  return `# PKG-02B Full Beta Guarded Dry-Run Execution Checkpoint V1
...
- \`docs/audits/verified_master_set_index_v1/english_master_index_v1/english_master_index_pkg02b_full_beta_guarded_dry_run_execution_v1.json\`
- \`docs/audits/verified_master_set_index_v1/english_master_index_v1/english_master_index_pkg02b_full_beta_guarded_dry_run_execution_v1.md\`
...
function updateCheckpointIndex() {
  ...
  const line = '| 2026-06-09 | [PKG-02B Full Beta Guarded Dry-Run Execution Checkpoint V1](20260609_pkg02b_full_beta_guarded_dry_run_execution_checkpoint_v1.md) | Records approved rollback-only dry-run execution for PKG-02B-FULL-BETA: 422 card_print updates simulated, 643 child printings preserved, 4 vault refs accepted, durable state unchanged. |';
  const current = fs.readFileSync(indexPath, 'utf8');
  if (current.includes('20260609_pkg02b_full_beta_guarded_dry_run_execution_checkpoint_v1.md')) {
    fs.writeFileSync(
      indexPath,
      current
        .split('\n')
        .map((existingLine) =>
          existingLine.includes('20260609_pkg02b_full_beta_guarded_dry_run_execution_checkpoint_v1.md') ? line : existingLine)
        .join('\n'),
    );
  } else {
...
const report = {
  ...
  version: 'english_master_index_pkg02b_full_beta_guarded_dry_run_execution_v1',
  audit_only: false,
  dry_run_only: true,
  ...
  dry_run_execution_status: pass
    ? 'pkg02b_full_beta_guarded_dry_run_passed_rolled_back_no_durable_change'
    : 'pkg02b_full_beta_guarded_dry_run_blocked_or_failed',
  package_scope: {
    package_id: PACKAGE_ID,
    package_fingerprint_sha256: PACKAGE_FINGERPRINT,
    selected_package_count: artifact.package_scope.selected_package_count,
    card_print_rows: artifact.package_scope.card_print_rows,
    child_printing_rows: artifact.package_scope.child_printing_rows,
    vault_references_accepted: artifact.fresh_snapshot.impact_counts.vault_items_found,
  },

...
const DRY_RUN_JSON = path.join(OUTPUT_DIR, 'enrich03b_active_identity_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'enrich03b_active_identity_real_apply_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'enrich03b_active_identity_real_apply_v1.md');

const PACKAGE_ID = 'ENRICH-03B-ACTIVE-IDENTITY-BACKFILL-POST-DUPLICATE-DELETE';
const EXPECTED_FINGERPRINT = '0c72e3998ba3544d7b335961fd3c4cb015619930caf498c8401bb44c01de6aab';
const EXPECTED_DRY_RUN_PROOF = '5638a80567d83bbdaa825e56ac0503f6d44068ee9366c8d08c61de17942e2cc2';
const EXPECTED_TARGET_ROWS = 940;
const APPROVAL_TEXT = 'Approve real ENRICH-03B-ACTIVE-IDENTITY-BACKFILL-POST-DUPLICATE-DELETE apply only. Fingerprint: 0c72e3998ba3544d7b335961fd3c4cb015619930caf498c8401bb44c01de6aab. Scope: 940 active card_print_identity inserts from public.card_print_identity_backfill_projection_v1. Dry-run proof: 5638a80567d83bbdaa825e56ac0503f6d44068ee9366c8d08c61de17942e2cc2 == 5638a80567d83bbdaa825e56ac0503f6d44068ee9366c8d08c61de17942e2cc2. No parent writes. No child writes. No deletes. No merges. No migrations. No image writes. No global apply.';
...
function guardPassed(guard, expectedCount) {
  return guard.target_count === expectedCount
    ...
    && guard.distinct_identity_hash_count === expectedCount
...
function validateDryRun(dryRun, packageFingerprint) {
  ...
  if (dryRun.execution?.inside_transaction_proof?.inserted_rows !== EXPECTED_TARGET_ROWS) findings.push('dry_run_insert_count_mismatch');
  return findings;
}
...
async function applyPackage(client, targets) {
  ...
  try {
    ...
    await client.query(
      `create temporary table enrich03b_targets (
    ...
    const inserted = await client.query(
      `insert into public.card_print_identity (
       ...
       select
         ...
         true
       from enrich03b_targets target
       returning card_print_id::text, identity_key_hash`,
    );

    const proof = await client.query(
      `select
         (select count(*)::int from enrich03b_targets) as target_count,
         (select count(*)::int
          from enrich03b_targets target
          join public.card_print_identity cpi
...
async function main() {
  ...
  try {
    ...
    const report = {
      version: 'ENRICH03B_ACTIVE_IDENTITY_REAL_APPLY_V1',
      package_id: PACKAGE_ID,

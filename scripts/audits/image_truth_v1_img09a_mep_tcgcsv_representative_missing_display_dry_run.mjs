...
const SOURCE_AUDIT_JSON = path.join(OUTPUT_DIR, 'image_truth_mep_tcgcsv_finish_image_audit_v1.json');
const STAGING_ASSET_DIR = path.join('tmp', 'nonproduction_image_staging', 'image_truth_v1', 'img09a');
const DRY_RUN_JSON = path.join(OUTPUT_DIR, 'image_truth_img09a_mep_tcgcsv_representative_missing_display_dry_run_v1.json');
const DRY_RUN_MD = path.join(OUTPUT_DIR, 'image_truth_img09a_mep_tcgcsv_representative_missing_display_dry_run_v1.md');

const PACKAGE_ID = 'IMG-09A-MEP-TCGCSV-REPRESENTATIVE-MISSING-DISPLAY-CHILD-IMAGE-DRY-RUN';
...
async function normalizeAsset(row) {
  ...
  const plannedStoragePath = [
    'warehouse-derived',
    'image-truth-v1',
    'img09a-mep-tcgcsv-representative',
    row.set_code,
...
async function main() {
  ...
  const sourceAudit = JSON.parse(await fs.readFile(SOURCE_AUDIT_JSON, 'utf8'));
  const sourceRows = (sourceAudit.rows ?? [])
    .filter((row) => row.status === 'representative_candidate')
    .map((row) => ({
      ...row,
      ...
      source_key: 'tcgcsv_tcgplayer_catalog_live',
      source_kind: 'marketplace_checklist',
      source_url: row.selected_source_url,
      asset_url: row.selected_image_url,
    }))
    ...
    .filter((row) => row.source_key === 'tcgcsv_tcgplayer_catalog_live')
  ...
  const report = {
    ...
    package_id: PACKAGE_ID,
    source_manifest: SOURCE_AUDIT_JSON,
  ...
  await fs.writeFile(DRY_RUN_MD, `# Image Truth IMG-09A MEP TCGCSV Representative Missing-Display Dry Run V1

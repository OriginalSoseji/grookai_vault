...
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg17e1_stamped_active_finish_parent_insert_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg17e1_stamped_active_finish_parent_insert_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg17e1_stamped_active_finish_parent_insert_real_apply_v1.md');

const PACKAGE_ID = 'PKG-17E1-STAMPED-ACTIVE-FINISH-PARENT-INSERTS';
const CREATED_BY = 'pkg17e1_stamped_active_finish_parent_insert_real_apply_v1';
const EXPECTED_PACKAGE_FINGERPRINT = 'c676b6adc30ea0202a563677ee646314d7ad1f45c31a47633c4faf04ff69e7c7';
const EXPECTED_DRY_RUN_PROOF = '8d28777242700d8bba19746afee8e65510fa73fec3acc92364a36e10a507ef07';
const EXPECTED_PRE_APPLY_HASH = 'aa9e883dd3809f1e5b4fe4742d68c7334d859cdd3652d15fd674bc2659fbb856';
const EXPECTED_TARGET_COUNT = 6;
const APPROVAL_TEXT = 'Approve real PKG-17E1-STAMPED-ACTIVE-FINISH-PARENT-INSERTS apply only. Fingerprint: c676b6adc30ea0202a563677ee646314d7ad1f45c31a47633c4faf04ff69e7c7. Scope: 6 stamped parent inserts, 6 identity inserts, 6 child printing inserts; finishes cosmos=1, holo=2, reverse=3; sets sv03.5=4, swsh12.5=2. Dry-run proof: aa9e883dd3809f1e5b4fe4742d68c7334d859cdd3652d15fd674bc2659fbb856 == aa9e883dd3809f1e5b4fe4742d68c7334d859cdd3652d15fd674bc2659fbb856. No global apply. No migrations. No deletes. No merges. No unsupported cleanup.';
...
function validateDryRun(dryRun) {
  ...
  if (dryRun.execution?.dry_run_status !== 'pkg17e1_completed_rolled_back_no_durable_change') findings.push('dry_run_status_not_passed');
...
async function applyPackage(client, targets, fingerprint) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg17e1_targets (
    ...
    const guard = await client.query(
      `with projection as (
         ...
         from pkg17e1_targets target
         join public.card_prints base on base.id = target.base_parent_id
         left join public.sets s on s.id = base.set_id
       )
       select
         (select count(*)::int from pkg17e1_targets) as target_count,
         (select count(distinct target_parent_id)::int from pkg17e1_targets) as target_parent_count,
         (select count(distinct target_child_id)::int from pkg17e1_targets) as target_child_count,
         (select count(*)::int from pkg17e1_targets where target_variant_key is null or target_variant_key = '' or target_variant_key = 'stamped') as unsafe_variant_count,
         (select count(*)::int from pkg17e1_targets where target_printed_identity_modifier <> target_variant_key) as modifier_mismatch_count,
         (select count(*)::int from pkg17e1_targets target left join public.card_prints base on base.id = target.base_parent_id where base.id is null) as missing_base_count,
         (select count(*)::int from pkg17e1_targets target join public.card_prints base on base.id = target.base_parent_id where coalesce(base.variant_key, '') <> '' or base.printed_identity_modifier is not null) as base_parent_not_unstamped_count,
         (select count(*)::int from pkg17e1_targets target left join public.finish_keys fk on fk.key = target.target_finish_key and fk.is_active = true where fk.key is null) as inactive_finish_count,
         (select count(*)::int
          from pkg17e1_targets target
          join public.card_prints base on base.id = target.base_parent_id
         ...
         (select count(*)::int from pkg17e1_targets target join public.card_printings cpr on cpr.id = target.target_child_id) as child_collision_count,
         (select count(*)::int from pkg17e1_targets target join public.card_print_identity cpi on cpi.card_print_id = target.target_parent_id and cpi.is_active = true) as identity_target_collision_count,
    ...
    const proof = await client.query(
      `select
         ...
         (select count(*)::int from pkg17e1_targets) as target_rows,
         (select count(*)::int from public.card_prints cp join pkg17e1_targets target on target.target_parent_id = cp.id) as inserted_parent_rows,
         (select count(*)::int from public.card_prints cp join pkg17e1_targets target on target.target_parent_id = cp.id and cp.printed_identity_modifier = target.target_printed_identity_modifier) as inserted_parent_modifier_rows,
         (select count(*)::int from public.card_print_identity cpi join pkg17e1_targets target on target.target_parent_id = cpi.card_print_id and cpi.is_active = true) as inserted_identity_rows,
         (select count(*)::int from public.card_printings cpr join pkg17e1_targets target on target.target_child_id = cpr.id) as inserted_child_rows,
         (select count(*)::int from public.card_printings cpr join pkg17e1_targets target on target.target_child_id = cpr.id and cpr.finish_key = 'stamped') as forbidden_stamped_child_rows`,
    ...
    return {
      apply_status: 'pkg17e1_real_apply_committed',
      guard: guardRow,
      proof: proof.rows[0],
      write_counts: {
...
function renderMarkdown(report) {
  return `# PKG-17E1 Stamped Active Finish Parent Insert Real Apply V1

Approved real apply for PKG-17E1 only.
...
async function main() {
  ...
  const report = {
    ...
    version: 'english_master_index_pkg17e1_stamped_active_finish_parent_insert_real_apply_v1',

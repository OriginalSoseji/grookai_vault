...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg08g_true_parent_child_insert_real_apply_gate_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg08g_true_parent_child_insert_real_apply_gate_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260610_pkg08g_true_parent_child_insert_real_apply_gate_checkpoint_v1.md');
...
function renderMarkdown(report) {
  return `# PKG-08G True Parent+Child Insert Real Apply Gate V1
...
function updateCheckpointIndex() {
  ...
  const line = '| 2026-06-10 | [PKG-08G True Parent+Child Insert Real Apply Gate Checkpoint V1](20260610_pkg08g_true_parent_child_insert_real_apply_gate_checkpoint_v1.md) | Records no-write real-apply gate for 9 true parent+child inserts. No durable writes or migrations. |';
  const current = fs.existsSync(indexPath) ? fs.readFileSync(indexPath, 'utf8') : '# Master Index Checkpoints\n';
  if (current.includes('20260610_pkg08g_true_parent_child_insert_real_apply_gate_checkpoint_v1.md')) {
    fs.writeFileSync(indexPath, current.split('\n').map((existingLine) => (
      existingLine.includes('20260610_pkg08g_true_parent_child_insert_real_apply_gate_checkpoint_v1.md') ? line : existingLine
    )).join('\n'));
  } else {
...
if (dryRun.scope?.target_parent_rows !== 9) findings.push('target_parent_rows_not_9');
if (dryRun.scope?.target_child_rows !== 9) findings.push('target_child_rows_not_9');
if (dryRun.scope?.target_external_mappings !== 9) findings.push('target_external_mapping_rows_not_9');
if (Object.keys(dryRun.scope?.by_set ?? {}).length !== 2) findings.push('set_count_not_2');
if (dryRun.scope?.by_finish?.holo !== 1) findings.push('holo_count_not_1');
if (dryRun.scope?.by_finish?.normal !== 8) findings.push('normal_count_not_8');

const report = {
  generated_at: new Date().toISOString(),
  version: 'english_master_index_pkg08g_true_parent_child_insert_real_apply_gate_v1',
  audit_only: true,

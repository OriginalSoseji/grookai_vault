...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'wb_kids_stamp_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'wb_kids_stamp_guarded_dry_run_v1.md');

const PACKAGE_ID = 'SPECIAL-VAR-02-WB-KIDS-PROMO-STAMP-PARENT-INSERTS';
const CREATED_BY = 'english_master_index_special_variant_wb_kids_stamp_guarded_dry_run_v1';
const EXPECTED_TARGET_COUNT = 9;
const TARGET_VARIANTS = new Set(['wb_kids_stamp', 'inverted_wb_kids_stamp', 'missing_wb_kids_stamp']);
...
function buildSourceTargets(sourceReport) {
  return (sourceReport.rows ?? [])
    .filter((row) => row.set_key === 'basep'
      && TARGET_VARIANTS.has(row.proposed_variant_key)
      && row.proposed_finish_key === 'normal'
...
function sqlHash() {
  return sha256(stableJson({
    package_id: PACKAGE_ID,
    writes: [
      'insert public.card_prints cloned from base Wizards Black Star Promo parent with WB Kids stamp or recognized WB stamp error identity modifier',
      'insert public.card_print_identity via card_print_identity_backfill_projection_v1',
      'insert public.card_printings normal child',
    ],
...
async function runDryRun(client, targets, fingerprint) {
  ...
  try {
    ...
    return {
      dry_run_status: 'wb_kids_stamp_parent_insert_failed_rolled_back',
...
function renderMarkdown(report) {
  return `# WB Kids Promo Stamp Guarded Dry Run V1
...
async function main() {
  ...
  if (!conn) {
    ...
    try {
      ...
      } else if (targets.some((row) => !row.base_parent_id || !row.base_child_finishes.includes(row.target_finish_key))) {
        execution = {
          dry_run_status: 'blocked_missing_base_parent_or_normal_child',
          rollback_verified: false,
          dry_run_proof_sha256: null,
          simulated_write_counts: { parent_inserts: 0, identity_inserts: 0, child_inserts: 0, deletes: 0, merges: 0 },
          stop_findings: ['missing_base_parent_or_normal_child'],
        };
      } else {
  ...
  const pass = execution.dry_run_status === 'wb_kids_stamp_parent_insert_completed_rolled_back_no_durable_change'

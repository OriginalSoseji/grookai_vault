...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06e_active_finish_child_printing_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg06e_active_finish_child_printing_guarded_dry_run_v1.md');
const OUTPUT_SQL = path.join(SQL_DIR, 'english_master_index_pkg06e_active_finish_child_printing_guarded_dry_run_transaction_v1.sql');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260609_pkg06e_active_finish_child_printing_guarded_dry_run_checkpoint_v1.md');
...
const EXPECTED_FINISH_COUNTS = { cosmos: 2, holo: 2, normal: 1, reverse: 386 };
const EXPECTED_CHILD_ROWS = 391;
const EXPECTED_PARENT_ROWS = 387;
const PROVENANCE_SOURCE = 'verified_master_set_index_v1';
const CREATED_BY = 'pkg06e_active_finish_child_printing_dry_run_v1';
...
begin
  ...
  if child_count <> ${EXPECTED_CHILD_ROWS} then raise exception 'PKG-06E child count drift: %', child_count; end if;
  if parent_count <> ${EXPECTED_PARENT_ROWS} then raise exception 'PKG-06E parent count drift: %', parent_count; end if;
...
async function runDryRun(client, sql, rows) {
  ...
  let status = 'pkg06e_active_finish_child_printing_completed_rolled_back_no_durable_change';
  ...
  try {
    ...
    status = 'pkg06e_active_finish_child_printing_failed';
...
function evaluate({ readiness, rows, activeFinishKeys, execution }) {
  ...
  if (rows.length !== EXPECTED_CHILD_ROWS) findings.push('child_count_not_391');
  if (new Set(rows.map((row) => row.card_print_id)).size !== EXPECTED_PARENT_ROWS) findings.push('parent_count_not_387');
  ...
  if (execution.status !== 'pkg06e_active_finish_child_printing_completed_rolled_back_no_durable_change') findings.push('dry_run_transaction_did_not_complete');
...
function buildMarkdown(report) {
  return `# PKG-06E Active Finish Child Printing Guarded Dry-Run V1
...
function buildCheckpoint(report) {
  return `# PKG-06E Active Finish Child Printing Guarded Dry-Run Checkpoint V1
...
async function updateCheckpointIndex() {
  ...
  const line = '| 2026-06-09 | [PKG-06E Active Finish Child Printing Guarded Dry-Run Checkpoint V1](20260609_pkg06e_active_finish_child_printing_guarded_dry_run_checkpoint_v1.md) | Rollback-dry-runs 391 active-finish child-only inserts for ex10, ex13, ex7, and ex8; no writes or migrations. |';
  const current = await fs.readFile(indexPath, 'utf8').catch(() => '# Master Index Checkpoints\n');
  if (current.includes('20260609_pkg06e_active_finish_child_printing_guarded_dry_run_checkpoint_v1.md')) {
    await fs.writeFile(indexPath, current.split('\n').map((existing) => (
      existing.includes('20260609_pkg06e_active_finish_child_printing_guarded_dry_run_checkpoint_v1.md') ? line : existing
    )).join('\n'));
  } else {
...
async function main() {
  ...
  try {
    ...
    const report = {
      ...
      version: 'english_master_index_pkg06e_active_finish_child_printing_guarded_dry_run_v1',
      package_id: PACKAGE_ID,
      source_readiness_fingerprint_sha256: readiness.package_fingerprint_sha256,
      ...
      next_allowed_step: stopFindings.length === 0
        ? 'Prepare a real-apply gate for PKG-06E only if separately requested.'
...
main().catch((error) => {
  console.error('[pkg06e][active-finish-child-printing] failed:', error?.message ?? error);
  process.exitCode = 1;
});

...
const version='20260907060000';
const filename=`${version}_pokemon_sealed_production_foundation_v1.sql`;
...
const functions=['get_active_pokemon_sealed_pricing_v1','pokemon_sealed_image_object_signing_authorized_v1'];
...
assert.equal(localDefinitions.length,2,'Local replay is missing Pokemon functions');
...
try{
  ...
  const body={version:'POKEMON_SEALED_SCHEMA_EXECUTION_V1',producer_commit:head,migration:filename,
    ...
    protected_boundary:before,ledger_hash:hash(ledger),scope:'one migration, nullable manufacturer, hidden Pokemon control, two new RPCs'};
  ...
  if(args.mode==='plan'){
    ...
    if(!existing){
      ...
      assert.equal((await c.query("select 1 from sealed_product_game_release_controls where game_key='pokemon'")).rowCount,0,'Control collision');
    ...
    const summary={mode:args.mode,status:'passed',committed,rows_written:committed&&!existing?2:0,

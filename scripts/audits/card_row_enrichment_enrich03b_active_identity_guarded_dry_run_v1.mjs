...
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'enrich03b_active_identity_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'enrich03b_active_identity_guarded_dry_run_v1.md');
...
async function main() {
  ...
  try {
    ...
    if (execution.inside_transaction_proof?.inserted_rows !== targets.length) stopFindings.push('inserted_row_count_mismatch');
    if (execution.inside_transaction_proof?.proof?.matching_active_identity_count !== targets.length) stopFindings.push('matching_active_identity_count_mismatch');
    if (execution.inside_transaction_proof?.proof?.duplicate_active_identity_hash_exists !== 0) stopFindings.push('duplicate_active_identity_hash_after_insert');

    const report = {
      version: 'ENRICH03B_ACTIVE_IDENTITY_GUARDED_DRY_RUN_V1',
      package_id: PACKAGE_ID,
      ...
      recommended_approval_text: stopFindings.length === 0
        ? `Approve real ${PACKAGE_ID} apply only. Fingerprint: ${packageFingerprint}. Scope: ${targets.length} active card_print_identity inserts from public.card_print_identity_backfill_projection_v1. Dry-run proof: ${execution.before_snapshot.hash_sha256} == ${execution.after_rollback_snapshot.hash_sha256}. No parent writes. No child writes. No deletes. No merges. No migrations. No image writes. No global apply.`
        : null,
    };
    ...
    const md = [
      '# ENRICH-03B Active Identity Guarded Dry Run V1',
      '',
      `Package: \`${PACKAGE_ID}\``,
      '',
      '## Result',

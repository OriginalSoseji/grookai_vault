...
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'retailer_stamp_active_finish_route_v1', 'retailer_stamp_xy_parent_insert_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'retailer_stamp_active_finish_route_v1', 'retailer_stamp_xy_parent_insert_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'retailer_stamp_active_finish_route_v1', 'retailer_stamp_xy_parent_insert_real_apply_v1.md');

const PACKAGE_ID = 'RETAILER-STAMP-03-XY-PARENT-IDENTITY-INSERTS';
const CREATED_BY = 'retailer_stamp_xy_parent_insert_real_apply_v1';
const EXPECTED_PACKAGE_FINGERPRINT = '103681d2519e9bf317e49388b26934fdf17b9a15f507dad7bf44ae73d68d96d0';
const EXPECTED_DRY_RUN_PROOF = 'ebe230af9770a266c2e6821e751cc0ff764da26e323ca34d8dd49314f1ec2dfe';
const EXPECTED_PRE_APPLY_HASH = '40ed64e90c439ab5380aec16faf6814b7a5df102a7bdf82dd103f85542ce949b';
...
function validateDryRun(dryRun) {
  ...
  if (dryRun.execution?.dry_run_status !== 'retailer_stamp_xy_parent_insert_completed_rolled_back_no_durable_change') findings.push('dry_run_status_not_passed');
...
async function applyPackage(client, targets, expectedMissingBaseFinishCount) {
  ...
  try {
    ...
    return {
      apply_status: 'retailer_stamp_xy_parent_insert_applied',
      guard: guardRow,
      proof: proof.rows[0],
...
function renderMarkdown(report) {
  ...
  return `# Retailer Stamp XY Parent Insert Real Apply V1

Real apply for the approved remaining XY-era Build-A-Bear Workshop and Toys R Us stamped parent identity insert package.
...
async function main() {
  ...
  const report = {
    ...
    version: 'retailer_stamp_xy_parent_insert_real_apply_v1',

...
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'final_source_ready_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'final_source_ready_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'final_source_ready_real_apply_v1.md');

const PACKAGE_ID = 'SPECIAL-VAR-06-FINAL-SOURCE-READY-PARENT-INSERTS';
const CREATED_BY = 'english_master_index_special_variant_final_source_ready_real_apply_v1';
const EXPECTED_PACKAGE_FINGERPRINT = '74ba5eeb13f6418db5ccfe71c53f2bddb92eadaaa87503341b267fce991825d3';
const EXPECTED_SQL_HASH = 'b215ed71141a18e174dc36eb1624a669a21b3fac0990d6fc7848173deef33e33';
const EXPECTED_DRY_RUN_PROOF = '9e9e8c8d47eefa19cad4ef9f82c59ba9ac55da5209ccf50d80dadfa783974871';
const EXPECTED_PRE_APPLY_HASH = '679968ce5317ac491bcb483172963c8509cc7ae418bcaa5d830099f9b306ffb0';
...
function validateDryRun(dryRun) {
  ...
  if (dryRun.execution?.dry_run_status !== 'final_source_ready_parent_insert_completed_rolled_back_no_durable_change') findings.push('dry_run_status_not_passed');
  ...
  if (dryRun.scope?.by_set?.base1 !== 1 || dryRun.scope?.by_set?.basep !== 2) findings.push('set_scope_mismatch');
  if (dryRun.scope?.by_finish?.normal !== 2 || dryRun.scope?.by_finish?.holo !== 1) findings.push('finish_scope_mismatch');
  ...
  for (const row of dryRun.scope?.targets ?? []) {
    if (!['base1', 'basep'].includes(row.set_key)) findings.push(`target_set_mismatch:${row.source_candidate_key}`);
...
async function applyPackage(client, targets) {
  ...
  try {
    ...
    return {
      apply_status: 'final_source_ready_parent_insert_applied',
      guard: guardRow,
      proof: proofRow,
...
function renderMarkdown(report) {
  return `# Final Source Ready Variants Real Apply V1

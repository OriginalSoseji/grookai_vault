...
const ADJUDICATION_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg15b_stamped_generic_variant_adjudication_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg15c_stamped_reverse_burger_king_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg15c_stamped_reverse_burger_king_guarded_dry_run_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260611_pkg15c_stamped_reverse_burger_king_guarded_dry_run_checkpoint_v1.md');
...
const CREATED_BY = 'pkg15c_stamped_reverse_burger_king_guarded_dry_run_v1';
...
async function runDryRun(client, targets, fingerprint) {
  ...
  try {
    ...
    return {
      dry_run_status: 'pkg15c_stamped_reverse_burger_king_failed_rolled_back',
...
function renderMarkdown(report) {
  ...
  return `# PKG-15C Stamped Reverse Burger King Guarded Dry Run V1
...
function checkpointMarkdown(report) {
  return `# PKG-15C Stamped Reverse Burger King Guarded Dry Run Checkpoint V1
...
function updateCheckpointIndex() {
  const line = '| 2026-06-11 | [PKG-15C Stamped Reverse Burger King Guarded Dry Run Checkpoint V1](20260611_pkg15c_stamped_reverse_burger_king_guarded_dry_run_checkpoint_v1.md) | Rollback-only dry-run for 8 deterministic Burger King stamped identity parent inserts with reverse child printings. No durable writes or migrations. |';
  const current = fsSync.existsSync(CHECKPOINT_INDEX) ? fsSync.readFileSync(CHECKPOINT_INDEX, 'utf8') : '# Master Index Checkpoints\n';
  if (current.includes('20260611_pkg15c_stamped_reverse_burger_king_guarded_dry_run_checkpoint_v1.md')) {
    fsSync.writeFileSync(CHECKPOINT_INDEX, current.split('\n').map((existingLine) => (
      existingLine.includes('20260611_pkg15c_stamped_reverse_burger_king_guarded_dry_run_checkpoint_v1.md') ? line : existingLine
    )).join('\n'));
  } else {
...
const report = {
  ...
  version: 'english_master_index_pkg15c_stamped_reverse_burger_king_guarded_dry_run_v1',
  package_id: PACKAGE_ID,

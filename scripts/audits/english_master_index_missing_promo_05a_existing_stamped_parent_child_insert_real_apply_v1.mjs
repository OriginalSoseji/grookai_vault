...
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'existing_stamped_parent_child_insert_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'existing_stamped_parent_child_insert_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'existing_stamped_parent_child_insert_real_apply_v1.md');

const PACKAGE_ID = 'MISSING-PROMO-05A-EXISTING-STAMPED-PARENT-FINISH-CHILD-INSERTS';
const CREATED_BY = 'english_master_index_missing_promo_05a_existing_stamped_parent_child_insert_real_apply_v1';
const APPROVED_FINGERPRINT = 'f47b3153e46da5ee80c1ca2e72607c214cb5ee9b6c20afdb94790253b01112f2';
const APPROVED_SQL_HASH = 'b58b60555914d267cde158ef3da39358078995e0d8d1f9a77ebb82afe008f12f';
const APPROVED_DRY_RUN_PROOF = '5cb4beb4de2f2697ca867e91e412336f71b4dc280c816eb32e722c24c53e82a3';
...
function assertApproved(dryRun) {
  ...
  if (dryRun.summary?.target_count !== 9) errors.push('target count mismatch');
  if (dryRun.summary?.by_finish?.reverse !== 9) errors.push('finish scope mismatch');
  ...
  if (dryRun.guard?.printing_gv_id_collision_count !== 0) errors.push('dry-run guard had printing GV-ID collision');
...
async function runApply(client, targets) {
  ...
  try {
    ...
    return { beforeSnapshot, afterSnapshot, guard: guardRow, inserted_rows: insertResult.rows };
  } catch (error) {
    try {
...
function renderMarkdown(report) {
  return [
    '# Existing Stamped Parent Child Insert Real Apply V1',
    '',
    'Approved real apply for child printings on existing stamped parents.',
    '',
    markdownTable(
      ['metric', 'value'],
      [
...
async function main() {
  ...
  try {
    const result = await runApply(client, targets);
    const proof = sha256(stableJson({
      package_id: PACKAGE_ID,
      package_fingerprint_sha256: dryRun.package_fingerprint_sha256,
      inserted_rows: result.inserted_rows,
      guard: result.guard,
      after_counts: result.afterSnapshot.counts,
    }));
    const matchingRows = result.afterSnapshot.rows.filter((row) => row.row_type === 'target_child' && targets.some((target) => target.target_child_id === row.row_id));
    const report = {
      package_id: PACKAGE_ID,
      ...
      input_artifacts: {
        guarded_dry_run: rel(DRY_RUN_JSON),
      },
      output_json: rel(OUTPUT_JSON),
      output_md: rel(OUTPUT_MD),
      summary: {
        target_count: targets.length,
        inserted_child_rows: result.inserted_rows.length,

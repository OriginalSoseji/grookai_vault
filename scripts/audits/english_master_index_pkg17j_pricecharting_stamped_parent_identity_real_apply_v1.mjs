...
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg17j_pricecharting_stamped_parent_identity_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg17j_pricecharting_stamped_parent_identity_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg17j_pricecharting_stamped_parent_identity_real_apply_v1.md');

const PACKAGE_ID = 'PKG-17J-PRICECHARTING-STAMPED-PARENT-IDENTITY-CANDIDATES';
const CREATED_BY = 'pkg17j_pricecharting_stamped_parent_identity_real_apply_v1';
const EXPECTED_PACKAGE_FINGERPRINT = '07b6829f296dcf8056eca134e9e9be9a476a7fc0c5b59076d77da8443127b990';
const EXPECTED_DRY_RUN_PROOF = 'ff6edbbe4d0130e84dd40c68e9bd6090156c29a8036cd428b1ff8e06840d3a37';
const EXPECTED_PRE_APPLY_HASH = '35adc4d3547641e577432d30e230221006e404b5d727779de727f2dbbe4bb9fb';
const EXPECTED_TARGET_COUNT = 9;
const APPROVAL_TEXT = 'Approve real PKG-17J-PRICECHARTING-STAMPED-PARENT-IDENTITY-CANDIDATES apply only. Fingerprint: 07b6829f296dcf8056eca134e9e9be9a476a7fc0c5b59076d77da8443127b990. Scope: 9 stamped parent inserts, 9 identity inserts, 9 child printing inserts; finishes holo=4, normal=3, reverse=2; stamp labels Prize Pack Stamp=4, Winner Stamp=4, Professor Program Stamp=1; sets bog=4, sv02=1, sv05=1, swsh12=1, swsh7=1, swshp=1. Dry-run proof: 35adc4d3547641e577432d30e230221006e404b5d727779de727f2dbbe4bb9fb == 35adc4d3547641e577432d30e230221006e404b5d727779de727f2dbbe4bb9fb. No global apply. No migrations. No deletes. No merges. No unsupported cleanup.';
...
function validateDryRun(dryRun) {
  ...
  if (dryRun.execution?.dry_run_status !== 'pkg17j_completed_rolled_back_no_durable_change') findings.push('dry_run_status_not_passed');
...
async function applyPackage(client, targets, fingerprint) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg17j_targets (
    ...
    const guard = await client.query(
      `with projection as (
         ...
         from pkg17j_targets target
         join public.card_prints base on base.id = target.base_parent_id
         left join public.sets s on s.id = base.set_id
       )
       select
         (select count(*)::int from pkg17j_targets) as target_count,
         (select count(distinct target_parent_id)::int from pkg17j_targets) as target_parent_count,
         (select count(distinct target_child_id)::int from pkg17j_targets) as target_child_count,
         (select count(*)::int from pkg17j_targets target left join public.card_prints base on base.id = target.base_parent_id where base.id is null) as missing_base_count,
         (select count(*)::int from pkg17j_targets target left join public.finish_keys fk on fk.key = target.target_finish_key and fk.is_active = true where fk.key is null) as inactive_finish_count,
         (select count(*)::int
          from pkg17j_targets target
          join public.card_prints base on base.id = target.base_parent_id
         ...
         (select count(*)::int from pkg17j_targets target join public.card_printings cpr on cpr.id = target.target_child_id) as child_collision_count,
         (select count(*)::int from pkg17j_targets target join public.card_print_identity cpi on cpi.card_print_id = target.target_parent_id and cpi.is_active = true) as identity_target_collision_count,
    ...
    const childInsert = await client.query(
      `insert into public.card_printings (
       ...
       select
         ...
         concat(target.set_key, ':', target.card_number, ':pricecharting_stamp_label:', target.target_variant_key, ':', target.target_finish_key),
         $1::text,
         null, null, null, null, null,
         'representative_shared_stamp',
         concat('Stamped identity child finish routed from PriceCharting exact stamp label candidate: ', target.target_finish_key)
       from pkg17j_targets target`,
      [CREATED_BY],
    );
    ...
    const proof = await client.query(
      `select
         ...
         (select count(*)::int from pkg17j_targets) as target_rows,
         (select count(*)::int from public.card_prints cp join pkg17j_targets target on target.target_parent_id = cp.id) as inserted_parent_rows,
         (select count(*)::int from public.card_prints cp join pkg17j_targets target on target.target_parent_id = cp.id and cp.printed_identity_modifier = target.target_printed_identity_modifier) as inserted_parent_modifier_rows,
         (select count(*)::int from public.card_print_identity cpi join pkg17j_targets target on target.target_parent_id = cpi.card_print_id and cpi.is_active = true) as inserted_identity_rows,
         (select count(*)::int from public.card_printings cpr join pkg17j_targets target on target.target_child_id = cpr.id) as inserted_child_rows,
         (select count(*)::int from public.card_printings cpr join pkg17j_targets target on target.target_child_id = cpr.id and cpr.finish_key = 'stamped') as forbidden_stamped_child_rows`,
      [PACKAGE_ID, fingerprint],
    );
    await client.query('commit');
    const afterSnapshot = await captureSnapshot(client, targets);
    return {
      apply_status: 'pkg17j_real_apply_committed',
      guard: guardRow,
      proof: proof.rows[0],
      write_counts: {
...
function renderMarkdown(report) {
  ...
  return `# PKG-17J PriceCharting Stamped Parent Identity Real Apply V1

Approved real apply for stamped parent identity inserts whose exact stamp label came from PriceCharting CSV and whose active child finish already passed readiness.
...
async function main() {
  ...
  const report = {
    ...
    version: 'english_master_index_pkg17j_pricecharting_stamped_parent_identity_real_apply_v1',
    package_id: PACKAGE_ID,

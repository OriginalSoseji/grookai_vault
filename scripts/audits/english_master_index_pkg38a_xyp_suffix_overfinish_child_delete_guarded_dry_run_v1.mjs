...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg38a_xyp_suffix_overfinish_child_delete_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg38a_xyp_suffix_overfinish_child_delete_guarded_dry_run_v1.md');
...
function isAllowedRedundantVariant(row) {
  const variant = String(row.variant_key ?? '').trim().toLowerCase();
  const number = String(row.number ?? row.card_number ?? '').trim();
  return variant === '' || variant === 'base' || (variant === 'xya' && /^XY\d+a$/i.test(number));
}
...
function buildTargets(source) {
  ...
  for (const row of source.rows ?? []) {
    ...
    const allowedShape = row.governance_bucket === TARGET_BUCKET
      ...
      && row.canonical_set_key === 'xyp'
      && TARGET_NUMBERS.has(String(row.number ?? row.card_number ?? '').toLowerCase())
      && row.card_printing_id
      && row.card_print_id
      && dependencyTotal === 0
      && isAllowedRedundantVariant(row)
      && modifier === ''
      && ALLOWED_FINISHES.has(row.finish_key)
      && (row.known_index_finishes ?? []).includes('holo')
    ...
    if (!allowedShape || seen.has(row.card_printing_id)) {
      blocked.push({
        ...
        blocked_reason: seen.has(row.card_printing_id)
          ...
          : 'target_shape_not_allowed_for_pkg38a',
      });
      continue;
    }
...
async function runDryRun(client, targets) {
  ...
  try {
    ...
    if (
      ...
      throw new Error(`PKG-38A guard failed: ${JSON.stringify(guard)}`);
    }
    ...
    if (deleteResult.rowCount !== targets.length) {
      throw new Error(`PKG-38A delete simulation mismatch: ${deleteResult.rowCount}`);
    }
  ...
  return {
    dry_run_status: 'pkg38a_guarded_dry_run_passed_rolled_back_no_durable_change',
    before_snapshot: beforeSnapshot,
...
function renderMarkdown(report) {
  return `# PKG-38A XYP Suffix Overfinish Child Delete Guarded Dry Run V1
...
if (targets.length !== 10) throw new Error(`PKG-38A expected 10 targets, found ${targets.length}`);
...
const report = {
  ...
  version: 'english_master_index_pkg38a_xyp_suffix_overfinish_child_delete_guarded_dry_run_v1',
  package_id: PACKAGE_ID,
...
report.recommended_real_apply_approval_text = `Approve real ${PACKAGE_ID} apply only. Fingerprint: ${packageFingerprint}. Scope: ${targets.length} XYP suffix overfinish child deletes; finishes ${Object.entries(report.scope.by_finish).map(([finish, count]) => `${finish}=${count}`).join(', ')}; target rows XY150a/XY177a/XY198a/XY200a/XY67a normal/reverse only. Dry-run proof: ${execution.before_snapshot.hash_sha256} == ${execution.after_snapshot.hash_sha256}. No global apply. No migrations. No parent writes. No merges. No quarantine. Holo rows preserved.`;

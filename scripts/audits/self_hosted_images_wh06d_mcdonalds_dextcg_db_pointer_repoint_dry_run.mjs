...
const UPLOAD_RESULT_JSONL = path.join(OUTPUT_DIR, 'self_hosted_images_wh06c_mcdonalds_dextcg_storage_upload_apply_result_v1.jsonl');
const PLAN_JSONL = path.join(OUTPUT_DIR, 'self_hosted_images_wh06d_mcdonalds_dextcg_db_pointer_repoint_plan_v1.jsonl');
const SUMMARY_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh06d_mcdonalds_dextcg_db_pointer_repoint_dry_run_summary_v1.json');
const SUMMARY_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh06d_mcdonalds_dextcg_db_pointer_repoint_dry_run_summary_v1.md');
const PACKAGE_ID = 'IMG-HOST-WH-06D-MCDONALDS-DEXTCG-DB-POINTER-REPOINT-DRY-RUN';
const SOURCE_UPLOAD_PACKAGE_ID = 'IMG-HOST-WH-06C-MCDONALDS-DEXTCG-STORAGE-UPLOAD-APPLY';
const SOURCE_UPLOAD_PROOF_HASH = '86515052fed84d61fe96774c7a80c2e8cdff4e9bf06b7c1ccb81a3ef363c7bba';
...
function proposedImageNote(row) {
  return clean(row.proposed_image_note)
    ?? `Self-hosted McDonald's image uploaded by ${SOURCE_UPLOAD_PACKAGE_ID}; exact image claim is not changed.`;
}
...
async function main() {
  ...
  const unsupportedRows = completedManifestRows.filter((row) =>
    !row.card_print_id ||
    !ALLOWED_SET_CODES.has(String(row.set_code ?? '')) ||
    row.source_lane !== 'external_dextcg' ||
    row.proposed_image_source !== 'identity' ||
    !ALLOWED_STATUSES.has(String(row.proposed_image_status ?? '')) ||

...
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'enrich03_active_identity_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'enrich03_active_identity_guarded_dry_run_v1.md');
...
async function main() {
  ...
  try {
    ...
    if (targets.length !== 11468) stop_findings.push(`target_count_drift:${targets.length}`);
    ...
    const report = {
      version: 'ENRICH03_ACTIVE_IDENTITY_GUARDED_DRY_RUN_V1',
      package_id: PACKAGE_ID,
    ...
    const md = [
      '# ENRICH-03 Active Identity Guarded Dry Run V1',
      '',
      `Package: \`${PACKAGE_ID}\``,
      '',
      '## Result',

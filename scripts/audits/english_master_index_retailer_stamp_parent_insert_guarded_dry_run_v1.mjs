...
const READINESS_JSON = path.join(
  ...
  'retailer_stamp_active_finish_route_v1.json',
);
...
const OUTPUT_MD = path.join(
  AUDIT_DIR,
  'retailer_stamp_active_finish_route_v1',
  'retailer_stamp_parent_insert_guarded_dry_run_v1.md',
);

const PACKAGE_ID = 'RETAILER-STAMP-02-PARENT-IDENTITY-INSERTS';
const CREATED_BY = 'retailer_stamp_parent_insert_guarded_dry_run_v1';
...
async function runDryRun(client, targets, fingerprint, expectedMissingBaseFinishCount) {
  ...
  try {
    ...
    return {
      dry_run_status: 'retailer_stamp_parent_insert_failed_rolled_back',
...
function renderMarkdown(report) {
  ...
  return `# Retailer Stamp Parent Insert Guarded Dry Run V1
...
async function main() {
  ...
  const sourceUniqueActiveFinishCount = targets.filter((row) => !row.base_parent_child_finishes.includes(row.target_finish_key)).length;
  const recommended = execution.dry_run_status === 'retailer_stamp_parent_insert_completed_rolled_back_no_durable_change'
    && execution.rollback_verified
    && execution.stop_findings.length === 0
    ? `Approve real ${PACKAGE_ID} apply only. Fingerprint: ${fingerprint}. Scope: ${targets.length} retailer-stamped parent inserts, ${targets.length} identity inserts, ${targets.length} child printing inserts; finishes ${Object.entries(byFinish).map(([finish, count]) => `${finish}=${count}`).join(', ')}; stamp labels ${Object.entries(byStampLabel).map(([label, count]) => `${label}=${count}`).join(', ')}; sets ${Object.entries(bySet).map(([set, count]) => `${set}=${count}`).join(', ')}; source-unique active-finish rows: ${sourceUniqueActiveFinishCount}. Dry-run proof: ${execution.before_snapshot.hash_sha256} == ${execution.after_rollback_snapshot.hash_sha256}. Excluded blocked base-parent rows: 5. No global apply. No migrations. No deletes. No merges. No unsupported cleanup.`
    : 'Not approval-ready; dry-run did not pass cleanly.';

  const report = {
    generated_at: new Date().toISOString(),
    version: 'retailer_stamp_parent_insert_guarded_dry_run_v1',
    package_id: PACKAGE_ID,
    ...
    scope: {
      ...
      excluded_blocked_base_parent_rows: 5,

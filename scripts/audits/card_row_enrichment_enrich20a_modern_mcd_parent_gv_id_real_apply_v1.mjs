...
const DRY_RUN_JSON = path.join(OUTPUT_DIR, 'enrich20a_modern_mcd_parent_gv_id_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'enrich20a_modern_mcd_parent_gv_id_real_apply_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'enrich20a_modern_mcd_parent_gv_id_real_apply_v1.md');
const PACKAGE_ID = 'ENRICH-20A-MODERN-MCD-PARENT-GV-ID-BACKFILL';
const TARGET_SET_CODES = ['2023sv', '2024sv'];
const EXPECTED_FINGERPRINT = 'f75a35c1261f98193185f842ad1ab148c93e2c930757833fe491c0db727a4271';
const EXPECTED_DRY_RUN_PROOF = '7c6a1d6573960b09ad9f963779786115dfcaf2e8404fb31c86d471c10e266a35';
const EXPECTED_TARGET_ROWS = 30;
const APPROVAL_TEXT = 'Approve real ENRICH-20A-MODERN-MCD-PARENT-GV-ID-BACKFILL apply only. Fingerprint: f75a35c1261f98193185f842ad1ab148c93e2c930757833fe491c0db727a4271. Scope: 30 modern McDonald\'s parent card_print gv_id updates using governed MCD year namespaces 2023/2024. Dry-run proof: 7c6a1d6573960b09ad9f963779786115dfcaf2e8404fb31c86d471c10e266a35 == 7c6a1d6573960b09ad9f963779786115dfcaf2e8404fb31c86d471c10e266a35. No child writes. No identity writes. No external mapping writes. No species writes. No deletes. No merges. No migrations. No image writes. No global apply.';
  ...
  const validation = await client.query(
    `with target as (
     ...
     select
       ...
       (select count(*)::int from target where set_code = '2023sv' and proposed_gv_id not like 'GV-PK-MCD-2023-%') as bad_2023_namespace_count,
       (select count(*)::int from target where set_code = '2024sv' and proposed_gv_id not like 'GV-PK-MCD-2024-%') as bad_2024_namespace_count,
       ...
       (select count(*)::int
        ...
        where cp.set_code <> all($2::text[])
           ...
           or s.name not like 'McDonald''s Collection 20%') as out_of_scope_parent_count,
...
function validateDryRun(dryRun, fingerprint) {
  ...
  if (dryRun.package_fingerprint_sha256 !== fingerprint) findings.push('package_fingerprint_mismatch');
...
async function applyPackage(client, targets) {
  ...
  try {
    ...
    if (
      ...
      preflight.bad_2023_namespace_count !== 0 ||
      preflight.bad_2024_namespace_count !== 0 ||
    ...
    await client.query(
      `create temporary table enrich20a_targets (
    ...
    const proof = await client.query(
      `select
         (select count(*)::int from enrich20a_targets) as target_count,
         (select count(*)::int
          from enrich20a_targets target
          join public.card_prints cp on cp.id = target.card_print_id
...
async function main() {
  ...
  try {
    ...
    if (targets.length !== EXPECTED_TARGET_ROWS) targetFindings.push(`target_count_drift:${targets.length}`);
    const stopFindings = [...dryRunFindings, ...approvalFindings, ...targetFindings];
    ...
    const report = {
      version: 'ENRICH20A_MODERN_MCD_PARENT_GV_ID_REAL_APPLY_V1',
      package_id: PACKAGE_ID,
      ...
      target_validation_findings: targetFindings,
    ...
    await writeText(OUTPUT_MD, [
      '# ENRICH-20A Modern McDonald Parent GV-ID Real Apply V1',
      '',
      `Package: \`${PACKAGE_ID}\``,
      '',
      '## Result',

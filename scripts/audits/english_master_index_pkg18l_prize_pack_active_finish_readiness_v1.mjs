...
const SOURCE_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg18d_prize_pack_finish_mapping_closure_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg18l_prize_pack_active_finish_readiness_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg18l_prize_pack_active_finish_readiness_v1.md');

const PACKAGE_ID = 'PKG-18L-PRIZE-PACK-ACTIVE-FINISH-READINESS';
const FUTURE_PACKAGE_ID = 'PKG-18M-PRIZE-PACK-STAMPED-PARENT-INSERTS';
...
function sourceCandidates(sourceReport) {
  return (sourceReport.rows ?? [])
    .filter((row) => row.closure_status === 'ready_for_guarded_dry_run')
    .map((row) => {
      ...
      const pricechartingMatch = row.pricecharting_matches?.[0] ?? {};
      return {
        set_key: row.set_key,
        ...
        source_url: pricechartingMatch.source_url ?? null,
        evidence: {
          source_package_id: sourceReport.package_id,
          source_fingerprint_sha256: sourceReport.fingerprint_sha256,
          source_status: row.closure_status,
          source_url: pricechartingMatch.source_url ?? null,
          evidence_label: pricechartingMatch.evidence_label ?? 'PriceCharting exact Prize Pack product title corroborates active finish',
          product_name: pricechartingMatch.product_name ?? null,
          console_name: pricechartingMatch.console_name ?? null,
          stamped_variant_key: variantKey,
          stamp_label: row.stamp_label,
          active_child_finish_key: finishKey,
          caveat: 'PriceCharting corroborates exact Prize Pack product identity and active finish; original printed set and card identity come from existing Master Index evidence.',
          source_evidence: row.evidence ?? [],
          pricecharting_matches: row.pricecharting_matches ?? [],
        },
      };
    })
...
function renderMarkdown(report) {
  ...
  return `# PKG-18L Prize Pack Active Finish Readiness V1

Read-only DB readiness view for Prize Pack stamped parent identity inserts.
...
async function main() {
  ...
  const report = {
    ...
    version: 'english_master_index_pkg18l_prize_pack_active_finish_readiness_v1',

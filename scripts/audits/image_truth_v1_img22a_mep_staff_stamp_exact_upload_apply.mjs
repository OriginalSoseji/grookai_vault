import fs from 'node:fs/promises';
import path from 'node:path';
import crypto from 'node:crypto';
import process from 'node:process';
import { execFile } from 'node:child_process';
import { promisify } from 'node:util';
import pg from 'pg';
import dotenv from 'dotenv';
import { createClient } from '@supabase/supabase-js';

dotenv.config({ path: '.env.local', quiet: true });
dotenv.config({ quiet: true });

const { Client } = pg;
const execFileAsync = promisify(execFile);

const OUTPUT_DIR = 'docs/audits/image_truth_v1';
const PLAN_JSON = path.join(OUTPUT_DIR, 'image_truth_img22a_mep_staff_stamp_exact_upload_apply_plan_v1.json');
const PLAN_MD = path.join(OUTPUT_DIR, 'image_truth_img22a_mep_staff_stamp_exact_upload_apply_plan_v1.md');
const RESULT_JSON = path.join(OUTPUT_DIR, 'image_truth_img22a_mep_staff_stamp_exact_upload_apply_result_v1.json');
const RESULT_MD = path.join(OUTPUT_DIR, 'image_truth_img22a_mep_staff_stamp_exact_upload_apply_result_v1.md');
const STAGING_ASSET_DIR = path.join('tmp', 'nonproduction_image_staging', 'image_truth_v1', 'img22a');
const STORAGE_BUCKET = 'user-card-images';
const PACKAGE_ID = 'IMG-22A-MEP-STAFF-STAMP-EXACT-CHILD-IMAGE-UPLOAD-APPLY';
const TCGCSV_GROUP_PRODUCTS_URL = 'https://tcgcsv.com/tcgplayer/3/24451/products';
const TCGCSV_GROUP_PRICES_URL = 'https://tcgcsv.com/tcgplayer/3/24451/prices';

const TARGETS = [
  {
    product_id: 685498,
    set_code: 'mep',
    number: '064',
    card_name: 'Serperior',
    finish_key: 'holo',
    variant_key: 'staff_stamp',
    printing_gv_id: 'GV-PK-MEP-064-STAFF-STAMP-HOLO',
    product_name: 'Serperior - 064 [Staff]',
    source_url: 'https://www.tcgplayer.com/product/685498/pokemon-me-mega-evolution-promo-serperior-064-staff',
    source_image_url: 'https://product-images.tcgplayer.com/685498.jpg',
    tcgcsv_image_url: 'https://tcgplayer-cdn.tcgplayer.com/product/685498_200w.jpg',
  },
  {
    product_id: 685501,
    set_code: 'mep',
    number: '067',
    card_name: 'Doublade',
    finish_key: 'holo',
    variant_key: 'staff_stamp',
    printing_gv_id: 'GV-PK-MEP-067-STAFF-STAMP-HOLO',
    product_name: 'Doublade - 067 [Staff]',
    source_url: 'https://www.tcgplayer.com/product/685501/pokemon-me-mega-evolution-promo-doublade-067-staff',
    source_image_url: 'https://product-images.tcgplayer.com/685501.jpg',
    tcgcsv_image_url: 'https://tcgplayer-cdn.tcgplayer.com/product/685501_200w.jpg',
  },
];

function parseArgs(argv) {
  const args = { apply: false, fingerprint: null };
  for (let index = 2; index < argv.length; index += 1) {
    const arg = argv[index];
    if (arg === '--apply') args.apply = true;
    else if (arg === '--fingerprint') args.fingerprint = argv[++index] ?? null;
    else throw new Error(`Unknown argument: ${arg}`);
  }
  return args;
}

function requireDbUrl() {
  return process.env.SUPABASE_DB_URL
    ?? process.env.DATABASE_URL
    ?? process.env.POSTGRES_URL
    ?? process.env.POSTGRES_PRISMA_URL
    ?? null;
}

function createStorageClient() {
  if (!process.env.SUPABASE_URL) throw new Error('Missing SUPABASE_URL.');
  if (!process.env.SUPABASE_SECRET_KEY) throw new Error('Missing SUPABASE_SECRET_KEY.');
  return createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SECRET_KEY, { auth: { persistSession: false } });
}

function clean(value) {
  const normalized = String(value ?? '').trim();
  return normalized.length > 0 ? normalized : null;
}

function normalizeText(value) {
  return String(value ?? '')
    .trim()
    .toLowerCase()
    .replace(/&/g, 'and')
    .replace(/['']/g, '')
    .replace(/[^a-z0-9]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}

function normalizeNumber(value) {
  return String(value ?? '').trim().toLowerCase().replace(/^0+(?=\d)/, '');
}

function sha256Hex(bufferOrText) {
  return crypto.createHash('sha256').update(bufferOrText).digest('hex');
}

function canonicalizeJson(value) {
  if (Array.isArray(value)) return value.map((entry) => canonicalizeJson(entry));
  if (!value || typeof value !== 'object') return value;
  return Object.keys(value).sort((left, right) => left.localeCompare(right)).reduce((acc, key) => {
    acc[key] = canonicalizeJson(value[key]);
    return acc;
  }, {});
}

function proofHash(value) {
  return sha256Hex(JSON.stringify(canonicalizeJson(value)));
}

async function fetchJson(url) {
  const { stdout } = await execFileAsync('curl.exe', [
    '--ssl-no-revoke',
    '--silent',
    '--show-error',
    '--location',
    '--max-time',
    '120',
    '--user-agent',
    'Grookai Image Truth Audit/1.0',
    url,
  ], { timeout: 140000, maxBuffer: 80 * 1024 * 1024 });
  return JSON.parse(stdout);
}

async function fetchBuffer(url) {
  const result = await execFileAsync('curl.exe', [
    '--ssl-no-revoke',
    '--location',
    '--silent',
    '--show-error',
    '--max-time',
    '60',
    '--user-agent',
    'Grookai Image Truth Audit/1.0',
    url,
  ], {
    encoding: 'buffer',
    timeout: 80000,
    maxBuffer: 12 * 1024 * 1024,
  });
  return result.stdout;
}

function extendedValue(product, name) {
  return product.extendedData?.find((entry) => entry.name === name || entry.displayName === name)?.value ?? null;
}

function productMatchesTarget(product, target) {
  return Number(product.productId) === target.product_id
    && normalizeText(product.name) === normalizeText(target.product_name)
    && normalizeNumber(extendedValue(product, 'Number')) === normalizeNumber(target.number)
    && clean(product.url) === target.source_url
    && clean(product.imageUrl) === target.tcgcsv_image_url;
}

function priceMatchesTarget(prices, target) {
  return prices.some((price) => {
    return Number(price.productId) === target.product_id
      && normalizeText(price.subTypeName) === 'holofoil';
  });
}

async function stageAsset(target) {
  const buffer = await fetchBuffer(target.source_image_url);
  const sha = sha256Hex(buffer);
  const fileName = `${target.set_code}_${normalizeNumber(target.number)}_${target.variant_key}_${target.finish_key}_${sha.slice(0, 16)}.jpg`;
  const localPath = path.join(STAGING_ASSET_DIR, fileName);
  await fs.mkdir(STAGING_ASSET_DIR, { recursive: true });
  await fs.writeFile(localPath, buffer);
  const storagePath = [
    'warehouse-derived',
    'image-truth-v1',
    'img22a-mep-staff-stamp-exact',
    target.set_code,
    target.printing_gv_id.toLowerCase(),
    `${sha.slice(0, 24)}.jpg`,
  ].join('/');

  return {
    source_image_url: target.source_image_url,
    tcgcsv_image_url: target.tcgcsv_image_url,
    local_nonproduction_asset_path: localPath.replaceAll('\\', '/'),
    normalized_sha256: sha,
    normalized_size_bytes: buffer.length,
    proposed_storage_path: storagePath,
  };
}

async function storageObjectExists(supabase, storagePath) {
  const slashIndex = storagePath.lastIndexOf('/');
  const folder = slashIndex >= 0 ? storagePath.slice(0, slashIndex) : '';
  const fileName = slashIndex >= 0 ? storagePath.slice(slashIndex + 1) : storagePath;
  const { data, error } = await supabase.storage.from(STORAGE_BUCKET).list(folder, { limit: 100, search: fileName });
  if (error) throw new Error(`storage_probe_failed:${storagePath}:${error.message}`);
  return (data ?? []).some((entry) => entry.name === fileName);
}

async function fetchTargetRow(client, target) {
  const result = await client.query(
    `
      select
        cpi.id as card_printing_id,
        cpi.card_print_id,
        cpi.printing_gv_id,
        cpi.finish_key,
        cpi.image_source,
        cpi.image_path,
        cpi.image_url,
        cpi.image_alt_url,
        cpi.image_status,
        cpi.image_note,
        cp.gv_id as parent_gv_id,
        cp.set_code,
        cp.number,
        cp.name as card_name,
        cp.variant_key,
        cp.printed_identity_modifier,
        cp.image_source as parent_image_source,
        cp.image_path as parent_image_path,
        cp.image_url as parent_image_url,
        cp.image_alt_url as parent_image_alt_url,
        cp.representative_image_url as parent_representative_image_url,
        cp.image_status as parent_image_status,
        cp.image_note as parent_image_note
      from public.card_printings cpi
      join public.card_prints cp on cp.id = cpi.card_print_id
      where cpi.printing_gv_id = $1
      limit 1
    `,
    [target.printing_gv_id],
  );
  return result.rows[0] ?? null;
}

function parentSnapshot(row) {
  return {
    parent_gv_id: row?.parent_gv_id ?? null,
    set_code: row?.set_code ?? null,
    number: row?.number ?? null,
    card_name: row?.card_name ?? null,
    variant_key: row?.variant_key ?? null,
    printed_identity_modifier: clean(row?.printed_identity_modifier),
    parent_image_source: clean(row?.parent_image_source),
    parent_image_path: clean(row?.parent_image_path),
    parent_image_url: clean(row?.parent_image_url),
    parent_image_alt_url: clean(row?.parent_image_alt_url),
    parent_representative_image_url: clean(row?.parent_representative_image_url),
    parent_image_status: clean(row?.parent_image_status),
    parent_image_note: clean(row?.parent_image_note),
  };
}

function validateTargetRow(target, row) {
  const errors = [];
  if (!row) return ['target_card_printing_not_found'];
  if (normalizeText(row.set_code) !== normalizeText(target.set_code)) errors.push('set_code_mismatch');
  if (normalizeNumber(row.number) !== normalizeNumber(target.number)) errors.push('number_mismatch');
  if (normalizeText(row.card_name) !== normalizeText(target.card_name)) errors.push('card_name_mismatch');
  if (row.variant_key !== target.variant_key) errors.push('variant_key_mismatch');
  if (row.printing_gv_id !== target.printing_gv_id) errors.push('printing_gv_id_mismatch');
  if (row.finish_key !== target.finish_key) errors.push('finish_key_mismatch');
  if (clean(row.image_path)) errors.push('child_image_path_already_present');
  if (clean(row.image_url)) errors.push('child_image_url_already_present');
  if (clean(row.image_alt_url)) errors.push('child_image_alt_url_already_present');
  return errors;
}

function imageNote(target) {
  return [
    `${PACKAGE_ID}:tcgcsv_staff_product:${target.source_url}`,
    `tcgcsv_image:${target.tcgcsv_image_url}`,
    'manual_visual_review:staff_stamp_visible',
  ].join('; ');
}

function markdownTable(rows, columns) {
  if (rows.length === 0) return '_None._';
  const header = `| ${columns.map((column) => column.label).join(' | ')} |`;
  const divider = `| ${columns.map(() => '---').join(' | ')} |`;
  const body = rows.map((row) => {
    const cells = columns.map((column) => String(column.value(row) ?? '').replace(/\|/g, '\\|'));
    return `| ${cells.join(' | ')} |`;
  });
  return [header, divider, ...body].join('\n');
}

function buildMarkdown(report) {
  return `# ${PACKAGE_ID} ${report.mode === 'real_apply' ? 'Result' : 'Plan'}

Generated: ${report.generated_at}

## Safety

- mode: ${report.mode}
- db_writes_performed: ${report.db_writes_performed}
- storage_uploads_performed: ${report.storage_uploads_performed}
- migrations_created: ${report.migrations_created}
- cleanup_performed: ${report.cleanup_performed}
- quarantine_performed: ${report.quarantine_performed}
- target_table: ${report.target_table}
- parent_overwrite_allowed: ${report.parent_overwrite_allowed}
- storage_bucket: ${report.storage_bucket}
- image_status: exact

## Summary

- source_rows: ${report.source_rows}
- ready_rows: ${report.ready_rows ?? report.db_updated_rows}
- blocked_rows: ${report.blocked_rows ?? 0}
- ready_for_real_apply: ${report.ready_for_real_apply ?? report.committed}
- storage_uploaded_rows: ${report.storage_uploaded_rows ?? 0}
- db_updated_rows: ${report.db_updated_rows ?? 0}
- fingerprint: \`${report.fingerprint}\`
- proof_hash: \`${report.proof_hash ?? '-'}\`

## Rows

${markdownTable(report.rows, [
  { label: 'status', value: (row) => row.validation_errors?.length ? row.validation_errors.join(', ') : 'ready' },
  { label: 'set', value: (row) => row.set_code },
  { label: 'card', value: (row) => row.card_name },
  { label: 'number', value: (row) => row.number },
  { label: 'printing', value: (row) => row.printing_gv_id },
  { label: 'image_path', value: (row) => row.image_path },
  { label: 'source', value: (row) => row.source_url },
])}

## Approval Command

\`\`\`powershell
node scripts/audits/image_truth_v1_img22a_mep_staff_stamp_exact_upload_apply.mjs --apply --fingerprint ${report.fingerprint}
\`\`\`

## Evidence

TCGCSV live product titles must include [Staff], TCGCSV live price subtypes must include Holofoil, and the product images were manually visually reviewed before packet generation for visible STAFF stamps.
`;
}

async function buildPlan() {
  const [productsPayload, pricesPayload] = await Promise.all([
    fetchJson(TCGCSV_GROUP_PRODUCTS_URL),
    fetchJson(TCGCSV_GROUP_PRICES_URL),
  ]);
  const products = productsPayload.results ?? [];
  const prices = pricesPayload.results ?? [];
  const supabase = createStorageClient();
  const client = new Client({ connectionString: requireDbUrl(), ssl: { rejectUnauthorized: false } });
  const rows = [];
  let rollbackCompleted = false;

  await client.connect();
  try {
    await client.query('begin');
    for (const target of TARGETS) {
      const product = products.find((entry) => Number(entry.productId) === target.product_id);
      const asset = await stageAsset(target);
      const storageExists = await storageObjectExists(supabase, asset.proposed_storage_path);
      const before = await fetchTargetRow(client, target);
      const validationErrors = [];

      if (!productMatchesTarget(product, target)) validationErrors.push('tcgcsv_staff_product_mismatch');
      if (!priceMatchesTarget(prices, target)) validationErrors.push('tcgcsv_holofoil_price_subtype_missing');
      if (storageExists) validationErrors.push('planned_storage_object_already_exists');
      validationErrors.push(...validateTargetRow(target, before));

      const parentBeforeHash = before ? proofHash(parentSnapshot(before)) : null;
      let mutationPreview = null;
      let parentImageFieldsUnchanged = false;
      if (validationErrors.length === 0) {
        const update = await client.query(
          `
            update public.card_printings
               set image_source = $2,
                   image_path = $3,
                   image_status = $4,
                   image_note = $5
             where id = $1
               and image_path is null
               and (image_url is null or btrim(image_url) = '')
               and (image_alt_url is null or btrim(image_alt_url) = '')
            returning id, image_source, image_path, image_status, image_note
          `,
          [before.card_printing_id, 'identity', asset.proposed_storage_path, 'exact', imageNote(target)],
        );
        const after = await fetchTargetRow(client, target);
        parentImageFieldsUnchanged = parentBeforeHash === proofHash(parentSnapshot(after));
        if (update.rowCount === 1 && parentImageFieldsUnchanged) {
          mutationPreview = update.rows[0];
        } else {
          validationErrors.push('rollback_update_not_verified');
        }
      }

      rows.push({
        card_printing_id: before?.card_printing_id ?? null,
        card_print_id: before?.card_print_id ?? null,
        parent_gv_id: before?.parent_gv_id ?? null,
        printing_gv_id: target.printing_gv_id,
        set_code: target.set_code,
        number: target.number,
        card_name: target.card_name,
        finish_key: target.finish_key,
        variant_key: target.variant_key,
        source_url: target.source_url,
        source_product_id: target.product_id,
        source_product_name: product?.name ?? null,
        source_image_url: target.source_image_url,
        tcgcsv_image_url: target.tcgcsv_image_url,
        image_source: 'identity',
        image_status: 'exact',
        image_path: asset.proposed_storage_path,
        image_note: imageNote(target),
        asset,
        storage_object_already_exists: storageExists,
        parent_image_fields_unchanged: parentImageFieldsUnchanged,
        validation_errors: validationErrors,
        dry_run_status: validationErrors.length === 0 ? 'rollback_update_verified' : 'blocked',
        mutation_preview: mutationPreview,
      });
    }
    await client.query('rollback');
    rollbackCompleted = true;
  } catch (error) {
    try {
      await client.query('rollback');
    } catch (_) {}
    throw error;
  } finally {
    await client.end();
  }

  const proof = {
    package_id: PACKAGE_ID,
    target_table: 'card_printings',
    parent_overwrite_allowed: false,
    rollback_completed: rollbackCompleted,
    rows: rows.map((row) => ({
      card_printing_id: row.card_printing_id,
      card_print_id: row.card_print_id,
      printing_gv_id: row.printing_gv_id,
      parent_gv_id: row.parent_gv_id,
      set_code: row.set_code,
      number: row.number,
      card_name: row.card_name,
      finish_key: row.finish_key,
      variant_key: row.variant_key,
      source_url: row.source_url,
      source_product_id: row.source_product_id,
      source_image_url: row.source_image_url,
      tcgcsv_image_url: row.tcgcsv_image_url,
      image_source: row.image_source,
      image_status: row.image_status,
      image_path: row.image_path,
      image_note: row.image_note,
      normalized_sha256: row.asset.normalized_sha256,
      normalized_size_bytes: row.asset.normalized_size_bytes,
      validation_errors: row.validation_errors,
    })),
  };

  return {
    generated_at: new Date().toISOString(),
    package_id: PACKAGE_ID,
    mode: 'plan_only',
    db_writes_performed: false,
    storage_uploads_performed: false,
    migrations_created: false,
    cleanup_performed: false,
    quarantine_performed: false,
    target_table: 'card_printings',
    parent_overwrite_allowed: false,
    storage_bucket: STORAGE_BUCKET,
    source_rows: rows.length,
    ready_rows: rows.filter((row) => row.validation_errors.length === 0).length,
    blocked_rows: rows.filter((row) => row.validation_errors.length > 0).length,
    ready_for_real_apply: rows.length > 0 && rows.every((row) => row.validation_errors.length === 0),
    rows,
    proof,
    fingerprint: proofHash(proof),
  };
}

async function applyPlan(plan, fingerprint) {
  if (fingerprint !== plan.fingerprint) throw new Error(`fingerprint_mismatch: expected ${plan.fingerprint}`);
  if (plan.ready_for_real_apply !== true) throw new Error('plan_not_ready_for_real_apply');

  const supabase = createStorageClient();
  const client = new Client({ connectionString: requireDbUrl(), ssl: { rejectUnauthorized: false } });
  const uploaded = [];
  const appliedRows = [];
  let committed = false;

  for (const row of plan.rows) {
    const buffer = await fs.readFile(row.asset.local_nonproduction_asset_path);
    const sha = sha256Hex(buffer);
    if (sha !== row.asset.normalized_sha256) throw new Error(`local_asset_sha_mismatch:${row.printing_gv_id}`);
    if (buffer.length !== row.asset.normalized_size_bytes) throw new Error(`local_asset_size_mismatch:${row.printing_gv_id}`);
    const { error } = await supabase.storage.from(STORAGE_BUCKET).upload(row.image_path, buffer, {
      contentType: 'image/jpeg',
      upsert: false,
    });
    if (error) throw new Error(`storage_upload_failed:${row.image_path}:${error.message}`);
    uploaded.push(row.image_path);
  }

  await client.connect();
  try {
    await client.query('begin');
    for (const row of plan.rows) {
      const current = await fetchTargetRow(client, row);
      const validationErrors = validateTargetRow(row, current);
      if (validationErrors.length > 0) {
        throw new Error(`target_validation_failed:${row.printing_gv_id}:${validationErrors.join(',')}`);
      }
      const parentBeforeHash = proofHash(parentSnapshot(current));
      const update = await client.query(
        `
          update public.card_printings
             set image_source = $2,
                 image_path = $3,
                 image_status = $4,
                 image_note = $5
           where id = $1
             and image_path is null
             and (image_url is null or btrim(image_url) = '')
             and (image_alt_url is null or btrim(image_alt_url) = '')
          returning id, image_source, image_path, image_status, image_note
        `,
        [row.card_printing_id, row.image_source, row.image_path, row.image_status, row.image_note],
      );
      if (update.rowCount !== 1) throw new Error(`db_update_rowcount_mismatch:${row.printing_gv_id}`);
      const after = await fetchTargetRow(client, row);
      if (parentBeforeHash !== proofHash(parentSnapshot(after))) throw new Error(`parent_image_changed:${row.printing_gv_id}`);
      appliedRows.push(update.rows[0]);
    }
    await client.query('commit');
    committed = true;
  } catch (error) {
    try {
      await client.query('rollback');
    } catch (_) {}
    throw error;
  } finally {
    await client.end();
  }

  const resultRows = plan.rows.map((row) => ({
    card_printing_id: row.card_printing_id,
    card_print_id: row.card_print_id,
    parent_gv_id: row.parent_gv_id,
    printing_gv_id: row.printing_gv_id,
    set_code: row.set_code,
    number: row.number,
    card_name: row.card_name,
    finish_key: row.finish_key,
    variant_key: row.variant_key,
    source_url: row.source_url,
    source_image_url: row.source_image_url,
    image_path: row.image_path,
    image_status: row.image_status,
  }));

  const result = {
    generated_at: new Date().toISOString(),
    package_id: PACKAGE_ID,
    mode: 'real_apply',
    db_writes_performed: true,
    storage_uploads_performed: true,
    migrations_created: false,
    cleanup_performed: false,
    quarantine_performed: false,
    target_table: 'card_printings',
    parent_overwrite_allowed: false,
    storage_bucket: STORAGE_BUCKET,
    source_rows: resultRows.length,
    fingerprint: plan.fingerprint,
    storage_uploaded_rows: uploaded.length,
    db_updated_rows: appliedRows.length,
    committed,
    uploaded_storage_paths: uploaded,
    rows: resultRows,
  };
  result.proof_hash = proofHash({
    package_id: PACKAGE_ID,
    fingerprint: plan.fingerprint,
    storage_uploaded_rows: result.storage_uploaded_rows,
    db_updated_rows: result.db_updated_rows,
    committed: result.committed,
    rows: result.rows,
  });
  return result;
}

async function main() {
  const args = parseArgs(process.argv);
  const plan = await buildPlan();

  if (!args.apply) {
    await fs.writeFile(PLAN_JSON, `${JSON.stringify(plan, null, 2)}\n`);
    await fs.writeFile(PLAN_MD, buildMarkdown(plan));
    console.log(JSON.stringify({
      generated: [PLAN_JSON, PLAN_MD],
      source_rows: plan.source_rows,
      ready_rows: plan.ready_rows,
      blocked_rows: plan.blocked_rows,
      ready_for_real_apply: plan.ready_for_real_apply,
      fingerprint: plan.fingerprint,
    }, null, 2));
    return;
  }

  const result = await applyPlan(plan, args.fingerprint);
  await fs.writeFile(RESULT_JSON, `${JSON.stringify(result, null, 2)}\n`);
  await fs.writeFile(RESULT_MD, buildMarkdown(result));
  console.log(JSON.stringify({
    generated: [RESULT_JSON, RESULT_MD],
    storage_uploaded_rows: result.storage_uploaded_rows,
    db_updated_rows: result.db_updated_rows,
    committed: result.committed,
    proof_hash: result.proof_hash,
  }, null, 2));
}

main().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});

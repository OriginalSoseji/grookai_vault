...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06l_active_finish_child_printing_real_apply_gate_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg06l_active_finish_child_printing_real_apply_gate_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260609_pkg06l_active_finish_child_printing_real_apply_gate_checkpoint_v1.md');
...
function validate(dryRun) {
  ...
  if (dryRun.scope?.child_printing_rows !== EXPECTED_CHILD_ROWS) findings.push('child_count_not_35');
  if (dryRun.scope?.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('parent_count_not_34');
...
function renderMarkdown(report) {
  return `# PKG-06L Active Finish Child Printing Real Apply Gate V1
...
function renderCheckpoint(report) {
  return `# PKG-06L Active Finish Child Printing Real Apply Gate Checkpoint V1
...
function updateCheckpointIndex() {
  ...
  const line = '| 2026-06-09 | [PKG-06L Active Finish Child Printing Real Apply Gate Checkpoint V1](20260609_pkg06l_active_finish_child_printing_real_apply_gate_checkpoint_v1.md) | No-write gate for 35 active-finish child-only inserts across 10 sets; real apply blocked pending exact approval. |';
  const current = fs.existsSync(indexPath) ? fs.readFileSync(indexPath, 'utf8') : '# Master Index Checkpoints\n';
  if (current.includes('20260609_pkg06l_active_finish_child_printing_real_apply_gate_checkpoint_v1.md')) {
    fs.writeFileSync(indexPath, current.split('\n').map((existing) => (
      existing.includes('20260609_pkg06l_active_finish_child_printing_real_apply_gate_checkpoint_v1.md') ? line : existing
    )).join('\n'));
  } else {
...
const requiredApproval = `Approve real ${PACKAGE_ID} apply only. Fingerprint: ${dryRun.package_fingerprint_sha256}. SQL hash: ${dryRun.sql_hash_sha256}. Scope: 35 child-only card_printing inserts for ecard3/Skyridge, pop9/POP Series 9, sm4/Crimson Invasion, swsh12/Silver Tempest, swsh4.5/Shining Fates, 2015xy/McDonald's Collection 2015, bw1/Black & White, bw10/Plasma Blast, bw11/Legendary Treasures, and bw3/Noble Victories; finishes cosmos=18, holo=10, normal=7; target parents=34. Dry-run proof: ${dryRunProof}. No global apply. No migrations. No deletes. No merges. No unsupported cleanup. No parent writes.`;

const report = {
  generated_at: new Date().toISOString(),
  version: 'english_master_index_pkg06l_active_finish_child_printing_real_apply_gate_v1',
  audit_only: true,
  ...
  required_operator_decision: {
    ...
    approval_effect: 'This would authorize preparing and running the durable PKG-06L active-finish child-only apply path only.',
  },

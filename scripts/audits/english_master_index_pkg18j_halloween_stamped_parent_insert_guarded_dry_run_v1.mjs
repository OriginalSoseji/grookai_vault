...
const READINESS_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg18i_halloween_active_finish_readiness_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg18j_halloween_stamped_parent_insert_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg18j_halloween_stamped_parent_insert_guarded_dry_run_v1.md');

const PACKAGE_ID = 'PKG-18J-HALLOWEEN-STAMPED-PARENT-INSERTS';
const CREATED_BY = 'pkg18j_halloween_stamped_parent_insert_guarded_dry_run_v1';
...
function buildTargets(readiness) {
  return (readiness.rows ?? [])
    ...
    .map((row) => ({
      ...
      source_families: ['pricecharting_trick_or_trade', 'verified_master_index_identity'],
      evidence: {
...
async function runDryRun(client, targets, fingerprint) {
  ...
  try {
    ...
    const guard = await client.query(
      `with projection as (
       ...
       select
         ...
         (select count(*)::int from pkg18j_targets target join public.card_prints base on base.id = target.base_parent_id left join public.card_printings base_child on base_child.card_print_id = base.id and base_child.finish_key = target.target_finish_key where base_child.id is null) as missing_base_finish_count,
         (select count(*)::int from pkg18j_targets target join public.card_prints base on base.id = target.base_parent_id join public.card_prints cp on cp.set_id = base.set_id and coalesce(cp.number_plain, cp.number) = coalesce(base.number_plain, base.number) and lower(cp.name) = lower(base.name) and coalesce(cp.variant_key, '') = target.target_variant_key) as parent_collision_count,
    ...
    return {
      dry_run_status: 'pkg18j_completed_rolled_back_no_durable_change',
      guard: guardRow,
      proof: proof.rows[0],
...
function renderMarkdown(report) {
  ...
  return `# PKG-18J Halloween Stamped Parent Insert Guarded Dry Run V1
...
async function main() {
  ...
  const recommended = execution.dry_run_status === 'pkg18j_completed_rolled_back_no_durable_change' && execution.rollback_verified && execution.stop_findings.length === 0
  ...
  const report = {
    ...
    version: 'english_master_index_pkg18j_halloween_stamped_parent_insert_guarded_dry_run_v1',
    package_id: PACKAGE_ID,

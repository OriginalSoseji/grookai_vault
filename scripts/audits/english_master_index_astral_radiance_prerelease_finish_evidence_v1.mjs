...
const OUT_DIR = path.join(
  ROOT,
  'docs/audits/english_master_index_source_exhaustion_v1/astral_radiance_prerelease_finish_evidence_v1',
);
const FIXTURE_DIR = path.join(
  ROOT,
  'docs/audits/verified_master_set_index_v1/source_fixtures/generated_astral_radiance_prerelease_finish_evidence_v1',
);
const OUT_JSON = path.join(OUT_DIR, 'astral_radiance_prerelease_finish_evidence_v1.json');
const OUT_MD = path.join(OUT_DIR, 'astral_radiance_prerelease_finish_evidence_v1.md');
const FIXTURE_JSON = path.join(FIXTURE_DIR, 'swshp_astral_radiance_prerelease_holo_v1.json');

const CARDS = [
  {
    card_name: 'Hisuian Basculegion',
    card_number: 'SWSH205',
    source_slug: 'hisuian-basculegion-swsh205',
    secondary: {
      source_key: 'pokumon_card_page',
      source_kind: 'collector_reference',
      source_url: 'https://pokumon.com/card/hisuian-basculegion-swsh205-english-promo/',
      required_terms: ['Hisuian Basculegion', 'SWSH205', 'Holo', 'Prerelease'],
    },
  },
  {
    card_name: 'Wyrdeer',
    card_number: 'SWSH206',
    source_slug: 'wyrdeer-swsh206',
    secondary: {
      source_key: 'pokumon_card_page',
      source_kind: 'collector_reference',
      source_url: 'https://pokumon.com/card/wyrdeer-swsh206-english-promo/',
      required_terms: ['Wyrdeer', 'SWSH206', 'Holo', 'Prerelease'],
    },
  },
  {
    card_name: 'Hisuian Samurott',
    card_number: 'SWSH207',
    source_slug: 'hisuian-samurott-swsh207',
    secondary: {
      source_key: 'pokumon_card_page',
      source_kind: 'collector_reference',
      source_url: 'https://pokumon.com/card/hisuian-samurott-swsh207-english-promo/',
      required_terms: ['Hisuian Samurott', 'SWSH207', 'Holo', 'Prerelease'],
    },
  },
  {
    card_name: 'Magnezone',
    card_number: 'SWSH208',
    source_slug: 'magnezone-swsh208',
    secondary: {
      source_key: 'pokumon_card_page',
      source_kind: 'collector_reference',
      source_url: 'https://pokumon.com/card/magnezone-swsh208-english-promo/',
      required_terms: ['Magnezone', 'SWSH208', 'Holo', 'Prerelease'],
    },
  },
];
...
function pricechartingUrl(card) {
  return `https://www.pricecharting.com/game/pokemon-promo/${card.source_slug}`;
}
...
function buildMarkdown(report) {
  return `# Astral Radiance Prerelease Finish Evidence V1

Audit-only evidence packet for the SWSH Black Star Astral Radiance prerelease promos currently in the stamped/special queue.
...
${mdTable([
    ...
    ['review_only_candidates', report.summary.review_only_candidates],
...
for (const card of CARDS) {
  ...
  const primary = await inspect({
    source_key: 'pricecharting_promo_price_guide',
    source_kind: 'marketplace_checklist',
    source_url: pricechartingUrl(card),
    required_terms: [card.card_name, card.card_number, 'Prerelease', 'Holo'],
  });
  const secondary = await inspect(card.secondary);
  const sourceChecks = [primary, secondary];
  const sourceCount = sourceChecks.filter((source) => source.all_required_terms_found).length;
  const status = row && sourceCount >= 2
    ? 'source_ready_candidate_no_db_write'
    : row && sourceCount === 1
      ? 'review_only_single_source_or_partial_second_source'
      : 'blocked_source_terms_missing_or_not_in_queue';
  results.push({
  ...
  if (status === 'source_ready_candidate_no_db_write') {
    for (const source of sourceChecks.filter((item) => item.all_required_terms_found)) {
      records.push({
        ...
        raw_snapshot_ref: `generated_fixture:astral_radiance_prerelease_finish:${card.card_number}:${source.source_key}:${generatedAt}`,
...
const fixture = {
  ...
  source_key: 'astral_radiance_prerelease_finish_evidence_v1',
  source_kind: 'marketplace_checklist',
  source_url: 'multiple',
  ...
  retrieved_at: generatedAt,
  raw_snapshot_ref: `generated_fixture:astral_radiance_prerelease_finish_evidence_v1:${generatedAt}`,
  audit_only: true,
  db_writes_performed: false,
  migrations_created: false,
  cleanup_performed: false,
...
const reportCore = {
  version: 'astral_radiance_prerelease_finish_evidence_v1',
  ...
  summary: {
    ...
    review_only_candidates: results.filter((row) => row.status === 'review_only_single_source_or_partial_second_source').length,

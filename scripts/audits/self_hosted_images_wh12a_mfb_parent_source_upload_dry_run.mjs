...
const MANIFEST_JSONL = path.join(OUTPUT_DIR, 'self_hosted_images_wh12a_mfb_parent_source_upload_manifest_v1.jsonl');
const SUMMARY_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh12a_mfb_parent_source_upload_dry_run_summary_v1.json');
const SUMMARY_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh12a_mfb_parent_source_upload_dry_run_summary_v1.md');
const PACKAGE_ID = 'IMG-HOST-WH-12A-MFB-PARENT-SOURCE-UPLOAD-DRY-RUN';
const STORAGE_BUCKET = process.env.SELF_HOSTED_IMAGES_STORAGE_BUCKET ?? 'user-card-images';
const USER_AGENT = 'Grookai WH12A MFB Parent Image Audit/1.0';
const FETCH_TIMEOUT_MS = Number.parseInt(process.env.IMG_HOST_WH12A_FETCH_TIMEOUT_MS ?? '30000', 10);
const FETCH_CONCURRENCY = Math.max(1, Math.min(Number.parseInt(process.env.IMG_HOST_WH12A_FETCH_CONCURRENCY ?? '4', 10), 8));

const PRICECHARTING_MFB_SLUGS = {
  '1:bulbasaur': 'bulbasaur',
  '2:ivysaur': 'ivysaur',
  '3:oddish': 'oddish',
  '4:gloom': 'gloom',
  '5:exeggcute': 'exeggcute',
  '6:exeggutor': 'exeggutor',
  '7:scyther': 'scyther',
  '8:grass energy': 'basic-grass-energy',
  '9:charmander': 'charmander',
  '10:charmeleon': 'charmeleon',
  '11:vulpix': 'vulpix',
  '12:ninetales': 'ninetales',
  '13:growlithe': 'growlithe',
  '14:arcanine': 'arcanine',
  '15:magmar': 'magmar',
  '16:fire energy': 'basic-fire-energy',
  '17:pikachu': 'pikachu',
  '18:raichu': 'raichu',
  '19:magnemite': 'magnemite',
  '20:magneton': 'magneton',
  '21:voltorb': 'voltorb',
  '22:electrode': 'electrode',
  '23:electabuzz': 'electabuzz',
  '24:lightning energy': 'basic-lightning-energy',
  '25:squirtle': 'squirtle',
  '26:wartortle': 'wartortle',
  '27:poliwag': 'poliwag',
  '28:poliwhirl': 'poliwhirl',
  '29:magikarp': 'magikarp',
  '30:gyarados': 'gyarados',
  '31:lapras': 'lapras',
  '32:water energy': 'basic-water-energy',
};

const TCGCOLLECTOR_MFB_TRAINER_IMAGES = {
  '33:potion': {
...
function mfbKey(row) {
  return `${normalizeNumber(row.number)}:${normalizeComparable(row.name)}`;
}
...
function findMfbPriceChartingImage(row, html) {
  ...
  const images = extractImages(html).filter((image) => {
    ...
    const alt = normalizeComparable(image.alt);
    return alt.includes(name) && alt.includes('pokemon my first battle');
  });
  const preferred = images.find((image) => String(image.src ?? '').endsWith('/1600.jpg'))
  ...
  return {
    ...preferred,
    src: String(preferred.src).replace(/\/240\.jpg$/i, '/1600.jpg'),
  };
}
...
async function fetchRows(client) {
  const result = await client.query(`
    ...
    where cp.image_path is null
      and cp.set_code = 'mfb'
    order by cp.number_plain nulls last, cp.number, cp.name, cp.gv_id
  `);
  return result.rows;
}

function priceChartingMfbUrl(row) {
  const slug = PRICECHARTING_MFB_SLUGS[mfbKey(row)];
  if (!slug) return null;
  return `https://www.pricecharting.com/game/pokemon-my-first-battle/${slug}`;
}

async function acquirePriceChartingMfb(row) {
  const sourceUrl = priceChartingMfbUrl(row);
  const attempts = [];
  if (!sourceUrl) return { row, route: null, image: null, attempts: [{ route: 'pricecharting_mfb', error: 'no_slug_for_row' }] };
  try {
    const htmlResult = await fetchBuffer(sourceUrl);
    const title = htmlTitle(htmlResult.text);
    attempts.push({ route: 'pricecharting_mfb', source_url: sourceUrl, status: htmlResult.status, content_type: htmlResult.content_type, title });
    if (!htmlResult.ok || !titleMatchesMfbRepresentativePage(row, title)) {
      attempts[attempts.length - 1].error = 'title_not_representative_mfb_card_page';
      return { row, route: null, image: null, attempts };
    }
    const match = findMfbPriceChartingImage(row, htmlResult.text);
    if (!match) {
      attempts[attempts.length - 1].error = 'no_matching_pricecharting_image';
      return { row, route: null, image: null, attempts };
    }
    const image = await fetchBuffer(match.src);
    attempts.push({ route: 'pricecharting_mfb_image', source_url: match.src, status: image.status, content_type: image.content_type, size_bytes: image.size_bytes });
    ...
    return {
      row,
      route: 'pricecharting_mfb_representative',
      upstream_id: `pricecharting:mfb:${mfbKey(row)}`,
      upstream_set_id: 'mfb',
    ...
    attempts.push({ route: 'pricecharting_mfb', source_url: sourceUrl, error: String(error?.message ?? error) });
    return { row, route: null, image: null, attempts };
  }
}

async function acquireTcgCollectorMfbTrainer(row) {
  const record = TCGCOLLECTOR_MFB_TRAINER_IMAGES[mfbKey(row)];
  const attempts = [];
  if (!record) return { row, route: null, image: null, attempts: [{ route: 'tcgcollector_mfb_trainer', error: 'no_preserved_static_image_for_row' }] };
  try {
    const image = await fetchBuffer(record.source_image_url);
    attempts.push({ route: 'tcgcollector_mfb_trainer', source_url: record.source_image_url, status: image.status, content_type: image.content_type, size_bytes: image.size_bytes });
    if (!image.ok || !isImageContentType(image.content_type) || image.size_bytes <= 1024) {
      attempts[attempts.length - 1].error = 'image_url_not_valid';
      return { row, route: null, image: null, attempts };
    }
    return {
      row,
      route: 'tcgcollector_mfb_representative_trainer',
      upstream_id: `tcgcollector:mfb:${mfbKey(row)}`,
      upstream_set_id: 'mfb',
      source_url: record.source_image_url,
      source_page_url: record.source_url,
      source_label: record.evidence_label,
      image,
      attempts,
    };
  } catch (error) {
    attempts.push({ route: 'tcgcollector_mfb_trainer', source_url: record.source_image_url, error: String(error?.message ?? error) });
    return { row, route: null, image: null, attempts };
  }
}

async function acquireForRow(row) {
  if (TCGCOLLECTOR_MFB_TRAINER_IMAGES[mfbKey(row)]) return acquireTcgCollectorMfbTrainer(row);
  return acquirePriceChartingMfb(row);
}
...
function toManifestRow(acquired) {
  ...
  return {
    ...
    upstream_source: acquired.route.startsWith('tcgcollector') ? 'tcgcollector' : 'pricecharting',
...
async function main() {
  ...
  const stopFindings = [
    ...
    ...(unmatched.length ? ['unmatched_mfb_parent_rows'] : []),
  ];

...
const STAGING_ASSET_DIR = path.join('tmp', 'nonproduction_image_staging', 'image_truth_v1', 'img08a');
const DRY_RUN_JSON = path.join(OUTPUT_DIR, 'image_truth_img08a_pricecharting_mep_holo_representative_missing_display_dry_run_v1.json');
const DRY_RUN_MD = path.join(OUTPUT_DIR, 'image_truth_img08a_pricecharting_mep_holo_representative_missing_display_dry_run_v1.md');

const PACKAGE_ID = 'IMG-08A-PRICECHARTING-MEP-HOLO-REPRESENTATIVE-MISSING-DISPLAY-CHILD-IMAGE-DRY-RUN';
const PRICECHARTING_SOURCE_KEY = 'pricecharting_live_page';
const TARGET_SET_CODE = 'mep';
const TARGET_FINISH_KEY = 'holo';
...
async function fetchHtml(url) {
  try {
    const response = await fetch(url, {
      headers: {
        'user-agent': 'GrookaiImageTruthAudit/1.0 (+audit-only; targeted-img08)',
        accept: 'text/html',
      },
    });
...
function priceChartingImageMatches(row, image) {
  ...
  return normalizedAlt.includes('holo');
}

async function fetchPriceChartingAsset(row, source) {
  const html = await fetchHtml(source.source_url);
  if (!html) return null;
  const match = extractImages(html).find((image) => priceChartingImageMatches(row, image));
  if (!match) return null;
  return {
      asset_url: match.src,
      image_alt: match.alt ?? `PriceCharting product-page representative image for ${row.card_name} ${row.number} ${row.finish_key}`,
  };
}
...
async function normalizeAsset(row) {
  ...
  const plannedStoragePath = [
    'warehouse-derived',
    'image-truth-v1',
    'img08a-pricecharting-mep-holo-representative',
    row.set_code,
...
async function main() {
  ...
  for (const row of (sourcePacket.rows ?? [])) {
    ...
    if (normalizeKey(row.set_code) !== TARGET_SET_CODE) continue;
    if (normalizeKey(row.finish_key) !== TARGET_FINISH_KEY) continue;
    if (row.source_status !== 'source_url_preserved') continue;
    if (row.image_confidence !== 'representative') continue;
    const source = (row.preserved_source_urls ?? []).find((candidate) => {
      return candidate.source_key === PRICECHARTING_SOURCE_KEY && clean(candidate.source_url);
    });
    if (!source) continue;
    const asset = await fetchPriceChartingAsset(row, source);
    ...
    sourceRows.push({
      ...
      source_evidence_label: source.evidence_label,
  ...
  const filteredSourceRows = sourceRows
    ...
    .filter((row) => row.source_key === PRICECHARTING_SOURCE_KEY)
  ...
  const report = {
    ...
    package_id: PACKAGE_ID,
    source_manifest: SOURCE_PACKET_JSON,
    nonproduction_staging_asset_dir: STAGING_ASSET_DIR.replaceAll('\\', '/'),
    target_table: 'card_printings',
    parent_overwrite_allowed: false,
    source_rows: filteredSourceRows.length,
    normalized_asset_rows: rows.length,
    rollback_update_verified_rows: readyRows.length,
    blocked_rows: blockedRows.length,
    dry_run_ready_for_real_apply: filteredSourceRows.length > 0 && readyRows.length === filteredSourceRows.length && blockedRows.length === 0 && rollbackCompleted,
    rollback_completed: rollbackCompleted,
  ...
  await fs.writeFile(DRY_RUN_MD, `# Image Truth IMG-08A PriceCharting MEP Holo Representative Missing-Display Dry Run V1

Generated: ${report.generated_at}

Status: rollback-only dry run. No persisted DB writes. No migrations. No production image promotion.

This package is representative display coverage from exact PriceCharting MEP holo product pages, not exact variant imagery.

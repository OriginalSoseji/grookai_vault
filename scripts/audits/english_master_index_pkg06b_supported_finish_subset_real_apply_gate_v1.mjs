...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06b_supported_finish_subset_real_apply_gate_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg06b_supported_finish_subset_real_apply_gate_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260609_pkg06b_supported_finish_subset_real_apply_gate_checkpoint_v1.md');
...
function validate({ dryRun, sqlText }) {
  ...
  if (supported.child_printing_rows !== 120) findings.push('supported_child_count_not_120');
  if (supported.target_parent_rows !== 120) findings.push('supported_parent_count_not_120');
  if (supported.set_count !== 1) findings.push('supported_set_count_not_1');
  if (supported.set_counts?.me03 !== 120) findings.push('me03_count_not_120');
  if (supported.finish_counts?.normal !== 65) findings.push('normal_count_not_65');
  if (supported.finish_counts?.holo !== 55) findings.push('holo_count_not_55');
  if (blocked.child_printing_rows !== 268) findings.push('blocked_count_not_268');
  ...
  if (proof) {
    ...
    if (Number(proof.target_parent_rows) !== 120) findings.push('rollback_proof_parent_count_mismatch');
    if (Number(proof.planned_child_rows) !== 120) findings.push('rollback_proof_child_count_mismatch');
  }
...
function buildReport() {
  ...
  const exactApprovalPhrase =
    ...
    `SQL hash: ${SQL_HASH}. ` +
    'Scope: 120 child-only card_printing inserts for me03/Perfect Order, finishes normal=65 and holo=55. ' +
    'Blocked finish-taxonomy rows remain excluded: 268. ' +
  ...
  return {
    ...
    version: 'english_master_index_pkg06b_supported_finish_subset_real_apply_gate_v1',
    audit_only: true,
    ...
    required_operator_decision: {
      ...
      approval_effect:
        'This would authorize preparing and running the durable PKG-06B supported-finish subset apply path only, scoped to 120 child-only card_printing inserts. It does not authorize the 268 blocked finish-taxonomy rows, global apply, deletes, merges, parent writes, unsupported cleanup, or migrations.',
    },
...
function renderMarkdown(report) {
  return `# PKG-06B Supported Finish Subset Real Apply Gate V1
...
| normal | ${report.package_scope.finish_counts.normal} |
...
| sql_contains_card_printings_insert | ${report.dry_run_proof.sql_contains_card_printings_insert} |
| sql_contains_update_statement | ${report.dry_run_proof.sql_contains_update_statement} |
| sql_contains_delete_statement | ${report.dry_run_proof.sql_contains_delete_statement} |
| sql_contains_commit_statement | ${report.dry_run_proof.sql_contains_commit_statement} |
| sql_contains_rollback_statement | ${report.dry_run_proof.sql_contains_rollback_statement} |
...
function renderCheckpoint(report) {
  return `# PKG-06B Supported Finish Subset Real Apply Gate Checkpoint V1
...
function updateCheckpointIndex() {
  ...
  const line = '| 2026-06-09 | [PKG-06B Supported Finish Subset Real Apply Gate Checkpoint V1](20260609_pkg06b_supported_finish_subset_real_apply_gate_checkpoint_v1.md) | Records the no-write real-apply gate for 120 supported child-only me03 printing inserts, requiring exact approval before any durable insert. |';
  const current = fs.existsSync(indexPath) ? fs.readFileSync(indexPath, 'utf8') : '# Master Index Checkpoints\n';
  if (current.includes('20260609_pkg06b_supported_finish_subset_real_apply_gate_checkpoint_v1.md')) {
    fs.writeFileSync(
      indexPath,
      current
        .split('\n')
        .map((existingLine) =>
          existingLine.includes('20260609_pkg06b_supported_finish_subset_real_apply_gate_checkpoint_v1.md')
            ? line
            : existingLine)
        .join('\n'),
    );

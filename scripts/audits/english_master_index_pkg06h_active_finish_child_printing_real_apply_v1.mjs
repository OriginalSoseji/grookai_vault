...
const GATE_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06h_active_finish_child_printing_real_apply_gate_v1.json');
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06h_active_finish_child_printing_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06h_active_finish_child_printing_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg06h_active_finish_child_printing_real_apply_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260609_pkg06h_active_finish_child_printing_real_apply_checkpoint_v1.md');

const PACKAGE_ID = 'PKG-06H-ACTIVE-FINISH-CHILD-PRINTING-INSERTS';
const SOURCE_READINESS_FINGERPRINT = '1cdba6ca2ad8cca6c5edd527ffee2e879aa9d6276cf52ac900c7681a3472318e';
const PACKAGE_FINGERPRINT = '489d80ab40043f16badef31a9553b3cba2031aabaefd90b7ebe328a946173c36';
const SQL_HASH = 'd9ba66057a977458708e371ba3de1aa8a8b973864f7bf6142f8b08b4176c47c3';
const DRY_RUN_PROOF_HASH = 'd85b454e481cb36711be7a2830b593d0349331534b04364f170392901952f8df';
const APPROVAL_TEXT = 'Approve real PKG-06H-ACTIVE-FINISH-CHILD-PRINTING-INSERTS apply only. Fingerprint: 489d80ab40043f16badef31a9553b3cba2031aabaefd90b7ebe328a946173c36. SQL hash: d9ba66057a977458708e371ba3de1aa8a8b973864f7bf6142f8b08b4176c47c3. Scope: 110 child-only card_printing inserts for swsh6/Chilling Reign, sm1/Sun & Moon, sv03.5/151, ex1/Ruby & Sapphire, pop4/POP Series 4, pop6/POP Series 6, sv01/Scarlet & Violet, sv05/Temporal Forces, swsh10/Astral Radiance, and xy1/XY; finishes normal=34, cosmos=51, holo=14, reverse=11; target parents=105. Dry-run proof: d85b454e481cb36711be7a2830b593d0349331534b04364f170392901952f8df == d85b454e481cb36711be7a2830b593d0349331534b04364f170392901952f8df. No global apply. No migrations. No deletes. No merges. No unsupported cleanup. No parent writes.';

const EXPECTED_CHILD_ROWS = 110;
const EXPECTED_PARENT_ROWS = 105;
...
const EXPECTED_FINISH_COUNTS = { normal: 34, cosmos: 51, holo: 14, reverse: 11 };
...
function validatePrerequisites({ gate, dryRun, rows }) {
  ...
  if (dryRun.dry_run_execution_status !== 'pkg06h_active_finish_child_printing_completed_rolled_back_no_durable_change') findings.push('dry_run_not_passed');
  ...
  if (rows.length !== EXPECTED_CHILD_ROWS) findings.push('target_row_count_not_110');
  if (new Set(rows.map((row) => row.card_print_id)).size !== EXPECTED_PARENT_ROWS) findings.push('target_parent_count_not_105');
...
function validateBeforeSnapshot({ beforeTargetSnapshot, beforeParentSnapshot, dryRun }) {
  ...
  if (beforeTargetSnapshot.counts.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('before_parent_count_not_105');
  ...
  if (beforeParentSnapshot.count !== EXPECTED_PARENT_ROWS) findings.push('before_parent_snapshot_count_not_105');
  return findings;
}

function validateAfterSnapshot({ afterTargetSnapshot, beforeParentSnapshot, afterParentSnapshot, insertedRows }) {
  const findings = [];
  if (afterTargetSnapshot.counts.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('after_parent_count_not_105');
  if (afterTargetSnapshot.counts.existing_target_child_rows !== EXPECTED_CHILD_ROWS) findings.push('after_existing_target_child_count_not_110');
  if (afterTargetSnapshot.counts.planned_id_collision_rows !== EXPECTED_CHILD_ROWS) findings.push('after_planned_id_collision_count_not_110');
  if (beforeParentSnapshot.hash_sha256 !== afterParentSnapshot.hash_sha256) findings.push('parent_snapshot_changed_unexpectedly');
  if (insertedRows.counts.inserted_rows_found !== EXPECTED_CHILD_ROWS) findings.push('inserted_rows_found_not_110');
...
async function applyPackage({ dryRun, rows }) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg06h_real_apply_targets (
    ...
    await client.query(
      `insert into pkg06h_real_apply_targets
       select
    ...
    const shape = await client.query(
      `select
         ...
         count(*) filter (where finish_key not in ('normal', 'cosmos', 'holo', 'reverse'))::int as unsupported_rows
       from pkg06h_real_apply_targets`,
    );
    ...
    const grouped = await client.query(
      `select 'set' as kind, set_key as key, count(*)::int as count from pkg06h_real_apply_targets group by set_key
       union all
       select 'finish' as kind, finish_key as key, count(*)::int as count from pkg06h_real_apply_targets group by finish_key`,
    );
    ...
    const lockParents = await client.query(
      `select cp.id
       from public.card_prints cp
       join (select distinct card_print_id from pkg06h_real_apply_targets) target on target.card_print_id = cp.id
       for update of cp`,
    );
    ...
    const finishGuard = await client.query(
      `select count(*)::int as unsupported_finish_count
       from pkg06h_real_apply_targets target
    ...
    return {
      connected: true,
      apply_status: 'pkg06h_active_finish_child_printing_real_apply_committed',
      error_message: null,
      before_target_snapshot: beforeTargetSnapshot,
    ...
    return {
      connected: true,
      apply_status: 'pkg06h_active_finish_child_printing_real_apply_failed_rolled_back',
...
function renderMarkdown(report) {
  return `# PKG-06H Active Finish Child Printing Real Apply V1
...
function renderCheckpoint(report) {
  return `# PKG-06H Active Finish Child Printing Real Apply Checkpoint V1
...
Real apply was scoped to 110 child-only card_printing inserts across 10 sets. No parent writes, migrations, deletes, merges, unsupported cleanup, or quarantine were performed.
`;
}

function updateCheckpointIndex() {
  const indexPath = path.join(CHECKPOINT_DIR, 'CHECKPOINT_INDEX.md');
  const line = '| 2026-06-09 | [PKG-06H Active Finish Child Printing Real Apply Checkpoint V1](20260609_pkg06h_active_finish_child_printing_real_apply_checkpoint_v1.md) | Records approved real apply for 110 active-finish child-only card_printing inserts across 10 sets; parent rows unchanged, no migrations. |';
  const current = fs.existsSync(indexPath) ? fs.readFileSync(indexPath, 'utf8') : '# Master Index Checkpoints\n';
  if (current.includes('20260609_pkg06h_active_finish_child_printing_real_apply_checkpoint_v1.md')) {
    fs.writeFileSync(indexPath, current.split('\n').map((existingLine) => (
      existingLine.includes('20260609_pkg06h_active_finish_child_printing_real_apply_checkpoint_v1.md') ? line : existingLine
    )).join('\n'));
  } else {
...
const report = {
  ...
  version: 'english_master_index_pkg06h_active_finish_child_printing_real_apply_v1',
  audit_only: false,
  apply_only: true,
  approval_scope: {
  ...
  apply_status: pass
    ? 'pkg06h_active_finish_child_printing_real_apply_committed_and_verified'
    : 'pkg06h_active_finish_child_printing_real_apply_failed_or_blocked',
  ...
  verification_summary: {
    ...
    master_index_comparison_status: pass ? 'pkg06h_active_finish_child_printing_verified_after_apply' : 'not_verified',
  },

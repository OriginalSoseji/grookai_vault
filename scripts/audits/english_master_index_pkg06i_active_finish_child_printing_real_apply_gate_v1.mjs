...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06i_active_finish_child_printing_real_apply_gate_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg06i_active_finish_child_printing_real_apply_gate_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260609_pkg06i_active_finish_child_printing_real_apply_gate_checkpoint_v1.md');
...
function validate(dryRun) {
  ...
  if (dryRun.scope?.child_printing_rows !== EXPECTED_CHILD_ROWS) findings.push('child_count_not_84');
  if (dryRun.scope?.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('parent_count_not_78');
...
function renderMarkdown(report) {
  return `# PKG-06I Active Finish Child Printing Real Apply Gate V1
...
function renderCheckpoint(report) {
  return `# PKG-06I Active Finish Child Printing Real Apply Gate Checkpoint V1
...
function updateCheckpointIndex() {
  ...
  const line = '| 2026-06-09 | [PKG-06I Active Finish Child Printing Real Apply Gate Checkpoint V1](20260609_pkg06i_active_finish_child_printing_real_apply_gate_checkpoint_v1.md) | No-write gate for 84 active-finish child-only inserts across 10 sets; real apply blocked pending exact approval. |';
  const current = fs.existsSync(indexPath) ? fs.readFileSync(indexPath, 'utf8') : '# Master Index Checkpoints\n';
  if (current.includes('20260609_pkg06i_active_finish_child_printing_real_apply_gate_checkpoint_v1.md')) {
    fs.writeFileSync(indexPath, current.split('\n').map((existing) => (
      existing.includes('20260609_pkg06i_active_finish_child_printing_real_apply_gate_checkpoint_v1.md') ? line : existing
    )).join('\n'));
  } else {
...
const requiredApproval = `Approve real ${PACKAGE_ID} apply only. Fingerprint: ${dryRun.package_fingerprint_sha256}. SQL hash: ${dryRun.sql_hash_sha256}. Scope: 84 child-only card_printing inserts for pop8/POP Series 8, sve/Scarlet & Violet Energies, svp/Scarlet & Violet Black Star Promos, xy3/Furious Fists, 2019sm/McDonald's Collection 2019, bw4/Next Destinies, sv10/Destined Rivals, swsh8/Fusion Strike, xy7/Ancient Origins, and xy8/BREAKthrough; finishes cosmos=57, normal=9, holo=9, reverse=9; target parents=78. Dry-run proof: ${dryRunProof}. No global apply. No migrations. No deletes. No merges. No unsupported cleanup. No parent writes.`;

const report = {
  generated_at: new Date().toISOString(),
  version: 'english_master_index_pkg06i_active_finish_child_printing_real_apply_gate_v1',
  audit_only: true,
  ...
  required_operator_decision: {
    ...
    approval_effect: 'This would authorize preparing and running the durable PKG-06I active-finish child-only apply path only.',
  },

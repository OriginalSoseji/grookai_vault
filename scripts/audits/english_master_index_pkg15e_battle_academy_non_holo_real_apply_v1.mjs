...
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg15e_battle_academy_non_holo_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg15e_battle_academy_non_holo_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg15e_battle_academy_non_holo_real_apply_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260611_pkg15e_battle_academy_non_holo_real_apply_checkpoint_v1.md');
...
const CREATED_BY = 'pkg15e_battle_academy_non_holo_real_apply_v1';
const SOURCE_PACKAGE_ID = 'PKG-15D-BATTLE-ACADEMY-NON-HOLO-ADJUDICATION';
const EXPECTED_PACKAGE_FINGERPRINT = 'b268384eaf62aabdb13b1754cb1fdd3b92c1b4a7ca9075f8211b9ef4134369a9';
const EXPECTED_DRY_RUN_PROOF = 'aada8d9caee028c0ce1c340b69d4e9f2a3e416b8e7ddbd89c5040880c7e691ef';
...
function validateDryRun(dryRun) {
  ...
  if (dryRun.execution?.dry_run_status !== 'pkg15e_battle_academy_non_holo_completed_rolled_back_no_durable_change') findings.push('dry_run_status_not_passed');
  ...
  if (counts.parent_inserts !== 1) findings.push('dry_run_parent_insert_scope_mismatch');
  if (counts.identity_inserts !== 1) findings.push('dry_run_identity_insert_scope_mismatch');
  ...
  if ((dryRun.scope?.targets ?? []).length !== 1) findings.push('dry_run_targets_not_1');
  return findings;
}
...
async function applyPackage(client, targets, fingerprint) {
  ...
  try {
    ...
    return {
      apply_status: 'pkg15e_battle_academy_non_holo_real_apply_committed',
      guard: guardRow,
      proof: proof.rows[0],
      write_counts: {
...
function renderMarkdown(report) {
  ...
  return `# PKG-15E Battle Academy Non-Holo Real Apply V1

Approved real apply for one deterministic Battle Academy stamped canonical parent insert with adjudicated normal child finish.
...
function checkpointMarkdown(report) {
  return `# PKG-15E Battle Academy Non-Holo Real Apply Checkpoint V1
...
const report = {
  ...
  version: 'english_master_index_pkg15e_battle_academy_non_holo_real_apply_v1',
  package_id: PACKAGE_ID,
  ...
  db_writes_performed: execution.apply_status === 'pkg15e_battle_academy_non_holo_real_apply_committed',
  durable_db_writes_performed: execution.apply_status === 'pkg15e_battle_academy_non_holo_real_apply_committed',

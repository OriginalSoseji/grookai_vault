...
const WH11C_SUMMARY_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh11c_residual_parent_db_pointer_repoint_dry_run_summary_v1.json');
const WH11C_PLAN_JSONL = path.join(OUTPUT_DIR, 'self_hosted_images_wh11c_residual_parent_db_pointer_repoint_plan_v1.jsonl');
const PLAN_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh11d_residual_parent_db_pointer_repoint_apply_plan_v1.json');
const PLAN_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh11d_residual_parent_db_pointer_repoint_apply_plan_v1.md');
const RESULT_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh11d_residual_parent_db_pointer_repoint_apply_result_v1.json');
const RESULT_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh11d_residual_parent_db_pointer_repoint_apply_result_v1.md');
const PACKAGE_ID = 'IMG-HOST-WH-11D-RESIDUAL-PARENT-DB-POINTER-REPOINT-APPLY';
...
function buildUpdates(summary, planRows) {
  ...
  if (summary.ready_for_apply_package !== true) {
    stopFindings.push(`wh11c_not_apply_ready:${(summary.stop_findings ?? []).join(',')}`);
  }
...
function buildPlan(args, summary, updates) {
  const plan = {
    ...
    source_plan_jsonl: path.relative(ROOT, WH11C_PLAN_JSONL),
    ...
    by_source_route: countBy(updates.updates, (row) => row.source_route),
...
${markdownTable(topEntries(plan.by_source_route))}
`;
}
...
async function main() {
  ...
  const summary = await readJson(WH11C_SUMMARY_JSON);
  const planRows = await readJsonl(WH11C_PLAN_JSONL);

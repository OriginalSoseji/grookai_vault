...
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'enrich12c_tcgdex_catalog_metadata_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'enrich12c_tcgdex_catalog_metadata_guarded_dry_run_v1.md');
...
async function classifyCandidate(candidate) {
  try {
    ...
    const nameMatches = normalizeName(candidate.card_name) === normalizeName(sourceName);
...
async function main() {
  ...
  try {
    ...
    const report = {
      version: 'ENRICH12C_TCGDEX_CATALOG_METADATA_GUARDED_DRY_RUN_V1',
      package_id: PACKAGE_ID,

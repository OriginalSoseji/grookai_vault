...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06n_active_finish_child_printing_real_apply_gate_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg06n_active_finish_child_printing_real_apply_gate_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260609_pkg06n_active_finish_child_printing_real_apply_gate_checkpoint_v1.md');
...
function validate(dryRun) {
  ...
  if (dryRun.scope?.child_printing_rows !== EXPECTED_CHILD_ROWS) findings.push('child_count_not_21');
  if (dryRun.scope?.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('parent_count_not_21');
...
function renderMarkdown(report) {
  return `# PKG-06N Active Finish Child Printing Real Apply Gate V1
...
function renderCheckpoint(report) {
  return `# PKG-06N Active Finish Child Printing Real Apply Gate Checkpoint V1
...
function updateCheckpointIndex() {
  ...
  const line = '| 2026-06-09 | [PKG-06N Active Finish Child Printing Real Apply Gate Checkpoint V1](20260609_pkg06n_active_finish_child_printing_real_apply_gate_checkpoint_v1.md) | No-write gate for 21 active-finish child-only inserts across 10 sets; real apply blocked pending exact approval. |';
  const current = fs.existsSync(indexPath) ? fs.readFileSync(indexPath, 'utf8') : '# Master Index Checkpoints\n';
  if (current.includes('20260609_pkg06n_active_finish_child_printing_real_apply_gate_checkpoint_v1.md')) {
    fs.writeFileSync(indexPath, current.split('\n').map((existing) => (
      existing.includes('20260609_pkg06n_active_finish_child_printing_real_apply_gate_checkpoint_v1.md') ? line : existing
    )).join('\n'));
  } else {
...
const requiredApproval = `Approve real ${PACKAGE_ID} apply only. Fingerprint: ${dryRun.package_fingerprint_sha256}. SQL hash: ${dryRun.sql_hash_sha256}. Scope: 21 child-only card_printing inserts for xy9/BREAKpoint, 2011bw/McDonald's Collection 2011, bw2/Emerging Powers, bw5/Dark Explorers, bw9/Plasma Freeze, dp4/Great Encounters, dp7/Stormfront, dv1/Dragon Vault, hgssp/HGSS Black Star Promos, and neo1/Neo Genesis; finishes cosmos=15, holo=4, normal=1, reverse=1; target parents=21. Dry-run proof: ${dryRunProof}. No global apply. No migrations. No deletes. No merges. No unsupported cleanup. No parent writes.`;

const report = {
  generated_at: new Date().toISOString(),
  version: 'english_master_index_pkg06n_active_finish_child_printing_real_apply_gate_v1',
  audit_only: true,
  ...
  required_operator_decision: {
    ...
    approval_effect: 'This would authorize preparing and running the durable PKG-06N active-finish child-only apply path only.',
  },

...
const READINESS_JSON = path.join(OUTPUT_DIR, 'image_truth_img01c_storage_readiness_v1.json');
const PLAN_JSON = path.join(OUTPUT_DIR, 'image_truth_img01d_upload_apply_plan_v1.json');
const PLAN_MD = path.join(OUTPUT_DIR, 'image_truth_img01d_upload_apply_plan_v1.md');
const APPLY_JSON = path.join(OUTPUT_DIR, 'image_truth_img01d_upload_apply_result_v1.json');
const APPLY_MD = path.join(OUTPUT_DIR, 'image_truth_img01d_upload_apply_result_v1.md');
const PACKAGE_ID = 'IMG-01D-MISSING-DISPLAY-EXACT-CHILD-IMAGE-UPLOAD-APPLY';
...
function buildMarkdown(report) {
  return `# Image Truth V1 IMG-01D Upload Apply ${report.mode === 'real_apply' ? 'Result' : 'Plan'}
...
\`\`\`powershell
node scripts/audits/image_truth_v1_img01d_upload_apply.mjs --apply --fingerprint ${report.fingerprint}
\`\`\`
`;
}

...
function validateDryRun(dryRun) {
  ...
  if (dryRun.scope?.target_child_deletes !== 9453) findings.push(`scope_target_child_deletes_not_9453:${dryRun.scope?.target_child_deletes}`);
  if (dryRun.scope?.by_finish?.holo !== 5321) findings.push('scope_holo_not_5321');
  if (dryRun.scope?.by_finish?.reverse !== 2133) findings.push('scope_reverse_not_2133');
  if (dryRun.scope?.by_finish?.normal !== 1999) findings.push('scope_normal_not_1999');
  if ((dryRun.targets ?? []).length !== 9453) findings.push(`targets_length_not_9453:${(dryRun.targets ?? []).length}`);
...
async function applyPackage(client, targets) {
  ...
  try {
    ...
    if (
      ...
      throw new Error(`PKG-20A prewrite guard failed: ${JSON.stringify(guard)}`);
    }
    ...
    if (deleteResult.rowCount !== targets.length) {
      throw new Error(`PKG-20A delete count mismatch: ${deleteResult.rowCount}`);
    }
    ...
    if (proof.rows[0].remaining_target_children !== 0) {
      throw new Error(`PKG-20A post-delete proof failed: ${JSON.stringify(proof.rows[0])}`);
    }
    await client.query('commit');
    const afterSnapshot = await captureSnapshot(client, targets);
    return {
      apply_status: 'pkg20a_real_apply_committed',
      committed: true,
      before_snapshot: beforeSnapshot,
...
function renderMarkdown(report) {
  ...
  return `# PKG-20A Overgeneration Child Delete Real Apply V1
...
if (prerequisiteFindings.length > 0) {
  throw new Error(`PKG-20A dry-run prerequisite validation failed: ${prerequisiteFindings.join(', ')}`);
}
...
const report = {
  ...
  version: 'english_master_index_pkg20a_overgeneration_child_delete_real_apply_v1',

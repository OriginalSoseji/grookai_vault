...
const READINESS_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg17m_skarmory_league_reverse_readiness_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg17n_skarmory_league_reverse_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg17n_skarmory_league_reverse_guarded_dry_run_v1.md');

const PACKAGE_ID = 'PKG-17N-SKARMORY-LEAGUE-REVERSE-PARENT-INSERT';
const CREATED_BY = 'pkg17n_skarmory_league_reverse_guarded_dry_run_v1';
...
async function runDryRun(client, targets, fingerprint) {
  ...
  try {
    ...
    return {
      dry_run_status: 'pkg17n_failed_rolled_back',
...
function renderMarkdown(report) {
  ...
  return `# PKG-17N Skarmory League Reverse Guarded Dry Run V1
...
async function main() {
  ...
  const recommended = execution.dry_run_status === 'pkg17n_completed_rolled_back_no_durable_change' && execution.rollback_verified && execution.stop_findings.length === 0
  ...
  const report = {
    ...
    version: 'english_master_index_pkg17n_skarmory_league_reverse_guarded_dry_run_v1',
    package_id: PACKAGE_ID,

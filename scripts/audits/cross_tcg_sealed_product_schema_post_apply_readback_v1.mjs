...
function parseArgs(argv) {
  ...
  if (!args.outDir) throw new Error("--out-dir=<directory> is required");
  return args;
}
...
async function captureIndependentReadback(connectionString) {
  const client = new Client({
    ...
    application_name: "sealed-product-schema-independent-readback-v1",
  });
  ...
  try {
    ...
    readback = await captureSealedSchemaReadbackV1(client);
...
async function main() {
  ...
  const [planText, executionText] = await Promise.all([
    fs.readFile(path.join(ROOT, SEALED_SCHEMA_PLAN_PATH), "utf8"),
    fs.readFile(args.executionSummary, "utf8"),
  ]);
  const plan = JSON.parse(planText);
  const execution = JSON.parse(executionText);
  const preFindings = [];
  if (execution.version !== SEALED_SCHEMA_APPLY_VERSION) {
  ...
  if (Number(execution.boundaries?.sealed_data_rows_written) !== 0) {
    preFindings.push("execution_reported_sealed_data_rows");
  }
  ...
  if (preFindings.length) {
    throw new Error(`Execution proof is not eligible: ${preFindings.join(",")}`);
  }
  ...
  const result = {
    version: "CROSS_TCG_SEALED_PRODUCT_SCHEMA_INDEPENDENT_READBACK_V1",
    recorded_at: new Date().toISOString(),
    status: findings.length === 0 ? "pass" : "blocked",
    execution_summary_path: args.executionSummary,
    execution_summary_sha256: sealedSchemaApplySha256V1(executionText),
  ...
  const reportBody = `# Cross-TCG Sealed Product Schema Independent Readback V1\n\n` +
    `- Status: **${result.status.toUpperCase()}**\n` +
    `- Migration SHA-256: \`${result.migration_sha256}\`\n` +
    `- Sealed tables: \`${readback.tables.length}\`\n` +
    `- Sealed data rows: \`${readback.tables.reduce((sum, row) => sum + Number(row.row_count), 0)}\`\n` +

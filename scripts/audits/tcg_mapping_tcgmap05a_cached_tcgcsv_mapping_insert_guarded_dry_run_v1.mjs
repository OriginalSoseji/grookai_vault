...
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'tcgmap05a_cached_tcgcsv_mapping_insert_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'tcgmap05a_cached_tcgcsv_mapping_insert_guarded_dry_run_v1.md');
...
async function runRollbackDryRun(client, targets) {
  ...
  try {
    ...
    if (!guardPassed(guard, targets.length)) {
      throw new Error(`TCGMAP-05A guard failed: ${JSON.stringify(guard)}`);
    }
...
function buildMarkdown(report) {
  ...
  lines.push('');
  lines.push('Guarded rollback-only dry-run for cached TCGCSV-derived TCGplayer external mappings.');
  lines.push('');
  lines.push('## Result');
  lines.push('');
...
async function main() {
  ...
  if (targets.length === 0) throw new Error('No ready rows found in TCGMAP-05 readiness report.');
  ...
  const client = new Client({
    ...
    application_name: 'tcgmap05a_guarded_dry_run',
  });
  ...
  try {
    ...
    const report = {
      ...
      target_fingerprint_sha256: sha256(stableJson(targets)),
      execution,
      blocked_buckets: (readiness.classification_buckets ?? []).filter((bucket) => bucket.key !== 'ready_from_cached_tcgcsv_exact_identity'),
...
main().catch((error) => {
  console.error('[tcgmap05a-dry-run] failed:', error);
  process.exit(1);
});

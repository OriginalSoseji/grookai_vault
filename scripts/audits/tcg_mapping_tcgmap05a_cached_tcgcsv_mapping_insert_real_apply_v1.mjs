...
const READINESS_JSON = path.join(OUTPUT_DIR, 'tcgmap05_cached_tcgcsv_readiness_v1.json');
const DRY_RUN_JSON = path.join(OUTPUT_DIR, 'tcgmap05a_cached_tcgcsv_mapping_insert_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'tcgmap05a_cached_tcgcsv_mapping_insert_real_apply_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'tcgmap05a_cached_tcgcsv_mapping_insert_real_apply_v1.md');

const PACKAGE_ID = 'TCGMAP-05A-CACHED-TCGCSV-TCGPLAYER-MAPPING-INSERTS';
const EXPECTED_READINESS_FINGERPRINT = 'bee892d14f83b1a9ace7d35c4599b68f2ceb24d43ea010b7a4a19f104b98cbb7';
const EXPECTED_TARGET_FINGERPRINT = '88874689034c1e4807322b6d5d64347a01964a80b89a22f40894f566bcb6e0b5';
const EXPECTED_DRY_RUN_PROOF = '4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945';
const EXPECTED_TARGET_ROWS = 153;
const APPROVAL_TEXT = 'Approve real TCGMAP-05A-CACHED-TCGCSV-TCGPLAYER-MAPPING-INSERTS apply only.';
...
function buildTargets(readiness) {
  return (readiness.ready_rows ?? []).map((row) => ({
    ...
    source_cache_file: row.source_cache_file,
    source_group_id: row.source_group_id,
    match_rule: row.match_rule,
    meta: {
      derived_from: 'preserved_tcgcsv_tcgplayer_product_cache',
      source_url: row.source_url,
      source_cache_file: row.source_cache_file,
      source_group_id: row.source_group_id,
      ...
      match_rule: row.match_rule,
      promoted_by: 'tcg_mapping_tcgmap05a_cached_tcgcsv_mapping_insert_guarded_dry_run_v1',
    },
  }));
}
...
async function applyPackage(client, targets) {
  ...
  try {
    ...
    await client.query(
      `create temporary table tcgmap05a_targets (
    ...
    const insertResult = await client.query(
      `insert into public.external_mappings (
       ...
       select
         ...
         now(),
         meta
       from tcgmap05a_targets`,
    );

    const verification = await client.query(
      `with target as (
         select * from tcgmap05a_targets
       )
       select
         (select count(*)::int
          from public.external_mappings em
...
function buildMarkdown(report) {
  ...
  lines.push('');
  lines.push('Scoped real apply for cached TCGCSV-derived TCGplayer external mappings.');
  lines.push('');
  lines.push('## Result');
  lines.push('');
...
async function main() {
  ...
  const client = new Client({
    ...
    application_name: 'tcgmap05a_real_apply',
  });
...
main().catch((error) => {
  console.error('[tcgmap05a-real-apply] failed:', error);
  process.exit(1);
});

...
const PLAN_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh04c_tcgdex_replacement_storage_upload_apply_plan_v1.json');
const PLAN_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh04c_tcgdex_replacement_storage_upload_apply_plan_v1.md');
const RESULT_JSONL = path.join(OUTPUT_DIR, 'self_hosted_images_wh04c_tcgdex_replacement_storage_upload_apply_result_v1.jsonl');
const RESULT_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh04c_tcgdex_replacement_storage_upload_apply_result_v1.json');
const RESULT_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh04c_tcgdex_replacement_storage_upload_apply_result_v1.md');
const PACKAGE_ID = 'IMG-HOST-WH-04C-TCGDEX-REPLACEMENT-STORAGE-UPLOAD-APPLY';
const USER_AGENT = 'Grookai TCGdex Replacement Storage Upload Apply/1.0';

function parseArgs(argv) {
  const args = {
    apply: false,
    fingerprint: null,
    limit: Number.parseInt(process.env.TCGDEX_REPLACEMENT_UPLOAD_LIMIT ?? '0', 10),
    concurrency: Number.parseInt(process.env.TCGDEX_REPLACEMENT_UPLOAD_CONCURRENCY ?? '4', 10),
    timeoutMs: Number.parseInt(process.env.TCGDEX_REPLACEMENT_UPLOAD_TIMEOUT_MS ?? '45000', 10),
  };
  ...
  for (const row of manifestRows) {
    ...
    if (!existing) {
      byPath.set(key, {
        ...
        sample_rows: [row.source_audit_key],
      });
      continue;
    }
    existing.manifest_row_count += 1;
    if (existing.sample_rows.length < 10) existing.sample_rows.push(row.source_audit_key);
    if (
...
async function main() {
  ...
  const result = {
    ...
    db_writes_performed: false,
    storage_upload_apply_executed: true,
    storage_uploads_performed: allUploaded.length > 0,
    storage_uploads_performed_this_run: uploaded.length > 0,
    migrations_created: false,
    identity_table_writes_performed: false,
    price_writes_performed: false,
    deletes_or_merges_performed: false,
...
- Storage upload apply executed: ${result.storage_upload_apply_executed}
- Storage uploads performed in total: ${result.storage_uploads_performed}
- Storage uploads performed this run: ${result.storage_uploads_performed_this_run}

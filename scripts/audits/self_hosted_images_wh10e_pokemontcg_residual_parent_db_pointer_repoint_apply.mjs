...
const WH10D_SUMMARY_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh10d_pokemontcg_residual_parent_db_pointer_repoint_dry_run_summary_v1.json');
const WH10D_PLAN_JSONL = path.join(OUTPUT_DIR, 'self_hosted_images_wh10d_pokemontcg_residual_parent_db_pointer_repoint_plan_v1.jsonl');
const PLAN_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh10e_pokemontcg_residual_parent_db_pointer_repoint_apply_plan_v1.json');
const PLAN_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh10e_pokemontcg_residual_parent_db_pointer_repoint_apply_plan_v1.md');
const RESULT_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh10e_pokemontcg_residual_parent_db_pointer_repoint_apply_result_v1.json');
const RESULT_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh10e_pokemontcg_residual_parent_db_pointer_repoint_apply_result_v1.md');
const PACKAGE_ID = 'IMG-HOST-WH-10E-POKEMONTCG-RESIDUAL-PARENT-DB-POINTER-REPOINT-APPLY';
const APPROVED_SQL_HASH = '981c32875b74d4d8442426f7b91d06e59602c05dc5dd305d7b7cccb1bf1fa169';
...
function buildUpdates(summary, planRows) {
  ...
  if (summary.ready_for_apply_package !== true) {
    stopFindings.push(`wh10d_not_apply_ready:${(summary.stop_findings ?? []).join(',')}`);
  }
  ...
  for (const row of planRows) {
    ...
    if (!Array.isArray(row.preserved_columns) || !row.preserved_columns.includes('image_status') || !row.preserved_columns.includes('image_note')) {
      stopFindings.push(`missing_preserved_column_guard:${row.target_row_id}`);
      continue;
    }
    const imageSource = clean(row.proposed_values?.image_source);
    const imagePath = clean(row.proposed_values?.image_path);
    if (imageSource !== 'identity') {
      stopFindings.push(`non_identity_source:${row.target_row_id}`);
      continue;
    }
    if (!imagePath) {
      stopFindings.push(`missing_image_path:${row.target_row_id}`);
      continue;
    }
    if (row.current_values?.image_status !== row.proposed_values?.image_status) {
      stopFindings.push(`image_status_change:${row.target_row_id}`);
      continue;
    }
    if (row.current_values?.image_note !== row.proposed_values?.image_note) {
      stopFindings.push(`image_note_change:${row.target_row_id}`);
      continue;
    }
    if (row.changed_columns?.length === 0) continue;
    const changedColumns = new Set(row.changed_columns ?? []);
    for (const column of changedColumns) {
      if (!['image_source', 'image_path'].includes(column)) {
        stopFindings.push(`unsupported_changed_column:${column}:${row.target_row_id}`);
      }
    }
    updates.push({
      ...
      current_image_status: row.current_values?.image_status ?? null,
    });
  }
...
function buildPlan(args, summary, updates) {
  const plan = {
    ...
    source_plan_jsonl: path.relative(ROOT, WH10D_PLAN_JSONL),
    ...
    by_current_image_status: countBy(updates.updates, (row) => row.current_image_status ?? 'null'),
    planned_columns: ['image_source', 'image_path'],
  ...
  plan.ready_for_apply = plan.stop_findings.length === 0 && plan.metadata_pointer_update_count > 0;
...
${markdownTable(topEntries(plan.by_current_image_status))}
`;
}
...
async function main() {
  ...
  const summary = await readJson(WH10D_SUMMARY_JSON);
  const planRows = await readJsonl(WH10D_PLAN_JSONL);

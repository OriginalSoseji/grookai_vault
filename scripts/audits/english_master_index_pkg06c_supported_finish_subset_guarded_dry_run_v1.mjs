...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06c_supported_finish_subset_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg06c_supported_finish_subset_guarded_dry_run_v1.md');
const OUTPUT_SQL = path.join(SQL_DIR, 'english_master_index_pkg06c_supported_finish_subset_guarded_dry_run_transaction_v1.sql');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260609_pkg06c_supported_finish_subset_guarded_dry_run_checkpoint_v1.md');
...
const EXPECTED_FINISH_COUNTS = { holo: 8 };
const PROVENANCE_SOURCE = 'verified_master_set_index_v1';
const CREATED_BY = 'pkg06c_supported_finish_subset_dry_run_v1';
...
begin
  ...
  if child_count <> ${EXPECTED_CHILD_ROWS} then raise exception 'PKG-06C child count drift: %', child_count; end if;
  if parent_count <> ${EXPECTED_PARENT_ROWS} then raise exception 'PKG-06C parent count drift: %', parent_count; end if;
...
async function runDryRun(client, sql, rows) {
  ...
  let status = 'pkg06c_supported_finish_subset_completed_rolled_back_no_durable_change';
  ...
  try {
    ...
    status = 'pkg06c_supported_finish_subset_failed';
...
function evaluate({ readiness, supportedRows, blockedRows, activeFinishKeys, execution }) {
  const findings = [];
  const finishCounts = countBy(supportedRows, (row) => row.finish_key);
  const setCounts = countBy(supportedRows, (row) => row.set_key);
  ...
  if (supportedRows.length !== EXPECTED_CHILD_ROWS) findings.push('supported_child_count_not_8');
  if (new Set(supportedRows.map((row) => row.card_print_id)).size !== EXPECTED_PARENT_ROWS) findings.push('supported_parent_count_not_8');
  if (new Set(supportedRows.map((row) => row.set_key)).size !== 1) findings.push('supported_set_count_not_1');
  if (setCounts[EXPECTED_SET_KEY] !== EXPECTED_CHILD_ROWS) findings.push('supported_neo4_count_not_8');
  for (const [finishKey, count] of Object.entries(EXPECTED_FINISH_COUNTS)) {
    if (finishCounts[finishKey] !== count) findings.push(`supported_${finishKey}_count_not_${count}`);
  }
  if (blockedRows.length !== EXPECTED_BLOCKED_ROWS) findings.push('blocked_count_not_380');
  if (supportedRows.some((row) => !activeFinishKeys.has(row.finish_key))) findings.push('supported_contains_inactive_finish_key');
  if (execution.status !== 'pkg06c_supported_finish_subset_completed_rolled_back_no_durable_change') {
...
function buildMarkdown(report) {
  return `# PKG-06C Supported Finish Subset Guarded Dry-Run V1
...
function buildCheckpoint(report) {
  return `# PKG-06C Supported Finish Subset Guarded Dry-Run Checkpoint V1
...
async function updateCheckpointIndex() {
  ...
  const line = '| 2026-06-09 | [PKG-06C Supported Finish Subset Guarded Dry-Run Checkpoint V1](20260609_pkg06c_supported_finish_subset_guarded_dry_run_checkpoint_v1.md) | Rollback-dry-runs 8 supported child-only neo4 printing inserts, holo=8, with 380 finish-taxonomy rows excluded. |';
  const current = await fs.readFile(indexPath, 'utf8').catch(() => '# Master Index Checkpoints\n');
  if (current.includes('20260609_pkg06c_supported_finish_subset_guarded_dry_run_checkpoint_v1.md')) {
    await fs.writeFile(indexPath, current.split('\n').map((existing) => (
      existing.includes('20260609_pkg06c_supported_finish_subset_guarded_dry_run_checkpoint_v1.md') ? line : existing
    )).join('\n'));
  } else {
...
async function main() {
  ...
  try {
    ...
    const report = {
      ...
      version: 'english_master_index_pkg06c_supported_finish_subset_guarded_dry_run_v1',
      package_id: PACKAGE_ID,
      source_readiness_fingerprint_sha256: readiness.package_fingerprint_sha256,
      ...
      next_allowed_step: stopFindings.length === 0
        ? 'Prepare real-apply gate for PKG-06C supported subset only if separately requested.'
...
main().catch((error) => {
  console.error('[pkg06c][supported-finish-subset] failed:', error?.message ?? error);
  process.exitCode = 1;
});

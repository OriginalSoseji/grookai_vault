...
const SOURCE_SUMMARY_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh14a_energy_template_pointer_dry_run_summary_v1.json');
const SOURCE_PLAN_JSONL = path.join(OUTPUT_DIR, 'self_hosted_images_wh14a_energy_template_pointer_plan_v1.jsonl');
const PLAN_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh14b_energy_template_pointer_apply_plan_v1.json');
const PLAN_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh14b_energy_template_pointer_apply_plan_v1.md');
const RESULT_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh14b_energy_template_pointer_apply_result_v1.json');
const RESULT_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh14b_energy_template_pointer_apply_result_v1.md');
const PACKAGE_ID = 'IMG-HOST-WH-14B-ENERGY-TEMPLATE-POINTER-APPLY';
...
function buildUpdates(summary, planRows) {
  ...
  if (summary.ready_for_apply_package !== true) {
    stopFindings.push(`wh14a_not_apply_ready:${(summary.stop_findings ?? []).join(',')}`);
  }
  if (summary.sql_hash !== APPROVED_SQL_HASH) stopFindings.push(`sql_hash_drift:${summary.sql_hash}`);

  const updates = [];
  for (const row of planRows) {
    if (row.plan_type !== 'energy_template_representative_pointer') {

...
const DRY_RUN_JSON = path.join(OUTPUT_DIR, 'enrich20b_modern_mcd_child_printing_gv_id_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'enrich20b_modern_mcd_child_printing_gv_id_real_apply_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'enrich20b_modern_mcd_child_printing_gv_id_real_apply_v1.md');
const PACKAGE_ID = 'ENRICH-20B-MODERN-MCD-CHILD-PRINTING-GV-ID-BACKFILL';
const TARGET_SET_CODES = ['2023sv', '2024sv'];
const EXPECTED_FINGERPRINT = 'cced1de0bf2c69cbe3adff0b107fbfea536f1686ec9a31c73ac8c872f47ded4e';
const EXPECTED_DRY_RUN_PROOF = '96a6d6f381845847785dbf5d5c4cf3b96737fde1cdc4c9f475772bd71d90c96c';
const EXPECTED_TARGET_ROWS = 30;
const APPROVAL_TEXT = 'Approve real ENRICH-20B-MODERN-MCD-CHILD-PRINTING-GV-ID-BACKFILL apply only. Fingerprint: cced1de0bf2c69cbe3adff0b107fbfea536f1686ec9a31c73ac8c872f47ded4e. Scope: 30 modern McDonald\'s child card_printing printing_gv_id updates using governed finish suffixes normal=STD and holo=HOLO. Dry-run proof: 96a6d6f381845847785dbf5d5c4cf3b96737fde1cdc4c9f475772bd71d90c96c == 96a6d6f381845847785dbf5d5c4cf3b96737fde1cdc4c9f475772bd71d90c96c. No parent writes. No identity writes. No external mapping writes. No species writes. No deletes. No merges. No migrations. No image writes. No global apply.';
const FINISH_GV_SUFFIX = Object.freeze({ normal: 'STD', holo: 'HOLO' });
  ...
  const validation = await client.query(
    `with target as (
     ...
     select
       ...
       (select count(*)::int from target where finish_key not in ('normal', 'holo')) as unsupported_finish_count,
       (select count(*)::int from target where suffix is distinct from case finish_key when 'normal' then 'STD' when 'holo' then 'HOLO' end) as suffix_mismatch_count,
       ...
       (select count(*)::int
        ...
        where cp.set_code <> all($2::text[])
           ...
           or s.name not like 'McDonald''s Collection 20%'
           or cp.gv_id is null
...
async function applyPackage(client, targets) {
  ...
  try {
    ...
    await client.query(
      `create temporary table enrich20b_targets (
    ...
    const proof = await client.query(
      `select
         (select count(*)::int from enrich20b_targets) as target_count,
         (select count(*)::int
          from enrich20b_targets target
          join public.card_printings cpr on cpr.id = target.card_printing_id
...
async function main() {
  ...
  try {
    ...
    const report = {
      version: 'ENRICH20B_MODERN_MCD_CHILD_PRINTING_GV_ID_REAL_APPLY_V1',
      package_id: PACKAGE_ID,
    ...
    await writeText(OUTPUT_MD, [
      '# ENRICH-20B Modern McDonald Child Printing GV-ID Real Apply V1',
      '',
      `Package: \`${PACKAGE_ID}\``,
      '',
      '## Result',

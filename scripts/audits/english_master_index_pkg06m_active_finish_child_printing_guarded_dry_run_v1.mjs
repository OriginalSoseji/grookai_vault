...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06m_active_finish_child_printing_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg06m_active_finish_child_printing_guarded_dry_run_v1.md');
const OUTPUT_SQL = path.join(SQL_DIR, 'english_master_index_pkg06m_active_finish_child_printing_guarded_dry_run_transaction_v1.sql');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260609_pkg06m_active_finish_child_printing_guarded_dry_run_checkpoint_v1.md');
...
const CREATED_BY = 'pkg06m_active_finish_child_printing_dry_run_v1';
...
begin
  ...
  if child_count <> ${EXPECTED_CHILD_ROWS} then raise exception 'PKG-06M child count drift: %', child_count; end if;
  if parent_count <> ${EXPECTED_PARENT_ROWS} then raise exception 'PKG-06M parent count drift: %', parent_count; end if;
...
async function runDryRun(client, sql, rows) {
  ...
  let status = 'pkg06m_active_finish_child_printing_completed_rolled_back_no_durable_change';
  ...
  try {
    ...
    status = 'pkg06m_active_finish_child_printing_failed';
...
function evaluate({ readiness, rows, activeFinishKeys, execution }) {
  ...
  if (rows.length !== EXPECTED_CHILD_ROWS) findings.push('child_count_not_30');
  if (new Set(rows.map((row) => row.card_print_id)).size !== EXPECTED_PARENT_ROWS) findings.push('parent_count_not_30');
  ...
  if (execution.status !== 'pkg06m_active_finish_child_printing_completed_rolled_back_no_durable_change') findings.push('dry_run_transaction_did_not_complete');
...
function buildMarkdown(report) {
  return `# PKG-06M Active Finish Child Printing Guarded Dry-Run V1
...
function buildCheckpoint(report) {
  return `# PKG-06M Active Finish Child Printing Guarded Dry-Run Checkpoint V1
...
PKG-06M is a taxonomy-safe active-finish dry-run only. Real apply remains blocked until a separate gate and exact approval.
`;
}

async function updateCheckpointIndex() {
  const indexPath = path.join(CHECKPOINT_DIR, 'CHECKPOINT_INDEX.md');
  const line = '| 2026-06-09 | [PKG-06M Active Finish Child Printing Guarded Dry-Run Checkpoint V1](20260609_pkg06m_active_finish_child_printing_guarded_dry_run_checkpoint_v1.md) | Rollback-dry-runs 30 active-finish child-only inserts across 10 sets; no writes or migrations. |';
  const current = await fs.readFile(indexPath, 'utf8').catch(() => '# Master Index Checkpoints\n');
  if (current.includes('20260609_pkg06m_active_finish_child_printing_guarded_dry_run_checkpoint_v1.md')) {
    await fs.writeFile(indexPath, current.split('\n').map((existing) => (
      existing.includes('20260609_pkg06m_active_finish_child_printing_guarded_dry_run_checkpoint_v1.md') ? line : existing
    )).join('\n'));
  } else {
...
try {
  ...
  const report = {
    ...
    version: 'english_master_index_pkg06m_active_finish_child_printing_guarded_dry_run_v1',
    package_id: PACKAGE_ID,
    source_readiness_fingerprint_sha256: readiness.package_fingerprint_sha256,
    ...
    next_allowed_step: stopFindings.length === 0
      ? 'Prepare a real-apply gate for PKG-06M only if separately requested.'

...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg08v_tcgcollector_parent_child_insert_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg08v_tcgcollector_parent_child_insert_real_apply_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260610_pkg08v_tcgcollector_parent_child_insert_real_apply_checkpoint_v1.md');
...
async function applyPackage(client, planned, dryRun, packageFingerprint) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg08v_parent_targets (
    ...
    await client.query(
      `create temporary table pkg08v_child_targets (
    ...
    await client.query(
      `create temporary table pkg08v_mapping_targets (
    ...
    await client.query(
      `insert into pkg08v_parent_targets
    ...
    await client.query(
      `insert into pkg08v_child_targets
    ...
    await client.query(
      `insert into pkg08v_mapping_targets
    ...
    const guard = await client.query(
      `select
         (select count(*)::int from pkg08v_parent_targets) as parent_rows,
         (select count(*)::int from pkg08v_child_targets) as child_rows,
         (select count(*)::int from pkg08v_mapping_targets) as mapping_rows,
         (select count(*)::int from pkg08v_child_targets child left join pkg08v_parent_targets parent on parent.card_print_id = child.card_print_id where parent.card_print_id is null) as child_without_parent,
         (select count(*)::int from pkg08v_child_targets child left join public.finish_keys fk on fk.key = child.finish_key and fk.is_active = true where fk.key is null) as inactive_finish_rows,
         (select count(*)::int
          from pkg08v_parent_targets target
          join public.card_prints cp
            on cp.id = target.card_print_id
            or (
              lower(coalesce(cp.set_code, '')) = lower(target.live_set_code)
         ...
         (select count(*)::int
          from pkg08v_child_targets target
          join public.card_printings cpr
            on cpr.id = target.card_printing_id
            or (
              cpr.card_print_id = target.card_print_id
         ...
         (select count(*)::int
          from pkg08v_mapping_targets target
          join public.external_mappings em
            on em.source = target.source
    ...
    const proof = await client.query(
      `select
         ...
         (select count(*)::int from pkg08v_parent_targets) as planned_parent_rows,
         (select count(*)::int from pkg08v_child_targets) as planned_child_rows,
         (select count(*)::int from pkg08v_mapping_targets) as planned_mapping_rows`,
    ...
    return {
      apply_status: afterFindings.length ? 'committed_but_after_validation_failed' : 'pkg08v_tcgcollector_parent_child_insert_real_apply_committed',
      committed: true,
      before_snapshot: beforeSnapshot,
    ...
    return {
      apply_status: 'pkg08v_tcgcollector_parent_child_insert_real_apply_failed_rolled_back',
      committed: false,
...
function renderMarkdown(report) {
  return `# PKG-08V TCGCollector Parent+Child Insert Real Apply V1
...
async function main() {
  ...
  try {
    ...
    const report = {
      ...
      version: 'english_master_index_pkg08v_tcgcollector_parent_child_insert_real_apply_v1',
      package_id: PACKAGE_ID,
    ...
    await writeText(CHECKPOINT_MD, `# PKG-08V TCGCollector Parent+Child Insert Real Apply Checkpoint V1

...
} from './image_storage_permanent_plan_v1.mjs';
import {
  TARGET_STORAGE_BUCKET,
  TARGET_SUPABASE_PROJECT_REF,
} from './image_storage_canary_plan_v1.mjs';

const PACKAGE_ID = 'JPN-MASTER-INDEX-V4-IMAGE-STORAGE-PERMANENT-APPLY-V1';
const PLAN_ARTIFACT =
...
const USER_AGENT = 'Grookai Japanese V4 Permanent Storage Apply/1.0';
...
async function loadAssets(descriptor) {
  ...
  if (rows.length !== descriptor.row_count
    ...
    throw new Error('Permanent Storage asset dataset verification failed.');
  }
  return rows;
}
...
async function fetchSource(asset) {
  ...
  try {
    ...
    if (errors.length) throw new Error(`${asset.gv_id}:${errors.join(',')}`);
...
function folderAndName(storagePath) {
  ...
  return { folder: storagePath.slice(0, index), name: storagePath.slice(index + 1) };
}
...
async function downloadAndVerify(client, staged) {
  ...
  const observed = inspectImageBuffer(buffer, staged.asset.source_expected.content_type);
  for (const key of ['size_bytes', 'sha256', 'width', 'height', 'format']) {
    if (observed[key] !== staged.asset.source_expected[key]) {
...
async function removeAndVerifyAbsent(client, assets) {
  ...
  if (error) throw new Error(`Storage failure rollback failed: ${error.message}`);
  const states = await Promise.all(assets.map((asset) => objectExists(client, asset)));
  if (states.some(Boolean)) {
    throw new Error('Storage failure rollback absence check failed.');
  }
...
async function writeResult(result) {
  ...
  await fs.writeFile(
    path.join(RESULT_ROOT, 'jpn_image_storage_permanent_apply_v1.json'),
    stableJson(result),
  );
  await fs.writeFile(
    path.join(RESULT_ROOT, 'jpn_image_storage_permanent_apply_v1.md'),
    `# Japanese Master Index V4 Permanent Image Storage Apply V1
...
- Failure rollback removed: ${result.failure_rollback_removed}
- Failure rollback absent verified: ${result.failure_rollback_absent_verified}
...
async function main() {
  ...
  if (assets.length !== EXPECTED_PERMANENT_STORAGE_ROWS) {
    throw new Error(`Permanent Storage scope changed: ${assets.length}`);
  }
  const currentBundle = await computePermanentStorageCodeBundle();
  ...
  const recomputedApproval = contentFingerprint(
    permanentStorageApprovalPayload(assets, currentBundle.hash),
  );
  const recomputedPlanHash = permanentStoragePlanHash(
    recomputedApproval,
    currentBundle.hash,
  );
  ...
  try {
    // All exact source bytes are staged before the first Storage request.
    staged = await Promise.all(assets.map(fetchSource));
    client = storageClient();
    const existing = await Promise.all(
      assets.map((asset) => objectExists(client, asset)),
    );
    initiallyAbsent = existing.filter((state) => !state).length;
    ...
    if (runError && client && uploadedAssets.length > 0) {
      try {
        ...
        runError = new AggregateError(
          [runError, error],
          'Permanent upload and failure rollback failed.',
        );
      }
    }
  }

  const rollbackComplete = !runError
    || rollback.absent_verified === uploadedAssets.length;
  const result = {
    package_id: PACKAGE_ID,
    ...
    failure_rollback_removed: rollback.removed,
    failure_rollback_absent_verified: rollback.absent_verified,

...
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'enrich13f1_suffix_duplicate_transfer_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'enrich13f1_suffix_duplicate_transfer_guarded_dry_run_v1.md');
const OUTPUT_SQL = path.join(SQL_DIR, 'enrich13f1_suffix_duplicate_transfer_guarded_dry_run_v1.sql');

const PACKAGE_ID = 'ENRICH-13F1-SUFFIX-DUPLICATE-TRANSFER-DRY-RUN';
const EXPECTED_DUPLICATE_ROWS = 4;
const EXPECTED_BLOCKED_BASE_ROWS = 4;
...
function buildTargetRows(source) {
  return (source.duplicate_suffix_rows ?? []).map((row) => ({
    duplicate_card_print_id: row.source_card_print_id,
    canonical_owner_card_print_id: row.existing_owner_card_print_id,
    set_code: row.set_code,
    number: row.proposed_number,
    base_number: row.proposed_base_number,
    suffix: row.proposed_suffix,
    duplicate_name: row.source_name,
    canonical_owner_name: row.owner_name,
...
function buildSql(targetRows, packageFingerprint) {
  const values = targetRows.map((row) => `  (${[
    ...
    sqlString(row.base_number),
    sqlString(row.suffix),
...
-- Scope: ${targetRows.length} suffix duplicate parent dependency transfers.
...
begin
  ...
  if v_targets <> ${targetRows.length} then
    raise exception 'ENRICH-13F1 target count guard failed: expected ${targetRows.length}, got %', v_targets;
  end if;
...
async function runDryRun(sql, targetRows) {
  ...
  const client = new Client({ connectionString: cs, application_name: 'enrich13f1_suffix_duplicate_transfer_guarded_dry_run_v1' });
...
function validate({ source, targetRows, sql, execution }) {
  ...
  if (source.version !== 'ENRICH_13F_SUFFIX_VARIANT_COLLISION_GOVERNANCE_V1') findings.push('source_report_version_unexpected');
  if (targetRows.length !== EXPECTED_DUPLICATE_ROWS) findings.push(`duplicate_target_row_count_not_${EXPECTED_DUPLICATE_ROWS}`);
  if ((source.base_number_suffix_collision_rows ?? []).length !== EXPECTED_BLOCKED_BASE_ROWS) findings.push(`blocked_base_row_count_not_${EXPECTED_BLOCKED_BASE_ROWS}`);
  if (targetRows.some((row) => !row.suffix || !row.number.endsWith(row.suffix))) findings.push('non_suffix_row_included');
  ...
  if (Number(before.duplicate_parents_present ?? -1) !== EXPECTED_DUPLICATE_ROWS) findings.push('before_duplicate_parent_count_changed');
  if (Number(before.duplicate_child_printings ?? -1) !== 12) findings.push('before_duplicate_child_printing_count_changed');
  if (Number(before.duplicate_external_mappings ?? -1) !== EXPECTED_DUPLICATE_ROWS) findings.push('before_duplicate_mapping_count_changed');
  if (Number(before.duplicate_identity_rows ?? -1) !== 1) findings.push('before_duplicate_identity_count_changed');
  if (Number(before.duplicate_trait_rows ?? -1) !== EXPECTED_DUPLICATE_ROWS) findings.push('before_duplicate_trait_count_changed');
  if (Number(before.duplicate_species_rows ?? -1) !== 3) findings.push('before_duplicate_species_count_changed');
...
function renderMarkdown(report) {
  return `# ENRICH-13F1 Suffix Duplicate Transfer Guarded Dry-Run V1
...
| duplicate_target_rows | ${report.package_scope.duplicate_target_rows} |
| blocked_base_rows | ${report.package_scope.blocked_base_rows} |
...
${report.blocked_base_rows.map((row) => `- ${mdEscape(row.set_code)} ${mdEscape(row.proposed_number)} ${mdEscape(row.source_name)}: blocked from suffix-owner merge`).join('\n')}
...
const packageFingerprint = hash(stableJson({
  ...
  blocked_base_rows: source.base_number_suffix_collision_rows ?? [],
...
const report = {
  version: 'ENRICH_13F1_SUFFIX_DUPLICATE_TRANSFER_GUARDED_DRY_RUN_V1',
  generated_at: new Date().toISOString(),
  package_id: PACKAGE_ID,
  dry_run_execution_status: stopFindings.length === 0
    ? 'enrich13f1_guarded_dry_run_passed_rolled_back_no_durable_change'
    : 'enrich13f1_guarded_dry_run_blocked_or_failed',
  ...
  package_scope: {
    duplicate_target_rows: targetRows.length,
    blocked_base_rows: source.base_number_suffix_collision_rows?.length ?? null,
    sets: [...new Set(targetRows.map((row) => row.set_code))].sort(),
  ...
  required_real_apply_approval_text: `Approve real ENRICH-13F1-SUFFIX-DUPLICATE-TRANSFER apply only. Fingerprint: ${packageFingerprint}. SQL hash: ${sqlHash}. Scope: ${targetRows.length} duplicate suffix parent dependency transfers, ${execution.before_snapshot?.impact_counts?.duplicate_child_printings ?? 'unknown'} child printings deduped/transferred, ${execution.before_snapshot?.impact_counts?.duplicate_external_mappings ?? 'unknown'} external mappings handled; ${source.base_number_suffix_collision_rows?.length ?? 'unknown'} base-number rows remain blocked. Dry-run proof: ${execution.before_snapshot?.hash_sha256 ?? 'missing'} == ${execution.after_snapshot?.hash_sha256 ?? 'missing'}. No global apply. No migrations. No image writes.`,
};

...
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg17r_league_reverse_bulk_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg17r_league_reverse_bulk_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg17r_league_reverse_bulk_real_apply_v1.md');
...
function validateDryRun(dryRun) {
  ...
  if (dryRun.execution?.dry_run_status !== 'pkg17r_completed_rolled_back_no_durable_change') findings.push('dry_run_status_not_passed');
...
function buildTargets(readiness) {
  return (readiness.rows ?? [])
    ...
    .map((row) => ({
      ...
      evidence_label: 'Two independent exact sources support League Stamp reverse',
      evidence: {
...
async function applyPackage(client, targets, fingerprint) {
  ...
  try {
    ...
    return {
      apply_status: 'pkg17r_real_apply_committed',
      guard: guardRow,
      proof: proof.rows[0],
      write_counts: {
...
function renderMarkdown(report) {
  ...
  return `# PKG-17R League Reverse Bulk Real Apply V1

Approved real apply for two-source League Stamp reverse parent identity inserts.
...
async function main() {
  ...
  const report = {
    ...
    version: 'english_master_index_pkg17r_league_reverse_bulk_real_apply_v1',
    package_id: PACKAGE_ID,

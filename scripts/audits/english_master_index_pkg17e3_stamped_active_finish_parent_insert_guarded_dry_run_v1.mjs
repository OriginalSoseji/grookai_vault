...
const READINESS_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg17e_stamped_active_finish_web_evidence_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg17e3_stamped_active_finish_parent_insert_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg17e3_stamped_active_finish_parent_insert_guarded_dry_run_v1.md');

const PACKAGE_ID = 'PKG-17E3-STAMPED-ACTIVE-FINISH-PARENT-INSERTS';
const CREATED_BY = 'pkg17e3_stamped_active_finish_parent_insert_guarded_dry_run_v1';
const EXPECTED_TARGET_COUNT = 4;
const TARGET_STATUS = 'ready_for_guarded_dry_run';
...
function baseCandidateRows(readiness) {
  return (readiness.rows ?? [])
    .filter((row) => row.status === TARGET_STATUS)
    .filter((row) => row.accepted_finish_key)
    .map((row) => {
      const numberKey = String(row.card_number ?? '').trim();
      const nameKey = normalizeText(row.card_name);
      const variantKey = row.variant_key;
      const finishKey = row.accepted_finish_key;
      return {
        ...
        stamp_label: variantKey,
        source_readiness_status: row.status,
        evidence: {
          ...
          stale_source_base_parent_child_finishes: row.base_parent_child_finishes,
...
async function selectLiveTargets(client, readiness) {
  const candidates = baseCandidateRows(readiness);
  const result = await client.query(
    `with candidate as (
     ...
     select
       ...
       count(distinct target_child_collision.id)::int as target_child_collision_count,
       count(distinct target_parent_collision.id)::int as target_parent_id_collision_count
     from candidate
     join public.card_prints base on base.id = candidate.base_parent_id
     ...
     left join public.card_prints existing_parent
       on existing_parent.set_id = base.set_id
      and coalesce(existing_parent.number_plain, existing_parent.number) = coalesce(base.number_plain, base.number)
      and lower(existing_parent.name) = lower(base.name)
      and (
        coalesce(existing_parent.variant_key, '') = candidate.target_variant_key
        or coalesce(existing_parent.printed_identity_modifier, '') = candidate.target_printed_identity_modifier
      )
     left join public.card_printings target_child_collision on target_child_collision.id = candidate.target_child_id
     left join public.card_prints target_parent_collision on target_parent_collision.id = candidate.target_parent_id
     group by
       candidate.target_parent_id,
       candidate.target_child_id,
       candidate.base_parent_id,
       candidate.set_key,
       candidate.set_name,
       candidate.card_number,
       candidate.card_name,
       candidate.target_finish_key,
       candidate.target_variant_key,
       candidate.target_printed_identity_modifier,
       candidate.stamp_label,
  ...
  return result.rows
    .filter((row) => row.live_base_parent_child_finishes.includes(row.target_finish_key))
    .filter((row) => row.existing_variant_parent_count === 0)
    .filter((row) => row.target_child_collision_count === 0)
    .filter((row) => row.target_parent_id_collision_count === 0)
    .map((row) => ({
...
async function runDryRun(client, targets, fingerprint) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg17e3_targets (
    ...
    const guard = await client.query(
      `with projection as (
         ...
         from pkg17e3_targets target
         join public.card_prints base on base.id = target.base_parent_id
         left join public.sets s on s.id = base.set_id
       )
       select
         (select count(*)::int from pkg17e3_targets) as target_count,
         (select count(distinct target_parent_id)::int from pkg17e3_targets) as target_parent_count,
         (select count(distinct target_child_id)::int from pkg17e3_targets) as target_child_count,
         (select count(*)::int from pkg17e3_targets where target_variant_key is null or target_variant_key = '' or target_variant_key = 'stamped') as unsafe_variant_count,
         (select count(*)::int from pkg17e3_targets where target_printed_identity_modifier <> target_variant_key) as modifier_mismatch_count,
         (select count(*)::int from pkg17e3_targets target left join public.card_prints base on base.id = target.base_parent_id where base.id is null) as missing_base_count,
         (select count(*)::int from pkg17e3_targets target join public.card_prints base on base.id = target.base_parent_id where coalesce(base.variant_key, '') <> '' or base.printed_identity_modifier is not null) as base_parent_not_unstamped_count,
         (select count(*)::int from pkg17e3_targets target left join public.finish_keys fk on fk.key = target.target_finish_key and fk.is_active = true where fk.key is null) as inactive_finish_count,
         (select count(*)::int
          from pkg17e3_targets target
          join public.card_prints base on base.id = target.base_parent_id
         ...
         (select count(*)::int from pkg17e3_targets target join public.card_printings cpr on cpr.id = target.target_child_id) as child_collision_count,
         (select count(*)::int from pkg17e3_targets target join public.card_print_identity cpi on cpi.card_print_id = target.target_parent_id and cpi.is_active = true) as identity_target_collision_count,
         ...
         (select count(*)::int from pkg17e3_targets target where jsonb_array_length(coalesce(target.evidence->'source_families', '[]'::jsonb)) < 2) as insufficient_source_family_count`,
    );
    ...
    const childInsert = await client.query(
      `insert into public.card_printings (
       ...
       select
         ...
         concat('Stamped identity child finish routed from source-backed base parent: ', target.target_finish_key)
       from pkg17e3_targets target`,
      [CREATED_BY],
    );
    ...
    const proof = await client.query(
      `select
         ...
         (select count(*)::int from pkg17e3_targets) as target_rows,
         (select count(*)::int from public.card_prints cp join pkg17e3_targets target on target.target_parent_id = cp.id) as inserted_parent_rows,
         (select count(*)::int from public.card_prints cp join pkg17e3_targets target on target.target_parent_id = cp.id and cp.printed_identity_modifier = target.target_printed_identity_modifier) as inserted_parent_modifier_rows,
         (select count(*)::int from public.card_print_identity cpi join pkg17e3_targets target on target.target_parent_id = cpi.card_print_id and cpi.is_active = true) as inserted_identity_rows,
         (select count(*)::int from public.card_printings cpr join pkg17e3_targets target on target.target_child_id = cpr.id) as inserted_child_rows,
         (select count(*)::int from public.card_printings cpr join pkg17e3_targets target on target.target_child_id = cpr.id and cpr.finish_key = 'stamped') as forbidden_stamped_child_rows`,
    ...
    return {
      dry_run_status: 'pkg17e3_failed_rolled_back',
...
function renderMarkdown(report) {
  return `# PKG-17E3 Stamped Active Finish Parent Insert Guarded Dry Run V1
...
${markdownTable(
    ...
    report.scope.targets.map((row) => [
      ...
      row.live_base_parent_child_finishes.join(', '),
    ]),
  )}
...
async function main() {
  ...
  if (!conn) {
    ...
    try {
      targets = await selectLiveTargets(client, readiness);
      if (targets.length !== EXPECTED_TARGET_COUNT) {
        ...
        execution = await runDryRun(client, targets, packageFingerprint(readiness, targets));
      }
    } finally {
  ...
  const recommended = execution.dry_run_status === 'pkg17e3_completed_rolled_back_no_durable_change' && execution.rollback_verified && execution.stop_findings.length === 0
  ...
  const report = {
    ...
    version: 'english_master_index_pkg17e3_stamped_active_finish_parent_insert_guarded_dry_run_v1',
    package_id: PACKAGE_ID,

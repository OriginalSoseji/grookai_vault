...
const STAGING_ASSET_DIR = path.join('tmp', 'nonproduction_image_staging', 'image_truth_v1', 'img05a');
const DRY_RUN_JSON = path.join(OUTPUT_DIR, 'image_truth_img05a_tcgplayer_representative_missing_display_dry_run_v1.json');
const DRY_RUN_MD = path.join(OUTPUT_DIR, 'image_truth_img05a_tcgplayer_representative_missing_display_dry_run_v1.md');

const PACKAGE_ID = 'IMG-05A-TCGPLAYER-REPRESENTATIVE-MISSING-DISPLAY-CHILD-IMAGE-DRY-RUN';
...
async function normalizeAsset(row) {
  ...
  const plannedStoragePath = [
    'warehouse-derived',
    'image-truth-v1',
    'img05a-tcgplayer-representative',
    row.set_code,
...
async function main() {
  ...
  const sourceRows = (manifest.rows ?? [])
    ...
    .filter((row) => row.source_key === 'tcgcsv_tcgplayer_catalog_identity')
  ...
  await fs.writeFile(DRY_RUN_MD, `# Image Truth IMG-05A TCGPlayer Representative Missing-Display Dry Run V1

Generated: ${report.generated_at}

Status: rollback-only dry run. No persisted DB writes. No migrations. No production image promotion.

This package is representative display coverage from TCGPlayer product imagery, not exact finish/variant imagery.

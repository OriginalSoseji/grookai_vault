...
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg15c_stamped_reverse_burger_king_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg15c_stamped_reverse_burger_king_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg15c_stamped_reverse_burger_king_real_apply_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260611_pkg15c_stamped_reverse_burger_king_real_apply_checkpoint_v1.md');
const CHECKPOINT_INDEX = path.join(CHECKPOINT_DIR, 'CHECKPOINT_INDEX.md');

const PACKAGE_ID = 'PKG-15C-STAMPED-REVERSE-BURGER-KING-PARENT-INSERTS';
const CREATED_BY = 'pkg15c_stamped_reverse_burger_king_real_apply_v1';
const SOURCE_PACKAGE_ID = 'PKG-15B-STAMPED-GENERIC-VARIANT-ADJUDICATION';
const EXPECTED_PACKAGE_FINGERPRINT = 'f33e5f765038d4c6736e10e6efe877a0cc6c01649f4d35e3a842f5aeaea0f053';
const EXPECTED_DRY_RUN_PROOF = '35f15587cc9e0d5c3d8704f77d3f6c138643f68ae27c48d2419b1e1b8c7f6911';
...
function validateDryRun(dryRun) {
  ...
  if (dryRun.execution?.dry_run_status !== 'pkg15c_stamped_reverse_burger_king_completed_rolled_back_no_durable_change') findings.push('dry_run_status_not_passed');
  ...
  if (counts.parent_inserts !== 8) findings.push('dry_run_parent_insert_scope_mismatch');
  if (counts.identity_inserts !== 8) findings.push('dry_run_identity_insert_scope_mismatch');
  if (counts.child_inserts !== 8) findings.push('dry_run_child_insert_scope_mismatch');
  ...
  if ((dryRun.scope?.targets ?? []).length !== 8) findings.push('dry_run_targets_not_8');
  return findings;
}
...
async function applyPackage(client, targets, fingerprint) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg15c_targets (
    ...
    const guard = await client.query(
      `with projection as (
         ...
         from pkg15c_targets target
         join public.card_prints base on base.id = target.base_parent_id
         left join public.sets s on s.id = base.set_id
       )
       select
         (select count(*)::int from pkg15c_targets) as target_count,
         (select count(distinct target_parent_id)::int from pkg15c_targets) as target_parent_count,
         (select count(distinct target_child_id)::int from pkg15c_targets) as target_child_count,
         (select count(*)::int from pkg15c_targets target left join public.card_prints base on base.id = target.base_parent_id where base.id is null) as missing_base_count,
         (select count(*)::int from pkg15c_targets target left join public.finish_keys fk on fk.key = target.target_finish_key and fk.is_active = true where fk.key is null) as inactive_finish_count,
         (select count(*)::int
          from pkg15c_targets target
          join public.card_prints base on base.id = target.base_parent_id
         ...
         (select count(*)::int from pkg15c_targets target join public.card_printings cpr on cpr.id = target.target_child_id) as child_collision_count,
         (select count(*)::int from pkg15c_targets target join public.card_print_identity cpi on cpi.card_print_id = target.target_parent_id and cpi.is_active = true) as identity_target_collision_count,
    ...
    const childInsert = await client.query(
      `insert into public.card_printings (
       ...
       select
         ...
         concat('Stamped identity child finish routed from adjudicated Burger King evidence: ', target.target_finish_key)
       from pkg15c_targets target`,
      [CREATED_BY],
    );
    ...
    const proof = await client.query(
      `select
         ...
         (select count(*)::int from pkg15c_targets) as target_rows,
         (select count(*)::int from public.card_prints cp join pkg15c_targets target on target.target_parent_id = cp.id) as inserted_parent_rows,
         (select count(*)::int from public.card_print_identity cpi join pkg15c_targets target on target.target_parent_id = cpi.card_print_id and cpi.is_active = true) as inserted_identity_rows,
         (select count(*)::int from public.card_printings cpr join pkg15c_targets target on target.target_child_id = cpr.id) as inserted_child_rows`,
      [PACKAGE_ID, fingerprint],
    );
    await client.query('commit');
    const afterSnapshot = await captureSnapshot(client, targets);
    return {
      apply_status: 'pkg15c_stamped_reverse_burger_king_real_apply_committed',
      guard: guardRow,
      proof: proof.rows[0],
      write_counts: {
...
function renderMarkdown(report) {
  ...
  return `# PKG-15C Stamped Reverse Burger King Real Apply V1

Approved real apply for deterministic Burger King stamped canonical parent inserts with adjudicated reverse child finish.
...
function checkpointMarkdown(report) {
  return `# PKG-15C Stamped Reverse Burger King Real Apply Checkpoint V1
...
const report = {
  ...
  version: 'english_master_index_pkg15c_stamped_reverse_burger_king_real_apply_v1',
  package_id: PACKAGE_ID,
  ...
  source_artifact: path.relative(ROOT, DRY_RUN_JSON),
  db_writes_performed: execution.apply_status === 'pkg15c_stamped_reverse_burger_king_real_apply_committed',
  durable_db_writes_performed: execution.apply_status === 'pkg15c_stamped_reverse_burger_king_real_apply_committed',

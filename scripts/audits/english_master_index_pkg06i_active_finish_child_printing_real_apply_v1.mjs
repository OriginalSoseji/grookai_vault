...
const GATE_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06i_active_finish_child_printing_real_apply_gate_v1.json');
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06i_active_finish_child_printing_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06i_active_finish_child_printing_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg06i_active_finish_child_printing_real_apply_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260609_pkg06i_active_finish_child_printing_real_apply_checkpoint_v1.md');

const PACKAGE_ID = 'PKG-06I-ACTIVE-FINISH-CHILD-PRINTING-INSERTS';
const SOURCE_READINESS_FINGERPRINT = '0999b51bc37a2430923cd021763b38504c5fb388c5741f5deb0c1d0329b86ac5';
const PACKAGE_FINGERPRINT = '99587da59b29b726112cd2806663442f4b5ab02d906bc7dd112931ded15b142c';
const SQL_HASH = 'a84a6a34b41c0b8f834d9ad1144c12345ccd7d66ccfa58fe3384eb2523f13b55';
const DRY_RUN_PROOF_HASH = 'c2ad3609b2926b35de68fc5a75a6b8b6ebbe268087377eba575ee01139eedffb';
const APPROVAL_TEXT = 'Approve real PKG-06I-ACTIVE-FINISH-CHILD-PRINTING-INSERTS apply only. Fingerprint: 99587da59b29b726112cd2806663442f4b5ab02d906bc7dd112931ded15b142c. SQL hash: a84a6a34b41c0b8f834d9ad1144c12345ccd7d66ccfa58fe3384eb2523f13b55. Scope: 84 child-only card_printing inserts for pop8/POP Series 8, sve/Scarlet & Violet Energies, svp/Scarlet & Violet Black Star Promos, xy3/Furious Fists, 2019sm/McDonald\'s Collection 2019, bw4/Next Destinies, sv10/Destined Rivals, swsh8/Fusion Strike, xy7/Ancient Origins, and xy8/BREAKthrough; finishes cosmos=57, normal=9, holo=9, reverse=9; target parents=78. Dry-run proof: c2ad3609b2926b35de68fc5a75a6b8b6ebbe268087377eba575ee01139eedffb == c2ad3609b2926b35de68fc5a75a6b8b6ebbe268087377eba575ee01139eedffb. No global apply. No migrations. No deletes. No merges. No unsupported cleanup. No parent writes.';
...
function validatePrerequisites({ gate, dryRun, rows }) {
  ...
  if (dryRun.dry_run_execution_status !== 'pkg06i_active_finish_child_printing_completed_rolled_back_no_durable_change') findings.push('dry_run_not_passed');
  ...
  if (rows.length !== EXPECTED_CHILD_ROWS) findings.push('target_row_count_not_84');
  if (new Set(rows.map((row) => row.card_print_id)).size !== EXPECTED_PARENT_ROWS) findings.push('target_parent_count_not_78');
...
function validateBeforeSnapshot({ beforeTargetSnapshot, beforeParentSnapshot, dryRun }) {
  ...
  if (beforeTargetSnapshot.counts.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('before_parent_count_not_78');
  ...
  if (beforeParentSnapshot.count !== EXPECTED_PARENT_ROWS) findings.push('before_parent_snapshot_count_not_78');
  return findings;
}

function validateAfterSnapshot({ afterTargetSnapshot, beforeParentSnapshot, afterParentSnapshot, insertedRows }) {
  const findings = [];
  if (afterTargetSnapshot.counts.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('after_parent_count_not_78');
  if (afterTargetSnapshot.counts.existing_target_child_rows !== EXPECTED_CHILD_ROWS) findings.push('after_existing_target_child_count_not_84');
  if (afterTargetSnapshot.counts.planned_id_collision_rows !== EXPECTED_CHILD_ROWS) findings.push('after_planned_id_collision_count_not_84');
  if (beforeParentSnapshot.hash_sha256 !== afterParentSnapshot.hash_sha256) findings.push('parent_snapshot_changed_unexpectedly');
  if (insertedRows.counts.inserted_rows_found !== EXPECTED_CHILD_ROWS) findings.push('inserted_rows_found_not_84');
...
async function applyPackage({ dryRun, rows }) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg06i_real_apply_targets (
    ...
    await client.query(
      `insert into pkg06i_real_apply_targets
       select
    ...
    const shape = await client.query(
      `select
       ...
       from pkg06i_real_apply_targets`,
    );
    ...
    const grouped = await client.query(
      `select 'set' as kind, set_key as key, count(*)::int as count from pkg06i_real_apply_targets group by set_key
       union all
       select 'finish' as kind, finish_key as key, count(*)::int as count from pkg06i_real_apply_targets group by finish_key`,
    );
    ...
    const lockParents = await client.query(
      `select cp.id
       from public.card_prints cp
       join (select distinct card_print_id from pkg06i_real_apply_targets) target on target.card_print_id = cp.id
       for update of cp`,
    );
    ...
    const finishGuard = await client.query(
      `select count(*)::int as unsupported_finish_count
       from pkg06i_real_apply_targets target
    ...
    return {
      connected: true,
      apply_status: 'pkg06i_active_finish_child_printing_real_apply_committed',
      error_message: null,
      before_target_snapshot: beforeTargetSnapshot,
    ...
    return {
      connected: true,
      apply_status: 'pkg06i_active_finish_child_printing_real_apply_failed_rolled_back',
...
function renderMarkdown(report) {
  return `# PKG-06I Active Finish Child Printing Real Apply V1
...
function renderCheckpoint(report) {
  return `# PKG-06I Active Finish Child Printing Real Apply Checkpoint V1
...
Real apply was scoped to 84 child-only card_printing inserts across 10 sets. No parent writes, migrations, deletes, merges, unsupported cleanup, or quarantine were performed.
`;
}

function updateCheckpointIndex() {
  const indexPath = path.join(CHECKPOINT_DIR, 'CHECKPOINT_INDEX.md');
  const line = '| 2026-06-09 | [PKG-06I Active Finish Child Printing Real Apply Checkpoint V1](20260609_pkg06i_active_finish_child_printing_real_apply_checkpoint_v1.md) | Records approved real apply for 84 active-finish child-only card_printing inserts across 10 sets; parent rows unchanged, no migrations. |';
  const current = fs.existsSync(indexPath) ? fs.readFileSync(indexPath, 'utf8') : '# Master Index Checkpoints\n';
  if (current.includes('20260609_pkg06i_active_finish_child_printing_real_apply_checkpoint_v1.md')) {
    fs.writeFileSync(indexPath, current.split('\n').map((existingLine) => (
      existingLine.includes('20260609_pkg06i_active_finish_child_printing_real_apply_checkpoint_v1.md') ? line : existingLine
    )).join('\n'));
  } else {
...
const report = {
  ...
  version: 'english_master_index_pkg06i_active_finish_child_printing_real_apply_v1',
  audit_only: false,
  apply_only: true,
  approval_scope: {
  ...
  apply_status: pass
    ? 'pkg06i_active_finish_child_printing_real_apply_committed_and_verified'
    : 'pkg06i_active_finish_child_printing_real_apply_failed_or_blocked',
  ...
  verification_summary: {
    ...
    master_index_comparison_status: pass ? 'pkg06i_active_finish_child_printing_verified_after_apply' : 'not_verified',
  },

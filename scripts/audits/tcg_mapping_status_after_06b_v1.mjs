...
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'tcg_mapping_status_after_06b_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'tcg_mapping_status_after_06b_v1.md');
const APPLY_JSON = path.join(OUTPUT_DIR, 'tcgmap06b_fresh_tcgcsv_mapping_insert_real_apply_v1.json');
...
function buildMarkdown(report) {
  const lines = [];
  lines.push('# TCG Mapping Status After 06B V1');
  lines.push('');
  lines.push('Post-apply status after `TCGMAP-06B-FRESH-TCGCSV-TCGPLAYER-MAPPING-INSERTS`.');
  lines.push('');
  lines.push('## Summary');
  lines.push('');
...
async function main() {
  ...
  const client = new Client({
    ...
    application_name: 'tcg_mapping_status_after_06b_v1',
  });
  ...
  try {
    ...
    const reportCore = {
      contract: 'TCG_MAPPING_STATUS_AFTER_06B_V1',
...
main().catch((error) => {
  console.error('[tcg-status-after-06b] failed:', error);
  process.exit(1);
});

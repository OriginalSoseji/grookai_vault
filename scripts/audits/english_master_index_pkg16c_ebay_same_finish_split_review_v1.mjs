...
const INPUT_JSON = path.join(
  ...
  'english_master_index_pkg16b_same_finish_stamped_split_readiness_v1.json',
);
const OUTPUT_DIR = path.join(
  ...
  'ebay_browse_same_finish_split_review_v1',
);
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'ebay_browse_same_finish_split_review_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'ebay_browse_same_finish_split_review_v1.md');

const PACKAGE_ID = 'PKG-16C-EBAY-BROWSE-SAME-FINISH-SPLIT-REVIEW';
const DEFAULT_ROW_LIMIT = 26;
...
function queryForRow(row) {
  const identityTerms = [
    ...
    row.card_number,
    row.set_name,
    row.stamp_label?.replace(/\bStamp\b/gi, '').trim(),
...
function validateListing(row, listing) {
  ...
  const checks = {
    ...
    variant: titleHasVariant(titleNorm, row.variant_key),
...
async function main() {
  ...
  const reportBase = {
    ...
    version: 'ebay_browse_same_finish_split_review_v1',
    ...
    summary: {
      rows_available_from_pkg16b: targetRows(input).length,

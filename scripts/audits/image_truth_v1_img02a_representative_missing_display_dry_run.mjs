...
const STAGING_ASSET_DIR = path.join('tmp', 'nonproduction_image_staging', 'image_truth_v1', 'img02a');
const DRY_RUN_JSON = path.join(OUTPUT_DIR, 'image_truth_img02a_representative_missing_display_dry_run_v1.json');
const DRY_RUN_MD = path.join(OUTPUT_DIR, 'image_truth_img02a_representative_missing_display_dry_run_v1.md');

const PACKAGE_ID = 'IMG-02A-MISSING-DISPLAY-REPRESENTATIVE-CHILD-IMAGE-DRY-RUN';
...
async function fetchBuffer(url) {
  ...
  if (process.platform === 'win32') {
    ...
    let powershellWroteFile = false;
    try {
      await execFileAsync('powershell.exe', ['-NoProfile', '-Command', command, url, tempPath], {
        timeout: 45000,
        maxBuffer: 1024 * 1024,
      });
      powershellWroteFile = await fs.stat(tempPath).then((stat) => stat.size > 0, () => false);
    } catch {
      powershellWroteFile = false;
    }
    if (!powershellWroteFile) {
      await execFileAsync('curl.exe', ['--ssl-no-revoke', '-L', '--max-time', '45', '-o', tempPath, url], {
        timeout: 60000,
        maxBuffer: 1024 * 1024,
      });
    }
...
async function normalizeAsset(row) {
  ...
  const plannedStoragePath = [
    'warehouse-derived',
    'image-truth-v1',
    'img02a-missing-display-representative',
    row.set_code,
...
async function main() {
  ...
  await fs.writeFile(DRY_RUN_MD, `# Image Truth IMG-02A Representative Missing-Display Dry Run V1

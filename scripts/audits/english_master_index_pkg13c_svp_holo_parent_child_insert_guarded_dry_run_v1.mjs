...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg13c_svp_holo_parent_child_insert_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg13c_svp_holo_parent_child_insert_guarded_dry_run_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260610_pkg13c_svp_holo_parent_child_insert_guarded_dry_run_checkpoint_v1.md');
...
const CREATED_BY = 'pkg13c_svp_holo_parent_child_insert_guarded_dry_run_v1';
...
const TARGETS = [
  {
    ...
    mapping: {
      source: 'tcgplayer',
      external_id: '655095',
      source_url: 'https://www.tcgplayer.com/product/655095/pokemon-sv-scarlet-and-violet-promo-cards-espeon-ex-175',
    },
    evidence_urls: [
      'https://www.tcgplayer.com/product/655095/pokemon-sv-scarlet-and-violet-promo-cards-espeon-ex-175',
      'https://www.pricecharting.com/game/pokemon-promo/espeon-ex-175',
      'https://reverseholo.app/sets/svp',
      'https://www.thepricedex.com/set/svp/scarlet-and-violet-black-star-promos/price-list',
    ],
    tcgdex_conflict_url: 'https://api.tcgdex.net/v2/en/cards/svp-175',
    evidence_note: 'Exact marketplace/price-list evidence supports SVP 175 Espeon ex as holo; TCGdex currently reports normal and is recorded as a conflicting structured finish claim.',
  },
  {
    set_key: 'svp',
    live_set_code: 'svp',
    ...
    mapping: {
      source: 'tcgplayer',
      external_id: '655094',
      source_url: 'https://www.tcgplayer.com/product/655094/pokemon-sv-scarlet-and-violet-promo-cards-umbreon-ex-176',
    },
    evidence_urls: [
      'https://www.tcgplayer.com/product/655094/pokemon-sv-scarlet-and-violet-promo-cards-umbreon-ex-176',
      'https://www.pricecharting.com/game/pokemon-promo/umbreon-ex-176',
      'https://reverseholo.app/sets/svp',
      'https://www.thepricedex.com/set/svp/scarlet-and-violet-black-star-promos/price-list',
    ],
    tcgdex_conflict_url: 'https://api.tcgdex.net/v2/en/cards/svp-176',
    evidence_note: 'Exact marketplace/price-list evidence supports SVP 176 Umbreon ex as holo; TCGdex currently reports normal and is recorded as a conflicting structured finish claim.',
  },
];
...
async function resolveSet(client) {
  const result = await client.query(
    `select id::text as set_id, code, name
     from public.sets
     where game = 'pokemon' and lower(code) = 'svp'
     order by id`,
  );
  if (result.rows.length !== 1) throw new Error(`expected exactly one svp set row, found ${result.rows.length}`);
...
function buildPlan(setRow) {
  const parentRows = TARGETS.map((row) => ({
    ...
    ai_metadata: {
      ...
      package_id: PACKAGE_ID,
      finish_adjudication: 'human_marketplace_holo_over_tcgdx_normal_conflict',
      evidence_note: row.evidence_note,
      evidence_urls: row.evidence_urls,
      conflicting_structured_source: {
        source: 'tcgdex',
        source_url: row.tcgdex_conflict_url,
        claimed_finish: 'normal',
        handling: 'not used as finish authority for this insert',
      },
    },
    evidence_urls: row.evidence_urls,
    mapping: row.mapping,
  ...
  const childRows = parentRows.map((row) => ({
    ...
    provenance_ref: `${row.set_key}:${row.card_number}:holo`,
...
async function runDryRun(client, plan, fingerprint) {
  ...
  try {
    ...
    return {
      dry_run_status: 'pkg13c_svp_holo_parent_child_insert_failed_rolled_back',
...
function renderMarkdown(report) {
  return `# PKG-13C SVP Holo Parent+Child Insert Guarded Dry Run V1
...
${markdownTable(
  ...
  report.scope.parent_rows.map((row) => [
    ...
    `tcgdex normal: ${row.tcgdex_conflict_url}`,
  ]),
)}
...
async function updateCheckpointIndex() {
  const line = '| 2026-06-10 | [PKG-13C SVP Holo Parent+Child Insert Guarded Dry Run Checkpoint V1](20260610_pkg13c_svp_holo_parent_child_insert_guarded_dry_run_checkpoint_v1.md) | Rollback-only dry run for two SVP TCGplayer-backed holo parent+child inserts, with TCGdex normal conflict documented. No durable writes or migrations. |';
  let current = '';
  try {
  ...
  if (!current.includes('20260610_pkg13c_svp_holo_parent_child_insert_guarded_dry_run_checkpoint_v1.md')) {
...
async function main() {
  ...
  try {
    ...
    const report = {
      ...
      version: 'english_master_index_pkg13c_svp_holo_parent_child_insert_guarded_dry_run_v1',
      package_id: PACKAGE_ID,
    ...
    await writeText(CHECKPOINT_MD, `# PKG-13C SVP Holo Parent+Child Insert Guarded Dry Run Checkpoint V1

...
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'preserved_finish_child_insert_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'preserved_finish_child_insert_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'preserved_finish_child_insert_real_apply_v1.md');

const PACKAGE_ID = 'MISSING-PROMO-04H-PRESERVED-FINISH-CHILD-INSERTS';
const CREATED_BY = 'english_master_index_missing_promo_04h_preserved_finish_child_insert_real_apply_v1';
const APPROVED_FINGERPRINT = '3b8d5f93711ecd9ab0d8af5458625464f4c174ba7df48cdd0979cfd68acec7ad';
const APPROVED_SQL_HASH = '7f490cfc8cf1c8d3820e47013373b7e74f5d4edb449a5f72c6a7030db409fe15';
const APPROVED_DRY_RUN_PROOF = '5a2a8d9d53a11daa2287f98d9680c01344d3eb3fd4c9cd62e9e2910a52128f10';
...
function assertApproved(dryRun) {
  ...
  if (dryRun.rollback_proof_sha256 !== APPROVED_DRY_RUN_PROOF) errors.push('rollback proof mismatch');
  if (dryRun.durable_after_snapshot_matches_before_snapshot !== true) errors.push('rollback proof did not match before snapshot');
  if (dryRun.summary?.target_count !== 3) errors.push('target count mismatch');
  if (dryRun.summary?.by_finish?.reverse !== 3) errors.push('finish scope mismatch');
...
function renderMarkdown(report) {
  return [
    '# Preserved Finish Child Insert Real Apply V1',
    '',
...
async function main() {
  ...
  try {
    ...
    const report = {
      ...
      input_artifacts: {
        preserved_finish_child_insert_guarded_dry_run: rel(DRY_RUN_JSON),
      },

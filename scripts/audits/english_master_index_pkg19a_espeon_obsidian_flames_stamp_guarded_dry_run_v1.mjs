...
const READINESS_JSON = path.join(AUDIT_DIR, 'english_master_index_remaining_missing_reconciliation_lanes_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg19a_espeon_obsidian_flames_stamp_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg19a_espeon_obsidian_flames_stamp_guarded_dry_run_v1.md');

const PACKAGE_ID = 'PKG-19A-ESPEON-OBSIDIAN-FLAMES-STAMP-PARENT-INSERT';
const CREATED_BY = 'pkg19a_espeon_obsidian_flames_stamp_guarded_dry_run_v1';
const EXPECTED_TARGET_COUNT = 1;
const BASE_PARENT_ID = '067019a2-583e-4829-b498-5eb6e795ba8d';
const SET_KEY = 'sv03';
const SET_NAME = 'Obsidian Flames';
const CARD_NUMBER = '086';
const CARD_NAME = 'Espeon';
const TARGET_FINISH_KEY = 'reverse';
const TARGET_VARIANT_KEY = 'obsidian_flames_stamp';
const STAMP_LABEL = 'Obsidian Flames Stamp';
...
function buildTargets(readiness) {
  const targetRow = (readiness.rows ?? []).find((row) => (
    normalizeText(row.set_key) === SET_KEY
    && normalizeText(row.card_name) === normalizeText(CARD_NAME)
    && String(row.card_number ?? '').replace(/^0+/, '') === String(CARD_NUMBER).replace(/^0+/, '')
    && normalizeText(row.finish_key) === 'stamped'
  ));
  if (!targetRow) return [];
  return [{
    target_parent_id: uuidFromSeed(`${PACKAGE_ID}:parent:${SET_KEY}:${CARD_NUMBER}:${CARD_NAME}:${TARGET_VARIANT_KEY}`),
    target_child_id: uuidFromSeed(`${PACKAGE_ID}:child:${SET_KEY}:${CARD_NUMBER}:${CARD_NAME}:${TARGET_VARIANT_KEY}:${TARGET_FINISH_KEY}`),
    ...
    target_printed_identity_modifier: TARGET_VARIANT_KEY,
    stamp_label: STAMP_LABEL,
    source_url: 'https://bulbapedia.bulbagarden.net/wiki/Obsidian_Flames_%28TCG%29',
    evidence_label: 'Bulbapedia Additional Cards lists Reverse Holo Obsidian Flames stamp Best Buy gift with purchase exclusive; TCGCollector and ThePriceDex support Obsidian Flames Stamp variant.',
    evidence: {
      readiness_package_id: PACKAGE_ID,
      readiness_fingerprint_sha256: readiness.generated_at ?? null,
      source_keys: ['bulbapedia_catalog_compare', ...(targetRow.sources ?? [])],
      source_urls: [
        'https://bulbapedia.bulbagarden.net/wiki/Obsidian_Flames_%28TCG%29',
        ...(targetRow.evidence_urls ?? []),
      ],
      evidence_label: 'Reverse Holo Obsidian Flames stamp Best Buy gift with purchase exclusive.',
      raw_snapshot_ref: 'bulbapedia_catalog_compare_v1:name_mismatch.csv:sv03:086:Espeon:Additional Cards',
      stamp_label: STAMP_LABEL,
      variant_key: TARGET_VARIANT_KEY,
      active_child_finish_key: TARGET_FINISH_KEY,
    },
  }];
}
...
async function runDryRun(client, targets, fingerprint) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg19a_targets (
    ...
    const guard = await client.query(
      `with projection as (
         ...
         from pkg19a_targets target
         join public.card_prints base on base.id = target.base_parent_id
         left join public.sets s on s.id = base.set_id
       )
       select
         (select count(*)::int from pkg19a_targets) as target_count,
         (select count(distinct target_parent_id)::int from pkg19a_targets) as target_parent_count,
         (select count(distinct target_child_id)::int from pkg19a_targets) as target_child_count,
         (select count(*)::int from pkg19a_targets target left join public.card_prints base on base.id = target.base_parent_id where base.id is null) as missing_base_count,
         (select count(*)::int from pkg19a_targets target left join public.finish_keys fk on fk.key = target.target_finish_key and fk.is_active = true where fk.key is null) as inactive_finish_count,
         (select count(*)::int
          from pkg19a_targets target
          join public.card_prints base on base.id = target.base_parent_id
         ...
         (select count(*)::int from pkg19a_targets target join public.card_printings cpr on cpr.id = target.target_child_id) as child_collision_count,
         (select count(*)::int from pkg19a_targets target join public.card_print_identity cpi on cpi.card_print_id = target.target_parent_id and cpi.is_active = true) as identity_target_collision_count,
    ...
    const childInsert = await client.query(
      `insert into public.card_printings (
       ...
       select
         ...
         concat(target.set_key, ':', target.card_number, ':obsidian_flames_stamp:', target.target_variant_key, ':', target.target_finish_key),
         $1::text,
         null, null, null, null, null,
         'representative_shared_stamp',
         concat('Stamped identity child finish routed from Obsidian Flames Stamp evidence: ', target.target_finish_key)
       from pkg19a_targets target`,
      [CREATED_BY],
    );
    ...
    const proof = await client.query(
      `select
         ...
         (select count(*)::int from pkg19a_targets) as target_rows,
         (select count(*)::int from public.card_prints cp join pkg19a_targets target on target.target_parent_id = cp.id) as inserted_parent_rows,
         (select count(*)::int from public.card_prints cp join pkg19a_targets target on target.target_parent_id = cp.id and cp.printed_identity_modifier = target.target_printed_identity_modifier) as inserted_parent_modifier_rows,
         (select count(*)::int from public.card_print_identity cpi join pkg19a_targets target on target.target_parent_id = cpi.card_print_id and cpi.is_active = true) as inserted_identity_rows,
         (select count(*)::int from public.card_printings cpr join pkg19a_targets target on target.target_child_id = cpr.id) as inserted_child_rows,
         (select count(*)::int from public.card_printings cpr join pkg19a_targets target on target.target_child_id = cpr.id and cpr.finish_key = 'stamped') as forbidden_stamped_child_rows`,
    ...
    return {
      dry_run_status: 'pkg19a_failed_rolled_back',
...
function renderMarkdown(report) {
  ...
  return `# PKG-19A Espeon Obsidian Flames Stamp Guarded Dry Run V1
...
async function main() {
  ...
  const recommended = execution.dry_run_status === 'pkg19a_completed_rolled_back_no_durable_change' && execution.rollback_verified && execution.stop_findings.length === 0
  ...
  const report = {
    ...
    version: 'english_master_index_pkg19a_espeon_obsidian_flames_stamp_guarded_dry_run_v1',
    package_id: PACKAGE_ID,

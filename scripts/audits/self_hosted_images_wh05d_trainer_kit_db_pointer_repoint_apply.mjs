...
const WH05C_SUMMARY_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh05c_trainer_kit_db_pointer_repoint_dry_run_summary_v1.json');
const WH05C_PLAN_JSONL = path.join(OUTPUT_DIR, 'self_hosted_images_wh05c_trainer_kit_db_pointer_repoint_plan_v1.jsonl');
const PLAN_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh05d_trainer_kit_db_pointer_repoint_apply_plan_v1.json');
const PLAN_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh05d_trainer_kit_db_pointer_repoint_apply_plan_v1.md');
const RESULT_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh05d_trainer_kit_db_pointer_repoint_apply_result_v1.json');
const RESULT_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh05d_trainer_kit_db_pointer_repoint_apply_result_v1.md');
const PACKAGE_ID = 'IMG-HOST-WH-05D-TRAINER-KIT-DB-POINTER-REPOINT-APPLY';
const APPROVED_SOURCE_FINGERPRINT = '6fae84292d3c52afd0806d52de338398a7bc10f8d68cd8b2af9a528b0c962459';
...
function parseArgs(argv) {
  const args = {
    apply: false,
    fingerprint: null,
    chunkSize: Number.parseInt(process.env.TRAINER_KIT_DB_REPOINT_CHUNK_SIZE ?? '250', 10),
  };
...
function buildUpdates(summary, planRows) {
  ...
  if (!summary.ready_for_apply_package) {
    stopFindings.push(`wh05c_not_apply_ready:${(summary.stop_findings ?? []).join(',')}`);
  }
  ...
  for (const row of planRows) {
    ...
    if (!String(row.set_code ?? '').startsWith('tk-')) {
      stopFindings.push(`non_trainer_kit_row:${row.target_row_id}`);
      continue;
    }
...
function buildPlan(args, summary, updates) {
  const plan = {
    ...
    source_plan_jsonl: path.relative(ROOT, WH05C_PLAN_JSONL),
...
${markdownTable(topEntries(plan.set_codes, 40))}
`;
}
...
async function main() {
  ...
  const wh05cSummary = await readJson(WH05C_SUMMARY_JSON);
  const wh05cPlanRows = await readJsonl(WH05C_PLAN_JSONL);
  const updates = buildUpdates(wh05cSummary, wh05cPlanRows);
  const plan = buildPlan(args, wh05cSummary, updates);

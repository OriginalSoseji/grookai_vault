...
const SOURCE_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg40b_residual_stamped_active_finish_route_evidence_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg40b_residual_stamped_identity_backfill_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg40b_residual_stamped_identity_backfill_guarded_dry_run_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260614_pkg40b_residual_stamped_identity_backfill_guarded_dry_run_checkpoint_v1.md');

const PACKAGE_ID = 'PKG-40B-RESIDUAL-STAMPED-IDENTITY-BACKFILL';
...
async function runDryRun(client, targets, packageFingerprint) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg40b_residual_stamped_identity_targets (
    ...
    await client.query(
      `insert into pkg40b_residual_stamped_identity_targets
       select *
    ...
    const guard = await client.query(
      `with projection as (
         ...
         from pkg40b_residual_stamped_identity_targets target
         join public.card_prints cp on cp.id = target.card_print_id
         left join public.sets s on s.id = cp.set_id
       )
       select
         (select count(*)::int from pkg40b_residual_stamped_identity_targets) as target_count,
         (select count(distinct card_print_id)::int from pkg40b_residual_stamped_identity_targets) as target_parent_count,
         (select count(*)::int
          from pkg40b_residual_stamped_identity_targets target
       ...
       set
         ...
         ai_metadata = coalesce(cp.ai_metadata, '{}'::jsonb) || jsonb_build_object(
           'verified_master_index_identity_backfill', jsonb_build_object(
             ...
             'residual_stamped_identity_backfill', true
           )
         )
       from pkg40b_residual_stamped_identity_targets target
...
function renderMarkdown(report) {
  return `# PKG-40B Residual Stamped Identity Backfill Guarded Dry Run V1
...
if (targets.length !== 8) throw new Error(`Expected 8 PKG-40B identity targets, found ${targets.length}`);
...
const recommendedApproval = `Approve real ${PACKAGE_ID} apply only. Fingerprint: ${packageFingerprint}. Scope: ${targets.length} residual stamped parent identity backfills, ${targets.length} parent printed_identity_modifier updates, ${targets.length} active card_print_identity inserts, 0 child inserts, 0 deletes, 0 merges. Dry-run proof: ${dryRunProofHash} == ${dryRun.afterSnapshot.hash_sha256}. No global apply. No migrations. No cleanup. No quarantine.`;

const report = {
  generated_at: new Date().toISOString(),
  version: 'english_master_index_pkg40b_residual_stamped_identity_backfill_guarded_dry_run_v1',
  package_id: PACKAGE_ID,
  ...
  dry_run_status: 'pkg40b_residual_stamped_identity_backfill_completed_rolled_back_no_durable_change',

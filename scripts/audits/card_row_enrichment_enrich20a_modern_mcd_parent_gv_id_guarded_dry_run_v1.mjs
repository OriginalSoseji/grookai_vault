...
const OUTPUT_JSON = path.join(OUTPUT_DIR, 'enrich20a_modern_mcd_parent_gv_id_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(OUTPUT_DIR, 'enrich20a_modern_mcd_parent_gv_id_guarded_dry_run_v1.md');
...
async function runRollbackDryRun(client, targets) {
  ...
  try {
    ...
    if (
      ...
      guard.bad_2023_namespace_count !== 0 ||
      guard.bad_2024_namespace_count !== 0 ||
...
async function main() {
  ...
  try {
    ...
    if (preflight.bad_2023_namespace_count !== 0) stop_findings.push('bad_2023_mcd_namespace');
    if (preflight.bad_2024_namespace_count !== 0) stop_findings.push('bad_2024_mcd_namespace');
    ...
    const report = {
      version: 'ENRICH20A_MODERN_MCD_PARENT_GV_ID_GUARDED_DRY_RUN_V1',
      package_id: PACKAGE_ID,
      ...
      namespace_governance: {
        contract: 'mcd_namespace_contract_v1',
        registry_patch: ['2023sv -> 2023', '2024sv -> 2024'],
      },
      preflight,
      execution,
      ...
      recommended_approval_text: stop_findings.length === 0
        ? `Approve real ${PACKAGE_ID} apply only. Fingerprint: ${packageFingerprint}. Scope: ${targets.length} modern McDonald's parent card_print gv_id updates using governed MCD year namespaces 2023/2024. Dry-run proof: ${execution.before_snapshot.hash_sha256} == ${execution.after_rollback_snapshot.hash_sha256}. No child writes. No identity writes. No external mapping writes. No species writes. No deletes. No merges. No migrations. No image writes. No global apply.`
        : null,
    };

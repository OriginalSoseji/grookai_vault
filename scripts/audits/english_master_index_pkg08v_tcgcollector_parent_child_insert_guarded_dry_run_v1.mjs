...
const SOURCE_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg08u_tcgcollector_mapping_governance_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg08v_tcgcollector_parent_child_insert_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg08v_tcgcollector_parent_child_insert_guarded_dry_run_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260610_pkg08v_tcgcollector_parent_child_insert_guarded_dry_run_checkpoint_v1.md');

const PACKAGE_ID = 'PKG-08V-TCGCOLLECTOR-PARENT-CHILD-INSERTS';
const CREATED_BY = 'pkg08v_tcgcollector_parent_child_insert_guarded_dry_run_v1';
const PROVENANCE_SOURCE = 'verified_master_set_index_v1';
const READY_STATUS = 'governed_mapping_carrier_ready_for_dry_run';
...
function parentKey(row) {
  return [normalizeText(row.matched_set_alias), normalizeNumber(row.card_number), normalizeText(row.card_name)].join('|');
}

function numberIdentityParts(cardNumber) {
  const raw = String(cardNumber ?? '').trim();
  const [head] = raw.split('/');
  const match = head.match(/^([A-Za-z]+)(\d+[a-z]?)$/);
  if (!match) {
    return {
      number_plain: normalizeNumber(cardNumber),
      printed_identity_modifier: null,
    };
  }
  return {
    number_plain: match[2].replace(/^0+(?=\d)/, ''),
    printed_identity_modifier: `number_prefix:${match[1].toUpperCase()}`,
  };
}
...
function buildTargets({ sourceRows, setByAlias }) {
  ...
  for (const row of sourceRows) {
    ...
    if (!mapping?.source || !mapping?.external_id) {
      blockedRows.push({ ...row, blocked_reason: 'preferred_external_mapping_missing' });
      continue;
    }
    if (row.existing_source_mapping_collisions?.length) {
      blockedRows.push({ ...row, blocked_reason: 'preferred_external_mapping_collision_present' });
      continue;
    }
    ...
    if (!parentByKey.has(key)) {
      ...
      const identity = numberIdentityParts(row.card_number);
      parentByKey.set(key, {
        ...
        printed_identity_modifier: identity.printed_identity_modifier,
...
async function runDryRun(client, planned, packageFingerprint) {
  ...
  try {
    ...
    return {
      status: 'pkg08v_tcgcollector_parent_child_insert_completed_rolled_back_no_durable_change',
      error_message: null,
    ...
    return {
      status: 'pkg08v_tcgcollector_parent_child_insert_failed_rolled_back',
...
function renderMarkdown(report) {
  return `# PKG-08V TCGCollector Parent+Child Insert Guarded Dry Run V1
...
- source_readiness_fingerprint_sha256: \`${report.source_readiness_fingerprint_sha256 ?? 'missing'}\`
...
async function main() {
  ...
  const sourceRows = (source.rows ?? []).filter((row) => row.governance_status === READY_STATUS);
  ...
  if (!conn) {
    report = {
      ...
      version: 'english_master_index_pkg08v_tcgcollector_parent_child_insert_guarded_dry_run_v1',
      package_id: PACKAGE_ID,
    ...
    try {
      ...
      const stopFindings = [];
      if (source.package_id !== 'PKG-08U-TCGCOLLECTOR-MAPPING-GOVERNANCE') stopFindings.push('source_governance_package_mismatch');
      if (planned.parentRows.length !== 104) stopFindings.push('target_parent_count_not_104');
      if (planned.childRows.length !== 104) stopFindings.push('target_child_count_not_104');
      if (planned.mappingRows.length !== 104) stopFindings.push('target_mapping_count_not_104');
      if (planned.blockedRows.length !== 0) stopFindings.push('planned_blocked_rows_present');
      if (execution.status !== 'pkg08v_tcgcollector_parent_child_insert_completed_rolled_back_no_durable_change') stopFindings.push('dry_run_not_passed');
      ...
      report = {
        ...
        recommended_real_apply_approval_text: stopFindings.length === 0
          ? `Approve real PKG-08V-TCGCOLLECTOR-PARENT-CHILD-INSERTS apply only. Fingerprint: ${packageFingerprint}. Scope: ${planned.parentRows.length} parent card_print inserts, ${planned.childRows.length} child card_printing inserts, ${planned.mappingRows.length} TCGCollector external mappings across ${Object.keys(countBy(planned.childRows, (row) => row.set_key)).length} sets; finishes ${Object.entries(byFinish).map(([finish, count]) => `${finish}=${count}`).join(', ')}. Dry-run proof: ${execution.before_snapshot?.hash_sha256} == ${execution.after_snapshot?.hash_sha256}. No global apply. No migrations. No deletes. No merges. No unsupported cleanup.`
          : null,
      };
    } finally {

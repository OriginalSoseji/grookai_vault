...
const GATE_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06n_active_finish_child_printing_real_apply_gate_v1.json');
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06n_active_finish_child_printing_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg06n_active_finish_child_printing_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg06n_active_finish_child_printing_real_apply_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260609_pkg06n_active_finish_child_printing_real_apply_checkpoint_v1.md');

const PACKAGE_ID = 'PKG-06N-ACTIVE-FINISH-CHILD-PRINTING-INSERTS';
const SOURCE_READINESS_FINGERPRINT = '03593d1d1bc85e794ee1f4327bd26f4061ec3cf2eac2a20fc60b690bae1274dc';
const PACKAGE_FINGERPRINT = 'a21913de6bb88b867dfd8d081cc4e0c2a813feaa4f48fba62a129296bf713987';
const SQL_HASH = '5014306c5de0e4a26af224b91b55768908eeb05b8037774a4cd099b33e5467d5';
const DRY_RUN_PROOF_HASH = 'a9a30e6f53529bc0230ae3e85c2788a9224356849983175fa973b2f2b7f76546';
const APPROVAL_TEXT = 'Approve real PKG-06N-ACTIVE-FINISH-CHILD-PRINTING-INSERTS apply only. Fingerprint: a21913de6bb88b867dfd8d081cc4e0c2a813feaa4f48fba62a129296bf713987. SQL hash: 5014306c5de0e4a26af224b91b55768908eeb05b8037774a4cd099b33e5467d5. Scope: 21 child-only card_printing inserts for xy9/BREAKpoint, 2011bw/McDonald\'s Collection 2011, bw2/Emerging Powers, bw5/Dark Explorers, bw9/Plasma Freeze, dp4/Great Encounters, dp7/Stormfront, dv1/Dragon Vault, hgssp/HGSS Black Star Promos, and neo1/Neo Genesis; finishes cosmos=15, holo=4, normal=1, reverse=1; target parents=21. Dry-run proof: a9a30e6f53529bc0230ae3e85c2788a9224356849983175fa973b2f2b7f76546 == a9a30e6f53529bc0230ae3e85c2788a9224356849983175fa973b2f2b7f76546. No global apply. No migrations. No deletes. No merges. No unsupported cleanup. No parent writes.';

const EXPECTED_CHILD_ROWS = 21;
const EXPECTED_PARENT_ROWS = 21;
...
const EXPECTED_FINISH_COUNTS = { cosmos: 15, holo: 4, normal: 1, reverse: 1 };
...
function validatePrerequisites({ gate, dryRun, rows }) {
  ...
  if (dryRun.dry_run_execution_status !== 'pkg06n_active_finish_child_printing_completed_rolled_back_no_durable_change') findings.push('dry_run_not_passed');
  ...
  if (rows.length !== EXPECTED_CHILD_ROWS) findings.push('target_row_count_not_21');
  if (new Set(rows.map((row) => row.card_print_id)).size !== EXPECTED_PARENT_ROWS) findings.push('target_parent_count_not_21');
...
function validateBeforeSnapshot({ beforeTargetSnapshot, beforeParentSnapshot, dryRun }) {
  ...
  if (beforeTargetSnapshot.counts.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('before_parent_count_not_21');
  ...
  if (beforeParentSnapshot.count !== EXPECTED_PARENT_ROWS) findings.push('before_parent_snapshot_count_not_21');
  return findings;
}

function validateAfterSnapshot({ afterTargetSnapshot, beforeParentSnapshot, afterParentSnapshot, insertedRows }) {
  const findings = [];
  if (afterTargetSnapshot.counts.target_parent_rows !== EXPECTED_PARENT_ROWS) findings.push('after_parent_count_not_21');
  if (afterTargetSnapshot.counts.existing_target_child_rows !== EXPECTED_CHILD_ROWS) findings.push('after_existing_target_child_count_not_21');
  if (afterTargetSnapshot.counts.planned_id_collision_rows !== EXPECTED_CHILD_ROWS) findings.push('after_planned_id_collision_count_not_21');
  if (beforeParentSnapshot.hash_sha256 !== afterParentSnapshot.hash_sha256) findings.push('parent_snapshot_changed_unexpectedly');
  if (insertedRows.counts.inserted_rows_found !== EXPECTED_CHILD_ROWS) findings.push('inserted_rows_found_not_21');
...
async function applyPackage({ dryRun, rows }) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg06n_real_apply_targets (
    ...
    await client.query(
      `insert into pkg06n_real_apply_targets
       select
    ...
    const shape = await client.query(
      `select
       ...
       from pkg06n_real_apply_targets`,
    );
    ...
    const grouped = await client.query(
      `select 'set' as kind, set_key as key, count(*)::int as count from pkg06n_real_apply_targets group by set_key
       union all
       select 'finish' as kind, finish_key as key, count(*)::int as count from pkg06n_real_apply_targets group by finish_key`,
    );
    ...
    const lockParents = await client.query(
      `select cp.id
       from public.card_prints cp
       join (select distinct card_print_id from pkg06n_real_apply_targets) target on target.card_print_id = cp.id
       for update of cp`,
    );
    ...
    const finishGuard = await client.query(
      `select count(*)::int as unsupported_finish_count
       from pkg06n_real_apply_targets target
    ...
    return {
      connected: true,
      apply_status: 'pkg06n_active_finish_child_printing_real_apply_committed',
      error_message: null,
      before_target_snapshot: beforeTargetSnapshot,
    ...
    return {
      connected: true,
      apply_status: 'pkg06n_active_finish_child_printing_real_apply_failed_rolled_back',
...
function renderMarkdown(report) {
  return `# PKG-06N Active Finish Child Printing Real Apply V1
...
function renderCheckpoint(report) {
  return `# PKG-06N Active Finish Child Printing Real Apply Checkpoint V1
...
Real apply was scoped to 21 child-only card_printing inserts across 10 sets. No parent writes, migrations, deletes, merges, unsupported cleanup, or quarantine were performed.
`;
}

function updateCheckpointIndex() {
  const indexPath = path.join(CHECKPOINT_DIR, 'CHECKPOINT_INDEX.md');
  const line = '| 2026-06-09 | [PKG-06N Active Finish Child Printing Real Apply Checkpoint V1](20260609_pkg06n_active_finish_child_printing_real_apply_checkpoint_v1.md) | Records approved real apply for 21 active-finish child-only card_printing inserts across 10 sets; parent rows unchanged, no migrations. |';
  const current = fs.existsSync(indexPath) ? fs.readFileSync(indexPath, 'utf8') : '# Master Index Checkpoints\n';
  if (current.includes('20260609_pkg06n_active_finish_child_printing_real_apply_checkpoint_v1.md')) {
    fs.writeFileSync(indexPath, current.split('\n').map((existingLine) => (
      existingLine.includes('20260609_pkg06n_active_finish_child_printing_real_apply_checkpoint_v1.md') ? line : existingLine
    )).join('\n'));
  } else {
...
const report = {
  ...
  version: 'english_master_index_pkg06n_active_finish_child_printing_real_apply_v1',
  audit_only: false,
  apply_only: true,
  approval_scope: {
  ...
  apply_status: pass
    ? 'pkg06n_active_finish_child_printing_real_apply_committed_and_verified'
    : 'pkg06n_active_finish_child_printing_real_apply_failed_or_blocked',
  ...
  verification_summary: {
    ...
    master_index_comparison_status: pass ? 'pkg06n_active_finish_child_printing_verified_after_apply' : 'not_verified',
  },

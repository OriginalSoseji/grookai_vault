...
const DRY_RUN_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg23b_subset_parallel_base_finish_overgeneration_child_delete_guarded_dry_run_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg23b_subset_parallel_base_finish_overgeneration_child_delete_real_apply_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg23b_subset_parallel_base_finish_overgeneration_child_delete_real_apply_v1.md');

const PACKAGE_ID = 'PKG-23B-SUBSET-PARALLEL-BASE-FINISH-OVERGENERATION-CHILD-DELETE';
const PACKAGE_FINGERPRINT = '3e08b1f570eb7c46257e2889d33cdaf483a4c5450deaf2c6800044c98bf023fa';
const DRY_RUN_PROOF_HASH = 'd269c1fd3eef94de8a5c966bede541a39ea900f3b476ebe1e7a5a0dbf9473c63';
const APPROVAL_TEXT = 'Approve real PKG-23B-SUBSET-PARALLEL-BASE-FINISH-OVERGENERATION-CHILD-DELETE apply only. Fingerprint: 3e08b1f570eb7c46257e2889d33cdaf483a4c5450deaf2c6800044c98bf023fa. Scope: 592 subset/parallel base finish-overgeneration child deletes; finishes holo=286, normal=174, reverse=132; top sets swsh45sv=194, bw11=108, g1=96, col1=87, pl3=86, swsh9tg=16, cel25=3, pl1=1. Dry-run proof: d269c1fd3eef94de8a5c966bede541a39ea900f3b476ebe1e7a5a0dbf9473c63 == d269c1fd3eef94de8a5c966bede541a39ea900f3b476ebe1e7a5a0dbf9473c63. No global apply. No migrations. No parent writes. No merges. No quarantine. Variant/modifier rows excluded.';
const TARGET_BUCKET = 'subset_parallel_base_finish_overgeneration_candidate';
...
function validateDryRun(dryRun) {
  ...
  if (dryRun.scope?.target_child_deletes !== 592) findings.push(`scope_target_child_deletes_not_592:${dryRun.scope?.target_child_deletes}`);
  if (dryRun.scope?.by_finish?.holo !== 286) findings.push('scope_holo_not_286');
  if (dryRun.scope?.by_finish?.normal !== 174) findings.push('scope_normal_not_174');
  if (dryRun.scope?.by_finish?.reverse !== 132) findings.push('scope_reverse_not_132');
  if ((dryRun.targets ?? []).length !== 592) findings.push(`targets_length_not_592:${(dryRun.targets ?? []).length}`);
...
async function applyPackage(client, targets) {
  ...
  try {
    ...
    await client.query(
      `create temporary table pkg23b_targets (
    ...
    const guards = await client.query(
      `select
         (select count(*)::int from pkg23b_targets) as target_rows,
         (select count(*)::int from pkg23b_targets where finish_key not in ('normal', 'holo', 'reverse')) as disallowed_finish_rows,
         (select count(*)::int from pkg23b_targets where governance_bucket <> 'subset_parallel_base_finish_overgeneration_candidate') as disallowed_bucket_rows,
         (select count(*)::int from public.card_printings cpr join pkg23b_targets t on t.card_printing_id = cpr.id and t.card_print_id = cpr.card_print_id and t.finish_key = cpr.finish_key) as matching_child_rows,
         (select count(*)::int from public.card_prints cp join pkg23b_targets t on t.card_print_id = cp.id where coalesce(cp.variant_key, '') not in ('', 'base') or coalesce(cp.printed_identity_modifier, '') <> '') as identity_modifier_or_variant_rows,
         (select count(*)::int from public.vault_item_instances vii join pkg23b_targets t on t.card_printing_id = vii.card_printing_id and vii.archived_at is null) as vault_item_instance_refs,
         (select count(*)::int from public.external_printing_mappings epm join pkg23b_targets t on t.card_printing_id = epm.card_printing_id) as external_printing_mapping_refs,
         (select count(*)::int from public.canon_warehouse_candidates cwc join pkg23b_targets t on t.card_printing_id = cwc.promoted_card_printing_id) as canon_warehouse_candidate_refs`,
    );
    ...
    if ((await existingOptionalTables(client)).has('justtcg_grookai_mappings')) {
      const justtcg = await client.query(
        `select count(*)::int as refs
         ...
         join pkg23b_targets t on t.card_printing_id = jgm.card_printing_id`,
      );
    ...
    if (
      ...
      throw new Error(`PKG-23B prewrite guard failed: ${JSON.stringify(guard)}`);
    }
    const deleteResult = await client.query(
      `delete from public.card_printings cpr
       using pkg23b_targets target
       where cpr.id = target.card_printing_id`,
    );
    if (deleteResult.rowCount !== targets.length) throw new Error(`PKG-23B delete count mismatch: ${deleteResult.rowCount}`);
    const proof = await client.query(
      `select
         $1::text as package_id,
         $2::text as package_fingerprint,
         (select count(*)::int from public.card_printings cpr join pkg23b_targets target on target.card_printing_id = cpr.id) as remaining_target_children,
    ...
    if (proof.rows[0].remaining_target_children !== 0) {
      throw new Error(`PKG-23B post-delete proof failed: ${JSON.stringify(proof.rows[0])}`);
    }
    await client.query('commit');
    const afterSnapshot = await captureSnapshot(client, targets);
    return {
      apply_status: 'pkg23b_real_apply_committed',
      committed: true,
      before_snapshot: beforeSnapshot,
...
function renderMarkdown(report) {
  return `# PKG-23B Subset/Parallel Base Finish Overgeneration Child Delete Real Apply V1

Approved real apply for subset/parallel base finish-overgeneration child cleanup.
...
if (prerequisiteFindings.length > 0) {
  throw new Error(`PKG-23B dry-run prerequisite validation failed: ${prerequisiteFindings.join(', ')}`);
}
...
const report = {
  ...
  version: 'english_master_index_pkg23b_subset_parallel_base_finish_overgeneration_child_delete_real_apply_v1',

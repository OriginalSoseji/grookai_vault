...
const SOURCE_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg23a_subset_parallel_governance_readiness_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg23b_subset_parallel_base_finish_overgeneration_child_delete_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg23b_subset_parallel_base_finish_overgeneration_child_delete_guarded_dry_run_v1.md');
...
function buildTargets(source) {
  ...
  for (const row of source.rows ?? []) {
    ...
    if (!allowedShape || seen.has(row.card_printing_id)) {
      blocked.push({ ...row, blocked_reason: seen.has(row.card_printing_id) ? 'duplicate_card_printing_id' : 'target_shape_not_allowed_for_pkg23b' });
      continue;
    }
...
async function runDryRun(client, targets) {
  ...
  try {
    ...
    if (
      ...
      throw new Error(`PKG-23B guard failed: ${JSON.stringify(guard)}`);
    }
    ...
    if (deleteResult.rowCount !== targets.length) {
      throw new Error(`PKG-23B delete simulation mismatch: ${deleteResult.rowCount}`);
    }
  ...
  return {
    dry_run_status: 'pkg23b_guarded_dry_run_passed_rolled_back_no_durable_change',
    before_snapshot: beforeSnapshot,
...
function renderMarkdown(report) {
  return `# PKG-23B Subset/Parallel Base Finish Overgeneration Child Delete Guarded Dry Run V1
...
if (targets.length !== 592) throw new Error(`PKG-23B expected 592 targets, found ${targets.length}`);
...
const report = {
  ...
  version: 'english_master_index_pkg23b_subset_parallel_base_finish_overgeneration_child_delete_guarded_dry_run_v1',
  package_id: PACKAGE_ID,
...
report.recommended_real_apply_approval_text = `Approve real ${PACKAGE_ID} apply only. Fingerprint: ${packageFingerprint}. Scope: ${targets.length} subset/parallel base finish-overgeneration child deletes; finishes ${Object.entries(report.scope.by_finish).map(([finish, count]) => `${finish}=${count}`).join(', ')}; top sets ${Object.entries(report.scope.by_set).slice(0, 8).map(([set, count]) => `${set}=${count}`).join(', ')}. Dry-run proof: ${execution.before_snapshot.hash_sha256} == ${execution.after_snapshot.hash_sha256}. No global apply. No migrations. No parent writes. No merges. No quarantine. Variant/modifier rows excluded.`;

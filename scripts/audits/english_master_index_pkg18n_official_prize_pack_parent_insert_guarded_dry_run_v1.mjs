...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg18n_official_prize_pack_parent_insert_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg18n_official_prize_pack_parent_insert_guarded_dry_run_v1.md');

const PACKAGE_ID = 'PKG-18N-OFFICIAL-PRIZE-PACK-PARENT-INSERTS';
const CREATED_BY = 'pkg18n_official_prize_pack_parent_insert_guarded_dry_run_v1';
  ...
  return (readiness.rows ?? [])
    ...
    .map((row) => ({
      ...
      evidence: {
        ...
        evidence_label: 'Official Pokemon Prize Pack PDF checklist corroborates active finish',
...
async function runDryRun(client, targets, fingerprint) {
  ...
  try {
    ...
    return {
      dry_run_status: 'pkg18n_failed_rolled_back',
...
function renderMarkdown(report) {
  ...
  return `# PKG-18N Official Prize Pack Parent Insert Guarded Dry Run V1
...
async function main() {
  ...
  const recommended = execution.dry_run_status === 'pkg18n_completed_rolled_back_no_durable_change' && execution.rollback_verified && execution.stop_findings.length === 0
  ...
  const report = {
    ...
    version: 'english_master_index_pkg18n_official_prize_pack_parent_insert_guarded_dry_run_v1',
    package_id: PACKAGE_ID,

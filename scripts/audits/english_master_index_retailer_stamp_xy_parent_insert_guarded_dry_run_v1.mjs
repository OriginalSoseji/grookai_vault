...
const READINESS_JSON = path.join(AUDIT_DIR, 'retailer_stamp_active_finish_route_v1', 'retailer_stamp_active_finish_route_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'retailer_stamp_active_finish_route_v1', 'retailer_stamp_xy_parent_insert_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'retailer_stamp_active_finish_route_v1', 'retailer_stamp_xy_parent_insert_guarded_dry_run_v1.md');

const PACKAGE_ID = 'RETAILER-STAMP-03-XY-PARENT-IDENTITY-INSERTS';
const CREATED_BY = 'retailer_stamp_xy_parent_insert_guarded_dry_run_v1';
const EXPECTED_TARGET_COUNT = 5;
const TARGET_CLASSIFICATION = 'ready_for_guarded_dry_run';
...
function buildTargets(readiness) {
  return (readiness.rows ?? [])
    ...
    .map((row) => {
      const baseParent = row.base_parent_rows?.[0] ?? null;
      ...
      return {
        ...
        base_parent_id: baseParent?.card_print_id ?? null,
        base_parent_gv_id: baseParent?.gv_id ?? null,
        base_parent_child_finishes: baseParent?.child_finishes ?? [],
        set_key: row.set_key,
        ...
        evidence: {
          ...
          source_status: row.classification,
          selected_base_parent_id: baseParent?.card_print_id ?? null,
          base_parent_child_finishes: baseParent?.child_finishes ?? [],
...
async function runDryRun(client, targets, fingerprint, expectedMissingBaseFinishCount) {
  ...
  try {
    ...
    await client.query(
      `create temporary table retailer_stamp_xy_targets (
    ...
    await client.query(
      `insert into retailer_stamp_xy_targets
       select *
    ...
    const guard = await client.query(
      `with projection as (
         ...
         from retailer_stamp_xy_targets target
         join public.card_prints base on base.id = target.base_parent_id
         left join public.sets s on s.id = base.set_id
       )
       select
         (select count(*)::int from retailer_stamp_xy_targets) as target_count,
         (select count(distinct target_parent_id)::int from retailer_stamp_xy_targets) as target_parent_count,
         (select count(distinct target_child_id)::int from retailer_stamp_xy_targets) as target_child_count,
         (select count(*)::int from retailer_stamp_xy_targets target left join public.card_prints base on base.id = target.base_parent_id where base.id is null) as missing_base_count,
         (select count(*)::int from retailer_stamp_xy_targets target left join public.finish_keys fk on fk.key = target.target_finish_key and fk.is_active = true where fk.key is null) as inactive_finish_count,
         (select count(*)::int from retailer_stamp_xy_targets target join public.card_prints base on base.id = target.base_parent_id left join public.card_printings base_child on base_child.card_print_id = base.id and base_child.finish_key = target.target_finish_key where base_child.id is null) as missing_base_finish_count,
         ...
         (select count(*)::int from retailer_stamp_xy_targets target join public.card_printings cpr on cpr.id = target.target_child_id) as child_collision_count,
         (select count(*)::int from retailer_stamp_xy_targets target join public.card_print_identity cpi on cpi.card_print_id = target.target_parent_id and cpi.is_active = true) as identity_target_collision_count,
    ...
    const childInsert = await client.query(
      `insert into public.card_printings (
       ...
       select
         ...
         concat(target.set_key, ':', target.card_number, ':retailer_stamp_xy:', target.target_variant_key, ':', target.target_finish_key),
         $1::text,
    ...
    const proof = await client.query(
      `select
         ...
         (select count(*)::int from retailer_stamp_xy_targets) as target_rows,
         (select count(*)::int from public.card_prints cp join retailer_stamp_xy_targets target on target.target_parent_id = cp.id) as inserted_parent_rows,
         (select count(*)::int from public.card_prints cp join retailer_stamp_xy_targets target on target.target_parent_id = cp.id and cp.printed_identity_modifier = target.target_printed_identity_modifier) as inserted_parent_modifier_rows,
         (select count(*)::int from public.card_print_identity cpi join retailer_stamp_xy_targets target on target.target_parent_id = cpi.card_print_id and cpi.is_active = true) as inserted_identity_rows,
         (select count(*)::int from public.card_printings cpr join retailer_stamp_xy_targets target on target.target_child_id = cpr.id) as inserted_child_rows,
         (select count(*)::int from public.card_printings cpr join retailer_stamp_xy_targets target on target.target_child_id = cpr.id and cpr.finish_key = 'stamped') as forbidden_stamped_child_rows`,
    ...
    return {
      dry_run_status: 'retailer_stamp_xy_parent_insert_failed_rolled_back',
...
function renderMarkdown(report) {
  ...
  return `# Retailer Stamp XY Parent Insert Guarded Dry Run V1
...
async function main() {
  ...
  if (targets.length !== EXPECTED_TARGET_COUNT) {
    ...
    execution = {
      dry_run_status: 'blocked_missing_base_parent_target',
      rollback_verified: false,
      dry_run_proof_sha256: null,
      simulated_write_counts: { parent_inserts: 0, identity_inserts: 0, child_inserts: 0, deletes: 0, merges: 0 },
      stop_findings: ['missing_base_parent_target'],
    };
  } else if (!conn) {
    execution = {
    ...
    const sourceUniqueActiveFinishCount = targets.filter((row) => !row.base_parent_child_finishes.includes(row.target_finish_key)).length;
    const client = new Client({ connectionString: conn });
    await client.connect();
    try {
      execution = await runDryRun(client, targets, fingerprint, sourceUniqueActiveFinishCount);
    } finally {
  ...
  const sourceUniqueActiveFinishCount = targets.filter((row) => !row.base_parent_child_finishes.includes(row.target_finish_key)).length;
  const recommended = execution.dry_run_status === 'retailer_stamp_xy_parent_insert_completed_rolled_back_no_durable_change'
    && execution.rollback_verified
    && execution.stop_findings.length === 0
    ? `Approve real ${PACKAGE_ID} apply only. Fingerprint: ${fingerprint}. Scope: ${targets.length} XY retailer-stamped parent inserts, ${targets.length} identity inserts, ${targets.length} child printing inserts; finishes ${Object.entries(byFinish).map(([finish, count]) => `${finish}=${count}`).join(', ')}; stamp labels ${Object.entries(byStampLabel).map(([label, count]) => `${label}=${count}`).join(', ')}; sets ${Object.entries(bySet).map(([set, count]) => `${set}=${count}`).join(', ')}; source-unique active-finish rows: ${sourceUniqueActiveFinishCount}. Dry-run proof: ${execution.before_snapshot.hash_sha256} == ${execution.after_rollback_snapshot.hash_sha256}. No global apply. No migrations. No deletes. No merges. No unsupported cleanup.`
    : 'Not approval-ready; dry-run did not pass cleanly.';

  const report = {
    generated_at: new Date().toISOString(),
    version: 'retailer_stamp_xy_parent_insert_guarded_dry_run_v1',
    package_id: PACKAGE_ID,
    ...
    scope: {
      ...
      source_unique_active_finish_rows: sourceUniqueActiveFinishCount,
      base_finish_aligned_rows: targets.length - sourceUniqueActiveFinishCount,
      by_set: bySet,
      by_finish: byFinish,

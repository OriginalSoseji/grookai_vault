...
function validateDryRunReport(report) {
  ...
  if (targets.some((row) => row.source !== EXPECTED_SOURCE)) findings.push('unexpected_source_lane');
  if (traitRows.some((row) => row.source !== EXPECTED_SOURCE)) findings.push('unexpected_trait_source');
     ...
     select
       ...
       (select count(*)::int
        ...
        where nullif(trait_type, '') is null
           ...
           or source <> $3) as invalid_trait_shape_count`,
    [
      JSON.stringify(targets.map((row) => ({ card_print_id: row.card_print_id }))),
...
async function main() {
  ...
  const report = {
    version: 'ENRICH11_SOURCE_MAPPED_TRAIT_REAL_APPLY_V1',
  ...
  const md = [
    '# ENRICH-11 Source-Mapped Trait Real Apply V1',
    '',
    `Package: \`${PACKAGE_ID}\``,
    '',
    '## Result',

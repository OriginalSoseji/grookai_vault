...
const APPLY_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg17j_pricecharting_stamped_parent_identity_real_apply_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg17j_pricecharting_stamped_parent_identity_post_apply_reconciliation_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg17j_pricecharting_stamped_parent_identity_post_apply_reconciliation_v1.md');
...
function buildMarkdown(report) {
  return `# PKG-17J PriceCharting Stamped Parent Identity Post-Apply Reconciliation V1

Read-only post-apply verification for PKG-17J.
...
${markdownTable(
    ['set', 'number', 'card', 'stamp', 'variant', 'modifier', 'finish', 'status', 'blockers'],
    report.rows.map((row) => [
      row.set_key,
      row.card_number,
      row.card_name,
...
async function main() {
  ...
  const report = {
    ...
    version: 'english_master_index_pkg17j_pricecharting_stamped_parent_identity_post_apply_reconciliation_v1',

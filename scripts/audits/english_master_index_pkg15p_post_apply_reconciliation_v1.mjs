...
const APPLY_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg15p_stamped_second_wave_real_apply_v1.json');
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg15p_post_apply_reconciliation_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg15p_post_apply_reconciliation_v1.md');
...
function renderMarkdown(report) {
  ...
  return `# PKG-15P Post-Apply Reconciliation V1
...
async function main() {
  ...
  if (apply.execution?.apply_status !== 'pkg15p_real_apply_committed') stopFindings.push('apply_status_not_committed');
  if (targets.length !== 6) stopFindings.push(`unexpected_target_count:${targets.length}`);
  ...
  const reportBase = {
    ...
    version: 'english_master_index_pkg15p_post_apply_reconciliation_v1',

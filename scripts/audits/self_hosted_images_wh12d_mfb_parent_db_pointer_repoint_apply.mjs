...
const WH12C_SUMMARY_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh12c_mfb_parent_db_pointer_repoint_dry_run_summary_v1.json');
const WH12C_PLAN_JSONL = path.join(OUTPUT_DIR, 'self_hosted_images_wh12c_mfb_parent_db_pointer_repoint_plan_v1.jsonl');
const PLAN_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh12d_mfb_parent_db_pointer_repoint_apply_plan_v1.json');
const PLAN_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh12d_mfb_parent_db_pointer_repoint_apply_plan_v1.md');
const RESULT_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh12d_mfb_parent_db_pointer_repoint_apply_result_v1.json');
const RESULT_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh12d_mfb_parent_db_pointer_repoint_apply_result_v1.md');
const PACKAGE_ID = 'IMG-HOST-WH-12D-MFB-PARENT-DB-POINTER-REPOINT-APPLY';
...
function buildUpdates(summary, planRows) {
  ...
  if (summary.ready_for_apply_package !== true) {
    stopFindings.push(`wh12c_not_apply_ready:${(summary.stop_findings ?? []).join(',')}`);
  }
...
function buildPlan(args, summary, updates) {
  const plan = {
    ...
    source_plan_jsonl: path.relative(ROOT, WH12C_PLAN_JSONL),
...
async function main() {
  ...
  const summary = await readJson(WH12C_SUMMARY_JSON);
  const planRows = await readJsonl(WH12C_PLAN_JSONL);

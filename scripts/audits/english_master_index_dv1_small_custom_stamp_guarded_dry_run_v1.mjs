...
const OUTPUT_MD = path.join(
  AUDIT_DIR,
  'english_master_index_dv1_small_custom_stamp_guarded_dry_run_v1.md',
);
...
const CREATED_BY = 'english_master_index_dv1_small_custom_stamp_guarded_dry_run_v1';
...
async function runRollbackDryRun(client, packet) {
  const targets = packet.targets;
  const beforeSnapshot = await captureSnapshot(client, targets);
  let transientProof = null;
  ...
  return {
    ...
    transient_proof: transientProof,
    after_snapshot: afterSnapshot,
    durable_state_unchanged: beforeSnapshot.hash_sha256 === afterSnapshot.hash_sha256,
    dry_run_proof_sha256: sha256(stableJson({
      before_hash: beforeSnapshot.hash_sha256,
      after_hash: afterSnapshot.hash_sha256,
      transient_counts: {
        parent: transientProof?.inserted_parent_rows ?? null,
        identity: transientProof?.inserted_identity_rows ?? null,
        child: transientProof?.inserted_child_rows ?? null,
      },
      guard: transientProof?.guard ?? null,
    })),
  };
}

function buildMarkdown(report) {
  return `# DV1 Small Custom Stamp Guarded Dry-Run V1
...
${markdownTable(['metric', 'value'], [
    ...
    ['inserted_parent_rows_transient', report.summary.inserted_parent_rows_transient],
    ['inserted_identity_rows_transient', report.summary.inserted_identity_rows_transient],
    ['inserted_child_rows_transient', report.summary.inserted_child_rows_transient],
    ['before_hash', `\`${report.dry_run.before_snapshot.hash_sha256}\``],
    ['after_hash', `\`${report.dry_run.after_snapshot.hash_sha256}\``],
    ['durable_state_unchanged', report.dry_run.durable_state_unchanged],
    ['dry_run_proof_sha256', `\`${report.dry_run.dry_run_proof_sha256}\``],
...
async function main() {
  ...
  if (packet.package_fingerprint_sha256 !== EXPECTED_FINGERPRINT) {
    throw new Error(`fingerprint mismatch: ${packet.package_fingerprint_sha256}`);
  }
  ...
  try {
    const dryRun = await runRollbackDryRun(client, packet);
    if (!dryRun.durable_state_unchanged) throw new Error('rollback proof failed: durable before/after hash mismatch');

    const report = {
      generated_at: new Date().toISOString(),
      version: 'english_master_index_dv1_small_custom_stamp_guarded_dry_run_v1',
      package_id: packet.package_id,
      package_fingerprint_sha256: packet.package_fingerprint_sha256,
      approval_text_required: packet.required_approval_text,
      ...
      summary: {
        ...
        inserted_parent_rows_transient: dryRun.transient_proof.inserted_parent_rows,
        inserted_identity_rows_transient: dryRun.transient_proof.inserted_identity_rows,
        inserted_child_rows_transient: dryRun.transient_proof.inserted_child_rows,
        durable_state_unchanged: dryRun.durable_state_unchanged,
      },
    ...
    console.log(JSON.stringify({
      ...
      durable_state_unchanged: report.dry_run.durable_state_unchanged,
      transient_inserts: {
        parents: report.summary.inserted_parent_rows_transient,
        identities: report.summary.inserted_identity_rows_transient,
        children: report.summary.inserted_child_rows_transient,
      },
      db_writes_performed: false,
      migrations_created: false,
    }, null, 2));

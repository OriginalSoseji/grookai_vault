...
const READINESS_JSON = path.join(OUTPUT_DIR, 'image_truth_img06b_xya_pkmncards_representative_storage_readiness_v1.json');
const PLAN_JSON = path.join(OUTPUT_DIR, 'image_truth_img06c_xya_pkmncards_representative_upload_apply_plan_v1.json');
const PLAN_MD = path.join(OUTPUT_DIR, 'image_truth_img06c_xya_pkmncards_representative_upload_apply_plan_v1.md');
const APPLY_JSON = path.join(OUTPUT_DIR, 'image_truth_img06c_xya_pkmncards_representative_upload_apply_result_v1.json');
const APPLY_MD = path.join(OUTPUT_DIR, 'image_truth_img06c_xya_pkmncards_representative_upload_apply_result_v1.md');
const PACKAGE_ID = 'IMG-06C-XYA-PKMNCARDS-REPRESENTATIVE-MISSING-DISPLAY-CHILD-IMAGE-UPLOAD-APPLY';
...
async function buildPlan(readiness) {
  if (readiness.ready_for_upload_then_apply !== true) {
    throw new Error('IMG-06B readiness is not ready_for_upload_then_apply.');
  }
...
function buildMarkdown(report) {
  return `# Image Truth V1 IMG-06C XYA PKMNCards Representative Upload Apply ${report.mode === 'real_apply' ? 'Result' : 'Plan'}
...
\`\`\`powershell
node scripts/audits/image_truth_v1_img06c_xya_pkmncards_representative_upload_apply.mjs --apply --fingerprint ${report.fingerprint}
\`\`\`
`;
}

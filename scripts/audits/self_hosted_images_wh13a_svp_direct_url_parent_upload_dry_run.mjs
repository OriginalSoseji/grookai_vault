...
const MANIFEST_JSONL = path.join(OUTPUT_DIR, 'self_hosted_images_wh13a_svp_direct_url_parent_upload_manifest_v1.jsonl');
const SUMMARY_JSON = path.join(OUTPUT_DIR, 'self_hosted_images_wh13a_svp_direct_url_parent_upload_dry_run_summary_v1.json');
const SUMMARY_MD = path.join(OUTPUT_DIR, 'self_hosted_images_wh13a_svp_direct_url_parent_upload_dry_run_summary_v1.md');
const PACKAGE_ID = 'IMG-HOST-WH-13A-SVP-DIRECT-URL-PARENT-UPLOAD-DRY-RUN';
const STORAGE_BUCKET = process.env.SELF_HOSTED_IMAGES_STORAGE_BUCKET ?? 'user-card-images';
const USER_AGENT = 'Grookai WH13A SVP Direct URL Image Audit/1.0';
const FETCH_TIMEOUT_MS = Number.parseInt(process.env.IMG_HOST_WH13A_FETCH_TIMEOUT_MS ?? '30000', 10);
const FETCH_CONCURRENCY = Math.max(1, Math.min(Number.parseInt(process.env.IMG_HOST_WH13A_FETCH_CONCURRENCY ?? '6', 10), 10));
...
function toManifestRow(candidate, fetched) {
  ...
  return {
    ...
    current_image_source: candidate.image_source,
    current_image_status: candidate.image_status,
    current_image_note: candidate.image_note,
    ...
    proposed_db_plan: {
      ...
      proposed_image_status: clean(candidate.image_status),
      proposed_image_note: clean(candidate.image_note),
...
async function main() {
  ...
  const rowCounts = countBy(manifestRows, (row) => row.source_row_id);
  const duplicateSourceRows = Object.entries(rowCounts).filter(([, count]) => count > 1);
  const stopFindings = [
    ...(duplicateTargetPaths.length ? ['duplicate_target_storage_paths'] : []),
    ...(duplicateSourceRows.length ? ['multiple_valid_source_urls_for_single_parent_row'] : []),
    ...(failedRows.length ? ['failed_svp_source_url_validation'] : []),
  ];

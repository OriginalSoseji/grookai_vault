...
const PACKAGE_ID = "MEE-CORE-INTERNAL-REVIEW-ACTION-FUNCTION-LOW-SIGNAL-50-BATCH-PLAN-V1";
const REVIEW_ACTOR = "system_low_signal_50_batch_plan";
...
const targets = selectedRows.map((row, index) => ({
  ...
  reason_code: null,
  review_note: "MEE core 50-row low_signal_monitor batch proof: confirm monitor_only remains internal only.",
  before: row,
  expected_after: {
...
from public.apply_market_evidence_review_action_v1(
  ...
  null::text,
  ${sqlLiteral("MEE core 50-row low_signal_monitor batch proof: confirm monitor_only remains internal only.")}::text,
  ${sqlJsonb(actionPayload(row, index))}
);`)
  .join("\n\n");

const applySql = `-- MEE_CORE_INTERNAL_REVIEW_ACTION_FUNCTION_LOW_SIGNAL_50_BATCH_PLAN_V1 apply candidate.
...
const rollbackSql = `-- MEE_CORE_INTERNAL_REVIEW_ACTION_FUNCTION_LOW_SIGNAL_50_BATCH_PLAN_V1 rollback candidate.
...
const preflightSql = `-- MEE_CORE_INTERNAL_REVIEW_ACTION_FUNCTION_LOW_SIGNAL_50_BATCH_PLAN_V1 preflight SQL.
...
const readbackSql = `-- MEE_CORE_INTERNAL_REVIEW_ACTION_FUNCTION_LOW_SIGNAL_50_BATCH_PLAN_V1 readback SQL.
-- Run after approved apply.

with target_ids(id) as (
  values
${selectedRows.map((row) => `    (${sqlLiteral(row.id)}::uuid)`).join(",\n")}
), package_events as (
  select e.*
  from public.market_evidence_review_action_events e
...
select
  'MEE_CORE_INTERNAL_REVIEW_ACTION_FUNCTION_LOW_SIGNAL_50_BATCH_PLAN_V1_READBACK'::text as package_id,
...
const report = {
  ...
  mode: "plan_only_low_signal_50_batch_review_action_function_invoke",
  ...
  artifacts: {
    ...
    apply_sql: "docs/sql/mee_core_internal_review_action_function_low_signal_50_batch_v1_apply_candidate.sql",
    rollback_sql: "docs/sql/mee_core_internal_review_action_function_low_signal_50_batch_v1_rollback_candidate.sql",
    readback_sql: "docs/sql/mee_core_internal_review_action_function_low_signal_50_batch_v1_readback.sql",
    preflight_sql: "docs/sql/mee_core_internal_review_action_function_low_signal_50_batch_v1_preflight.sql",
    ...
    plan_md: "docs/plans/market_evidence_engine_v1/MEE_CORE_INTERNAL_REVIEW_ACTION_FUNCTION_LOW_SIGNAL_50_BATCH_PLAN_V1.md",
  },
  findings: [],
  boundary_proof: {
    db_writes: false,
...
const planMd = `# MEE Core Internal Review Action Function Low Signal 50 Batch Plan V1

Status: plan only

## Objective

Prepare a controlled 50-row invocation package for \`public.apply_market_evidence_review_action_v1\`.

## Selected Action

\`confirm_monitor_only\` against fifty eligible \`low_signal_monitor\` dispositions.
...
writeFileSync(path.join(PLAN_DIR, "MEE_CORE_INTERNAL_REVIEW_ACTION_FUNCTION_LOW_SIGNAL_50_BATCH_PLAN_V1.md"), planMd);
writeFileSync(path.join(SQL_DIR, "mee_core_internal_review_action_function_low_signal_50_batch_v1_apply_candidate.sql"), applySql);
writeFileSync(path.join(SQL_DIR, "mee_core_internal_review_action_function_low_signal_50_batch_v1_rollback_candidate.sql"), rollbackSql);
writeFileSync(path.join(SQL_DIR, "mee_core_internal_review_action_function_low_signal_50_batch_v1_readback.sql"), readbackSql);
writeFileSync(path.join(SQL_DIR, "mee_core_internal_review_action_function_low_signal_50_batch_v1_preflight.sql"), preflightSql);

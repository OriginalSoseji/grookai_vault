...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'final_source_ready_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'final_source_ready_guarded_dry_run_v1.md');

const PACKAGE_ID = 'SPECIAL-VAR-06-FINAL-SOURCE-READY-PARENT-INSERTS';
const CREATED_BY = 'english_master_index_special_variant_final_source_ready_guarded_dry_run_v1';
const EXPECTED_TARGET_COUNT = 3;
const TARGET_KEYS = new Set([
  'base1-12-ninetales-black-flame',
  'basep-3-mewtwo-missing-wb-kids-stamp',
  'basep-5-dragonite-missing-wb-kids-stamp',
]);
...
function sqlHash() {
  return sha256(stableJson({
    package_id: PACKAGE_ID,
    writes: [
      'insert public.card_prints cloned from source-backed final WOTC parent with exact recognized error/correction identity modifier',
...
async function runDryRun(client, targets, fingerprint) {
  ...
  try {
    ...
    await client.query(
      `create temporary table final_source_ready_targets (
    ...
    await client.query(
      `insert into final_source_ready_targets
       select *
    ...
    const guard = await client.query(
      `with projection as (
         ...
         from final_source_ready_targets target
         join public.card_prints base on base.id = target.base_parent_id
         left join public.sets s on s.id = base.set_id
       )
       select
         (select count(*)::int from final_source_ready_targets) as target_count,
         (select count(distinct target_parent_id)::int from final_source_ready_targets) as target_parent_count,
         (select count(distinct target_child_id)::int from final_source_ready_targets) as target_child_count,
         (select count(*)::int from final_source_ready_targets target left join public.card_prints base on base.id = target.base_parent_id where base.id is null) as missing_base_count,
         (select count(*)::int from final_source_ready_targets target left join public.finish_keys fk on fk.key = target.target_finish_key and fk.is_active = true where fk.key is null) as inactive_finish_count,
         (select count(*)::int from final_source_ready_targets target join public.card_prints base on base.id = target.base_parent_id left join public.card_printings base_child on base_child.card_print_id = base.id and base_child.finish_key = target.target_finish_key where base_child.id is null) as source_unique_finish_count,
         ...
         (select count(*)::int from final_source_ready_targets target join public.card_printings cpr on cpr.id = target.target_child_id) as child_collision_count,
         (select count(*)::int from final_source_ready_targets target join public.card_print_identity cpi on cpi.card_print_id = target.target_parent_id and cpi.is_active = true) as identity_target_collision_count,
    ...
    const parentInsert = await client.query(
      `insert into public.card_prints (
       ...
       select
         ...
         concat('Source-backed final WOTC special-case identity: ', target.target_variant_key, '. Representative base image only until exact variant image is available.')
       from final_source_ready_targets target
       join public.card_prints base on base.id = target.base_parent_id`,
      [PACKAGE_ID],
    );
    ...
    const childInsert = await client.query(
      `insert into public.card_printings (
       ...
       select
         ...
         concat(target.set_key, ':', target.card_number, ':final_source_ready_variant:', target.target_variant_key, ':', target.target_finish_key),
         $1::text,
         ...
         'Source-backed final WOTC special-case child. Representative image only until exact variant image is available.'
       from final_source_ready_targets target`,
      [CREATED_BY],
    );
    ...
    const proof = await client.query(
      `select
         ...
         (select count(*)::int from final_source_ready_targets) as target_rows,
         (select count(*)::int from public.card_prints cp join final_source_ready_targets target on target.target_parent_id = cp.id) as inserted_parent_rows,
         (select count(*)::int from public.card_prints cp join final_source_ready_targets target on target.target_parent_id = cp.id and cp.printed_identity_modifier = target.target_printed_identity_modifier and cp.variant_key = target.target_variant_key) as inserted_parent_identity_rows,
         (select count(*)::int from public.card_print_identity cpi join final_source_ready_targets target on target.target_parent_id = cpi.card_print_id and cpi.is_active = true) as inserted_identity_rows,
         (select count(*)::int from public.card_printings cpr join final_source_ready_targets target on target.target_child_id = cpr.id and cpr.finish_key = target.target_finish_key) as inserted_child_rows,
         (select count(*)::int from public.card_printings cpr join final_source_ready_targets target on target.target_child_id = cpr.id and cpr.finish_key = 'stamped') as forbidden_stamped_child_rows`,
    ...
    return {
      dry_run_status: 'final_source_ready_parent_insert_failed_rolled_back',
...
function renderMarkdown(report) {
  return `# Final Source Ready Variants Guarded Dry Run V1
...
async function main() {
  ...
  const pass = execution.dry_run_status === 'final_source_ready_parent_insert_completed_rolled_back_no_durable_change'
  ...
  const recommended = pass
    ? `Approve real ${PACKAGE_ID} apply only. Fingerprint: ${fingerprint}. SQL hash: ${sqlHashSha256}. Scope: 3 final source-ready special-case parent inserts, 3 active identity inserts, 3 child printing inserts; source_unique_finish_rows=${sourceUniqueFinishRows}; sets base1=1, basep=2. Dry-run proof: ${dryRunProof}. No global apply. No migrations. No deletes. No merges. No unsupported cleanup.`

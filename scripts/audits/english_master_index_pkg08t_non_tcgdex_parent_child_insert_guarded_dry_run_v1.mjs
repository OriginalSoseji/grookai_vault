...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg08t_non_tcgdex_parent_child_insert_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg08t_non_tcgdex_parent_child_insert_guarded_dry_run_v1.md');
const CHECKPOINT_MD = path.join(CHECKPOINT_DIR, '20260610_pkg08t_non_tcgdex_parent_child_insert_guarded_dry_run_checkpoint_v1.md');

const PACKAGE_ID = 'PKG-08T-NON-TCGDEX-PARENT-CHILD-INSERTS';
const CREATED_BY = 'pkg08t_non_tcgdex_parent_child_insert_guarded_dry_run_v1';
const PROVENANCE_SOURCE = 'verified_master_set_index_v1';
const READY_STATUS = 'ready_for_non_tcgdex_parent_insert_dry_run';
...
async function runDryRun(client, planned, packageFingerprint) {
  ...
  try {
    ...
    return {
      status: 'pkg08t_non_tcgdex_parent_child_insert_completed_rolled_back_no_durable_change',
      error_message: null,
    ...
    return {
      status: 'pkg08t_non_tcgdex_parent_child_insert_failed_rolled_back',
...
function renderMarkdown(report) {
  return `# PKG-08T Non-TCGdex Parent+Child Insert Guarded Dry Run V1
...
async function main() {
  ...
  const sourceRows = (source.rows ?? []).filter((row) => row.readiness_status === READY_STATUS);
  ...
  if (!conn) {
    report = {
      ...
      version: 'english_master_index_pkg08t_non_tcgdex_parent_child_insert_guarded_dry_run_v1',
      package_id: PACKAGE_ID,
    ...
    try {
      ...
      const stopFindings = [];
      if (source.package_id !== 'PKG-08S-NON-TCGDEX-PARENT-INSERT-READINESS') stopFindings.push('source_readiness_package_mismatch');
      if (planned.parentRows.length !== 34) stopFindings.push('target_parent_count_not_34');
      if (planned.childRows.length !== 34) stopFindings.push('target_child_count_not_34');
      if (planned.mappingRows.length !== 34) stopFindings.push('target_mapping_count_not_34');
      if (planned.blockedRows.length !== 0) stopFindings.push('planned_blocked_rows_present');
      if (execution.status !== 'pkg08t_non_tcgdex_parent_child_insert_completed_rolled_back_no_durable_change') stopFindings.push('dry_run_not_passed');
      ...
      report = {
        ...
        recommended_real_apply_approval_text: stopFindings.length === 0
          ? `Approve real PKG-08T-NON-TCGDEX-PARENT-CHILD-INSERTS apply only. Fingerprint: ${packageFingerprint}. Scope: ${planned.parentRows.length} parent card_print inserts, ${planned.childRows.length} child card_printing inserts, ${planned.mappingRows.length} external mappings across ${Object.keys(countBy(planned.childRows, (row) => row.set_key)).length} sets; mapping sources ${Object.entries(countBy(planned.mappingRows, (row) => row.source)).map(([sourceName, count]) => `${sourceName}=${count}`).join(', ')}; finishes ${Object.entries(byFinish).map(([finish, count]) => `${finish}=${count}`).join(', ')}. Dry-run proof: ${execution.before_snapshot?.hash_sha256} == ${execution.after_snapshot?.hash_sha256}. No global apply. No migrations. No deletes. No merges. No unsupported cleanup.`
          : null,
      };
    } finally {

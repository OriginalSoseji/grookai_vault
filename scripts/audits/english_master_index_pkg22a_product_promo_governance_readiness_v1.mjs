...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg22a_product_promo_governance_readiness_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg22a_product_promo_governance_readiness_v1.md');
const PACKAGE_ID = 'PKG-22A-PRODUCT-PROMO-GOVERNANCE-READINESS';
...
function hasVariant(row) {
  ...
  if (
    String(row.canonical_set_key ?? row.set_code ?? '').toLowerCase() === 'xyp'
    && variant.toLowerCase() === 'xya'
    && /^xy\d+a$/i.test(String(row.number ?? row.card_number ?? ''))
  ) {
    return false;
  }
...
function classify(row) {
  ...
  if (dependencyTotal > 0) {
    return {
      governance_bucket: 'dependency_blocked_product_promo_child',
  ...
  if (hasVariant(row) || hasModifier(row)) {
    return {
      governance_bucket: finishKnownForCard
        ? 'product_promo_identity_modifier_or_variant_supported_finish_review'
        : 'product_promo_identity_modifier_or_variant_source_review',
      cleanup_readiness: 'blocked',
      reason: 'Variant/modifier identity must be resolved as parent identity or source-backed display modifier before any child cleanup.',
    };
  }
  if (hasKnownCard && !finishKnownForCard) {
    return {
      governance_bucket: 'product_promo_base_finish_overgeneration_candidate',
      cleanup_readiness: 'dry_run_candidate',
      reason: 'Same set/card is Master-index known, this child has no dependencies or variant/modifier, and this finish is not supported for the promo card.',
    };
  }
  if (hasKnownCard && finishKnownForCard) {
    return {
      governance_bucket: 'product_promo_base_supported_finish_key_mismatch_review',
      cleanup_readiness: 'blocked',
      reason: 'Same finish appears supported at card level, but exact reconciliation key did not match; needs key-shape inspection before any write.',
    };
  }
  return {
    governance_bucket: 'product_promo_source_coverage_gap',
    cleanup_readiness: 'blocked',
    reason: 'No Master Index finish fact matched this promo card; source acquisition or alias governance is required.',
  };
}
...
function renderMarkdown(report) {
  ...
  const candidateSetRows = Object.entries(report.candidate_summary.by_set).map(([set, count]) => [set, count]);
  const candidateFinishRows = Object.entries(report.candidate_summary.by_finish).map(([finish, count]) => [finish, count]);
  return `# PKG-22A Product/Promo Governance Readiness V1
...
${markdownTable(['set', 'rows'], candidateSetRows)}

${markdownTable(['finish', 'rows'], candidateFinishRows)}
...
const rows = (source.rows ?? []).filter((row) => row.lane === 'product_or_promo_source_review');
...
const report = {
  ...
  version: 'english_master_index_pkg22a_product_promo_governance_readiness_v1',
  package_id: PACKAGE_ID,

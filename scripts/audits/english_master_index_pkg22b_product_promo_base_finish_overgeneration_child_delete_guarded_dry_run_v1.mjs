...
const OUTPUT_JSON = path.join(AUDIT_DIR, 'english_master_index_pkg22b_product_promo_base_finish_overgeneration_child_delete_guarded_dry_run_v1.json');
const OUTPUT_MD = path.join(AUDIT_DIR, 'english_master_index_pkg22b_product_promo_base_finish_overgeneration_child_delete_guarded_dry_run_v1.md');
...
function buildTargets(source) {
  ...
  for (const row of source.rows ?? []) {
    ...
    if (!allowedShape || seen.has(row.card_printing_id)) {
      blocked.push({ ...row, blocked_reason: seen.has(row.card_printing_id) ? 'duplicate_card_printing_id' : 'target_shape_not_allowed_for_pkg22b' });
      continue;
    }
...
async function runDryRun(client, targets) {
  ...
  try {
    ...
    if (
      ...
      throw new Error(`PKG-22B guard failed: ${JSON.stringify(guard)}`);
    }
    ...
    if (deleteResult.rowCount !== targets.length) {
      throw new Error(`PKG-22B delete simulation mismatch: ${deleteResult.rowCount}`);
    }
  ...
  return {
    dry_run_status: 'pkg22b_guarded_dry_run_passed_rolled_back_no_durable_change',
    before_snapshot: beforeSnapshot,
...
function renderMarkdown(report) {
  return `# PKG-22B Product/Promo Base Finish Overgeneration Child Delete Guarded Dry Run V1
...
if (targets.length !== 2064) throw new Error(`PKG-22B expected 2064 targets, found ${targets.length}`);
...
const report = {
  ...
  version: 'english_master_index_pkg22b_product_promo_base_finish_overgeneration_child_delete_guarded_dry_run_v1',
  package_id: PACKAGE_ID,

...
import '../search/unified_search_sheet.dart';
...
class _ScannerPageState extends State<ScannerPage> {
  ...
  Future<void> _showConfirmationSheet() async {
    ...
    // Open unified search sheet with best candidate context
    await UnifiedSearchSheet.show(
      context,
      initialQuery: '${best.setCode} ${best.collectorNumber}',
    );
    return;
    // ignore: dead_code
    ...
    // ignore: dead_code
    final viable = list
        .where((c) => (best.confidence - c.confidence).abs() <= 0.05)
        .take(3)
        .toList();
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: GVSpacing.s4),
                            Text(
                              'Confidence ${(sel.confidence * 100).toStringAsFixed(0)}%',
                            ),
                          ],
                        ),
                      ),

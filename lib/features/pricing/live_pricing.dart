    ...
    final channel = _supa.channel('ticks_$cardPrintId')
      ..onPostgresChanges(
        ...
        callback: (payload) {
          ...
          if (r['card_print_id'] == cardPrintId) controller.add(r);
        },
      )
      ..subscribe();
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          Text(
            _money(market, currency: currency),
            style: const TextStyle(fontWeight: FontWeight.w600),
          ),
          const SizedBox(width: 8),
          Icon(
            pctChange24h != null && pctChange24h! >= 0
                ? Icons.arrow_upward
          ...
          Text(
            pctChange24h == null ? '—' : '${pctChange24h!.toStringAsFixed(2)}%',
...
class TickerListTile extends StatefulWidget {
  ...
  @override
  State<TickerListTile> createState() => _TickerListTileState();
}
...
class _TickerListTileState extends State<TickerListTile> {
  ...
  void initState() {
    super.initState();
    repo = PricingRepo(Supabase.instance.client);
    _load();
  }
    ...
    if (printId != null) {
      _sub = repo.subscribeTicksByPrintId(printId).listen((tick) {
        ...
        setState(() {
          _ticker = {
            ...?_ticker,
            'market_now': tick['market'],
            'last_updated': tick['captured_at'],
          };
        });
      });
    }
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      ...
      trailing: PricePill(
        market: _ticker?['market_now'] as num?,
        pctChange24h: _ticker?['pct_change_24h'] as num?,
        currency: 'USD',
      ),
      ...
      _sub = repo.subscribeTicksByPrintId(printId).listen((tick) {
        ...
        setState(() {
          _data = List.of(_data)
            ..add(
              PricePoint(
                DateTime.parse(tick['captured_at'] as String),
                (tick['market'] as num?)?.d ?? 0,
              ),
            );
        });
      });
...
class _LineChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final spots = <FlSpot>[];
    for (final p in data) {
      final x = p.t.difference(start).inMinutes / 60.0;
        ...
        titlesData: FlTitlesData(
          ...
          leftTitles: AxisTitles(
            sideTitles: SideTitles(
              showTitles: true,
              reservedSize: 44,
              getTitlesWidget: (v, meta) => Text(_money(v)),
            ),
          ),

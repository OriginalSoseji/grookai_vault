...
class _PricingProbePageState extends State<PricingProbePage> {
  ...
  Future<void> _enqueueOne() async {
    ...
    try {
      ...
      final r = await sb.functions.invoke(
        'enqueue_import',
        body: {'set_code': 'sv6', 'number': '001', 'lang': 'en'},
      );
          ...
          : RefreshIndicator(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  ..._viewSample.map(
                    (e) => Text(
                      'card_id=${e['card_id']} mid=${e['price_mid']} at=${e['observed_at']}',
                    ),
                  ),
                  const SizedBox(height: 12),
                  const Text(
                  ...
                  Wrap(
                    ...
                    children: [
                      ElevatedButton(
                        onPressed: _runUpdatePrices,
                        child: const Text('Run update_prices (5)'),
                      ),
                      ElevatedButton(
                        onPressed: _enqueueOne,
                        child: const Text('Import test (sv6/001)'),
                      ),
                    ],
                  ),
                ],

  ...
  Future<ServerResolveResult?> resolve({
    ...
    try {
      final body = <String, dynamic>{
        ...
        if (imageJpegBytes != null)
          'image_base64': base64Encode(imageJpegBytes),
      ...
      final best = data['best'] == null
          ? null
          : Map<String, dynamic>.from(data['best']);
      final alts = List<Map<String, dynamic>>.from(
        (data['alternatives'] as List? ?? const []).map(
          (e) => Map<String, dynamic>.from(e),
        ),
      );
      final used = (data['used_embedding'] ?? false) == true;
      return ServerResolveResult(
        best: best,
        alternatives: alts,
        usedEmbedding: used,
      );

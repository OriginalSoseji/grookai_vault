...
// lib/main.dart
import 'dart:convert'; // for fixMojibake
import 'dart:io';
...
/// Fixes mojibake like "Ãƒ¢" if a string was saved/viewed in a wrong encoding.
...
/// - If it's already a URL, normalize TCGDex format:
///   https://assets.tcgdex.net/<lang>/<series>/<set>/<number>/high.png
/// - If it's a Supabase Storage path, build a public URL from the `public` bucket.
...
/// Priority: image_url -> photo_url -> image_path -> (set_code+number ? TCGDex)
                    ...
                    final tile = ListTile(
                      ...
                      subtitle: Wrap(
                        ...
                        children: [
                          ...
                          _chip(cond),
                          Text('Qty: $qty'),
                          if (mp > 0) Text('ï¿½ \$${mp.toStringAsFixed(2)} ea'),
                        ],
                      ),
                      trailing: Row(
                        mainAxisSize: MainAxisSize.min,
            ...
            itemBuilder: (context, i) {
              ...
              return Card(
                child: ListTile(
                  ...
                  subtitle: Text(
                    '$setCode \u00B7 #$number${mp > 0 ? ' \u00B7 \$${mp.toStringAsFixed(2)}' : ''}',
                  ),
                  trailing: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                          ...
                          child: ListTile(
                            ...
                            onTap: () {
                              ...
                              if (current.isEmpty && url.isNotEmpty) {
                                out['image_url'] =
                                    url; // ensure callers get a solid URL
                              }
...
class _ScanPageState extends State<ScanPage> {
            ...
            children: [
              Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      '${card['name'] ?? 'Card'} ï¿½ ${card['set_code'] ?? ''} #${card['number'] ?? ''}',
                      maxLines: 2,
                    ),
                  ),
                ],
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        const Text(
          'Point your camera at a card. Weï¿½ll identify it, grade condition, fetch market price, and add it to your Vault automatically.',
        ...
        FilledButton.icon(
          onPressed: _busy
              ? null
              : _captureIdentifyAndAdd, // pass the function, don't call it
          icon: const Icon(Icons.camera_alt),
          label: Text(_busy ? 'Workingï¿½' : 'Scan & Add to Vault'),
        ),
        const SizedBox(height: 12),

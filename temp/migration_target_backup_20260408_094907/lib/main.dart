...
import 'dev/camera_test_screen.dart';
...
const bool kUseCameraTestHarness = bool.fromEnvironment(
  'GROOKAI_CAMERA_TEST',
  defaultValue: false,
);
...
Future<void> main() async {
  ...
  if (kUseCameraTestHarness) {
    runApp(const CameraTestHarnessApp());
    return;
  }
  await _loadEnv();
...
class CameraTestHarnessApp extends StatelessWidget {
  const CameraTestHarnessApp({super.key});

  @override
  Widget build(BuildContext context) {
    return const GrookaiAppFrame(home: CameraTestScreen());
  }
}

...
class VaultPageState extends State<VaultPage> {
  ...
  Widget _buildVaultCards(List<Map<String, dynamic>> rows) {
    if (_cardViewMode == AppCardViewMode.grid) {
      ...
      return GridView.builder(
        ...
        itemBuilder: (context, index) => _buildVaultGridTile(rows[index]),
      );
    }

    return _buildVaultList(rows);
  }
          ...
          _ProductSurfaceCard(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildVaultCards(groups[index].rows),
              ],
            ),
          ),
  ...
  Widget build(BuildContext context) {
    ...
    switch (_view) {
      ...
      case _VaultStructuralView.onWall:
        vaultContent = _loading
            ? const Center(child: CircularProgressIndicator())
            : onWallRows.isNotEmpty
            ? _buildVaultCards(onWallRows)
            : _search.trim().isNotEmpty
            ? const _ProductEmptyState(
                title: 'No wall cards match your search',
                body: 'Try a different search or clear the current query.',
              )
            : const _ProductEmptyState(
                title: 'No wall items yet',
                body: 'Cards you add to your wall will appear here.',
              );
        break;
            ...
            : Column(
                ...
                children: [
                  ...
                  if (pokemonRows.isNotEmpty)
                    _buildVaultCards(pokemonRows)
                  else if (_pokemonSearch.trim().isNotEmpty)
                    const _ProductEmptyState(
                      title: 'No matching cards',
                      body: 'Try a different Pokemon name.',
                    )
                  else
                    const _ProductEmptyState(
                      title: 'No cards found in your vault',
                      body: 'Add cards to start browsing by Pokemon name.',
                    ),
                ],
              );
        break;
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          Text(
            '$totalCards cards • ${_items.length} unique • $setCount sets • ${_lastAddedLabel(_items)}',
            style: theme.textTheme.labelMedium?.copyWith(
            ...
            children: [
              Expanded(
                child: SingleChildScrollView(
                  ...
                  child: Row(
                    children: [
                      _buildVaultViewChip(_VaultStructuralView.all, 'All'),
                      const SizedBox(width: 6),
                      _buildVaultViewChip(_VaultStructuralView.onWall, 'Wall'),
                      const SizedBox(width: 6),
                      _buildVaultViewChip(
                      ...
                      const SizedBox(width: 6),
                      _buildVaultViewChip(_VaultStructuralView.bySet, 'Sets'),
                      const SizedBox(width: 6),
                      _buildVaultViewChip(

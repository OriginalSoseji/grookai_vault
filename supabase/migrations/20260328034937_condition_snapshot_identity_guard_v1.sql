-- PURPOSE:
-- Enforce identity boundary: user uploads must NEVER set card_print_id.
-- This is a forward-only patch for the live database.
-- Safe to replay because it patches the existing overload in place.
...
begin
  ...
  -- HARD GUARD:
  -- canonical identity must NEVER be injected at upload time
  -- always override the incoming value
  p_card_print_id := null;

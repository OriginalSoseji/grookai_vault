-- POKEMON_SEALED_CATALOG_FILTERS_V1
-- New bounded endpoint; existing pricing and other game endpoints remain unchanged.
begin;
set local lock_timeout = '5s';
create or replace function public.get_active_pokemon_sealed_catalog_v1(
  p_game_key text,
  p_query text default null,
  p_limit integer default 50,
  p_offset integer default 0,
  p_package_form text default null,
...
as $$
  ...
  where coalesce(auth.role(), '') in ('authenticated', 'service_role')
    ...
    and (nullif(btrim(p_package_form), '') is null or variant.package_form = lower(btrim(p_package_form)))
    and (nullif(btrim(p_language_code), '') is null or variant.language_code = lower(btrim(p_language_code)))
...
revoke all on function public.get_active_pokemon_sealed_catalog_v1(
  text, text, integer, integer, text, text
) from public, anon, authenticated, service_role;

grant execute on function public.get_active_pokemon_sealed_catalog_v1(
  text, text, integer, integer, text, text
) to authenticated, service_role;

comment on function public.get_active_pokemon_sealed_catalog_v1(
  text, text, integer, integer, text, text
) is

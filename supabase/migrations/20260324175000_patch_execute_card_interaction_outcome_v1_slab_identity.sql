...
begin
  ...
  v_source_card_print_id := v_source_instance.card_print_id;

  if v_source_card_print_id is null and v_source_instance.slab_cert_id is not null then
    select sc.card_print_id
    into v_source_card_print_id
    from public.slab_certs sc
    where sc.id = v_source_instance.slab_cert_id;
  end if;

  if v_source_card_print_id is null then
    raise exception 'source_instance_missing_card_print' using errcode = 'P0001';
  end if;

  if v_source_card_print_id <> v_interaction.card_print_id then
  ...
  if v_source_bucket.card_id <> v_source_card_print_id then
  ...
  from public.admin_vault_instance_create_v1(
    p_user_id => v_target_user_id,
    p_card_print_id => case when v_source_instance.slab_cert_id is null then v_source_card_print_id else null end,
    p_slab_cert_id => v_source_instance.slab_cert_id,
  ...
  return jsonb_build_object(
    ...
    'card_print_id', v_source_card_print_id,

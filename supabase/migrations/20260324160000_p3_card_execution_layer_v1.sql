...
create table if not exists public.card_execution_events (
  id uuid primary key default gen_random_uuid(),
  execution_type text not null
    check (execution_type in ('sale', 'trade')),
  initiated_by_user_id uuid not null references auth.users(id),
  created_at timestamptz not null default now()
);

create table if not exists public.card_interaction_outcomes (
  id uuid primary key default gen_random_uuid(),
  execution_event_id uuid not null references public.card_execution_events(id),
  latest_interaction_id uuid not null references public.card_interactions(id),
  card_print_id uuid not null references public.card_prints(id),
  source_instance_id uuid not null references public.vault_item_instances(id),
  source_vault_item_id uuid not null references public.vault_items(id),
  source_user_id uuid not null references auth.users(id),
  target_user_id uuid not null references auth.users(id),
  result_instance_id uuid not null references public.vault_item_instances(id),
  result_vault_item_id uuid not null references public.vault_items(id),
  outcome_type text not null
    check (outcome_type in ('sale', 'trade')),
  price_amount numeric null
    check (price_amount is null or price_amount >= 0),
  price_currency text null
    check (
      price_currency is null or
      (price_currency = upper(price_currency) and char_length(price_currency) = 3)
    ),
...
create unique index if not exists uq_outcome_source_instance
on public.card_interaction_outcomes(source_instance_id);

create unique index if not exists uq_outcome_result_instance
on public.card_interaction_outcomes(result_instance_id);

create index if not exists idx_outcome_event
on public.card_interaction_outcomes(execution_event_id);

create index if not exists idx_outcome_source_user
on public.card_interaction_outcomes(source_user_id, created_at desc);

create index if not exists idx_outcome_target_user
on public.card_interaction_outcomes(target_user_id, created_at desc);

create index if not exists idx_outcome_card
on public.card_interaction_outcomes(card_print_id, created_at desc);

alter table public.card_execution_events enable row level security;
alter table public.card_interaction_outcomes enable row level security;

revoke all on table public.card_execution_events from anon;
revoke all on table public.card_execution_events from authenticated;
revoke all on table public.card_interaction_outcomes from anon;
revoke all on table public.card_interaction_outcomes from authenticated;

grant select on table public.card_execution_events to authenticated;
grant select on table public.card_interaction_outcomes to authenticated;

drop policy if exists card_execution_events_select_participants on public.card_execution_events;
create policy card_execution_events_select_participants
on public.card_execution_events
for select
to authenticated
using (
  exists (
    ...
    where outcomes.execution_event_id = card_execution_events.id
      and (outcomes.source_user_id = auth.uid() or outcomes.target_user_id = auth.uid())
  )
);

drop policy if exists card_interaction_outcomes_select_participants on public.card_interaction_outcomes;
create policy card_interaction_outcomes_select_participants
on public.card_interaction_outcomes
for select
to authenticated
using (

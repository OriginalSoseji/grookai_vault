-- 1) Deduplicate condition_prices by (card_id, condition_label, currency), keep latest ts
WITH ranked AS (
  SELECT ctid, row_number() OVER (
    PARTITION BY card_id, condition_label, currency
    ORDER BY ts DESC NULLS LAST
  ) AS rn
  FROM public.condition_prices
)
DELETE FROM public.condition_prices cp
USING ranked r
WHERE cp.ctid = r.ctid AND r.rn > 1;

-- 2) Deduplicate graded_prices by (card_id, grade_company, grade_value, currency), keep latest ts
WITH ranked AS (
  SELECT ctid, row_number() OVER (
    PARTITION BY card_id, grade_company, grade_value, currency
    ORDER BY ts DESC NULLS LAST
  ) AS rn
  FROM public.graded_prices
)
DELETE FROM public.graded_prices gp
USING ranked r
WHERE gp.ctid = r.ctid AND r.rn > 1;

-- 3) Create the unique indexes we rely on (safe if already there)
...
-- Convenience: set condition on a vault item

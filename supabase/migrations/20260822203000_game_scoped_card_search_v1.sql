-- GAME_SCOPED_CARD_SEARCH_V1
-- Bounded cross-TCG search that checks release authority once instead of
-- evaluating the card_prints release RLS predicate for every candidate row.

begin;

create or replace function public.search_game_card_prints_v1(
  game_code_in text,
  q text default null,
...
comment on function public.search_game_card_prints_v1(
  text,
  text,
  text,
  text,
  ...
  'Read-only bounded canonical search for a released game. Release authority is checked once before base-table lookup.';
...
grant execute on function public.search_game_card_prints_v1(
  text,
  text,
  text,
  text,

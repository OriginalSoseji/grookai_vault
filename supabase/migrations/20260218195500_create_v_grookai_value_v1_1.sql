create or replace view public.v_grookai_value_v1_1 as
with base as (
  select
    cap.card_print_id,
    cap.nm_floor,
...
calc as (
  select
    ...
    case
      ...
      else greatest(b.nm_floor::numeric, (b.nm_median::numeric * 0.70::numeric))
  ...
  select
    ...
    case
      when c.effective_floor_nm is null then null::numeric
        ...
        ((1::numeric - c.w_liquidity) * c.effective_floor_nm::numeric)
      )
    end as gv_raw
  from calc c
)
select
  ...
  g.conf_factor,
  case
    when g.effective_floor_nm is null or g.nm_median is null then null::numeric
    else greatest(
      least(g.effective_floor_nm, g.nm_median),
      least(
        greatest(g.effective_floor_nm, g.nm_median),
        (g.gv_raw * g.conf_factor)
      )
    )::numeric
  end as grookai_value_nm,
  (coalesce(g.confidence::numeric, null::numeric)) as confidence_out
from gv g;

-- 20251104121000_search_view_prices.sql
-- Enhance v_card_search to include latest prices when available.

-- Use conditional create to tolerate environments without pricing tables/views
DO $$
BEGIN
  IF EXISTS (
    ...
    -- With pricing view available: join to pick a reasonable latest price
    ...
    LEFT JOIN LATERAL (
      ...
      ORDER BY
        -- prefer common listing-friendly conditions if present
        CASE
    ...
    COMMENT ON VIEW public.v_card_search IS
      'Stable app-facing search view. Guarantees image_url, thumb_url, number_* variants, and latest prices when available.';
    ELSE
    -- No pricing view: keep contract, prices are NULL
    DROP VIEW IF EXISTS public.v_card_search CASCADE;
    CREATE OR REPLACE VIEW public.v_card_search AS
    SELECT
      cp.id::uuid                                AS id,
    ...
    END IF;
  ELSE
    -- No card_prints table: provide an empty shim view with stable contract
    DROP VIEW IF EXISTS public.v_card_search CASCADE;
    CREATE OR REPLACE VIEW public.v_card_search AS
    SELECT
      NULL::uuid    AS id,

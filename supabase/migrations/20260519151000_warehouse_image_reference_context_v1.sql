...
declare
  ...
  v_reference_hints_payload jsonb := coalesce(p_reference_hints_payload, '{}'::jsonb);
begin
  ...
  if jsonb_typeof(v_reference_hints_payload) <> 'object' then
    raise exception 'invalid_reference_hints_payload';
  end if;
  ...
  if p_submission_intent = 'MISSING_IMAGE'
     ...
     and p_identity_snapshot_id is null
     and nullif(trim(v_reference_hints_payload->>'card_gv_id'), '') is null then
  ...
  values (
    ...
    v_user_id,
    'USER',
    jsonb_build_object('reference_hints_payload', v_reference_hints_payload),
    v_now
  );

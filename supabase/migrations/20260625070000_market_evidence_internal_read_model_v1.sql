-- MEE_CORE_INTERNAL_EVIDENCE_READ_MODEL_SCHEMA_CANDIDATE_V1 local migration candidate.
-- Purpose: internal-only read models for card-level Market Evidence Engine coverage, review burden,
-- source mix, lifecycle state, and publication-readiness candidates.
-- Boundary: local candidate only until explicitly approved for targeted remote schema apply.
-- No evidence backfill, provider calls, source fetches, app-visible pricing, public price rollups,
-- identity writes, vault writes, image/storage writes, deletes, merges, or global apply.
...
select
  'MEE_CORE_INTERNAL_EVIDENCE_READ_MODEL_SCHEMA_CANDIDATE_V1'::text as package_id,
  2::int as proposed_view_count,

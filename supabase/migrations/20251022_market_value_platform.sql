-- Market Value Platform (idempotent)
...
create table if not exists public.pricing_config (
  ...
  version_label text not null default 'gv-1.0',
...
-- 3) price_observations (append-only)
...
-- 4) app_sales (gold-standard minimal)
...
-- 5) Extend public.card_prices with MV fields (if missing)
do $$
begin
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='card_prices') then
    if not exists (select 1 from information_schema.columns where table_schema='public' and table_name='card_prices' and column_name='market_value') then
      alter table public.card_prices add column market_value numeric;
    end if;
    if not exists (select 1 from information_schema.columns where table_schema='public' and table_name='card_prices' and column_name='market_algo_version') then
      alter table public.card_prices add column market_algo_version text;
    end if;
...
-- 6) Refresh client view to expose MV + confidence (legacy schema shape)
...
-- 7) Seed default config if missing
do $$
begin
  ...
  if not exists (select 1 from public.pricing_formula_versions where version_label='gv-1.0') then

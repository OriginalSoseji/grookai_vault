...
-- Each join below is one-to-zero-or-one by an existing unique constraint:
-- external_mappings(source, external_id), active card_print_identity(card_print_id),
-- card_printings(card_print_id, finish_key), and the assignment source/version key.
-- The original view grouped every wide source row only to recover those 0/1 counts,
-- forcing a large sort over product JSON. Preserve the contract without that sort.
...
comment on view public.v_tcgplayer_market_qualification_candidates_v1 is
  'Latest reconciled Pokémon TCGCSV market observations joined one-to-one to canonical mapping and exact finish evidence without a wide aggregation.';

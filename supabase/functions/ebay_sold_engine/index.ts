...
type InBody = { cardId?: string; query?: string; condition?: string; lookbackDays?: number }
...
Deno.serve(async (req) => {
  ...
  const { cardId, query, condition, lookbackDays = 30 } = (await req.json().catch(() => ({}))) as InBody
  ...
  try {
    ...
    if (r.ok) {
      ...
      for (const x of items) {
        ...
        if (typeof eff === 'number') prices.push(eff)
      }
    }
  } catch {}
  ...
  return json({ ok: true, sold_avg: avg, sold_low_p10: low, sold_high_p90: high, count: prices.length, observedAt })
})

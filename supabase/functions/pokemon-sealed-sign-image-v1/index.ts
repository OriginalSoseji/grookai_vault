...
const OBJECT_PATH =
  /^sealed\/pokemon\/sha256\/[0-9a-f]{2}\/[0-9a-f]{64}\.(jpg|png|gif|webp)$/;
...
export async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    const { data: authorized, error: authorizationError } =
      await callerClient.rpc("pokemon_sealed_image_object_signing_authorized_v1", {
        p_bucket_id: bucket,
        p_object_name: objectPath,
      });
    if (authorizationError) {
      console.error("[pokemon-sealed-sign-image-v1] authorization failed", {
    ...
    if (error || typeof data?.signedUrl !== "string" || !data.signedUrl) {
      console.error("[pokemon-sealed-sign-image-v1] signing failed", {
        code: error?.name,
      });
    ...
    console.error("[pokemon-sealed-sign-image-v1] request failed", { code });

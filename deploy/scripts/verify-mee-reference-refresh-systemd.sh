...
REPO_DIR="${REPO_DIR:-/opt/grookai_mee_current}"
...
ls -lt "${ARTIFACT_ROOT}"/mee_reference_source_refresh_worker_v1_*.json 2>/dev/null | head -5 || true
ls -lt "${ARTIFACT_ROOT}"/mee_reference_source_refresh_worker_v1_*.md 2>/dev/null | head -5 || true

echo
echo "Latest reference warehouse delta artifacts:"
ls -lt "${ARTIFACT_ROOT}"/mee_reference_warehouse_delta_writer_v1_*.json 2>/dev/null | head -5 || true
ls -lt "${ARTIFACT_ROOT}"/mee_reference_warehouse_delta_writer_v1_*.md 2>/dev/null | head -5 || true

echo
echo "Manual dry-run command:"
printf 'cd %q\nexport MEE_RUNTIME_ARTIFACT_ROOT=%q\n' "${REPO_DIR}" "${ARTIFACT_ROOT}"

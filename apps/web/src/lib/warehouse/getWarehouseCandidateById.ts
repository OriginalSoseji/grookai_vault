...
export type WarehouseEvidenceDetailRow = WarehouseEvidenceRow & {
...
export type WarehouseCandidateDetailResult = {
  candidate: WarehouseCandidateRow | null;
  evidenceRows: WarehouseEvidenceDetailRow[];
...
export async function getWarehouseCandidateById(candidateId: string): Promise<WarehouseCandidateDetailResult> {
  ...
  const { data: candidateData, error: candidateError } = await admin
    ...
    .select(
      "id,submitted_by_user_id,intake_channel,submission_type,notes,tcgplayer_id,submission_intent,state,current_review_hold_reason,interpreter_decision,interpreter_reason_code,interpreter_explanation,interpreter_resolved_finish_key,needs_promotion_review,proposed_action_type,created_at,updated_at",
    )
    .eq("id", normalizedCandidateId)
    .maybeSingle();

  if (candidateError) {
    throw new Error(`Warehouse candidate detail query failed: ${candidateError.message}`);
  }

  const candidate = (candidateData as WarehouseCandidateRow | null) ?? null;
  ...
  const [{ data: evidenceData, error: evidenceError }, { data: eventData, error: eventError }] = await Promise.all([
    admin
  ...
  if (evidenceError) {
    throw new Error(`Warehouse evidence detail query failed: ${evidenceError.message}`);
  }
  if (eventError) {
    throw new Error(`Warehouse event detail query failed: ${eventError.message}`);
  }
  ...
  if (identitySnapshotError) {
    throw new Error(`Warehouse identity snapshot join failed: ${identitySnapshotError.message}`);
  }
  if (conditionSnapshotError) {
    throw new Error(`Warehouse condition snapshot join failed: ${conditionSnapshotError.message}`);
  }
  if (scanEventError) {
    throw new Error(`Warehouse scan event join failed: ${scanEventError.message}`);
  }
  if (scanEventResultError) {
    throw new Error(`Warehouse scan event result join failed: ${scanEventResultError.message}`);
  }
  ...
  const detailEvidenceRows = evidenceRows.map((row) => {
    ...
    const dedupedPreviews = Array.from(
      new Map(previews.map((preview) => [preview.url, preview])).values(),
    );

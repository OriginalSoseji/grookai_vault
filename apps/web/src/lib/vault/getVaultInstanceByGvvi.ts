...
export async function getVaultInstanceByGvvi(userId: string, gvviId: string): Promise<VaultInstanceDetail | null> {
  ...
  const { data: instanceData, error: instanceError } = await admin
    ...
    .select(
      "id,user_id,gv_vi_id,card_print_id,slab_cert_id,legacy_vault_item_id,condition_label,intent,notes,created_at,archived_at,grade_company,grade_value,grade_label,photo_url,image_url",
    )
  ...
  const [imageResult, outcomeResult] = await Promise.all([
    legacyVaultItemId
      ? admin
          .from("user_card_images")
          .select("side")
          .eq("user_id", normalizedUserId)
          .eq("vault_item_id", legacyVaultItemId)
      : Promise.resolve({ data: [] as UserCardImageRow[], error: null }),
    admin
  ...
  const imageRows = ((imageResult.data ?? []) as UserCardImageRow[]).filter((row) => row.side === "front" || row.side === "back");
  ...
  return {
    ...
    legacyVaultItemId,
    hasFrontPhoto: imageRows.some((row) => row.side === "front"),
    hasBackPhoto: imageRows.some((row) => row.side === "back"),
    photoUrl: getBestPublicCardImageUrl(instance.photo_url, instance.image_url) ?? null,

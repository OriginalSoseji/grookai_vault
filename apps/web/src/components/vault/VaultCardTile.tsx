...
export function VaultCardTile({ item, isPending, error, onQuantityChange }: VaultCardTileProps) {
  return (
    <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm transition duration-200 hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-[0_12px_24px_rgba(15,23,42,0.055)]">
      ...
      <div className="space-y-4 border-t border-slate-200 p-5">
        ...
        <div className="rounded-[1.5rem] border border-slate-200 bg-slate-50/70 px-4 py-3">
          <p className="text-[10px] font-medium uppercase tracking-[0.2em] text-slate-400">Condition</p>
          <p className="mt-2 text-sm font-medium text-slate-900">{item.condition_label}</p>
        </div>

        <div className="space-y-2.5">
          <p className="text-[10px] font-medium uppercase tracking-[0.2em] text-slate-400">Quantity</p>

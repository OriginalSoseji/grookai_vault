...
| `IMAGE_CONFIDENCE_CONTRACT_V1` | Active | Enrichment | Image | Yes | worker: source image enrichment; audit: image truth confidence reports | Partially enforced | Full confidence vocabulary is not yet DB/UI encoded | Use audit/report labels now; add schema/UI support only through a dry-run-proven image package |

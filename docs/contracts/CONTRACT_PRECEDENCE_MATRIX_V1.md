...
| `INGESTION_PIPELINE_CONTRACT_V1` | Active | Ingestion Contracts | Governed exact printing truth and per-set invariants beat provider transport fields, pricing taxonomy, and direct canon mutation | Raw-first staging reaches canon only after exact plan/pre-commit/readback gates | A price key, unsuffixed default, or count-drifted plan cannot create a child printing |
...
| `TCGDEX_SOURCE_CONTRACT_V1` | Active | Ingestion Contracts | Deterministic TCGdex normalization may classify evidence but cannot override governed printing truth | Explicit source semantics select an existing mapped child for review-only evidence | An unsuffixed Cardmarket metric cannot manufacture a Normal child printing |
...
| `IMAGE_CONFIDENCE_CONTRACT_V1` | Active | Enrichment Contracts | Exact image confidence beats representative display coverage; representative rows cannot self-upgrade | Correct card can display as `representative` or `missing_variant_visual` | Base image cannot claim Master Ball, stamp, reverse, or other exact child-variant truth |

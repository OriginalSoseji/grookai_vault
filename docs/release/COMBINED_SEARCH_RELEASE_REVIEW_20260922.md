...
| Physical iPhone preparation | Isolated app/helper now INSTALLED; existing build 324 and other Grookai app metadata unchanged. Apple blocks launch until the user trusts the development profile. Test/relay stopped, isolated packages retained for trust; 225 native hashes match. Physical acceptance OPEN. See combined_search_iphone_trust_20260922.md. |
...
| Physical Android / iPhone | Samsung PASS: isolated full shell and local cold/warm search intents, including sign-out cleanup. iPhone OPEN: isolated packages installed, but the 2026-09-23 01:59 UTC retry again failed Apple's developer-certificate trust check before the journey. Existing apps preserved; test processes and relay stopped. |

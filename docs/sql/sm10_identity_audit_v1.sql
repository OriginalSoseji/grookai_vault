...
-- Live proof for adaptive identity resolution on sm10.
-- Expected live counts on 2026-04-07:
--   unresolved_source_count = 56
--   canonical_target_count = 234
--   exact_match_count = 0
--   same_token_different_name_count = 53
--   exact_unmatched_count = 56
--   classification = BASE_VARIANT_COLLAPSE
--   lawful_base_variant_map_count = 56
--   name_normalize_v1_count = 53
--   suffix_variant_count = 3

begin;

drop table if exists tmp_sm10_source_unresolved_v1;
drop table if exists tmp_sm10_canonical_targets_v1;
drop table if exists tmp_sm10_exact_match_audit_v1;
drop table if exists tmp_sm10_base_variant_map_v1;

create temp table tmp_sm10_source_unresolved_v1 on commit drop as
select
  cp.id as old_id,
  cp.name as old_name,
...
where cpi.is_active = true
  ...
  and cpi.set_code_identity = 'sm10'
  and cp.gv_id is null;

create temp table tmp_sm10_canonical_targets_v1 on commit drop as
select
  cp.id as new_id,
  cp.name as new_name,
...
where cp.set_code = 'sm10'
  and cp.gv_id is not null;

create temp table tmp_sm10_exact_match_audit_v1 on commit drop as
with token_candidates as (
  ...
  from tmp_sm10_source_unresolved_v1 s
  left join tmp_sm10_canonical_targets_v1 c
...
create temp table tmp_sm10_base_variant_map_v1 on commit drop as
select
  s.old_id,
  s.old_name,
  s.old_set_code,
...
from tmp_sm10_source_unresolved_v1 s
join tmp_sm10_canonical_targets_v1 c
...
from tmp_sm10_source_unresolved_v1;

-- 2. Canonical target count
select count(*)::int as canonical_target_count
from tmp_sm10_canonical_targets_v1;

-- 3. Exact-token audit used for class detection
select
  (select count(*)::int from tmp_sm10_exact_match_audit_v1 where exact_match_count = 1) as exact_match_count,
  (select count(*)::int from tmp_sm10_exact_match_audit_v1 where exact_match_count > 1) as exact_multiple_match_count,
  (select count(*)::int from tmp_sm10_exact_match_audit_v1 where same_token_different_name_count > 0) as same_token_different_name_count,
  (select count(*)::int from tmp_sm10_exact_match_audit_v1 where exact_match_count = 0) as exact_unmatched_count;

-- 4. Classification decision
with audit as (
  select
    (select count(*)::int from tmp_sm10_source_unresolved_v1) as source_count,
    (select count(*)::int from tmp_sm10_exact_match_audit_v1 where exact_match_count = 1) as exact_matches,
    (select count(*)::int from tmp_sm10_exact_match_audit_v1 where same_token_different_name_count > 0) as conflicts,
    (select count(*)::int from tmp_sm10_exact_match_audit_v1 where exact_match_count = 0) as unmatched
)
select
  source_count,
  exact_matches,
...
-- 5. Scope proof and old-parent null surface
select
  ...
  count(*) filter (where old_set_code is not null and old_set_code <> 'sm10')::int as out_of_scope_old_set_count
from tmp_sm10_source_unresolved_v1;

-- 6. Base-variant map proof
with old_counts as (
  select old_id, count(*)::int as match_count
  from tmp_sm10_base_variant_map_v1
  group by old_id
),
new_counts as (
...
select
  (select count(*)::int from tmp_sm10_base_variant_map_v1) as lawful_base_variant_map_count,
  ...
  (
    ...
    where not exists (
      select 1
      from tmp_sm10_base_variant_map_v1 m
  ...
  (
    select count(*)::int
    from tmp_sm10_base_variant_map_v1
    where new_set_code <> 'sm10'
  ) as cross_set_target_count;

-- 7. Normalization path summary
select
  match_category,
...
-- 8. Invalid base groups after normalization
with grouped as (
  select
...
where card_print_id in (select old_id from tmp_sm10_base_variant_map_v1)
union all
...
where card_id in (select old_id from tmp_sm10_base_variant_map_v1);

-- 10. Sample before/after mappings
select
  old_id,
  old_name,
...
from tmp_sm10_base_variant_map_v1
where old_id in (
  '3477458e-9507-4aa0-9a04-b812229ef9c8',
  'c96276ef-67b2-404d-a542-db003ab63174',
  'b1f9d7e9-854b-4a9e-822e-8896545f6d66'
)

...
-- Live proof for resolving xy2 via Class F normalization with EX punctuation handling
-- and one suffix-route fan-in against an already-active canonical target.
-- Expected live counts on 2026-04-07:
--   unresolved_source_count = 16
--   canonical_target_count = 109
--   exact_match_count = 0
--   same_token_different_name_count = 15
--   exact_unmatched_count = 16
--   classification = BASE_VARIANT_COLLAPSE
--   lawful_base_variant_map_count = 16
...
drop table if exists tmp_xy2_source_unresolved_v1;
drop table if exists tmp_xy2_canonical_targets_v1;
drop table if exists tmp_xy2_exact_match_audit_v1;
drop table if exists tmp_xy2_base_variant_map_v1;
drop table if exists tmp_xy2_active_identity_candidates_v1;
drop table if exists tmp_xy2_fan_in_groups_v1;

create temp table tmp_xy2_source_unresolved_v1 on commit drop as
select
  cp.id as old_id,
  cp.name as old_name,
...
where cpi.is_active = true
  ...
  and cpi.set_code_identity = 'xy2'
  and cp.gv_id is null;

create temp table tmp_xy2_canonical_targets_v1 on commit drop as
select
  cp.id as new_id,
  cp.name as new_name,
...
where cp.set_code = 'xy2'
  and cp.gv_id is not null;

create temp table tmp_xy2_exact_match_audit_v1 on commit drop as
select
  s.old_id,
...
from tmp_xy2_source_unresolved_v1 s
left join tmp_xy2_canonical_targets_v1 c
  on c.new_number = s.source_printed_number
group by s.old_id, s.source_exact_name_key;

create temp table tmp_xy2_base_variant_map_v1 on commit drop as
select
  s.old_id,
  s.old_name,
  s.old_set_code,
...
from tmp_xy2_source_unresolved_v1 s
join tmp_xy2_canonical_targets_v1 c
  on c.new_number_plain = s.source_base_number_plain
 and c.target_name_normalized_v3 = s.source_name_normalized_v3;

create temp table tmp_xy2_active_identity_candidates_v1 on commit drop as
select
...
from tmp_xy2_base_variant_map_v1 m
...
create temp table tmp_xy2_fan_in_groups_v1 on commit drop as
select
...
from tmp_xy2_active_identity_candidates_v1
group by target_card_print_id
having count(*) > 1;

-- 1. unresolved source count
select count(*)::int as unresolved_source_count
from tmp_xy2_source_unresolved_v1;

-- 2. canonical target count
select count(*)::int as canonical_target_count
from tmp_xy2_canonical_targets_v1;

-- 3. class-detection exact-token audit
select
  (select count(*)::int from tmp_xy2_exact_match_audit_v1 where exact_match_count = 1) as exact_match_count,
  (select count(*)::int from tmp_xy2_exact_match_audit_v1 where exact_match_count > 1) as exact_multiple_match_count,
  (select count(*)::int from tmp_xy2_exact_match_audit_v1 where same_token_different_name_count > 0) as same_token_different_name_count,
  (select count(*)::int from tmp_xy2_exact_match_audit_v1 where exact_match_count = 0) as exact_unmatched_count;
...
with old_counts as (
  ...
  from tmp_xy2_base_variant_map_v1
  group by old_id
),
new_counts as (
...
select
  (select count(*)::int from tmp_xy2_base_variant_map_v1) as lawful_base_variant_map_count,
...
where card_print_id in (select old_id from tmp_xy2_base_variant_map_v1)
union all
...
where card_id in (select old_id from tmp_xy2_base_variant_map_v1);

-- 9. sample planned mappings: first, fan-in suffix row, last
select
...
from tmp_xy2_base_variant_map_v1 m
where m.old_id in (
  '66f01d2a-95ec-4cc1-86ce-4cdd763ce49f',
  '22ea58d5-cea2-4061-b31c-4863ed8e4980',
  '24dd70bd-a6d0-44e9-a8d1-f0a164775459'
)
order by

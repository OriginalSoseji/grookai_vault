-- LC_ALIAS_REALIGNMENT_COLLAPSE_TO_BASE6_V1
-- Read-only proof queries for collapsing the lc alias lane into canonical base6.
-- The runner applies normalizeCardNameV1() to the source rows returned below.

-- 1. Unresolved lc surface count
with unresolved as (
...
-- 2. Canonical base6 occupancy and target identity occupancy
with target_ids as (
  select cp.id
...
select
  (select count(*)::int from target_ids) as canonical_base6_count,
...
-- 3A. Runner source rows: unresolved lc parents
select
  cp.id as old_id,
  cp.name as old_name,
...
-- 3B. Runner source rows: canonical base6 parents
select
  cp.id as new_id,
  cp.name as new_name,
...
-- 4. Strict DB-lane overlap summary against canonical base6
with unresolved as (
  select
    cp.id as old_id,
    cp.name as old_name,
...
-- 5. Strict anomaly rows that the runner repairs via normalizeCardNameV1()
with unresolved as (
  select
    cp.id as old_id,
    cp.name as old_name,
...
-- 6. Same-number collapse-map proof after repo normalization approval
-- The lc audit already proved that exact-number zero-match = 0 and multiple-match = 0.
with unresolved as (
  select
    cp.id as old_id,
    cp.name as old_name,
...
select
  ...
  (
    ...
    where set_code = 'base6'
      and gv_id is not null
  ) as canonical_base6_count;

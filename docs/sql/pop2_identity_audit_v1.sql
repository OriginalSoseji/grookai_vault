...
-- Live proof for resolving pop2 via Class F normalization driven by unicode apostrophe repair.
...
drop table if exists tmp_pop2_source_unresolved_v1;
drop table if exists tmp_pop2_canonical_targets_v1;
drop table if exists tmp_pop2_exact_match_audit_v1;
drop table if exists tmp_pop2_base_variant_map_v1;
drop table if exists tmp_pop2_active_identity_candidates_v1;
drop table if exists tmp_pop2_fan_in_groups_v1;

create temp table tmp_pop2_source_unresolved_v1 on commit drop as
select
  cp.id as old_id,
  cp.name as old_name,
...
where cpi.is_active = true
  ...
  and cpi.set_code_identity = 'pop2'
  and cp.gv_id is null;

create temp table tmp_pop2_canonical_targets_v1 on commit drop as
select
  cp.id as new_id,
  cp.name as new_name,
...
where cp.set_code = 'pop2'
  and cp.gv_id is not null;

create temp table tmp_pop2_exact_match_audit_v1 on commit drop as
select
  s.old_id,
...
from tmp_pop2_source_unresolved_v1 s
left join tmp_pop2_canonical_targets_v1 c
  on c.new_number = s.source_printed_number
group by s.old_id, s.source_exact_name_key;

create temp table tmp_pop2_base_variant_map_v1 on commit drop as
select
  s.old_id,
  s.old_name,
  s.old_set_code,
...
from tmp_pop2_source_unresolved_v1 s
join tmp_pop2_canonical_targets_v1 c
  on c.new_number_plain = s.source_base_number_plain
 and c.target_name_normalized_v3 = s.source_name_normalized_v3;

create temp table tmp_pop2_active_identity_candidates_v1 on commit drop as
select
...
from tmp_pop2_base_variant_map_v1 m
...
create temp table tmp_pop2_fan_in_groups_v1 on commit drop as
select
...
from tmp_pop2_active_identity_candidates_v1
group by target_card_print_id
having count(*) > 1;

-- 1. unresolved source count
select count(*)::int as unresolved_source_count
from tmp_pop2_source_unresolved_v1;

-- 2. canonical target count
select count(*)::int as canonical_target_count
from tmp_pop2_canonical_targets_v1;

-- 3. class-detection exact-token audit
select
  (select count(*)::int from tmp_pop2_exact_match_audit_v1 where exact_match_count = 1) as exact_match_count,
  (select count(*)::int from tmp_pop2_exact_match_audit_v1 where exact_match_count > 1) as exact_multiple_match_count,
  (select count(*)::int from tmp_pop2_exact_match_audit_v1 where same_token_different_name_count > 0) as same_token_different_name_count,
  (select count(*)::int from tmp_pop2_exact_match_audit_v1 where exact_match_count = 0) as exact_unmatched_count;
...
with old_counts as (
  ...
  from tmp_pop2_base_variant_map_v1
  group by old_id
),
new_counts as (
...
select
  (select count(*)::int from tmp_pop2_base_variant_map_v1) as lawful_base_variant_map_count,
...
from tmp_pop2_fan_in_groups_v1;
...
where card_print_id in (select old_id from tmp_pop2_base_variant_map_v1)
union all
...
where card_id in (select old_id from tmp_pop2_base_variant_map_v1);
...
from tmp_pop2_base_variant_map_v1 m
where m.old_id = '969520ee-5b28-42da-b320-0769712c5561'
order by

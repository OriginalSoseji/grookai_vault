...
-- Read-only proof queries for collapsing the unresolved 2012bw alias lane into canonical mcd12.

-- 1. Unresolved 2012bw surface counts.
with unresolved as (
...
-- 2. Canonical mcd12 lane summary and target identity occupancy.
with target_ids as (
  select cp.id
...
select
  (select count(*)::int from target_ids) as canonical_mcd12_count,
...
with unresolved as (
  ...
  from public.card_print_identity cpi
  join public.card_prints cp on cp.id = cpi.card_print_id
  where cpi.is_active = true and cpi.identity_domain = 'pokemon_eng_standard' and cpi.set_code_identity = '2012bw' and cp.gv_id is null
),
canonical as (
  select cp.id as new_id, coalesce(nullif(ltrim(cp.number, '0'), ''), '0') as new_number_normalized,
         lower(regexp_replace(btrim(cp.name), '\s+', ' ', 'g')) as normalized_name
  from public.card_prints cp
  where cp.set_code = 'mcd12' and cp.gv_id is not null
),
collapse_map as (
  select u.old_id, c.new_id
  from unresolved u
...
select
  (select count(*)::int from public.card_print_identity cpi join public.card_prints cp on cp.id = cpi.card_print_id where cpi.is_active = true and cpi.identity_domain = 'pokemon_eng_standard' and cpi.set_code_identity = '2012bw' and cp.gv_id is null) as remaining_unresolved_null_gvid_rows_for_2012bw,
  (select count(*)::int from public.card_prints where set_code = 'mcd12' and gv_id is not null) as canonical_mcd12_count;

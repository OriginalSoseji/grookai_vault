...
-- Live proof for adaptive identity resolution on sm4.
...
drop table if exists tmp_sm4_source_unresolved_v1;
drop table if exists tmp_sm4_canonical_targets_v1;
drop table if exists tmp_sm4_exact_match_audit_v1;
drop table if exists tmp_sm4_base_variant_map_v1;

create temp table tmp_sm4_source_unresolved_v1 on commit drop as
select
  cp.id as old_id,
  cp.name as old_name,
...
create temp table tmp_sm4_canonical_targets_v1 on commit drop as
select
  cp.id as new_id,
  cp.name as new_name,
...
create temp table tmp_sm4_exact_match_audit_v1 on commit drop as
with token_candidates as (
  ...
  from tmp_sm4_source_unresolved_v1 s
  left join tmp_sm4_canonical_targets_v1 c
...
create temp table tmp_sm4_base_variant_map_v1 on commit drop as
select
  s.old_id,
  s.old_name,
  s.old_set_code,
...
from tmp_sm4_source_unresolved_v1 s
join tmp_sm4_canonical_targets_v1 c
...
from tmp_sm4_source_unresolved_v1;

-- 2. Canonical target count
select count(*)::int as canonical_target_count
from tmp_sm4_canonical_targets_v1;

-- 3. Exact-token audit used for class detection
select
  (select count(*)::int from tmp_sm4_exact_match_audit_v1 where exact_match_count = 1) as exact_match_count,
  (select count(*)::int from tmp_sm4_exact_match_audit_v1 where exact_match_count > 1) as exact_multiple_match_count,
  (select count(*)::int from tmp_sm4_exact_match_audit_v1 where same_token_different_name_count > 0) as same_token_different_name_count,
  (select count(*)::int from tmp_sm4_exact_match_audit_v1 where exact_match_count = 0) as exact_unmatched_count;
...
with audit as (
  select
    (select count(*)::int from tmp_sm4_source_unresolved_v1) as source_count,
    (select count(*)::int from tmp_sm4_exact_match_audit_v1 where exact_match_count = 1) as exact_matches,
    (select count(*)::int from tmp_sm4_exact_match_audit_v1 where same_token_different_name_count > 0) as conflicts,
    (select count(*)::int from tmp_sm4_exact_match_audit_v1 where exact_match_count = 0) as unmatched
)
select
  source_count,
  exact_matches,
...
select
  ...
  count(*) filter (where old_set_code is not null and old_set_code <> 'sm4')::int as out_of_scope_old_set_count
from tmp_sm4_source_unresolved_v1;

-- 7. Base-variant map proof
with old_counts as (
  select old_id, count(*)::int as match_count
  from tmp_sm4_base_variant_map_v1
  group by old_id
),
new_counts as (
...
select
  (select count(*)::int from tmp_sm4_base_variant_map_v1) as lawful_base_variant_map_count,
  ...
  (
    ...
    where not exists (
      select 1
      from tmp_sm4_base_variant_map_v1 m
...
-- 8. Reused-target blocking proof
select
  new_id,
  new_name,
  new_number,
  new_gv_id,
  count(*)::int as reused_source_count,
  array_agg(old_id order by source_printed_number, old_id) as source_old_ids,
  array_agg(old_name order by source_printed_number, old_id) as source_old_names,
  array_agg(source_printed_number order by source_printed_number, old_id) as source_printed_numbers,
...
-- 10. Invalid base groups after normalization
with grouped as (
  select
...
-- 11. FK readiness snapshot
select 'card_print_identity' as table_name, count(*)::int as row_count
from public.card_print_identity
where card_print_id in (select old_id from tmp_sm4_base_variant_map_v1)
union all
...
where card_id in (select old_id from tmp_sm4_base_variant_map_v1);

-- 12. Sample planned mappings
select
  old_id,
  old_name,
...
where old_id in (
  ...
  '193de958-9670-45be-bbfb-a86e3b86d3f7',

...
-- Read-only proof queries for the remaining null-gv_id swsh10 identity surface.
...
with unresolved as (
  ...
  where cpi.is_active = true
    ...
    and cpi.set_code_identity = 'swsh10'
...
-- 2. Numeric-lane exact overlap against canonical swsh10
with unresolved as (
  select
...
-- 3. Non-numeric / TG-lane exact overlap against canonical swsh10
with unresolved as (
  select
    cp.id as card_print_id,
    cpi.printed_number,
...
-- 4. Canonical swsh10 surface snapshot
select
...
-- 5. First 25 numeric canonical swsh10 rows
select
  cp.id,
  cp.gv_id,
  cp.name,
...
-- 6. First 25 non-numeric canonical swsh10 rows
select
  cp.id,
  cp.gv_id,
  cp.name,
...
-- 7. Strict same-name same-number audit inside canonical swsh10
with unresolved as (
  select
    cp.id as card_print_id,
...
select
  ...
  count(*) filter (where lane = 'non_numeric' and strict_same_name_same_number_count > 0)::int as strict_non_numeric_overlap_count,
...
-- 8. Supporting proof: zero-padded numeric duplicate evidence against canonical swsh10
with unresolved as (
  select
    cp.id as card_print_id,
    cp.name as unresolved_name,
...
-- 9. Supporting proof: TG-family overlap against canonical swsh10tg
...
canonical_tg as (
  select
    ...
    lower(regexp_replace(btrim(cp.name), '\s+', ' ', 'g')) as normalized_name
  from public.card_prints cp
  where cp.set_code = 'swsh10tg'
    and cp.gv_id is not null
)
select
  count(*) filter (where canonical_match_count > 0)::int as tg_with_swsh10tg_canonical_match_count,
  count(*) filter (where strict_same_name_same_number_count > 0)::int as tg_with_swsh10tg_strict_same_name_same_number_overlap_count,

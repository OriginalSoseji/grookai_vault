...
-- Exact read-only proof used by backend/identity/ecard3_exact_token_promotion_apply_v1.mjs.
-- Under ECARD3_IDENTITY_CONTRACT_V1, builder derivation is exactly:
--   GV-PK-SK-<PRINTED_NUMBER>
-- for valid numeric and H# tokens with base variant_key.
...
select
  ...
  count(*) filter (where printed_number ~ '^(?:[0-9]+|H[0-9]+)$')::int as valid_token_count,
  ...
  count(*) filter (where printed_number !~ '^(?:[0-9]+|H[0-9]+)$')::int as invalid_token_count
from candidates;
...
-- Exact-token promotion requires zero canonical rows with the same token.
with candidates as (
  select
    cp.id as card_print_id,
    cp.name,
  ...
  where cpi.is_active = true
    ...
    and cpi.set_code_identity = 'ecard3'
    and cp.gv_id is null
    and cpi.printed_number ~ '^(?:[0-9]+|H[0-9]+)$'
),
canonical as (
  select
...
with candidates as (
  select
    ...
    cp.variant_key,
    cpi.printed_number,
    'GV-PK-SK-' || cpi.printed_number as proposed_gv_id
  from public.card_print_identity cpi
  join public.card_prints cp
    on cp.id = cpi.card_print_id
  where cpi.is_active = true
...
from candidates
order by
  case when printed_number ~ '^H[0-9]+$' then 1 else 0 end,

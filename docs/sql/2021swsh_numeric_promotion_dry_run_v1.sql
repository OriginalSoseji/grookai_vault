...
-- Exact read-only proof queries for the 2021swsh numeric-only promotion surface.
...
with candidate_surface as (
  ...
  where cpi.is_active = true
    ...
    and cpi.set_code_identity = '2021swsh'
...
-- Sample 25 candidate rows with proposed gv_id.
with candidate_surface as (
  select
    cp.id as card_print_id,
    cp.name,

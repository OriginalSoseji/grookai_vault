...
-- Verification status:
--   failed (residual normalization drift remains on canonical rows)
...
drop table if exists tmp_g1_rc_prefix_validation_v1;

create temp table tmp_g1_rc_prefix_validation_v1 on commit drop as
select *
from (
  values
...
from public.card_prints cp
join tmp_g1_rc_prefix_validation_v1 expected

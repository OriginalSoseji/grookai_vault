...
-- Read-only audit for the final blocked xy9 row.

begin;

drop table if exists tmp_xy9_blocked_row_v1;
drop table if exists tmp_xy9_blocked_candidates_v1;
drop table if exists tmp_xy9_blocked_source_evidence_v1;
drop table if exists tmp_xy9_blocked_metrics_v1;
drop table if exists tmp_xy9_blocked_classification_v1;

create temp table tmp_xy9_blocked_row_v1 on commit drop as
select
...
create temp table tmp_xy9_blocked_candidates_v1 on commit drop as
select
  b.old_parent_id,
...
create temp table tmp_xy9_blocked_source_evidence_v1 on commit drop as
select
  b.old_parent_id,
...
create temp table tmp_xy9_blocked_metrics_v1 on commit drop as
select
  b.old_parent_id,
  ...
  bool_or(coalesce(c.candidate_external_id, '') like 'xy9-98a') as has_98a_target,
  bool_or(coalesce(c.candidate_external_id, '') like 'xy9-98b') as has_98b_target,
  min(se.source_external_id) as source_external_id,
  min(se.source_provenance_ref) as source_provenance_ref
from tmp_xy9_blocked_row_v1 b
left join tmp_xy9_blocked_candidates_v1 c
  on c.old_parent_id = b.old_parent_id
left join tmp_xy9_blocked_source_evidence_v1 se
  on se.old_parent_id = b.old_parent_id
group by b.old_parent_id;

create temp table tmp_xy9_blocked_classification_v1 on commit drop as
select
  b.old_parent_id as row_id,
  case
    when m.candidate_count > 1
      and m.exact_candidate_count = 0
      and m.suffix_candidate_count = m.candidate_count
      and b.source_suffix is null
      and m.distinct_number_plain_count = 1
      and m.distinct_candidate_name_count = 1
  ...
  case
    when m.candidate_count > 1
      ...
      and m.distinct_candidate_name_count = 1
      then
        'The blocked row is an unsuffixed base-token surface (`98`) while the only canonical targets are explicit suffix owners (`98a`, `98b`). Both candidates share the same normalized name and base token, so NAME_NORMALIZE_V3 + TOKEN_NORMALIZE_V1 cannot assign suffix ownership deterministically.'
    when m.candidate_count > 1 then
        'Multiple canonical targets remain reachable from the blocked row and current normalization cannot reduce them safely.'
    when m.candidate_count = 0 then
        'No lawful canonical target exists inside xy9.'
    else
        'Residual blocked surface requires a narrower contract audit.'
  end as reasoning,
  case
    when m.candidate_count > 1
      and m.exact_candidate_count = 0
      ...
      and b.source_suffix is null
      then
        'Collapse is blocked because the source evidence stays unsuffixed (`xy9-98`) while the canonical surfaces are explicitly suffixed (`xy9-98a`, `xy9-98b`). Choosing one would arbitrarily destroy suffix ownership.'
    when m.candidate_count > 1 then
...
from tmp_xy9_blocked_row_v1 b
join tmp_xy9_blocked_metrics_v1 m
...
select
  ...
  variant_key,
  match_type
from tmp_xy9_blocked_candidates_v1
...
select
  ...
  coalesce(
    (
      ...
      from tmp_xy9_blocked_candidates_v1 t
      where t.old_parent_id = c.row_id
    ),
    '[]'::json
  ) as candidate_targets,
  c.why_collapse_is_blocked
from tmp_xy9_blocked_classification_v1 c;
...
from tmp_xy9_blocked_classification_v1;

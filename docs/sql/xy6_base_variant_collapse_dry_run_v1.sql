...
-- Read-only dry run for the apply-safe xy6 base-variant surface.
-- Expected live shape:
--   total_unresolved_count = 20
--   apply_scope_count = 19
--   blocked_scope_count = 1
--   normalized_map_count = 19
--   blocked_old_id = dc8c3dce-bede-47d2-ac8a-095bb633a3ba
...
select
  ...
  m.base_match_count,
  case
    when u.old_id = 'dc8c3dce-bede-47d2-ac8a-095bb633a3ba' then 'BLOCKED'
...
select
  ...
  (select count(*)::int from tmp_xy6_base_classification where row_scope = 'APPLY') as apply_scope_count,
  (select count(*)::int from tmp_xy6_base_classification where row_scope = 'BLOCKED') as blocked_scope_count,
  (select count(*)::int from tmp_xy6_base_collapse_map) as normalized_map_count,
  (
    select count(*)::int
    from tmp_xy6_base_metrics m
    join tmp_xy6_base_classification c on c.old_id = m.old_id
    where c.row_scope = 'APPLY'
  ...
  (select count(*)::int from tmp_xy6_base_classification where row_scope = 'UNCLASSIFIED') as invalid_groups_count;
...
select
  ...
  c.row_scope
from tmp_xy6_base_classification c
left join tmp_xy6_base_collapse_map m

...
-- Exact read-only queries for auditing whether unresolved 2011bw rows are
-- duplicate canonical rows that map 1:1 onto canonical mcd11.

-- Phase 1: unresolved 2011bw surface counts.
with unresolved as (
...
-- Phase 2: canonical mcd11 target summary.
select
...
where set_code = 'mcd11';

-- Phase 2: canonical mcd11 sample rows.
select
  cp.id,
  cp.gv_id,
  cp.name,
...
-- Phase 5: FK readiness snapshot for unresolved 2011bw parent ids.

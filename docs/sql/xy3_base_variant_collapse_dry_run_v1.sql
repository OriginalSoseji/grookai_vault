...
-- Read-only dry run for the apply-safe xy3 base-variant surface.
-- Expected live shape:
--   total_unresolved_count = 13
--   apply_scope_count = 12
--   blocked_scope_count = 1
--   normalized_map_count = 12
--   blocked_old_id = 696cf830-c004-4fcf-9284-00e4e39eaf25

drop table if exists tmp_xy3_base_unresolved;
drop table if exists tmp_xy3_base_canonical;
drop table if exists tmp_xy3_base_exact_audit;
drop table if exists tmp_xy3_base_match_rows;
drop table if exists tmp_xy3_base_metrics;
drop table if exists tmp_xy3_base_classification;
drop table if exists tmp_xy3_base_collapse_map;

create temp table tmp_xy3_base_unresolved on commit drop as
select
  cp.id as old_id,
  cp.name as old_name,
...
create temp table tmp_xy3_base_canonical on commit drop as
select
  cp.id as new_id,
  cp.name as new_name,
...
where cp.set_code = 'xy3'
  and cp.gv_id is not null;

create temp table tmp_xy3_base_exact_audit on commit drop as
select
  u.old_id,
...
from tmp_xy3_base_unresolved u
left join tmp_xy3_base_canonical c
  on c.new_number = u.source_printed_number
group by u.old_id, u.source_exact_name_key, u.source_printed_number;

create temp table tmp_xy3_base_match_rows on commit drop as
select
  u.old_id,
  u.old_name,
  u.old_parent_number,
...
from tmp_xy3_base_unresolved u
join tmp_xy3_base_exact_audit ea
  on ea.old_id = u.old_id
left join tmp_xy3_base_match_rows mr
  on mr.old_id = u.old_id
group by u.old_id, ea.exact_match_count, ea.same_token_different_name_count;

create temp table tmp_xy3_base_classification on commit drop as
select
  u.old_id,
  u.old_name,
  u.source_printed_number,
  ...
  m.base_match_count,
  case
    when u.old_id = '696cf830-c004-4fcf-9284-00e4e39eaf25' then 'BLOCKED'
    when m.base_match_count = 1 then 'APPLY'
    else 'UNCLASSIFIED'
  end as row_scope
from tmp_xy3_base_unresolved u
join tmp_xy3_base_metrics m
  on m.old_id = u.old_id;

create temp table tmp_xy3_base_collapse_map on commit drop as
select
  row_number() over (
    order by
  ...
  mr.*
from tmp_xy3_base_match_rows mr
join tmp_xy3_base_classification c
  on c.old_id = mr.old_id
where c.row_scope = 'APPLY';

with reused_targets as (
  select new_id
  from tmp_xy3_base_collapse_map
  group by new_id
  having count(*) > 1
)
select
  (select count(*)::int from tmp_xy3_base_unresolved) as total_unresolved_count,
  (select count(*)::int from tmp_xy3_base_classification where row_scope = 'APPLY') as apply_scope_count,
  (select count(*)::int from tmp_xy3_base_classification where row_scope = 'BLOCKED') as blocked_scope_count,
  (select count(*)::int from tmp_xy3_base_collapse_map) as normalized_map_count,
  (
    select count(*)::int
    from tmp_xy3_base_metrics m
    join tmp_xy3_base_classification c on c.old_id = m.old_id
    where c.row_scope = 'APPLY'
  ...
  (select count(*)::int from tmp_xy3_base_classification where row_scope = 'UNCLASSIFIED') as invalid_groups_count;
...
select
  ...
  c.row_scope
from tmp_xy3_base_classification c
left join tmp_xy3_base_collapse_map m

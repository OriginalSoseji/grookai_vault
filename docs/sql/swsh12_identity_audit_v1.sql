...
-- Read-only proof queries for the remaining null-gv_id swsh12 identity surface.
...
with unresolved as (
  ...
  where cpi.is_active = true
    ...
    and cpi.set_code_identity = 'swsh12'
...
-- 2. Numeric-lane exact overlap against canonical swsh12
with unresolved as (
  select
...
-- 3. Supporting proof: normalized-digit numeric duplicate evidence against canonical swsh12
with unresolved as (
  select
    cp.id as card_print_id,
...
-- 4. Non-numeric / TG-lane exact overlap against canonical swsh12
with unresolved as (
  select
    cp.id as card_print_id,
    cpi.printed_number,
...
-- 5. TG-family overlap against canonical swsh12tg
...
select
  ...
  count(*) filter (where strict_same_name_same_number_count > 0)::int as tg_with_swsh12tg_strict_same_name_same_number_overlap_count,

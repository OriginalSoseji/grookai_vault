...
-- Live proof for adaptive identity resolution on sm2.
-- Expected live counts on 2026-04-07:
--   unresolved_source_count = 43
--   canonical_target_count = 169
--   exact_match_count = 0
--   same_token_different_name_count = 38
--   exact_unmatched_count = 43
--   classification = BASE_VARIANT_COLLAPSE
--   lawful_base_variant_map_count = 43
--   name_normalize_v1_count = 38
--   suffix_variant_count = 5

begin;

drop table if exists tmp_sm2_source_unresolved_v1;
drop table if exists tmp_sm2_canonical_targets_v1;
drop table if exists tmp_sm2_exact_match_audit_v1;
drop table if exists tmp_sm2_base_variant_map_v1;

create temp table tmp_sm2_source_unresolved_v1 on commit drop as
select
  cp.id as old_id,
  cp.name as old_name,
...
where cpi.is_active = true
  ...
  and cpi.set_code_identity = 'sm2'
  and cp.gv_id is null;

create temp table tmp_sm2_canonical_targets_v1 on commit drop as
select
  cp.id as new_id,
  cp.name as new_name,
...
where cp.set_code = 'sm2'
  and cp.gv_id is not null;

create temp table tmp_sm2_exact_match_audit_v1 on commit drop as
with token_candidates as (
  ...
  from tmp_sm2_source_unresolved_v1 s
  left join tmp_sm2_canonical_targets_v1 c
...
create temp table tmp_sm2_base_variant_map_v1 on commit drop as
select
  s.old_id,
  s.old_name,
  s.old_set_code,
...
from tmp_sm2_source_unresolved_v1 s
join tmp_sm2_canonical_targets_v1 c
...
from tmp_sm2_source_unresolved_v1;

-- 2. Canonical target count
select count(*)::int as canonical_target_count
from tmp_sm2_canonical_targets_v1;

-- 3. Exact-token audit used for class detection
select
  (select count(*)::int from tmp_sm2_exact_match_audit_v1 where exact_match_count = 1) as exact_match_count,
  (select count(*)::int from tmp_sm2_exact_match_audit_v1 where exact_match_count > 1) as exact_multiple_match_count,
  (select count(*)::int from tmp_sm2_exact_match_audit_v1 where same_token_different_name_count > 0) as same_token_different_name_count,
  (select count(*)::int from tmp_sm2_exact_match_audit_v1 where exact_match_count = 0) as exact_unmatched_count;

-- 4. Classification decision
with audit as (
  select
    (select count(*)::int from tmp_sm2_source_unresolved_v1) as source_count,
    (select count(*)::int from tmp_sm2_exact_match_audit_v1 where exact_match_count = 1) as exact_matches,
    (select count(*)::int from tmp_sm2_exact_match_audit_v1 where same_token_different_name_count > 0) as conflicts,
    (select count(*)::int from tmp_sm2_exact_match_audit_v1 where exact_match_count = 0) as unmatched
)
select
  source_count,
  exact_matches,
...
select
  ...
  count(*) filter (where old_set_code is not null and old_set_code <> 'sm2')::int as out_of_scope_old_set_count
from tmp_sm2_source_unresolved_v1;

-- 6. Base-variant map proof
with old_counts as (
  select old_id, count(*)::int as match_count
  from tmp_sm2_base_variant_map_v1
  group by old_id
),
new_counts as (
...
select
  (select count(*)::int from tmp_sm2_base_variant_map_v1) as lawful_base_variant_map_count,
  ...
  (
    ...
    where not exists (
      select 1
      from tmp_sm2_base_variant_map_v1 m
...
where card_print_id in (select old_id from tmp_sm2_base_variant_map_v1)
union all
...
where card_id in (select old_id from tmp_sm2_base_variant_map_v1);
...
from tmp_sm2_base_variant_map_v1
where old_id in (
  '3921725d-e3d2-4e00-9212-30ae1e2b3147',
  'e54d06db-566d-403e-b6e1-24cb9d47758b',
  '65e847d0-0830-4b72-9f92-2e1e1065de51'
)

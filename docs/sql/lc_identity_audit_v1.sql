...
-- Read-only proof queries for the remaining null-gv_id lc identity surface.
...
-- 6. Strict overlap audit against canonical base6 using DB-lane normalized names
with unresolved as (
  select
...
select
  ...
  count(*) filter (where same_name_same_number_match_count > 1)::int as multiple_canonical_match_count,
  count(*) filter (
    where same_name_same_number_match_count = 0
      and same_number_different_name_match_count = 0
      and same_name_different_number_match_count = 0
  )::int as zero_canonical_match_count
from overlap_map;
...
where same_name_same_number_match_count = 0
   ...
   or same_name_same_number_match_count > 1
order by
  coalesce(nullif(regexp_replace(printed_number, '[^0-9]', '', 'g'), ''), '0')::int,
  printed_number,
  old_card_print_id;

-- 8. Source rows used by the runner for proposed-gv_id collision audit via buildCardPrintGvIdV1
select
...
-- 9. Live canonical rows used by the runner to resolve proposed-gv_id collisions
select
  cp.id,
  cp.gv_id,
  cp.set_code,
  ...
  cp.name
from public.card_prints cp
where cp.set_code in ('lc', 'base6')
  and cp.gv_id is not null
order by cp.gv_id, cp.id;

-- 10. Printed-set-abbrev validation for lc and any shared canonical sets
select
  s.id,
  s.code,
  s.name,
  ...
  s.source
from public.sets s
where s.code in ('lc', 'base6')
   or s.printed_set_abbrev = 'LC'
order by s.code, s.id;

-- 11. Raw/provenance evidence for unresolved lc rows
with unresolved as (
  select
    cp.id as card_print_id,
    cp.name,

...
-- Read-only proof queries for the swsh10 mixed collapse:
-- - numeric duplicate collapse into canonical swsh10
-- - TG duplicate collapse into canonical swsh10tg
...
select
  ...
  (
    ...
    where set_code = 'swsh10'
      and gv_id is not null
  ) as canonical_swsh10_count,
  (
    select count(*)::int
    from public.card_prints
    where set_code = 'swsh10tg'
      and gv_id is not null
  ) as canonical_swsh10tg_count;

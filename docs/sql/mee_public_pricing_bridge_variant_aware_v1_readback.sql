select jsonb_build_object(
  'readback_scope', 'bounded_function_arceus_charizard_mightyena_and_ascended_pikachu',
  'sample_boundary_rows_checked', (
    ...
    where card.gv_id in ('GV-PK-AR-1', 'GV-PK-HP-101', 'GV-PK-ASC-276')
  ),
  ...
  'sample_review_required_grookai_value_leak_rows', (
    select count(*)
    from public.card_prints card
    cross join lateral public.get_market_evidence_public_pricing_bridge_variant_aware_v1(card.id) sample
    where card.gv_id in ('GV-PK-AR-1', 'GV-PK-HP-101', 'GV-PK-ASC-276')
      and sample.grookai_value_mid is not null
      and sample.reference_review_status is distinct from 'review_ready_multi_source'
  ),
  'ascended_pikachu_regression_rows', (
    select jsonb_agg(
      jsonb_build_object(
        'pricing_scope', sample.pricing_scope,
        'gv_id', sample.gv_id,
        'reference_anchor_mid', sample.reference_anchor_mid,
        'reference_review_status', sample.reference_review_status,
        'reference_review_flags', sample.reference_review_flags,
    ...
    from public.card_prints card
    cross join lateral public.get_market_evidence_public_pricing_bridge_variant_aware_v1(card.id) sample
    where card.gv_id = 'GV-PK-ASC-276'
  ),

...
-- Live proof for resolving dc1 via Class F normalization with EX punctuation handling.
-- Expected live counts on 2026-04-07:
--   unresolved_source_count = 2
--   canonical_target_count = 34
--   exact_match_count = 0
--   same_token_different_name_count = 2
--   exact_unmatched_count = 2
--   classification = BASE_VARIANT_COLLAPSE
--   lawful_base_variant_map_count = 2
...
drop table if exists tmp_dc1_source_unresolved_v1;
drop table if exists tmp_dc1_canonical_targets_v1;
drop table if exists tmp_dc1_exact_match_audit_v1;
drop table if exists tmp_dc1_base_variant_map_v1;
drop table if exists tmp_dc1_active_identity_candidates_v1;
drop table if exists tmp_dc1_fan_in_groups_v1;

create temp table tmp_dc1_source_unresolved_v1 on commit drop as
select
  cp.id as old_id,
  cp.name as old_name,
...
where cpi.is_active = true
  ...
  and cpi.set_code_identity = 'dc1'
  and cp.gv_id is null;

create temp table tmp_dc1_canonical_targets_v1 on commit drop as
select
  cp.id as new_id,
  cp.name as new_name,
...
where cp.set_code = 'dc1'
  and cp.gv_id is not null;

create temp table tmp_dc1_exact_match_audit_v1 on commit drop as
select
  s.old_id,
...
from tmp_dc1_source_unresolved_v1 s
left join tmp_dc1_canonical_targets_v1 c
  on c.new_number = s.source_printed_number
group by s.old_id, s.source_exact_name_key;

create temp table tmp_dc1_base_variant_map_v1 on commit drop as
select
  s.old_id,
  s.old_name,
  s.old_set_code,
...
from tmp_dc1_source_unresolved_v1 s
join tmp_dc1_canonical_targets_v1 c
  on c.new_number_plain = s.source_base_number_plain
 and c.target_name_normalized_v3 = s.source_name_normalized_v3;

create temp table tmp_dc1_active_identity_candidates_v1 on commit drop as
select
...
from tmp_dc1_base_variant_map_v1 m
...
create temp table tmp_dc1_fan_in_groups_v1 on commit drop as
select
...
from tmp_dc1_active_identity_candidates_v1
group by target_card_print_id
having count(*) > 1;

-- 1. unresolved source count
select count(*)::int as unresolved_source_count
from tmp_dc1_source_unresolved_v1;

-- 2. canonical target count
select count(*)::int as canonical_target_count
from tmp_dc1_canonical_targets_v1;

-- 3. class-detection exact-token audit
select
  (select count(*)::int from tmp_dc1_exact_match_audit_v1 where exact_match_count = 1) as exact_match_count,
  (select count(*)::int from tmp_dc1_exact_match_audit_v1 where exact_match_count > 1) as exact_multiple_match_count,
  (select count(*)::int from tmp_dc1_exact_match_audit_v1 where same_token_different_name_count > 0) as same_token_different_name_count,
  (select count(*)::int from tmp_dc1_exact_match_audit_v1 where exact_match_count = 0) as exact_unmatched_count;
...
with old_counts as (
  ...
  from tmp_dc1_base_variant_map_v1
  group by old_id
),
new_counts as (
...
select
  (select count(*)::int from tmp_dc1_base_variant_map_v1) as lawful_base_variant_map_count,
...
where card_print_id in (select old_id from tmp_dc1_base_variant_map_v1)
union all
...
where card_id in (select old_id from tmp_dc1_base_variant_map_v1);

-- 9. sample planned mappings
select
...
from tmp_dc1_base_variant_map_v1 m
where m.old_id in (
  'a7f846a7-2d62-4752-807d-e1b4da3586cc',
  '44734f4a-f97d-4258-a185-8c030d73b98b'
)
order by

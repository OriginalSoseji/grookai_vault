...
-- Read-only prioritization audit for the remaining mixed-execution bucket after xy10 closure.
-- Live schema notes:
...
-- Target set list:
--   xy6, xy9, xy3, xy4, ecard2, g1, pl2, pl4
-- Expected live results on 2026-04-09:
--   selected_next_target = xy9
--   selected_execution_class = BASE_VARIANT_COLLAPSE

begin;

drop table if exists tmp_mixed_next_target_sets_v2;
drop table if exists tmp_mixed_next_target_unresolved_v2;
drop table if exists tmp_mixed_next_target_canonical_in_set_v2;
drop table if exists tmp_mixed_next_target_canonical_all_v2;
drop table if exists tmp_mixed_next_target_exact_match_rows_v2;
drop table if exists tmp_mixed_next_target_same_token_diff_name_rows_v2;
drop table if exists tmp_mixed_next_target_base_match_rows_v2;
drop table if exists tmp_mixed_next_target_same_base_diff_name_rows_v2;
drop table if exists tmp_mixed_next_target_cross_lane_candidate_rows_v2;
drop table if exists tmp_mixed_next_target_row_metrics_v2;
drop table if exists tmp_mixed_next_target_preclassification_v2;
drop table if exists tmp_mixed_next_target_fan_in_groups_v2;
drop table if exists tmp_mixed_next_target_set_metrics_v2;
drop table if exists tmp_mixed_next_target_ranked_v2;

create temp table tmp_mixed_next_target_sets_v2 (
  set_code text primary key
) on commit drop;

insert into tmp_mixed_next_target_sets_v2 (set_code)
values
  ('xy6'),
  ('xy9'),
  ('xy3'),
...
create temp table tmp_mixed_next_target_unresolved_v2 on commit drop as
select
...
join tmp_mixed_next_target_sets_v2 ts
  on ts.set_code = cpi.set_code_identity
where cpi.identity_domain = 'pokemon_eng_standard'
  and cpi.is_active = true
  and cp.gv_id is null;

create temp table tmp_mixed_next_target_canonical_in_set_v2 on commit drop as
select
...
create temp table tmp_mixed_next_target_canonical_all_v2 on commit drop as
select
...
create temp table tmp_mixed_next_target_exact_match_rows_v2 on commit drop as
select
  u.target_set_code,
  u.old_parent_id,
  c.candidate_target_id
from tmp_mixed_next_target_unresolved_v2 u
join tmp_mixed_next_target_canonical_in_set_v2 c
...
create temp table tmp_mixed_next_target_same_token_diff_name_rows_v2 on commit drop as
select
  u.target_set_code,
  u.old_parent_id,
  c.candidate_target_id
...
create temp table tmp_mixed_next_target_base_match_rows_v2 on commit drop as
select
  u.target_set_code,
  u.old_parent_id,
  c.candidate_target_id
...
create temp table tmp_mixed_next_target_same_base_diff_name_rows_v2 on commit drop as
select
  u.target_set_code,
  u.old_parent_id,
  c.candidate_target_id
...
create temp table tmp_mixed_next_target_cross_lane_candidate_rows_v2 on commit drop as
select
  u.target_set_code,
  u.old_parent_id,
  c.candidate_target_id,
...
from tmp_mixed_next_target_unresolved_v2 u
join tmp_mixed_next_target_canonical_all_v2 c
  on c.candidate_target_set_code <> u.target_set_code
 and c.candidate_target_number_plain = u.base_number_plain
 and c.normalized_name_v3 = u.normalized_name_v3;

create temp table tmp_mixed_next_target_row_metrics_v2 on commit drop as
select
  u.target_set_code,
  u.old_parent_id,
...
left join lateral (
  select candidate_target_id
  from tmp_mixed_next_target_base_match_rows_v2 bm
...
left join lateral (
  select candidate_target_id
  from tmp_mixed_next_target_cross_lane_candidate_rows_v2 cl
...
create temp table tmp_mixed_next_target_fan_in_groups_v2 on commit drop as
select
  target_set_code,
  candidate_target_id as target_card_print_id,
  count(*)::int as incoming_sources
from tmp_mixed_next_target_preclassification_v2
...
create temp table tmp_mixed_next_target_set_metrics_v2 on commit drop as
select
  ts.set_code,
  coalesce((
    select count(*)::int
  ...
  coalesce((
    select count(*)::int
    from tmp_mixed_next_target_canonical_in_set_v2 c
    where c.target_set_code = ts.set_code
  ), 0) as canonical_parent_count,
  coalesce((
    select count(*)::int
    from tmp_mixed_next_target_row_metrics_v2 rm
  ...
  coalesce((
    select count(*)::int
    from tmp_mixed_next_target_fan_in_groups_v2 f
    where f.target_set_code = ts.set_code
  ), 0) as fan_in_group_count,
  coalesce((
    select count(*)::int
    from tmp_mixed_next_target_preclassification_v2 p
...
from tmp_mixed_next_target_sets_v2 ts;

create temp table tmp_mixed_next_target_ranked_v2 on commit drop as
select
  m.*,
  case
    when m.unclassified_count > 0 then 'BLOCKED'
    when m.promotion_candidate_count > greatest(m.normalization_count, m.blocked_conflict_count)
      then 'PROMOTION_HEAVY'
    when m.fan_in_group_count > 0 then 'MIXED_EXECUTION'
    when m.blocked_conflict_count > greatest(m.normalization_count, m.promotion_candidate_count)
  ...
  row_number() over (
    order by
      case
        when m.unclassified_count > 0 then 5
        when m.promotion_candidate_count > 0 then 4
        when m.fan_in_group_count > 0 then 3
        when m.blocked_conflict_count > 1 then 2
        else 1
      end,
      m.blocked_conflict_count,
      m.promotion_candidate_count,
      m.fan_in_group_count,
      (m.normalization_count::numeric / nullif(m.unresolved_parent_count, 0)) desc,
      m.unresolved_parent_count,
      m.set_code
  ) as safety_rank
from tmp_mixed_next_target_set_metrics_v2 m;
...
select
  ...
  normalization_ratio,
  safety_rank
from tmp_mixed_next_target_ranked_v2

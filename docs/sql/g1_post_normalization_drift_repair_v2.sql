...
-- Read-only audit for canonical g1 rows that violate full NAME_NORMALIZE_V3 display punctuation rules.
-- This audit is case-preserving for stored names while enforcing:
--   - unicode apostrophes -> ASCII apostrophe
--   - em/en dash separators -> space
  ...
  select
    ...
    lower(
      btrim(
        regexp_replace(
          regexp_replace(
            regexp_replace(s.expected_normalized_name, '-GX$', ' GX', 'i'),
            '-EX$',
            ' EX',
            'i'
          ),
...
drift_rows as (
  ...
  where current_name is distinct from expected_normalized_name
),
post_apply_surface as (
  select
    ...
    variant_key,
    coalesce(
      (select d.expected_normalized_name from drift_rows d where d.id = c.id),
      c.current_name
    ) as final_name
...
select
  (select count(*)::int from drift_rows) as drift_row_count,
  (select count(*)::int from collisions) as collision_count_after_normalization,

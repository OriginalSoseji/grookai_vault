...
-- Read-only prioritization audit for the remaining sets after xy4 closure.
-- Live schema notes:
...
--   selected_next_target = ecard2
--   selected_execution_class = PROMOTION_HEAVY

begin;

drop table if exists tmp_mixed_next_target_sets_v4;
drop table if exists tmp_mixed_next_target_unresolved_v4;
drop table if exists tmp_mixed_next_target_canonical_in_set_v4;
drop table if exists tmp_mixed_next_target_canonical_all_v4;
drop table if exists tmp_mixed_next_target_exact_match_rows_v4;
drop table if exists tmp_mixed_next_target_same_token_diff_name_rows_v4;
drop table if exists tmp_mixed_next_target_base_match_rows_v4;
drop table if exists tmp_mixed_next_target_same_base_diff_name_rows_v4;
drop table if exists tmp_mixed_next_target_cross_lane_candidate_rows_v4;
drop table if exists tmp_mixed_next_target_row_metrics_v4;
drop table if exists tmp_mixed_next_target_preclassification_v4;
drop table if exists tmp_mixed_next_target_fan_in_groups_v4;
drop table if exists tmp_mixed_next_target_set_metrics_v4;
drop table if exists tmp_mixed_next_target_ranked_v4;

create temp table tmp_mixed_next_target_sets_v4 (
  set_code text primary key
) on commit drop;

insert into tmp_mixed_next_target_sets_v4 (set_code)
values
  ('ecard2'),
  ('g1'),
  ('pl2'),
...
create temp table tmp_mixed_next_target_unresolved_v4 on commit drop as
select
...
join tmp_mixed_next_target_sets_v4 ts
  on ts.set_code = cpi.set_code_identity
where cpi.identity_domain = 'pokemon_eng_standard'
  and cpi.is_active = true
  and cp.gv_id is null;

create temp table tmp_mixed_next_target_canonical_in_set_v4 on commit drop as
select
...
create temp table tmp_mixed_next_target_canonical_all_v4 on commit drop as
select
...
create temp table tmp_mixed_next_target_exact_match_rows_v4 on commit drop as
select
  u.target_set_code,
  u.old_parent_id,
  c.candidate_target_id
from tmp_mixed_next_target_unresolved_v4 u
join tmp_mixed_next_target_canonical_in_set_v4 c
...
create temp table tmp_mixed_next_target_same_token_diff_name_rows_v4 on commit drop as
select
  u.target_set_code,
  u.old_parent_id,
  c.candidate_target_id
...
create temp table tmp_mixed_next_target_base_match_rows_v4 on commit drop as
select
  u.target_set_code,
  u.old_parent_id,
  c.candidate_target_id
...
create temp table tmp_mixed_next_target_same_base_diff_name_rows_v4 on commit drop as
select
  u.target_set_code,
  u.old_parent_id,
  c.candidate_target_id
...
create temp table tmp_mixed_next_target_cross_lane_candidate_rows_v4 on commit drop as
select
  u.target_set_code,
  u.old_parent_id,
  c.candidate_target_id,
...
from tmp_mixed_next_target_unresolved_v4 u
join tmp_mixed_next_target_canonical_all_v4 c
  on c.candidate_target_set_code <> u.target_set_code
 and c.candidate_target_number_plain = u.base_number_plain
 and c.normalized_name_v3 = u.normalized_name_v3;

create temp table tmp_mixed_next_target_row_metrics_v4 on commit drop as
select
  u.target_set_code,
  u.old_parent_id,
...
left join lateral (
  select candidate_target_id
  from tmp_mixed_next_target_base_match_rows_v4 bm
...
left join lateral (
  select candidate_target_id
  from tmp_mixed_next_target_cross_lane_candidate_rows_v4 cl
...
create temp table tmp_mixed_next_target_fan_in_groups_v4 on commit drop as
select
  target_set_code,
  candidate_target_id as target_card_print_id,
  count(*)::int as incoming_sources
from tmp_mixed_next_target_preclassification_v4
...
create temp table tmp_mixed_next_target_set_metrics_v4 on commit drop as
select
  ts.set_code,
  coalesce((
    select count(*)::int
...
from tmp_mixed_next_target_sets_v4 ts;

create temp table tmp_mixed_next_target_ranked_v4 on commit drop as
select
  m.*,
  case
    when m.unclassified_count > 0 then 'BLOCKED'
    when m.promotion_candidate_count >= m.blocked_conflict_count then 'PROMOTION_HEAVY'
    else 'BLOCKED_CONFLICT_HEAVY'
  end as execution_class,
  row_number() over (
    order by
      m.blocked_conflict_count,
      m.promotion_candidate_count,
      (m.normalization_count - m.unresolved_parent_count),
      m.unresolved_parent_count,
      m.set_code
  ) as safety_rank
from tmp_mixed_next_target_set_metrics_v4 m;
...
select
  ...
  execution_class,
  safety_rank
from tmp_mixed_next_target_ranked_v4

...
-- Dry-run proof for the single xy3 exact-token precedence collapse.

begin;

drop table if exists tmp_xy3_exact_source_v1;
drop table if exists tmp_xy3_exact_candidates_v1;
drop table if exists tmp_xy3_exact_metrics_v1;
drop table if exists tmp_xy3_exact_collapse_map_v1;

create temp table tmp_xy3_exact_source_v1 on commit drop as
select
  cp.id as old_id,
  cp.name as old_name,
...
create temp table tmp_xy3_exact_candidates_v1 on commit drop as
select
  s.old_id,
  s.old_name,
  s.old_printed_token,
...
from tmp_xy3_exact_source_v1 s
join public.card_prints cp
  on cp.set_code = 'xy3'
...
create temp table tmp_xy3_exact_metrics_v1 on commit drop as
select
  s.old_id,
...
from tmp_xy3_exact_source_v1 s
left join tmp_xy3_exact_candidates_v1 c
  on c.old_id = s.old_id
group by s.old_id;

create temp table tmp_xy3_exact_collapse_map_v1 on commit drop as
select
  s.old_id,
  s.old_name,
  s.old_printed_token,
...
from tmp_xy3_exact_source_v1 s
join tmp_xy3_exact_candidates_v1 c
  on c.old_id = s.old_id
 and c.match_type = 'exact_token';

with reused_targets as (
  select chosen_target_id
  from tmp_xy3_exact_collapse_map_v1
  group by chosen_target_id
  having count(*) > 1
)
select
  (select count(*)::int from tmp_xy3_exact_source_v1) as source_count,
  (select count(*)::int from tmp_xy3_exact_candidates_v1) as candidate_count,
  (select count(*)::int from tmp_xy3_exact_candidates_v1 where match_type = 'exact_token') as exact_token_candidate_count,
  (select count(*)::int from tmp_xy3_exact_candidates_v1 where match_type = 'suffix') as suffix_candidate_count,
  (select count(*)::int from tmp_xy3_exact_metrics_v1 where additional_candidate_count > 0) as additional_candidate_count,
  (select count(*)::int from tmp_xy3_exact_metrics_v1 where candidate_count = 0) as unmatched_count,
  (select count(*)::int from reused_targets) as reused_target_count,
  (select count(*)::int from tmp_xy3_exact_collapse_map_v1) as chosen_target_count;
...
select
  ...
  'exact-token candidate preserves printed token 55 while suffix candidate requires escalation to 55a; exact-token precedence therefore resolves the row to GV-PK-FFI-55' as proof_reason
from tmp_xy3_exact_source_v1 s
left join tmp_xy3_exact_candidates_v1 c
  on c.old_id = s.old_id
left join tmp_xy3_exact_collapse_map_v1 m

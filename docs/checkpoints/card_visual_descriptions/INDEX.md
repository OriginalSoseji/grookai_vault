...
- Next gate: run one final branch-stratified 25-card OpenAI dry run only, with no database writes, no approvals, no embeddings, no semantic search, no app-facing exposure, no downstream integrations, and no mid-run rule patching.

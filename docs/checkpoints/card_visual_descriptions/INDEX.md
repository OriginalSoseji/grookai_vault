...
- Next gate: implement a narrow deterministic repair for the remaining false negatives around personality/species interpretation, trainer emotion language, dramatic inferred action language, object-material wording, and generic `standard card border visible`. Run targeted contract/fixture validation only. Do not run another 25-card OpenAI sample, apply rows, approve rows, generate embeddings, build semantic search, expose app-facing reads, or integrate downstream systems until that repair is accepted.

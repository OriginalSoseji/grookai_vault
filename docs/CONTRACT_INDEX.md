...
| MTG_SEALED_AUTHENTICATED_IMAGE_READ_V1_CANDIDATE | Offline candidate | docs/contracts/MTG_SEALED_AUTHENTICATED_IMAGE_READ_V1_CANDIDATE.md — Defines an unapplied authenticated signing-authorization RPC and undeployed trusted one-object signer for active byte-verified MTG sealed images, without client Storage access or anonymous reads |
| MTG_SEALED_IMAGE_MIGRATION_PACKAGE_V1 | Promoted; unapplied | docs/contracts/MTG_SEALED_IMAGE_MIGRATION_PACKAGE_V1.md — Freezes the atomic image schema and signing-authorization migration, trusted signer hashes, read-only production preflight, and exact no-data-write apply boundary |

...
| POCKET_GV_ID_NAMESPACE_CONTRACT_V1 | Active | docs/contracts/POCKET_GV_ID_NAMESPACE_CONTRACT_V1.md — Deterministic `GV-TCGP-*` namespace for TCG Pocket rows, explicitly separated from English physical `GV-PK-*` IDs and public physical canon |
...
| TK_SM_R_HIDDEN_SET_EXECUTOR_V1 | Dry-run gate | docs/contracts/TK_SM_R_HIDDEN_SET_EXECUTOR_V1.md — Freezes the exact 11-parent `tk-sm-r` delta, requires rollback absence, and keeps apply disconnected pending a separate execution decision |
...
| BASE_SET_PRINT_RUN_LANES_V1 | Active | docs/contracts/BASE_SET_PRINT_RUN_LANES_V1.md — Governs Base Set Unlimited, Shadowless, 1st Edition, and 1999-2000 collector lanes as audit-first lane identities with special Pikachu cheek-color handling and no blind row cloning |
...
| REPOSITORY_RECONCILIATION_SAFETY_CONTRACT_V1 | Active | docs/contracts/REPOSITORY_RECONCILIATION_SAFETY_CONTRACT_V1.md — Preservation-first rules for reconciling branches and worktrees into a fresh-main candidate without deletion, production mutation, or loss of committed or uncommitted history |
| SYSTEM_PARITY_CRAWL_V1 | Active | docs/contracts/SYSTEM_PARITY_CRAWL_V1.md — Immutable current-main repository, database, runtime, and signed-out product baseline plus serial fresh-main no-loss comparison gate |
...
| SCANNER_FULL_DB_IDENTITY_INDEX_CONTRACT_V1 | Active | Scanner & Condition | docs/contracts/SCANNER_FULL_DB_IDENTITY_INDEX_CONTRACT_V1.md | 2026-05-09 | Full-DB scanner identity index contract: read-only `card_prints` source authority, image eligibility, coverage reporting, metadata provenance, and latency gate before production rollout |
| SCANNER_NO_OCR_IDENTITY_AUTHORITY_CONTRACT_V1 | Active | Scanner & Condition | docs/contracts/SCANNER_NO_OCR_IDENTITY_AUTHORITY_CONTRACT_V1.md | 2026-05-13 | Permanent scanner architecture guardrail: OCR is rejected as production identity authority, live hot-path lock signal, or under-2-second strategy; scanner identity must remain visual/canonical-reference driven |
...
| VENDOR_MODE_WORKSPACE_V1 | Active | docs/contracts/VENDOR_MODE_WORKSPACE_V1.md — Owner-only exact-copy inventory workspace for condition, governed exact-market comparison, asking price, Wall publication, sections, and durable GVVI sharing without creating parallel inventory or pricing authority |
...
| MOBILE_WEB_NATIVE_VISUAL_PARITY_CONTRACT_V1 | Active | docs/contracts/MOBILE_WEB_NATIVE_VISUAL_PARITY_CONTRACT_V1.md — Makes the installed Android app the mobile-web visual authority below 900px, amended by the approved `Pulse · Wall · Scan · Vault · Search` dock, with deterministic visual/accessibility gates and no database or production authority |
...
| MTG_SEALED_IMAGE_SCHEMA_CANDIDATE_V1 | Candidate preserved; schema applied | docs/contracts/MTG_SEALED_IMAGE_SCHEMA_CANDIDATE_V1.md — Preserves the reviewed append-only sealed image evidence/object/assertion/release design and deterministic 17-object transient canary source plan; promoted schema is applied but tables remain empty |
| MTG_SEALED_TRANSIENT_STORAGE_CANARY_V1 | Passed | docs/contracts/MTG_SEALED_TRANSIENT_STORAGE_CANARY_V1.md — Exact 17-object source/upload/readback/removal proof passed with mandatory bundled-plus-system CA verification, zero errors, zero reconciliation mismatches, and all transient paths absent |
...
| MTG_SEALED_IMAGE_BACKED_PRICING_RPC_V3_CANDIDATE | Offline candidate | docs/contracts/MTG_SEALED_IMAGE_BACKED_PRICING_RPC_V3_CANDIDATE.md — Defines an unapplied signed-in read model that requires fresh exact pricing and exact self-hosted image evidence bound to the same frozen release authority |
| MTG_SEALED_AUTHENTICATED_IMAGE_READ_V1_CANDIDATE | Predicate applied; signer undeployed | docs/contracts/MTG_SEALED_AUTHENTICATED_IMAGE_READ_V1_CANDIDATE.md — Governs the applied authenticated signing-authorization predicate and still-undeployed trusted one-object signer, without client Storage access or anonymous reads |
| MTG_SEALED_IMAGE_MIGRATION_PACKAGE_V1 | Applied and verified | docs/contracts/MTG_SEALED_IMAGE_MIGRATION_PACKAGE_V1.md — Preserves the applied atomic image schema/signing-authorization migration, trusted signer hashes, exact production readback, empty image state, and undeployed signer boundary |
| MTG_SEALED_DISABLED_CLIENTS_V1 | Prepared; hard-disabled | docs/contracts/MTG_SEALED_DISABLED_CLIENTS_V1.md — Defines typed web and Flutter RPC V3 adapters with private signed images, fail-closed states, literal false source flags, and no product-surface wiring |
| MTG_SEALED_SIGNED_IN_CLIENT_ROLLOUT_V1 | Production web active; mobile pending | docs/contracts/MTG_SEALED_SIGNED_IN_CLIENT_ROLLOUT_V1.md — Governs the bounded, authenticated web and Flutter catalog clients, trusted signed-image boundary, 24-row limit, and independent client rollback flags |
| MTG_SEALED_IMAGE_DIMENSION_CONSTRAINT_REPAIR_V1 | Prepared; unapplied | docs/contracts/MTG_SEALED_IMAGE_DIMENSION_CONSTRAINT_REPAIR_V1.md — Preserves applied migration history and defines a forward-only, schema-only replacement that rejects partially null image dimension tuples |

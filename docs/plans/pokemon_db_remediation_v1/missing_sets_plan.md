...
| Shiny Vault | Collections | 94 | `https://pkmncards.com/collection/shiny-vault/` | Promo/subset/special collection needing policy decision | High-priority collector subset; must decide if it is canonical set, subset route, or child collection under Hidden Fates. |
...
| Rumble (RM) | Platinum | 16 | `https://pkmncards.com/set/rumble/` | Needs further evidence | Audit also has unmatched DB row `ru1` Pokemon Rumble (RU), printed_total 16. Resolve alias before adding. |
...
1. Rumble alias review: compare PkmnCards `Rumble (RM)` with existing unmatched DB row `ru1`.
2. High-demand missing product sets: TCG Classic Venusaur, Charizard, Blastoise, then McDonald's Match Battle 2023.
3. Shiny Vault collection policy: decide whether it is its own canonical set or a collection/subset route.

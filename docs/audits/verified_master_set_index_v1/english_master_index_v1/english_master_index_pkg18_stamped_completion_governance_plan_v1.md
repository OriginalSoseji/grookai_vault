...
| metric | value |
| --- | --- |
| remaining_queue_rows | 579 |
| governance_rules | 13 |
| bulk_lanes | 7 |
| deterministic_no_source_rows | 301 |
| source_required_rows | 275 |
| manual_conflict_rows | 3 |
| fingerprint_sha256 | `5cffb65e20841afa61b0d3108b4530e0bd1ee6f06f8d670ad78df06e77f770ba` |
...
| 4 | lane_d_prize_pack_finish_mapping | 63 | false | PKG-18D Prize Pack finish label mapping adjudication; then bulk readiness if conflicts close. | prize_pack_stamp=63 |
| 5 | lane_e_variant_family_finish_acquisition | 194 | false | PKG-18E variant-family source acquisition in bulk; produce large guarded packages only for exact two-source rows. | league_stamp=71, pikachu_jack_o_lantern_stamp=27, prerelease_stamp=12, professor_program_stamp=12, league_cup_staff_stamp=8, staff_stamp=7, play_pok_mon_thank_you_stamp=5, dragon_vault_stamp=4, player_rewards_crosshatch_stamp=4, regional_championships_stamp=4 |
...
| league_crosshatch_finish_alias_rule | 84 | bounded_alias_rule | false | can_unlock_bulk_readiness_when exact sources exist | For League/Player Rewards contexts, Crosshatch Holo is a governed source label that may map to the active reverse lane only when exact set/card/stamp evidence exists. |
| prize_pack_finish_label_mapping_rule | 63 | needs_governed_mapping | false | blocked_until_standard_set_vs_foil_mapping | Prize Pack source labels must map Standard Set, Standard Set Foil, H, and Reverse Holo to active finishes at card level before promotion. |
...
| halloween_stamp_display_identity_rule | 28 | source_required_rule | false | no_write_without_exact_sources | Keep source-law requirements active; use rule only for classification and package routing. |
| event_staff_stamp_hierarchy_rule | 26 | variant_normalization_rule | false | can_unlock_bulk_identity_after second source | Event and staff stamps normalize into controlled hierarchy: event stamp, staff event stamp, league staff stamp, finalist placement. |

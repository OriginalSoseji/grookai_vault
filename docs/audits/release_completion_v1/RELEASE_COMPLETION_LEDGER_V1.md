...
- The initial build `289` physical preflight was blocked while the connected iPhone still held build `288` and later entered a locked state. That blocker was subsequently resolved. A fresh authenticated Play Console readback on August 10 still resolves to `/console/signup` and presents developer-account creation. No account type was selected and no enrollment data was submitted, so developer enrollment and listing verification remain external prerequisites.

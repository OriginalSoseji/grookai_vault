...
- The initial build `289` physical preflight was blocked while the connected iPhone still held build `288` and later entered a locked state. That blocker was subsequently resolved. The August 10 Play Console account-creation result came from the wrong Chrome profile. An August 11 readback from the intended Grookai profile verifies an active developer account, the Grookai Vault app record, package `com.grookai.vault`, and an open-testing bundle `1.0.0 (3)`. The app remains a draft, Production is inactive, initial setup is `5/11`, and the open-testing release has not been submitted or rolled out.
...
Obtain genuine fresh-user comprehension evidence and complete the Google Play listing, declarations, and open-testing submission. Reconcile both non-soak gates before starting the 72-hour soak.

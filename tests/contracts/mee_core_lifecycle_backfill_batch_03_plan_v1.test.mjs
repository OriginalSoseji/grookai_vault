...
test("MEE_CORE_LIFECYCLE_BACKFILL_BATCH_03_PLAN_V1 prepares a bounded keyset plan only", () => {
  ...
  const manifest = JSON.parse(
    readFileSync(
      new URL(
        "../../docs/audits/market_evidence_engine_v1/MEE_CORE_LIFECYCLE_BACKFILL_BATCH_03_PLAN_V1/manifest.json",
        import.meta.url,
      ),
      "utf8",
    ),
  );
  assert.equal(manifest.package_id, "MEE-CORE-LIFECYCLE-BACKFILL-BATCH-03-V1");
  ...
  assert.match(script, /MEE_LIFECYCLE_LISTING_KEYSET_CREATED_AT/);
  assert.match(script, /MEE_LIFECYCLE_LISTING_KEYSET_AFTER_ID/);

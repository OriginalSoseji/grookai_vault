...
function fixture(){
  const sha='d'.repeat(64),object={id:uuid('object'),game_key:'pokemon',storage_bucket:'user-card-images',
    object_path:`sealed/pokemon/sha256/dd/${sha}.jpg`,content_sha256:sha,storage_readback_sha256:sha,
  ...
  const baseline=Array.from({length:20},(_,index)=>{
    ...
    return{variant_id:variant,source_mapping_id:mapping,source_product_id:product,source_category_id:3,
      source_payload_hash:'a'.repeat(64),game_key:'pokemon',previous_market_price:50,image_object:object,
      image_evidence:{game_key:'pokemon',variant_id:variant,source_mapping_id:mapping,source_provider:'tcgplayer',source_category_id:3,
  ...
  return{baseline,source:baseline.map(r=>({product_id:r.source_product_id,category_id:3,source_active:true,payload_hash:'a'.repeat(64)})),

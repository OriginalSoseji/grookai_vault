...
const packageId = "MEE-CORE-INTERNAL-REVIEW-ACTION-FUNCTION-LOW-SIGNAL-100-BATCH-PLAN-V1";
const rowManifestHash = "bdfb717c8dcc58e400d1eca0fb245384f93ac063fec99b1b33acc1a4c36d302d";
const reportPath = "docs/audits/market_evidence_engine_v1/MEE-CORE-INTERNAL-REVIEW-ACTION-FUNCTION-LOW-SIGNAL-100-BATCH-PLAN-V1/report.json";
const rowManifestPath = "docs/audits/market_evidence_engine_v1/MEE-CORE-INTERNAL-REVIEW-ACTION-FUNCTION-LOW-SIGNAL-100-BATCH-PLAN-V1/row_manifest.jsonl";
const applySqlPath = "docs/sql/mee_core_internal_review_action_function_low_signal_100_batch_v1_apply_candidate.sql";
const rollbackSqlPath = "docs/sql/mee_core_internal_review_action_function_low_signal_100_batch_v1_rollback_candidate.sql";
const readbackSqlPath = "docs/sql/mee_core_internal_review_action_function_low_signal_100_batch_v1_readback.sql";
const preflightSqlPath = "docs/sql/mee_core_internal_review_action_function_low_signal_100_batch_v1_preflight.sql";
...
test("MEE core low-signal 100 batch plan captures exactly one hundred safe targets", () => {
  ...
  assert.equal(report.package_fingerprint_sha256, "fa48e0f26db2d375b7d26cd557ed225fcf1bfc6d6702bed7a34dc4dd1e235b2a");
  assert.equal(report.batch.size, 100);
  assert.equal(report.batch.action_name, "confirm_monitor_only");
  assert.equal(report.batch.review_actor, "system_low_signal_100_batch_plan");
  ...
  for (const [index, row] of rows.entries()) {
    ...
    assert.equal(row.review_actor, "system_low_signal_100_batch_plan");
...
test("MEE core low-signal 100 batch plan hashes match generated artifacts", () => {
  const report = loadReport();
  ...
  assert.equal(report.hashes.apply_sql_sha256, "5b53a42219577bcfc365902a4657aa269e0b999f10494300af8fcdb0a799bd07");
  assert.equal(report.hashes.rollback_sql_sha256, "d224d0455558ff3773c949ac9a4ecc56ff7e660e002474f6c0ef2ea2a71c0c69");
  assert.equal(report.hashes.readback_sql_sha256, "5284c2a67615ab73e64817b260551e498176bad978965a201edbeffa7cc2cbae");
  assert.equal(report.hashes.preflight_sql_sha256, "e49733ce5bb04f6cd07682c416052acd87749b2d54d044239b6357e9f18efaae");
});

test("MEE core low-signal 100 apply candidate invokes exactly one hundred approved function calls", () => {
  ...
  assert.match(sql, /'system_low_signal_100_batch_plan'::text/);
...
test("MEE core low-signal 100 rollback, preflight, and readback are package scoped", () => {
...
test("MEE core low-signal 100 batch generator stays plan-only", () => {
  const script = read("scripts/audits/market_evidence_review_action_function_low_signal_100_batch_plan_v1.mjs");
  assert.match(script, /MEE-CORE-INTERNAL-REVIEW-ACTION-FUNCTION-LOW-SIGNAL-100-BATCH-PLAN-V1/);
  ...
  for (const artifactPath of [
    "docs/audits/market_evidence_engine_v1/MEE-CORE-INTERNAL-REVIEW-ACTION-FUNCTION-LOW-SIGNAL-100-BATCH-PLAN-V1.md",
    reportPath,
    rowManifestPath,
    applySqlPath,
    rollbackSqlPath,
    readbackSqlPath,
    preflightSqlPath,
    "docs/plans/market_evidence_engine_v1/MEE_CORE_INTERNAL_REVIEW_ACTION_FUNCTION_LOW_SIGNAL_100_BATCH_PLAN_V1.md",
  ]) {

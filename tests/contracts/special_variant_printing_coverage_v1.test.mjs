...
test('printing gaps require authoritative finish evidence before becoming candidates', () => {
  assert.match(source, /missing_child_authoritative_finish_evidence_available/);
  assert.match(source, /missing_child_reference_finish_evidence_review_required/);
  assert.match(source, /missing_child_no_source_finish_evidence/);
  assert.match(source, /AUTHORITATIVE_PRINTING_SOURCES = new Set\(\['tcgdex', 'pokemonapi'\]\)/);
  assert.match(source, /JustTCG-only finish labels remain review evidence and never authorize a child write/i);

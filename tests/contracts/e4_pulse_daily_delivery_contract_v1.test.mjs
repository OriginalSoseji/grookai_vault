...
test("E4 Pulse daily migration cuts over legacy want-match digest", () => {
  const sql = readSource(
    "supabase/migrations/20260708150000_product_evolution_e4_pulse_daily_delivery_v1.sql",
  );

  assert.match(
    sql,
    /create table if not exists public\.pulse_daily_job_state/i,
  );
  assert.match(
    sql,
    /create or replace function public\.enqueue_want_match_digest_notifications_v1/i,
  );
  assert.match(sql, /E4 cutover no-op/i);
  assert.match(
    sql,
    /create or replace function public\.skip_undelivered_want_match_digest_for_pulse_daily_v1/i,
  );
  assert.match(sql, /superseded_by_pulse_daily/i);
  assert.match(sql, /notification_dispatcher_mark_skipped_v1/i);
  assert.match(
    sql,
    /create or replace function public\.enqueue_pulse_daily_notifications_v1/i,
  );
  assert.match(sql, /'pulse_daily'/i);
  assert.match(sql, /'daily_pulse'/i);
  assert.match(
    sql,
    /'pulse_daily:' \|\| v_user\.recipient_user_id::text \|\| ':' \|\| v_window_key/i,
  );
  ...
  assert.match(
    sql,
    /create or replace function public\.run_pulse_daily_aggregation_v1/i,
  );
  assert.match(
    sql,
    /create or replace function public\.notification_dispatcher_reschedule_digest_fold_v1/i,
  );
...
test("E4 dispatcher formats and reschedules pulse_daily", () => {
  ...
  assert.match(
    dispatcher,
    /row\.event_type === "want_match_digest"\s*\|\|\s*row\.event_type === "pulse_daily"/,
  );
...
test("E4 Pulse daily local smoke covers PR2 gate", () => {
  const script = readSource(
    "scripts/audits/e4_pulse_daily_local_fixture_smoke_v1.mjs",
  );

...
test('V5 acquisition waves assign every affected release exactly once', () => {
  ...
  assert.equal(rows.length, 1292);
  assert.equal(new Set(rows.map((row) => row.release_key)).size, rows.length);
  assert.equal(summary.baseline.original_issue_rows, 2705);
  assert.equal(summary.baseline.reconciled_issue_rows, 2497);
  assert.equal(summary.baseline.reconciled_container_dispositions, 72);
  assert.equal(summary.baseline.completed_official_product_scopes, 22);
  assert.equal(summary.baseline.completed_official_release_scopes, 10);
  assert.equal(summary.baseline.newly_resolved_official_identities, 419);
  assert.equal(summary.baseline.projected_v5_working_identity_count, 71910);
  assert.deepEqual(summary.baseline.base_identity_coverage, {
    covered_slots: 8274,
    expected_slots: 22085,
    percent: 37.46,
  });
  assert.equal(summary.baseline.reconciled_active_container_count, 1359);
...
test('V5 acquisition waves preserve the expected release ordering', () => {
  ...
  assert.deepEqual(
    ...
    {
      zero_inventory_acquisition: 572,
